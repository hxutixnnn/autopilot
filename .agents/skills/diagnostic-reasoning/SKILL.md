...
This skill is the single owner of Autopilot's bug-diagnosis reasoning procedure.
Autopilot applies it when briefing delegated investigation and evaluating the resulting evidence, without taking over project-specific investigation itself.
...
Implementation still requires the pilot's request or another existing lifecycle authority, and the reproduction should become the regression test when a fix is authorized.
