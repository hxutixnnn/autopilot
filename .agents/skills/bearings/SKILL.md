...
name: bearings
description: >-
  Generate a "pick up where I left off" fleet digest from autopilot's live fleet state.
  Use when the pilot invokes /bearings or asks for a bearings report, morning brief, status report, catch-up, "where did I leave off", or "what's in the works".
...
Generate a complete current snapshot from the fleet's current state, so the pilot can resume in one read after a break, a night, or a context reset.
...
- When the pilot asks to include PRs, pass the snapshot command's live-PR opt-in.
...
1. **Gather live fleet state with one deterministic command.**
   Run `bin/ap-bearings-snapshot.sh` at invocation time and read its compact output.
   ...
   Keep the default local-only read unless the pilot asks to include PRs.
   For registered copilots, use the snapshot's structured-home classification and provenance.
   ...
   Structured pilot-held decisions come from `decision-hold-lifecycle` and appear under `decisions_open`.
   ...
   Render it under Charted Next with the related `omitted` disclosure, never invent an Underway row from backlog-only state, and never move it into Pilot's Call.

2. **Compose the four-section chat digest from the fresh snapshot.**
   The gather step is deterministic; your judgment is scoped to ranking the command's facts by what matters right now and writing scannable pilot-facing prose.
...
3. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Title** - `# Bearings - <day> <YYYY-MM-DD>` (use "Morning status" only when the pilot specifically asks for a morning brief), followed by two or three sentences framing where things stand.
   - **Pilot's Call** - every open decision summarized with its options from the structured decision record, plus each PR ready to merge and each needed credential or login, every PR with the full `https://...` URL, never a bare `#number`.
   - **Recently Landed** - the bounded current recent-completions baseline from structured state across the main fleet and every registered copilot home, rendered in full on every run.
...
Every `/bearings` chat response renders EXACTLY these four sections, in THIS order, with no fifth section or other structural content:

1. **Pilot's Call** - ONLY items that need the pilot's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the pilot can clear.
   Empty-state: "Nothing needs your action right now."
2. **Recently Landed** - the bounded current recent-completions baseline: merged PRs, completed recon tasks, and finished local-only merges across the main fleet and every registered copilot home.
   Empty-state: "No recent completions are in the current baseline."
3. **Underway** - live work progressing on its own, one line of current state per direct report.
   Empty-state: "Nothing is underway."
4. **Charted Next** - queued or gated work waiting on the fleet or a date, plus action-free fleet-integrity warnings, never on the pilot.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Pilot's Call, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free fleet-integrity warning is Charted Next.
...
- The optional file-mode report is a private, pilot-facing internal artifact that lives in gitignored `data/`, so unlike normal pilot chat it MAY reference task ids, PR URLs, and repo names.
- The pilot works with those directly and needs them to resume; keep the report organized and scannable, not a raw dump.
