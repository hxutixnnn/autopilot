...
name: afk
description: >-
  Enter away-mode supervision when the pilot invokes /afk, says they are going afk, `state/.afk` exists, an incoming message starts with `AP_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
  It sets a durable away-mode flag so the sub-supervisor daemon can self-handle routine wakes and escalate pilot-relevant events plus bounded declared-external-wait rechecks as batched digests during walk-away stretches, then exits automatically when any real unmarked message returns autopilot to full per-wake responsiveness.
user-invocable: true
metadata:
  internal: true
---
...
tradeoff **consented** and **explicit**: the pilot is stepping away, so the
sub-supervisor may triage routine wakes in bash instead of waking autopilot's
LLM for each one. Escalations still reach the pilot, but as one pre-read,
batched digest rather than per-wake injections.

## What it does

1. **Enter the lifecycle through `bin/ap-afk-launch.sh`.**
   This owns the durable state write, session-scoped stale-artifact clearing,
   terminal record, and rollback.
   The flag survives an autopilot restart, so recovery re-enters afk when it is present.
...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     `bin/ap-afk-launch.sh start-native`, then run
     `AP_AFK_STATE_PREPARED=1 bin/ap-afk-start.sh` through that native tool.
     This is a deliberate no-separate-terminal exception because the harness-hosted job creates no terminal or layout mutation, and a shell launcher cannot invoke a harness-native background tool.
     The launcher still owns lifecycle state and records the no-terminal mode, while the daemon inherits and auto-discovers the pilot pane.
     If the native launch fails, run `bin/ap-afk-launch.sh stop` to roll back the prepared lifecycle.
     Do not wrap it in `nohup ... &` (Codex/herdr can reap fire-and-forget shell children after a tool call returns).
   - **Harness WITHOUT one** (e.g. pi): run `bin/ap-afk-launch.sh start`. It is
     ...
     a detached tmux session), records its exact id, and passes the pilot pane
     in as `AP_SUPERVISOR_TARGET` so the daemon injects into the pilot, not its
     own new pane. **Never manufacture a terminal by splitting the pilot's
     active pane** (`herdr pane split`): a split co-tenants the tab and visibly
     shrinks the pilot's pane (docs/herdr-backend.md "Away-mode supervisor
     support").
   Both paths share `bin/ap-afk-start.sh` as the daemon entry.
   ...
   It exits immediately if the identity-backed daemon lock already names a live process, otherwise it execs `bin/ap-supervise-daemon.sh` in the foreground.
   The daemon is **presence-gated**: it injects escalations only while
   `state/.afk` exists, and stays quiet otherwise.

3. **Do not separately arm `ap-watch.sh`.** The daemon manages the watcher as
   its child; the singleton lock no-ops a stray arm harmlessly.

4. **Acknowledge** in `AGENTS.md` section 9 language: "Pilot, away mode is active; I will batch routine updates and surface only decisions, failures, credentials, or review-ready work until you return."

## How to exit afk

No `/back` is needed. The first genuine message is the return signal:

- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the pilot is back.
  Run `bin/ap-afk-return.sh` before acting on the message that brought the pilot back.
  ...
  If it reports an autopilot-actionable `blocked:` event, remediate it immediately through the normal lifecycle, or explicitly reclassify it with a durable reason and close its decision key with `resolved [key=...]`, then run `bin/ap-afk-return.sh check`.
  ...
  Do not answer a Bearings request or perform any other ordinary pilot work until the check exits successfully.
- A message **with** the current operational prefix (`AP_OPERATIONAL_PREFIX`, U+2063 INVISIBLE SEPARATOR followed by `AUTOPILOT_OP: `), or a legacy bare `AP_INJECT_MARK` daemon escalation -> stay afk and process it.
- Re-invoking `/afk` while already away -> stay afk (refresh the flag); this
  does **not** trigger an exit.

Bias ambiguous cases toward exit: a present pilot beats token savings, and
a false exit is self-correcting (the pilot re-runs `/afk`).

## Orthogonal to approval authority

afk changes how aggressively autopilot surfaces things, **not who approves what**.
"Away" never means "approves more" or "approves less."
A PR ready for merge or a needs-decision finding keeps the same configured authority and exceptions from `AGENTS.md` section 7, while anything requiring the pilot still waits for the pilot's explicit word.
The daemon only batches the notification.

## Operational prefix contract

The daemon constructs every current injection as the `away-supervisor` kind owned by `bin/ap-operational-input.sh`, beginning with `AP_OPERATIONAL_PREFIX`: `AP_INJECT_MARK` (U+2063 INVISIBLE SEPARATOR) followed by the stable `AUTOPILOT_OP: ` label.
The bare `AP_INJECT_MARK` form remains accepted for legacy daemon escalations during rollout.
U+2063 has no normal keyboard keystroke and survives terminal transport as UTF-8 text.
This is how autopilot tells a daemon escalation apart from a real message in the same pane.
...
injection, dispatched through `bin/ap-backend.sh` for the supervisor's own
...
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`unknown` verdict from `ap_backend_composer_state` and injects only when it is affirmatively `empty`.
  ...
  The shared `bin/ap-composer-lib.sh` owns the content decision after each backend captures and structurally identifies its own composer row.
...
In afk mode the composer guard is belt-and-suspenders (no human is typing), but it protects against the race window between the pilot returning and their message landing, a dead shell, and the daemon's own previous injection sitting unsent.

**Max-defer escape (the daemon must never silently wedge).**
If anything stays buffered past `AP_MAX_DEFER_SECS` (default 300), the daemon
...
`ap-send.sh` uses the same primitive and exits non-zero
when a steer's Enter is positively swallowed, so autopilot learns an instruction
...
busy opencode pane. The shared `ap_tmux_submit_enter_core` falls back to
`ap_pane_is_busy` once the Enter-retry budget is spent: a busy pane means the
...
and the lenient-`pending`-fails-for-`ap-send` policy still holds for steer
...
The daemon wraps `ap-watch.sh`, runs the watcher as a child, classifies each
wake reason in bash, and self-handles the routine majority without consuming a
autopilot turn.
Pilot-relevant events, plus a bounded recheck of a declared external wait that remains idle, escalate to autopilot's context as one pre-read, single-line, batched digest.
The classification predicates (the pilot-relevant verb set, declared-pause vocabulary, signal/stale tests, and fleet-scan) live in the shared `bin/ap-classify-lib.sh`, the same library the always-on watcher uses for its own triage when afk is off, so the two modes apply one identical policy.
While `state/.afk` exists the daemon owns the watcher, so the watcher reverts to one-shot and lets the daemon do the triage - the two never run their triage at the same time.

Classify each wake this way:

- `signal` with a terminal pilot verb (`done:`, `needs-decision:`, `blocked:`, or `failed:`) -> escalate.
  ...
  Other signals with no pilot-relevant status -> self-handle.
- `signal` or `stale` for a declared `paused:` external wait -> self-handle and track the pause rather than a wedge.
  If it remains declared and idle past `AP_PAUSE_RESURFACE_SECS` (default 3600s), housekeeping sends one awaiting-external recheck and resets the pause window.
- `check` -> always escalate. Check scripts print only when autopilot should wake.
- `stale` with a terminal status or bare legacy pilot-relevant line -> escalate.
  Nonterminal progress remains transient even when its prose contains a legacy free-text token or its seen-status marker already matches, so record a marker and self-handle.
  If the pane is still idle past `AP_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge.
  This bounds wedge-detection latency to the threshold plus a tick: a delay, never a loss.
  Healthy flight crew members are autonomous and do not wait on autopilot mid-task.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash fleet scan
  every `AP_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for a
  pilot-relevant status line the per-wake classifier might miss.
- Unknown reason, or any uncertainty -> escalate fail-safe.

Escalations are buffered up to `AP_ESCALATE_BATCH_SECS` (default 90s; 0 =
...
the operational prefix lets autopilot distinguish it from a real pilot message.
...
- **Single-line digest** - embedded newlines are collapsed to a literal
  ...
  harness.
- **Busy and composer guards on the supervisor pane** - before injecting, the daemon runs the detected-primary-harness rendered busy guard and reads `ap_backend_composer_state` directly.
  ...
  Every other result preserves the buffer for retry, so the daemon never merges its digest into the pilot's half-typed line or types it into a shell.
- The shared composer classifier receives a candidate row only after the active backend performs its own capture and structural row recognition.
  tmux and herdr route their raw styled candidate rows through the shared `ap_composer_strip_ghost` extractor, which removes dim/faint and dark-TRUECOLOR ghost/placeholder text before classification.
  ...
  `AP_COMPOSER_IDLE_RE` still overrides tmux empty-composer matching after shared ghost and border stripping, and `AP_BUSY_REGEX` overrides the rendered delivery guards plus Grok's isolated task-state fallback.
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  buffered past `AP_MAX_DEFER_SECS` (default 300s), the daemon attempts one
...
- **Marker strip** - `strip_injection_marker` removes the current operational
  ...
  text autopilot sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`ap-wake-lib.sh`) instead of `flock`, which is absent on macOS.
- **Dedupe across signal/stale/scan** - `classify_signal` and terminal `classify_stale` paths check the seen-status marker before escalating, so a pilot-relevant status escalated by one path is not re-escalated by another in the same digest.
...
Always enter through `bin/ap-afk-launch.sh`, which clears prior-session artifacts only for a fresh entry and preserves the current session's buffer on refresh.
Always exit through `bin/ap-afk-launch.sh stop`, which keeps `state/.afk` present through the daemon's shutdown flush and clears it last.
...
- Nothing is lost. The durable queue plus `ap-wake-drain.sh` recover any missed
...
`AP_INJECT_SKIP` (default `heartbeat`) force-self-handles matching kinds,
