---
name: autopilot-coding-guidelines
description: >-
  Agent-only reference for changing autopilot's shared, tracked material per AGENTS.md section 1.
  Use before editing any of that material, whether working as autopilot directly or as a flight crew member briefed on an autopilot-repo task.
...
# autopilot-coding-guidelines

Load this before changing autopilot's shared, tracked material, as defined by `AGENTS.md` section 1.
...
1. Does the autopilot AGENT need this on every session or every turn to operate?
...
Briefs for tasks that touch autopilot's own tracked material should tell the flight crew member to load this skill.
`bin/ap-brief.sh`'s `REPO` argument is a caller-supplied string with no reliable signal that it names autopilot's own repo, unlike a project registered in `data/projects.md`, so there is no clean point inside the scaffold to detect this case automatically.
Autopilot adds this skill's load instruction to autopilot-repo briefs by hand instead.
...
Run `bin/ap-doc-audience-check.sh`; it enforces classification, README setup routing, local link targets, and owner pointers without keyword-linting legitimate evidence prose.
...
- Run `bin/ap-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, and pinned shellcheck version) that CI and the no-mistakes pre-push gate both invoke, and it refuses to run under any other shellcheck version.
