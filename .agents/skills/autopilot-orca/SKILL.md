---
name: autopilot-orca
description: Agent-only operator checklist for Autopilot's Orca runtime backend. Use when switching to Orca, spawning or supervising Orca-backed work, smoke-testing Orca backend behavior, debugging Orca task state, or reconciling Orca-backed task metadata.
...
Use this as the operator checklist for Autopilot's experimental Orca runtime backend.
...
Prefer the `bin/ap-*` helpers over raw `orca` commands.
Use raw `orca` only when the helper surface cannot answer the inspection question, and keep the recorded autopilot metadata as the task identity.

## Preflight

Work from the current autopilot home or repo root.
If `AP_HOME` is set, remember that operational state lives under `$AP_HOME` while the helper scripts still run from this repo's `bin/`.

Before switching or spawning against Orca:

- Confirm Orca is intentionally selected through `--backend orca`, `AP_BACKEND=orca`, or local `config/backend`.
...
Use `bin/ap-spawn.sh` so autopilot creates the brief, worktree, terminal, metadata, status file, and watcher surface together.
...
After spawn, check the task with autopilot helpers:

- `bin/ap-peek.sh ap-<id>` for launch failures, trust dialogs, or first output.
- `state/<id>.meta` for `backend=orca`, `terminal=`, `orca_worktree_id=`, and `worktree=`.
- `bin/ap-flight-crew-state.sh <id>` when the current run state matters.
- `bin/ap-watch.sh` whenever there are tasks in flight and this session owns supervision.

Do not manually create the Orca worktree or terminal for a normal autopilot task.
Do not manually patch metadata to make an externally-created Orca terminal look like an autopilot task.

## Supervision

Use `bin/ap-peek.sh`, `bin/ap-send.sh`, `bin/ap-flight-crew-state.sh`, and `bin/ap-teardown.sh` for routine operation.
For steer messages, send short lines through `bin/ap-send.sh <id> '...'`; the stable `ap-<id>` alias also works.
Put long instructions in the task brief or a temporary file and point the flight crew member at that file.

When supervising, treat `state/<id>.meta` as the routing record and Orca's own ids as backend implementation details.
The stable autopilot alias is `ap-<id>`.
The recorded `terminal=` and `orca_worktree_id=` fields are what backend helpers use under the hood.

If `ap-send` fails to submit, do not immediately repeat the same long instruction.
...
4. Prefer autopilot helpers for peek, send, state, and teardown.
...
Teardown remains governed by the normal autopilot landing rules.
Recon work can be torn down after the report exists and the `decision-hold-lifecycle` completion gate passes.
Flight work can be torn down only after the work is landed by its project mode.
...
2. Spawn through `bin/ap-spawn.sh`.
3. Confirm metadata records the Orca backend, terminal, Orca worktree id, and isolated worktree path.
4. Verify `bin/ap-peek.sh`, a short `bin/ap-send.sh` steer, watcher wake behavior, and `bin/ap-flight-crew-state.sh`.
5. Tear down through `bin/ap-teardown.sh` after the task is safely disposable or landed.
