...
# Bearings before Autopilot owned remediation. The shared script now stops,
...
TMP_ROOT=$(ap_test_tmproot ap-afk-return-tests)

install_runner() {  # <case-dir>
  local dir=$1
  mkdir -p "$dir/bin" "$dir/home/state" "$dir/home/data" "$dir/home/config"
  cp "$ROOT/bin/ap-afk-return.sh" "$dir/bin/"
  cp "$ROOT/bin/ap-wake-lib.sh" "$dir/bin/"
  cp "$ROOT/bin/ap-classify-lib.sh" "$dir/bin/"
  cat > "$dir/bin/ap-afk-launch.sh" <<'SH'
#!/usr/bin/env bash
[ "${1:-}" = stop ] || exit 2
printf 'stop\n' >> "$AP_HOME/stop.log"
rm -f "$AP_HOME/state/.afk"
if [ -e "$AP_HOME/state/.fail-terminal-stop-once" ]; then
  rm -f "$AP_HOME/state/.fail-terminal-stop-once"
  exit 1
fi
rm -f "$AP_HOME/state/.afk-daemon-terminal"
SH
  cat > "$dir/bin/ap-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
file="$AP_HOME/state/.fake-drain"
[ -f "$file" ] && cat "$file"
: > "$file"
SH
  chmod +x "$dir/bin/"*.sh
...
run_return() {  # <case-dir> <mode>
  local dir=$1 mode=$2
  AP_HOME="$dir/home" AP_STATE_OVERRIDE="$dir/home/state" "$dir/bin/ap-afk-return.sh" "$mode" 2>&1
}

seed_live_blocker() {  # <case-dir> <backend> <key>
  local dir=$1 backend=$2 key=$3 target
  case "$backend" in
    tmux) target='synthetic:ap-repair-task' ;;
    herdr) target='ap-lab-synthetic:w1:p2' ;;
  esac
...
kind=flight
EOF
  printf 'blocked [key=%s]: autopilot can refresh the synthetic token\n' "$key" > "$dir/home/state/repair-task.status"
}
...
test_return_gate_orders_catchup_before_bearings() {
  ...
  printf 'ap away-mode inject WEDGED: 4555s undelivered\n' > "$dir/home/state/.subsuper-inject-wedged"
  {
  ...
  assert_contains "$out" 'autopilot-actionable blocker: repair-task [key=synthetic-dependency]' "return output did not assign blocker remediation to Autopilot"
  ...
  grep -F $'evidence\twedge\tap away-mode inject WEDGED: 4555s undelivered' "$gate" >/dev/null || fail "wedge evidence was not retained in the durable gate"
  ...
  set +e
  out=$(AP_HOME="$dir/home" AP_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/ap-bearings-snapshot.sh" --json 2>&1)
  rc=$?
  set -e
...
test_pilot_decision_does_not_masquerade_as_autopilot_blocker() {
  local dir out
  dir="$TMP_ROOT/pilot-decision"
  install_runner "$dir"
  cat > "$dir/home/state/decision-task.meta" <<'EOF'
window=synthetic:ap-decision-task
backend=tmux
kind=flight
EOF
  printf 'needs-decision [key=api-shape]: pilot must choose the synthetic API shape\n' > "$dir/home/state/decision-task.status"
  date +%s > "$dir/home/state/.afk"
  printf '1784074271\t1\tsignal\tdecision-task.status\tsignal: synthetic decision\n' > "$dir/home/state/.fake-drain"
  out=$(run_return "$dir" begin) || fail "approval decision should not be treated as an autopilot blocker: $out"
  assert_contains "$out" 'catch-up wake:' "approval decision notification was not surfaced in catch-up"
  [ ! -e "$dir/home/state/.afk-return-catchup" ] || fail "approval decision incorrectly opened an autopilot blocker gate"
  pass "needs-decision remains reportable without masquerading as an autopilot-actionable blocker"
}
...
test_away_reentry_refuses_pending_return_gate() {
  ...
  printf 'schema\tap-afk-return.v1\nphase\tblocked\n' > "$dir/home/state/.afk-return-catchup"
  set +e
  out=$(AP_HOME="$dir/home" AP_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/ap-afk-launch.sh" start-native 2>&1)
  rc=$?
  set -e
