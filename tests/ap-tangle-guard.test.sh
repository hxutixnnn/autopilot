...
#
# Autopilot is a treehouse-pooled git repo of itself: linked worktrees and
# copilot homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (AP_ROOT) is a normal checkout on a real branch. The "tangle"
# is a flight crew member branching/committing in the primary instead of its own worktree,
...
#            ap-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - ap-guard and ap-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the ap-guard banner,
# the ap-bootstrap problem line, the brief assertion ordering, and the ap-spawn
...
. "$ROOT/bin/ap-tangle-lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-tangle-guard)
...
# ap_primary_tangle_branch is the whole scoping decision: a NAMED non-default
...
test_lib_classification() {
  ...
  while IFS='|' read -r label state branch expect; do
    ...
    out=$(ap_primary_tangle_branch "$repo" || true)
    [ "$out" = "$expect" ] || fail "$label: expected tangle='$expect', got '$out'"
  done <<'ROWS'
on the default branch is healthy|default||
on a feature branch is the tangle|feature|ap/readme-restructure-d3|ap/readme-restructure-d3
detached HEAD on default is healthy (worktrees, copilot homes)|detached||
ROWS
  # A non-git directory is not a tangle and must not error.
  out=$(ap_primary_tangle_branch "$TMP_ROOT" || true)
  [ -z "$out" ] || fail "non-git dir wrongly reported a tangle: '$out'"
  pass "ap_primary_tangle_branch: feature branch alarms; default/detached/non-git stay silent"
}

# --- GUARD 2a: ap-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  AP_ROOT_OVERRIDE="$1" AP_HOME="$1" "$ROOT/bin/ap-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "ap-guard: bordered tangle banner fires only for a feature branch and suppresses repair commands in read-only mode"
}

# --- GUARD 2b: ap-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  AP_ROOT_OVERRIDE="$1" AP_HOME="$1" "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  git -C "$repo" checkout -q -B ap/tangle-bb2
  out=$(run_bootstrap "$repo" | grep '^TANGLE:' || true)
  assert_contains "$out" "ap/tangle-bb2" "bootstrap did not report the tangled branch"
  assert_contains "$out" "checkout main" "bootstrap TANGLE line lacked the restore remediation"
  out=$(AP_ROOT_OVERRIDE="$repo" AP_HOME="$repo" AP_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null | grep '^TANGLE:' || true)
  assert_contains "$out" "ap/tangle-bb2" "detect-only bootstrap did not report the tangled branch"
  ...
  pass "ap-bootstrap: TANGLE problem line fires only for a feature branch and suppresses repair commands in detect-only mode"
}

# --- GUARD 1a: brief isolation assertion ------------------------------------

# The generated flight brief must carry the isolation assertion AHEAD of the
# `git checkout -b` step, so the flight crew member verifies its worktree before branching.
test_brief_assertion_precedes_branch() {
  local home brief iso br
  home="$TMP_ROOT/brief-home"
  mkdir -p "$home/data"
  AP_HOME="$home" "$ROOT/bin/ap-brief.sh" tangle-brief-cc3 alpha >/dev/null 2>&1
  ...
  br=$(grep -n 'git checkout -b ap/' "$brief" | head -1 | cut -d: -f1)
  if [ -z "$iso" ] || [ -z "$br" ]; then
  ...
  pass "ap-brief: flight brief asserts worktree isolation before the branch step"
}

# --- GUARD 1b: ap-spawn isolation abort -------------------------------------

# A fake tmux that reports AP_FAKE_PANE_PATH as the post-`treehouse get` pane cwd
...
SH
  ...
  ap_fake_exit0 "$fakebin" treehouse
  printf '%s\n' "$fakebin"
}
...
run_spawn() {
  ...
  AP_ROOT_OVERRIDE='' AP_HOME="$home" \
    ...
    AP_SPAWN_NO_GUARD=1 AP_FAKE_PANE_PATH="$pane" TMUX="fake,1,0" \
    PATH="$fakebin:$PATH" \
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" codex 2>&1
}
...
test_spawn_isolation_abort() {
  ...
  pass "ap-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}

# --- GUARD 1c: ap-spawn tmux window construction ----------------------------

# The prevention guard also depends on ap-spawn building robust tmux commands
...
#     are disabled so the ap-<id> name survives treehouse cd'ing into the worktree;
...
#     display-message fall back to the active client's window and misread autopilot's
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "new-window -dP -F #{window_id} -t autopilot: -n ap-rec-win-gg7" "$rec" \
    "new-window must append at the session (trailing colon) and capture the window id"
  assert_no_grep "new-window -dP -F #{window_id} -t autopilot -n" "$rec" \
  ...
  pass "ap-spawn: appends windows by session-colon, pins the name, and targets the window id"
}
