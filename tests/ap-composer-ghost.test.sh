...
# the one shared ANSI-aware owner (ap_composer_strip_ghost, bin/ap-composer-lib.sh,
# reached here through the ap_tmux_strip_ghost thin adapter):
...
# These tests pin:
#   1. ap_tmux_strip_ghost drops dim/faint AND dark-truecolor runs, keeping
#      normal-intensity, brightly-coloured text.
#   2. ap_pane_input_pending reads a ghost-only composer (either style) as NOT
...
#   4. The human/LLM-facing capture path (ap-peek.sh) stays PLAIN - no escape codes
#      ever reach autopilot's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/ap-tmux-lib.sh"
PEEK="$ROOT/bin/ap-peek.sh"

# shellcheck source=/dev/null
. "$LIB"

TMP_ROOT=$(ap_test_tmproot ap-ghost-tests)
...
# from AP_FAKE_CY. The fake deliberately returns the complete fixture for every
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    f="${AP_FAKE_STYLED:-/dev/null}"
    if [ -n "${AP_FAKE_ROW:-}" ] \
       && [ "$start" = "${AP_FAKE_CY:-0}" ] \
       && [ "$end" = "${AP_FAKE_CY:-0}" ]; then
      f=$AP_FAKE_ROW
    fi
    if [ "$has_e" = 1 ]; then
      cat "$f" 2>/dev/null
...
test_strip_ghost_drops_dim_keeps_normal() {
  ...
  pass "ap_tmux_strip_ghost drops dim/faint runs, keeps normal and bold text"
}
...
test_strip_ghost_handles_combined_and_boundary_codes() {
  ...
  pass "ap_tmux_strip_ghost handles combined SGR, ESC[22m, and reset-then-dim"
}
...
test_strip_ghost_keeps_colored_text_with_2_payloads() {
  ...
  pass "ap_tmux_strip_ghost keeps bright colored text with 2 payloads"
}
...
test_strip_ghost_drops_dark_truecolor_ghost() {
  ...
  pass "ap_tmux_strip_ghost drops a dark/muted truecolor foreground (grok placeholder)"
}

# --- ap_pane_input_pending: dim ghost is not pending ------------------------
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=0 \
     ap_pane_input_pending "fakepane"; then
    fail "dim ghost-only composer falsely read as pending"
  fi
  pass "ap_pane_input_pending: a dim ghost-only composer is NOT pending"
}
...
test_dim_ghost_inside_bordered_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=1 \
     ap_pane_input_pending "fakepane"; then
    fail "dim ghost in a bordered composer falsely read as pending"
  fi
  pass "ap_pane_input_pending: dim ghost inside a bordered composer is NOT pending"
}
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=0 \
    ap_pane_input_pending "fakepane" \
    || fail "real typed text was not detected as pending"
  pass "ap_pane_input_pending: normal-intensity typed text is still pending"
}
...
test_colored_text_with_2_payload_still_pending() {
  ...
  pass "ap_pane_input_pending: bright colored text with 2 payloads is still pending"
}
...
test_dark_truecolor_ghost_only_composer_is_not_pending() {
  ...
  pass "ap_pane_input_pending: a dark truecolor ghost-only composer (grok placeholder) is NOT pending"
}
...
test_dark_truecolor_bare_shell_prompt_is_unknown() {
  ...
  for prompt in '$' 'user@host $'; do
    ...
    out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=0 \
      ap_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "dark truecolor bare shell prompt '$prompt' must read unknown, got '$out'"
  done
  pass "ap_tmux_composer_state: dark truecolor shell prompts read unknown"
}
...
test_real_text_with_trailing_ghost_is_pending() {
  ...
  pass "ap_pane_input_pending: real text plus a trailing ghost run is still pending"
}

# --- ap_tmux_composer_state: structural multi-row box scan ------------------
...
╰────────────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=2 \
    ap_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
    || fail "two-row composer text above the empty cursor row should be pending, got '$out'"
  pass "ap_tmux_composer_state: text above an empty cursor row is pending"
}
...
╰────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=4 \
    ap_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
    || fail "a three-row wrapped composer should be pending, got '$out'"
  pass "ap_tmux_composer_state: a message wrapped across three rows is pending"
}
...
test_bottom_border_cursor_reads_ghost_only_box_as_empty() {
  ...
  pass "ap_tmux_composer_state: Grok's bottom-border cursor quirk reads an empty box structurally"
}
...
test_bordered_busy_signatures_are_pending() {
  ...
  for signature in 'Working...' 'Ctrl+c:cancel'; do
    printf '╭────────────────────╮\n│ %-18s │\n╰────────────────────╯\n' "$signature" > "$capture"
    out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=1 \
      ap_tmux_composer_state "fakepane")
    [ "$out" = pending ] \
      || fail "typed bordered busy signature '$signature' should be pending, got '$out'"
  done
  pass "ap_tmux_composer_state: typed Pi and Grok busy signatures inside a box are pending"
}
...
test_non_bordered_busy_footer_remains_empty() {
  ...
  pass "ap_tmux_composer_state: non-bordered busy footers retain compatibility behavior"
}
...
test_clipped_bordered_box_is_unknown() {
  ...
  pass "ap_tmux_composer_state: an unbounded bordered box fails closed as unknown"
}
...
test_asymmetric_composer_edges_are_unknown() {
  ...
  for row in '│ > text' 'text │' '│ > text ┃' '──────' '+----' 'text +'; do
    printf '%s\n' "$row" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C AP_FAKE_STYLED="$capture" AP_FAKE_CY=0 \
      ap_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "unbounded composer-edge row '$row' should be unknown, got '$out'"
  done
  pass "ap_tmux_composer_state: clipped and asymmetric composer edges fail closed"
}
...
test_mismatched_box_families_are_unknown() {
  ...
  pass "ap_tmux_composer_state: inconsistent box geometry fails closed"
}
...
test_misaligned_box_is_unknown() {
  ...
  pass "ap_tmux_composer_state: misaligned box bounds fail closed"
}
...
test_unproved_empty_geometry_is_unknown() {
  ...
  for fixture in ghost idle malformed-top; do
    case "$fixture" in
      ...
      idle)
        printf '╭────────────╮\n│ idle hint │\n╰────────────╯\n' > "$capture"
        out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=1 \
          AP_COMPOSER_IDLE_RE='^idle hint$' ap_tmux_composer_state "fakepane")
        ;;
      malformed-top)
        printf '╭────x───────╮\n│            │\n╰────────────╯\n' > "$capture"
        out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY=1 \
  ...
  pass "ap_tmux_composer_state: unproved ghost, idle, and border geometry stays unknown"
}
...
test_differing_widths_use_asymmetric_verdicts() {
  ...
  pass "ap_tmux_composer_state: differing widths prefer pending or unknown, never empty"
}
...
test_wide_composer_text_is_pending() {
  ...
  for text in '修复登录问题' 'fix the bug 🔧'; do
    printf '╭────────────────────╮\n│ > %s │\n╰────────────────────╯\n' "$text" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C AP_FAKE_STYLED="$capture" AP_FAKE_CY=1 \
      ap_tmux_composer_state "fakepane")
    [ "$out" = pending-unproven ] \
      || fail "wide composer text '$text' should be pending-unproven, got '$out'"
  done
  pass "ap_tmux_composer_state: emoji and CJK text remain pending under the C locale"
}
...
test_all_tmux_harness_composers_share_classification() {
  ...
  pass "ap_tmux_composer_state: all tmux harnesses share empty and pending classification"
}

test_unrecognized_state_defers_input_guard() {
  (
    # shellcheck disable=SC2329
    ap_tmux_composer_state() { printf 'future-state'; }
    ap_pane_input_pending "fakepane"
  ) || fail "an unrecognized composer state should defer the input guard"
  pass "ap_pane_input_pending: unrecognized states defer by default"
}
...
test_fallback_capture_race_with_edge_is_unknown() {
  ...
  printf '│ > │\n' > "$row_capture"
  out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_ROW="$row_capture" AP_FAKE_CY=0 \
    ap_tmux_composer_state "fakepane")
  [ "$out" = unknown ] \
    || fail "an edge appearing between full-pane and fallback captures should be unknown, got '$out'"
  pass "ap_tmux_composer_state: fallback capture races cannot admit unbounded edges"
}
...
test_legitimate_empty_routes_remain_empty() {
  ...
  for fixture in bordered double-bordered agent-prompt blank; do
    ...
    out=$(PATH="$fb:$PATH" AP_FAKE_STYLED="$capture" AP_FAKE_CY="$cursor" \
      ap_tmux_composer_state "fakepane")
    [ "$out" = empty ] \
      || fail "legitimate empty route '$fixture' should remain empty, got '$out'"
  done
  pass "ap_tmux_composer_state: only proven structural and non-bordered empty routes stay empty"
}
...
test_non_bordered_composer_uses_compatibility_fallback() {
  ...
  pass "ap_tmux_composer_state: panes without bordered structure retain compatibility fallback"
}
...
test_non_bordered_interior_edges_are_pending() {
  ...
  pass "ap_tmux_composer_state: interior edge glyphs retain non-bordered fallback"
}

# --- ap-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  # surface NONE of these escape codes into autopilot's context.
  printf 'normal output line\n\xe2\x9d\xaf \033[2mpredicted next prompt\033[0m\n' > "$capture"
  # Empty AP_HOME so ap-guard.sh finds no in-flight task and stays silent.
  home="$dir/home"; mkdir -p "$home/state"
  # Pass an explicit session:window so resolution needs no metadata.
  out=$(PATH="$fb:$PATH" AP_HOME="$home" AP_FAKE_STYLED="$capture" \
        "$PEEK" "sess:win" 2>/dev/null)
  case "$out" in
    *"$ESC"*) fail "ap-peek surfaced ANSI escape codes into LLM-facing output" ;;
  esac
  # And it should still carry the real content.
  case "$out" in
    *"predicted next prompt"*) : ;;
    *) fail "ap-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "ap-peek output is escape-free (no raw -e bytes reach autopilot context)"
}
