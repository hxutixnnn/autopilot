#!/usr/bin/env bash
# Contract tests for bin/ap-test-run.sh - the single owner of behavior suite
...
# focused scheduler checks, not the complete Autopilot suite.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

RUNNER="$ROOT/bin/ap-test-run.sh"

assert_present "$RUNNER" "bin/ap-test-run.sh is missing"
[ -x "$RUNNER" ] || fail "bin/ap-test-run.sh must be executable"
...
test_family_selection() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/ap-test-run.test.sh' \
    || fail "pure-contract-unit must include ap-test-run.test.sh"
...
test_single_script_selection() {
  local listed
  listed=$("$RUNNER" --list tests/ap-lint.test.sh)
  [ "$listed" = "tests/ap-lint.test.sh" ] \
    || fail "single-script list expected tests/ap-lint.test.sh, got: $listed"
...
test_changed_file_selection_is_conservative() {
  ...
  listed=$("$RUNNER" --list tests/ap-brief.test.sh)
  [ "$listed" = "tests/ap-brief.test.sh" ] \
...
init_changed_fixture_repo() {
  ...
  cp "$RUNNER" "$repo/bin/ap-test-run.sh"
  chmod +x "$repo/bin/ap-test-run.sh"
  for script in \
    ap-brief.test.sh \
    ap-ask-user-authority.test.sh \
    ap-cd-pretool-check.test.sh \
    ap-daemon.test.sh \
    ap-backend-herdr-smoke.test.sh \
    ap-copilot-safety.test.sh \
    ap-session-start.test.sh \
    ap-afk-pi-herdr-return-e2e.test.sh \
    ap-backend.test.sh \
    ap-pr-merge.test.sh \
    ap-pi-watch-extension.test.sh \
    ap-afk-return.test.sh \
    ap-bearings-snapshot.test.sh \
    ap-backend-cmux.test.sh \
    ap-backend-zellij.test.sh \
    ap-backend-orca.test.sh; do
    printf '#!/usr/bin/env bash\n# tests/lib.sh\n' >"$repo/tests/$script"
    chmod +x "$repo/tests/$script"
  done
  : >"$repo/tests/lib.sh"
  : >"$repo/tests/ap-backend-herdr-eventwait.test.py"
  : >"$repo/bin/ap-supervisor-target-lib.sh"
...
test_changed_dependency_selection_and_unmapped_failure() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-changed.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"

  printf '\n' >>"$repo/tests/lib.sh"
  listed=$(cd "$repo" && bin/ap-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/ap-pr-merge.test.sh" "shared helper selects pr-forge dependents"
  assert_contains "$listed" "tests/ap-copilot-safety.test.sh" "shared helper selects copilot dependents"
  assert_contains "$listed" "tests/ap-bearings-snapshot.test.sh" "shared helper selects snapshot dependents"
  git -C "$repo" add tests/lib.sh
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm helper-change

  printf '\n' >>"$repo/tests/ap-backend-herdr-eventwait.test.py"
  listed=$(cd "$repo" && bin/ap-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/ap-backend-herdr-smoke.test.sh" "eventwait test selects Herdr coverage"
  assert_contains "$listed" "tests/ap-backend.test.sh" "eventwait test selects backend coverage"
  git -C "$repo" add tests/ap-backend-herdr-eventwait.test.py
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm eventwait-change

  printf '\n' >>"$repo/bin/ap-supervisor-target-lib.sh"
  listed=$(cd "$repo" && bin/ap-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/ap-daemon.test.sh" "supervisor target selects daemon coverage"
  assert_contains "$listed" "tests/ap-afk-return.test.sh" "supervisor target selects afk coverage"
  git -C "$repo" add bin/ap-supervisor-target-lib.sh
  ...
  printf '\n' >>"$repo/.pi/extensions/ap-primary-turnend-guard.ts"
  listed=$(cd "$repo" && bin/ap-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/ap-ask-user-authority.test.sh" "skill source selects pure contract coverage"
  assert_contains "$listed" "tests/ap-cd-pretool-check.test.sh" "Claude and Pi source selects hook coverage"
  ...
  printf '\n' >>"$repo/src/unmapped.ts"
  set +e
  (cd "$repo" && bin/ap-test-run.sh --list --changed --base HEAD) >"$tmp/out" 2>"$tmp/err"
  rc=$?
  set -e
...
test_empty_selection_emits_summary() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-empty.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  printf 'documentation only\n' >"$repo/README.md"
  out=$(cd "$repo" && bin/ap-test-run.sh --changed --base HEAD --json "$tmp/artifacts/timing.json" 2>"$tmp/err") \
    || fail "empty valid changed selection must pass"
  [ "$out" = "AP_TEST_SUMMARY total=0 failed=0 skipped_gate=0 duration_ms=0" ] \
...
test_timing_markers_and_json() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-timing.XXXXXX")
...
SH
  ...
  begin_n=$(grep -c '^AP_TEST_BEGIN ' "$out" || true)
  end_n=$(grep -c '^AP_TEST_END ' "$out" || true)
  [ "$begin_n" -eq 1 ] || fail "expected one AP_TEST_BEGIN, got $begin_n"
  [ "$end_n" -eq 1 ] || fail "expected one AP_TEST_END, got $end_n"
  grep -Eq '^AP_TEST_BEGIN .+ family=unclassified expected_gate_skip=none$' "$out" \
    || fail "BEGIN line missing family/expected_gate_skip: $(grep '^AP_TEST_BEGIN' "$out")"
  grep -Eq '^AP_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=false$' "$out" \
    || fail "END line missing exit/duration/gate_skip: $(grep '^AP_TEST_END' "$out")"
  summary=$(grep '^AP_TEST_SUMMARY ' "$out" || true)
  ...
  grep -q '^AP_TEST_SLOWEST rank=1 ' "$out" \
    || fail "expected AP_TEST_SLOWEST rank=1"
...
test_aggregate_exit_behavior() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-agg.XXXXXX")
...
SH
  ...
  grep -q 'AP_TEST_SUMMARY total=2 failed=1' "$tmp/out.txt" \
    || fail "summary should report total=2 failed=1: $(grep AP_TEST_SUMMARY "$tmp/out.txt")"
  # All-green stays 0.
  set +e
...
test_gate_skip_accounting() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-skip.XXXXXX")
...
SH
  ...
  grep -Eq '^AP_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$out" \
    || fail "END must mark gate_skip=true: $(grep '^AP_TEST_END' "$out")"
  grep -q 'AP_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$out" \
    || fail "summary must count skipped_gate=1: $(grep AP_TEST_SUMMARY "$out")"
  python3 -c '
import json, sys
doc = json.load(open(sys.argv[1]))
...
test_fail_on_gate_skip_token() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-fail-skip.XXXXXX")
  skip_f="$tmp/skip.test.sh"
  out="$tmp/out.txt"
  cat >"$skip_f" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  grep -q 'AP_TEST_SUMMARY total=1 failed=1' "$out" \
    || fail "summary must report failed=1 under fail-on-gate-skip: $(grep AP_TEST_SUMMARY "$out")"
...
test_exclude_family() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/ap-backend-herdr-smoke.test.sh' \
    && fail "exclude-family real-herdr-gated left a real-herdr script"
  printf '%s\n' "$listed" | grep -Fq 'tests/ap-lint.test.sh' \
...
test_portable_shard_union_and_coverage_guard() {
  ...
  printf '%s\n' "$s1" "$s2" "$serial" | grep -Fq 'tests/ap-backend-herdr-smoke.test.sh' \
    && fail "portable lanes must not include real-herdr-gated smoke"
  printf '%s\n' "$herdr" | grep -Fq 'tests/ap-backend-herdr-smoke.test.sh' \
    || fail "herdr family must include smoke"
  out=$("$RUNNER" --check-coverage)
  assert_contains "$out" "AP_TEST_COVERAGE ok" "coverage guard success marker"
  all_count=$("$RUNNER" --list --all | wc -l | tr -d ' ')
  ...
  first=$(printf '%s\n' "$s1" | head -n 1)
  [ "$first" = "tests/ap-cd-pretool-check.test.sh" ] \
...
test_jobs_requires_proven_isolated() {
  local tmp rc
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-jobs.XXXXXX")
  set +e
  ...
  set +e
  "$RUNNER" --jobs 2 tests/ap-watcher-lock.test.sh >"$tmp/out2" 2>"$tmp/err2"
  rc=$?
  set -e
...
test_jobs_parallel_scheduler_and_failure_propagation() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-jobs-sched.XXXXXX")
  repo="$tmp/repo"
  runner="$repo/bin/ap-test-run.sh"
  evidence="$tmp/evidence"
  fake_bin="$tmp/fake-bin"
  a=tests/ap-brief.test.sh
  b=tests/ap-composer-lib.test.sh
  c=tests/ap-lint.test.sh
  d=tests/ap-supervision-instructions.test.sh
...
SH
  ...
  begin_n=$(grep -c '^AP_TEST_BEGIN ' "$tmp/out" || true)
  end_n=$(grep -c '^AP_TEST_END ' "$tmp/out" || true)
  [ "$begin_n" -eq 3 ] || fail "expected 3 BEGIN markers, got $begin_n"
  [ "$end_n" -eq 3 ] || fail "expected 3 END markers, got $end_n"
  grep -q 'AP_TEST_SUMMARY total=3 failed=0' "$tmp/out" \
    || fail "summary missing for jobs run: $(grep AP_TEST_SUMMARY "$tmp/out")"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
...
SH
  ...
  grep -q 'AP_TEST_SUMMARY total=2 failed=1' "$tmp/out4" \
    || { rm -rf "$tmp"; fail "jobs failure summary wrong: $(grep AP_TEST_SUMMARY "$tmp/out4")"; }
...
SH
  ...
  grep -q 'AP_TEST_SUMMARY total=1 failed=1' "$tmp/out5" \
    || { rm -rf "$tmp"; fail "parallel stderr hard-fail summary wrong: $(grep AP_TEST_SUMMARY "$tmp/out5")"; }

  "$runner" --jobs 2 "$d" >"$tmp/out6" 2>"$tmp/err6" \
    || { rm -rf "$tmp"; fail "ordinary parallel stderr gate skip must remain successful"; }
  grep -Eq '^AP_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr gate skip was not recorded"; }
  grep -q 'AP_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr skip summary wrong: $(grep AP_TEST_SUMMARY "$tmp/out6")"; }
...
test_aggregate_json() {
  local tmp a b
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run-aggjson.XXXXXX")
  cat >"$tmp/a.json" <<'JSON'
{
  "run_id": "a",
...
JSON
  ...
  assert_contains "$out" "AP_TEST_AGGREGATE lanes=2 total=3 failed=1" "aggregate summary line"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
