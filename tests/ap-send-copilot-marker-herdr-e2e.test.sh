#!/usr/bin/env bash
# Real Pi/Herdr regression for exact-id copilot marker delivery.
#
...
# ap-spawn.sh --copilot launch, captures Pi's before_agent_start prompt bytes,
# and proves both sides of the routing boundary:
#   - exact task id through explicit AP_HOME receives exactly one marker;
#   - direct terminal input remains unmarked.
#
# Every Herdr call, including calls made inside the production backend adapter,
# is routed through bin/ap-herdr-lab.sh. The PATH shim strips only the adapter's
...
if [ "${AP_SEND_MARKER_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set AP_SEND_MARKER_HERDR_E2E=1 to run the real Pi/Herdr copilot-marker regression"
  exit 0
fi
...
SESSION=$("$LAB_HELPER" name ap-send-copilot-marker-v7)
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ap-send-marker-herdr-e2e.XXXXXX")
SENDER_HOME="$TMP_ROOT/sender-home"
SECOND_HOME="$TMP_ROOT/copilot-home"
CAPTURE="$TMP_ROOT/pi-before-agent.jsonl"
FAKEBIN="$TMP_ROOT/fakebin"
ORIGINAL_PATH=$PATH
REAL_PI=$(command -v pi)
ID='marker-pi-copilot'
REQUEST='AP_MARKER_HERDR_E2E exact-id request'
DIRECT='AP_MARKER_HERDR_DIRECT pilot input'
...
printf '%s\n' "$ID" > "$SECOND_HOME/.ap-copilot-home"
cat > "$SECOND_HOME/data/charter.md" <<'EOF'
# Isolated marker capture copilot

You are a task-local copilot used only for the marker transport regression.
...
# copilot launch and its own extension arguments unchanged.
CAPTURE_JSON=$(printf '%s' "$CAPTURE" | jq -Rs .)
CAPTURE_EXTENSION="$TMP_ROOT/ap-send-marker-capture.ts"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" AP_GATE_REFUSE_BYPASS=1 AP_HOME="$SENDER_HOME" HERDR_SESSION="$SESSION" \
  "$ROOT/bin/ap-spawn.sh" "$ID" "$SECOND_HOME" --copilot --harness pi --backend herdr >/dev/null

META="$SENDER_HOME/state/$ID.meta"
[ -f "$META" ] || fail "real copilot spawn did not write exact-id metadata"
[ "$(ap_meta_get "$META" kind)" = copilot ] || fail "real copilot metadata did not record kind=copilot"
TARGET=$(ap_backend_target_of_meta "$META")
PANE=${TARGET#*:}
case "$TARGET" in
  "$SESSION":w*:p*) : ;;
  *) fail "real copilot metadata recorded an unexpected Herdr target: $TARGET" ;;
esac
...
wait_for_prompt 'Isolated marker capture copilot' \
...
PATH="$FAKEBIN:$ORIGINAL_PATH" AP_GATE_REFUSE_BYPASS=1 AP_HOME="$SENDER_HOME" \
  "$ROOT/bin/ap-send.sh" "$ID" "$REQUEST" >/dev/null
wait_for_prompt "$REQUEST" || fail "real Pi did not receive the exact-id ap-send request"
GOT=$(jq -r --arg needle "$REQUEST" 'select(.prompt | contains($needle)) | .prompt' "$CAPTURE" | tail -1)
[ "$GOT" = "${AP_FROM_AUTOPILOT_MARK}${REQUEST}" ] \
...
pass "real Pi/Herdr: exact-id AP_HOME send delivers exactly one from-autopilot marker"
wait_for_idle || fail "real Pi did not become idle after the exact-id capture"

# Direct terminal input bypasses ap-send's metadata-routed transformation and
# therefore remains conversational pilot input.
...
[ "$GOT" = "$DIRECT" ] || fail "direct pilot input was changed or marked"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$GOT" | od -An -tx1)"
if ap_message_from_autopilot "$GOT"; then
  fail "direct pilot input was classified as from-autopilot"
fi
printf 'evidence: direct-input received-hex=%s\n' "$(printf '%s' "$GOT" | od -An -tx1 | tr -d ' \n')"
pass "real Pi/Herdr: direct pilot terminal input stays unmarked"
