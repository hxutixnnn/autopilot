...
OWNER="$ROOT/bin/ap-operational-input.sh"
...
test_current_generic_matrix() {
  ...
  prefix_hex=$(printf '%s' "$AP_OPERATIONAL_PREFIX" | od -An -tx1 | tr -d ' \n')
  [ "$prefix_hex" = e281a34155544f50494c4f545f4f503a20 ] \
    || fail "current operational prefix lost the landed U+2063 AUTOPILOT_OP bytes: $prefix_hex"

  for kind in session-start watcher turn-end-guard away-supervisor launch-brief; do
    body="CURRENT_BODY_FOR_${kind}"
    ap_operational_input_encode "$kind" "$body" encoded \
      || fail "could not encode current $kind fixture"
    ap_operational_input_kind "$encoded" parsed \
    ...
    ap_operational_input_body "$encoded" stripped \
...
test_current_from_autopilot_carrier() {
  local encoded parsed separator
  separator=$(printf '\342\201\243')
  ap_message_mark_from_autopilot "corr=0123456789abcdef inspect the report" encoded
  [ "${encoded#"[ap-from-autopilot]$separator"}" != "$encoded" ] \
    || fail "from-autopilot lost its live-charter-compatible leading carrier"
  ap_operational_input_kind "$encoded" parsed \
    || fail "from-autopilot current carrier did not parse"
  [ "$parsed" = from-autopilot ] \
    || fail "from-autopilot current carrier became $parsed"
  [ "$(classify_cli "$encoded")" = from-autopilot ] \
    || fail "cross-language classifier lost from-autopilot"
  pass "operational input: the established from-autopilot carrier remains structurally typed and byte-compatible"
}

test_landed_untyped_prefix_is_explicitly_legacy() {
  local untyped parsed
  untyped="${AP_OPERATIONAL_PREFIX}body whose historical subtype is unknowable"
  ap_legacy_operational_input_kind "$untyped" parsed \
    || fail "landed untyped AUTOPILOT_OP input was not retained"
  [ "$parsed" = legacy-operational ] \
    || fail "landed untyped AUTOPILOT_OP input falsely became $parsed"
  ! ap_operational_input_kind "$untyped" parsed \
    || fail "untyped AUTOPILOT_OP input passed the current typed parser"
  [ "$(classify_cli "$untyped")" = legacy-operational ] \
    || fail "CLI did not expose the untyped prefix as legacy-operational"
  pass "operational input: untyped landed AUTOPILOT_OP transcripts are explicit legacy-operational input"
}

test_isolated_legacy_matrix() {
  local watcher turnend away parsed
  watcher="${AP_LEGACY_WATCHER_PREFIX}signal: legacy${AP_LEGACY_WATCHER_SUFFIX}"
  turnend="${AP_LEGACY_TURNEND_PREFIX}watcher: FAILED - legacy"
  away="${AP_LEGACY_AWAY_PREFIX}1 event(s)): done: legacy"

  for fixture in \
    "session-start|$AP_LEGACY_SESSIONSTART" \
    "watcher|$watcher" \
  ...
  do
    expected=${fixture%%|*}
    message=${fixture#*|}
    ! ap_operational_input_kind "$message" parsed \
      || fail "legacy $expected fixture leaked into the current parser"
    ap_legacy_operational_input_kind "$message" parsed \
...
test_genuine_near_misses_remain_unclassified() {
  ...
  marker=$AP_OPERATIONAL_MARK
  while IFS= read -r fixture || [ -n "$fixture" ]; do
    [ -n "$fixture" ] || continue
    ! ap_operational_input_classify "$fixture" parsed \
...
Pilot quote: ${AP_OPERATIONAL_PREFIX}v1 watcher
AUTOPILOT_OP: v1 watcher
$marker arbitrary pilot text
Pilot quote: $AP_LEGACY_SESSIONSTART
${AP_LEGACY_SESSIONSTART} Please explain this sentence.
AUTOPILOT WATCHER WAKE: can you explain this phrase?
TURN WOULD END BLIND - can you make this warning friendlier?
Supervisor escalate (1 event(s)): is this wording clear?
[ap-from-autopilot] inspect this visible label
EOF
...
test_cross_language_adapter_uses_the_owner() {
  ...
  encoded=$(AP_TEST_ROOT="$ROOT" HELPER="$ROOT/.opencode/plugins/lib/ap-operational-input.js" \
    node --input-type=module <<'JS'
import { pathToFileURL } from "node:url";
const { encodeAutopilotOperationalInput } = await import(pathToFileURL(process.env.HELPER).href);
process.stdout.write(await encodeAutopilotOperationalInput(process.env.AP_TEST_ROOT, "watcher", "CROSS_LANGUAGE_BODY"));
JS
