#!/usr/bin/env bash
# Parity guard for autopilot's shell-lint definition.
#
# bin/ap-lint.sh must be the single owner that BOTH CI
...
# ShellCheck retired in 0.11.0. ap-lint.sh now pins one exact version and both
...
LINT="$ROOT/bin/ap-lint.sh"
INSTALLER="$ROOT/bin/ap-install-shellcheck.sh"
...
test_list_files_reports_the_shell_inventory() {
  ...
  [ "$(printf '%s\n' "$listed" | LC_ALL=C sort)" = "$expected" ] \
    || fail "ap-lint.sh --list-files did not return the complete shell inventory"
  pass "ap-lint.sh --list-files reports the complete shell inventory"
}

test_pins_an_explicit_version() {
  [ -n "$REQUIRED" ] || fail "ap-lint.sh --required-version printed nothing"
  # The pilot-agreed pin: adopt ShellCheck 0.11.0's rule set consistently,
  # which is also what drops the upstream-retired, false-positive-prone SC2015.
  assert_contains "$REQUIRED" "0.11.0" "ap-lint.sh must pin ShellCheck 0.11.0"
  pass "ap-lint.sh pins an explicit ShellCheck version ($REQUIRED)"
}

test_installer_retries_transient_download_failure() {
  local tmp fakebin destination out
  tmp=$(ap_test_tmproot ap-shellcheck-download)
  fakebin=$(ap_fakebin "$tmp")
  destination="$tmp/bin"
...
test_rejects_wrong_shellcheck_version() {
  ...
  tmp=$(ap_test_tmproot ap-lint-ver)
  fakebin=$(ap_fakebin "$tmp")
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "ap-lint.sh accepted a shellcheck version other than the pin"$'\n'"$out"
  assert_contains "$out" "$REQUIRED" "ap-lint.sh did not name the required version on mismatch"
  assert_contains "$out" "0.9.9" "ap-lint.sh did not report the resolved (wrong) version"
  pass "ap-lint.sh refuses to lint under a non-pinned ShellCheck version"
}
...
test_catches_a_real_lint_defect() {
  ...
  tmp=$(ap_test_tmproot ap-lint-bad)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  rc=0
  out=$("$LINT" "$bad" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "ap-lint.sh passed a known-bad fixture"$'\n'"$out"
  assert_contains "$out" "SC1007" "ap-lint.sh did not report the expected ShellCheck finding"
  pass "ap-lint.sh catches a real lint defect the old no-op gate passed"
}
...
test_ignores_ambient_shellcheck_opts() {
  ...
  local tmp bad out rc
  tmp=$(ap_test_tmproot ap-lint-opts)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "ap-lint.sh allowed ambient SHELLCHECK_OPTS to hide a finding"$'\n'"$out"
  assert_contains "$out" "SC1007" "ap-lint.sh did not neutralize ambient SHELLCHECK_OPTS"
  pass "ap-lint.sh ignores ambient ShellCheck options"
}
...
test_clean_fixture_passes() {
  ...
  local tmp good rc
  tmp=$(ap_test_tmproot ap-lint-good)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  cat > "$good" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -eq 0 ] || fail "ap-lint.sh flagged a clean fixture (exit $rc)"
  pass "ap-lint.sh passes a clean fixture"
}
...
test_jobs_are_deterministic_and_complete() {
  ...
  tmp=$(ap_test_tmproot ap-lint-jobs)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  bad_a="$tmp/bad-a.sh"
  bad_b="$tmp/bad-b.sh"
...
SH
  ...
  rc_clean_1=0
  out_clean_1=$(AP_LINT_JOBS=1 "$LINT" "$good" 2>&1) || rc_clean_1=$?
  rc_clean_2=0
  out_clean_2=$(AP_LINT_JOBS=2 "$LINT" "$good" 2>&1) || rc_clean_2=$?
  ...
  rc_fail_1=0
  out_fail_1=$(AP_LINT_JOBS=1 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_1=$?
  rc_fail_2=0
  out_fail_2=$(AP_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2=$?
  rc_fail_2b=0
  out_fail_2b=$(AP_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2b=$?
  ...
  rc_bad_jobs=0
  AP_LINT_JOBS=3 "$LINT" "$good" >/dev/null 2>&1 || rc_bad_jobs=$?
  [ "$rc_bad_jobs" -eq 2 ] || fail "the lint owner must reject unbounded worker counts"

  telemetry_out=$(AP_LINT_JOBS=2 AP_LINT_TELEMETRY="$telemetry" "$LINT" "$good" 2>&1) \
    || fail "telemetry-enabled clean lint failed"
  [ "$telemetry_out" = "$out_clean_2" ] || fail "quiet telemetry changed routine lint output"
  assert_grep $'format\tap-lint-telemetry-v1' "$telemetry" "telemetry format marker is missing"
  ...
  mkdir -p "$cleanup_tmp"
  cleanup_out=$(TMPDIR="$cleanup_tmp" AP_LINT_JOBS=2 "$LINT" "$good" 2>&1) \
    || fail "cleanup fixture lint failed"
  [ "$cleanup_out" = "$out_clean_2" ] || fail "cleanup fixture changed routine diagnostics"
  [ -z "$(find "$cleanup_tmp" -mindepth 1 -maxdepth 1 -name 'ap-lint.*' -print -quit)" ] \
...
test_worker_trees_stop_on_signal() {
  ...
  tmp=$(ap_test_tmproot ap-lint-signal)
  mkdir -p "$tmp"
  fakebin=$(ap_fakebin "$tmp")
  fixture="$tmp/good.sh"
  cat > "$fixture" <<'SH'
...
SH
  ...
  for jobs in 1 2; do
    for telemetry in off on; do
      ...
      PATH="$fakebin:$PATH" TMPDIR="$lint_tmp" AP_LINT_JOBS="$jobs" \
        AP_LINT_TELEMETRY="$telemetry_file" AP_TEST_SHELLCHECK_PID="$pid_file" \
      ...
      [ -z "$(find "$lint_tmp" -mindepth 1 -maxdepth 1 -name 'ap-lint.*' -print -quit)" ] \
...
test_seeded_module_boundary_parity() {
  ...
  tmp=$(mktemp -d "$ROOT/.ap-lint-parity.XXXXXX")
  if [ "${#AP_TEST_CLEANUP_DIRS[@]}" -eq 0 ]; then
    trap ap_test_cleanup EXIT
  fi
  AP_TEST_CLEANUP_DIRS+=("$tmp")
  rel=${tmp#"$ROOT/"}
...
SH
  ...
  rc=0
  out=$(AP_LINT_JOBS=2 "$LINT" "$dispatcher" "$adapter" "$owner" "$test_root" 2>&1) || rc=$?
