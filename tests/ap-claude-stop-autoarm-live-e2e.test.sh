...
# (bin/ap-claude-stop-autoarm.sh + bin/ap-turnend-guard.sh --claude).
...
# session lock can run ap-session-start.sh first; session start reclaims the
...
# The project and AP_HOME are isolated; Claude keeps using its existing managed
...
if [ "${AP_CLAUDE_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set AP_CLAUDE_LIVE_E2E=1 to run the Claude Stop auto-arm regression"
  exit 0
fi
...
printf '%s\n' "$P" | jq -r '.tool_input.command // "unknown"' >> "$AP_HOME/state/tool-calls.log" 2>/dev/null
exit 0
SH
...
# harness owner under ap_harness_pid_alive, matching the reproduced incident.
printf '9999999\n' > "$HOME_DIR/state/.lock"
...
cat > "$PROJECT/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
N=$(cat "$AP_HOME/state/arm-count" 2>/dev/null || echo 0); N=$((N+1)); echo "$N" > "$AP_HOME/state/arm-count"
echo "arm-run=$N pid=$$" >> "$AP_HOME/state/arm-ran"
if [ "$N" -ge 3 ]; then
...
# Stop-owned cycles.
cat > "$PROJECT/bin/ap-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
N=$(cat "$AP_HOME/state/drain-count" 2>/dev/null || echo 0); N=$((N+1)); echo "$N" > "$AP_HOME/state/drain-count"
echo "drain-run=$N" >> "$AP_HOME/state/drain-ran"
if [ "$N" -ge 3 ]; then
  rm -f "$AP_HOME/state/task.meta"
fi
printf 'stale: fixture-rapid drained\n'
SH
chmod +x "$PROJECT/bin/ap-watch-arm.sh" "$PROJECT/bin/ap-wake-drain.sh"

PROMPT='Run exactly `bin/ap-session-start.sh` with Bash as your first tool call. After reading its complete digest, reply with exactly CYCLE0 and stop. Whenever a Stop hook feedback message wakes you, run exactly `bin/ap-wake-drain.sh` once with Bash, then reply with exactly ACK and stop. Never run bin/ap-watch-arm.sh or any other arm command, and never use any other tool.'

(
  cd "$PROJECT" || exit 1
  AP_HOME="$HOME_DIR" CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false \
...
[ "$(sed -n '1p' "$HOME_DIR/state/tool-calls.log" 2>/dev/null)" = 'bin/ap-session-start.sh' ] \
...
if [ -f "$HOME_DIR/state/tool-calls.log" ]; then
  ! grep -q 'ap-watch-arm.sh' "$HOME_DIR/state/tool-calls.log" \
...
printf '%s\n' '{"session_id":"live-owner-control"}' \
  | AP_HOME="$LIVE_OWNER_HOME" AP_ROOT_OVERRIDE="$PROJECT" "$FAKE_CLAUDE" -c '"$AP_ROOT_OVERRIDE/bin/ap-claude-stop-autoarm.sh"' \
