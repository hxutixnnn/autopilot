#!/usr/bin/env bash
# tests/ap-backend-zellij.test.sh - fake-zellij-CLI unit tests for the zellij
# session-provider adapter (bin/backends/zellij.sh), P3 of
# data/ap-backend-design-d7 (report.md "Zellij Backend"). Mirrors
# tests/ap-backend-herdr.test.sh's fakebin/command-log convention: a small,
...
# tests/ap-backend-zellij-smoke.test.sh, gated on the zellij binary actually
# being installed.
set -u
...
TMP_ROOT=$(ap_test_tmproot ap-backend-zellij-tests)

# make_zellij_fakebin: a `zellij` stub that logs every invocation (one line,
# unit-separated args, to $AP_ZELLIJ_LOG) and returns the canned response for
# that call read from $AP_ZELLIJ_RESPONSES/<n>.out, consumed IN ORDER (call 1
# reads 1.out, call 2 reads 2.out, ...), mirroring
# tests/ap-backend-herdr.test.sh's make_herdr_fakebin. A missing response file
...
# specially (not call-counted) since ap_backend_zellij_session_exists calls
...
# AP_ZELLIJ_SESSION_LIST instead.
...
if [ "${1:-}" = --version ]; then
  printf 'zellij %s\n' "${AP_ZELLIJ_FAKE_VERSION:-0.44.0}"
  exit 0
fi
if [ "${1:-}" = list-sessions ]; then
  printf '%s\n' "${AP_ZELLIJ_SESSION_LIST:-}"
  exit 0
fi
if [ "${1:-}" = attach ]; then
  exit "${AP_ZELLIJ_ATTACH_EXIT:-0}"
fi
...
zellij_tab_response() {
  local dir=$1 n=$2 tab=${3:-3} name=${4:-ap-task}
...
# reimplementations of ap_backend_zellij_home_label/scoped_title (mirroring
# tests/ap-backend-cmux.test.sh's identical cmux_expected_* helpers), used to
...
zellij_expected_scoped_title() {  # <ap-task-label> [home] [root]
  local label=$1 home=${2:-$ROOT} root=${3:-$ROOT} rest
  case "$label" in
    ap-*) rest=${label#ap-} ;;
    *) rest=$label ;;
  esac
  printf 'ap-%s-%s' "$(zellij_expected_home_label "$home" "$root")" "$rest"
}
...
test_version_check_accepts_current_version() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_FAKE_VERSION=0.44.0 \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_version_check' "$ROOT"
  status=$?
  expect_code 0 "$status" "version_check should accept 0.44.0 (the verified minimum)"
  pass "ap_backend_zellij_version_check: accepts the verified minimum (0.44.0)"
}
...
test_version_check_accepts_newer_version() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_FAKE_VERSION=0.45.2 \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_version_check' "$ROOT"
  status=$?
  expect_code 0 "$status" "version_check should accept a newer minor (0.45.2)"
  pass "ap_backend_zellij_version_check: accepts a newer version (0.45.2)"
}
...
test_version_check_refuses_old_version() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_FAKE_VERSION=0.38.1 \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_version_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_zellij_version_check: refuses an old version loudly"
}
...
test_version_check_refuses_missing_zellij() {
  ...
  pass "ap_backend_zellij_version_check: refuses loudly when zellij is not installed"
}

# --- session name resolution --------------------------------------------------

test_session_defaults_to_autopilot() {
  local out
  out=$( bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_session' "$ROOT" )
  [ "$out" = autopilot ] || fail "default session should be 'autopilot', got '$out'"
  pass "ap_backend_zellij_session: defaults to 'autopilot' when AP_ZELLIJ_SESSION is unset"
}

test_session_honors_override() {
  local out
  out=$( AP_ZELLIJ_SESSION=ap-test-iso bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_session' "$ROOT" )
  [ "$out" = ap-test-iso ] || fail "AP_ZELLIJ_SESSION override was not honored, got '$out'"
  pass "ap_backend_zellij_session: honors the AP_ZELLIJ_SESSION test-isolation override"
}
...
test_parse_target() {
  ( . "$ROOT/bin/backends/zellij.sh"
    ap_backend_zellij_parse_target "autopilot:5" || exit 1
    [ "$AP_BACKEND_ZELLIJ_SESSION" = autopilot ] || { echo "session mismatch: $AP_BACKEND_ZELLIJ_SESSION" >&2; exit 1; }
    [ "$AP_BACKEND_ZELLIJ_PANE" = "5" ] || { echo "pane mismatch: $AP_BACKEND_ZELLIJ_PANE" >&2; exit 1; }
  ) || fail "ap_backend_zellij_parse_target did not split session:pane correctly"
  pass "ap_backend_zellij_parse_target: splits '<session>:<pane_id>' on the first colon"
}

test_normalize_key() {
  ( . "$ROOT/bin/backends/zellij.sh"
    [ "$(ap_backend_zellij_normalize_key Enter)" = Enter ] || { echo "Enter failed" >&2; exit 1; }
    [ "$(ap_backend_zellij_normalize_key Escape)" = Esc ] || { echo "Escape failed" >&2; exit 1; }
    [ "$(ap_backend_zellij_normalize_key Esc)" = Esc ] || { echo "Esc failed" >&2; exit 1; }
    [ "$(ap_backend_zellij_normalize_key C-c)" = "Ctrl c" ] || { echo "C-c failed" >&2; exit 1; }
    [ "$(ap_backend_zellij_normalize_key ctrl+c)" = "Ctrl c" ] || { echo "ctrl+c failed" >&2; exit 1; }
  ) || fail "ap_backend_zellij_normalize_key did not map autopilot's key vocabulary to zellij's verified names"
  pass "ap_backend_zellij_normalize_key: Enter/Escape/C-c map to zellij's verified Enter/Esc/'Ctrl c'"
}
...
test_scoped_title_uses_primary_home_label() {
  ...
  expected=$(zellij_expected_scoped_title ap-task1 "$dir")
  out=$( AP_HOME="$dir" bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_scoped_title ap-task1' "$ROOT" )
  [ "$out" = "$expected" ] || fail "primary scoped title should be $expected, got '$out'"
  pass "ap_backend_zellij_scoped_title: scopes a primary task title with autopilot plus root hash"
}

test_scoped_title_uses_copilot_home_label() {
  local dir out expected
  dir="$TMP_ROOT/scoped-title-copilot"; mkdir -p "$dir"
  printf 'copilot-one\n' > "$dir/.ap-copilot-home"
  expected=$(zellij_expected_scoped_title ap-task1 "$dir")
  out=$( AP_HOME="$dir" bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_scoped_title ap-task1' "$ROOT" )
  [ "$out" = "$expected" ] || fail "copilot scoped title should be $expected, got '$out'"
  pass "ap_backend_zellij_scoped_title: scopes a copilot task title with the home marker plus root hash"
}
...
test_scoped_title_changes_with_root_path() {
  ...
  expected_one=$(zellij_expected_scoped_title ap-task1 "$home" "$root_one")
  expected_two=$(zellij_expected_scoped_title ap-task1 "$home" "$root_two")
  out_one=$( AP_HOME="$home" AP_ROOT_OVERRIDE="$root_one" bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_scoped_title ap-task1' "$ROOT" )
  out_two=$( AP_HOME="$home" AP_ROOT_OVERRIDE="$root_two" bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_scoped_title ap-task1' "$ROOT" )
  ...
  [ "$out_one" != "$out_two" ] || fail "scoped titles should differ for distinct AP_ROOT paths"
  pass "ap_backend_zellij_scoped_title: includes the resolved AP_ROOT hash in the home label"
}
...
test_expected_label_accepts_unambiguous_untagged_legacy_tab() {
  ...
  # the legacy fallback in ap_backend_zellij_tab_matches_label trusts it.
  zellij_tab_response "$dir" 2 3 ap-legacy
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_key autopilot:7 Escape ap-legacy' "$ROOT"
  expect_code 0 $? "send_key should still reach a task tab spawned before home-scoping delivered, when its untagged title is unambiguous"
  ...
  pass "ap_backend_zellij_tab_matches_label: accepts an untagged legacy ap-<id> title when it is the only live tab carrying it"
}
...
test_expected_label_refuses_ambiguous_untagged_tab() {
  ...
  # untagged legacy title "ap-shared" (our pane's owning tab_id=3 is one of
  ...
  zellij_multi_tab_response "$dir" 2 3 ap-shared 9 ap-shared
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_key autopilot:7 Escape ap-shared' "$ROOT"
  status=$?
  ...
  pass "ap_backend_zellij_tab_matches_label: refuses an untagged legacy label match when 2+ live tabs share it (migration ambiguity guard)"
}
...
test_list_live_scopes_to_own_home_tag() {
  ...
  own_title=$(zellij_expected_scoped_title ap-task1)
  foreign_title=$(zellij_expected_scoped_title ap-task2 "$ROOT" "$other_root")
  # 1: list-tabs --json -> our own home-scoped tab, a DIFFERENT installation's
  # home-scoped tab (same prefix shape, different AP_ROOT hash), and an
  # unrelated non-autopilot tab.
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_list_live autopilot' "$ROOT" )
  [ "$out" = $'autopilot:7\tap-task1' ] \
    || fail "list_live should list only this home's own tagged tab with its plain ap-<id> label, got '$out'"
  pass "ap_backend_zellij_list_live: scopes to this home's own tag - excludes a different installation's tagged tab and unrelated tabs"
}

test_resolve_bare_selector_prefers_scoped_title() {
  local dir fb out title
  dir="$TMP_ROOT/resolve-scoped"; mkdir -p "$dir/responses"
  title=$(zellij_expected_scoped_title ap-resolve1)
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_resolve_bare_selector ap-resolve1' "$ROOT" )
  [ "$out" = "autopilot:12" ] || fail "resolve_bare_selector should resolve the home-scoped tagged tab, got '$out'"
  pass "ap_backend_zellij_resolve_bare_selector: matches the home-scoped tagged title first"
}
...
test_resolve_bare_selector_refuses_ambiguous_untagged() {
  ...
  zellij_multi_tab_response "$dir" 1 3 ap-resolve2 9 ap-resolve2
  zellij_multi_tab_response "$dir" 2 3 ap-resolve2 9 ap-resolve2
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_resolve_bare_selector ap-resolve2' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_zellij_resolve_bare_selector: refuses an ambiguous untagged legacy label shared by 2+ live tabs"
}
...
test_resolve_bare_selector_prefers_later_session_scoped_title_over_legacy() {
  ...
  title=$(zellij_expected_scoped_title ap-resolve3)
  zellij_tab_response "$dir" 1 3 ap-resolve3
  zellij_tab_response "$dir" 2 6 "$title"
  zellij_pane_response "$dir" 3 12 6
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST=$'legacy-session\nscoped-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_resolve_bare_selector ap-resolve3' "$ROOT" )
  ...
  pass "ap_backend_zellij_resolve_bare_selector: checks every session for the scoped title before legacy fallback"
}
...
test_resolve_bare_selector_refuses_cross_session_ambiguous_untagged() {
  ...
  zellij_tab_response "$dir" 1 3 ap-resolve4
  zellij_tab_response "$dir" 2 9 ap-resolve4
  zellij_tab_response "$dir" 3 3 ap-resolve4
  zellij_tab_response "$dir" 4 9 ap-resolve4
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST=$'one-session\ntwo-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_resolve_bare_selector ap-resolve4' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_zellij_resolve_bare_selector: requires untagged legacy labels to be globally unique across sessions"
}
...
test_session_exists_true_when_listed() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST=$'autopilot\nother-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_session_exists autopilot' "$ROOT"
  expect_code 0 $? "session_exists should report true when the session is in the list"
  pass "ap_backend_zellij_session_exists: true when the session name is listed"
}
...
test_session_exists_false_when_absent() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST=$'other-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_session_exists autopilot' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "session_exists should report false when the session is absent"
  pass "ap_backend_zellij_session_exists: false when the session name is not listed"
}
...
test_server_ensure_skips_attach_when_already_exists() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_server_ensure autopilot' "$ROOT"
  ...
  pass "ap_backend_zellij_server_ensure: reuses an existing session without calling attach"
}

# --- dispatch wiring (ap-backend.sh) ------------------------------------------

test_dispatch_routes_zellij_backend() {
  ap_backend_validate zellij 2>/dev/null || fail "ap_backend_validate should accept zellij (P3 adds it to AP_BACKEND_KNOWN)"
  pass "ap_backend_validate: zellij is a known backend (P3)"
}

test_dispatch_busy_state_unknown_for_zellij() {
  # shellcheck source=/dev/null
  . "$ROOT/bin/ap-backend.sh"
  [ "$(ap_backend_busy_state zellij 'autopilot:5')" = unknown ] \
    || fail "ap_backend_busy_state should report unknown for zellij (no native agent-state primitive; D5: watcher falls back to regex, same as tmux)"
  pass "ap_backend_busy_state: zellij (no native primitive) always reports unknown, same as tmux"
}
...
test_create_task_refuses_duplicate_label() {
  ...
  title=$(zellij_expected_scoped_title ap-dup1)
  # 1: list-tabs --json -> existing tab already carrying the home-scoped title
  printf '[{"tab_id":2,"name":"%s","active":false}]\n' "$title" > "$dir/responses/1.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_create_task autopilot ap-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_zellij_create_task: refuses a duplicate home-scoped tab title (zellij's own new-tab has no uniqueness check)"
}

test_create_task_creates_and_parses_ids() {
  local dir fb out title
  dir="$TMP_ROOT/create-task"; mkdir -p "$dir/responses"
  title=$(zellij_expected_scoped_title ap-newtask)
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_create_task autopilot ap-newtask /tmp/proj' "$ROOT" )
  ...
  pass "ap_backend_zellij_create_task: creates a home-scoped tab and parses tab_id/pane_id from the response"
}
...
test_create_task_restores_previously_active_tab() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_create_task autopilot ap-focustest /tmp/proj' "$ROOT" )
  ...
  pass "ap_backend_zellij_create_task: restores focus to the previously-active tab after the steal-focus new-tab call"
}
...
test_create_task_no_restore_when_new_tab_was_already_active() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_create_task autopilot ap-noclient /tmp/proj' "$ROOT" )
  ...
  pass "ap_backend_zellij_create_task: skips the restore call when there was no previously-active tab"
}
...
test_capture_small_reads_use_viewport_and_trim() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_capture autopilot:7 2' "$ROOT" )
  ...
  pass "ap_backend_zellij_capture: small reads use viewport-only dump-screen and trim to N lines locally"
}
...
test_capture_large_reads_use_full_scrollback_and_trim() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_capture autopilot:7 80' "$ROOT" )
  ...
  pass "ap_backend_zellij_capture: reads above the watcher-size threshold request --full scrollback"
}
...
test_capture_fails_when_pane_absent() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_capture autopilot:7 5' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_zellij_capture: fails when the specific pane is absent"
}
...
test_capture_fails_when_session_absent() {
  ...
  pass "ap_backend_zellij_capture: fails when the target session is not listed as active (session_exists pre-check)"
}
...
test_send_key_normalizes_and_targets_pane() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_key autopilot:7 Escape' "$ROOT"
  ...
  pass "ap_backend_zellij_send_key: normalizes the key (Escape -> Esc) and targets the explicit pane id"
}
...
test_send_literal_uses_paste_separator_for_option_shaped_text() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_literal autopilot:7 "--help"' "$ROOT"
  ...
  pass "ap_backend_zellij_send_literal: calls paste with an explicit pane id and a -- separator"
}
...
test_expected_label_allows_matching_task_tab() {
  ...
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 ap-label
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_key autopilot:7 Escape ap-label' "$ROOT"
  expect_code 0 $? "send_key should succeed when the pane belongs to the expected ap-id tab"
  ...
  pass "ap_backend_zellij_target_ready: expected labels allow matching ap-<id> tabs"
}
...
test_expected_label_rejects_reused_pane_id() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    ...
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_key autopilot:7 Escape ap-label' "$ROOT"
  status=$?
  [ "$status" -ne 0 ] || fail "send_key should reject a pane whose tab name does not match the expected ap-id label"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''list-tabs'$'\x1f''--json' \
    "expected-label readiness did not check the pane's owning tab"
  assert_not_contains "$(cat "$dir/log")" $'\x1f''send-keys' \
    "send_key should not run after expected-label readiness fails"
  pass "ap_backend_zellij_target_ready: expected labels reject stale pane ids reused by another tab"
}
...
test_current_path_probes_with_marker_and_ignores_prompt_paths() {
  ...
  printf '%s\n' 'scratch-e2e-project HEAD' \
    ...
    '__AP_ZELLIJ_CWD_BEGIN__' \
    '/home/fixture/.treehouse/fake-' \
    'worktree' \
    '__AP_ZELLIJ_CWD_END__' \
    '/home/fixture/.treehouse/fake-worktree ❯' \
    > "$dir/responses/7.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_current_path autopilot:7' "$ROOT" )
  ...
  assert_contains "$(cat "$dir/log")" "__AP_ZELLIJ_CWD_BEGIN__" "current_path did not send the cwd begin marker via paste"
  ...
  pass "ap_backend_zellij_current_path: actively probes with marked begin/end lines and reconstructs wrapped cwd output"
}
...
test_current_path_ignores_tilde_prefixed_banner_lines() {
  ...
  printf '%s\n' "🌳 Entered worktree at ~/.treehouse/scratch-e2e-project/1. Type 'exit' to return." \
    'scratch-e2e-project HEAD' '__AP_ZELLIJ_CWD_BEGIN__' '/home/fixture/.treehouse/real-worktree' '__AP_ZELLIJ_CWD_END__' '❯' \
    > "$dir/responses/7.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
  ...
  pass "ap_backend_zellij_current_path: never picks up a ~-prefixed banner line as the answer"
}
...
test_kill_resolves_tab_and_closes_by_id() {
  ...
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_kill autopilot:7' "$ROOT"
  ...
  pass "ap_backend_zellij_kill: resolves the owning tab id fresh and calls close-tab-by-id (never a bare close-pane)"
}
...
test_kill_falls_back_to_close_pane_when_tab_lookup_empty() {
  ...
  pass "ap_backend_zellij_kill: falls back to close-pane when the owning tab cannot be resolved"
}
...
test_kill_closes_recorded_tab_when_pane_already_gone() {
  ...
  printf '[]\n' > "$dir/responses/1.out"
  printf '[{"tab_id":3,"name":"ap-zghost"}]\n' > "$dir/responses/2.out"
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_kill autopilot:7 3 ap-zghost' "$ROOT"
  ...
  pass "ap_backend_zellij_kill: closes the recorded tab id only after label verification"
}
...
test_kill_skips_recorded_tab_when_label_mismatches() {
  ...
  pass "ap_backend_zellij_kill: skips a stale recorded tab id whose label does not match"
}
...
test_kill_is_noop_when_session_absent() {
  ...
  pass "ap_backend_zellij_kill: never fails when the target session no longer exists"
}
...
test_teardown_passes_recorded_tab_id_to_zellij_kill() {
  ...
  ap_write_meta "$state/zghost.meta" \
    "window=autopilot:7" \
  ...
  printf '[{"tab_id":3,"name":"ap-zghost"}]\n' > "$dir/responses/2.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-teardown.sh" zghost 2>&1 )
  status=$?
  expect_code 0 "$status" "ap-teardown should succeed for a zellij recon whose worktree is already gone: $out"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''list-tabs'$'\x1f''--json' \
    "ap-teardown did not verify the recorded zellij_tab_id against the task label"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-tab-by-id'$'\x1f''3' \
    "ap-teardown did not pass a verified recorded zellij_tab_id through to kill"
  assert_not_contains "$(cat "$dir/log")" $'\x1f''close-pane' \
    "ap-teardown should close the recorded tab id instead of falling back to close-pane"
  pass "ap-teardown.sh: passes recorded zellij_tab_id with the expected task label"
}

test_forced_copilot_teardown_kills_zellij_children_with_child_home_tag() {
  local dir state data config home project fb out status child_title
  dir="$TMP_ROOT/teardown-zellij-copilot-child"; state="$dir/state"; data="$dir/data"; config="$dir/config"; home="$dir/copilot-home"; project="$dir/project"
  mkdir -p "$state" "$data" "$config" "$home/state" "$home/data" "$home/config" "$home/projects" "$project" "$dir/responses"
  printf 'smz\n' > "$home/.ap-copilot-home"
  ap_write_meta "$state/smz.meta" \
    "window=autopilot:99" \
    ...
    "mode=copilot" \
    "home=$home"
  ap_write_meta "$home/state/childz.meta" \
    "window=autopilot:7" \
    ...
    "project=$project" \
    "kind=recon"
  child_title=$(zellij_expected_scoped_title ap-childz "$home" "$home")
  zellij_pane_response "$dir" 1 7 4
  zellij_tab_response "$dir" 2 4 "$child_title"
  printf '[]\n' > "$dir/responses/3.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
    AP_ROOT_OVERRIDE="$ROOT" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-teardown.sh" smz --force 2>&1 )
  status=$?
  expect_code 0 "$status" "ap-teardown should force-retire a copilot with a zellij child: $out"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-tab-by-id'$'\x1f''4' \
    "forced copilot teardown did not close a child zellij tab scoped to the child home"
  pass "ap-teardown.sh: force cleanup kills zellij children using the child home tag"
}
...
test_send_text_submit_detects_landed_send() {
  ...
  printf '%s' $'❯ hello pilot' > "$dir/responses/4.out"
  printf '%s' $'hello pilot\n❯' > "$dir/responses/8.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_text_submit autopilot:7 "hello pilot" 3 0.01 0.01' "$ROOT" )
  ...
  assert_contains "$(cat "$dir/log")" $'\x1f''paste'$'\x1f''--pane-id'$'\x1f''7'$'\x1f''--'$'\x1f''hello pilot' "send_text_submit did not type the literal text first"
  pass "ap_backend_zellij_send_text_submit: reports 'empty' once the pane content changes after Enter (submitted)"
}
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  printf '%s' $'❯ hello pilot' > "$dir/responses/4.out"
  printf '%s' $'❯ hello pilot' > "$dir/responses/8.out"
  printf '%s' $'❯ hello pilot' > "$dir/responses/12.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="autopilot" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_text_submit autopilot:7 "hello pilot" 2 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_zellij_send_text_submit: reports 'pending' when the pane never changes after retried Enters (swallowed)"
}
...
test_send_text_submit_send_failed_when_session_absent() {
  ...
  out=$( PATH="$fb:$PATH" AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" \
    AP_ZELLIJ_SESSION_LIST="" \
    bash -c '. "$0/bin/backends/zellij.sh"; ap_backend_zellij_send_text_submit autopilot:7 "x" 2 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "send_text_submit should report send-failed when the session does not exist, got '$out'"
  pass "ap_backend_zellij_send_text_submit: reports 'send-failed' when the target session is not active"
}
...
test_send_text_submit_send_failed_when_pane_absent() {
  ...
  pass "ap_backend_zellij_send_text_submit: reports 'send-failed' when the target pane is absent"
}

# --- ap-*.sh script routing via explicit backend-tagged meta ------------------
...
test_scripts_route_explicit_target_through_meta_backend() {
  ...
  ap_write_meta "$state/zellij-stale.meta" "window=autopilot:7" "backend=zellij"
...
SH
  ...
  out=$( PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-peek.sh" autopilot:7 5 2>/dev/null )
  [ "$out" = "captured zellij pane" ] || fail "ap-peek did not capture through zellij for an explicit metadata-matched target, got '$out'"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''dump-screen' \
    "ap-peek did not verify the pane before capture"
  assert_contains "$(cat "$dir/log")" $'\x1f''dump-screen'$'\x1f''--pane-id'$'\x1f''7' \
    "ap-peek did not route the explicit metadata-matched target through zellij capture"

  : > "$dir/log"
  PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_HOME="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-send.sh" autopilot:7 --key Escape >/dev/null 2>&1
  expect_code 0 $? "ap-send --key should route an explicit metadata-matched target through zellij"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''send-keys' \
    "ap-send did not verify the pane before send-key"
  assert_contains "$(cat "$dir/log")" $'\x1f''send-keys'$'\x1f''--pane-id'$'\x1f''7'$'\x1f''Esc' \
    "ap-send did not route the explicit metadata-matched target through zellij send-key"

  pass "ap-peek/ap-send: explicit metadata-matched targets use the recorded zellij backend"
}

test_scripts_verify_label_for_ap_targets() {
  local dir state fb neutral out
  dir="$TMP_ROOT/script-ap-target-label"; state="$dir/state"; mkdir -p "$state" "$dir/responses"
  neutral="$dir/neutral-root"; mkdir -p "$neutral"
  ap_write_meta "$state/zlabel.meta" "window=autopilot:7" "backend=zellij"
  touch "$state/.last-watcher-beat"
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 ap-zlabel
  printf 'captured through ap-id\n' > "$dir/responses/3.out"
  fb=$(make_zellij_fakebin "$dir")

  out=$( PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-peek.sh" ap-zlabel 5 2>/dev/null )
  [ "$out" = "captured through ap-id" ] || fail "ap-peek did not capture through zellij for an ap-id target with a matching tab label, got '$out'"
  zellij_assert_call_order "$dir/log" $'\x1f''list-tabs'$'\x1f''--json' $'\x1f''dump-screen' \
    "ap-peek did not verify the ap-id tab label before capture"

  pass "ap-peek: ap-id zellij targets verify the owning tab label before capture"
}

test_scripts_reject_ap_target_label_mismatch() {
  local dir state fb neutral status
  dir="$TMP_ROOT/script-ap-target-label-mismatch"; state="$dir/state"; mkdir -p "$state" "$dir/responses"
  neutral="$dir/neutral-root"; mkdir -p "$neutral"
  ap_write_meta "$state/zreuse.meta" "window=autopilot:7" "backend=zellij"
  touch "$state/.last-watcher-beat"
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 not-the-task
  fb=$(make_zellij_fakebin "$dir")
  ...
  PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_HOME="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_ZELLIJ_LOG="$dir/log" AP_ZELLIJ_RESPONSES="$dir/responses" AP_ZELLIJ_SESSION_LIST="autopilot" \
    "$ROOT/bin/ap-send.sh" ap-zreuse --key Escape >/dev/null 2>&1
  status=$?
  [ "$status" -ne 0 ] || fail "ap-send --key should reject an ap-id zellij target whose pane belongs to a differently named tab"
  assert_not_contains "$(cat "$dir/log")" $'\x1f''send-keys' \
    "ap-send should not send a key after ap-id label verification fails"
  pass "ap-send: ap-id zellij targets reject pane ids whose tab label no longer matches"
}
