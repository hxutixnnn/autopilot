...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/ap-herdr-presentation.XXXXXX")
...
export AP_BACKEND_HERDR_WORKSPACE_MOVER="$FAKEBIN/herdr-workspace-mover"
...
HERDR_LAB_SESSION=$(PATH="$HERDR_ORIGINAL_PATH" \
  "$HERDR_LAB_HELPER" name ap-herdr-presentation-projection)
...
spawn_task() {  # <id> <home> <project>
  ...
  AP_GATE_REFUSE_BYPASS=1 AP_SPAWN_NO_GUARD=1 AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/ap-spawn.sh" "$id" "$project" "sh -c 'sleep 120'" --backend herdr
}

spawn_copilot_task() {
  local id=$1 home=$2
  AP_GATE_REFUSE_BYPASS=1 AP_SPAWN_NO_GUARD=1 AP_HOME="$HOME_DIR" AP_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/ap-spawn.sh" "$id" "$home" "sh -c 'sleep 120'" --copilot --backend herdr
}

teardown_task() {  # <id> <home>
  local id=$1 home=$2
  AP_GATE_REFUSE_BYPASS=1 AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" \
    AP_STATE_OVERRIDE="$home/state" AP_DATA_OVERRIDE="$home/data" \
    AP_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/ap-teardown.sh" "$id" --force
}
...
session_presentation_lock_path() {
  PATH="$FAKEBIN:$PATH" HERDR_SESSION="$HERDR_LAB_SESSION" bash -c '
    ...
    ap_backend_herdr_presentation_session_lock_path "$1"
...
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config" \
  ...
  "$HOME_DIR/data/order-fail" "$HOME_DIR/data/ap-hibit-resume-r1" \
...
# Keep one ordinary primary task live so the durable autopilot workspace is
...
AUTOPILOT_WSID=$(grep '^herdr_workspace_id=' "$ANCHOR_META" | cut -d= -f2-)
[ -n "$AUTOPILOT_WSID" ] || fail "anchor metadata did not record the autopilot workspace"
...
SECOND_ONE_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label copilot-alpha --no-focus) \
  || fail "could not create the first copilot presentation fixture"
SECOND_TWO_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label copilot-bravo --focus) \
  || fail "could not create the focused copilot presentation fixture"
...
[ -n "$SECOND_ONE_WSID" ] && [ -n "$SECOND_TWO_WSID" ] && [ -n "$SECOND_TWO_TAB" ] && [ -n "$SECOND_TWO_PANE" ] \
  || fail "copilot presentation fixtures returned incomplete IDs"
SECOND_ORDER_BEFORE=$(printf '%s\n%s\n' "$SECOND_ONE_WSID" "$SECOND_TWO_WSID")
PILOT_FOCUS="$SECOND_TWO_WSID/$SECOND_TWO_TAB"
assert_focus_is "$PILOT_FOCUS" "focused copilot fixture"
...
assert_focus_is "$PILOT_FOCUS" "projected spawn"
...
printf '%s' "$PROJECTED_TABS" | jq -e --arg tab "$PROJECTED_TAB" \
  '.result.tabs[0].tab_id == $tab and .result.tabs[0].label == "ap-shape"' >/dev/null 2>&1 \
  || fail "projected workspace's only tab was not the normal ap-shape task tab"
...
SECOND_TWO_INFO=$(lab workspace get "$SECOND_TWO_WSID") || fail "focused copilot disappeared during projected create"
[ "$(printf '%s' "$SECOND_TWO_INFO" | jq -r '.result.workspace.focused')" = true ] \
  || fail "projected create or workspace.move stole focus from the pilot's current space"
...
grep -F "target is the pilot's active tab" "$TMP_ROOT/active-seeded.err" >/dev/null 2>&1 \
...
lab tab focus "$SECOND_TWO_TAB" >/dev/null || fail "could not restore the captured pilot tab after the active seeded-tab fixture"
assert_focus_is "$PILOT_FOCUS" "active seeded-tab fixture restoration"
...
PATH="$FAKEBIN:$PATH" AP_HOME="$HOME_DIR" bash -c '
  . "$0/bin/ap-wake-lib.sh"
  . "$0/bin/backends/herdr.sh"
  lock=$1
  ap_lock_acquire_wait "$lock"
  ap_backend_herdr_projection_cleanup_exact "$2" "$3" "$4"
  ap_lock_release "$lock"
' "$ROOT" "$ACTIVE_SEEDED_LOCK" "$HERDR_LAB_SESSION" "$ACTIVE_SEEDED_TASK_PANE" "$ACTIVE_SEEDED_PANE"
assert_focus_is "$PILOT_FOCUS" "active seeded-tab fixture cleanup"
assert_cleanup_focus_preserved "$ACTIVE_SEEDED_CLEANUP_FOCUS_START" "$ACTIVE_SEEDED_PANE" "$PILOT_FOCUS"
...
ROOT="$ROOT" READY="$LOCK_CONTENTION_READY" RELEASE="$LOCK_CONTENTION_RELEASE" \
  ...
  . "$ROOT/bin/ap-wake-lib.sh"
  ap_lock_try_acquire "$LOCK" || exit 1
  : > "$READY"
...
[ "$LOCK_CONTENTION_WSID" = "$AUTOPILOT_WSID" ] \
  || fail "bounded lock contention did not use the ordinary flat autopilot workspace"
...
assert_focus_is "$PILOT_FOCUS" "bounded presentation lock flat fallback"
...
assert_focus_is "$PILOT_FOCUS" "bounded presentation lock flat fallback teardown"
...
assert_focus_is "$PILOT_FOCUS" "concurrent projected spawns"
...
EXPECTED_LABELS=$(printf 'autopilot\n%s\n%s\ncopilot-alpha\ncopilot-bravo' "$PROJECTED_LABEL" "$CREATED_LABELS")
ACTUAL_LABELS=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[].label')
[ "$ACTUAL_LABELS" = "$EXPECTED_LABELS" ] || fail "workspace order was not autopilot, stable primary block, copilots: $ACTUAL_LABELS"
PRIMARY_IDS=$(printf '%s' "$ORDER_LIST" | jq -r '
  .result.workspaces[]
  | select((.label | startswith("└ ")) or (.label | startswith("autopilot/")))
  | .workspace_id
')
...
SECOND_ORDER_AFTER=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[] | select(.label | startswith("copilot-")) | .workspace_id')
[ "$SECOND_ORDER_AFTER" = "$SECOND_ORDER_BEFORE" ] \
  || fail "primary workspace ordering changed copilot relative order"
...
AP_BACKEND_HERDR_WORKSPACE_MOVER="$FAIL_MOVER" \
...
assert_focus_is "$PILOT_FOCUS" "failed presentation ordering"
...
assert_focus_is "$PILOT_FOCUS" "concurrent post-create abort cleanup"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_A_PANE" "$PILOT_FOCUS"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_B_PANE" "$PILOT_FOCUS"
...
assert_focus_is "$PILOT_FOCUS" "projected teardown"
assert_cleanup_focus_steal_was_restored "$SHAPE_CLEANUP_AUDIT_START" "$PROJECTED_PANE" "$PILOT_FOCUS"
...
lab pane get "$SECOND_TWO_PANE" >/dev/null 2>&1 \
  || fail "projected teardown affected the focused copilot workspace"
[ ! -e "$JOURNAL" ] || fail "confirmed projected teardown did not retire its presentation journal"
pass "real Herdr lab: exact task-pane close restores the exact pilot workspace/tab after Herdr's raw focus steal"
...
assert_focus_is "$PILOT_FOCUS" "concurrent projected teardowns"
...
assert_focus_is "$PILOT_FOCUS" "failed-order projection teardown"
...
for ROUND in 1 2 3; do
  ...
  assert_focus_is "$PILOT_FOCUS" "focus wave $ROUND concurrent spawns"
  ...
  WAVE_EXPECTED=$(printf 'autopilot\n%s\ncopilot-alpha\ncopilot-bravo' "$WAVE_LABELS")
  WAVE_ACTUAL=$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "autopilot" or (.label | startswith("└ ")) or (.label | startswith("copilot-"))) | .label')
  [ "$WAVE_ACTUAL" = "$WAVE_EXPECTED" ] \
    || fail "focus wave $ROUND lost stable contiguous ordering: $WAVE_ACTUAL"
  WAVE_SECOND_ORDER=$(lab workspace list | jq -r '.result.workspaces[] | select(.label | startswith("copilot-")) | .workspace_id')
  [ "$WAVE_SECOND_ORDER" = "$SECOND_ORDER_BEFORE" ] \
    || fail "focus wave $ROUND changed copilot relative order"
  ...
  assert_focus_is "$PILOT_FOCUS" "focus wave $ROUND concurrent teardowns"
  WAVE_REMAINING=$(lab workspace list | jq -r '.result.workspaces[].label')
  [ "$WAVE_REMAINING" = $'autopilot\ncopilot-alpha\ncopilot-bravo' ] \
...
# ------------------------------------------------------------------
# Multi-home topology: real copilot AP_HOME spawn paths, inheritance,
# concurrent cross-home waves, and session-scoped lock contention.
# ------------------------------------------------------------------
SECOND_HOME_A="$TMP_ROOT/home-copilot-alpha"
SECOND_HOME_B="$TMP_ROOT/home-copilot-bravo"
...
git -C "$SECOND_HOME_A" -c user.name='Autopilot Tests' -c user.email='tests@example.invalid' commit -qm init
git -C "$SECOND_HOME_B" -c user.name='Autopilot Tests' -c user.email='tests@example.invalid' commit -qm init
mkdir -p "$SECOND_HOME_A/bin"
printf '# Autopilot copilot fixture\n' > "$SECOND_HOME_A/AGENTS.md"
printf 'Copilot alpha charter.\n' > "$SECOND_HOME_A/data/charter.md"

# Primary flag only; real inheritance must push presence into both copilot homes.
...
[ ! -e "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "copilot A unexpectedly had the presentation flag before inheritance"
[ ! -e "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "copilot B unexpectedly had the presentation flag before inheritance"
SECOND_SPAWN_LOG_START=$(log_line_count)
spawn_copilot_task alpha "$SECOND_HOME_A" > "$TMP_ROOT/alpha.out" 2> "$TMP_ROOT/alpha.err" \
  || fail "copilot alpha spawn failed: $(cat "$TMP_ROOT/alpha.err")"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "copilot spawn did not inherit the presentation flag"
[ ! -e "$HOME_DIR/state/alpha.herdr-presentation" ] \
  || fail "copilot spawn published a presentation journal"
SECOND_META="$HOME_DIR/state/alpha.meta"
[ "$(grep '^kind=' "$SECOND_META" | cut -d= -f2-)" = copilot ] \
  || fail "copilot spawn did not record kind=copilot"
...
[ "$SECOND_LABEL" = copilot-alpha ] \
  || fail "copilot spawn did not use its flat parent workspace: $SECOND_LABEL"
[ -z "$(projection_labels_from_log "$SECOND_SPAWN_LOG_START")" ] \
  || fail "copilot spawn created a corner projection workspace"
if sed -n "$((SECOND_SPAWN_LOG_START + 1)),\$p" "$HERDR_CALL_LOG" \
  | grep -E $'^(workspace\tmove|session\tlist)' >/dev/null 2>&1; then
  fail "copilot spawn attempted presentation ordering"
fi
# shellcheck source=/dev/null
. "$ROOT/bin/ap-config-inherit-lib.sh"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_A/config" \
  || fail "inheritance into copilot A failed"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_B/config" \
  || fail "inheritance into copilot B failed"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "primary presentation flag did not reach copilot A"
[ -f "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "primary presentation flag did not reach copilot B"
pass "real Herdr lab: primary presentation opt-in inherits into real copilot homes"

# Keep the pre-existing copilot-alpha/bravo workspaces as owning parents and pilot focus.
assert_focus_is "$PILOT_FOCUS" "multi-home pilot focus"
...
spawn_task a1 "$SECOND_HOME_A" "$PROJECT_DIR" > "$TMP_ROOT/a1.out" 2> "$TMP_ROOT/a1.err" \
  || fail "multi-home copilot A a1 failed: $(cat "$TMP_ROOT/a1.err")"
spawn_task a2 "$SECOND_HOME_A" "$PROJECT_DIR" > "$TMP_ROOT/a2.out" 2> "$TMP_ROOT/a2.err" \
  || fail "multi-home copilot A a2 failed: $(cat "$TMP_ROOT/a2.err")"
spawn_task b1 "$SECOND_HOME_B" "$PROJECT_DIR" > "$TMP_ROOT/b1.out" 2> "$TMP_ROOT/b1.err" \
  || fail "multi-home copilot B b1 failed: $(cat "$TMP_ROOT/b1.err")"
spawn_task b2 "$SECOND_HOME_B" "$PROJECT_DIR" > "$TMP_ROOT/b2.out" 2> "$TMP_ROOT/b2.err" \
  || fail "multi-home copilot B b2 failed: $(cat "$TMP_ROOT/b2.err")"
...
assert_focus_is "$PILOT_FOCUS" "multi-home sequential spawns"
...
case "$A1_LABEL" in $'└ a1 · p:'*) ;; *) fail "copilot A a1 label wrong: $A1_LABEL" ;; esac
case "$A2_LABEL" in $'└ a2 · p:'*) ;; *) fail "copilot A a2 label wrong: $A2_LABEL" ;; esac
case "$B1_LABEL" in $'└ b1 · p:'*) ;; *) fail "copilot B b1 label wrong: $B1_LABEL" ;; esac
case "$B2_LABEL" in $'└ b2 · p:'*) ;; *) fail "copilot B b2 label wrong: $B2_LABEL" ;; esac
...
MULTI_LABELS=$(printf '%s' "$MULTI_LIST" | jq -r '
  ...
  | select(
      .label == "autopilot"
      or .label == "copilot-alpha"
      or .label == "copilot-bravo"
      or (.label | startswith("└ "))
    )
  | .label
')
MULTI_EXPECTED=$(printf '%s\n' \
  autopilot "$P1_LABEL" "$P2_LABEL" \
  copilot-alpha "$A1_LABEL" "$A2_LABEL" \
  copilot-bravo "$B1_LABEL" "$B2_LABEL")
[ "$MULTI_LABELS" = "$MULTI_EXPECTED" ] \
  || fail "multi-home topology was not owning-parent grouped: $MULTI_LABELS"
pass "real Herdr lab: primary and two copilot homes each own a top-level contiguous child block"
...
assert_focus_is "$PILOT_FOCUS" "cross-home concurrent wave"
...
printf '%s' "$CROSS_LIST" | jq -e '
  ([.result.workspaces[].label] | index("autopilot")) as $primary
  | ([.result.workspaces[].label] | index("copilot-alpha")) as $a
  | ([.result.workspaces[].label] | index("copilot-bravo")) as $b
  | $primary != null and $a != null and $b != null
  and $primary < $a and $a < $b
...
case "$PCW_LABEL" in $'└ pcw · p:'*|autopilot) ;; *) fail "cross-home primary label wrong: $PCW_LABEL" ;; esac
case "$ACW_LABEL" in $'└ acw · p:'*|copilot-alpha) ;; *) fail "cross-home A label wrong: $ACW_LABEL" ;; esac
case "$BCW_LABEL" in $'└ bcw · p:'*|copilot-bravo) ;; *) fail "cross-home B label wrong: $BCW_LABEL" ;; esac
...
[ "$AFLAT_LABEL" = copilot-alpha ] \
  || fail "cross-home lock contention did not use the ordinary copilot home workspace: $AFLAT_LABEL"
[ ! -e "$SECOND_HOME_A/state/aflat.herdr-presentation" ] \
  || fail "cross-home lock contention published a projection journal"
assert_focus_is "$PILOT_FOCUS" "cross-home lock contention flat fallback"
...
pass "real Herdr lab: session lock contention from a copilot home falls back flat with no journal"

# Same-identity recovery replaces only one exact agent-free husk in its
# original projected workspace.
# Exercise both the leading ap- identity style seen in Hi Bit work and the
# project-name identity style used by Wheelhouse work.
for RESTART_ID in ap-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  EXPECTED_CONCISE=${RESTART_ID#ap-}
  ...
  if [ "$RESTART_ID" = ap-hibit-resume-r1 ]; then
...
# A copilot child binds and reclaims only inside its own home and parent.
...
[ "$CROSS_BOUND_HOME" = "$(cd "$SECOND_HOME_A" && pwd -P)" ] \
  || fail "cross-home restart journal did not bind the copilot's exact home"
...
[ "$CROSS_NEW_WSID" = "$CROSS_OLD_WSID" ] && [ "$CROSS_NEW_PANE" != "$CROSS_OLD_PANE" ] \
  || fail "cross-home reclaim did not replace one pane inside the same copilot child workspace"
[ "$(lab workspace get "$CROSS_NEW_WSID" | jq -r '.result.workspace.label')" = "$CROSS_OLD_LABEL" ] \
  || fail "cross-home reclaim changed the copilot child's presentation label"
...
pass "real Herdr lab: copilot restart binding and reclaim stay isolated to the exact child home and parent"
...
printf 'Concurrent copilot recovery fixture.\n' > "$SECOND_HOME_B/data/$BRAVO_WAVE_ID/brief.md"
...
spawn_task "$BRAVO_WAVE_ID" "$SECOND_HOME_B" "$PROJECT_DIR" > "$TMP_ROOT/bravo-wave-first.out" 2> "$TMP_ROOT/bravo-wave-first.err" \
  || fail "copilot recovery-wave fixture failed: $(cat "$TMP_ROOT/bravo-wave-first.err")"
...
wait "$BRAVO_WAVE_PID" || fail "concurrent copilot recovery failed: $(cat "$TMP_ROOT/bravo-wave-resume.err")"
...
teardown_task "$BRAVO_WAVE_ID" "$SECOND_HOME_B" > "$TMP_ROOT/bravo-wave-teardown.out" 2> "$TMP_ROOT/bravo-wave-teardown.err" \
  || fail "concurrent copilot recovery teardown failed"
...
# Seed a legacy old-format primary projection and a flat copilot tab; correction must not migrate them.
LEGACY_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "autopilot/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" --no-focus) \
...
FLAT_TAB_OUT=$(lab tab create --workspace "$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "copilot-alpha") | .workspace_id' | head -1)" --cwd "$PROJECT_DIR" --label ap-flat-legacy-tab --no-focus) \
  || fail "could not seed a flat copilot child tab"
...
[ "$(lab workspace get "$LEGACY_WSID" | jq -r '.result.workspace.label')" = "autopilot/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" ] \
  || fail "correction renamed or moved the seeded legacy projection"
lab tab get "$FLAT_TAB_ID" >/dev/null 2>&1 \
  || fail "correction removed the seeded flat copilot child tab"
pass "real Herdr lab: legacy projection labels and flat copilot tabs are left unmigrated"
...
assert_focus_is "$PILOT_FOCUS" "multi-home teardown"
pass "real Herdr lab: multi-home exact-pane teardowns restore pilot focus without workspace close authority"
...
ap_backend_herdr_projection_journal_create "$MISSING_STATE" missing1 >/dev/null
MISSING_JOURNAL=$(ap_backend_herdr_projection_journal_path "$MISSING_STATE" missing1)
START=$(log_line_count)
ap_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$MISSING_JOURNAL" missing1 \
...
RENAMED_TOKEN=$(ap_backend_herdr_projection_journal_create "$RENAMED_STATE" renamed1)
RENAMED_JOURNAL=$(ap_backend_herdr_projection_journal_path "$RENAMED_STATE" renamed1)
RENAMED_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "autopilot/renamed1 · p:$RENAMED_TOKEN" --no-focus)
...
START=$(log_line_count)
ap_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$RENAMED_JOURNAL" renamed1 \
...
DUP_TOKEN=$(ap_backend_herdr_projection_journal_create "$DUP_STATE" duplicate1)
DUP_JOURNAL=$(ap_backend_herdr_projection_journal_path "$DUP_STATE" duplicate1)
DUP1=$(lab workspace create --cwd "$PROJECT_DIR" --label "autopilot/duplicate1 · p:$DUP_TOKEN" --no-focus)
...
START=$(log_line_count)
ap_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1 \
...
lab pane report-agent "$DUP1_PANE" --source ap-projection-e2e --agent test-agent --state idle >/dev/null \
  || fail "could not register the duplicate-live-agent risk fixture"
START=$(log_line_count)
if ap_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1; then
