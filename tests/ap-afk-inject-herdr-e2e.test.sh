#!/usr/bin/env bash
# tests/ap-afk-inject-herdr-e2e.test.sh - real-herdr end-to-end test for the
# away-mode daemon's herdr transport (bin/ap-supervise-daemon.sh), the herdr
# counterpart of tests/ap-afk-inject-e2e.test.sh's private-socket tmux e2e.
# Mirrors tests/ap-backend-herdr-smoke.test.sh and tests/herdr-test-safety.sh's
...
# at AP_SUPERVISOR_BACKEND=herdr, AP_SUPERVISOR_TARGET="<session>:<pane-id>",
...
# bordered branch of ap_backend_herdr_composer_state, and logs every submitted
# line (hex + text + injection/user classification) - the same technique
# tests/ap-afk-inject-e2e.test.sh uses for its tmux supervisor pane, so this
...
# ap_backend_herdr_send_text_submit's confirmation is native agent-state
...
# --- source the daemon (for afk_enter/afk_exit/AP_INJECT_MARK) + the backend -
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-herdr-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
...
TASK_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "ap-afk-e2e-supervisor" /tmp "$SEEDED_TAB_ID") \
...
for _ in $(seq 1 100); do
  PROCESS_INFO=$(ap_backend_herdr_cli "$SESSION" pane process-info --pane "$PANE_ID" 2>/dev/null || true)
...
# e2e's fake ap-fake-c1 flight crew member window - not required by scan_signals (which
...
FAKE_FLIGHT_CREW_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "ap-fake-c1" /tmp) \
  || fail "could not create the fake flight crew member scratch tab"
read -r _FAKE_TAB_ID FAKE_FLIGHT_CREW_PANE_ID <<EOF
$FAKE_FLIGHT_CREW_IDS
EOF

# --- deterministic bordered-composer loop, drawn in the scratch pane ---------
# Mirrors tests/ap-afk-inject-e2e.test.sh's supervisor-loop.sh, but draws a
# "│ > <buf> │" border so the bordered branch of
# ap_backend_herdr_composer_state recognizes it, exactly like a bordered-TUI
...
# submission: ap_backend_herdr_send_text_submit's confirmation is now native
...
MARK=$'\xE2\x81\xA3'
LOG="$1"
AGENT_SOURCE=ap-test-supervisor
AGENT_LABEL=ap-test-supervisor
...
submit_line() {
  ...
  # ap_backend_herdr_send_text_submit now confirms against. The 0.6s "working"
  # window comfortably covers the daemon's AP_INJECT_CONFIRM_SLEEP=0.5
...
ap_backend_herdr_send_text_line "$SUPERVISOR_TARGET" "bash '$LOOP_SCRIPT' '$LOG_FILE'" \
...
HERDR_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ap-herdr-shim.XXXXXX")
...
start_daemon() {
  ...
  AP_STATE_OVERRIDE="$STATE_DIR" \
  AP_SUPERVISOR_BACKEND=herdr \
  AP_SUPERVISOR_TARGET="$SUPERVISOR_TARGET" \
  AP_ESCALATE_BATCH_SECS=0 \
  AP_HOUSEKEEPING_TICK=1 \
  AP_POLL=1 \
  AP_SIGNAL_GRACE=1 \
  AP_HEARTBEAT=999999 \
  AP_CHECK_INTERVAL=999999 \
  AP_INJECT_CONFIRM_SLEEP=0.5 \
  AP_INJECT_CONFIRM_RETRIES=6 \
  AP_STALE_ESCALATE_SECS=999999 \
...
# Verify pane_input_pending (dispatched through ap_backend_composer_state for
...
selfcheck_pane_input_pending() {
  ...
  ap_backend_herdr_send_literal "$SUPERVISOR_TARGET" "$check_text" \
  ...
  if PATH="$HERDR_SHIM_DIR:$PATH" pane_input_pending "$SUPERVISOR_TARGET" herdr; then
    ap_backend_herdr_send_key "$SUPERVISOR_TARGET" Enter
    sleep 0.5
    return 0
  fi
  echo "pane_input_pending cannot detect typed text in this real-herdr environment" >&2
  ap_backend_herdr_capture "$SUPERVISOR_TARGET" 10 | sed 's/^/    /' >&2
  ap_backend_herdr_send_key "$SUPERVISOR_TARGET" Enter
...
test_scenario_a() {
  ...
  ap_backend_herdr_send_literal "$SUPERVISOR_TARGET" "human draft text"
  sleep 0.5
...
test_scenario_c() {
  ...
  stop_daemon
  pass "real herdr Scenario C: a normal pilot status injects exactly one clean single-line sentinel digest"
}
...
# must never crash or hot-loop. Exercises ap_backend_composer_state(herdr, ...)
...
test_scenario_d_max_defer() {
  ...
  ap_backend_herdr_send_literal "$SUPERVISOR_TARGET" "stuck-in-the-box"
  sleep 0.5
  ...
  AP_CHECK_INTERVAL=999999 \
  AP_MAX_DEFER_SECS=3 \
  AP_INJECT_CONFIRM_SLEEP=0.3 \
  AP_INJECT_CONFIRM_RETRIES=2 \
  AP_STALE_ESCALATE_SECS=999999 \
  nohup "$DAEMON" >"$STATE_DIR/daemon.out" 2>"$STATE_DIR/daemon.err" &
  DAEMON_PID=$!
  ...
  ap_backend_herdr_send_key "$SUPERVISOR_TARGET" C-c >/dev/null 2>&1 || true
...
ap_backend_herdr_kill "$SUPERVISOR_TARGET" 2>/dev/null || true
ap_backend_herdr_kill "$SESSION:$FAKE_FLIGHT_CREW_PANE_ID" 2>/dev/null || true
cleanup_all
trap - EXIT
