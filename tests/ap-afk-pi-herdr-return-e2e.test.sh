...
# Herdr call is routed through ap-herdr-lab.sh. The scenario proves:
...
. "$ROOT/bin/ap-supervise-daemon.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/ap-backend.sh"

if [ "${AP_AFK_PI_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set AP_AFK_PI_HERDR_E2E=1 to run the real Pi/Herdr away-return regression"
  exit 0
fi
...
LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/ap-herdr-lab.sh}
SESSION=$("$LAB_HELPER" name ap-afk-pi-return-e2e)
TMP_ROOT=$(ap_test_tmproot ap-afk-pi-return-e2e)
...
cleanup() {
  ...
  if [ "$DAEMON_STARTED" -eq 1 ]; then
    PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
      AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="$PRIMARY_TARGET" \
      "$ROOT/bin/ap-afk-launch.sh" stop >/dev/null 2>&1 || true
  fi
...
printf '# Synthetic isolated Autopilot primary\n' > "$PROJECT/AGENTS.md"
...
const capturePath = process.env.AP_PI_CAPTURE_PATH!;
...
export AP_STATE_OVERRIDE='$STATE'
export AP_ESCALATE_BATCH_SECS=0
export AP_HOUSEKEEPING_TICK=1
export AP_POLL=1
export AP_SIGNAL_GRACE=1
export AP_HEARTBEAT=999999
export AP_CHECK_INTERVAL=999999
export AP_MAX_DEFER_SECS=3
export AP_STALE_ESCALATE_SECS=999999
export AP_WEDGE_ALARM_EXEC='$TMP_ROOT/wedge-recorder'
exec '$ROOT/bin/ap-afk-start.sh'
EOF
...
EXT="$CAPTURE_EXT"
PI_CMD=$(printf 'exec env PI_CODING_AGENT_DIR=%q AP_HOME=%q AP_PI_CAPTURE_PATH=%q pi -e %q --no-context-files --no-session' "$PI_DIR" "$HOME_DIR" "$CAPTURE" "$EXT")
...
CHILD_OUT=$("$LAB_HELPER" run "$SESSION" tab create --workspace "$WORKSPACE" --cwd "$PROJECT" --label ap-repair-task --no-focus)
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="$PRIMARY_TARGET" AP_AFK_LAUNCH_ENTRY="$TMP_ROOT/daemon-entry" \
  "$ROOT/bin/ap-afk-launch.sh" start >/dev/null
DAEMON_STARTED=1
...
sleep 0.5
composer=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" ap_backend_composer_state herdr "$PRIMARY_TARGET")
[ "$composer" = pending ] || fail "real Pi draft did not classify pending (got $composer)"
CHILD_CMD=$(printf "printf 'blocked [key=synthetic-dependency]: autopilot can refresh the synthetic token\\n' >> %q; exec sleep 120" "$STATE/repair-task.status")
...
# The pilot returns with an ordinary unmarked Bearings request. The request is
...
set +e
RETURN_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_ROOT_OVERRIDE="$PROJECT" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="$PRIMARY_TARGET" "$ROOT/bin/ap-afk-return.sh" begin 2>&1)
RETURN_RC=$?
set -e
DAEMON_STARTED=0
[ "$RETURN_RC" -eq 3 ] || fail "return catch-up did not gate the still-live blocker (rc=$RETURN_RC): $RETURN_OUT"
assert_contains "$RETURN_OUT" 'autopilot-actionable blocker: repair-task [key=synthetic-dependency]' "return gate did not assign remediation"
set +e
BEARINGS_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_ROOT_OVERRIDE="$PROJECT" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/ap-bearings-snapshot.sh" --json 2>&1)
BEARINGS_RC=$?
set -e
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_ROOT_OVERRIDE="$PROJECT" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/ap-afk-return.sh" check >/dev/null || fail "remediated blocker did not clear return catch-up"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_ROOT_OVERRIDE="$PROJECT" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/ap-bearings-snapshot.sh" --json >/dev/null || fail "Bearings remained gated after blocker remediation"
...
DAEMON_STARTED=1
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" AP_ROOT_OVERRIDE="$PROJECT" AP_HOME="$HOME_DIR" AP_STATE_OVERRIDE="$STATE" \
  AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="$PRIMARY_TARGET" "$ROOT/bin/ap-afk-return.sh" begin >/dev/null \
