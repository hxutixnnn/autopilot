#!/usr/bin/env bash
# tests/ap-backend-herdr.test.sh - fake-herdr-CLI unit tests for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/ap-backend-design-d7 (herdr-addendum.md). Mirrors tests/ap-backend.test.sh's
...
# The real-binary smoke test lives in tests/ap-backend-herdr-smoke.test.sh,
...
TMP_ROOT=$(ap_test_tmproot ap-backend-herdr-tests)
export AP_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0

# make_herdr_fakebin: a `herdr` stub that logs every invocation (one line,
# unit-separated args, to $AP_HERDR_LOG) and returns the canned response for
# that call read from $AP_HERDR_RESPONSES/<n>.out, consumed IN ORDER (call 1
...
if [ "${1:-}" = status ] && [ "${2:-}" = --json ] && [ "${AP_HERDR_SCRIPT_STATUS:-0}" != 1 ]; then
...
# repeatedly and the "one persistent autopilot workspace, no orphans"
...
# ($AP_FAKE_HERDR_STATE) mutated with real jq. Modeled behaviors, all
...
# a pane with no registered agent). Every call is logged to $AP_HERDR_LOG in
...
# Used to exercise ap_backend_herdr_workspace_prune_seeded_default_tab's
...
# herdr_case <name> -> sets up AP_HERDR_LOG/AP_HERDR_RESPONSES/fb for one test,
...
test_version_check_accepts_current_protocol() {
  ...
  PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_version_check' "$ROOT"
  status=$?
  ...
  pass "ap_backend_herdr_version_check: accepts the current protocol (14)"
}
...
test_version_check_refuses_old_protocol() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_version_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_version_check: refuses an old protocol loudly"
}
...
test_version_check_refuses_missing_herdr() {
  ...
  pass "ap_backend_herdr_version_check: refuses loudly when herdr is not installed"
}

# --- workspace_label: per-autopilot-HOME resolution (P3, herdr-copilot-spaces-k4) -

test_workspace_label_primary_home_no_marker() {
  local home
  home="$TMP_ROOT/primary-home-no-marker"; mkdir -p "$home"
  out=$( AP_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "autopilot" ] || fail "a primary home (no .ap-copilot-home marker) should resolve to label 'autopilot', got '$out'"
  pass "ap_backend_herdr_workspace_label: a primary home (no marker) resolves to 'autopilot'"
}

test_workspace_label_copilot_home_uses_marker_id() {
  local home
  home="$TMP_ROOT/copilot-home"; mkdir -p "$home"
  printf 'sshhip-h7\n' > "$home/.ap-copilot-home"
  out=$( AP_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "copilot-sshhip-h7" ] || fail "a copilot home should resolve to 'copilot-<id>', got '$out'"
  pass "ap_backend_herdr_workspace_label: a copilot home (.ap-copilot-home) resolves to 'copilot-<id>'"
}

test_workspace_label_copilot_marker_trims_whitespace() {
  local home
  home="$TMP_ROOT/copilot-home-ws"; mkdir -p "$home"
  printf '  sshhip-h7  \n\n' > "$home/.ap-copilot-home"
  out=$( AP_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "copilot-sshhip-h7" ] || fail "the marker id should be trimmed of surrounding whitespace, got '$out'"
  pass "ap_backend_herdr_workspace_label: trims whitespace around the marker's copilot id"
}

test_workspace_label_empty_marker_falls_back_to_primary() {
  local home
  home="$TMP_ROOT/copilot-home-empty"; mkdir -p "$home"
  : > "$home/.ap-copilot-home"
  out=$( AP_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "autopilot" ] || fail "an empty/unreadable marker should fall back to 'autopilot', got '$out'"
  pass "ap_backend_herdr_workspace_label: an empty marker file falls back to the primary label 'autopilot'"
}

test_workspace_label_different_copilots_get_different_labels() {
  local home1 home2 out1 out2
  home1="$TMP_ROOT/copilot-a"; mkdir -p "$home1"; printf 'alpha-a1\n' > "$home1/.ap-copilot-home"
  home2="$TMP_ROOT/copilot-b"; mkdir -p "$home2"; printf 'bravo-b2\n' > "$home2/.ap-copilot-home"
  out1=$( AP_HOME="$home1" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  out2=$( AP_HOME="$home2" bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_label' "$ROOT" )
  [ "$out1" = "copilot-alpha-a1" ] || fail "copilot home1 label mismatch: $out1"
  [ "$out2" = "copilot-bravo-b2" ] || fail "copilot home2 label mismatch: $out2"
  [ "$out1" != "$out2" ] || fail "two different copilot homes must not collide on the same label"
  pass "ap_backend_herdr_workspace_label: two different copilot homes get two different, non-colliding labels"
}

# --- ap_backend_herdr_cli: session targeting (2026-07-02 incident fix) -------

test_cli_helper_sets_env_and_appends_trailing_session_flag() {
  local dir log resp fb
  dir="$TMP_ROOT/cli-helper"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_cli fmtest workspace list' "$ROOT"
  expect_code 0 $? "ap_backend_herdr_cli should succeed"
  assert_contains "$(cat "$log")" "HERDR_SESSION=fmtest"$'\x1f''workspace'$'\x1f''list' \
    "ap_backend_herdr_cli did not set the HERDR_SESSION env var"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''list'$'\x1f''--session'$'\x1f''fmtest' \
    "ap_backend_herdr_cli did not append a trailing --session <name> flag (the fix for the env-var-alone routing bug)"
  pass "ap_backend_herdr_cli: sets HERDR_SESSION AND appends a trailing --session flag on every call"
}
...
# every process it manages a pane for, so an autopilot or copilot agent's own
# tool calls carry the identity of the workspace the pilot is watching it in.
...
test_launcher_identity_absent_without_a_herdr_pane() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
  ...
  pass "ap_backend_herdr_launcher_identity: an autopilot not running inside herdr has no launcher workspace to inherit"
}
...
test_launcher_identity_absent_when_herdr_env_alone_is_set() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" HERDR_ENV=1 \
    \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
  expect_code 2 "$status" "HERDR_ENV=1 alone is a backend-selection marker, not a parent binding"
  pass "ap_backend_herdr_launcher_identity: HERDR_ENV=1 without a pane id selects the backend but binds no parent"
}
...
test_launcher_identity_resolves_the_exact_pane_tab_and_workspace() {
  ...
  printf '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/ap-herdr-unit/fmtest.sock"}]}\n' > "$resp/1.out"
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w7","label":"autopilot"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/ap-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest || exit 1
      printf "%s|%s|%s" "$AP_BACKEND_HERDR_LAUNCHER_PANE_ID" "$AP_BACKEND_HERDR_LAUNCHER_TAB_ID" "$AP_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID"' "$ROOT" )
  [ "$out" = 'w7:p3|w7:t3|w7' ] \
  ...
  pass "ap_backend_herdr_launcher_identity: resolves the launcher's exact workspace even when a same-labeled workspace sorts first"
}
...
test_launcher_identity_refuses_a_pane_from_another_session_name() {
  ...
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    ...
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_launcher_identity: refuses a launcher pane that names a different herdr session"
}
...
test_launcher_identity_refuses_a_missing_server_socket() {
  ...
  pass "ap_backend_herdr_launcher_identity: refuses a claimed pane without exact server identity"
}
...
test_launcher_identity_refuses_a_pane_from_another_server_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/ap-herdr-unit/other.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_launcher_identity: refuses a launcher pane whose injected socket belongs to another herdr server"
}
...
test_launcher_identity_refuses_an_unreadable_pane() {
  ...
  pass "ap_backend_herdr_launcher_identity: refuses when the launcher's own pane no longer resolves"
}
...
test_launcher_identity_refuses_a_pane_and_tab_that_disagree() {
  ...
  pass "ap_backend_herdr_launcher_identity: refuses when the launcher's pane and tab disagree about their workspace"
}
...
test_launcher_identity_refuses_a_workspace_missing_from_the_session() {
  ...
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/ap-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  ...
  pass "ap_backend_herdr_launcher_identity: refuses when the launcher's workspace is gone from its own session"
}
...
test_workspace_ensure_prefers_the_launcher_over_the_first_label_match() {
  ...
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/ap-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_ensure fmtest /tmp' "$ROOT" )
  ...
  pass "ap_backend_herdr_workspace_ensure: places a worker in the launcher's exact workspace, not the first same-labeled one"
}
...
test_workspace_ensure_refuses_an_ambiguous_label_with_no_launcher() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w7","label":"autopilot"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_ensure fmtest /tmp' "$ROOT" 2>&1 )
  status=$?
  expect_code 3 "$status" "two same-labeled home workspaces with no launcher identity must refuse"
  assert_contains "$out" "labeled 'autopilot'" "the ambiguity refusal did not name the duplicated label"
  ...
  pass "ap_backend_herdr_workspace_ensure: refuses to guess between two same-labeled home workspaces"
}
...
test_workspace_ensure_other_home_ignores_the_launcher_identity() {
  ...
  # --copilot launch stands up a different home's workspace by design.
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_ensure fmtest /tmp other-home' "$ROOT" )
  ...
  pass "ap_backend_herdr_workspace_ensure: a --copilot container resolves that home's own workspace, not the launcher's"
}
...
test_container_ensure_refuses_an_ambiguous_home_label() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /tmp' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "container_ensure must fail when the home workspace is ambiguous"
  assert_contains "$out" "labeled 'autopilot'" "container_ensure buried the specific ambiguity it refused"
  assert_not_contains "$out" "failed to ensure herdr workspace" "container_ensure added a generic message over the specific one"
  pass "ap_backend_herdr_container_ensure: surfaces the exact ambiguous-placement refusal instead of a generic failure"
}
...
test_container_ensure_starts_server_and_workspace() {
  ...
  # 5: workspace list -> empty (no "autopilot" workspace yet)
  ...
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"autopilot"},"tab":{"tab_id":"w1:t9"},"root_pane":{"pane_id":"w1:p9"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /tmp' "$ROOT" )
  ...
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''autopilot' \
    "container_ensure did not create the autopilot workspace with the given cwd"
  pass "ap_backend_herdr_container_ensure: version-gates, starts the server, ensures the autopilot workspace, echoes session:workspace_id + the seeded default tab id"
}
...
test_container_ensure_reuses_existing_workspace() {
  ...
  printf '{"client":{"version":"0.7.1","protocol":14}}\n' > "$resp/1.out"
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w9","label":"autopilot"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w9\t' ] || fail "container_ensure should reuse the existing autopilot workspace id with an EMPTY seeded-tab field (an ADOPTED workspace is never a prune candidate), got '$out'"
  assert_not_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create' "container_ensure should not create a workspace that already exists"
  pass "ap_backend_herdr_container_ensure: reuses an existing autopilot workspace without recreating it, and reports no seeded default tab (adopted, not created)"
}

test_create_task_refuses_duplicate_label() {
  local dir log resp fb out status
  dir="$TMP_ROOT/dup-task"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-dup1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_create_task: refuses a duplicate tab label (herdr's own tab create has no uniqueness check)"
}
...
# panes) across a server restart, including a reboot. A restored ap-<id> task
# tab comes back a HUSK - a dead pane, or a plain agent-less shell sitting in
# the saved cwd - never the flight crew member that used to be there. Before this fix,
...
test_create_task_refuses_duplicate_label_when_agent_live() {
  ...
  pass "ap_backend_herdr_create_task: a same-labeled tab with a live (even idle) registered agent still refuses exactly as before"
}
...
test_create_task_refuses_when_any_duplicate_label_is_live() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-mixed1","workspace_id":"w1"},{"tab_id":"w1:t3","label":"ap-mixed1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-mixed1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_create_task: scans every same-labeled tab and refuses if any duplicate is live"
}
...
test_create_task_closes_and_replaces_dead_pane_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-husk1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"ap-husk1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-husk1 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''ap-husk1' \
  ...
  pass "ap_backend_herdr_create_task: closes and replaces a same-labeled tab whose pane is dead (pane_not_found)"
}
...
test_create_task_closes_and_replaces_no_agent_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-husk2","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"ap-husk2","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-husk2 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''ap-husk2' \
    "create_task did not create the replacement tab"
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''close'$'\x1f''w1:t2' "create_task did not close the no-agent husk's tab"
  pass "ap_backend_herdr_create_task: closes and replaces a same-labeled tab whose pane is alive but hosts no registered agent (a restored plain shell)"
}
...
test_create_task_closes_all_duplicate_husks_after_replacement() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-husk-many","workspace_id":"w1"},{"tab_id":"w1:t3","label":"ap-husk-many","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/5.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t4","label":"ap-husk-many","workspace_id":"w1"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-husk-many /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  pass "ap_backend_herdr_create_task: closes every confirmed same-labeled husk only after creating the replacement"
}
...
test_create_task_refuses_when_preexisting_husk_tab_remains() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '1\n' > "$resp/6.exit"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-stale-husk","workspace_id":"w1"},{"tab_id":"w1:t3","label":"ap-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-stale-husk /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_create_task: refuses success when a preexisting husk tab remains after replacement"
}
...
test_create_task_refuses_when_agent_state_ambiguous() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-ambig1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-ambig1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_create_task: refuses (fail-safe) rather than guessing when the duplicate's agent state cannot be classified confidently"
}
...
test_create_task_husk_replacement_creates_before_closing() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"ap-order1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"error":{"code":"pane_not_found","message":"pane w1:p2 not found"}}\n' > "$resp/3.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/4.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"ap-order1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-order1 /tmp/proj' "$ROOT" ) \
  ...
  pass "ap_backend_herdr_create_task: creates the replacement tab BEFORE closing the husk tab, never the reverse"
}
...
test_create_task_creates_and_parses_ids() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task fmtest:w1 ap-newtask /tmp/proj' "$ROOT" )
  [ "$out" = "w1:t2 w1:p2" ] || fail "create_task should echo '<tab_id> <pane_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''ap-newtask' \
  ...
  pass "ap_backend_herdr_create_task: creates a tab and parses tab_id/pane_id from the JSON response, prunes nothing when no seeded tab id is given"
}
...
test_container_ensure_creates_with_no_focus_flag() {
  ...
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[]}}\n' > "$resp/3.out"
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"autopilot"},"tab":{"tab_id":"w1:t1"},"root_pane":{"pane_id":"w1:p1"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w1\tw1:t1' ] || fail "container_ensure should still echo '<session>:<workspace_id>\\t<seeded_default_tab_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''autopilot'$'\x1f''--no-focus' \
    "container_ensure's workspace create did not pass --no-focus (focus-safety: never steal the pilot's attention on spawn)"
  pass "ap_backend_herdr_container_ensure: workspace create passes --no-focus"
}

test_container_ensure_uses_copilot_home_label() {
  local dir log resp fb out home
  dir="$TMP_ROOT/container-copilot-label"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  home="$TMP_ROOT/container-copilot-home"; mkdir -p "$home"; printf 'sshhip-h7\n' > "$home/.ap-copilot-home"
  printf '{"client":{"version":"0.7.1","protocol":14}}\n' > "$resp/1.out"
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[]}}\n' > "$resp/3.out"
  printf '{"result":{"workspace":{"workspace_id":"w9","label":"copilot-sshhip-h7"},"tab":{"tab_id":"w9:t1"},"root_pane":{"pane_id":"w9:p1"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HOME="$home" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w9\tw9:t1' ] || fail "container_ensure did not echo the expected session:workspace_id + seeded default tab id, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''copilot-sshhip-h7' \
    "container_ensure did not create the workspace under this copilot home's own label"
  pass "ap_backend_herdr_container_ensure: creates the workspace under the COPILOT home's own label, not 'autopilot'"
}
...
test_create_task_creates_with_no_focus_flag() {
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''ap-newtask'$'\x1f''--no-focus' \
    "create_task's tab create did not pass --no-focus"
  pass "ap_backend_herdr_create_task: tab create passes --no-focus"
}
...
test_projection_journal_is_atomic_and_uses_128_bit_token() {
  ...
  out=$(bash -c '
    ...
    token=$(ap_backend_herdr_projection_journal_create "$1" task-p1) || exit 1
    parsed=$(ap_backend_herdr_projection_journal_token "$1/task-p1.herdr-presentation" task-p1) || exit 1
    printf "%s\n%s\n" "$token" "$parsed"
    ap_backend_herdr_projection_journal_create "$1" task-p1 >/dev/null 2>&1
  ' "$ROOT" "$state" 2>&1)
  status=$?
...
test_projection_journal_v2_binds_and_advances_exact_endpoint() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(ap_backend_herdr_projection_journal_create "$1" ap-hibit-r1) || exit 1
    journal="$1/ap-hibit-r1.herdr-presentation"
    home=$(ap_backend_herdr_projection_home_identity "$2") || exit 1
    label=$(ap_backend_herdr_projection_workspace_label ap-hibit-r1 "$token")
    ap_backend_herdr_projection_journal_bind \
      "$journal" ap-hibit-r1 "$home" lab-session w2 w2:t2 w2:p2 w1 autopilot "$label" ap-ap-hibit-r1 || exit 1
    ap_backend_herdr_projection_journal_snapshot "$journal" ap-hibit-r1 || exit 1
    printf "%s|%s|%s|%s|%s|%s|%s\n" \
      "$AP_BACKEND_HERDR_JOURNAL_VERSION" \
      "$AP_BACKEND_HERDR_JOURNAL_HOME" \
      "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" \
      "$AP_BACKEND_HERDR_JOURNAL_TAB_ID" \
      "$AP_BACKEND_HERDR_JOURNAL_PANE_ID" \
      "$AP_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" \
      "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL"
    ap_backend_herdr_projection_journal_replace_endpoint \
      "$journal" ap-hibit-r1 w2:t2 w2:p2 w2:t3 w2:p3 || exit 1
    ap_backend_herdr_projection_journal_snapshot "$journal" ap-hibit-r1 || exit 1
    printf "%s|%s\n" "$AP_BACKEND_HERDR_JOURNAL_TAB_ID" "$AP_BACKEND_HERDR_JOURNAL_PANE_ID"
  ' "$ROOT" "$state" "$home") || fail "version 2 projection journal binding failed"
  token=$(sed -n 's/^projection_id=//p' "$state/ap-hibit-r1.herdr-presentation")
  ...
  [ "$(wc -l < "$state/ap-hibit-r1.herdr-presentation" | tr -d '[:space:]')" = 12 ] \
    || fail "version 2 projection journal must have exactly 12 fields"
  printf 'pane_id=duplicate\n' >> "$state/ap-hibit-r1.herdr-presentation"
  if bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_journal_snapshot "$1" ap-hibit-r1' \
    "$ROOT" "$state/ap-hibit-r1.herdr-presentation"; then
...
test_projection_create_uses_exact_response_ids_and_leaves_one_task_pane() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"ap-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"ap-task-p2","workspace_id":"w9"}]}}\n' > "$resp/8.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"}]}}\n' > "$resp/9.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      ap_backend_herdr_projection_focus_snapshot() { printf "pilot-ws\tpilot-tab"; }
      ap_backend_herdr_projection_focus_restore() { return 0; }
      token=$(ap_backend_herdr_projection_journal_create "$1" task-p2) || exit 1
      label=$(ap_backend_herdr_projection_workspace_label task-p2 "$token")
      ap_backend_herdr_projection_create_task /tmp/proj "$label" ap-task-p2 || exit 1
      printf "%s %s %s %s %s\n" \
        "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
        "$AP_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
        "$AP_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID" \
        "$AP_BACKEND_HERDR_PROJECTION_TAB_ID" \
        "$AP_BACKEND_HERDR_PROJECTION_PANE_ID"
  ...
  token=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_journal_token "$1" task-p2' "$ROOT" "$journal") \
  ...
  assert_contains "$(cat "$log")" $'tab\x1fcreate\x1f--workspace\x1fw9\x1f--cwd\x1f/tmp/proj\x1f--label\x1fap-task-p2\x1f--no-focus' \
...
test_projection_create_never_closes_a_concurrent_same_label_tab() {
  ...
  printf '{"result":{"workspace":{"workspace_id":"w9"},"tab":{"tab_id":"w9:t1"},"root_pane":{"pane_id":"w9:p1"}}}\n' > "$resp/1.out"
  printf '{"result":{"tab":{"tab_id":"w9:t2"},"root_pane":{"pane_id":"w9:p2"}}}\n' > "$resp/2.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"ap-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"ap-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p1","tab_id":"w9:t1"},{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/4.out"
  printf '{"error":{"code":"agent_not_found"}}\n' > "$resp/5.out"
  printf '{"result":{"pane":{"pane_id":"w9:p1","tab_id":"w9:t1","workspace_id":"w9"}}}\n' > "$resp/6.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"ap-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"ap-task-p2","workspace_id":"w9"}]}}\n' > "$resp/8.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/9.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "pilot-ws\tpilot-tab"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_create_task /tmp/proj label ap-task-p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_focus_snapshot_requires_exact_workspace_and_tab() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot fmtest' "$ROOT") \
...
test_projection_close_restores_exact_prior_focus() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_close_pane_focus_preserving fmtest w9:p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_refuses_active_tab() {
  ...
  assert_contains "$out" "target is the pilot's active tab" \
  ...
  pass "herdr presentation focus: cleanup refuses rather than close the pilot's active tab"
}
...
test_projection_close_rechecks_required_agent_state_at_boundary() {
  ...
  out=$(ROOT="$ROOT" LOG="$log" bash -c '
    ...
    ap_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }
    ap_backend_herdr_pane_agent_state() { printf live; }
    ap_backend_herdr_cli() {
      printf "%s\n" "$*" >> "$LOG"
      case "$2 $3" in
    ...
    set +e
    ap_backend_herdr_projection_close_pane_focus_preserving fmtest w9:p2 no-agent
    rc=$?
    set -e
    printf "%s:%s" "$rc" "$AP_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE"
  ')
  status=${out%%:*}
...
test_projection_seeded_prune_refuses_active_tab() {
  ...
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9","focused":true},{"tab_id":"w9:t2","label":"ap-task","workspace_id":"w9","focused":false}]}}' > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_prune_seeded_default_tab fmtest w9 w9:t1 focus-preserving' "$ROOT" 2>&1)
  status=$?
  ...
  assert_not_contains "$(cat "$log")" $'pane\x1fclose' \
    "projected seeded prune closed the pilot's active tab"
...
test_projection_label_builder_uses_corner_and_strips_owner_prefixes() {
  local primary copilot token
  token='AbCdEfGhIjKlMnOpQrStUv'
  [ "${#token}" -eq 22 ] || fail "fixture token must be 22 characters"
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_workspace_label task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "primary child label was wrong: $primary"
  copilot=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_workspace_label copilot-child-demo '"$token" "$ROOT")
  [ "$copilot" = "└ copilot-child-demo · p:$token" ] \
    || fail "copilot child label was wrong: $copilot"
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_workspace_label autopilot/task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "autopilot/ owner prefix was not stripped: $primary"
  copilot=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_workspace_label copilot-fmdev-f2/child '"$token" "$ROOT")
  [ "$copilot" = "└ child · p:$token" ] \
    || fail "copilot owner prefix was not stripped: $copilot"
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_workspace_label ap-task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "presentation ap- owner prefix was not stripped: $primary"
  case "$primary" in $'└ '*) ;; *) fail "label must start with U+2514 and one space" ;; esac
  pass "herdr presentation labels: └ concise-task · p:<full-token> for primary and copilot children"
}
...
test_projection_order_moves_only_exact_new_workspace_and_preserves_relative_order() {
  ...
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":false},{"workspace_id":"w2","label":"autopilot/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w3","label":"copilot-alpha","focused":false},{"workspace_id":"w4","label":"copilot-bravo","focused":true},{"workspace_id":"w5","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  ...
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$AP_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"ap-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"autopilot","focused":false},{"workspace_id":"w2","label":"autopilot/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w5","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false},{"workspace_id":"w3","label":"copilot-alpha","focused":false},{"workspace_id":"w4","label":"copilot-bravo","focused":true}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t2"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_order_best_effort fmtest w5 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_copilot_parent_block() {
  local dir log resp fb mover mover_log out status
  dir="$TMP_ROOT/projection-order-copilot"; mkdir -p "$dir/responses"
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  # autopilot, primary child, copilot-A, A-child legacy, copilot-B, human, NEW for A
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"└ primary · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"copilot-alpha"},{"workspace_id":"w4","label":"copilot-alpha/old · p:AbCdEfGhIjKlMnOpQrStU1"},{"workspace_id":"w5","label":"copilot-bravo"},{"workspace_id":"wH","label":"human-notes"},{"workspace_id":"w6","label":"└ new-a · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$AP_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"ap-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"└ primary · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"copilot-alpha"},{"workspace_id":"w4","label":"copilot-alpha/old · p:AbCdEfGhIjKlMnOpQrStU1"},{"workspace_id":"w6","label":"└ new-a · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"w5","label":"copilot-bravo"},{"workspace_id":"wH","label":"human-notes"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "w5\tw5:t1"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_order_best_effort fmtest w6 copilot-alpha' "$ROOT" 2>&1)
  status=$?
  [ "$status" -eq 0 ] || fail "copilot parent ordering must not fail the spawn: $out"
  [ -z "$out" ] || fail "successful copilot ordering emitted a warning: $out"
  [ "$(cat "$mover_log")" = "$(cd /tmp && pwd -P)/fmtest.sock"$'\t'"w6"$'\t'"4" ] \
    || fail "copilot child was not inserted after its parent block: $(cat "$mover_log")"
  assert_not_contains "$(cat "$log")" $'workspace\x1frename' "copilot ordering renamed a legacy child"
  pass "herdr presentation ordering: copilot children append under their owning parent block"
}
...
test_projection_order_foreign_legacy_child_is_read_only() {
  ...
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"copilot-alpha"},{"workspace_id":"w3","label":"copilot-bravo/foreign · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w4","label":"└ new-alpha · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$AP_FAKE_MOVER_CALLED"
exit 0
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w4 copilot-alpha' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_allows_intervening_parent_child_block() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"autopilot/old · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"copilot-alpha"},{"workspace_id":"w4","label":"copilot-bravo"},{"workspace_id":"w5","label":"└ bravo-child · p:QqWwEeRrTtYyUuIiOoPpAa"},{"workspace_id":"w6","label":"└ new-first · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$AP_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"ap-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"autopilot/old · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w6","label":"└ new-first · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"w3","label":"copilot-alpha"},{"workspace_id":"w4","label":"copilot-bravo"},{"workspace_id":"w5","label":"└ bravo-child · p:QqWwEeRrTtYyUuIiOoPpAa"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t1"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_order_best_effort fmtest w6 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_human_spaces_never_move_targets() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"wH1","label":"notes"},{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"wH2","label":"scratch"},{"workspace_id":"w2","label":"copilot-alpha"},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$AP_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"ap-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"wH1","label":"notes"},{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"wH2","label":"scratch"},{"workspace_id":"w2","label":"copilot-alpha"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "w2\tw2:t1"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_order_best_effort fmtest w3 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_failure_warns_without_cleanup_or_spawn_failure() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":true},{"workspace_id":"w2","label":"copilot-alpha","focused":false},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016 # $defs is a literal JSON Schema key.
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
SH
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }; ap_backend_herdr_projection_focus_restore() { return 0; }; ap_backend_herdr_projection_order_best_effort fmtest w3 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_ambiguous_existing_block_is_read_only() {
  ...
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":true},{"workspace_id":"w2","label":"copilot-alpha","focused":false},{"workspace_id":"w3","label":"autopilot/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w4","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$AP_FAKE_MOVER_CALLED"
exit 0
SH
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w4 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_anchors_the_parent_by_exact_id() {
  ...
  layout='{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":false},{"workspace_id":"w7","label":"autopilot","focused":false},{"workspace_id":"wH","label":"human-notes","focused":false},{"workspace_id":"w8","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}'
...
SH
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w8 autopilot' "$ROOT" 2>&1)
  status=$?
  [ "$status" -eq 0 ] || fail "ambiguous projection ordering must not fail the spawn"
...
SH
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w8 autopilot w7' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_foreign_new_child_before_parent_is_read_only() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":true},{"workspace_id":"wH","label":"human-notes","focused":false},{"workspace_id":"w2","label":"└ foreign · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w3","label":"copilot-alpha","focused":false},{"workspace_id":"w4","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$AP_FAKE_MOVER_CALLED"
exit 0
...
test_projection_order_missing_parent_is_read_only() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$AP_FAKE_MOVER_CALLED"
exit 0
SH
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w2 copilot-missing' "$ROOT" 2>&1)
  status=$?
...
test_presentation_session_lock_path_is_shared_across_homes() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path_a=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
    || fail "session lock path resolution failed for home A"
  path_b=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
  ...
  case "$path_a" in
    /tmp/autopilot-herdr-presentation/order-*.lock) ;;
  ...
  path_other=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path other' "$ROOT") \
  ...
  if [ -L /tmp ] || [ "$(cd /tmp && pwd -P)" != /tmp ]; then
    : > /tmp/ap-herdr-lock-canon-$$.sock
    printf '%s\n' '{"sessions":[{"name":"canon","running":true,"socket_path":"/tmp/ap-herdr-lock-canon-'"$$"'.sock"}]}' > "$resp/4.out"
    printf '%s\n' "{\"sessions\":[{\"name\":\"canon\",\"running\":true,\"socket_path\":\"$(cd /tmp && pwd -P)/ap-herdr-lock-canon-$$.sock\"}]}" > "$resp/5.out"
    path_tmp=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
      || fail "lock path with /tmp socket failed"
    path_private=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
      || fail "lock path with canonical socket failed"
    rm -f /tmp/ap-herdr-lock-canon-$$.sock
...
test_presentation_session_lock_path_rejects_malformed_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_presentation_session_lock_path fmtest' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_rejects_malformed_socket() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"wH","label":"copilot-alpha"},{"workspace_id":"w2","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
...
SH
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_HERDR_SCRIPT_STATUS=1 \
    AP_BACKEND_HERDR_WORKSPACE_MOVER="$mover" AP_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_order_best_effort fmtest w2 autopilot' "$ROOT" 2>&1)
  status=$?
...
test_projection_reclaim_refusal_matrix_is_non_mutating() {
  ...
  token=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(ap_backend_herdr_projection_journal_create "$1" refusal-r1) || exit 1
    label=$(ap_backend_herdr_projection_workspace_label refusal-r1 "$token")
    ap_backend_herdr_projection_journal_bind \
      "$1/refusal-r1.herdr-presentation" refusal-r1 "$2" fmtest \
      w2 w2:t2 w2:p2 w1 autopilot "$label" ap-refusal-r1 || exit 1
    printf "%s" "$token"
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_journal_create "$1" refusal-r1 >/dev/null' \
  ...
  out=$(ROOT="$ROOT" JOURNAL="$journal" LEGACY="$legacy" HOME_A="$home" HOME_B="$other_home" MUTATIONS="$mutation_log" \
    bash -c '
      . "$ROOT/bin/backends/herdr.sh"
      ap_backend_herdr_cli() { printf "%s\n" "$*" >> "$MUTATIONS"; return 1; }
      run_case() {
        mode=$1; journal=$2; home=$3
        ap_backend_herdr_projection_live_binding_matches() {
          [ "$mode" != ambiguous ]
        }
        ap_backend_herdr_pane_agent_state() {
          case "$mode" in
            live) printf live ;;
        ...
        ap_backend_herdr_projection_focus_snapshot() {
          [ "$mode" != focus-unknown ] || return 1
          printf "w1\tw1:t1"
        }
        set +e
        ap_backend_herdr_projection_reclaim_task \
          fmtest "$journal" refusal-r1 "$home" w2 w2:t2 w2:p2 autopilot ap-refusal-r1 /tmp/project \
          >/dev/null 2>&1
        rc=$?
        set -e
        printf "%s:%s\n" "$mode" "$rc"
...
test_projection_reclaim_replaces_only_exact_husk_and_advances_binding() {
  ...
  token=$(bash -c '
    ...
    label=$(ap_backend_herdr_projection_workspace_label ap-hibit-r1 "$token")
    ap_backend_herdr_projection_journal_bind \
      "$1/ap-hibit-r1.herdr-presentation" ap-hibit-r1 "$2" fmtest \
      w2 w2:t2 w2:p2 w1 autopilot "$label" ap-ap-hibit-r1 || exit 1
    printf "%s" "$token"
  ' "$ROOT" "$state" "$home_real") || fail "could not create exact reclaim journal fixture"
  journal="$state/ap-hibit-r1.herdr-presentation"
  label="└ hibit-r1 · p:$token"
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w0\",\"label\":\"autopilot\",\"focused\":false,\"active_tab_id\":\"w0:t1\"},{\"workspace_id\":\"w1\",\"label\":\"autopilot\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/1.out"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t2","label":"ap-ap-hibit-r1"}]}}' > "$resp/2.out"
  ...
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w1\",\"label\":\"autopilot\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/6.out"
  ...
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t3","label":"ap-ap-hibit-r1"}]}}' > "$resp/25.out"
  ...
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      ap_backend_herdr_projection_reclaim_task \
        fmtest "$1" ap-hibit-r1 "$2" w2 w2:t2 w2:p2 autopilot ap-ap-hibit-r1 /tmp/project || exit 1
      printf "%s %s" "$AP_BACKEND_HERDR_PROJECTION_TAB_ID" "$AP_BACKEND_HERDR_PROJECTION_PANE_ID"
...
test_projection_recovery_is_read_only_and_refuses_live_duplicate_risk() {
  ...
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  token=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_journal_create "$1" task-p3' "$ROOT" "$state")
  journal="$state/task-p3.herdr-presentation"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot/task-p3 · p:%s"},{"workspace_id":"w2","label":"copy/task-p3 · p:%s"}]}}\n' "$token" "$token" > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_recovery_allows_flat fmtest "$1" task-p3' "$ROOT" "$journal" \
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot/task-p3 · p:%s"}]}}\n' "$token" > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p1","tab_id":"w1:t1"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p1"}}}\n' > "$resp/3.out"
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/4.out"
  out=$(PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_projection_recovery_allows_flat fmtest "$1" task-p3' "$ROOT" "$journal" 2>&1)
  status=$?
...
test_workspace_find_matches_only_this_homes_own_label() {
  ...
  # A workspace list carrying BOTH the primary's "autopilot" space and this
  # copilot's own "copilot-bravo-b2" space (as would be true once several
  # homes share one herdr session) - find must pick the one matching THIS
  # home's own label, never the primary's or a sibling copilot's.
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"copilot-bravo-b2"},{"workspace_id":"w3","label":"copilot-alpha-a1"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HOME="$home" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_workspace_find fmtest' "$ROOT" )
  [ "$out" = "w2" ] || fail "workspace_find should have matched this home's own label (copilot-bravo-b2 -> w2), got '$out'"
  pass "ap_backend_herdr_workspace_find: matches only THIS home's own label among several coexisting workspaces"
}
...
test_list_live_scoped_to_this_homes_workspace_only() {
  ...
  # 1: workspace_find's `workspace list` - two homes coexist, copilot's is w2
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot"},{"workspace_id":"w2","label":"copilot-bravo-b2"}]}}\n' > "$resp/1.out"
  # 2: tab list --workspace w2 (this copilot's own tabs only)
  printf '{"result":{"tabs":[{"tab_id":"w2:t1","label":"ap-copilottask"}]}}\n' > "$resp/2.out"
  # 3: pane_for_tab's `pane list --workspace w2`
  printf '{"result":{"panes":[{"pane_id":"w2:p1","tab_id":"w2:t1"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HOME="$home" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_list_live fmtest' "$ROOT" )
  [ "$out" = $'fmtest:w2:p1\tap-copilottask' ] || fail "list_live should report only this home's own tab, got '$out'"
  ...
  assert_not_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''list'$'\x1f''--workspace'$'\x1f''w1' \
    "list_live must never query the primary's (or a sibling copilot's) workspace"
  pass "ap_backend_herdr_list_live: scoped to this home's own workspace, never a sibling home's"
}
...
test_parse_target() {
  ( . "$ROOT/bin/backends/herdr.sh"
    ap_backend_herdr_parse_target "default:w1:p2" || exit 1
    [ "$AP_BACKEND_HERDR_SESSION" = default ] || { echo "session mismatch: $AP_BACKEND_HERDR_SESSION" >&2; exit 1; }
    [ "$AP_BACKEND_HERDR_PANE" = "w1:p2" ] || { echo "pane mismatch: $AP_BACKEND_HERDR_PANE" >&2; exit 1; }
  ) || fail "ap_backend_herdr_parse_target did not split session:pane on the first colon only"
  pass "ap_backend_herdr_parse_target: splits '<session>:<pane_id>' on the FIRST colon (pane_id itself contains one)"
}

test_normalize_key() {
  ( . "$ROOT/bin/backends/herdr.sh"
    [ "$(ap_backend_herdr_normalize_key Enter)" = enter ] || exit 1
    [ "$(ap_backend_herdr_normalize_key Escape)" = escape ] || exit 1
    [ "$(ap_backend_herdr_normalize_key C-c)" = ctrl+c ] || exit 1
    [ "$(ap_backend_herdr_normalize_key ctrl+c)" = ctrl+c ] || exit 1
  ) || fail "ap_backend_herdr_normalize_key did not map autopilot's key vocabulary to herdr's verified names"
  pass "ap_backend_herdr_normalize_key: Enter/Escape/C-c map to herdr's verified enter/escape/ctrl+c"
}
...
test_capture_calls_pane_read() {
  ...
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_capture default:w1:p2 250' "$ROOT" )
  ...
  pass "ap_backend_herdr_capture: calls 'pane read <pane> --source recent --lines N' with the session set"
}
...
test_capture_works_around_small_lines_bug() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_capture default:w1:p2 2' "$ROOT" )
  ...
  pass "ap_backend_herdr_capture: works around the verified small-N '--lines' bug by over-fetching and trimming locally"
}
...
test_capture_preserves_pane_read_failure() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_capture default:w1:p2 2' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_herdr_capture: ensures the session and preserves pane read failure"
}
...
test_send_key_normalizes_and_targets_pane() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_key default:w1:p2 Escape' "$ROOT"
  ...
  pass "ap_backend_herdr_send_key: normalizes the key and targets the right pane"
}
...
test_kill_is_best_effort() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_kill default:w1:p2' "$ROOT"
  ...
  pass "ap_backend_herdr_kill: calls pane close and stays best-effort on failure"
}
...
test_current_path_reads_cwd() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_current_path default:w1:p2' "$ROOT" )
  ...
  pass "ap_backend_herdr_current_path: reads pane foreground_cwd (the live running process), not the frozen creation-time cwd"
}
...
test_busy_state_working_maps_to_busy() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_busy_state default:w1:p2' "$ROOT" )
  ...
  pass "ap_backend_herdr_busy_state: working -> busy"
}
...
test_busy_state_done_and_blocked_map_to_idle() {
  ...
  pass "ap_backend_herdr_busy_state: done -> idle, blocked -> idle (surfaced like a stale pane, not suppressed as busy)"
}
...
test_busy_state_unknown_on_no_agent() {
  ...
  pass "ap_backend_herdr_busy_state: unparseable/absent agent state reports unknown, the regex-fallback cue"
}
...
test_composer_state_bare_prompt_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_composer_state default:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "a bare prompt glyph should read as empty, got '$out'"
  pass "ap_backend_herdr_composer_state: a bare '❯' composer row reads empty"
}
...
test_composer_state_ghost_placeholder_is_empty() {
  ...
  pass "ap_backend_herdr_composer_state: the ghost placeholder text reads empty, not pending"
}
...
test_composer_state_real_text_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: real composer text reads pending"
}
...
test_composer_state_popup_placeholder_fill_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: a slash-command popup's argument-hint placeholder still reads pending (the incident fix)"
}
...
test_composer_state_unknown_on_capture_failure() {
  ...
  pass "ap_backend_herdr_composer_state: reports unknown when the pane cannot be captured"
}
...
test_composer_state_unknown_when_no_composer_row_found() {
  ...
  pass "ap_backend_herdr_composer_state: reports unknown for bare shell prompts with no composer row"
}
...
test_composer_state_pi_separator_idle_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
  ...
  pass "ap_backend_herdr_composer_state: a native idle Pi separator composer reads empty"
}
...
test_composer_state_pi_separator_real_text_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: real Pi composer text remains pending"
}
...
test_composer_state_pi_incomplete_separator_below_stale_generic_is_unknown() {
  ...
  pass "ap_backend_herdr_composer_state: an incomplete lower Pi separator cannot inherit a stale empty row"
}
...
test_composer_state_pi_separator_requires_safe_native_identity() {
  ...
  pass "ap_backend_herdr_composer_state: Pi separators never authorize working, non-Pi, unreadable, or over-tall targets"
}
...
# bin/ap-supervise-daemon.sh's ap_backend_herdr_send_text_submit to never
...
test_composer_state_claude_unbordered_prompt_is_empty() {
  ...
  pass "ap_backend_herdr_composer_state: a real-claude unbordered '❯' prompt row (no border box in view) reads empty"
}
...
test_composer_state_claude_unbordered_prompt_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: a real-claude unbordered '❯ <text>' prompt row reads pending"
}
...
test_composer_state_claude_dim_prompt_suggestion_ghost_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_composer_state default:w1:p3' "$ROOT" )
  ...
  pass "ap_backend_herdr_composer_state: claude's dim prompt-suggestion ghost (the overnight wedge shape) reads empty"
}
...
test_composer_state_claude_dim_ghost_row_with_real_text_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: real typed text on the same claude prompt row still reads pending"
}
...
test_composer_state_grok_dark_truecolor_placeholder_is_empty() {
  ...
  pass "ap_backend_herdr_composer_state: grok's dark-truecolor placeholder (the TRUECOLOR gap) reads empty"
}
...
test_composer_state_grok_bright_truecolor_real_text_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: grok's real bright typed input still reads pending"
}
...
test_composer_state_codex_bare_prompt_glyph_is_empty() {
  ...
  pass "ap_backend_herdr_composer_state: a real-codex unbordered '›' prompt row reads empty"
}
...
test_composer_state_codex_faint_suggestion_is_empty() {
  ...
  pass "ap_backend_herdr_composer_state: a faint real-codex ghost suggestion reads empty"
}
...
test_composer_state_codex_non_faint_same_text_is_pending() {
  ...
  pass "ap_backend_herdr_composer_state: non-faint codex prompt text still reads pending"
}

# --- wait_for_working: the native agent-state poll-and-classify primitive ---
# Direct unit coverage for ap_backend_herdr_wait_for_working, the helper
# ap_backend_herdr_send_text_submit now uses instead of composer scraping
...
test_wait_for_working_returns_busy_on_first_poll() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_for_working default w1:p2 1 5' "$ROOT" )
  ...
  pass "ap_backend_herdr_wait_for_working: reports 'busy' immediately on the first poll, without spending the rest of the budget"
}
...
test_wait_for_working_catches_a_slow_transition_mid_window() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_for_working default w1:p2 0.03 3' "$ROOT" )
  ...
  pass "ap_backend_herdr_wait_for_working: a slow transition landing on a later sample within one window is still caught (robust against the 'slow transition' failure direction)"
}
...
test_wait_for_working_samples_budget_endpoint_without_final_sleep() {
  ...
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_SLEEP_LOG="$sleep_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$AP_SLEEP_LOG"; }; ap_backend_herdr_wait_for_working default w1:p2 0.5 6' "$ROOT" )
  ...
  pass "ap_backend_herdr_wait_for_working: spreads six samples across the full budget endpoint without a final trailing sleep"
}
...
test_send_text_submit_applies_herdr_minimum_confirm_budget() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/9.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_SLEEP_LOG="$sleep_log" AP_BACKEND_HERDR_SUBMIT_POLLS=6 AP_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6 \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$AP_SLEEP_LOG"; }; ap_backend_herdr_send_text_submit default:w1:p2 "hello pilot" 1 0.4 0' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: applies the herdr minimum confirmation budget before polling agent-state"
}
...
test_wait_for_working_returns_idle_when_never_busy_but_readable() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_for_working default w1:p2 0.02 2' "$ROOT" )
  [ "$out" = idle ] || fail "wait_for_working should report idle when the target was legibly read but never busy, got '$out'"
  pass "ap_backend_herdr_wait_for_working: reports 'idle' (readable, genuinely not yet working) when 'busy' never appears"
}
...
test_wait_for_working_returns_unknown_when_never_readable() {
  ...
  pass "ap_backend_herdr_wait_for_working: reports 'unknown' (a hard read failure, not a timing race) only when EVERY poll in the window fails"
}
...
test_wait_for_working_treats_blocked_as_submit_active() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_for_working default w1:p2 0.01 1' "$ROOT" )
  [ "$out" = busy ] || fail "wait_for_working should treat a post-Enter blocked state as submit-active, got '$out'"
  pass "ap_backend_herdr_wait_for_working: treats blocked as submit-active for confirmation without changing watcher busy-state semantics"
}
...
# AP_BACKEND_HERDR_SUBMIT_POLLS=1 pins most tests
...
test_send_text_submit_detects_landed_send() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "hello pilot" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty (submitted) once agent_status reports working, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''send-text'$'\x1f''w1:p2'$'\x1f''hello pilot' "send_text_submit did not type the literal text first"
  ...
  pass "ap_backend_herdr_send_text_submit: reports 'empty' once agent_status reports working after one Enter, without ever reading the composer"
}
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "hello pilot" 2 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: reports 'pending' when agent_status never reports working after retried Enters (swallowed)"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "/compact" 3 0.01 1.2' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: a slash-command popup's placeholder fill on Enter #1 never flips agent_status to working, so it does not short-circuit as submitted; Enter #2 is retried and lands it"
}
...
test_send_text_submit_confirms_blocked_after_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "needs approval" 3 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: a post-Enter blocked state confirms delivery without retrying into the prompt"
}
...
test_send_text_submit_preexisting_working_does_not_false_confirm_swallowed_enter() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/2.out"
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/3.out"
  printf '  \xe2\x9d\xaf hello pilot\n' > "$resp/4.out"
  printf '  \xe2\x9d\xaf hello pilot\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "hello pilot" 2 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: preexisting working is not accepted as submit proof when the composer still holds the message"
}
...
test_send_text_submit_confirms_despite_codex_idle_tip_composer() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "reply with just OK" 3 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: confirms submission via native agent-state alone, immune to a codex-style dynamic idle-tip composer that would have misread as 'pending' under the old composer-based confirmation"
}

# Companion regression for the pre-injection empty-box guard itself
# (bin/ap-supervise-daemon.sh's pane_input_pending): a real Codex idle
...
test_composer_state_codex_dynamic_idle_tip_reads_empty_when_faint() {
  ...
  pass "ap_backend_herdr_composer_state: a faint real-codex dynamic idle-tip composer row reads empty"
}

# Regression guard for the PRE-injection empty-box guard itself
# (bin/ap-supervise-daemon.sh's pane_input_pending, dispatched via
# ap_backend_composer_state -> ap_backend_herdr_composer_state): this task
...
test_composer_state_guard_still_refuses_real_pending_text_after_submit_confirmation_change() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/ap-backend.sh"; ap_backend_composer_state herdr default:w1:p2' "$ROOT" )
  ...
  pass "ap_backend_composer_state (herdr): the pre-injection empty-box guard still refuses a genuinely non-empty composer, unaffected by the submit-confirmation change"
}
...
test_send_text_submit_slow_transition_within_one_enter_needs_no_extra_enter() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=3 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "hello pilot" 3 0.03 0.01' "$ROOT" )
  ...
  pass "ap_backend_herdr_send_text_submit: a slow transition landing on a later sample within one Enter's budget is confirmed WITHOUT sending a needless extra Enter"
}
...
test_send_text_submit_send_failed() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" AP_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_send_text_submit default:w1:p2 "x" 2 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "send_text_submit should report send-failed when the literal send itself fails, got '$out'"
  pass "ap_backend_herdr_send_text_submit: reports 'send-failed' when the literal send-text call itself errors"
}
...
test_send_text_submit_unknown_on_capture_failure() {
  ...
  pass "ap_backend_herdr_send_text_submit: reports 'unknown' when the post-Enter agent-get read fails (never retries past an unreadable target)"
}

# --- ap-backend.sh dispatch wiring -------------------------------------------

test_dispatch_routes_herdr_backend() {
  ap_backend_validate herdr 2>/dev/null || fail "ap_backend_validate should accept herdr (P2 adds it to AP_BACKEND_KNOWN)"
  pass "ap_backend_validate: herdr is a known backend (P2)"
}

test_dispatch_busy_state_unknown_for_tmux() {
  # shellcheck source=bin/ap-backend.sh
  . "$ROOT/bin/ap-backend.sh"
  [ "$(ap_backend_busy_state tmux 'sess:win')" = unknown ] \
    || fail "ap_backend_busy_state should report unknown for tmux (no native agent-state primitive; watcher falls back to regex)"
  pass "ap_backend_busy_state: tmux (no native primitive) always reports unknown, preserving the P1 regex-only path"
}

test_dispatch_composer_state_routes_by_backend() {
  # ap_backend_composer_state (the generic per-backend composer/pending-input
  # classifier the away-mode daemon dispatches through - bin/ap-supervise-daemon.sh's
  ...
  # Sourced-guards are pre-set so ap_backend_source no-ops and these stubs are
  # never clobbered by the real per-backend files trying (and failing) a live call.
  (
    # shellcheck source=bin/ap-backend.sh
    . "$ROOT/bin/ap-backend.sh"
    _AP_BACKEND_TMUX_SOURCED=1
    _AP_BACKEND_HERDR_SOURCED=1
    _AP_BACKEND_ORCA_SOURCED=1
    _AP_BACKEND_ZELLIJ_SOURCED=1
    ap_tmux_composer_state() { [ "$1" = "sess:win" ] || fail "tmux composer_state got wrong target: $1"; printf 'pending'; }
    ap_backend_herdr_composer_state() { [ "$1" = "default:w1:p2" ] || fail "herdr composer_state got wrong target: $1"; printf 'empty'; }
    ap_backend_orca_composer_state() { [ "$1" = "term-1" ] || fail "orca composer_state got wrong target: $1"; printf 'empty'; }
    [ "$(ap_backend_composer_state tmux sess:win)" = pending ] || fail "composer_state did not dispatch to the tmux classifier"
    [ "$(ap_backend_composer_state herdr default:w1:p2)" = empty ] || fail "composer_state did not dispatch to the herdr classifier"
    [ "$(ap_backend_composer_state orca term-1)" = empty ] || fail "composer_state did not dispatch to the orca classifier"
    [ "$(ap_backend_composer_state zellij sess:win)" = unknown ] || fail "composer_state should report unknown for zellij (no named classifier yet)"
    [ "$(ap_backend_composer_state bogus x)" = unknown ] || fail "composer_state should report unknown for an unrecognized backend"
  ) || fail "composer_state dispatch subshell failed"
  pass "ap_backend_composer_state dispatches tmux/herdr/orca to their named classifiers, unknown for zellij/unrecognized backends"
}
...
test_scripts_route_explicit_target_through_meta_backend() {
  ...
  ap_write_meta "$state/herdr-stale.meta" "window=default:w1:p2" "backend=herdr"
...
SH
  ...
  out=$( PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    "$ROOT/bin/ap-peek.sh" default:w1:p2 5 2>/dev/null )
  [ "$out" = "captured herdr pane" ] || fail "ap-peek did not capture through herdr for an explicit metadata-matched target, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''read'$'\x1f''w1:p2' \
    "ap-peek did not route the explicit stale target through herdr capture"

  : > "$log"
  PATH="$fb:$PATH" AP_ROOT_OVERRIDE="$neutral" AP_HOME="$neutral" AP_STATE_OVERRIDE="$state" \
    AP_HERDR_LOG="$log" AP_HERDR_RESPONSES="$resp" \
    "$ROOT/bin/ap-send.sh" default:w1:p2 --key Escape >/dev/null 2>&1
  expect_code 0 $? "ap-send --key should route an explicit metadata-matched target through herdr"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''send-keys'$'\x1f''w1:p2'$'\x1f''escape' \
    "ap-send did not route the explicit stale target through herdr send-key"

  pass "ap-peek/ap-send: explicit stale targets matching metadata use the recorded backend"
}
...
test_workspace_ensure_prunes_default_tab() {
  ...
  raw=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /proj' "$ROOT" ) \
  ...
  ids=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "ap-prunetest" "$seeded" ) \
...
EOF
  ...
  jq -r --arg w "$wsid" '[.tabs[]|select(.workspace_id==$w)][0].label' "$state" | grep -qx 'ap-prunetest' \
  ...
  pass "ap_backend_herdr_create_task: prunes exactly the seeded default tab container_ensure identified, once the first real task tab exists"
}
...
test_repeated_cycles_reuse_one_workspace_no_orphans() {
  ...
  for i in 1 2 3; do
    ...
    ids=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "ap-cycle$i" "$seeded" ) \
      || fail "cycle $i: create_task failed"
    read -r _ pane <<EOF
$ids
EOF
    [ -n "$pane" ] || fail "cycle $i: create_task returned no pane id"
    PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_kill "$1"' "$ROOT" "fmtest:$pane" \
      || fail "cycle $i: kill failed"
  done
  # exactly one autopilot workspace survives three spawn/teardown cycles
  wscount=$(jq -r '[.workspaces[]|select(.label=="autopilot")]|length' "$state")
  [ "$wscount" = 1 ] || fail "expected exactly 1 autopilot workspace after 3 cycles, got $wscount: $(jq -c '.workspaces' "$state")"
  ...
  # zero tabs remain: every ap- task tab torn down AND the default tab pruned
  ...
  pass "herdr repeated spawn/teardown: one persistent autopilot workspace reused, zero orphans, default tab pruned, create ran once"
}

# --- created-vs-adopted default-tab-prune safety (2026-07-02 self-kill fix) -
#
# Root cause and fix are documented at
# ap_backend_herdr_workspace_prune_seeded_default_tab in bin/backends/herdr.sh
...
test_adopted_workspace_never_prunes_default_tab() {
  # An ADOPTED workspace (ap_backend_herdr_workspace_find matched a
  ...
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"autopilot"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{}}' > "$state"
  raw=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  ...
  ids=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "ap-adopttest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  pass "ap_backend_herdr_create_task: an ADOPTED workspace's pre-existing tab is never pruned (the created-vs-adopted gate)"
}

test_label_collision_startup_workspace_leaves_live_tab_alone() {
  # The exact live-fire incident shape (2026-07-02): a pilot launches herdr
  # directly inside a directory named "autopilot", so herdr auto-derives that
  # workspace's DISPLAYED label from the cwd basename - "autopilot" - byte-
  # identical to the primary autopilot home's own derived label, with no
  # --label ever passed and no autopilot involvement at all. That workspace's
  # single auto-created tab (label "1") holds the pilot's own live agent.
  # The very next flight crew member spawn must adopt-and-leave-alone, never prune.
  local dir log state fb raw container seeded ids pane
  dir="$TMP_ROOT/label-collision"; mkdir -p "$dir"; log="$dir/log"; state="$dir/state.json"; : > "$log"
  fb=$(make_herdr_statefake "$dir")
  # Mimic a bare `herdr workspace create --cwd <dir-named-autopilot>` (no
  ...
  # alone, from autopilot's own freshly-seeded default tab. Its pane hosts a
  # live agent (agent_status=working), exactly like the pilot's own pane.
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"autopilot"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{"w1:p1":"working"}}' > "$state"
  raw=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  seeded=${raw#*$'\t'}
  [ "$container" = "fmtest:w1" ] || fail "container_ensure should adopt the pilot's coincidentally-labeled workspace, got '$container'"
  ...
  ids=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "ap-collisiontest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  jq -e '.tabs[] | select(.tab_id == "w1:t1")' "$state" >/dev/null \
    || fail "REGRESSION: the pilot's live tab was closed - this is the exact 2026-07-02 self-kill incident"
  assert_not_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''close'$'\x1f''w1:p1' \
    "REGRESSION: create_task closed the pilot's live pane in the label-collision scenario"
  pass "ap_backend_herdr_create_task: the label-collision startup-workspace scenario (2026-07-02 incident) leaves the pilot's live tab untouched"
}
...
test_prune_refuses_a_working_agent_pane_defense_in_depth() {
  ...
  ids=$( PATH="$fb:$PATH" AP_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "ap-busytest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  pass "ap_backend_herdr_workspace_prune_seeded_default_tab: refuses to close the seeded default tab when its pane reports a working agent (defense in depth)"
}

# --- native event push: normalize / policy-routing / dedupe / wait ----------
#
# These exercise the herdr subscriber (ap_backend_herdr_wait_transition and its
...
# makes - `session list --json` (echoes one session, name AP_FAKE_SESSION_NAME,
# socket AP_FAKE_SOCKET), `status --json`, and `agent get <pane>` (per-pane
# status read from $AP_FAKE_AGENT_DIR/<key>.status, else agent_not_found).
...
case "$cmd $sub" in
  ...
  "session list")
    printf '{"sessions":[{"name":"%s","running":true,"default":false,"socket_path":"%s"}]}\n' \
      "${AP_FAKE_SESSION_NAME:-default}" "${AP_FAKE_SOCKET:-/tmp/ap-fake.sock}" ;;
  "agent get")
    if [ -n "${AP_FAKE_READER_READY_FILE:-}" ] && [ ! -e "$AP_FAKE_READER_READY_FILE" ]; then
      exit 9
    fi
    pane=${3:-}
    key=$(printf '%s' "$pane" | tr ':/.' '___')
    f="${AP_FAKE_AGENT_DIR:-/tmp}/$key.status"
    if [ -f "$f" ]; then
      printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$(cat "$f")"
    else
      printf '{"error":{"code":"agent_not_found"}}\n' >&2
...
# the socket, streams the TAB-separated lines in $AP_FAKE_READER_LINES to stdout
# (one projected event per line: pane_id\tworkspace_id\tagent_status\tagent),
# then exits $AP_FAKE_READER_EXIT (default 0). A non-zero exit with no lines
...
if [ -n "${AP_FAKE_READER_READY_FILE:-}" ]; then
  : > "$AP_FAKE_READER_READY_FILE"
fi
printf '%s\n' "${AP_FAKE_READER_ACK:-@subscribed}"
if [ -n "${AP_FAKE_READER_LINES:-}" ] && [ -f "$AP_FAKE_READER_LINES" ]; then
  cat "$AP_FAKE_READER_LINES"
fi
exit "${AP_FAKE_READER_EXIT:-0}"
SH
  chmod +x "$path"
  printf '%s\n' "$path"
}
...
test_normalize_event_leaves_from_empty() {
  local rec
  rec=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_normalize_event wG:pQ wG blocked claude' "$ROOT")
  [ "$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_pane_id "$1"' "$ROOT" "$rec")" = "wG:pQ" ] \
    || fail "normalize_event pane_id wrong: $rec"
  [ "$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_from_status "$1"' "$ROOT" "$rec")" = "" ] \
    || fail "normalize_event should leave from_status empty (herdr carries no previous status): $rec"
  [ "$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_to_status "$1"' "$ROOT" "$rec")" = "blocked" ] \
    || fail "normalize_event to_status wrong: $rec"
  pass "ap_backend_herdr_normalize_event routes through the shared record with an empty from_status"
}

test_escalation_marker_keys_like_watcher() {
  local m
  m=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_escalation_marker /st default:wG:pQ' "$ROOT")
  [ "$m" = "/st/.herdr-escalated-default_wG_pQ" ] \
    || fail "escalation marker key must match the watcher's tr ':/.' '___' scheme, got '$m'"
  pass "ap_backend_herdr_escalation_marker keys the dedupe marker exactly like the watcher's .stale-<key>"
}
...
test_apply_transition_blocked_requires_commit_to_dedupe() {
  ...
  rec=$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_record wG:pQ wG "" blocked claude' "$ROOT")
  marker="$state/.herdr-escalated-default_wG_pQ"
  out=$(bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"); rc=$?
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_commit_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"
  ...
  pass "ap_backend_herdr_apply_transition: blocked dedupe starts only after explicit commit"
}
...
test_apply_transition_working_clears_marker() {
  ...
  marker="$state/.herdr-escalated-default_wG_pQ"
  blocked=$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_record wG:pQ wG "" blocked claude' "$ROOT")
  working=$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_record wG:pQ wG "" working claude' "$ROOT")
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_commit_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$blocked"
  [ -e "$marker" ] || fail "setup: committed blocked edge should have set the marker"
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$working"; rc=$?
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$blocked" >/dev/null; rc=$?
  [ "$rc" = 0 ] || fail "a re-block after a working clear must re-fire (return 0), got $rc"
  pass "ap_backend_herdr_apply_transition: a working edge clears the marker so the next ->blocked re-escalates"
}
...
test_clear_transition_removes_task_marker() {
  ...
  marker="$state/.herdr-escalated-default_wG_pQ"
  : > "$marker"
  bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_clear_transition "$1" "$2"' "$ROOT" "$state" default:wG:pQ
  [ ! -e "$marker" ] || fail "clear_transition must remove the marker owned by a torn-down pane"
  pass "ap_backend_herdr_clear_transition removes task-owned dedupe state"
}
...
test_apply_transition_defer_and_fallback_are_noops() {
  ...
  for s in idle "done" unknown ""; do
    local rec
    rec=$(bash -c '. "$0/bin/ap-transition-lib.sh"; ap_transition_record wG:pQ wG "" "$1" claude' "$ROOT" "$s")
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"; rc=$?
  ...
  pass "ap_backend_herdr_apply_transition: idle/done (defer) and unknown/empty (fallback) take no fast action"
}

test_wait_transition_no_panes_returns_2() {
  local rc
  bash -c '. "$0/bin/backends/herdr.sh"; AP_BACKEND_HERDR_EVENTS_FORCE=1 ap_backend_herdr_wait_transition default 1 /tmp/st' "$ROOT"; rc=$?
  [ "$rc" = 2 ] || fail "wait_transition with no pane windows must return 2 (fall back to sleep), got $rc"
  pass "ap_backend_herdr_wait_transition: a home with no herdr panes falls back to polling (rc 2)"
}
...
test_wait_transition_not_capable_returns_2() {
  ...
  rc=$(PATH="$fb:$PATH" AP_BACKEND_HERDR_EVENTS_FORCE=0 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  [ "$rc" = 2 ] || fail "wait_transition must return 2 when events are below capability (fail closed to poll), got $rc"
  pass "ap_backend_herdr_wait_transition: below-capability protocol/schema falls back to polling (rc 2)"
}
...
test_wait_transition_reconcile_blocked_returns_record() {
  ...
  out=$(PATH="$fb:$PATH" TMPDIR="$temp" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ' "$ROOT" "$state"); rc=$?
  ...
  pass "ap_backend_herdr_wait_transition: reconnect level-reconcile returns an uncommitted blocked pane"
}
...
test_wait_transition_subscribes_before_reconcile() {
  ...
  rc=$(PATH="$fb:$PATH" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" AP_FAKE_READER_READY_FILE="$ready" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  [ "$rc" = 1 ] || fail "subscription must be acknowledged before reconciliation begins, got $rc"
  pass "ap_backend_herdr_wait_transition: subscribes before reconnect level-reconcile"
}
...
test_wait_transition_reconcile_dedupes_when_marked() {
  ...
  pass "ap_backend_herdr_wait_transition: a still-blocked, already-escalated pane is not re-delivered on reconnect"
}
...
test_wait_transition_stream_blocked_returns_record() {
  ...
  marker="$state/.herdr-escalated-sess_wG_pQ"
  out=$(PATH="$fb:$PATH" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 2 "$1" sess:wG:pQ' "$ROOT" "$state"); rc=$?
  ...
  pass "ap_backend_herdr_wait_transition: a streamed ->blocked edge returns the record sub-poll"
}
...
test_wait_transition_stream_absorb_clears_then_timeout() {
  ...
  rc=$(PATH="$fb:$PATH" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 2 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  ...
  pass "ap_backend_herdr_wait_transition: streamed working clears the marker, idle/done are deferred (clean timeout)"
}
...
test_wait_transition_reader_failure_returns_2() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  rc=$(PATH="$fb:$PATH" TMPDIR="$temp" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" AP_FAKE_READER_EXIT=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  ...
  pass "ap_backend_herdr_wait_transition: a reader/subscribe failure falls back to polling (rc 2)"
}
...
test_wait_transition_bad_ack_returns_2_and_cleans_up() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  result=$(PATH="$fb:$PATH" TMPDIR="$temp" AP_BACKEND_HERDR_EVENTS_FORCE=1 AP_FAKE_SESSION_NAME=sess AP_FAKE_SOCKET="$dir/x.sock" AP_FAKE_AGENT_DIR="$agent" \
    AP_BACKEND_HERDR_EVENT_READER="$reader" AP_FAKE_READER_LINES="$lines" AP_FAKE_READER_ACK=invalid \
    /bin/bash -c '. "$0/bin/backends/herdr.sh"; ap_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; rc=$?; [ -e /dev/fd/9 ] && fd_open=yes || fd_open=no; printf "%s %s\n" "$rc" "$fd_open"' "$ROOT" "$state")
  ...
  pass "ap_backend_herdr_wait_transition: Bash 3.2-safe bad-ack path closes fd 9 and removes its FIFO"
}
...
test_wait_transition_clean_timeout_returns_1() {
  ...
  pass "ap_backend_herdr_wait_transition: stock macOS Bash clean timeout closes fd 9 and returns 1"
}
