...
CHECK="$ROOT/bin/ap-doc-audience-check.sh"
INVENTORY="$ROOT/docs/documentation-audiences.json"
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ap-doc-audiences.XXXXXX")
...
test_repository_inventory_passes() {
  ...
  assert_contains "$out" "ap-doc-audience-check: ok surfaces=" \
...
Observed version 1.2.3 on branch `ap/example`.
MD
