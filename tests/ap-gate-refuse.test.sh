...
#
# A confused no-mistakes gate agent runs inside an autopilot checkout, adopts the
# pilot identity from AGENTS.md, and reaches for ap-spawn/ap-send/ap-teardown.
# bin/ap-gate-refuse-lib.sh is the autopilot capability-removal half: sourced at
...
# A normal autopilot session (real primary, real flight crew worktree) has NEITHER
...
#
# Finally, assert autopilot's TRACKED .no-mistakes.yaml parses and sets
...
# normal primary/flight crew checkout: its git-common-dir is <dir>/.git, never a gate.
make_normal_repo() {
  local dir=$1
...
run_guard_lib() {
  ...
  (
    cd "$cwd" || exit 111
    unset NO_MISTAKES_GATE AP_GATE_REFUSE_BYPASS
    case "$marker" in
...
test_helper_env_marker_refuses() {
  ...
  pass "ap-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set"
}
...
test_helper_empty_env_marker_refuses() {
  ...
  pass "ap-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set empty"
}
...
test_helper_path_backstop_refuses() {
  ...
  pass "ap-gate-refuse-lib: refuses from a gate worktree via git-common-dir (marker unset)"
}
...
test_helper_normal_is_noop() {
  ...
  pass "ap-gate-refuse-lib: no-op for a normal session (neither signal, set -eu clean)"
}

# --- ap-spawn ---------------------------------------------------------------

# A fake tmux/treehouse so ap-spawn resolves the flight crew worktree from a controlled
# pane path and completes without a live terminal (mirrors tests/ap-tangle-guard).
make_spawn_fakebin() {
  local dir=$1 fakebin
...
run_spawn() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u AP_GATE_REFUSE_BYPASS \
...
test_spawn_refuses_and_admits() {
  ...
  pass "ap-spawn: refuses on marker and gate-worktree backstop; a normal flight crew spawn is unaffected"
}

# --- ap-send ----------------------------------------------------------------

# A fake tmux that logs send-keys to AP_TMUX_LOG and reports live endpoints
# (mirrors tests/ap-send-strict), so a successful send is observable and a
...
run_send() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u AP_GATE_REFUSE_BYPASS \
      "PATH=$fakebin:$PATH" "AP_HOME=$home" "AP_ROOT_OVERRIDE=$home" \
      "AP_TMUX_LOG=$log" "AP_SEND_SETTLE=0" "$@" \
...
test_send_refuses_and_admits() {
  ...
  # env-marker refuse.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" ap-lane-ok "hello pilot" NO_MISTAKES_GATE=1); rc=$?
  ...
  : > "$log"
  out=$(run_send "$GATE_WT" "$home" "$fakebin" "$log" ap-lane-ok "hello pilot"); rc=$?
  ...
  # no-regression.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" ap-lane-ok "hello pilot"); rc=$?
  ...
  assert_contains "$(cat "$log")" "target=sess:ap-lane-ok literal=1 arg=hello pilot" "send: normal send should type the text"
  pass "ap-send: refuses on marker and gate-worktree backstop; a normal steer is unaffected"
}

# --- ap-teardown ------------------------------------------------------------

# make_teardown_case <name> -> echoes a case dir holding a LANDED no-mistakes flight
# task (HEAD reachable from origin), so a normal teardown genuinely succeeds and a
# refused one leaves the task untouched (mirrors tests/ap-teardown make_case).
make_teardown_case() {
...
SH
  ...
  ap_write_meta "$case_dir/state/task-x1.meta" \
    "window=autopilot:ap-task-x1" "endpoint_task_id=task-x1" \
    "worktree=$case_dir/wt" "project=$case_dir/project" \
    "kind=flight" "mode=no-mistakes"
...
run_teardown() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u AP_GATE_REFUSE_BYPASS \
      "AP_ROOT_OVERRIDE=$ROOT" "AP_STATE_OVERRIDE=$case_dir/state" \
      "AP_CONFIG_OVERRIDE=$case_dir/config" "PATH=$case_dir/fakebin:$PATH" "$@" \
...
test_teardown_refuses_and_admits() {
  ...
  pass "ap-teardown: refuses on marker and gate-worktree backstop; a normal teardown is unaffected"
}
