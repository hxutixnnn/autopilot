#!/usr/bin/env bash
# tests/ap-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH_ARM="$ROOT/bin/ap-watch-arm.sh"
DRAIN="$ROOT/bin/ap-wake-drain.sh"
LIB="$ROOT/bin/ap-wake-lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  i=0
  while [ "$i" -lt 50 ]; do
    live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  live=0
  lock_pid=
...
test_live_stale_watch_lock_is_actionable() {
  ...
  status=0
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=1 AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" 2> "$err" || status=$?
...
test_guard_warnings() {
  ...
  # AP_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  CLAUDECODE=1 PI_CODING_AGENT='' GROK_AGENT='' AP_ROOT_OVERRIDE="$dir" AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=1 "$ROOT/bin/ap-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  # Non-git AP_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  AP_ROOT_OVERRIDE="$dir" AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=300 "$ROOT/bin/ap-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_lock_single_winner_under_concurrency() {
  ...
  i=1
  while [ "$i" -le 40 ]; do
    AP_STATE_OVERRIDE="$state" bash -c '
      . "$1"
      if ap_lock_try_acquire "$2"; then
        printf "%s\n" "$$" >> "$3"
  ...
  pass "concurrent ap_lock_try_acquire yields exactly one winner"
}
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if ap_lock_try_acquire "$2"; then cat "$2/pid"; else exit 7; fi
  ' _ "$LIB" "$lockdir") || rc=$?
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    ap_lock_try_acquire "$2.steal" || exit 7
    printf "%s\n" "${BASHPID:-$$}" > "$3"
    sleep 2
    ap_lock_release "$2.steal"
  ...
  out=$(AP_LOCK_STALE_AFTER=0 AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if ap_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${AP_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if ap_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${AP_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via AP_LOCK_HELD_PID: $out" ;;
  esac
...
test_lock_late_claim_loses_after_recreate() {
  ...
  out=$(AP_LOCK_STALE_AFTER=0 AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner1=$(ap_lock_owner_dir "$2") || exit 20
    ln -s "$owner1" "$2" || exit 21
    touch -h -t 200001010000 "$2" 2>/dev/null || sleep 2
    if ! ap_lock_try_acquire "$2"; then exit 22; fi
    before=$(cat "$2/pid" 2>/dev/null || true)
    if ap_lock_claim "$2" "$owner1"; then late=won; else late=lost; fi
...
test_lock_paused_mid_acquire_claim_fails_during_steal() {
  ...
  out=$(AP_LOCK_STALE_AFTER=0 AP_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner=$(ap_lock_owner_dir "$2") || exit 20
    ln -s "$owner" "$2" || exit 21
    ap_lock_try_acquire "$2.steal" || exit 22
    steal_owner=${AP_LOCK_OWNER_DIR:-}
    if ap_lock_claim "$2" "$owner"; then late=won; else late=lost; fi
    if ap_lock_try_create "$2" "$steal_owner"; then stealer=won; else stealer=lost; fi
...
test_watch_restart_rejects_reused_pid() {
  ...
  printf '%s\n' "$dir" > "$state/.watch.lock/ap-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "stale watcher identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" AP_HOME="$dir" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  peer=$!
  identity=$(AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  touch "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" AP_HOME="$dir" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 AP_ARM_ATTACH_POLL=0.1 AP_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" --restart > "$out" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=0.2 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_self_eviction_is_loud_without_successor() {
  ...
  armout="$dir/arm.out"
  mark_pr_check_migration_complete "$state"
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=0.2 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 AP_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_attaches_and_waits_for_live_fresh_watcher() {
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_ARM_ATTACH_POLL=0.1 AP_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" AP_HOME="$dir" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
      grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
test_arm_hup_cleans_child_and_temp_output() {
  ...
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  PATH="$fakebin:$PATH" AP_HOME="$dir" AP_STATE_OVERRIDE="$state" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
SH
  ...
  AP_STATE_OVERRIDE="$state" "$ROOT/bin/ap-check-register.sh" task >/dev/null \
    || fail "could not register immediate-wake custom check"
  rc=0
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=0 AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=0 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" || rc=$?
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate arm wake failed"
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" AP_HOME="$dir" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 AP_ARM_CONFIRM_TIMEOUT=1 AP_ARM_ATTACH_POLL=0.1 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  touch -t 200001010000 "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 AP_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
SH
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=0 AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=0 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  first_arm=$!
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_WATCH_PREDECESSOR_ARM_PID="$first_arm" AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  successor_arm=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
  ...
  while [ "$iteration" -lt 6 ]; do
    ...
    PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_WATCH_CYCLE_LOG_MAX_BYTES=1400 AP_WATCH_CYCLE_LOG_KEEP_LINES=2 AP_POLL=5 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    successor_arm=$!
    i=0
    while [ "$i" -lt 80 ]; do
      grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
test_stopped_watcher_is_live_but_stale_then_exit_is_classified() {
  ...
  touch -t 200001010000 "$state/.last-watcher-beat"
  AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_alive "$2"' _ "$LIB" "$watcher_pid" \
    || fail "SIGSTOP watcher was not classified as a live pid"
  if AP_HOME="$dir" AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_watcher_healthy "$2" "$3" 300 "$4"' _ "$LIB" "$state" "$WATCH" "$dir"; then
...
test_pid_identity_is_locale_invariant() {
  ...
  # ap_pid_identity, so its output must be byte-identical regardless of the caller's
  ...
  # LC_ALL=C pin in ap_pid_identity therefore breaks the equality assertions below
...
SH
  chmod +x "$fakebin/ps"
  baseline=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" AP_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=C bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_all=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" AP_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=ko_KR.UTF-8 bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_time=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" AP_PROC_ROOT_OVERRIDE="$no_proc" LC_TIME=ko_KR.UTF-8 bash -c 'unset LC_ALL; . "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  ...
  if LC_ALL=C ps -p "$live" -o lstart= -o command= >/dev/null 2>&1; then
    real_first=$(AP_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=C bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
    real_second=$(AP_PROC_ROOT_OVERRIDE="$no_proc" LC_TIME=ko_KR.UTF-8 bash -c 'unset LC_ALL; . "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  fi
  kill "$live" 2>/dev/null || true
  wait "$live" 2>/dev/null || true
  [ -n "$baseline" ] || fail "ap_pid_identity produced no baseline identity under LC_ALL=C"
  [ "$via_lc_all" = "$baseline" ] || fail "ap_pid_identity varied with exported LC_ALL (got '$via_lc_all', want '$baseline')"
  [ "$via_lc_time" = "$baseline" ] || fail "ap_pid_identity varied with exported LC_TIME (got '$via_lc_time', want '$baseline')"
  while read -r observed; do
    [ "$observed" = C ] || fail "ap_pid_identity invoked ps without pinning LC_ALL=C (saw '$observed')"
  done < "$locale_log"
  if [ -n "$real_first" ]; then
    ...
    pass "ap_pid_identity real ps fallback is locale-invariant"
  else
    pass "real ps fallback locale check skipped where ps -o lstart= is unsupported"
  fi
  pass "ap_pid_identity is locale-invariant across LC_ALL/LC_TIME"
}
...
write_fake_proc_identity() {
  ...
  printf 'bash\0/path with spaces/ap-watch.sh\0--flag\0' > "$proc_root/$pid/cmdline"
}
...
test_proc_pid_identity_ignores_wall_clock_and_detects_pid_reuse() {
  ...
  before=$(AP_PROC_ROOT_OVERRIDE="$proc_root" AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$pid") \
    || fail "could not read initial fake Linux process identity"
  printf 'btime 1784094016\n' > "$proc_root/stat"
  after_time_jump=$(AP_PROC_ROOT_OVERRIDE="$proc_root" AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$pid") \
  ...
  [ "$before" = "$identity_key=987654 cmdline-hex=62617368002f706174682077697468207370616365732f61702d77617463682e7368002d2d666c616700" ] \
  ...
  after_pid_reuse=$(AP_PROC_ROOT_OVERRIDE="$proc_root" AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$pid") \
...
test_msys_pid_identity_uses_proc() {
  ...
  sleep 300 &
  live=$!
  identity=$(bash -c '. "$1"; ap_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  kill "$live" 2>/dev/null || true
  wait "$live" 2>/dev/null || true
  case "$identity" in
