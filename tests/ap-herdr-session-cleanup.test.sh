#!/usr/bin/env bash
# Focused safety tests for bin/ap-herdr-session-cleanup.sh.
...
TMP_ROOT=$(ap_test_tmproot ap-herdr-session-cleanup)
AP_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap ap_test_cleanup EXIT

export AP_HOME="$TMP_ROOT/home"
export AP_STATE_OVERRIDE="$AP_HOME/state"
export AP_CONFIG_OVERRIDE="$AP_HOME/config"
mkdir -p "$AP_STATE_OVERRIDE" "$AP_CONFIG_OVERRIDE"
touch "$AP_CONFIG_OVERRIDE/herdr-presentation-spaces"
printf '%s\n' herdr > "$AP_CONFIG_OVERRIDE/backend"
FAKEBIN=$(ap_fakebin "$TMP_ROOT")
ap_fake_exit0 "$FAKEBIN" herdr
export PATH="$FAKEBIN:$PATH"
export AP_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1
# shellcheck source=/dev/null
. "$ROOT/bin/ap-herdr-session-cleanup.sh"
unset AP_HERDR_SESSION_CLEANUP_SOURCE_ONLY

LINUX_PROCESS_INFO='{"result":{"process_info":{"foreground_processes":[{"argv":["/bin/sh"],"name":"sh","pid":67}]}}}'
[ "$(ap_herdr_cleanup_process_argv0 "$LINUX_PROCESS_INFO")" = /bin/sh ] \
  || fail "Linux Herdr process argv array was not accepted"
if ap_herdr_cleanup_process_argv0 \
...
ap_backend_name() { printf herdr; }
ap_backend_herdr_session() { printf test; }
ap_backend_herdr_presentation_session_lock_path() { printf '%s/presentation.lock' "$TMP_ROOT"; }
ap_lock_try_acquire() {
  printf '%s\n' "$1" >> "$LOCK_LOG"
  mkdir "$1" 2>/dev/null
}
ap_lock_release() { rm -rf -- "$1"; }
ap_herdr_cleanup_process_is_idle_shell() { [ ! -e "$FIXTURE_DIR/process-unsafe" ]; }
...
fixture_workspaces() {
  ...
  printf '[{"workspace_id":"w1","label":"autopilot","focused":%s,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},' \
...
fixture_tabs() {
  ...
  while [ "$i" -le "$count" ]; do
    [ "$i" -eq 1 ] || printf ','
    printf '{"tab_id":"%s:t%s","workspace_id":"%s","focused":false,"label":"ap-task"}' "$WS" "$i" "$WS"
    i=$((i + 1))
  done
  printf ']'
}
...
ap_backend_herdr_cli() {
  ...
  if [ -e "$FIXTURE_DIR/closed" ]; then
    case "$first $second" in
      ...
      "workspace list") printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1}]}}'; return 0 ;;
    esac
  fi
  if [ -e "$FIXTURE_DIR/race" ] && [ -e "$FIXTURE_DIR/snapshotted" ] && [ "$first $second" = "workspace list" ]; then
    printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"autopilot","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},{"workspace_id":"w2","label":"renamed","focused":false,"active_tab_id":"w2:t1","tab_count":1,"pane_count":1}]}}'
    return 0
  fi
  title=$(cat "$FIXTURE_DIR/title")
  tabs=$(cat "$FIXTURE_DIR/tabs")
...
write_v1() { # <id> [token]
  ...
  } > "$AP_STATE_OVERRIDE/$id.herdr-presentation"
}
...
reset_fixture() {
  rm -rf "$FIXTURE_DIR" "$TMP_ROOT"/*.lock "${AP_STATE_OVERRIDE:?}/"*
  mkdir -p "$FIXTURE_DIR"
  : > "$LOCK_LOG"; : > "$CLOSE_LOG"
...
assert_preserved() { # <case>
  ...
  [ -f "$AP_STATE_OVERRIDE/$ID.herdr-presentation" ] && had_journal=1
  ap_herdr_session_cleanup >/dev/null 2>&1
  if [ "$had_journal" -eq 1 ]; then
    [ -f "$AP_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "$name retired the journal"
  fi
...
reset_fixture
ap_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$AP_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "positive cleanup kept the journal"
[ "$(wc -l < "$CLOSE_LOG" | tr -d ' ')" = 1 ] || fail "positive cleanup did not close exactly once"
[ "$(sed -n '1p' "$LOCK_LOG")" = "$AP_STATE_OVERRIDE/.spawn-$ID.lock" ] || fail "task lock was not acquired first"
...
reset_fixture; printf 'version=1\ntask_id=%s\nprojection_id=short\n' "$ID" > "$AP_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "malformed journal"
...
reset_fixture; rm -f "$AP_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "zero journal match"
reset_fixture; write_v1 ap-task; assert_preserved "multiple journal matches"
reset_fixture; rm -f "$AP_STATE_OVERRIDE/$ID.herdr-presentation"; write_cross_home_v2; assert_preserved "cross-home journal"
reset_fixture; write_v2 "$AP_HOME" w9 "$TAB" "$PANE"; assert_preserved "v2 workspace binding mismatch"
reset_fixture; write_v2 "$AP_HOME" "$WS" w9:t1 "$PANE"; assert_preserved "v2 tab binding mismatch"
reset_fixture; write_v2 "$AP_HOME" "$WS" "$TAB" w9:p1; assert_preserved "v2 pane binding mismatch"
reset_fixture; write_v2 "$AP_HOME" "$WS" "$TAB" "$PANE"
ap_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$AP_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "matching v2 cleanup kept the journal"
...
reset_fixture; : > "$AP_STATE_OVERRIDE/$ID.meta"; assert_preserved "current task metadata"
...
cat > "$INTEGRATION_ROOT/bin/ap-herdr-session-cleanup.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "${AP_HOME:?}" >> "${AP_HERDR_CLEANUP_TRACE:?}"
SH
chmod +x "$INTEGRATION_ROOT/bin/ap-herdr-session-cleanup.sh"
printf '%s\n' manual > "$INTEGRATION_ROOT/home/config/backlog-backend"
AP_HOME="$INTEGRATION_ROOT/home" AP_HERDR_CLEANUP_TRACE="$TRACE" AP_BOOTSTRAP_DETECT_ONLY=1 \
  "$INTEGRATION_ROOT/bin/ap-bootstrap.sh" >/dev/null 2>&1
[ ! -e "$TRACE" ] || fail "detect-only bootstrap ran stale projection cleanup"
AP_HOME="$INTEGRATION_ROOT/home" AP_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/ap-bootstrap.sh" >/dev/null 2>&1
...
cat > "$INTEGRATION_ROOT/bin/ap-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'lock acquired'
SH
chmod +x "$INTEGRATION_ROOT/bin/ap-lock.sh"
AP_HOME="$INTEGRATION_ROOT/home" AP_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
  AP_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/ap-session-start.sh" >/dev/null 2>&1 \
...
cat > "$INTEGRATION_ROOT/bin/ap-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'error: another live autopilot session holds the lock' >&2
exit 1
SH
chmod +x "$INTEGRATION_ROOT/bin/ap-lock.sh"
AP_HOME="$INTEGRATION_ROOT/home" AP_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
...
printf 'all ap-herdr-session-cleanup tests passed\n'
