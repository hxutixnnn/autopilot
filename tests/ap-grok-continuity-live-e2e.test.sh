...
if [ "${AP_GROK_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set AP_GROK_LIVE_E2E=1 to run the interactive Grok continuity regression"
  exit 0
fi
...
"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env AP_HOME='$HOME_DIR' AP_ROOT_OVERRIDE='$PROJECT' AP_POLL=1 AP_SIGNAL_GRACE=0 AP_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$AP_HOME/state/.lock\"; grok --trust --always-approve --reasoning-effort low; rc=\$?; printf \"GROK_EXIT=%s\\n\" \"\$rc\"; sleep 300'"

wait_for_text "Grok Build" 180 || fail "Grok did not reach its ready composer"
sleep 1
# shellcheck disable=SC2016 # Backticks are literal prompt markup.
PROMPT='Use run_terminal_command with background=true to run exactly `bin/ap-watch-arm.sh`. Never use a shell ampersand. Once it reports started, respond briefly.'
