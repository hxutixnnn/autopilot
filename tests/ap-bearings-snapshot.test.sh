#!/usr/bin/env bash
# Behavior tests for the bearings projection wrapper over ap-fleet-snapshot.sh.
...
BEARINGS="$ROOT/bin/ap-bearings-snapshot.sh"
TMP_ROOT=$(ap_test_tmproot ap-bearings)
...
# makes no network call. gh returns one fixture open PR keyed to the flight task.
make_fakebin() {  # <dir>
  local fb
  fb=$(ap_fakebin "$1")
...
case "${1:-}" in
  ...
  capture-pane)
    case "$*" in
      *ap-domain-alpha*) printf 'stale terminal summary: Phase 7 started\n> \n' ;;
      *) printf 'all quiet\n> \n' ;;
    esac
    ;;
esac
...
if [ "${FAKE_GH_MANY:-0}" = 1 ]; then
  cat <<'JSON'
[{"number":1,"title":"One","url":"https://github.com/acme/repo/pull/1","headRefName":"ap/one","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":2,"title":"Two","url":"https://github.com/acme/repo/pull/2","headRefName":"ap/two","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":3,"title":"Three","url":"https://github.com/acme/repo/pull/3","headRefName":"ap/three","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]}]
JSON
  exit 0
fi
cat <<'JSON'
[{"number":9,"title":"Flight the thing","url":"https://github.com/hxutixnnn/autopilot/pull/9","headRefName":"ap/flight-task","reviewDecision":"APPROVED","mergeable":"MERGEABLE","statusCheckRollup":[{"conclusion":"SUCCESS","status":"COMPLETED"}]}]
JSON
SH
  cat > "$fb/gh-axi" <<'SH'
#!/usr/bin/env bash
...
record_claude_state() {  # <state-dir> <id> <busy|idle>
  ...
  "$ROOT/bin/ap-busy-event.sh" apply "$state" "$id" "$semantic_state" --gen "$gen" \
    --source claude-hook --event "$event"
}

fixture_copilot_home() {  # <parent-home>
  printf '%s/%s-copilot-home\n' "$TMP_ROOT" "$(basename "$1")"
}

# Standard fixture: a flight task with a recorded PR, a recon task with a report, a
# copilot with a MASKED open decision (needs-decision then a later unrelated
# done), and a backlog with a superseded queued item.
write_fixture() {  # <home>
  local home=$1 copilot
  copilot=$(fixture_copilot_home "$home")
  mkdir -p "$home/projects/flight-wt" "$home/data/recon-x" "$copilot/data" "$copilot/state" "$copilot/config" "$copilot/projects" "$copilot/bin"
  printf '# Autopilot fixture\n' > "$copilot/AGENTS.md"
  printf 'copilot\n' > "$copilot/.ap-copilot-home"
  printf -- '- copilot - fixture domain (home: %s; scope: fixture work; projects: autopilot; added 2026-07-11)\n' \
    "$copilot" > "$home/data/copilots.md"
  cat > "$home/data/backlog.md" <<EOF
## In flight
- [ ] flight-task - Flight the thing (repo: autopilot) (kind: flight) (since 2026-07-11)
- [ ] recon-x - Investigate the thing data/recon-x/report.md (repo: autopilot) (kind: recon) (since 2026-07-11)

## Queued
- [ ] live-gate - Real queued work blocked-by: flight-task (repo: autopilot) (kind: flight)
- [ ] dead-gate - Old conditional work (repo: autopilot) (kind: recon)
  NOT REQUIRED - superseded 2026-07-11; kept as reference only.

## Done
- [x] done-a - Landed thing https://github.com/hxutixnnn/autopilot/pull/7 (repo: autopilot) (kind: flight) (merged 2026-07-10)
EOF
  printf '# Recon report\n' > "$home/data/recon-x/report.md"
  ap_write_meta "$home/state/flight-task.meta" \
    "window=autopilot:ap-flight-task" \
    "worktree=$home/projects/flight-wt" \
    "project=autopilot" \
    "harness=claude" \
    "kind=flight" \
    "mode=no-mistakes" \
    "pr=https://github.com/hxutixnnn/autopilot/pull/9"
  record_claude_state "$home/state" flight-task busy
  printf 'working: building the thing\n' > "$home/state/flight-task.status"
  ap_write_meta "$home/state/recon-x.meta" \
    "window=autopilot:ap-recon-x" \
    "worktree=$home/projects/flight-wt" \
    "project=autopilot" \
    "harness=claude" \
    "kind=recon" \
    "mode=recon"
  record_claude_state "$home/state" recon-x idle
  printf 'done: report ready\n' > "$home/state/recon-x.status"
  ap_write_meta "$home/state/copilot.meta" \
    "window=autopilot:ap-copilot" \
    "worktree=$copilot" \
    "project=$copilot" \
    "harness=codex" \
    "kind=copilot" \
    ...
    "home=$copilot" \
    "projects=autopilot"
  printf 'needs-decision [key=race]: pick subscribe order\n' > "$home/state/copilot.status"
  printf 'done: an unrelated subtask finished\n' >> "$home/state/copilot.status"
  ap_write_meta "$home/state/external-wait.meta" \
    "window=autopilot:ap-external-wait" \
    "worktree=$home/projects/flight-wt" \
    "project=autopilot" \
    "harness=claude" \
    "kind=flight" \
  ...
  # The copilot's OWN home backlog records a merge it managed. This lands in the
  # copilot home, never the main backlog, so landed-work views only see it via the
  # bounded cross-home Done roll-up.
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight
- [ ] copilot - Decide subscription order (repo: autopilot) (kind: flight) (since 2026-07-11)

## Queued
- [ ] copilot-decision-race - Choose subscription order (repo: autopilot) (kind: pilot) (hold: pilot choice pending) (hold-kind: external)

## Done
- [x] copilot-landed - Copilot-managed fix https://github.com/hxutixnnn/autopilot/pull/50 (repo: autopilot) (kind: flight) (merged 2026-07-11)
EOF
  mkdir -p "$copilot/projects/copilot"
  ap_write_meta "$copilot/state/copilot.meta" \
    "window=autopilot:ap-copilot" "worktree=$copilot/projects/copilot" "project=autopilot" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  record_claude_state "$copilot/state" copilot idle
  printf 'needs-decision [key=race]: pick subscribe order\n' > "$copilot/state/copilot.status"
}

run() {  # <home> <fakebin> <args...>
  local home=$1 fakebin=$2; shift 2
  PATH="$fakebin:$PATH" AP_HOME="$home" AP_BEARINGS_NOW=2026-07-11T18:00:00Z NET_LOG="$home/net.log" "$BEARINGS" "$@"
}
...
write_domain_alpha_fixture() {  # <parent-home> <copilot-home>
  local home=$1 copilot=$2 i
  mkdir -p "$copilot/state" "$copilot/data" "$copilot/config" "$copilot/projects" "$copilot/bin"
  printf '# Autopilot fixture\n' > "$copilot/AGENTS.md"
  printf 'domain-alpha\n' > "$copilot/.ap-copilot-home"
  printf -- '- domain-alpha - sample rollout (home: %s; scope: sample rollout and legal release; projects: sample; added 2026-07-13)\n' \
    "$copilot" > "$home/data/copilots.md"
  ap_write_copilot_meta "$home/state/domain-alpha.meta" "$copilot" "autopilot:ap-domain-alpha" sample
  printf 'working [key=phase7]: Phase 7 started\n' > "$home/state/domain-alpha.status"
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight

## Queued
- [ ] legal-release - Release approval blocked-by: external-legal - external legal dependency (repo: sample) (kind: flight)

## Done
EOF
  i=1
  while [ "$i" -le 7 ]; do
    printf -- '- [x] phase%s - Sample rollout Phase %s (repo: sample) (kind: flight) (done 2026-07-%02d)\n' \
      "$i" "$i" "$i" >> "$copilot/data/backlog.md"
    i=$((i + 1))
  done
}
...
test_domain_alpha_stale_parent_event_does_not_become_current_work() {
  local home copilot fakebin json canonical
  home=$(make_home domain-alpha-parent)
  copilot="$TMP_ROOT/domain-alpha-home"
  write_domain_alpha_fixture "$home" "$copilot"
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_NOW_EPOCH=1783792800 AP_SNAPSHOT_TERMINAL_LINES=2 AP_SNAPSHOT_TERMINAL_BYTES=64 \
    NET_LOG="$home/net.log" FAKE_GH_FAIL=1 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "domain-alpha")
...
test_gnu_stat_uses_file_formats_without_bsd_fallback_pollution() {
  local home copilot fakebin canonical stat_log
  home=$(make_home gnu-stat-parent)
  copilot="$TMP_ROOT/gnu-stat-home"
  write_domain_alpha_fixture "$home" "$copilot"
...
SH
  ...
  canonical=$(PATH="$fakebin:$PATH" STAT_LOG="$stat_log" AP_HOME="$home" \
    AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z AP_SNAPSHOT_NOW_EPOCH=1783792800 \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "domain-alpha")
    | .provenance.selected == "structured-home"
      and .parent_event.activity_scan.available == true
  ' >/dev/null || fail "GNU stat fixture corrupted the authoritative copilot summary: $canonical"
...
test_parent_activity_evidence_is_bounded_and_disclosed() {
  local home copilot fakebin canonical json i
  home=$(make_home bounded-parent-activity)
  copilot="$TMP_ROOT/bounded-parent-activity-home"
  write_domain_alpha_fixture "$home" "$copilot"
  : > "$home/state/domain-alpha.status"
  i=1
  while [ "$i" -le 6 ]; do
  ...
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_PARENT_ACTIVITY_LINES=4 AP_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    AP_SNAPSHOT_PARENT_ACTIVITIES=2 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "domain-alpha")
  ...
  json=$(AP_SNAPSHOT_PARENT_ACTIVITY_LINES=4 AP_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    AP_SNAPSHOT_PARENT_ACTIVITIES=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    .omitted | any(.surface == "copilot parent activity evidence truncated for 1 record(s)")
...
test_active_child_overrides_old_parent_event() {
  ...
  copilot="$TMP_ROOT/active-child-home"
  write_domain_alpha_fixture "$home" "$copilot"
  mkdir -p "$copilot/projects/phase8"
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight
- [ ] phase8 - Sample rollout Phase 8 (repo: sample) (kind: flight) (since 2026-07-13)

## Queued

## Done
- [x] phase7 - Sample rollout Phase 7 (repo: sample) (kind: flight) (done 2026-07-12)
EOF
  ap_write_meta "$copilot/state/phase8.meta" \
    "window=autopilot:ap-phase8" "worktree=$copilot/projects/phase8" "project=sample" \
    "harness=codex" "kind=flight" "mode=no-mistakes"
  printf 'working [key=phase8]: implementing Phase 8 parity\nneeds-decision [key=release]: choose release A or B\n' \
    > "$copilot/state/phase8.status"
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.copilots | any(.[]; .id == "domain-alpha" and .state != "pilot_decision"
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "domain-alpha") | .endpoints[] | select(.id == "phase8")
...
test_structured_child_decision_reaches_pilots_call() {
  local home copilot fakebin json
  home=$(make_home child-decision-parent)
  copilot="$TMP_ROOT/child-decision-home"
  write_domain_alpha_fixture "$home" "$copilot"
  mkdir -p "$copilot/projects/phase8"
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight
...
## Queued
- [ ] phase8-decision-release - Choose sample release (repo: sample) (kind: pilot) (hold: pilot release choice pending) (hold-kind: external)
...
EOF
  ap_write_meta "$copilot/state/phase8.meta" \
    "window=autopilot:ap-phase8" "worktree=$copilot/projects/phase8" "project=sample" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  record_claude_state "$copilot/state" phase8 idle
  ...
  printf '%s' "$json" | jq -e '
    (.copilots | any(.[]; .id == "domain-alpha" and .state == "pilot_decision"))
      and (.decisions_open | any(.[]; .id == "domain-alpha/phase8-decision-release"
        and .key == "phase8-decision-release" and .verb == "pilot-hold"))
      and (.in_flight | any(.[]; .id == "domain-alpha") | not)
  ' >/dev/null || fail "structured child pilot hold did not reach Pilot Call: $json"
  pass "a structured child pilot hold reaches Pilot's Call"
}

make_valid_copilot_home() {  # <id> <home>
  local id=$1 home=$2
...
append_copilot_registry() {  # <parent> <id> <home>
  printf -- '- %s - fixture domain (home: %s; scope: fixture; projects: sample; added 2026-07-13)\n' \
    "$2" "$3" >> "$1/data/copilots.md"
}

append_landed_row() {  # <copilot-home> <id> <title> <date>
  printf -- '- [x] %s - %s (repo: autopilot) (kind: flight) (merged %s)\n' \
    "$2" "$3" "$4" >> "$1/data/backlog.md"
}

make_landed_copilot() {  # <parent> <id>
  local parent=$1 id=$2 copilot
  copilot="$TMP_ROOT/$(basename "$parent")-$id-home"
  make_valid_copilot_home "$id" "$copilot"
  append_copilot_registry "$parent" "$id" "$copilot"
  printf '%s\n' "$copilot"
}

write_parent_copilot_event() {  # <parent> <id> <home> <note>
  ap_write_copilot_meta "$1/state/$2.meta" "$3" "autopilot:ap-$2" sample
  printf 'working [key=%s]: %s\n' "$2" "$4" > "$1/state/$2.status"
}

test_bad_copilot_homes_never_revive_parent_work() {
  local home fakebin missing invalid unreadable malformed timedout wt json
  home=$(make_home bad-homes)
  : > "$home/data/copilots.md"
  ...
  append_copilot_registry "$home" missing "$missing"

  make_valid_copilot_home invalid "$invalid"
  printf 'someone-else\n' > "$invalid/.ap-copilot-home"
  append_copilot_registry "$home" invalid "$invalid"
  write_parent_copilot_event "$home" invalid "$invalid" "old invalid work"

  make_valid_copilot_home unreadable "$unreadable"
  chmod 000 "$unreadable/data"
  append_copilot_registry "$home" unreadable "$unreadable"
  write_parent_copilot_event "$home" unreadable "$unreadable" "old unreadable work"

  make_valid_copilot_home malformed "$malformed"
  printf '## In flight\nthis current row is not structured\n' > "$malformed/data/backlog.md"
  append_copilot_registry "$home" malformed "$malformed"
  write_parent_copilot_event "$home" malformed "$malformed" "old malformed work"

  make_valid_copilot_home timedout "$timedout"
  wt="$timedout/projects/slow"
  ap_git_init_commit "$wt"
  git -C "$wt" checkout -q -b ap/slow
  printf '## In flight\n- [ ] slow - Slow child (repo: sample) (kind: flight) (since 2026-07-13)\n\n## Queued\n\n## Done\n' > "$timedout/data/backlog.md"
  ap_write_meta "$timedout/state/slow.meta" \
    "window=autopilot:ap-slow" "worktree=$wt" "project=sample" \
    "harness=codex" "kind=flight" "mode=no-mistakes"
  append_copilot_registry "$home" timedout "$timedout"
  write_parent_copilot_event "$home" timedout "$timedout" "old timed work"

  fakebin=$(make_fakebin "$home")
  json=$(FAKE_NM_SLEEP=1 AP_SNAPSHOT_COPILOT_TIMEOUT=1 run "$home" "$fakebin" --json)
  chmod 700 "$unreadable/data"
  printf '%s' "$json" | jq -e '
    (.copilots | length) == 5
      and all(.copilots[]; .state == "unknown")
      and (.in_flight | map(.id) | all(. != "invalid" and . != "unreadable" and . != "malformed" and . != "timedout"))
      and (.copilots | any(.[]; .id == "missing" and .provenance == "unknown"
        and .freshness == "unknown" and (.reason | contains("invalid home"))))
      and ([.copilots[] | select(.id != "missing")]
        | all(.provenance == "parent-event-fallback" and .freshness == "historical-event"))
      and (.copilots | any(.[]; .id == "invalid" and (.reason | contains("marked for"))))
      and (.copilots | any(.[]; .id == "unreadable" and (.reason | test("invalid home|unreadable"))))
      and (.copilots | any(.[]; .id == "malformed" and (.reason | contains("unstructured current backlog row"))))
      and (.copilots | any(.[]; .id == "timedout" and (.reason | contains("timed out"))))
...
test_oversized_copilot_summary_stays_strict_unknown() {
  local home copilot fakebin json i
  home=$(make_home oversized-home)
  copilot="$TMP_ROOT/oversized-copilot-home"
  make_valid_copilot_home oversized "$copilot"
  append_copilot_registry "$home" oversized "$copilot"
  ap_write_copilot_meta "$home/state/oversized.meta" "$copilot" "autopilot:ap-oversized" sample
...
EOF
  i=1
  while [ "$i" -le 30 ]; do
    printf -- '- [x] landed-%02d - Bounded landed fixture %02d (repo: sample) (kind: flight) (done 2026-07-01)\n' \
      "$i" "$i" >> "$copilot/data/backlog.md"
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(AP_SNAPSHOT_COPILOT_MAX_BYTES=512 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.copilots | any(.id == "oversized" and .state == "unknown"
  ...
  pass "an oversized copilot summary retains the strict empty unknown fallback"
}

test_copilot_and_child_bounds_are_disclosed() {
  local home fakebin id copilot child json expanded canonical i
  home=$(make_home copilot-bounds)
  : > "$home/data/copilots.md"
  for id in a b c; do
    copilot="$TMP_ROOT/bounds-$id"
    make_valid_copilot_home "$id" "$copilot"
    append_copilot_registry "$home" "$id" "$copilot"
  done
  copilot="$TMP_ROOT/bounds-a"
  : > "$copilot/data/backlog.md"
  printf '## In flight\n' >> "$copilot/data/backlog.md"
  i=1
  while [ "$i" -le 3 ]; do
    child="child-$i"
    mkdir -p "$copilot/projects/$child"
    printf -- '- [ ] %s - Active %s (repo: sample) (kind: flight) (since 2026-07-13)\n' "$child" "$child" >> "$copilot/data/backlog.md"
    ap_write_meta "$copilot/state/$child.meta" \
      "window=autopilot:ap-$child" "worktree=$copilot/projects/$child" "project=sample" \
      "harness=claude" "kind=flight" "mode=no-mistakes"
    record_claude_state "$copilot/state" "$child" busy
    printf 'working [key=%s]: active child %s\n' "$child" "$i" > "$copilot/state/$child.status"
    i=$((i + 1))
  done
  printf '\n## Queued\n\n## Done\n' >> "$copilot/data/backlog.md"
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_COPILOTS=2 AP_SNAPSHOT_COPILOT_CHILDREN=2 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.total_registered == 3
      and .copilot_current.shown == 2
      and .copilot_current.truncated == 1
      and (.copilot_current.records[] | select(.id == "a")
        | .counts.active_children == 3 and (.active_children | length) == 2
          and (.omitted | any(.surface == "active_children" and .count == 1)))
      and (.copilot_current.records | any(.id == "b" and .current.state == "no_active_work"))
  ' >/dev/null || fail "canonical copilot or child bounds were not enforced: $canonical"
  json=$(AP_SNAPSHOT_COPILOTS=2 AP_SNAPSHOT_COPILOT_CHILDREN=2 AP_BEARINGS_COPILOTS=1 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.copilots | length) == 1
      and ([.omitted[].surface] | any(test("copilots showing 1 of 2")))
      and ([.omitted[].surface] | any(test("registered copilots omitted by snapshot bound: 1")))
  ' >/dev/null || fail "bearings copilot bound was not disclosed: $json"
  expanded=$(AP_SNAPSHOT_COPILOT_CHILDREN=2 AP_BEARINGS_COPILOTS=1 \
    run "$home" "$fakebin" --json --all-copilots)
  printf '%s' "$expanded" | jq -e '
    (.copilots | length) == 3
      and ([.omitted[].surface] | any(test("copilots showing|registered copilots omitted")) | not)
  ' >/dev/null || fail "--all-copilots did not expand the canonical and bearings bounds: $expanded"
  pass "copilot and per-home child counts are bounded, disclosed, and explicitly expandable"
}

test_parent_decision_is_untrusted_contradiction_only() {
  local home copilot fakebin canonical json
  home=$(make_home parent-decision-only)
  copilot="$TMP_ROOT/parent-decision-only-home"
  make_valid_copilot_home authority "$copilot"
  append_copilot_registry "$home" authority "$copilot"
  ap_write_copilot_meta "$home/state/authority.meta" "$copilot" "autopilot:ap-authority" sample
  printf 'needs-decision [key=stale]: old parent question\n' > "$home/state/authority.status"
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "authority")
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.copilots | any(.[]; .id == "authority" and .state == "no_active_work" and .contradiction == true))
...
test_parent_evidence_reconciles_by_verb_and_key() {
  local home hold blocked decision fakebin canonical copilot child
  ...
  make_valid_copilot_home hold "$hold"
  make_valid_copilot_home blocked "$blocked"
  make_valid_copilot_home decision "$decision"
  append_copilot_registry "$home" hold "$hold"
  append_copilot_registry "$home" blocked "$blocked"
  append_copilot_registry "$home" decision "$decision"
  ap_write_copilot_meta "$home/state/hold.meta" "$hold" "autopilot:ap-hold" sample
  ap_write_copilot_meta "$home/state/blocked.meta" "$blocked" "autopilot:ap-blocked" sample
  ap_write_copilot_meta "$home/state/decision.meta" "$decision" "autopilot:ap-decision" sample
...
## Queued
- [ ] legal-release - Legal release blocked-by: external-legal - legal review (repo: sample) (kind: flight)
...
## Queued
- [ ] vendor-release - Vendor release blocked-by: external-vendor - vendor review (repo: sample) (kind: flight)
...
## In flight
- [ ] $child - Decision child (repo: sample) (kind: flight) (since 2026-07-11)

## Queued

## Done
EOF
  ap_write_meta "$decision/state/$child.meta" \
    "window=autopilot:ap-$child" "worktree=$decision/projects/$child" "project=sample" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    (.copilot_current.records[] | select(.id == "hold")
      ...
      and (.copilot_current.records[] | select(.id == "blocked")
        | .current.state == "externally_held"
      ...
      and (.copilot_current.records[] | select(.id == "decision")
        | .current.state == "pilot_decision"
          and .contradiction == true
          and .terminal_evidence.captured == false
          and (.parent_event.reconciliation.activities
...
test_nonprogressing_child_states_are_explicit() {
  local home copilot fakebin canonical
  home=$(make_home child-state-classification)
  copilot="$TMP_ROOT/child-state-classification-home"
  make_valid_copilot_home states "$copilot"
  append_copilot_registry "$home" states "$copilot"
  mkdir -p "$copilot/projects/parked" "$copilot/projects/done" "$copilot/projects/failed"
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight
- [ ] parked - Parked child (repo: sample) (kind: flight) (since 2026-07-11)

## Queued

## Done
EOF
  ap_write_meta "$copilot/state/parked.meta" \
    "window=autopilot:ap-parked" "worktree=$copilot/projects/parked" "project=sample" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  record_claude_state "$copilot/state" parked idle
  printf 'needs-decision [key=parked]: choose a route\n' > "$copilot/state/parked.status"
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "states")
    | .current.state == "pilot_decision"
      and .active_children == []
...
EOF
  ...
  cat > "$copilot/data/backlog.md" <<'EOF'
## In flight
- [ ] done - Done child still in flight (repo: sample) (kind: flight) (since 2026-07-11)
- [ ] failed - Failed child still in flight (repo: sample) (kind: flight) (since 2026-07-11)

## Queued

## Done
EOF
  ap_write_meta "$copilot/state/done.meta" \
    "window=autopilot:ap-done" "worktree=$copilot/projects/done" "project=sample" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  ap_write_meta "$copilot/state/failed.meta" \
    "window=autopilot:ap-failed" "worktree=$copilot/projects/failed" "project=sample" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  record_claude_state "$copilot/state" "done" idle
  record_claude_state "$copilot/state" failed idle
  printf 'done: complete\n' > "$copilot/state/done.status"
  printf 'failed: stopped\n' > "$copilot/state/failed.status"
  rm "$copilot/state/parked.meta" "$copilot/state/parked.status"
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "states")
...
test_registry_unavailability_and_bounds_are_explicit() {
  local home fakebin json canonical id copilot boundary
  home=$(make_home registry-unavailable)
  copilot="$TMP_ROOT/registry-hidden"
  make_valid_copilot_home hidden "$copilot"
  printf -- '- hidden - fixture (home: %s; scope: fixture; projects: sample; added 2026-07-11)\n' "$copilot" > "$home/data/copilots.md"
  ap_write_copilot_meta "$home/state/hidden.meta" "$copilot" "autopilot:ap-hidden" sample
  chmod 000 "$home/data/copilots.md"
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  json=$(run "$home" "$fakebin" --json)
  chmod 600 "$home/data/copilots.md"
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry.complete == false
      and (.copilot_current.records[] | select(.id == "hidden")
        | .registered == null
  ...
  printf '%s' "$json" | jq -e '
    (.copilots | any(.[]; .id == "(registry)" and .state == "unknown"
      and .provenance == "registered-table" and .freshness == "unavailable"))
      and (.omitted | any(.surface | contains("copilot registry unavailable")))
  ' >/dev/null || fail "unreadable registry disappeared from bearings: $json"
  home=$(make_home registry-bounds)
  : > "$home/data/copilots.md"
  for id in one two three; do
    copilot="$TMP_ROOT/registry-$id"
    make_valid_copilot_home "$id" "$copilot"
    append_copilot_registry "$home" "$id" "$copilot"
  done
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_REGISTRY_RECORDS=2 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_REGISTRY_LINES=2 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_REGISTRY_BYTES=100 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry
  ...
  boundary=$(LC_ALL=C head -n 1 "$home/data/copilots.md" | wc -c | tr -d ' ')
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_REGISTRY_BYTES="$((boundary - 1))" "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry
  ...
  json=$(AP_SNAPSHOT_REGISTRY_RECORDS=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    .omitted | any(.surface == "copilot registry records omitted by bounded read")
  ' >/dev/null || fail "bearings omitted registry truncation disclosure: $json"
  copilot="$TMP_ROOT/registry-z-hidden"
  make_valid_copilot_home z-hidden "$copilot"
  append_copilot_registry "$home" z-hidden "$copilot"
  ap_write_copilot_meta "$home/state/z-hidden.meta" "$copilot" "autopilot:ap-z-hidden" sample
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    AP_SNAPSHOT_REGISTRY_RECORDS=3 "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.registry.complete == false
      and (.copilot_current.records[] | select(.id == "z-hidden")
        | .registered == null
          and (.current.reason | contains("registration is unknown")))
...
EOF
  ...
  printf '%s' "$two" | jq -e '
    (.landed | any(.id == "done-a"))
      and (.landed | any(.id == "copilot-landed"))
...
test_default_is_bounded_and_local_only() {
  ...
  local canon; canon=$(PATH="$fakebin:$PATH" AP_HOME="$home" "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  ...
  printf '%s' "$json" | jq -e '.schema == "ap-bearings.v1"' >/dev/null || fail "json schema wrong"
...
test_open_decision_surfaces_end_to_end() {
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    .decisions_open | any(.[]; .id == "copilot/copilot-decision-race"
      and .key == "copilot-decision-race" and .verb == "pilot-hold")
  ' >/dev/null || fail "an authoritative pilot hold must surface in decisions_open: $json"
  pass "an authoritative pilot hold surfaces end-to-end"
}
...
test_report_pointers_surface() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e --arg p "$home/data/recon-x/report.md" '
    .reports | any(.[]; .id == "recon-x" and .path == $p)
  ' >/dev/null || fail "current recon report pointer must surface: $json"
...
test_include_prs_is_the_only_fetch_path() {
  ...
  printf '%s' "$json" | jq -e '
    .candidate_prs | any(.[]; .num == "9" and .task == "flight-task" and .checks == "passing" and .review == "APPROVED")
...
test_perl_fallback_bounds_github_call() {
  ...
  started=$(date +%s)
  json=$(PATH="$fakebin:$toolbin" AP_HOME="$home" AP_BEARINGS_NOW=2026-07-11T18:00:00Z \
    AP_BEARINGS_PR_TIMEOUT=1 NET_LOG="$home/net.log" FAKE_GH_SLEEP=1 "$BEARINGS" --include-prs --json)
...
write_large_fixture() {  # <home> <count>
  ...
  while [ "$i" -le "$count" ]; do
    ...
    printf -- '- [ ] gate-%s - Gate %s blocked-by: task-%s (repo: repo-%s) (kind: flight)\n' "$i" "$i" "$i" "$i" >> "$home/data/backlog.md"
    printf -- '- [ ] decision-%s - Decision %s (repo: repo-%s) (kind: pilot) (hold: pilot choice pending) (hold-kind: external)\n' "$i" "$i" "$i" >> "$home/data/backlog.md"
    ap_write_meta "$home/state/$id.meta" \
      "window=autopilot:ap-$id" \
...
test_section_caps_and_expansion_flags() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(AP_BEARINGS_IN_FLIGHT=2 AP_BEARINGS_DECISIONS=2 AP_BEARINGS_GATES=2 \
    AP_BEARINGS_REPORTS=2 AP_BEARINGS_RECORDED_PRS=2 AP_BEARINGS_UNHEALTHY=2 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  expanded=$(AP_BEARINGS_IN_FLIGHT=2 AP_BEARINGS_DECISIONS=2 AP_BEARINGS_GATES=2 \
    AP_BEARINGS_REPORTS=2 AP_BEARINGS_RECORDED_PRS=2 AP_BEARINGS_UNHEALTHY=2 \
...
test_pr_repository_cap_and_expansion() {
  ...
  fakebin=$(make_fakebin "$home"); : > "$home/net.log"
  json=$(AP_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --json)
  ...
  : > "$home/net.log"
  expanded=$(AP_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --all-pr-repos --json)
...
test_per_repository_pr_cap_is_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(AP_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs --json)
  toon=$(AP_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs)
  printf '%s' "$json" | jq -e '
    (.candidate_prs | length) == 2
    and (.prs | test("2 shown, at least 3 open; capped in 1 repo"))
    and ([.omitted[] | select(.surface == "candidate_prs showing 2 of at least 3; capped in 1 repo(s)" and .reveal == "raise AP_BEARINGS_PR_LIMIT")] | length) == 1
...
# The Lavish-103 defect, end to end: a COMPLETED recon that raised a decision and
...
test_completed_recon_report_not_pending() {
  local home fakebin json
  home=$(make_home completed-recon); write_fixture "$home"
  fakebin=$(make_fakebin "$home")
  ...
  printf '# Lavish 103\nThe open question is whether to adopt approach A or B; this needs a pilot decision.\n' > "$home/data/lavish-103/report.md"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.[]; .id == "lavish-103") | not)
      and (.reports | any(.[]; .id == "lavish-103"))
  ' >/dev/null || fail "completed recon must be a report pointer, never a pending decision: $json"
  pass "a completed recon with decision-like report prose is a pointer, not pending"
}

# Recently Landed must include merges a copilot managed. Those completion records
# live in the copilot home's OWN backlog, not the main one, so the projection must
# roll them up. Local, deterministic, no GitHub call.
test_landed_includes_copilot_home_merges() {
  local home fakebin json
  home=$(make_home copilot-landed); write_fixture "$home"
  fakebin=$(make_fakebin "$home"); : > "$home/net.log"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.landed | any(.[]; .id == "copilot-landed" and (.artifact | test("/pull/50"))))
      and (.landed | any(.[]; .id == "done-a"))
  ' >/dev/null || fail "landed must merge copilot-home Done with main-home Done: $json"
  ...
  pass "landed includes copilot-managed merges alongside main-home merges"
}
...
test_landed_default_balances_dominant_and_sparse_homes() {
  ...
  : > "$home/data/copilots.md"
  printf '## Done\n' > "$home/data/backlog.md"
  dominant=$(make_landed_copilot "$home" dominant)
  sparse_a=$(make_landed_copilot "$home" sparse-a)
  sparse_b=$(make_landed_copilot "$home" sparse-b)
  sparse_c=$(make_landed_copilot "$home" sparse-c)
  i=1
  while [ "$i" -le 12 ]; do
...
test_landed_default_refills_capacity_after_sparse_homes_exhaust() {
  ...
  printf '## Done\n' > "$home/data/backlog.md"
  dominant=$(make_landed_copilot "$home" dominant)
  sparse=$(make_landed_copilot "$home" sparse)
  i=1
  while [ "$i" -le 5 ]; do
    append_landed_row "$dominant" "$(printf 'dominant-landed-%02d' "$i")" \
...
test_landed_default_uses_deterministic_home_order_when_homes_exceed_cap() {
  local home copilot fakebin json actual expected i id
  ...
  while [ "$i" -le 8 ]; do
    ...
    copilot=$(make_landed_copilot "$home" "$id")
    append_landed_row "$copilot" "$id-landed-01" "$id landed 01" 2026-07-10
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
...
test_landed_default_preserves_internal_order_for_ties() {
  ...
  : > "$home/data/copilots.md"
  printf '## Done\n' > "$home/data/backlog.md"
  tie_a=$(make_landed_copilot "$home" tie-a)
  tie_b=$(make_landed_copilot "$home" tie-b)
...
test_all_landed_keeps_complete_global_order() {
  ...
  : > "$home/data/copilots.md"
  printf '## Done\n' > "$home/data/backlog.md"
  alpha=$(make_landed_copilot "$home" alpha)
  beta=$(make_landed_copilot "$home" beta)
  ...
  fakebin=$(make_fakebin "$home")
  json=$(AP_BEARINGS_LANDED=1 run "$home" "$fakebin" --json --all-landed)
  actual=$(printf '%s' "$json" | jq -r '.landed[] | "\(.owner)/\(.id)"')
...
test_landed_bounded_and_disclosed() {
  local home copilot fakebin json i expected actual
  home=$(make_home copilot-landed-caps); write_fixture "$home"
  copilot=$(fixture_copilot_home "$home")
  {
    printf '## In flight\n'
    printf '%s\n\n' '- [ ] copilot - Decide subscription order (repo: autopilot) (kind: flight) (since 2026-07-11)'
  ...
  i=1
  while [ "$i" -le 12 ]; do
    printf -- '- [x] copilot-landed-%02d - Copilot fix %02d (repo: autopilot) (kind: flight) (merged 2026-06-%02d)\n' \
      "$i" "$i" "$((13 - i))" >> "$copilot/data/backlog.md"
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(AP_BEARINGS_LANDED=20 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.landed[].id | select(startswith("copilot-landed-"))] | length) == 10
...
test_live_blocker_is_not_charted_queue_work() {
  ...
  printf 'blocked [key=synthetic-dependency]: autopilot can refresh the synthetic token\n' > "$home/state/flight-task.status"
  record_claude_state "$home/state" flight-task idle
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.in_flight | any(.[]; .id == "flight-task" and .state == "blocked"))
      and (.decisions_open | any(.[]; .id == "flight-task") | not)
      and (.gates | any(.[]; .id == "flight-task") | not)
...
# Pilot's Call is populated only from the durable keyed open-decision set. The
# anti-leak guard: action-free highlights - a working task, a completed recon,
# queued/gated items, landed work - must never surface as an open decision, so they
# cannot leak into Pilot's Call. The standard fixture has exactly one genuine open
# decision (the copilot's structured pilot hold).
test_pilots_call_anti_leak() {
  local home fakebin json canonical
  home=$(make_home anti-leak); write_fixture "$home"
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  jq -n -e --argjson bearings "$json" --argjson canonical "$canonical" '
    ([$bearings.decisions_open[].id] == ["copilot/copilot-decision-race"])
      and ($canonical.copilot_current.records[] | select(.id == "copilot")
        | (.decisions_open | any(.source == "status"))
          and (.decisions_open | any(.source == "backlog")))
      and ([$bearings.decisions_open[].id] | index("flight-task") | not)
      and ([$bearings.decisions_open[].id] | index("recon-x") | not)
      and ([$bearings.decisions_open[].id] | index("external-wait") | not)
      and ([$bearings.decisions_open[].id] | index("done-a") | not)
      and ([$bearings.decisions_open[].id] | index("copilot-landed") | not)
      and ([$bearings.decisions_open[].id] | index("live-gate") | not)
      and ([$bearings.decisions_open[].id] | index("dead-gate") | not)
  ' >/dev/null || fail "only genuine open decisions may feed Pilot's Call: $json"
  pass "action-free items (working/done/queued/landed) do not leak into Pilot's Call"
}
...
test_main_orphan_in_flight_is_disclosed_not_invented() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] only-orphan - Structured in flight without meta (repo: autopilot) (kind: flight) (since 2026-07-11)
...
test_main_unstructured_current_is_disclosed_with_structured_sibling() {
  ...
  : > "$home/data/copilots.md"
  mkdir -p "$home/projects/structured-flight"
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
this current row is not structured
- [ ] structured-flight - Visible structured sibling (repo: autopilot) (kind: flight) (since 2026-07-11)

## Queued
another free-form note without checkbox
- [ ] structured-queued - Structured queued (repo: autopilot) (kind: flight)

## Done
EOF
  ap_write_meta "$home/state/structured-flight.meta" \
    "window=autopilot:ap-structured-flight" \
    "worktree=$home/projects/structured-flight" \
    "project=autopilot" \
    "harness=codex" \
    "kind=flight" \
    "mode=no-mistakes"
  printf 'working: structured sibling still projects\n' > "$home/state/structured-flight.status"
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.in_flight[].id] == ["structured-flight"])
...
test_main_orphan_counterfactual_meta_clears_inventory_warning() {
  ...
  : > "$home/data/copilots.md"
  mkdir -p "$home/projects/orphan-flight"
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] orphan-flight - Gains meta in counterfactual (repo: autopilot) (kind: flight) (since 2026-07-11)
- [ ] visible-flight - Already live (repo: autopilot) (kind: flight) (since 2026-07-11)

## Queued
- [ ] queued-flight - Ordinary queue (repo: autopilot) (kind: flight)

## Done
EOF
  ap_write_meta "$home/state/visible-flight.meta" \
    "window=autopilot:ap-visible-flight" \
    "worktree=$home/projects/orphan-flight" \
    "project=autopilot" \
    "harness=codex" \
    "kind=flight" \
    "mode=no-mistakes"
  ...
  printf '%s' "$json_before" | jq -e '
    ([.in_flight[].id] == ["visible-flight"])
      and ([.in_flight[].id] | index("orphan-flight") | not)
      and (.omitted | any(.surface == "main in-flight backlog item(s) have no child metadata: 1"))
      and (.gates | any(.id == "(main-inventory)"))
      and ([.gates[].id] | index("queued-flight") != null)
...
test_mixed_copilot_roles_partial_state_and_pilot_readiness() {
  ...
  make_valid_copilot_home hibit "$hibit"
  make_valid_copilot_home wheel "$wheel"
  make_valid_copilot_home sshhip "$sshhip"
  make_valid_copilot_home home-assistant "$ha"
  append_copilot_registry "$home" hibit "$hibit"
  append_copilot_registry "$home" wheel "$wheel"
  append_copilot_registry "$home" sshhip "$sshhip"
  append_copilot_registry "$home" home-assistant "$ha"
...
- [ ] hibit-worker - Finalize progress (repo: hibit) (kind: flight)

## Queued

## Done
EOF
  ap_write_meta "$hibit/state/hibit-worker.meta" \
    "window=autopilot:ap-hibit-worker" "worktree=$hibit/projects/worker" "project=hibit" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
...
## In flight
- [ ] production-observation - Observe production (repo: wheelhouse) (kind: recon) (hold: documented no live worker) (hold-kind: external)
- [ ] wheel-worker - Initial triage (repo: wheelhouse) (kind: flight)

## Queued

## Done
EOF
  ap_write_meta "$wheel/state/wheel-worker.meta" \
    "window=autopilot:ap-wheel-worker" "worktree=$wheel/projects/worker" "project=wheelhouse" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
...
## In flight
- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: flight)

## Queued
- [ ] reviewer-decision - Choose reviewer remediation (repo: sshhip) (kind: pilot) (hold: choose reviewer remediation A or B) (hold-kind: external)

## Done
- [x] prior-release - Prior release (repo: sshhip) (kind: flight) (done 2026-07-21)
EOF
  ap_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=autopilot:dead-sshhip-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=codex" "kind=flight" "mode=no-mistakes"

  cat > "$ha/data/backlog.md" <<'EOF'
## In flight
- [ ] prep - Prepare canary (repo: home-assistant) (kind: flight)

## Queued
- [ ] security - Security review (repo: home-assistant) (kind: flight)
- [ ] pilot-run - Run pilot canary blocked-by: prep blocked-by: security (repo: home-assistant) (kind: pilot) (hold: pilot runs canary) (hold-kind: external)

## Done
EOF
  ap_write_meta "$ha/state/prep.meta" \
    "window=autopilot:ap-prep" "worktree=$ha/projects/prep" "project=home-assistant" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  ...
  printf '%s' "$canonical" | jq -e '
    (.copilot_current.records[] | select(.id == "hibit")
      ...
      and (.copilot_landed.partial | length) == 1
      and (.copilot_landed.partial[0] | endswith("/mixed-sshhip-home"))
      and (.copilot_landed.unreadable | length) == 0
      and (.copilot_current.records[] | select(.id == "home-assistant")
        | .current.state == "active_child_work"
          and .decisions_open == []
          and [.active_children[].id] == ["prep"]
          and (.queued[] | select(.id == "pilot-run")
  ...
  printf '%s' "$json" | jq -e '
    ([.in_flight[].id] | sort) == ["hibit", "home-assistant", "wheel"]
      ...
      and (.decisions_open | any(.id == "home-assistant/pilot-run") | not)
      and (.gates | any(.id == "production-observation" and .owner == "wheel"
        and .reason == "documented no live worker"))
      and (.gates | any(.id == "pilot-run" and .owner == "home-assistant"
        and .blocked_by == "prep,security"))
      and (.copilots | any(.id == "sshhip" and .state == "unknown"
...
- [ ] ordinary-orphan - Unowned release task (repo: sshhip) (kind: flight)' \
    "$sshhip/data/backlog.md" > "$sshhip/data/backlog.next"
  mv "$sshhip/data/backlog.next" "$sshhip/data/backlog.md"
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "sshhip")
    | .current.state == "unknown"
...
- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: flight)' \
  ...
  ap_write_meta "$wheel/state/production-observation.meta" \
    "window=autopilot:ap-production-observation" "worktree=$wheel/projects/worker" "project=wheelhouse" \
  ...
  canonical=$(PATH="$fakebin:$PATH" AP_HOME="$home" AP_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    "$ROOT/bin/ap-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .copilot_current.records[] | select(.id == "wheel")
  ...
  ap_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=autopilot:ap-unreadable-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=claude" "kind=flight" "mode=no-mistakes"
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.copilots | any(.id == "sshhip" and .state == "pilot_decision" and .reason == "-"))
...
## Done
- [x] prep - Prepare canary (repo: home-assistant) (kind: flight) (done 2026-07-22)
EOF
  rm "$ha/state/prep.meta" "$ha/state/prep.status"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "home-assistant/pilot-run") | not)
      and (.gates | any(.id == "pilot-run" and .owner == "home-assistant" and .blocked_by == "security"))
...
## Done
- [x] prep - Prepare canary (repo: home-assistant) (kind: flight) (done 2026-07-22)
- [x] security - Security review (repo: home-assistant) (kind: flight) (done 2026-07-22)
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.decisions_open[] | select(.id == "home-assistant/pilot-run")] | length) == 1
      and (.gates | any(.id == "pilot-run" and .owner == "home-assistant") | not)
  ' >/dev/null || fail "zero Home Assistant blockers did not yield exactly one pilot action: $json"
  ...
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "home-assistant/pilot-run") | not)
      and (.gates | any(.id == "pilot-run" and .owner == "home-assistant" and .blocked_by == "missing"))
  ...
  pass "mixed copilot roles, partial state, and pilot readiness project independently"
}

test_main_pilot_readiness_matches_copilot_projection() {
  local home fakebin json
  home=$(make_home main-pilot-readiness)
  : > "$home/data/copilots.md"
  mkdir -p "$home/projects/prep" "$home/projects/observation"
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] observation - Held observation (repo: autopilot) (kind: recon) (hold: watch production) (hold-kind: external)
- [ ] prep - Prepare canary (repo: autopilot) (kind: flight)

## Queued
- [ ] review - Security review (repo: autopilot) (kind: flight)
- [ ] pilot-run - Run pilot canary blocked-by: prep blocked-by: review (repo: autopilot) (kind: pilot) (hold: pilot runs canary) (hold-kind: external)

## Done
EOF
  ap_write_meta "$home/state/prep.meta" \
    "window=autopilot:ap-prep" "worktree=$home/projects/prep" "project=autopilot" \
    "harness=codex" "kind=flight" "mode=no-mistakes"
  printf 'working: preparing main canary\n' > "$home/state/prep.status"
  ap_write_meta "$home/state/observation.meta" \
    "window=autopilot:ap-observation" "worktree=$home/projects/observation" "project=autopilot" \
    "harness=codex" "kind=recon" "mode=recon"
  ...
  printf '%s' "$json" | jq -e '
    (.in_flight | any(.id == "prep"))
      ...
      and (.decisions_open | any(.id == "pilot-run") | not)
      and (.gates | any(.id == "pilot-run" and .blocked_by == "prep,review"))
  ' >/dev/null || fail "main blocked pilot action or held-child projection was wrong: $json"
...
## Done
- [x] prep - Prepare canary (repo: autopilot) (kind: flight) (done 2026-07-22)
EOF
  rm "$home/state/prep.meta" "$home/state/prep.status" \
    "$home/state/observation.meta" "$home/state/observation.status"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "pilot-run") | not)
      and (.gates | any(.id == "pilot-run" and .blocked_by == "review"))
  ' >/dev/null || fail "main one-blocker pilot action became premature: $json"
...
## Done
- [x] prep - Prepare canary (repo: autopilot) (kind: flight) (done 2026-07-22)
- [x] review - Security review (repo: autopilot) (kind: flight) (done 2026-07-22)
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.decisions_open[] | select(.id == "pilot-run")] | length) == 1
      and (.gates | any(.id == "pilot-run") | not)
  ' >/dev/null || fail "main zero-blocker pilot action was not projected exactly once: $json"
  pass "main and copilot pilot actionability use the same blocker readiness"
}
