#!/usr/bin/env bash
# tests/ap-backend-cmux-smoke.test.sh - real cmux smoke test for the cmux
# session-provider adapter (bin/backends/cmux.sh), verified against the real
# cmux 0.64.17 binary (docs/cmux-backend.md). Mirrors
# tests/ap-backend-zellij-smoke.test.sh's/ap-backend-herdr-smoke.test.sh's
...
# Orca). So this test creates ONLY `ap-test-`-prefixed task labels, touches and
...
ap_backend_source cmux || { echo "skip: could not source the cmux adapter"; exit 0; }

ap_backend_cmux_tool_check >/dev/null 2>&1 || { echo "skip: cmux CLI not found on PATH or at the bundle path"; exit 0; }
ap_backend_cmux_version_check >/dev/null 2>&1 || { echo "skip: installed cmux is older than the verified minimum"; exit 0; }
PING_STATE=$(ap_backend_cmux_ping_state)
...
WS2=""
cleanup_all() {
  [ -z "$WS1" ] || cmux_safe_close_workspace "$WS1" "ap-test-smoke1"
  [ -z "$WS2" ] || cmux_safe_close_workspace "$WS2" "ap-test-smoke2"
}
trap cleanup_all EXIT

# --- create_task + duplicate refusal -----------------------------------------

LABEL="ap-test-smoke1"
TASK_IDS=$(ap_backend_cmux_create_task "$LABEL" /tmp) || fail "create_task failed"
read -r WS1 SF1 <<EOF
$TASK_IDS
EOF
if [ -z "$WS1" ] || [ -z "$SF1" ]; then
...
if ap_backend_cmux_create_task "$LABEL" /tmp >/dev/null 2>&1; then
...
ap_backend_cmux_send_key "$TARGET" Escape "$LABEL" \
  || fail "send_key with a matching expected task label should succeed"
if ap_backend_cmux_send_key "$TARGET" Escape "ap-test-not-$LABEL" >/dev/null 2>&1; then
...
ap_backend_cmux_send_literal "$TARGET" 'echo literal-then-key-pilot' \
  || fail "send_literal failed"
sleep 0.3
ap_backend_cmux_send_key "$TARGET" Enter || fail "send_key Enter failed"
sleep 0.5
out=$(ap_backend_cmux_capture "$TARGET" 20) || fail "capture failed after send_literal+send_key"
case "$out" in
...
ap_backend_cmux_send_text_line "$TARGET" "echo pilot-on-deck-line" \
  || fail "send_text_line failed"
sleep 0.5
out=$(ap_backend_cmux_capture "$TARGET" 20) || fail "capture failed after send_text_line"
case "$out" in
  *pilot-on-deck-line*) : ;;
...
ap_backend_cmux_send_text_line "$TARGET" "cd /tmp"
sleep 0.3
p=$(ap_backend_cmux_current_path "$TARGET") || fail "current_path failed"
case "$p" in
  */tmp) : ;;
...
# subshell's own cd. ap_backend_cmux_current_path's active pwd-probe is what
# ap-spawn.sh's worktree-discovery poll actually depends on, so this must be
# proven against a real subshell, not just a plain cd in the top-level shell.
ap_backend_cmux_send_text_line "$TARGET" 'cd / && bash'
sleep 0.5
ap_backend_cmux_send_text_line "$TARGET" "cd /private/tmp"
sleep 0.3
p2=$(ap_backend_cmux_current_path "$TARGET") || fail "current_path failed inside a nested subshell"
case "$p2" in
  */private/tmp|*/tmp) : ;;
...
ap_backend_cmux_send_text_line "$TARGET" 'exit'
sleep 0.3

# --- key names: Escape and Ctrl-C, verified names --------------------------

ap_backend_cmux_send_key "$TARGET" Escape || fail "send_key Escape failed"
pass "real cmux: send_key Escape (natively supported, unlike Orca) succeeds"

ap_backend_cmux_send_key "$TARGET" C-c || fail "send_key C-c (normalized to 'ctrl-c') failed"
...
bs=$(ap_backend_busy_state cmux "$TARGET")
[ "$bs" = unknown ] || fail "ap_backend_busy_state should report unknown for cmux (no native primitive), got '$bs'"
pass "real cmux: ap_backend_busy_state reports unknown (watcher falls back to pane-regex, same as tmux/zellij/orca)"
...
WININFO=$(ap_backend_cmux_window_of_workspace "$WS1")
case "$WININFO" in
  *' '[0-9]*) : ;;
...
ap_backend_cmux_kill "$TARGET"
sleep 0.5
STILL_LIVE=$(ap_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null | jq -r --arg id "$WS1" '.workspaces[]? | select(.id == $id) | .id' 2>/dev/null)
...
ap_backend_cmux_kill "$TARGET" || fail "kill on an already-dead target must stay best-effort (never fail)"
pass "real cmux: kill removes the whole workspace and is idempotent/best-effort"

# --- list_live (title-based recovery discovery) ------------------------------

LABEL2="ap-test-smoke2"
TASK_IDS2=$(ap_backend_cmux_create_task "$LABEL2" /tmp) || fail "second create_task failed"
read -r WS2 _SF2 <<EOF
$TASK_IDS2
EOF
live=$(ap_backend_cmux_list_live)
case "$live" in
  *"$LABEL2"*) : ;;
  *) fail "list_live did not report the freshly created task workspace by title"$'\n'"--- got ---"$'\n'"$live" ;;
esac
pass "real cmux: list_live discovers a live task workspace by ap-<id> title"
