#!/usr/bin/env bash
# Strict no-emit contract check for the tracked Autopilot Pi extensions.
set -u
...
PI_PACKAGE_DIR=${AP_PI_PACKAGE_DIR:-"$(npm root -g)/@earendil-works/pi-coding-agent"}
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ap-pi-primary-types.XXXXXX")
cleanup() {
  rm -rf "$TMP_ROOT"
}
trap cleanup EXIT
...
cp "$ROOT/.pi/extensions/ap-calm.ts" "$TMP_ROOT/ap-calm.ts"
cp "$ROOT/.pi/extensions/ap-primary-pi-watch.ts" "$TMP_ROOT/ap-primary-pi-watch.ts"
cp "$ROOT/.pi/extensions/ap-primary-turnend-guard.ts" "$TMP_ROOT/ap-primary-turnend-guard.ts"
cp "$ROOT/.pi/extensions/lib/ap-calm-assistant-layout.ts" "$TMP_ROOT/lib/ap-calm-assistant-layout.ts"
cp "$ROOT/.pi/extensions/lib/ap-calm-operational-user-layout.ts" "$TMP_ROOT/lib/ap-calm-operational-user-layout.ts"
cp "$ROOT/.pi/extensions/lib/ap-calm-visibility.ts" "$TMP_ROOT/lib/ap-calm-visibility.ts"
cp "$ROOT/.pi/extensions/lib/ap-calm-working-flight.ts" "$TMP_ROOT/lib/ap-calm-working-flight.ts"
cp "$ROOT/.pi/extensions/lib/ap-operational-input.ts" "$TMP_ROOT/lib/ap-operational-input.ts"
