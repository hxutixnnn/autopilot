#!/usr/bin/env bash
# tests/ap-backend-cmux.test.sh - fake-cmux-CLI unit tests for the cmux
# session-provider adapter (bin/backends/cmux.sh), verified against the real
# cmux 0.64.17 binary (docs/cmux-backend.md). Mirrors
# tests/ap-backend-zellij.test.sh's/tests/ap-backend-herdr.test.sh's
...
# tests/ap-backend-cmux-smoke.test.sh, gated on the cmux binary actually
...
TMP_ROOT=$(ap_test_tmproot ap-backend-cmux-tests)

# make_cmux_fakebin: a `cmux` stub that logs every invocation (one line,
# unit-separated args, to $AP_CMUX_LOG) and returns the canned response for
# that call read from $AP_CMUX_RESPONSES/<n>.out, consumed IN ORDER (call 1
# reads 1.out, call 2 reads 2.out, ...), mirroring
# tests/ap-backend-zellij.test.sh's make_zellij_fakebin. A missing response
...
# queue) since ap_backend_cmux_version_check/ap_backend_cmux_ping_state are
...
if [ "${1:-}" = version ]; then
  printf 'cmux %s (97) [abcdef1]\n' "${AP_CMUX_FAKE_VERSION:-0.64.17}"
  exit 0
fi
if [ "${1:-}" = ping ]; then
  printf '%s\n' "${AP_CMUX_FAKE_PING:-PONG}"
  exit "${AP_CMUX_FAKE_PING_EXIT:-0}"
fi
...
cmux_expected_scoped_title() {  # <ap-task-label> [home] [root]
  local label=$1 home=${2:-$ROOT} root=${3:-$ROOT} rest
  case "$label" in
    ap-*) rest=${label#ap-} ;;
    *) rest=$label ;;
  esac
  printf 'ap-%s-%s' "$(cmux_expected_home_label "$home" "$root")" "$rest"
}
...
test_version_check_accepts_current_version() {
  ...
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_VERSION=0.64.17 \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_version_check' "$ROOT"
  status=$?
  expect_code 0 "$status" "version_check should accept 0.64.17 (the verified minimum)"
  pass "ap_backend_cmux_version_check: accepts the verified minimum (0.64.17)"
}
...
test_version_check_accepts_newer_version() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_VERSION=0.70.0 \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_version_check' "$ROOT"
  status=$?
  expect_code 0 "$status" "version_check should accept a newer minor (0.70.0)"
  pass "ap_backend_cmux_version_check: accepts a newer version (0.70.0)"
}
...
test_version_check_refuses_old_version() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_VERSION=0.50.0 \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_version_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_cmux_version_check: refuses an old version loudly"
}

test_version_check_refuses_missing_cmux() {
  local dir out status
  dir="$TMP_ROOT/version-missing"; mkdir -p "$dir/empty-fakebin"
  # AP_BACKEND_CMUX_BUNDLE_BIN must also be overridden to a nonexistent path:
  ...
  out=$( PATH="$dir/empty-fakebin:/usr/bin:/bin" AP_BACKEND_CMUX_BUNDLE_BIN="$dir/no-such-cmux" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_version_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_cmux_version_check: refuses loudly when cmux is not found on PATH or at the bundle path"
}
...
test_password_reads_from_config_file() {
  ...
  out=$( AP_HOME="$dir" bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_password' "$ROOT" )
  [ "$out" = "sekret-pw" ] || fail "password should be read from config/cmux-socket-password, got '$out'"
  pass "ap_backend_cmux_password: reads the first non-empty line of config/cmux-socket-password"
}
...
test_password_preserves_config_file_whitespace() {
  ...
  pass "ap_backend_cmux_password: preserves spaces and tabs in config/cmux-socket-password"
}
...
test_password_respects_config_override() {
  ...
  out=$( AP_HOME="$dir/home" AP_CONFIG_OVERRIDE="$override_cfg" bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_password' "$ROOT" )
  [ "$out" = "override-pw" ] || fail "password should be read from AP_CONFIG_OVERRIDE, got '$out'"
  pass "ap_backend_cmux_password: respects AP_CONFIG_OVERRIDE"
}
...
test_password_empty_when_config_absent() {
  ...
  pass "ap_backend_cmux_password: empty when config/cmux-socket-password is absent"
}
...
test_cli_exports_password_only_when_configured() {
  ...
  printf 'sekret-pw\n' > "$dir/config/cmux-socket-password"
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_HOME="$dir" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_cli ping' "$ROOT" >/dev/null
  assert_contains "$(cat "$dir/log")" "CMUX_SOCKET_PASSWORD=sekret-pw" \
    "ap_backend_cmux_cli did not export the configured password"
  pass "ap_backend_cmux_cli: exports CMUX_SOCKET_PASSWORD when config/cmux-socket-password is set"
}
...
test_parse_target() {
  ( . "$ROOT/bin/backends/cmux.sh"
    ap_backend_cmux_parse_target "11111111-1111-1111-1111-111111111111:22222222-2222-2222-2222-222222222222" || exit 1
    [ "$AP_BACKEND_CMUX_WORKSPACE" = "11111111-1111-1111-1111-111111111111" ] || { echo "workspace mismatch: $AP_BACKEND_CMUX_WORKSPACE" >&2; exit 1; }
    [ "$AP_BACKEND_CMUX_SURFACE" = "22222222-2222-2222-2222-222222222222" ] || { echo "surface mismatch: $AP_BACKEND_CMUX_SURFACE" >&2; exit 1; }
  ) || fail "ap_backend_cmux_parse_target did not split workspace:surface correctly"
  pass "ap_backend_cmux_parse_target: splits '<workspace_uuid>:<surface_uuid>' on the first colon"
}

test_normalize_key() {
  ( . "$ROOT/bin/backends/cmux.sh"
    [ "$(ap_backend_cmux_normalize_key Enter)" = enter ] || { echo "Enter failed" >&2; exit 1; }
    [ "$(ap_backend_cmux_normalize_key Escape)" = escape ] || { echo "Escape failed" >&2; exit 1; }
    [ "$(ap_backend_cmux_normalize_key Esc)" = escape ] || { echo "Esc failed" >&2; exit 1; }
    [ "$(ap_backend_cmux_normalize_key C-c)" = ctrl-c ] || { echo "C-c failed" >&2; exit 1; }
    [ "$(ap_backend_cmux_normalize_key ctrl+c)" = ctrl-c ] || { echo "ctrl+c failed" >&2; exit 1; }
  ) || fail "ap_backend_cmux_normalize_key did not map autopilot's key vocabulary to cmux's verified names"
  pass "ap_backend_cmux_normalize_key: Enter/Escape/C-c map to cmux's verified enter/escape/ctrl-c"
}

test_scoped_title_uses_primary_home_label() {
  local dir out expected
  dir="$TMP_ROOT/scoped-title-primary"; mkdir -p "$dir"
  expected=$(cmux_expected_scoped_title ap-task1 "$dir")
  out=$( AP_HOME="$dir" bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_scoped_title ap-task1' "$ROOT" )
  [ "$out" = "$expected" ] || fail "primary scoped title should be $expected, got '$out'"
  pass "ap_backend_cmux_scoped_title: scopes a primary task title with autopilot plus root hash"
}
...
test_scoped_title_uses_copilot_home_label() {
  ...
  pass "ap_backend_cmux_scoped_title: scopes a copilot task title with the home marker plus root hash"
}
...
test_scoped_title_changes_with_root_path() {
  ...
  expected_one=$(cmux_expected_scoped_title ap-task1 "$home" "$root_one")
  expected_two=$(cmux_expected_scoped_title ap-task1 "$home" "$root_two")
  out_one=$( AP_HOME="$home" AP_ROOT_OVERRIDE="$root_one" bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_scoped_title ap-task1' "$ROOT" )
  out_two=$( AP_HOME="$home" AP_ROOT_OVERRIDE="$root_two" bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_scoped_title ap-task1' "$ROOT" )
  ...
  pass "ap_backend_cmux_scoped_title: includes the resolved AP_ROOT hash in the home label"
}

# --- dispatch wiring (ap-backend.sh) ------------------------------------------

test_dispatch_routes_cmux_backend() {
  ap_backend_validate cmux 2>/dev/null || fail "ap_backend_validate should accept cmux"
  pass "ap_backend_validate: cmux is a known backend"
}

test_dispatch_busy_state_unknown_for_cmux() {
  # shellcheck source=/dev/null
  . "$ROOT/bin/ap-backend.sh"
  [ "$(ap_backend_busy_state cmux '11111111-1111-1111-1111-111111111111:22222222-2222-2222-2222-222222222222')" = unknown ] \
    || fail "ap_backend_busy_state should report unknown for cmux (no native agent-state primitive)"
  pass "ap_backend_busy_state: cmux (no native primitive) always reports unknown, same as tmux/zellij/orca"
}
...
test_dispatch_composer_state_routes_cmux() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/ap-backend.sh"; ap_backend_composer_state cmux "$1"' "$ROOT" "$target" )
  [ "$out" = pending ] || fail "ap_backend_composer_state should route cmux to its classifier, got '$out'"
  pass "ap_backend_composer_state: routes cmux to the cmux composer classifier"
}
...
test_ping_state_ok() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING=PONG \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ping_state' "$ROOT" )
  [ "$out" = ok ] || fail "ping_state should report ok on PONG, got '$out'"
  pass "ap_backend_cmux_ping_state: reports 'ok' on PONG"
}
...
test_ping_state_denied() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING_EXIT=1 \
    AP_CMUX_FAKE_PING="Error: ERROR: Access denied - only processes started inside cmux can connect" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ping_state' "$ROOT" )
  [ "$out" = denied ] || fail "ping_state should report denied on the cmuxOnly rejection text, got '$out'"
  pass "ap_backend_cmux_ping_state: reports 'denied' when socketControlMode=cmuxOnly rejects the connection"
}
...
test_ping_state_unauth() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING_EXIT=1 \
    AP_CMUX_FAKE_PING="Error: ERROR: Authentication required - send auth <password> first" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ping_state' "$ROOT" )
  [ "$out" = unauth ] || fail "ping_state should report unauth when no password was presented, got '$out'"
  pass "ap_backend_cmux_ping_state: reports 'unauth' when password mode rejects a missing/wrong password"
}
...
test_ping_state_invalid_password() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING_EXIT=1 \
    AP_CMUX_FAKE_PING="Error: ERROR: Invalid password" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ping_state' "$ROOT" )
  [ "$out" = unauth ] || fail "ping_state should report unauth on the wrong-password rejection text, got '$out'"
  pass "ap_backend_cmux_ping_state: reports 'unauth' when password mode rejects a wrong password (Invalid password)"
}
...
test_ping_state_down() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING_EXIT=1 \
    AP_CMUX_FAKE_PING="Error: Socket not found at /home/x/.local/state/cmux/cmux.sock" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ping_state' "$ROOT" )
  [ "$out" = down ] || fail "ping_state should report down when the socket does not exist yet, got '$out'"
  pass "ap_backend_cmux_ping_state: reports 'down' when the app is not running yet"
}
...
SH
  chmod +x "$fb/open"
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING=PONG \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ensure_running' "$ROOT"
  status=$?
  expect_code 0 "$status" "ensure_running should succeed immediately when already reachable"
  pass "ap_backend_cmux_ensure_running: returns immediately when cmux is already reachable"
}
...
test_ensure_running_fails_fast_on_denied_without_launching() {
  ...
  cat > "$fb/open" <<'SH'
#!/usr/bin/env bash
echo "LAUNCHED" >> "${AP_CMUX_LAUNCH_MARKER:?}"
exit 0
SH
  chmod +x "$fb/open"
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" AP_CMUX_FAKE_PING_EXIT=1 \
    AP_CMUX_FAKE_PING="Error: ERROR: Access denied - only processes started inside cmux can connect" \
    AP_CMUX_LAUNCH_MARKER="$dir/launched" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_ensure_running' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_cmux_ensure_running: fails fast on a denied socket without attempting to launch, naming every viable mode"
}
...
SH
  ...
  pass "ap_backend_cmux_ensure_running: fails fast on an unauthenticated socket, naming the password config and the Automation mode alternative"
}
...
test_create_task_refuses_duplicate_label() {
  ...
  title=$(cmux_expected_scoped_title ap-dup1)
  cmux_workspace_list_response "$dir" 1 "aaaaaaaa-0000-0000-0000-000000000000" "$title"
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_create_task ap-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_cmux_create_task: refuses a duplicate workspace title (cmux's own new-workspace has no uniqueness check)"
}

test_create_task_creates_and_parses_ids() {
  local dir fb out title
  dir="$TMP_ROOT/create-task"; mkdir -p "$dir/responses"
  title=$(cmux_expected_scoped_title ap-newtask)
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_create_task ap-newtask /tmp/proj' "$ROOT" )
  ...
  pass "ap_backend_cmux_create_task: creates a workspace and parses workspace_id/surface_id from list responses"
}
...
test_target_ready_fails_when_target_absent() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_target_ready "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT"
  status=$?
  [ "$status" -ne 0 ] || fail "target_ready should fail when list-panes reports the surface not found"
  pass "ap_backend_cmux_target_ready: fails when the workspace/surface is not found (list-panes structural check)"
}

test_target_ready_checks_expected_label() {
  local dir fb title
  dir="$TMP_ROOT/ready-label-ok"; mkdir -p "$dir/responses"
  title=$(cmux_expected_scoped_title ap-label)
  cmux_workspace_list_response "$dir" 1 "aaaaaaaa-0000-0000-0000-000000000000" "$title"
  # 2: list-panes --json --id-format uuids -> matching surface
  cmux_panes_response "$dir" 2 "bbbbbbbb-1111-1111-1111-111111111111"
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_target_ready "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" ap-label' "$ROOT"
  ...
  pass "ap_backend_cmux_target_ready: verifies the workspace title against the expected label first"
}
...
test_target_ready_rejects_label_mismatch() {
  ...
  pass "ap_backend_cmux_target_ready: rejects a workspace id reused under a different title"
}
...
test_capture_trims_locally() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_capture "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" 2' "$ROOT" )
  ...
  pass "ap_backend_cmux_capture: fetches generously and trims to N lines locally"
}
...
test_capture_fails_when_read_screen_fails_empty() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_capture "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" 5' "$ROOT"
  status=$?
  ...
  pass "ap_backend_cmux_capture: propagates a read-screen failure even when stdout is empty"
}
...
test_capture_fails_when_target_not_ready() {
  ...
  pass "ap_backend_cmux_capture: fails when the target surface is absent"
}
...
test_send_key_normalizes_and_targets() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_key "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" Escape' "$ROOT"
  ...
  pass "ap_backend_cmux_send_key: normalizes the key (Escape -> escape) and targets the explicit workspace/surface"
}
...
test_send_key_recovers_stale_target_by_label() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_key "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" Enter ap-label' "$ROOT"
  ...
  pass "ap_backend_cmux_send_key: recovers stale workspace/surface ids by expected label"
}
...
test_send_literal_uses_separator_for_option_shaped_text() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_literal "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "--help"' "$ROOT"
  ...
  pass "ap_backend_cmux_send_literal: calls send with an explicit workspace/surface and a -- separator"
}
...
test_current_path_probes_with_marker() {
  ...
  cmux_read_screen_response "$dir" 7 $'/tmp/proj\n❯ printf marker\n__AP_CMUX_CWD_BEGIN__\n/home/fixture/.treehouse/fake-worktree\n__AP_CMUX_CWD_END__\n/home/fixture/.treehouse/fake-worktree ❯'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_current_path "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = "/home/fixture/.treehouse/fake-worktree" ] || fail "current_path should read only the marked cwd line, got '$out'"
  assert_contains "$(cat "$dir/log")" "__AP_CMUX_CWD_BEGIN__" "current_path did not send the cwd begin marker"
  ...
  pass "ap_backend_cmux_current_path: actively probes with marked begin/end lines (zellij-shape frozen cwd)"
}
...
test_composer_state_bare_prompt_is_empty() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a bare prompt glyph should read as empty, got '$out'"
  pass "ap_backend_cmux_composer_state: a bare '❯' composer row reads empty"
}
...
test_composer_state_ghost_placeholder_is_empty() {
  ...
  pass "ap_backend_cmux_composer_state: the ghost placeholder text reads empty, not pending"
}
...
test_composer_state_real_text_is_pending() {
  ...
  pass "ap_backend_cmux_composer_state: real composer text reads pending"
}
...
test_composer_state_popup_placeholder_fill_is_pending() {
  ...
  pass "ap_backend_cmux_composer_state: a slash-command popup's argument-hint placeholder still reads pending (the incident fix)"
}
...
test_composer_state_unknown_on_capture_failure() {
  ...
  pass "ap_backend_cmux_composer_state: reports unknown when the surface cannot be captured"
}
...
test_composer_state_unknown_when_no_composer_row_found() {
  ...
  pass "ap_backend_cmux_composer_state: reports unknown when no border-delimited composer row is found"
}
...
test_send_text_submit_detects_landed_send() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "hello pilot" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty (submitted) once the composer row reads empty, got '$out'"
  assert_contains "$(cat "$dir/log")" $'\x1f''send'$'\x1f''--workspace'$'\x1f''aaaaaaaa-0000-0000-0000-000000000000'$'\x1f''--surface'$'\x1f''bbbbbbbb-1111-1111-1111-111111111111'$'\x1f''--'$'\x1f''hello pilot' \
  ...
  pass "ap_backend_cmux_send_text_submit: reports 'empty' once the composer row reads empty after one Enter"
}
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "hello pilot" 2 0.01 0.01' "$ROOT" )
  [ "$out" = pending ] || fail "send_text_submit should report pending once retries are exhausted with no visible change, got '$out'"
  pass "ap_backend_cmux_send_text_submit: reports 'pending' when the composer never clears after retried Enters (swallowed)"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "/compact" 3 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_cmux_send_text_submit: retries past a popup-placeholder-fill Enter and lands the real second Enter (the incident fix)"
}
...
test_send_text_submit_send_failed_when_target_absent() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "x" 2 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "send_text_submit should report send-failed when the target is absent, got '$out'"
  pass "ap_backend_cmux_send_text_submit: reports 'send-failed' when the target workspace/surface is absent"
}
...
test_window_of_workspace_finds_window_and_count() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_window_of_workspace "aaaaaaaa-0000-0000-0000-000000000000"' "$ROOT" )
  ...
  pass "ap_backend_cmux_window_of_workspace: walks windows and counts the membership-confirming workspace list"
}
...
test_window_of_workspace_empty_when_not_found() {
  ...
  pass "ap_backend_cmux_window_of_workspace: echoes nothing when no window holds the workspace"
}
...
test_kill_closes_workspace_directly_when_not_last() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_kill "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT"
  ...
  pass "ap_backend_cmux_kill: closes the task workspace directly when it is not the last in its window"
}
...
test_kill_adds_sibling_when_last_in_window() {
  ...
  assert_not_contains "$(cat "$dir/log")" $'\x1f''new-workspace'$'\x1f''--name' \
    "the throwaway sibling must stay an unnamed default workspace, never an ap- task title"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-workspace'$'\x1f''--workspace'$'\x1f''aaaaaaaa-0000-0000-0000-000000000000' \
  ...
  pass "ap_backend_cmux_kill: adds a throwaway sibling then closes the target when it is the last workspace in its window"
}
...
test_kill_is_best_effort_when_close_workspace_fails() {
  ...
  assert_not_contains "$(cat "$dir/log")" $'\x1f''close-surface' \
    "kill should not call close-surface"
  pass "ap_backend_cmux_kill: never fails even when close-workspace fails"
}
...
test_kill_recovers_stale_target_by_label() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_kill "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "" ap-label' "$ROOT"
  ...
  assert_not_contains "$(cat "$dir/log")" $'\x1f''close-surface' \
    "kill should not call close-surface"
  pass "ap_backend_cmux_kill: recovers stale workspace/surface ids by expected label"
}
...
test_list_live_filters_by_title_prefix() {
  ...
  title=$(cmux_expected_scoped_title ap-task1)
  other_title=$(cmux_expected_scoped_title ap-task2 "$ROOT" "$other_root")
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" AP_CMUX_LOG="$dir/log" AP_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; ap_backend_cmux_list_live' "$ROOT" )
  [ "$out" = $'aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111\tap-task1' ] \
    || fail "list_live should list only the in-home task workspace with its plain label and surface id, got '$out'"
  pass "ap_backend_cmux_list_live: lists only this home's scoped task workspaces using plain ap-<id> labels"
}

# --- ap-spawn.sh: --copilot refuses backend=cmux --------------------------

test_copilot_spawn_refuses_cmux_backend() {
  local dir state data config projects out status
  dir="$TMP_ROOT/copilot-refuse"; state="$dir/state"; data="$dir/data"; config="$dir/config"; projects="$dir/projects"
  mkdir -p "$state" "$data" "$config" "$projects"
  out=$( AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" AP_PROJECTS_OVERRIDE="$projects" \
    "$ROOT/bin/ap-spawn.sh" copilot-cmux-test --copilot --backend cmux 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "ap-spawn.sh should refuse a --copilot spawn with --backend cmux"
  assert_contains "$out" "does not support --copilot" "ap-spawn.sh did not report the cmux copilot refusal"
  pass "ap-spawn.sh: refuses backend=cmux for --copilot spawns (mirrors Orca's refusal; no copilot launch design exists yet)"
}
