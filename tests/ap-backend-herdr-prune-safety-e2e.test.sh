#!/usr/bin/env bash
# tests/ap-backend-herdr-prune-safety-e2e.test.sh - isolated real-herdr
...
# HERDR_SESSION (never the pilot's default): a startup-workspace-shaped
# layout - one tab labeled "1" in a pre-existing workspace labeled
# "autopilot" - with a live long-running process in that pane, exactly as
# the pilot's own live flight crew member session looked at incident time. Then
...
# ap-<id> task tab), mirroring tests/ap-backend-herdr-smoke.test.sh's broader
...
SESSION="ap-lab-prune-safety-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/ap-herdr-prune-safety.XXXXXX")
cleanup_all() {
...
# Explicitly label the startup workspace "autopilot" to create the collision
...
LIVE_CWD="$SCRATCH/autopilot"
mkdir -p "$LIVE_CWD"

ap_backend_herdr_server_ensure "$SESSION" || fail "could not start the isolated session's server"

CREATE_OUT=$(ap_backend_herdr_cli "$SESSION" workspace create --cwd "$LIVE_CWD" --label autopilot --no-focus) \
...
[ "$LIVE_LABEL" = autopilot ] || fail "the startup workspace label should be 'autopilot', got '$LIVE_LABEL' - repro setup is wrong"
pass "repro setup: a pre-existing workspace labeled 'autopilot' collides with the primary home's own label"
...
for _ in $(seq 1 100); do
  PROCESS_INFO=$(ap_backend_herdr_cli "$SESSION" pane process-info --pane "$LIVE_PANE_ID" 2>/dev/null || true)
...
ap_backend_herdr_cli "$SESSION" pane run "$LIVE_PANE_ID" \
...
RAW=$(ap_backend_herdr_container_ensure "$LIVE_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
...
TASK_IDS=$(ap_backend_herdr_create_task "$CONTAINER" ap-prunesafety-e2e "$LIVE_CWD" "$SEEDED_TAB_ID") \
...
ap_backend_herdr_kill "$SESSION:$NEW_PANE_ID"
ap_backend_herdr_kill "$SESSION:$LIVE_PANE_ID"

# --- 4. happy path still works: a genuinely fresh workspace gets its seeded -
# default tab pruned, leaving exactly one clean ap-<id> task tab -------------

HAPPY_CWD="$SCRATCH/happy-project"
mkdir -p "$HAPPY_CWD"
HAPPY_RAW=$(ap_backend_herdr_container_ensure "$HAPPY_CWD") || fail "happy-path container_ensure failed"
...
HAPPY_TASK_IDS=$(ap_backend_herdr_create_task "$HAPPY_CONTAINER" ap-prunesafety-happy "$HAPPY_CWD" "$HAPPY_SEEDED") \
...
pass "happy path: a genuinely fresh workspace's seeded default tab is still pruned, leaving exactly one clean ap-<id> task tab"

ap_backend_herdr_kill "$SESSION:$HAPPY_PANE"
