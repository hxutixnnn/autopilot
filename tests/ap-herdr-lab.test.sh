#!/usr/bin/env bash
# Behavior tests for bin/ap-herdr-lab.sh using a stateful fake Herdr client.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-herdr-lab)
FAKEBIN=$(ap_fakebin "$TMP_ROOT")
...
case "$1 ${2:-}" in
  "session list")
    ...
    ;;
  "server --session")
    if [ "${AP_FAKE_HERDR_SERVER_DELAY:-0}" != 0 ]; then
      "$AP_FAKE_HERDR_REAL_SLEEP" "$AP_FAKE_HERDR_SERVER_DELAY"
    fi
  ...
  "session delete")
    [ "$3" = "$session" ] || exit 92
    [ "${AP_FAKE_HERDR_DELETE_FAIL:-}" != 1 ] || exit 93
...
. "$ROOT/bin/ap-herdr-lab.sh"

run_with_fake() {
  PATH="$FAKEBIN:$PATH" \
    AP_FAKE_HERDR_STATE="$FAKE_STATE" \
    AP_FAKE_HERDR_LOG="$FAKE_LOG" \
    AP_FAKE_HERDR_REAL_SLEEP="$REAL_SLEEP" \
    AP_FAKE_HERDR_SERVER_DELAY="${AP_FAKE_HERDR_SERVER_DELAY:-0}" \
    AP_FAKE_HERDR_FAST_POLL="${AP_FAKE_HERDR_FAST_POLL:-}" \
    AP_FAKE_HERDR_DELETE_FAIL="${AP_FAKE_HERDR_DELETE_FAIL:-}" \
    AP_HERDR_LAB_STATE_DIR="$TRIPWIRES" \
    "$@"
}

test_refuses_unsafe_names() {
  local status=0 generated
  ap_herdr_lab_validate_name default >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "literal default must be refused"
  status=0
  ap_herdr_lab_validate_name arbitrary-session >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "non-lab prefix must be refused"
  ap_herdr_lab_validate_name ap-lab-safe-123 || fail "valid lab session name was refused"
  generated=$(ap_herdr_lab_name ap-autodetect-smoke-concurrency-h3)
  ap_herdr_lab_validate_name "$generated" || fail "generated lab session name was refused"
  [ "${#generated}" -le 40 ] || fail "generated lab session name is too long for Herdr socket paths: $generated"
  pass "ap-herdr-lab: names fail closed and require the lab prefix"
}

test_provision_run_and_guarded_teardown() {
  local name='' line_count status=0 stop_line delete_line
  name="ap-lab-behavior-$$"
  : > "$FAKE_LOG"
  run_with_fake ap_herdr_lab_provision "$name" || fail "provision failed"
  ...
  run_with_fake ap_herdr_lab_cli "$name" workspace list >/dev/null || fail "safe run command failed"
  run_with_fake ap_herdr_lab_cli "$name" server >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "bare server start outside provision must be refused"
  status=0
  run_with_fake ap_herdr_lab_cli "$name" server stop >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "server-global stop must be refused"
  status=0
  run_with_fake ap_herdr_lab_cli "$name" session delete "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "direct session delete must be refused"
  status=0
  run_with_fake ap_herdr_lab_cli "$name" status --session default >/dev/null 2>&1 || status=$?
  ...
  status=0
  run_with_fake ap_herdr_lab_cli "$name" --handoff server stop >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option shifting server stop past the guard must be refused"
  status=0
  run_with_fake ap_herdr_lab_cli "$name" --no-session session delete "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option shifting session delete past the guard must be refused"
  status=0
  run_with_fake ap_herdr_lab_cli "$name" --remote host workspace list >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option subverting session isolation must be refused"

  run_with_fake ap_herdr_lab_teardown "$name" || fail "guarded teardown failed"
  ...
  pass "ap-herdr-lab: provisioning, scoped calls, guarded teardown, and fleet tripwire are deterministic"
}

test_missing_tripwire_blocks_destruction() {
  local name="ap-lab-no-tripwire-$$" status=0 before after
  printf '%s\n' running > "$FAKE_STATE/$name"
  : > "$FAKE_LOG"
  before=$(wc -l < "$FAKE_LOG")
  run_with_fake ap_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  ...
  pass "ap-herdr-lab: missing tripwire refuses teardown before any Herdr call"
}

test_changed_default_trips_after_teardown() {
  local name="ap-lab-tripwire-change-$$" status=0
  : > "$FAKE_LOG"
  run_with_fake ap_herdr_lab_provision "$name" || fail "tripwire fixture provision failed"
  ...
  pass "ap-herdr-lab: changed default fleet state is a hard failure"
}

test_stopped_owned_lab_can_reprovision() {
  local name="ap-lab-reprovision-$$"
  : > "$FAKE_LOG"
  run_with_fake ap_herdr_lab_provision "$name" || fail "initial provision failed"
  run_with_fake ap_herdr_lab_stop "$name" || fail "guarded stop failed"
  ...
  run_with_fake ap_herdr_lab_provision "$name" || fail "re-provision after guarded stop failed"
  ...
  run_with_fake ap_herdr_lab_teardown "$name" || fail "teardown after re-provision failed"
  pass "ap-herdr-lab: an owned stopped lab can re-provision safely"
}

test_failed_delete_retains_tripwire() {
  local name="ap-lab-delete-failure-$$" status=0
  : > "$FAKE_LOG"
  run_with_fake ap_herdr_lab_provision "$name" || fail "delete-failure fixture provision failed"
  AP_FAKE_HERDR_DELETE_FAIL=1 run_with_fake ap_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  ...
  run_with_fake ap_herdr_lab_teardown "$name" || fail "retry after failed delete did not clean up the lab session"
  assert_absent "$TRIPWIRES/$name.fleet-state.json" "successful retry left the ownership tripwire behind"
  pass "ap-herdr-lab: failed deletion retains ownership until absence is confirmed"
}

test_timed_out_provision_cancels_late_launch() {
  local name="ap-lab-late-launch-$$" status=0
  cat > "$FAKEBIN/sleep" <<'SH'
#!/usr/bin/env bash
if [ "${AP_FAKE_HERDR_FAST_POLL:-}" = 1 ]; then
  exit 0
fi
exec "$AP_FAKE_HERDR_REAL_SLEEP" "$@"
SH
  chmod +x "$FAKEBIN/sleep"
  : > "$FAKE_LOG"
  AP_FAKE_HERDR_FAST_POLL=1 AP_FAKE_HERDR_SERVER_DELAY=30 \
    run_with_fake ap_herdr_lab_provision "$name" >/dev/null 2>&1 || status=$?
  ...
  run_with_fake ap_herdr_lab_teardown "$name" || fail "teardown after timed-out provision failed"
  ...
  pass "ap-herdr-lab: timed-out provisioning cancels the launch before teardown"
}
