#!/usr/bin/env bash
# Tests for the tracked Pi primary watcher extension and Pi copilot wiring.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-pi-watch-extension)
EXT="$ROOT/.pi/extensions/ap-primary-pi-watch.ts"
...
install_pi_watch_extension_fixture() {
  ...
  cp "$EXT" "$repo/.pi/extensions/ap-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/lib/ap-calm-visibility.ts" "$repo/.pi/extensions/lib/ap-calm-visibility.ts"
  ...
  chmod +x "$repo/bin/ap-operational-input.sh"
...
test_pi_extension_reports_external_healthy_watcher() {
  ...
  plugin="$repo/.pi/extensions/ap-primary-pi-watch.ts"
  cat > "$repo/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
...
const pi = {
  ...
  registerCommand(name, options) {
    if (name === "ap-watch-arm-pi") handler = options.handler;
  },
  registerTool() {},
...
writeFileSync(`${process.env.AP_HOME}/state/.lock`, `${process.pid}\n`);
...
if (!prompt.startsWith("\u2063AUTOPILOT_OP: v1 watcher: ")) {
  console.error(`untyped operational follow-up: ${prompt}`);
  process.exit(1);
}
if (!prompt.includes("AUTOPILOT WATCHER WAKE")) {
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" node --input-type=module 2>&1 <<'EOF'
import { writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  registerTool(candidate) {
    if (candidate.name === "ap_watch_arm_pi") tool = candidate;
  },
...
if (tool.label !== "Arm autopilot watcher") throw new Error(`unexpected label: ${tool.label}`);
...
while [ ! -e "$AP_STOP_FILE" ]; do sleep 0.02; done
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_STOP_FILE="$stop" node --input-type=module 2>&1 <<'EOF'
...
for (let i = 0; i < 100 && !existsSync(process.env.AP_ARM_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (!existsSync(process.env.AP_ARM_LOG)) throw new Error("initial arm child did not start");
await new Promise((resolve) => setTimeout(resolve, 100));
const rows = readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n");
if (rows.length !== 1) throw new Error(`redundant call spawned ${rows.length} arm children`);
writeFileSync(process.env.AP_STOP_FILE, "stop\n");
EOF
)
  status=$?
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_WATCH_REARM_RETRY_BASE_MS=10000 AP_WATCH_REARM_RETRY_MAX_MS=10000 node --input-type=module 2>&1 <<'EOF'
...
test_pi_actionable_close_starts_single_successor_before_delivery() {
  ...
  cat > "$repo/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'arm=%s predecessor=%s\n' "$$" "${AP_WATCH_PREDECESSOR_ARM_PID:-none}" >> "${AP_ARM_LOG:?}"
count=$(wc -l < "$AP_ARM_LOG" | tr -d '[:space:]')
printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
if [ "$count" -eq 1 ]; then
...
const pi = {
  ...
  sendUserMessage: async () => {
    rowsAtDelivery = existsSync(process.env.AP_ARM_LOG)
      ? readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n").length
      : 0;
...
for (let i = 0; i < 250; i += 1) {
  const rows = existsSync(process.env.AP_ARM_LOG)
    ? readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n")
    : [];
...
await new Promise((resolve) => setTimeout(resolve, 100));
const stableRows = readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n");
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_PI_ARM_READY_TIMEOUT_MS=250 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  sendUserMessage: async (message) => {
    prompt += message;
    rowsAtPrompt = existsSync(process.env.AP_ARM_LOG)
      ? readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n").length
      : 0;
  },
};
...
trap '' TERM INT
printf 'arm=%s\n' "$$" >> "${AP_ARM_LOG:?}"
while [ ! -e "$AP_RELEASE_FILE" ]; do sleep 0.1; done
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_RELEASE_FILE="$release" AP_PI_ARM_READY_TIMEOUT_MS=250 AP_WATCH_ARM_RETIRE_TIMEOUT_MS=20 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
writeFileSync(process.env.AP_RELEASE_FILE, "release\n");
...
if [ "$count" -eq 2 ]; then
  trap 'printf "retired\\n" > "${AP_UNRETIRED_RETIRE_FILE:?}"' TERM INT
  printf 'ready\n' > "${AP_UNRETIRED_READY_FILE:?}"
  while [ ! -e "$AP_RELEASE_FILE" ]; do sleep 0.02; done
  [ "$AP_LATE_KIND" = actionable ] && printf 'signal: late wake\n'
  exit 0
fi
printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
SH
    chmod +x "$repo/bin/ap-watch-arm.sh"
    out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_UNRETIRED_READY_FILE="$ready" AP_UNRETIRED_RETIRE_FILE="$retired" AP_RELEASE_FILE="$release" AP_STOP_FILE="$stop" AP_LATE_KIND="$kind" AP_PI_ARM_READY_TIMEOUT_MS=250 AP_WATCH_ARM_RETIRE_TIMEOUT_MS=20 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
writeFileSync(process.env.AP_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  if (rows().length >= 3 && (process.env.AP_LATE_KIND !== "actionable" || prompts.some((message) => message.includes("late wake")))) break;
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (rows().length !== 3) throw new Error(`late close did not restore one successor: ${rows().join(" | ")}`);
if (process.env.AP_LATE_KIND === "actionable") {
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_STOP_FILE="$stop" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" AP_RELEASE_FILE="$release" node --input-type=module 2>&1 <<'EOF'
...
const lock = `${process.env.AP_HOME}/state/.lock`;
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_ARM_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
const assertMissingLock = (result, label) => {
  ...
  if (!result.details.message.includes("bin/ap-session-start.sh") || !result.details.message.includes("re-arm")) {
    throw new Error(`${label} missing reclaim and re-arm guidance: ${result.details.message}`);
  }
  if (result.details.message.includes("held by another autopilot session")) {
...
try {
  ...
  if (liveOther.details.message !== "watcher: read-only - session lock is held by another autopilot session") {
...
if (existsSync(process.env.AP_ARM_LOG)) throw new Error("watcher arm ran without lock ownership");
writeFileSync(lock, `${process.pid}\n`);
...
for (let i = 0; i < 250 && !existsSync(process.env.AP_ARM_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.AP_ARM_LOG)) throw new Error("owned lock did not run the watcher arm");
EOF
)
  status=$?
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_CHILD_PID_FILE="$child_pid_file" AP_ARM_LOG="$arm_log" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
function liveArmPids() {
  if (!existsSync(process.env.AP_ARM_LOG)) return [];
  return readFileSync(process.env.AP_ARM_LOG, "utf8")
    .trim()
    .split(/\n/)
    .filter(Boolean)
    .map((line) => {
...
await waitFor(() => existsSync(process.env.AP_CHILD_PID_FILE), "startup child");
const startupChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
...
async function replaceSession(previous, reason) {
  const previousChild = existsSync(process.env.AP_CHILD_PID_FILE)
    ? readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim()
    : "";
  ...
  await waitFor(() => {
    if (!existsSync(process.env.AP_CHILD_PID_FILE)) return false;
    const child = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
...
const sameInstanceChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
...
const activeChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
...
const finalChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
...
test_pi_process_exit_cleanup_stops_arm_child() {
  ...
  cat > "$repo/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
trap 'printf "%s\n" "$$" >> "$AP_CLEANUP_LOG"; exit 0' TERM
printf '%s\n' "$$" > "$AP_CHILD_PID_FILE"
while :; do sleep 1; done
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" AP_HOME="$home" AP_ROOT_OVERRIDE="$repo" AP_CLEANUP_LOG="$cleanup_log" AP_CHILD_PID_FILE="$pid_file" node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
for (let i = 0; i < 250 && !existsSync(process.env.AP_CHILD_PID_FILE); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.AP_CHILD_PID_FILE)) throw new Error("arm child did not start");
const firstChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
await handlers.get("session_shutdown")?.({ type: "session_shutdown" }, {});
await handlers.get("session_start")?.({ type: "session_start" }, {});
await tool.execute("tool-call-replacement", {}, undefined, undefined, {});
for (let i = 0; i < 250; i += 1) {
  const currentChild = readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim();
  if (currentChild !== firstChild) break;
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (readFileSync(process.env.AP_CHILD_PID_FILE, "utf8").trim() === firstChild) {
...
test_opencode_plugin_package_boundary_is_explicit_esm() {
  ...
  plugin="$fixture/plugins/ap-primary-watch-arm.js"
  ...
  cp "$ROOT/.opencode/plugins/ap-primary-watch-arm.js" "$plugin"
  cp "$ROOT/.opencode/plugins/lib/ap-operational-input.js" "$fixture/plugins/lib/ap-operational-input.js"
...
test_opencode_primary_watch_plugin_uses_effective_state_home() {
  ...
  plugin="$ROOT/.opencode/plugins/ap-primary-watch-arm.js"
  ...
  cat > "$repo/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'home=%s root=%s\n' "${AP_HOME:-}" "${AP_ROOT_OVERRIDE:-}" >> "${AP_ARM_LOG:?}"
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" node 2>&1 <<'EOF'
import { existsSync, readFileSync, realpathSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";

const mod = await import(pathToFileURL(process.env.PLUGIN).href);
const client = { session: { promptAsync: async () => {} } };
const hooks = await mod.AutopilotPrimaryWatchArm({
  client,
...
if (!existsSync(process.env.AP_ARM_LOG)) {
  console.error("watch arm did not run");
  process.exit(1);
}
const text = readFileSync(process.env.AP_ARM_LOG, "utf8");
const expectedRoot = realpathSync(process.env.WORKTREE);
if (!text.includes(`home=${process.env.AP_HOME}`) || !text.includes(`root=${expectedRoot}`)) {
  console.error(text);
  process.exit(1);
}
EOF
  ...
  status=$?
  expect_code 0 "$status" "OpenCode watch plugin must use AP_HOME state outside the repo root"
  [ -z "$out" ] || fail "OpenCode effective-state test printed output: $out"
  pass "OpenCode watcher plugin uses the effective AP_HOME state"
}
...
writeFileSync(`${process.env.AP_HOME}/state/.lock`, "999999\n");
...
test_opencode_watch_arm_coordinator_respects_primary_scope() {
  ...
  ap_git_worktree "$base" "$repo" ap/opencode-coordinator
  mkdir -p "$repo/bin" "$home/state" "$home/config"
  : > "$repo/AGENTS.md"
  : > "$home/state/task.meta"
  cat > "$repo/bin/ap-watch-arm.sh" <<'SH'
...
const mod = await import(pathToFileURL(process.env.PLUGIN).href);
const client = { session: { promptAsync: async () => {} } };
await mod.AutopilotPrimaryWatchArm({
  client,
  directory: process.env.WORKTREE,
  worktree: process.env.WORKTREE,
});
writeFileSync(`${process.env.AP_HOME}/state/.lock`, `${process.pid}\n`);
const status = await globalThis.__autopilotOpenCodeWatchArm.ensureArmed("session-test", client);
await new Promise((resolve) => setTimeout(resolve, 120));
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_STOP_FILE="$stop" node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if [ "$count" -eq 2 ]; then
  ...
  trap 'printf "retired\\n" > "${AP_PRE_READY_RETIRED_FILE:?}"; exit 0' TERM INT
  while [ ! -e "$AP_PRE_READY_RELEASE_FILE" ]; do sleep 0.02; done
  exit 0
fi
printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_PRE_READY_RELEASE_FILE="$release" AP_PRE_READY_RETIRED_FILE="$retired" AP_STOP_FILE="$stop" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (existsSync(process.env.AP_PRE_READY_RETIRED_FILE)) throw new Error("pre-ready actionable successor was retired before its close");
writeFileSync(process.env.AP_PRE_READY_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  const successorRows = readFileSync(process.env.AP_ARM_LOG, "utf8").trim().split("\n");
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_OPENCODE_ARM_READY_TIMEOUT_MS=250 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_RELEASE_FILE="$release" AP_OPENCODE_ARM_READY_TIMEOUT_MS=250 AP_WATCH_ARM_RETIRE_TIMEOUT_MS=20 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
    chmod +x "$repo/bin/ap-watch-arm.sh"
    out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_UNRETIRED_READY_FILE="$ready" AP_UNRETIRED_RETIRE_FILE="$retired" AP_RELEASE_FILE="$release" AP_STOP_FILE="$stop" AP_LATE_KIND="$kind" AP_OPENCODE_ARM_READY_TIMEOUT_MS=250 AP_WATCH_ARM_RETIRE_TIMEOUT_MS=20 AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_STOP_FILE="$stop" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_WATCH_REARM_RETRY_BASE_MS=5 AP_WATCH_REARM_RETRY_MAX_MS=10 AP_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/ap-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_RELEASE_FILE="$release" node 2>&1 <<'EOF'
import { spawn } from "node:child_process";
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
test_opencode_watch_arm_coordinates_with_turnend_guard() {
  ...
  arm_plugin="$ROOT/.opencode/plugins/ap-primary-watch-arm.js"
  guard_plugin="$ROOT/.opencode/plugins/ap-primary-turnend-guard.js"
...
exit 2
SH
  chmod +x "$repo/bin/ap-watch-arm.sh" "$repo/bin/ap-turnend-guard.sh"
  out=$(ARM_PLUGIN="$arm_plugin" GUARD_PLUGIN="$guard_plugin" WORKTREE="$repo" AP_HOME="$home" AP_ARM_LOG="$log" AP_GUARD_LOG="$guard_log" node 2>&1 <<'EOF'
import { existsSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
await armMod.AutopilotPrimaryWatchArm({
  client,
  directory: process.env.WORKTREE,
  worktree: process.env.WORKTREE,
});
const guardHooks = await guardMod.AutopilotPrimaryTurnendGuard({
  client,
  directory: process.env.WORKTREE,
  worktree: process.env.WORKTREE,
});
...
if (existsSync(process.env.AP_GUARD_LOG)) {
...
writeFileSync(`${process.env.AP_HOME}/state/.lock`, `${process.pid}\n`);
await guardHooks.event({ event: { type: "session.idle", properties: { sessionID: "session-test" } } });
for (let i = 0; i < 250 && !existsSync(process.env.AP_GUARD_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.AP_ARM_LOG)) {
  console.error("watch arm did not run");
...
if (!readFileSync(process.env.AP_ARM_LOG, "utf8").includes("args=--restart")) {
