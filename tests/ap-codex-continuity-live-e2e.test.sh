...
if [ "${AP_CODEX_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set AP_CODEX_LIVE_E2E=1 to run the Codex continuity regression"
  exit 0
fi
...
PROMPT='Run exactly `bin/ap-watch-checkpoint.sh --seconds 1` as one foreground shell call. Do not use a background task and do not run ap-watch-arm.sh. After the checkpoint returns, reply briefly.'

(
  cd "$PROJECT" || exit 1
  printf '%s\n' "$$" > "$HOME_DIR/state/.lock"
  AP_HOME="$HOME_DIR" AP_ROOT_OVERRIDE="$PROJECT" codex exec \
