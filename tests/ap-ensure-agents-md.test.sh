#!/usr/bin/env bash
# Behavior tests for bin/ap-ensure-agents-md.sh.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-ensure-agents-md)

test_created_agents_md_includes_self_governance() {
  local repo agents
  repo="$TMP_ROOT/new-project"
  mkdir -p "$repo"
  "$ROOT/bin/ap-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "ap-ensure-agents-md.sh failed for empty project"
  ...
  pass "ap-ensure-agents-md.sh: created AGENTS.md includes self-governance section"
}
...
EOF
  "$ROOT/bin/ap-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "ap-ensure-agents-md.sh failed for CLAUDE.md promotion"
  agents="$repo/AGENTS.md"
  ...
  pass "ap-ensure-agents-md.sh: promoted CLAUDE.md includes self-governance section"
}
...
test_promoted_claude_md_without_trailing_newline_keeps_blank_separator() {
  ...
  "$ROOT/bin/ap-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "ap-ensure-agents-md.sh failed for newline-less CLAUDE.md promotion"
  agents="$repo/AGENTS.md"
  assert_grep "Run tests with make test." "$agents" \
  ...
  pass "ap-ensure-agents-md.sh: newline-less promotion keeps a blank separator line"
}
...
test_existing_agents_md_with_symlink_gains_self_governance() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed for existing AGENTS.md with symlink"
  ...
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed on idempotent re-run"
  ...
  pass "ap-ensure-agents-md.sh: existing symlinked AGENTS.md gains the section idempotently"
}
...
test_existing_agents_md_without_claude_gains_section_and_symlink() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed for existing AGENTS.md without CLAUDE.md"
  ...
  count=$(grep -Fc "## Maintaining this file" "$agents")
  [ "$count" -eq 1 ] || fail "injection wrote $count self-governance sections"
  pass "ap-ensure-agents-md.sh: existing AGENTS.md without CLAUDE.md gains section and symlink"
}
...
test_existing_agents_md_with_section_reports_unchanged() {
  ...
  "$ROOT/bin/ap-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "ap-ensure-agents-md.sh failed building the fully-formed fixture"
  agents="$repo/AGENTS.md"
  cp "$agents" "$repo/.before"
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed on already-formed project"
  ...
  pass "ap-ensure-agents-md.sh: AGENTS.md that already has the section stays unchanged"
}
...
test_existing_crlf_agents_md_with_section_stays_unchanged() {
  ...
  cp "$agents" "$repo/.before"
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed on CRLF AGENTS.md with the section"
  ...
  pass "ap-ensure-agents-md.sh: CRLF AGENTS.md with the section stays unchanged"
}
...
test_existing_crlf_agents_md_without_section_preserves_crlf() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/ap-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "ap-ensure-agents-md.sh failed injecting into CRLF AGENTS.md"
  ...
  cp "$agents" "$repo/.after-first"
  "$ROOT/bin/ap-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "ap-ensure-agents-md.sh failed on idempotent CRLF re-run"
  cmp -s "$repo/.after-first" "$agents" \
    || fail "idempotent CRLF re-run modified AGENTS.md"
  pass "ap-ensure-agents-md.sh: CRLF injection preserves line endings idempotently"
}
...
test_lowercase_agents_md_refuses_case_fragile_symlink() {
  ...
  pass "ap-ensure-agents-md.sh: refuses a case-variant lowercase agents.md (issue #389)"
}
