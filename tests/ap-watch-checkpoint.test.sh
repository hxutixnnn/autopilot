...
CHECKPOINT="$ROOT/bin/ap-watch-checkpoint.sh"
TMP_ROOT=$(ap_test_tmproot ap-watch-checkpoint)
...
test_quiet_checkpoint_exits_124_cleanly() {
  ...
  err="$home/err.txt"
  status=0
  AP_HOME="$home" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 "$CHECKPOINT" --seconds 1 >"$out" 2>"$err" || status=$?
...
test_signal_passes_through_and_exits_zero() {
  ...
  status=0
  AP_HOME="$home" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 "$CHECKPOINT" --seconds 8 >"$out" 2>"$err" || status=$?
  expect_code 0 "$status" "signal checkpoint exit"
  assert_contains "$(cat "$out")" "signal:" "signal wake was not passed through"
  drained=$(AP_HOME="$home" "$ROOT/bin/ap-wake-drain.sh")
...
test_registered_check_uses_preserved_watcher_environment() {
  ...
  out="$home/out.txt"
  err="$home/err.txt"
  printf '%s\n' ap-pr-check-migration-scan-v1 > "$home/state/.pr-check-migration-scan-v1"
  printf '%s\n' ap-pr-check-migration-v1 > "$home/state/.pr-check-migration-v1"
  chmod 0600 "$home/state/.pr-check-migration-scan-v1" "$home/state/.pr-check-migration-v1"
  cat > "$home/state/env-check.check.sh" <<'SH'
#!/usr/bin/env bash
printf 'env check fired with AP_CHECK_INTERVAL=%s\n' "${AP_CHECK_INTERVAL:-missing}"
SH
  chmod 0700 "$home/state/env-check.check.sh"
  AP_HOME="$home" "$ROOT/bin/ap-check-register.sh" env-check >/dev/null \
    || fail "could not register checkpoint custom check"
  status=0
  AP_HOME="$home" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=1 "$CHECKPOINT" --seconds 5 >"$out" 2>"$err" || status=$?
  expect_code 0 "$status" "check checkpoint exit"
  assert_contains "$(cat "$out")" "check:" "check wake was not passed through"
  assert_contains "$(cat "$out")" "AP_CHECK_INTERVAL=1" "watcher environment was not preserved"
...
test_existing_singleton_watcher_is_not_success() {
  ...
  status=0
  AP_HOME="$home" AP_GUARD_GRACE=300 "$CHECKPOINT" --seconds 5 >"$out" 2>"$err" || status=$?
