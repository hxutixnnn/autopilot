#!/usr/bin/env bash
# Tests for bin/ap-review-diff.sh: when a task has an open PR recorded in meta,
...
REVIEW_DIFF="$ROOT/bin/ap-review-diff.sh"
TMP_ROOT=$(ap_test_tmproot ap-review-diff-tests)
...
stale_and_pr_commits() {
  ...
  git -C "$case_dir/wt" checkout -q ap/task-x1
}
...
test_pr_meta_uses_pr_head_not_stale_local() {
  ...
  pass "ap-review-diff falls back to recorded pr_head when pull head cannot be fetched"
}
...
test_stale_recorded_pr_head_loses_to_fetched_pull_head() {
  ...
  stale_and_pr_commits "$case_dir"
  stale_sha=$(git -C "$case_dir/wt" rev-parse ap/task-x1)
  ...
  pass "ap-review-diff prefers freshly fetched PR head over a stale recorded pr_head="
}
...
test_pr_meta_fetches_pull_head_without_recorded_sha() {
  ...
  pass "ap-review-diff fetches refs/pull/<n>/head when pr_head= is absent"
}
...
test_no_pr_meta_uses_local_branch() {
  ...
  pass "ap-review-diff without pr= keeps the worktree-branch diff"
}
...
test_unreachable_pr_head_falls_back_with_warning() {
  ...
  pass "ap-review-diff falls back to local branch with a warning when PR head is unreachable"
}
