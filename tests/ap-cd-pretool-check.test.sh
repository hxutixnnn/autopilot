...
#
# bin/ap-cd-command-policy.mjs is the single owner of the block/allow decision;
# it reuses the shell classifier owned by bin/ap-arm-command-policy.mjs.
# bin/ap-cd-pretool-check.sh is the stable transport: it scopes the guard to the
...
# scoping (including the deliberate copilot-home difference from the turn-end
...
TMP_ROOT=$(ap_test_tmproot ap-cd-pretool-check)

# A primary-shaped checkout: plain (non-worktree) git repo, AGENTS.md, bin/ with
# the transport plus both policy files (ap-cd-command-policy.mjs imports the
# shared classifier from ap-arm-command-policy.mjs). This is what the transport's
# scoping treats as the real primary autopilot checkout.
install_cd_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/ap-cd-pretool-check.sh" "$dir/bin/ap-cd-pretool-check.sh"
  cp "$ROOT/bin/ap-cd-command-policy.mjs" "$dir/bin/ap-cd-command-policy.mjs"
  cp "$ROOT/bin/ap-arm-command-policy.mjs" "$dir/bin/ap-arm-command-policy.mjs"
  chmod +x "$dir/bin/ap-cd-pretool-check.sh" "$dir/bin/ap-cd-command-policy.mjs"
}
...
# Same shape as primary plus the .ap-copilot-home marker: a copilot's own
# primary session, which the cd-guard DOES guard (unlike the turn-end guard).
make_copilot_fixture() {
  local dir=$1
...
# A genuine linked git worktree - the shape bin/ap-spawn.sh hands flight crew member/recon
# tasks. git-dir and git-common-dir differ, so the guard must be inert.
make_child_worktree_fixture() {
  local base=$1 dir=$2
  ap_git_worktree "$base" "$dir" ap/cd-guard-test-branch
  : > "$dir/AGENTS.md"
  install_cd_scripts "$dir"
  printf '%s\n' "$dir"
}
...
CHECK="$PRIMARY/bin/ap-cd-pretool-check.sh"
...
MATRIX_TMP=$(mktemp -d "${TMPDIR:-/tmp}/ap-cd-policy-matrix.XXXXXX")
...
test_fires_in_copilot_home() {
  local dir out rc
  dir=$(make_copilot_fixture "$TMP_ROOT/copilot")
  out=$("$dir/bin/ap-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 2 "$rc" "cd-guard must fire in a copilot's own primary session (unlike the turn-end guard)"
  assert_contains "$out" '[persistent-cd]' "copilot-home block must carry the reason code"
  pass "cd-guard: fires in a copilot home (its own primary session is a primary)"
}
...
test_inert_in_child_worktree() {
  ...
  out=$("$dir/bin/ap-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 0 "$rc" "cd-guard must be inert in a flight crew member/recon linked worktree"
  [ -z "$out" ] || fail "cd-guard produced output in a child worktree: $out"
  pass "cd-guard: inert in a flight crew member/recon task worktree (linked git worktree)"
}

test_inert_when_not_autopilot_repo() {
  local dir out rc
  dir="$TMP_ROOT/not-autopilot"
  git init -q "$dir"
  git -C "$dir" commit -q --allow-empty -m init
  install_cd_scripts "$dir"   # bin/ present but no AGENTS.md
  out=$("$dir/bin/ap-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 0 "$rc" "cd-guard must be inert without AGENTS.md (not an autopilot checkout)"
  [ -z "$out" ] || fail "cd-guard produced output outside an autopilot checkout: $out"
  pass "cd-guard: inert in a non-autopilot repo (no AGENTS.md)"
}
...
test_e2e_cwd_leak_regression() {
  ...
  # `cd projects/clone` makes the next autopilot-owned backlog write land in the
...
test_fail_open_missing_node() {
  ...
  fakebin=$(ap_fakebin "$TMP_ROOT/nonode")
...
test_fail_open_missing_jq_on_stdin() {
  local fakebin tool tool_path out rc
  fakebin=$(ap_fakebin "$TMP_ROOT/nojq")
...
test_prefilter_skips_node_without_cd_substring() {
  ...
  make_primary_fixture "$dir" >/dev/null
  fakebin=$(ap_fakebin "$TMP_ROOT/prefilter-fake")
...
EOF
  ...
  out=$(PATH="$fakebin" "$dir/bin/ap-cd-pretool-check.sh" --command 'git status' 2>&1); rc=$?
...
test_policy_cli_direct() {
  local policy
  policy="$ROOT/bin/ap-cd-command-policy.mjs"
  ...
  pass "cd-guard: ap-cd-command-policy.mjs CLI honors the deny/allow output contract"
}
...
test_scripts_are_shellcheck_clean() {
  ...
  shellcheck "$ROOT/bin/ap-cd-pretool-check.sh" >/dev/null 2>&1 \
    || fail "bin/ap-cd-pretool-check.sh is not shellcheck-clean"
  pass "bin/ap-cd-pretool-check.sh is shellcheck-clean"
}
