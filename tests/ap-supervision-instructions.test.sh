...
TMP_ROOT=$(ap_test_tmproot ap-supervision-instructions)
RENDER="$ROOT/bin/ap-supervision-instructions.sh"
...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "bin/ap-watch-checkpoint.sh" "codex checkpoint helper missing"
...
test_conditional_stanzas() {
  ...
  out=$(AP_HOME="$home" AP_CONFIG_OVERRIDE="$config" "$RENDER" --harness codex --read-only 1 --afk 1)
...
test_repair_lines() {
  ...
  out=$(AP_HOME="$home" AP_CODEX_WATCH_CHECKPOINT=7 "$RENDER" --harness codex --repair-line)
  assert_contains "$out" "bin/ap-watch-checkpoint.sh --seconds 7" "codex repair line did not use checkpoint helper and env override"

  out=$(AP_HOME="$home" "$RENDER" --harness claude --queue-pending 1 --repair-line)
  ...
  out=$(AP_HOME="$home" AP_CODEX_WATCH_CHECKPOINT=7 "$RENDER" --harness codex --repair-line)
  assert_contains "$out" "bin/ap-watch-checkpoint.sh --seconds 7" "codex repair line lost the checkpoint helper"

  out=$(AP_HOME="$home" "$RENDER" --harness opencode --read-only 1 --repair-line)
  assert_contains "$out" "session holding the fleet lock" "read-only repair line missing"

  out=$(AP_HOME="$home" "$RENDER" --harness pi --repair-line)
  assert_contains "$out" "Pi tool ap_watch_arm_pi" "pi repair line does not direct the model to the extension-owned tool"
  assert_not_contains "$out" "extension command /ap-watch-arm-pi" "pi repair line still directs the model to the human slash command"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  assert_not_contains "$ordinary" "ap_watch_arm_pi" "pi ordinary-wake line incorrectly calls the recovery tool"
  out=$("$RENDER" --harness pi --repair-line)
  assert_contains "$out" "ap_watch_arm_pi" "pi recovery line lost the extension-owned repair tool"
  ...
  assert_not_contains "$ordinary" "bin/ap-watch-arm.sh" "opencode ordinary-wake line incorrectly calls the recovery probe"
  ...
  assert_contains "$ordinary" "bin/ap-claude-stop-autoarm.sh" "claude ordinary-wake line lost the auto-arm script name"
  assert_contains "$ordinary" "do not arm another cycle" "claude ordinary-wake line does not forbid a model re-arm"
  assert_not_contains "$ordinary" "bin/ap-watch-arm.sh" "claude ordinary-wake line incorrectly calls the manual arm"
  ...
  assert_contains "$out" "bin/ap-watch-arm.sh" "claude recovery line lost the arm command"
  ...
  assert_contains "$ordinary" "bin/ap-watch-arm.sh" "grok ordinary-wake line lost the background arm command"
  ...
  assert_contains "$out" "bin/ap-watch-arm.sh" "grok recovery line lost the arm command"

  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "next foreground" "codex ordinary-wake line lost its foreground checkpoint"
  assert_contains "$ordinary" "bin/ap-watch-checkpoint.sh" "codex ordinary-wake line lost the checkpoint command"
  assert_not_contains "$ordinary" "bin/ap-watch-arm.sh" "codex ordinary-wake line incorrectly uses a background arm"
  ...
  assert_contains "$out" "bin/ap-watch-checkpoint.sh" "codex recovery line lost the checkpoint command"
...
test_pi_signed_preserves_identity_with_pi_supervision_protocol() {
  ...
  assert_contains "$out" "Pi tool ap_watch_arm_pi" \
...
test_grok_is_background_notify() {
  ...
  assert_contains "$out" "bin/ap-watch-arm.sh" "grok snippet missing watcher arm"
...
test_pi_snippet_uses_effective_extension_path() {
  ...
  turnend="$ROOT/.pi/extensions/ap-primary-turnend-guard.ts"
  watch="$ROOT/.pi/extensions/ap-primary-pi-watch.ts"
  mkdir -p "$home/state" "$home/config"
  out=$(AP_HOME="$home" "$RENDER" --harness pi)
  ...
  assert_not_contains "$out" "__AP_PI_EXT__" "renderer leaked the Pi extension path placeholder"
  assert_not_contains "$out" "__AP_PI_TURNEND_EXT__" "renderer leaked the Pi turn-end extension path placeholder"
  assert_not_contains "$out" "state/ap-primary-pi-watch.ts" "pi snippet kept the old generated state-relative extension path"
