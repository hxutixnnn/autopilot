#!/usr/bin/env bash
# tests/ap-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
#     A pilot-relevant status must deliver exactly ONE sentinel-prefixed,
...
# private socket. The daemon points at a throwaway state dir (AP_STATE_OVERRIDE)
# and the test pane (AP_SUPERVISOR_TARGET). Nothing touches the live fleet.
# AP_SUPERVISOR_BACKEND=tmux is passed explicitly (not left to auto-detection):
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get AP_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ap-shim.XXXXXX")
...
# Create a fake flight crew member window (the watcher lists ap-* windows for stale
# detection). The pane is an inert shell - it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n ap-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  AP_STATE_OVERRIDE="$STATE_DIR" \
  AP_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  AP_SUPERVISOR_BACKEND=tmux \
...
test_scenario_a() {
  ...
  # pilot returning and starting to type before afk has been cleared.
  ...
  # Write a pilot-relevant status to trigger a real escalation through the
  # real watcher child.
...
test_scenario_b() {
  ...
  # Write a pilot-relevant status to trigger a real escalation.
...
# No human input, no swallowed Enter: a pilot-relevant status must produce
...
test_scenario_c() {
  ...
  stop_daemon
  pass "Scenario C: a normal pilot status injects exactly one clean single-line sentinel digest"
}
