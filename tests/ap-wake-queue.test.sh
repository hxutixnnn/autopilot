#!/usr/bin/env bash
# tests/ap-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in ap-watcher-lock.test.sh; daemon classification/injection in
# ap-daemon.test.sh.
set -u
...
TMP_ROOT=$(ap_test_tmproot ap-wake-tests)
...
test_concurrent_append_and_drain() {
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pids="$pids $!"
  for pid in $pids; do
    wait "$pid" || fail "concurrent append/drain subprocess failed"
  done
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "final drain failed"
  cat "$out1" "$out2" > "$all"
...
test_signal_catchup_without_running_watcher() {
  ...
  # watcher can absorb no-verb working: notes when the flight crew is provably working).
  # Use a pilot-relevant verb so the wake is surfaced and the catch-up path is
  # tested.
  printf 'blocked: first\n' > "$status_file"
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after first signal failed"
...
test_stale_enqueue_before_suppressor() {
  ...
  window="test:ap-stale"
  printf 'idle prompt' > "$capture_file"
  printf 'window=%s\nkind=flight\n' "$window" > "$state/stale.meta"
  # A stale pane sitting on a pilot-relevant status is actionable when the flight crew
  ...
  printf 'done: ready in branch ap/stale\n' > "$state/stale.status"
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$window" AP_FAKE_TMUX_CAPTURE="$capture_file" AP_STATE_OVERRIDE="$state" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after stale wake failed"
...
# whose flight crew is NOT provably working is surfaced immediately. That new path must keep
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # Non-terminal status (no pilot-relevant verb); prime .seen-* so the per-poll
  ...
  # fake ap-flight-crew-state.sh the watcher reads via AP_FLIGHT_CREW_STATE_BIN.)
  ...
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$window" AP_FAKE_TMUX_CAPTURE="$capture_file" \
    AP_STATE_OVERRIDE="$state" AP_FLIGHT_CREW_STATE_BIN="$fakebin/ap-flight-crew-state.sh" \
    AP_STALE_ESCALATE_SECS=999 AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=999999 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after the immediate stale wake failed"
...
SH
  ...
  PATH="$fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_POLL=1 AP_SIGNAL_GRACE=1 AP_CHECK_INTERVAL=0 AP_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after check wake failed"
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:ap-task' 'stale: s:ap-task' || fail "stale append failed"
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pid1=$!
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" &
  pid2=$!
  ...
  leftover=$(AP_STATE_OVERRIDE="$state" "$DRAIN" | awk 'NF { count++ } END { print count + 0 }')
...
test_drain_dedupes_obvious_duplicates() {
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "dedupe drain failed"
...
# watcher liveness via ap-guard.sh: a lapsed re-arm chain then surfaces even on a
...
test_drain_asserts_watcher_liveness() {
  ...
  state="$dir/state"
  err="$dir/drain.err"
  printf 'window=test:ap-x\nkind=flight\n' > "$state/x.meta"
  AP_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || fail "drain failed while asserting liveness"
  ...
  AP_STATE_OVERRIDE="$state" AP_GUARD_GRACE=300 "$DRAIN" >/dev/null 2> "$err" || fail "drain failed with a fresh beacon"
...
if [ "${1:-}" = -MFcntl=:DEFAULT ]; then
  for arg in "$@"; do
    if [ "$arg" = "${AP_WAKE_ENRICH_SWAP_PATH:-}" ]; then
      rm -f "$arg"
      ln -s "$AP_WAKE_ENRICH_SWAP_TARGET" "$arg"
      break
    fi
  done
fi
exec "$AP_WAKE_ENRICH_REAL_PERL" "$@"
SH
  ...
  append_wake "$state" stale test:ap-task "stale: test:ap-task" || fail "stale wake append failed"
  append_wake "$state" heartbeat heartbeat heartbeat || fail "heartbeat wake append failed"

  AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_wake_print_deduped "$2"' _ \
    "$ROOT/bin/ap-wake-lib.sh" "$state/.wake-queue" > "$expected"
  PATH="$dir/fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_WAKE_ENRICH_SWAP_PATH="$state/task.status" \
    AP_WAKE_ENRICH_SWAP_TARGET="$outside" AP_WAKE_ENRICH_REAL_PERL="$perl_bin" "$DRAIN" > "$out" \
...
SH
  ...
  PATH="$dir/fakebin:$PATH" AP_STATE_OVERRIDE="$state" AP_WAKE_ENRICH_PERL_LOG="$fake_perl_log" \
    AP_WAKE_ENRICH_REAL_PERL="$perl_bin" "$DRAIN" > "$out" \
...
test_slow_annotation_does_not_block_append_and_deleted_file_fails_open() {
  ...
  AP_STATE_OVERRIDE="$state" AP_WAKE_ENRICH_TEST_DELAY=3 "$DRAIN" > "$out1" &
  pid=$!
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "follow-up drain after concurrent append failed"
...
test_interruption_before_and_after_raw_commit() {
  ...
  AP_STATE_OVERRIDE="$state" AP_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT=5 "$DRAIN" > "$before_out" &
  pid=$!
  i=0
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$replay_out" || fail "restored pre-commit wake did not drain"
  ...
  AP_STATE_OVERRIDE="$state" AP_WAKE_ENRICH_TEST_DELAY=5 "$DRAIN" > "$after_out" &
  pid=$!
  ...
  wait "$pid"
  set -e
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$empty_out" || fail "drain after post-commit interruption failed"
