#!/usr/bin/env bash
# tests/ap-backend-herdr-eventwait-smoke.test.sh - REAL-herdr smoke test for the
# native pane.agent_status_changed push escalation (ap_backend_herdr_wait_transition,
...
# herdr_safe_stop_and_delete on a private ap-lab-* session, never a bare/ambient
# `herdr server stop`. Every lifecycle op goes through bin/ap-herdr-lab.sh, which
...
SESSION="ap-lab-eventwait-smoke-$$"
...
ap_herdr_lab_prepare "$SESSION" || fail "could not prepare the isolated Herdr lab session"
...
if ! ap_backend_herdr_events_capable "$SESSION"; then
...
IDS=$(ap_backend_herdr_create_task "$CONTAINER" "ap-evwait1" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# scratch autopilot state so window_to_task and the wake queue resolve
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/ap-evwait.XXXXXX")
...
ap_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source ap-evwait-test --agent claude --state idle >/dev/null 2>&1 \
...
( ap_backend_herdr_wait_transition "$SESSION" 8 "$STATE" "$TARGET" > "$OUT"; echo $? > "$RCF" ) &
WPID=$!
sleep 0.5   # let it connect, subscribe, and reconcile the idle baseline

START=$(python3 -c 'import time; print(time.time())')
ap_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source ap-evwait-test --agent claude --state blocked >/dev/null 2>&1 \
...
REC_PANE=$(ap_transition_pane_id "$REC")
REC_TO=$(ap_transition_to_status "$REC")
...
# shellcheck source=/dev/null
. "$ROOT/bin/ap-push-transition-lib.sh"
wake() { return 0; }
