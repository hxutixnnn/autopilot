#!/usr/bin/env bash
# Behavior tests for ap-fleet-sync.sh drift handling.
#
# ap-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
# (fetch_with_packed_refs_lock_guard, backed by bin/ap-lock-lib.sh's shared
...
TMP_ROOT=$(ap_test_tmproot ap-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated AP_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/ap-fleet-sync.sh" "$@" 2>/dev/null
}
...
# lsof shims mirror tests/ap-teardown.test.sh: no-holder (provably free), a live
...
# (AP_FLEET_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
run_sync_guarded() {
  ...
  AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/ap-fleet-sync.sh" "$@" >"$outf" 2>"$errf"
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  # Full bootstrap: no state/ dir -> copilot sync no-ops.
  # We only assert the fleet-sync relay lines; other detect lines are irrelevant.
  out=$(AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null)
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  AP_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_transient_packed_refs_lock_self_clears() {
  ...
  AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
