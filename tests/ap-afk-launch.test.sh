#!/usr/bin/env bash
# tests/ap-afk-launch.test.sh - the script-owned, backend-aware away-daemon
# launch (bin/ap-afk-launch.sh) and the away-mode stale-artifact lifecycle fixes
# (bin/ap-afk-start.sh). Two layers:
#
...
#   the pilot's active tab topology UNCHANGED, because the daemon lands in a
#   NON-VISIBLE separate terminal (a herdr dedicated workspace, a detached tmux
#   session), never a split of the pilot's pane. The herdr path runs on a
#   throwaway, NEVER-default HERDR_SESSION and asserts the default session is
#   byte-identical via the ap-herdr-lab.sh fleet-state tripwire; the tmux path
#   uses uniquely-named throwaway sessions killed by exact name. A harmless
#   sleeper replaces the real daemon (AP_AFK_LAUNCH_ENTRY) so the test observes
#   only the terminal lifecycle.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LAUNCH="$ROOT/bin/ap-afk-launch.sh"
START="$ROOT/bin/ap-afk-start.sh"

FAILED=0
fail() { printf 'not ok - %s\n' "$1" >&2; FAILED=1; }
pass() { printf 'ok - %s\n' "$1"; }

SLEEPER=$(mktemp "${TMPDIR:-/tmp}/ap-afk-sleeper.XXXXXX")
...
# ---------------------------------------------------------------------------
# UNIT 1: ap_afk_clear_stale_artifacts removes exactly the three stale artifacts.
# ---------------------------------------------------------------------------
unit_clear_stale() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-clear.XXXXXX")
  mkdir -p "$st/state"
  ...
  # Source ap-afk-start.sh inside a child bash (it sets `set -eu` and would
  # otherwise leak that into this test shell) and call the clear helper.
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" \
    bash -c '. "$1"; ap_afk_clear_stale_artifacts "$2"' _ "$START" "$st/state"
...
unit_relative_paths_are_absolute_before_daemon_launch() {
  ...
  root=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-relative-home.XXXXXX")
  mkdir -p "$root/home/state" "$root/cdpath/home/state"
  home=$(cd "$root/home" && pwd -P)
  state="$home/state"
  out=$(
    cd "$root" || exit 1
    CDPATH="$root/cdpath" AP_HOME=home AP_STATE_OVERRIDE=home/state \
      bash -c '. "$1"; printf "%s\n%s\n" "$AP_HOME" "$AP_AFK_LAUNCH_STATE"' _ "$LAUNCH"
  )
  if [ "$out" = "$home"$'\n'"$state" ]; then
  ...
  out=$(AP_HOME="$linked_home" AP_STATE_OVERRIDE="$linked_home/state" \
    bash -c '. "$1"; printf "%s\n%s\n" "$AP_HOME" "$AP_AFK_LAUNCH_STATE"' _ "$LAUNCH")
  ...
  out=$(
    cd "$root" || exit 1
    AP_HOME=missing-home "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "AP_HOME directory cannot be resolved: missing-home" >/dev/null; then
    pass "launcher paths: unresolved relative AP_HOME fails loudly"
  else
    fail "launcher paths: unresolved relative AP_HOME did not name the bad input ($out)"
  fi
  out=$(
    cd "$root" || exit 1
    AP_HOME=home AP_STATE_OVERRIDE=missing-state "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "AP_STATE_OVERRIDE directory cannot be resolved: missing-state" >/dev/null; then
    pass "launcher paths: unresolved relative AP_STATE_OVERRIDE fails loudly"
  else
    fail "launcher paths: unresolved relative AP_STATE_OVERRIDE did not name the bad input ($out)"
  fi
  rm -rf "$root"
}
...
unit_fresh_vs_refresh() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-refresh.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  : > "$st/state/.subsuper-inject-wedged"
  ...
  ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$sleep_pid" > "$lock/pid-identity" 2>/dev/null ) || true
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" "$START" >/dev/null 2>&1
...
# ---------------------------------------------------------------------------
# UNIT 3: exit ordering - ap_afk_launch_stop SIGTERMs the daemon WHILE .afk is
# still present (so its flush is not a no-op), and clears .afk last.
# ---------------------------------------------------------------------------
unit_stop_ordering() {
  local st lock marker daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-stop.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  ...
  ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$daemon_pid" > "$lock/pid-identity" 2>/dev/null ) || true
  printf 'none\t-\tnative\n' > "$st/state/.afk-daemon-terminal"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1
...
unit_stop_rejects_reused_pid() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-pid-reuse.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  sleep 600 &
  sleeper_pid=$!
...
unit_failed_start_rolls_back_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-failed-start.XXXXXX")
  mkdir -p "$st/state"
  printf 'pending\n' > "$st/state/.subsuper-escalations"
  printf 'wedged\n' > "$st/state/.subsuper-inject-wedged"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_SUPERVISOR_TARGET=unused \
    AP_SUPERVISOR_BACKEND=unsupported "$LAUNCH" start >/dev/null 2>&1; then
...
unit_concurrent_start_serialized() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-concurrent.XXXXXX")
  cap_session="ap-afk-concurrent-cap-$$"
  tmux new-session -d -s "$cap_session" 2>/dev/null || { fail "concurrent start: pilot session creation failed"; rm -rf "$st"; return 0; }
  TRACK_TMUX_SESSIONS="$TRACK_TMUX_SESSIONS $cap_session"
  cap_pane=$(tmux display-message -p -t "$cap_session" '#{pane_id}')
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_SUPERVISOR_TARGET="$cap_pane" \
    AP_SUPERVISOR_BACKEND=tmux AP_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 & first=$!
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_SUPERVISOR_TARGET="$cap_pane" \
    AP_SUPERVISOR_BACKEND=tmux AP_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 & second=$!
...
unit_lock_initialization_grace() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-lock-init.XXXXXX")
  ...
  (
    ...
    if [ -d "$st/state/.afk-launch.lock" ]; then
      ...
      ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$$" > "$st/state/.afk-launch.lock/pid-identity" 2>/dev/null ) || true
      : > "$marker"
      sleep 0.15
  ...
  initializer=$!
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
unit_signal_exits_with_lock_cleanup() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-signal.XXXXXX")
  marker="$st/resumed"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_afk_launch_start() { sleep 30; }
    ap_afk_launch_main start
    : > "$2"
  ' _ "$LAUNCH" "$marker" &
  child=$!
...
unit_herdr_partial_create_recovery() {
  local st recorded
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-herdr-partial.XXXXXX")
  recorded="$st/recorded"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_AFK_LAUNCH_ENTRY=/bin/true \
    AP_AFK_LAUNCH_LABEL=afk-exact-label RECORDED="$recorded" bash -c '
    . "$1"
    ap_backend_source() { return 0; }
    ap_backend_herdr_server_ensure() { return 0; }
    ...
    ap_afk_launch_record_write() { printf "%s:%s:%s" "$1" "$2" "$3" > "$RECORDED"; }
    ap_afk_launch_create_herdr lab:pilot herdr
  ' _ "$LAUNCH"
...
unit_herdr_error_with_exact_ids_closes_exact() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-herdr-error-exact.XXXXXX")
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_backend_source() { return 0; }
    ap_backend_herdr_server_ensure() { return 0; }
...
unit_herdr_run_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-herdr-run-fail.XXXXXX")
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_backend_source() { return 0; }
    ap_backend_herdr_server_ensure() { return 0; }
...
unit_record_failure_closes_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-record-fail.XXXXXX")
  closed="$st/closed"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    ap_afk_launch_record_write() { return 1; }
    ap_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    ! ap_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_rolls_back_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-not-ready.XXXXXX")
  closed="$st/closed"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    ap_afk_launch_wait_ready() { return 1; }
    ap_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    ap_afk_launch_terminal_absent() { [ -e "$CLOSED" ]; }
    ! ap_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-not-ready-unconfirmed.XXXXXX")
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_afk_launch_wait_ready() { return 1; }
    ap_afk_launch_close_terminal() { return 1; }
    ap_afk_launch_terminal_absent() { return 1; }
    ! ap_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_tmux_absence_distinguishes_probe_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-tmux-probe.XXXXXX")
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() { printf "%s" "can'\''t find session: exact-session" >&2; return 1; }
    ap_afk_launch_terminal_absent tmux exact-session
...
unit_native_lifecycle() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-native.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" "$LAUNCH" start-native >/dev/null 2>&1 \
...
unit_native_entry_preserves_prepared_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-native-entry.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  : > "$st/state/.subsuper-escalations"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_AFK_STATE_PREPARED=1 bash -c '
    . "$1"
    AP_AFK_DAEMON=/bin/true
    ap_afk_start_main
...
unit_close_failure_preserves_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-close-fail.XXXXXX")
  mkdir -p "$st/state"
...
unit_record_publication_atomic() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-record-atomic.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\told-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    mv() { return 1; }
    ! ap_afk_launch_record_write tmux new-session owned
  ' _ "$LAUNCH" \
...
unit_malformed_record_fails_closed() {
  local st acted
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-record-malformed.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  acted="$st/acted"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" ACTED="$acted" bash -c '
    . "$1"
    ap_afk_launch_close_terminal() { : > "$ACTED"; }
    ! ap_afk_launch_reconcile
  ' _ "$LAUNCH" \
...
unit_stop_malformed_record_fails_closed() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-stop-malformed.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
...
unit_tmux_planned_record_and_collision() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-tmux-plan.XXXXXX")
  mkdir -p "$st/state"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      if [ "$1" = new-session ]; then
        [ -s "$AP_AFK_LAUNCH_RECORD" ] || return 9
        printf "%s" "$4" > "$AP_HOME/created-name"
        return 1
      fi
      [ "$1" != kill-session ] || : > "$AP_HOME/killed"
      return 1
    }
    ! ap_afk_launch_create_tmux pilot:0 tmux
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-tmux-unique.XXXXXX")
  mkdir -p "$st/state"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      [ "$1" != new-session ] || { printf "%s" "$4" > "$AP_HOME/created-name"; return 1; }
      [ "$1" != kill-session ] || : > "$AP_HOME/killed"
      return 1
    }
    ! ap_afk_launch_create_tmux pilot:0 tmux
...
unit_stop_validates_before_signal() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-stop-validate.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  ...
  ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$sleeper_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1 || true
...
unit_lock_requires_complete_metadata() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-lock-metadata.XXXXXX")
  mkdir -p "$st/state"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_pid_identity() { return 1; }
    ! ap_afk_launch_lock_acquire
...
unit_stop_surfaces_afk_removal_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-stop-remove.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    rm() { local last=${!#}; [ "$last" != "$AP_AFK_LAUNCH_STATE/.afk" ]; }
    ! ap_afk_launch_stop
  ' _ "$LAUNCH"; then
...
unit_stop_confirms_daemon_exit() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-stop-live.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  : > "$st/state/.afk"
  printf 'none\t-\tnative\n' > "$st/state/.afk-daemon-terminal"
  ...
  ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    seq() { printf "1\n"; }
    sleep() { :; }
    kill() {
      command kill "$@"
      if [ "$1" = -TERM ]; then
        rm -rf "$AP_AFK_LAUNCH_STATE/.supervise-daemon.lock"
      fi
    }
    ! ap_afk_launch_stop
...
unit_refresh_validates_record() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-refresh-record.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  sleep 30 & daemon_pid=$!
  printf '%s' "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid"
  ( . "$ROOT/bin/ap-wake-lib.sh"; ap_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" AP_SUPERVISOR_TARGET=unused \
    AP_SUPERVISOR_BACKEND=tmux bash -c '
      . "$1"
      ! ap_afk_launch_start && ! ap_afk_launch_start_native
...
unit_clear_failure_aborts_entry() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-clear-fail.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_afk_launch_reconcile() { return 0; }
    ap_afk_clear_stale_artifacts() { return 1; }
...
unit_confirmed_absence_succeeds() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-confirmed-absent.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\texact-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    ap_afk_launch_close_terminal() { return 1; }
    ap_afk_launch_terminal_absent() { return 0; }
    ap_afk_launch_reconcile
...
unit_incomplete_restore_retains_backup() {
  local st backup
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-restore-fail.XXXXXX")
  mkdir -p "$st/state"
  ...
  if AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    cp() { return 1; }
    ! ap_afk_launch_restore_backup "$2" 1
...
unit_flag_write_failure_aborts() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-flag-fail.XXXXXX")
  mkdir -p "$st/state"
  AP_HOME="$st" AP_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
e2e_herdr() {
  ...
  SESSION="ap-lab-afk-launch-e2e-$$"
  export HERDR_SESSION="$SESSION"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-e2e-home.XXXXXX")
  E2E_HERDR_CLEANUP() {
    AP_HOME="$home_tmp" AP_STATE_OVERRIDE="$home_tmp/state" \
      AP_SUPERVISOR_TARGET="$target" AP_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1 || true
    herdr_safe_stop_and_delete "$SESSION" >/dev/null 2>&1 || true
    rm -rf "$home_tmp" 2>/dev/null || true
  }
  ap_herdr_lab_prepare "$SESSION" || { fail "herdr e2e: could not prepare isolated lab session"; return 0; }
  ap_backend_source herdr || { E2E_HERDR_CLEANUP; fail "herdr e2e: ap_backend_source herdr failed"; return 0; }
  ap_backend_herdr_server_ensure "$SESSION" || { E2E_HERDR_CLEANUP; fail "herdr e2e: lab server did not start"; return 0; }

  out=$(ap_backend_herdr_cli "$SESSION" workspace create --cwd "$ROOT" --label pilot --no-focus 2>/dev/null)
  ...
  if [ -z "$cap_ws" ] || [ -z "$cap_pane" ]; then E2E_HERDR_CLEANUP; fail "herdr e2e: could not create pilot workspace"; return 0; fi
  target="$SESSION:$cap_pane"
  before=$(ap_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_before=$(ap_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')

  AP_HOME="$home_tmp" AP_STATE_OVERRIDE="$home_tmp/state" \
    AP_SUPERVISOR_TARGET="$target" AP_SUPERVISOR_BACKEND=herdr AP_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1

  during=$(ap_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_during=$(ap_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
  dtgt=$(cut -f2 "$home_tmp/state/.afk-daemon-terminal" 2>/dev/null || true)
  dtab=$(ap_backend_herdr_cli "$SESSION" pane get "${dtgt#*:}" 2>/dev/null | jq -r '.result.pane.tab_id // empty')

  if [ "$before" = "$during" ]; then pass "herdr e2e: pilot tab pane count unchanged after start (no split)"; else fail "herdr e2e: pilot tab pane count changed ($before -> $during)"; fi
  ...
  if [ -n "$dtab" ] && [ "$dtab" != "$cap_tab" ]; then pass "herdr e2e: daemon pane is NOT in the pilot's tab"; else fail "herdr e2e: daemon pane shares the pilot tab ($dtab)"; fi
  ...
  AP_HOME="$home_tmp" AP_STATE_OVERRIDE="$home_tmp/state" \
    AP_SUPERVISOR_TARGET="$target" AP_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1

  after=$(ap_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_after=$(ap_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
  if [ "$after" = "$before" ]; then pass "herdr e2e: pilot tab pane count restored after stop"; else fail "herdr e2e: pilot tab pane count not restored ($before -> $after)"; fi
...
# ---------------------------------------------------------------------------
# E2E tmux: topology invariant (pilot window untouched; daemon in a separate
# detached session).
# ---------------------------------------------------------------------------
e2e_tmux() {
  ...
  cap_session="ap-afk-launch-cap-$$"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-afk-tmux-home.XXXXXX")
  tmux new-session -d -s "$cap_session" 2>/dev/null || { fail "tmux e2e: could not create pilot session"; rm -rf "$home_tmp"; return 0; }
  TRACK_TMUX_SESSIONS="$TRACK_TMUX_SESSIONS $cap_session"
  cap_pane=$(tmux display-message -p -t "$cap_session" '#{pane_id}')
  before=$(tmux list-panes -t "$cap_session" | wc -l | tr -d ' ')

  AP_HOME="$home_tmp" AP_STATE_OVERRIDE="$home_tmp/state" \
    AP_SUPERVISOR_TARGET="$cap_pane" AP_SUPERVISOR_BACKEND=tmux AP_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1

  during=$(tmux list-panes -t "$cap_session" | wc -l | tr -d ' ')
  rec=$(cut -f2 "$home_tmp/state/.afk-daemon-terminal" 2>/dev/null || true)
  TRACK_TMUX_SESSIONS="$TRACK_TMUX_SESSIONS $rec"
  if [ "$before" = "$during" ]; then pass "tmux e2e: pilot window pane count unchanged after start (no split-window)"; else fail "tmux e2e: pilot window pane count changed ($before -> $during)"; fi
  ...
  AP_HOME="$home_tmp" AP_STATE_OVERRIDE="$home_tmp/state" \
    AP_SUPERVISOR_TARGET="$cap_pane" AP_SUPERVISOR_BACKEND=tmux "$LAUNCH" stop >/dev/null 2>&1

  after=$(tmux list-panes -t "$cap_session" | wc -l | tr -d ' ')
  if [ "$after" = "$before" ]; then pass "tmux e2e: pilot window pane count unchanged after stop"; else fail "tmux e2e: pilot window changed ($before -> $after)"; fi
