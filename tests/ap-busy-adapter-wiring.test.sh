...
# bin/ap-spawn.sh installs under the contract owned by bin/ap-busy-lib.sh.
#
# These tests run the REAL ap-spawn against a fake tmux pane and an isolated
...
# bin/ap-busy-event.sh writer, and the real classifier are exercised together
...
TMP_ROOT=$(ap_test_tmproot ap-busy-adapter-wiring)
...
SH
  ...
  ap_fake_exit0 "$fakebin" treehouse pi opencode claude codex
  printf '%s\n' "$fakebin"
}
...
classify() {  # <harness> <id> <state-dir>
  ap_busy_classify tmux fake:w "$1" "$2" "$3"
}
...
test_pi_extension_semantic_lifecycle() {
  ...
  [ "$out" = "busy ap-spawn" ] || fail "seed after spawn must be 'busy ap-spawn', got '$out'"
  ...
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy ap-spawn" ] || fail "turn_end must stay a notification, not a state edge, got '$out'"
...
test_pi_extension_stale_incarnation_rejected() {
  ...
  "$ROOT/bin/ap-busy-event.sh" arm "$state" "$id" >/dev/null
  out=$(drive_pi_ext "$ext" settle-idle) || fail "stale settle drive failed: $out"
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy ap-spawn" ] || fail "a stale extension event must not change state, got '$out'"
...
test_opencode_plugin_semantic_lifecycle() {
  ...
  state="$HOME_DIR/state"
  plugin="$WT_DIR/.opencode/plugins/ap-busy-state.js"
...
test_claude_hooks_stale_incarnation_harmless() {
  ...
  out=$(classify claude "$id" "$state")
  [ "$out" = "busy ap-spawn" ] || fail "a stale-gen hook event must not change state, got '$out'"
...
test_codex_unverified_until_a_semantic_source_exists() {
  ...
  out=$(ap_busy_classify tmux fake:w codex "$id" "$state" '• Working (6s • esc to interrupt)')
...
echo "all ap-busy-adapter-wiring tests passed"
