...
if [ "${AP_GROK_STOP_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set AP_GROK_STOP_LIVE_E2E=1 with AP_GROK_NATIVE_BIN and AP_GROK_LEGACY_BIN"
  exit 0
fi
...
NATIVE_BIN=${AP_GROK_NATIVE_BIN:-}
LEGACY_BIN=${AP_GROK_LEGACY_BIN:-}
AUTH=${AP_GROK_AUTH_FILE:-$HOME/.grok/auth.json}
REAL_TMUX=$(command -v tmux || true)
ACTIVE_LAB=

[ -x "$NATIVE_BIN" ] || fail "AP_GROK_NATIVE_BIN must be an exact executable path"
[ -x "$LEGACY_BIN" ] || fail "AP_GROK_LEGACY_BIN must be an exact executable path"
[ -f "$AUTH" ] || fail "AP_GROK_AUTH_FILE must name the already-managed auth artifact"
...
cleanup_exact_cell() {
  ...
  AP_E2E_TMUX_SOCKET_ID="$lab/dedicated.sock" PATH="$lab/bin:$PATH" \
...
run_cell() {  # <native|legacy> <exact-binary>
  ...
  tmp_base=${TMPDIR:-/tmp}
  tmp_base=${tmp_base%/}
  lab=$(mktemp -d "$tmp_base/ap-grok-stop-$kind.XXXXXX") || return 1
  session="$kind-e2e"
  target="ap-$kind"
  ACTIVE_LAB=$lab
  umask 077
...
[ "\${AP_E2E_TMUX_SOCKET_ID:-}" = "\$expected" ] || exit 92
...
#!/usr/bin/env bash
printf 'active=%s' "${GROK_TURNEND_GUARD_ACTIVE:-}" >> "${AP_GROK_E2E_ROOT:?}/resume-invocations.log"
printf ' <%s>' "$@" >> "${AP_GROK_E2E_ROOT:?}/resume-invocations.log"
printf '\n' >> "${AP_GROK_E2E_ROOT:?}/resume-invocations.log"
exec "${AP_GROK_E2E_BIN:?}" "$@"
EOF
  ...
  if [ "$kind" = native ]; then
    cat > "$lab/project/.grok/hooks/00-ap-stop-live-e2e.json" <<EOF
...
EOF
    ...
    cp "$ROOT/.grok/hooks/ap-primary-turnend-guard.json" "$lab/grok-home/hooks/ap-primary-turnend-guard.json"
    rm -f "$lab/project/.grok/hooks/ap-primary-turnend-guard.json"
    cat > "$lab/grok-home/hooks/00-ap-stop-live-e2e.json" <<EOF
{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"bash -lc 'cat >> \"$lab/payloads.jsonl\"'","timeout":30}]}]}}
EOF
...
env -u TMUX -u TMUX_PANE HOME='$lab/home' GROK_HOME='$lab/grok-home' GROK_AGENT=1 \
  AP_HOME='$lab/fmhome' AP_ROOT_OVERRIDE='$lab/project' AP_GROK_E2E_ROOT='$lab' \
  AP_GROK_E2E_BIN='$binary' AP_E2E_TMUX_SOCKET_ID='$lab/dedicated.sock' PATH='$lab/bin':"\$PATH" \
...
EOF
  ...
  i=0
  while [ "$i" -lt "${AP_GROK_STOP_LIVE_TIMEOUT:-600}" ] && [ ! -f "$lab/done" ]; do
    sleep 1
    i=$((i + 1))
  done
