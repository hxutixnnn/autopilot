#!/usr/bin/env bash
# End-to-end tests for durable pilot-held decisions discovered by investigations
# and visual reviews.
set -u
...
TMP_ROOT=$(ap_test_tmproot ap-decision-hold)
...
## Done
EOF
  fakebin=$(ap_fakebin "$home")
  ap_fake_exit0 "$fakebin" tmux treehouse no-mistakes gh gh-axi
  printf '%s\n' "$home"
}

run_bearings() {  # <home>
  local home=$1
  PATH="$home/fakebin:$PATH" AP_HOME="$home" AP_BEARINGS_NOW=2026-07-14T12:00:00Z \
    "$BEARINGS" --json
}

run_teardown() {  # <home> <id>
  local home=$1 id=$2
  PATH="$home/fakebin:$PATH" AP_ROOT_OVERRIDE="$ROOT" AP_HOME="$home" \
    AP_STATE_OVERRIDE="$home/state" AP_DATA_OVERRIDE="$home/data" \
    AP_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id"
}
...
The pilot still needs to choose route north or route south before follow-up work starts.
EOF
...
run_decisions() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    AP_HOME="$home" AP_STATE_OVERRIDE="$home/state" AP_DATA_OVERRIDE="$home/data" \
    AP_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/ap-decision-hold.sh" "$@"
}
...
A separate recommendation is already resolved and requires no pilot action.
EOF

  if run_decisions "$home" complete "$id" route access > "$home/early-complete.out" 2> "$home/early-complete.err"; then
    fail "completion succeeded before unresolved decisions had pilot holds"
  fi
  ...
  route_hold=$(run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "pilot route choice pending" --repo sample) \
  ...
  run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "pilot route choice pending" --repo sample >/dev/null \
  ...
  access_hold=$(run_decisions "$home" hold "$id" access \
    --title "Choose the sample access level" --reason "pilot access choice pending" --repo sample) \
  ...
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/ap-classify-lib.sh" "$home/state/$id.status")
  [ -z "$open" ] || fail "pilot-held transfer did not close duplicate live status decisions: $open"

  before=$(shasum -a 256 "$home/data/backlog.md" | awk '{print $1}')
  json=$(run_bearings "$home") || fail "Bearings failed with pilot-held decisions"
  ...
  printf '%s' "$json" | jq -e --arg route "$route_hold" --arg access "$access_hold" '
    (.decisions_open | any(.id == $route and .verb == "pilot-hold" and .owner == "(main)"))
      and (.decisions_open | any(.id == $access and .verb == "pilot-hold" and .owner == "(main)"))
      and (.gates | any(.id == $route or .id == $access) | not)
  ' >/dev/null || fail "Bearings did not surface structured pilot holds: $json"
  ...
  printf '%s' "$json" | jq -e --arg route "$route_hold" --arg access "$access_hold" '
    (.decisions_open | any(.id == $route and .verb == "pilot-hold"))
      and (.decisions_open | any(.id == $access and .verb == "pilot-hold"))
      and (.in_flight | any(.id == "sample-systems-review") | not)
  ' >/dev/null || fail "teardown or archival erased a pilot-held decision: $json"

  tasks_in "$home" add sample-route-implementation "Apply the selected sample route" \
    --kind flight --repo sample >/dev/null \
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
    ...
    fail "pilot hold closed before dependent work had a durable routing edge"
  fi
  ...
  tasks_in "$home" add sample-route-followup "Check the selected sample route" \
    --kind flight --repo sample --blocked-by "$route_hold" >/dev/null \
    || fail "could not create second dependent work fixture"
  cat > "$home/fakebin/tasks-axi" <<'EOF'
#!/usr/bin/env bash
if [ "${1:-}" = unblock ] && [ "${2:-}" = sample-route-implementation ] \
  && [ ! -f "$AP_HOME/unblock-failed-once" ]; then
  : > "$AP_HOME/unblock-failed-once"
  exit 1
fi
exec "$REAL_TASKS_AXI" "$@"
EOF
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "partial resolution retry accepted a different pilot decision"
  fi
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "resolution retry accepted a different pilot decision"
  fi
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
  ...
  assert_contains "$show" "Resolution recorded by ap-decision-hold" "resolved hold lost the decision record"
  show=$(tasks_in "$home" show sample-route-implementation --full)
  ...
  pass "pilot holds are idempotent, distinct, teardown-safe, Bearings-visible, and durably routed before close"
}

test_recon_teardown_always_requires_inventory_verification() {
  local home id
...
test_visual_review_uses_shared_completion_owner() {
  ...
  printf '# Sample board investigation\n\nThe initial findings need no pilot choice.\n' > "$home/data/$id/report.md"
  ...
  hold=$(run_decisions "$home" hold "$id" layout \
    --title "Choose the sample layout" --reason "pilot layout choice pending" --repo sample) \
  ...
  printf '%s' "$json" | jq -e --arg hold "$hold" '
    .decisions_open | any(.id == $hold and .verb == "pilot-hold")
  ' >/dev/null || fail "ended visual review did not leave its durable Pilot Call: $json"
...
The recommendation is informational and needs no pilot action.
EOF
  run_decisions "$home" complete "$id" --none >/dev/null \
...
test_terminal_single_owner_status_decision_does_not_block_empty_inventory() {
  ...
  printf 'needs-decision [key=default]: choose route A or route B\ndone: report complete\n' \
    > "$home/state/$id.status"
  printf '# Terminal sample review\n\nNo unresolved pilot choice remains.\n' > "$home/data/$id/report.md"
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/ap-classify-lib.sh" "$home/state/$id.status")
  ...
  copilot=sample-copilot
  write_origin_meta "$home" "$copilot" copilot
  printf 'needs-decision [key=route]: choose route A or route B\ndone: heartbeat complete\n' \
    > "$home/state/$copilot.status"
  if run_decisions "$home" complete "$copilot" --none \
    > "$home/copilot-terminal.out" 2> "$home/copilot-terminal.err"; then
    fail "copilot terminal status decision was incorrectly cleared"
  fi
  pass "terminal single-owner stale status decisions do not block empty inventory"
}

test_copilot_hold_stays_in_authoritative_home() {
  local parent copilot origin hold json
  parent=$(make_home main-routing)
  copilot="$TMP_ROOT/sample-copilot-home"
  mkdir -p "$copilot/data" "$copilot/state" "$copilot/config" "$copilot/projects" "$copilot/bin"
  cp "$ROOT/.tasks.toml" "$copilot/.tasks.toml"
  printf '# Synthetic copilot home\n' > "$copilot/AGENTS.md"
  printf 'sample-copilot\n' > "$copilot/.ap-copilot-home"
...
## Done
EOF
  fakebin=$(ap_fakebin "$copilot")
  ap_fake_exit0 "$fakebin" tmux treehouse no-mistakes gh gh-axi
  origin=sample-copilot-review
  mkdir -p "$copilot/data/$origin"
  tasks_in "$copilot" add "$origin" "Investigate copilot sample" --kind recon --repo sample --start >/dev/null
  write_origin_meta "$copilot" "$origin"
  printf 'done: report and visual review complete\n' > "$copilot/state/$origin.status"
  printf '# Sample copilot review\n\nOne pilot choice remains.\n' > "$copilot/data/$origin/report.md"
  hold=$(run_decisions "$copilot" hold "$origin" release \
    --title "Choose the sample release" --reason "pilot release choice pending" --repo sample) \
    || fail "copilot-owned hold creation failed"
  run_decisions "$copilot" complete "$origin" release >/dev/null \
    || fail "copilot-owned completion failed"
  run_teardown "$copilot" "$origin" >/dev/null 2> "$copilot/teardown.err" \
    || fail "copilot investigation teardown failed: $(cat "$copilot/teardown.err")"
  tasks_in "$copilot" "done" "$origin" --report "data/$origin/report.md" --keep 0 >/dev/null

  printf -- '- sample-copilot - synthetic scope (home: %s; scope: sample reviews; projects: sample; added 2026-07-14)\n' \
    "$copilot" > "$parent/data/copilots.md"
  ap_write_copilot_meta "$parent/state/sample-copilot.meta" "$copilot" \
    "autopilot:ap-sample-copilot" sample
  json=$(run_bearings "$parent") || fail "parent Bearings could not read copilot hold"
  printf '%s' "$json" | jq -e --arg hold "$hold" '
    .decisions_open | any(.owner == "sample-copilot" and .verb == "pilot-hold" and (.id | endswith($hold)))
  ' >/dev/null || fail "copilot pilot hold did not surface with authoritative owner: $json"
  assert_no_grep "$hold" "$parent/data/backlog.md" "copilot hold leaked into the main backlog"
  assert_grep "$hold" "$copilot/data/backlog.md" "copilot hold left its authoritative backlog"
  pass "main-home and copilot-home pilot holds remain correctly routed"
}
...
test_resolve_matches_quoted_blocked_by_edges() {
  ...
  hold_first=$(run_decisions "$home" hold "$origin" edge-first \
    --title "First edge decision" --reason "pilot first pending" --repo sample) \
    || fail "could not register first-edge hold"
  hold_mid=$(run_decisions "$home" hold "$origin" edge-mid \
    --title "Middle edge decision" --reason "pilot mid pending" --repo sample) \
    || fail "could not register mid-edge hold"
  hold_last=$(run_decisions "$home" hold "$origin" edge-last \
    --title "Last edge decision" --reason "pilot last pending" --repo sample) \
    || fail "could not register last-edge hold"
  hold_absent=$(run_decisions "$home" hold "$origin" edge-absent \
    --title "Absent edge decision" --reason "pilot absent pending" --repo sample) \
    || fail "could not register absent-edge hold"

  tasks_in "$home" add pad-a "Pad A" --kind flight --repo sample >/dev/null \
    || fail "could not create pad-a blocker"
  tasks_in "$home" add pad-b "Pad B" --kind flight --repo sample >/dev/null \
    || fail "could not create pad-b blocker"

  tasks_in "$home" add dep-first "Dep first position" --kind flight --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-mid "Dep mid position" --kind flight --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-last "Dep last position" --kind flight --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-absent "Dep absent control" --kind flight --repo sample >/dev/null \
