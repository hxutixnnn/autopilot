#!/usr/bin/env bash
# Behavior tests for ap-vendor-auth-probe.sh - the one hard-bounded,
...
#    verdict.
#
# 2. The pilot-approved probe envelope must not depend on agent memory: fixed
...
TMP_ROOT=$(ap_test_tmproot ap-vendor-auth-probe-tests)
SCRIPT="$ROOT/bin/ap-vendor-auth-probe.sh"
...
if IFS= read -r -t 2 leaked; then
  printf '%s\n' "$leaked" >> "$AP_FAKE_GROK_STDIN"
fi
if [ "${1:-}" = --version ]; then
  printf 'grok %s (fakebuild) [stable]\n' "${AP_FAKE_GROK_VERSION:-0.2.117}"
  exit 0
fi
case "${AP_FAKE_GROK_MODE:-authenticated}" in
  authenticated)
...
run_probe() {
  ...
  out=$(env "PATH=$fakebin:$BASE_PATH" \
    "AP_FAKE_GROK_LOG=$RUN_GROK_LOG" \
    "AP_FAKE_GROK_STDIN=$RUN_GROK_STDIN" \
    "AP_FAKE_QUOTA_LOG=$RUN_QUOTA_LOG" \
    "${env_pairs[@]+"${env_pairs[@]}"}" \
...
test_probe_never_reads_quota() {
  ...
  for mode in authenticated unauthenticated; do
    run_probe "no-quota-$mode" grok -- "AP_FAKE_GROK_MODE=$mode"
...
test_probe_result_is_never_an_exit_status_verdict() {
  ...
  for mode in authenticated unauthenticated garbage empty; do
    run_probe "rc-$mode" grok -- "AP_FAKE_GROK_MODE=$mode"
...
test_authenticated_session_is_reported() {
  run_probe authenticated grok -- "AP_FAKE_GROK_MODE=authenticated"
...
test_unauthenticated_session_is_reported() {
  run_probe unauthenticated grok -- "AP_FAKE_GROK_MODE=unauthenticated"
...
test_unrecognized_output_is_indeterminate() {
  ...
  for mode in garbage leading-blank empty; do
    run_probe "indeterminate-$mode" grok -- "AP_FAKE_GROK_MODE=$mode"
...
test_missing_vendor_cli_is_reported_not_assumed() {
  ...
  line=$(env "PATH=$fakebin:$BASE_PATH" \
    "AP_FAKE_QUOTA_LOG=$case_dir/quota.log" \
...
test_hanging_probe_is_bounded_and_reported() {
  ...
  started=$(date +%s)
  run_probe grok-hang grok -- "AP_FAKE_GROK_MODE=hang" "AP_VENDOR_AUTH_PROBE_TIMEOUT=2"
  finished=$(date +%s)
...
test_zero_bound_falls_back_to_a_real_bound() {
  ...
  for value in 0 00; do
    started=$(date +%s)
    run_probe "bound-zero-$value" grok -- "AP_FAKE_GROK_MODE=hang" "AP_VENDOR_AUTH_PROBE_TIMEOUT=$value"
    finished=$(date +%s)
...
test_malformed_bound_is_replaced_not_forwarded() {
  ...
  for value in -1 abc 1.5 ' '; do
    run_probe "bound-${value// /space}" grok -- "AP_FAKE_GROK_MODE=authenticated" "AP_VENDOR_AUTH_PROBE_TIMEOUT=$value"
...
test_probe_never_inherits_caller_stdin() {
  run_probe grok-stdin grok -- "AP_FAKE_GROK_MODE=authenticated"
...
test_probe_argv_is_fixed_and_non_destructive() {
  ...
  for mode in authenticated unauthenticated garbage; do
    run_probe "argv-$mode" grok -- "AP_FAKE_GROK_MODE=$mode"
...
test_fact_line_carries_no_vendor_output_or_credential_material() {
  run_probe sanitized grok -- "AP_FAKE_GROK_MODE=authenticated"
...
test_probe_version_change_is_disclosed() {
  run_probe version-drift grok -- "AP_FAKE_GROK_MODE=authenticated" "AP_FAKE_GROK_VERSION=0.9.0"
...
test_probe_version_match_is_recorded() {
  run_probe version-pinned grok -- "AP_FAKE_GROK_MODE=authenticated"
