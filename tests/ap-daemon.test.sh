#!/usr/bin/env bash
# tests/ap-daemon.test.sh - supervise-daemon classifiers, the pilot-relevant
...
# ap-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in ap-afk-inject-e2e and ap-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/ap-supervise-daemon.sh"
AFK_START="$ROOT/bin/ap-afk-start.sh"
...
if [ -z "${AP_TEST_DAEMON_SOURCED:-}" ]; then
  export AP_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/ap-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(ap_test_tmproot ap-daemon-tests)
AP_DAEMON_PRIMARY_HARNESS=claude
export AP_DAEMON_PRIMARY_HARNESS
...
test_afk_start_refuses_when_flag_cannot_be_written() {
  ...
  out=$(AP_STATE_OVERRIDE="$state" AP_SUPERVISOR_BACKEND=unsupported "$AFK_START" 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "ap-afk-start.sh should fail when state/.afk cannot be written"
  assert_not_contains "$out" "starting supervise daemon" "ap-afk-start.sh continued into daemon startup after .afk write failure"
  assert_absent "$state/.supervise-daemon.log" "ap-afk-start.sh started the daemon after .afk write failure"
  pass "ap-afk-start.sh fails before daemon startup when the afk flag cannot be written"
}
...
test_afk_start_ignores_stale_pidfile_without_lock() {
  ...
  [ "$status" -ne 0 ] || fail "ap-afk-start.sh should attempt daemon startup instead of trusting a pidfile-only live pid"
  assert_contains "$out" "starting supervise daemon" "ap-afk-start.sh did not attempt daemon startup"
  assert_contains "$out" "does not support supervisor backend 'unsupported'" "daemon startup did not reach backend validation"
  assert_not_contains "$out" "daemon already running" "ap-afk-start.sh trusted a stale pidfile-only live pid"
  pass "ap-afk-start.sh ignores stale pidfile-only live pids"
}
...
test_afk_start_reclaims_stale_daemon_lock_reused_pid() {
  ...
  [ "$status" -ne 0 ] || fail "ap-afk-start.sh should attempt daemon startup after rejecting a reused-pid lock"
  assert_contains "$out" "starting supervise daemon" "ap-afk-start.sh did not attempt daemon startup after rejecting the stale lock"
  ...
  assert_not_contains "$out" "daemon already running" "ap-afk-start.sh trusted a stale daemon lock with a reused pid"
  assert_not_contains "$out" "another ap-supervise-daemon is already running" "daemon singleton lock still trusted the reused pid"
  pass "ap-afk-start.sh reclaims stale daemon locks whose live pid identity no longer matches"
}

test_daemon_state_root_uses_ap_home() {
  local dir home override out
  dir=$(make_supercase daemon-ap-home)
  home="$dir/autopilot-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(AP_HOME="$home" AP_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored AP_HOME: $out"

  out=$(AP_HOME="$home" AP_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored AP_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by AP_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(AP_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
            ...
            "ready in branch ap/t1" "merged"; do
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(AP_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
    case "$out" in escalate\|*) ;; *) fail "pilot verb did not escalate ($kw): $out" ;; esac
  done
  pass "pilot-relevant status verbs escalate"
}
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:ap-qux-w4" "$state"
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "sess:ap-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:ap-qux-w4")" | tr ':/.' '___')
...
test_stale_diagnostic_wedge_survives_busy_housekeeping() {
  ...
  for case_name in working prior-terminal paused; do
    ...
    ap_write_meta "$state/$task.meta" "window=$win" "backend=tmux"
    case "$case_name" in
    ...
    (
      kill() { printf 'kill %s\n' "$*" >> "$action_log"; }
      ap_backend_send_text_submit() { printf 'interrupt %s\n' "$*" >> "$action_log"; }
      LOG="$dir/daemon.log" AP_STATE_OVERRIDE="$state" handle_wake "$reason" "$state"
      PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$win" AP_FAKE_TMUX_CAPTURE="$pane" \
        AP_STATE_OVERRIDE="$state" AP_ESCALATE_BATCH_SECS=999999 housekeeping "$state"
    )
...
test_stale_terminal_escalates() {
  ...
  state="$dir/state"
  printf 'done: ready in branch ap/t1\n' > "$state/fin-t5.status"
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "sess:ap-fin-t5" "$state")
  case "$out" in escalate\|*) ;; *) fail "terminal stale did not escalate: $out" ;; esac
  ap_write_meta "$state/herdr-t5.meta" "window=default:w1:p2" "backend=herdr"
  printf 'done: ready in branch ap/herdr\n' > "$state/herdr-t5.status"
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "default:w1:p2" "$state")
...
test_stale_paused_classifies_pause() {
  ...
  status_is_pilot_relevant "$pause_reason" && fail "pause reason phrases made the status pilot-relevant"
  printf '%s\n' "$pause_reason" > "$state/held-w9.status"
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "sess:ap-held-w9" "$state")
  case "$out" in pause\|*) ;; *) fail "declared pause did not classify as pause: $out" ;; esac
  pass "paused reasons with pilot phrases remain pause-classified"
}
...
test_handle_wake_paused_records_pause_marker() {
  ...
  AP_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
  [ -e "$state/.subsuper-paused-$key" ] || fail "pause marker not recorded by handle_wake"
  [ ! -e "$state/.subsuper-stale-$key" ] || fail "wedge marker not cleared when the flight crew declared a pause"
...
test_handle_wake_paused_signal_records_pause_marker() {
  ...
  state="$dir/state"
  win="sess:ap-held-w10-signal"
  printf 'window=%s\nkind=flight\n' "$win" > "$state/held-w10-signal.meta"
  printf 'paused: awaiting the vendor rate-limit reset\n' > "$state/held-w10-signal.status"
  key=$(printf '%s' "held-w10-signal" | tr ':/.' '___')
  date +%s > "$state/.subsuper-stale-$key"
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-signal.status" "$state"
...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  ...
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-terminal.status" "$state"
...
test_housekeeping_migrates_watcher_unpaused_marker_to_clear() {
  ...
  state="$dir/state"
  win="sess:ap-held-w10-migrate-unpaused"
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  win="sess:ap-held-w11"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w11.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w11" | tr ':/.' '___')
  echo $(( $(date +%s) - 5000 )) > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$win" AP_FAKE_TMUX_CAPTURE="$pane" \
    AP_STATE_OVERRIDE="$state" AP_PAUSE_RESURFACE_SECS=240 housekeeping "$state"
...
# A pause whose pane became busy again (the flight crew resumed) drops its marker without
# escalating, exactly like a resumed wedge.
test_housekeeping_paused_resumed_cleared() {
  local dir state fakebin win pane key
  dir=$(make_supercase paused-resumed)
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:ap-held-w12"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w12.status"
  printf 'Working...\n' > "$pane"
  ap_write_meta "$state/held-w12.meta" "window=$win" "worktree=$dir/wt" "kind=flight" "harness=pi"
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$state" held-w12)
  "$ROOT/bin/ap-busy-event.sh" apply "$state" held-w12 busy --gen "$gen" \
...
# A pane still idle but whose status is no longer a pause (the flight crew changed state
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:ap-held-w13"; pane="$dir/pane.txt"
  ...
  [ ! -s "$state/.subsuper-escalations" ] || fail "a flight crew that left its pause was re-surfaced as a pause"
  pass "housekeeping clears a paused marker once the flight crew is no longer declaring the pause"
}

test_housekeeping_stale_marker_transitions_to_pause() {
  local dir state fakebin win pane key
  dir=$(make_supercase stale-to-paused)
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:ap-held-w14"; pane="$dir/pane.txt"
  ...
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$win" AP_FAKE_TMUX_CAPTURE="$pane" \
    AP_STATE_OVERRIDE="$state" AP_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:ap-held-w15"; pane="$dir/pane.txt"
  printf 'working: upstream landed, resuming\n' > "$state/held-w15.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w15" | tr ':/.' '___')
  date +%s > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$win" AP_FAKE_TMUX_CAPTURE="$pane" \
    AP_STATE_OVERRIDE="$state" AP_PAUSE_RESURFACE_SECS=999999 housekeeping "$state"
  [ ! -e "$state/.subsuper-paused-$key" ] || fail "pause marker remained after the flight crew resumed"
  ...
  pass "housekeeping clears tracking when a flight crew leaves pause"
}
...
test_housekeeping_resumed_stale_cleared() {
  ...
  printf 'working\n' > "$state/res-w6.status"
  printf 'Working...\n' > "$pane"
  # A resumed flight crew proves it is working through its own semantic busy-state
  # record (bin/ap-busy-lib.sh), not through the pane's rendered footer.
  ap_write_meta "$state/res-w6.meta" "window=$win" "worktree=$dir/wt" "kind=flight" "harness=pi"
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$state" res-w6)
  "$ROOT/bin/ap-busy-event.sh" apply "$state" res-w6 busy --gen "$gen" \
    --source pi-ext --event agent-start
...
test_housekeeping_herdr_persistent_stale_resolves_meta() {
  ...
  state="$dir/state"
  ap_write_meta "$state/herdr-w7.meta" "window=default:w1:p2" "backend=herdr"
  ...
  (
    ...
    ap_backend_capture herdr default:w1:p2 40 >/dev/null
    [ "$(ap_backend_busy_state herdr default:w1:p2)" = idle ] || fail "herdr busy stub did not report idle"
    AP_STATE_OVERRIDE="$state" AP_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
# A herdr flight crew whose native agent.get reads idle (generation state) but whose
...
test_housekeeping_herdr_idle_busy_record_clears_stale() {
  ...
  state="$dir/state"
  ap_write_meta "$state/herdr-footer.meta" "window=default:w1:p4" "backend=herdr" "harness=claude"
  printf 'working\n' > "$state/herdr-footer.status"
  gen=$("$ROOT/bin/ap-busy-event.sh" arm "$state" herdr-footer)
  "$ROOT/bin/ap-busy-event.sh" apply "$state" herdr-footer busy --gen "$gen" \
  ...
  (
    ...
    ap_backend_capture herdr default:w1:p4 40 >/dev/null
    [ "$(ap_backend_busy_state herdr default:w1:p4)" = idle ] || fail "herdr busy stub did not report idle"
    AP_STATE_OVERRIDE="$state" AP_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_resumed_stale_cleared() {
  ...
  state="$dir/state"
  ap_write_meta "$state/herdr-busy.meta" "window=default:w1:p3" "backend=herdr"
  ...
  (
    ...
    ap_backend_capture herdr default:w1:p3 40 >/dev/null
    [ "$(ap_backend_busy_state herdr default:w1:p3)" = busy ] || fail "herdr busy stub did not report busy"
    AP_STATE_OVERRIDE="$state" AP_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_orca_persistent_stale_resolves_terminal() {
  ...
  state="$dir/state"
  ap_write_meta "$state/orca-w8.meta" "window=ap-orca-w8" "terminal=term-orca-w8" "backend=orca"
  ...
  (
    ...
    ap_backend_capture orca term-orca-w8 40 >/dev/null
    [ "$(ap_backend_busy_state orca term-orca-w8)" = idle ] || fail "Orca busy stub did not report idle"
    AP_STATE_OVERRIDE="$state" AP_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_PANE_ALIVE=1 AP_FAKE_TMUX_SENT="$sent" \
    AP_FAKE_TMUX_CAPTURE="$capture" AP_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
    || fail "escalate_flush failed"
  grep -F 'AUTOPILOT_OP: v1 away-supervisor: ' "$sent" >/dev/null \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_PANE_ALIVE=1 AP_FAKE_TMUX_SENT="$sent" \
    AP_FAKE_TMUX_CAPTURE="$capture" AP_ESCALATE_BATCH_SECS=90 AP_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "pilot signal was not buffered by handle_wake"
  pass "handle_wake routes routine->self and pilot->escalate"
}
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  AP_STATE_OVERRIDE="$state" AP_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "INJECT_SKIP=signal did not force self-handle"
  pass "INJECT_SKIP forces self-handle, bypassing pilot-relevant classification"
}
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:ap-x" || fail "stale: not recognized as wake"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "pilot signal was not escalated"
  key=$(printf '%s' "sig-t8" | tr ':/.' '___')
  [ "$(cat "$state/.subsuper-seen-status-$key" 2>/dev/null || true)" = "done: PR https://x/y/pull/8" ] \
    || fail "pilot signal escalate did not write the seen-status marker"
  : > "$state/.subsuper-escalations"
  rm -f "$state/.subsuper-last-scan"
  AP_STATE_OVERRIDE="$state" housekeeping "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "catch-all scan re-fired an already-escalated signal"
  pass "pilot signal escalate marks seen so the catch-all scan does not re-fire"
}
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" AP_FAKE_TMUX_PANE_ALIVE=1 AP_FAKE_TMUX_SENT="$sent" \
    AP_FAKE_TMUX_CAPTURE="$capture" AP_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_marker_detection() {
  local marker_hex
  marker_hex=$(printf '%s' "$AP_INJECT_MARK" | od -An -tx1 | tr -d ' \n')
  [ "$marker_hex" = e281a3 ] \
    || fail "AP_INJECT_MARK must use terminal-safe U+2063 bytes, got $marker_hex"
  [ "$AP_OPERATIONAL_PREFIX" = "${AP_INJECT_MARK}AUTOPILOT_OP: " ] \
    || fail "away-mode operational prefix drifted from the shared pilot-boundary marker"
  # message_is_injection: marker present -> injection; absent -> real message
  message_is_injection "${AP_OPERATIONAL_PREFIX}Supervisor escalate: done" \
    || fail "operationally-prefixed message not detected as injection"
  message_is_injection "${AP_INJECT_MARK}Supervisor escalate: done" \
...
test_afk_turn_exemption() {
  ...
  # a non-/afk skill invocation DOES exit (the pilot is actively working)
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${AP_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  ...
  ap_operational_input_encode away-supervisor "Supervisor escalate: done" encoded \
  ...
  stripped=$(strip_injection_marker "${AP_OPERATIONAL_PREFIX}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
    || fail "landed untyped operational prefix not stripped: '$stripped'"
  stripped=$(strip_injection_marker "${AP_INJECT_MARK}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$AP_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_requires_proven_empty_prompt() {
  ...
  for prompt in '❯' '›'; do
    printf 'output\noutput\n%s \n' "$prompt" > "$capture"
    if PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=2 \
...
# The safety fix at the tmux classifier (task ap-composer-shellglyph-safety): a
...
test_tmux_composer_state_bare_shell_is_unknown() {
  ...
  for g in '$' '%' '#' '>'; do
    printf 'output\noutput\n%s \n' "$g" > "$capture"
    out=$(PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=2 \
      ap_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "bare shell prompt '$g' must classify unknown (dead shell, unsafe), got '$out'"
  done
  pass "ap_tmux_composer_state: a bare shell prompt (\$/%/#/>) reads unknown, never empty (dead-shell injection safety)"
}
...
test_tmux_composer_state_bordered_and_agent_rows_are_empty() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf '╭────────────────────────╮\n│ >                      │\n╰────────────────────────╯\n' > "$capture"
  out=$(PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=1 \
    ap_tmux_composer_state "fakepane")
  [ "$out" = empty ] || fail "a bordered '│ > │' composer should read empty, got '$out'"
  printf '%s\n' "❯ " > "$capture"
  out=$(PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=0 \
    ap_tmux_composer_state "fakepane")
  ...
  pass "ap_tmux_composer_state: a bordered composer box and bare agent glyphs (❯/›) still read empty"
}
...
test_tmux_composer_state_requires_matching_box_borders() {
  ...
  pass "ap_tmux_composer_state: only matching edge borders form a composer box"
}
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '╭────────────────╮\n│ custom idle>   │\n╰────────────────╯\n' > "$capture"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=1 \
    AP_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "AP_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors AP_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(AP_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "sess:ap-dup-s10" "$state")
...
test_afk_nonterminal_working_merged_keeps_wedge_aging() {
  ...
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "$win" "$state")
  case "$out" in
    self\|*transient*) ;;
...
test_afk_genuine_done_still_terminal_stale() {
  ...
  state="$dir/state"
  printf 'done: PR https://example.com/pull/76 checks green; stage 1 of 4 ready for autopilot merge\n' \
    > "$state/stage1-w2.status"
  out=$(AP_STATE_OVERRIDE="$state" classify_stale "sess:ap-stage1-w2" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in '>' '❯' ''; do
    ...
    if PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=1 \
      pane_input_pending "fakepane"; then
...
test_pane_input_pending_bordered_with_text_is_pending() {
  ...
  # still read as pending so the daemon defers (and the pilot-return race is
  # still protected).
  local dir state fakebin capture
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$dir/composer" AP_FAKE_SENT="$sent" \
    ap_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, ap-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$dir/composer" AP_FAKE_SENT="$sent" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 \
    ap_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$dir/composer" AP_FAKE_SENT="$sent" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 AP_INJECT_CONFIRM_SLEEP=0.05 \
    AP_ESCALATE_BATCH_SECS=99999 AP_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$dir/composer" AP_FAKE_SENT="$sent" \
    AP_ESCALATE_BATCH_SECS=99999 AP_MAX_DEFER_SECS=60 AP_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$dir/composer" AP_FAKE_SENT="$sent" \
    AP_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_PANE_ALIVE=1 AP_FAKE_TMUX_SENT="$sent" \
    AP_FAKE_TMUX_CAPTURE="$capture" AP_FAKE_TMUX_CURSOR_Y=0 \
    AP_ESCALATE_BATCH_SECS=99999 AP_MAX_DEFER_SECS=300 housekeeping "$state"
...
# ACTIVE alert channel must reach the pilot even when the wedged pane and its
...
# the AP_WEDGE_ALARM_EXEC seam, which tests/wake-helpers.sh forces to a recorder
# ($AP_WEDGE_ALARM_LOG logs "<channel>\t<summary>"); the daemon also defaults
...
#!/usr/bin/env bash
printf '%s\n' "${AP_FAKE_UNAME:-Darwin}"
SH
  # Fakes keep command discovery deterministic on any CI host.
  cat > "$fakebin/osascript" <<'SH'
#!/usr/bin/env bash
printf '%s\n' osascript >> "${AP_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
printf '%s\n' herdr >> "${AP_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
...
test_wedge_alarm_library_mode_defaults_to_discard() {
  ...
  # AP_WEDGE_ALARM_EXEC to "discard", so a sourced context (every test) cannot
  # fire a real notification even if it forgets to stub. Checked in a clean
  # subshell that first unsets this harness's recorder.
  local out
  # shellcheck disable=SC2016  # $1/$AP_WEDGE_ALARM_EXEC must expand in the child, not here
  out=$(env -u AP_WEDGE_ALARM_EXEC bash -c '. "$1"; printf "%s" "${AP_WEDGE_ALARM_EXEC:-UNSET}"' _ "$DAEMON")
  [ "$out" = discard ] \
    || fail "sourcing the daemon did not default the notifier seam to discard (got: $out)"
  pass "library mode: sourcing the daemon defaults AP_WEDGE_ALARM_EXEC to discard (no test can fire a real notification)"
}
...
test_wake_helpers_replace_inherited_notifier_override() {
  ...
  cat > "$unsafe" <<'SH'
#!/usr/bin/env bash
printf '%s\n' invoked >> "${AP_WEDGE_ALARM_UNSAFE_LOG:?}"
SH
  chmod +x "$unsafe"
  AP_WEDGE_ALARM_EXEC="$unsafe" AP_WEDGE_ALARM_UNSAFE_LOG="$unsafe_log" \
    AP_WEDGE_ALARM_LOG="$alert_log" AP_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_discard_seam_fires_nothing() {
  ...
  PATH="$dir/fakebin:$PATH" AP_WEDGE_ALARM_LOG="$log" AP_WEDGE_ALARM_EXEC=discard \
    AP_WEDGE_ALARM_CHANNEL=$'osascript\nherdr\n'"$channel" \
...
test_wedge_alarm_direct_notifiers_honor_discard_seam() {
  ...
  PATH="$dir/fakebin:$PATH" AP_WEDGE_ALARM_REAL_LOG="$real_log" AP_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_osascript "away-mode WEDGED 900s"
  PATH="$dir/fakebin:$PATH" AP_WEDGE_ALARM_REAL_LOG="$real_log" AP_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_herdr "away-mode WEDGED 900s"
  AP_WEDGE_ALARM_EXEC=discard wedge_alarm_via_command "$command" "away-mode WEDGED 900s"
...
test_wedge_alarm_osascript_channel_selected() {
  ...
  AP_WEDGE_ALARM_LOG="$log" AP_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_herdr_channel_selected() {
  ...
  AP_WEDGE_ALARM_LOG="$log" AP_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_command_channel_receives_summary() {
  ...
  AP_WEDGE_ALARM_EXEC='' AP_WEDGE_ALARM_CHANNEL="$chan" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  ...
  pass "command channel runs the pilot command with the summary on \$1 and on stdin"
}
...
test_wedge_alarm_command_failure_hides_configured_command() {
  ...
  LOG="$daemon_log" AP_WEDGE_ALARM_EXEC='' AP_WEDGE_ALARM_CHANNEL="command:exit 73 # $secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_unknown_channel_hides_configured_directive() {
  ...
  secret="https://alerts.example.invalid/hook?token=private-wedge-token"
  LOG="$daemon_log" AP_WEDGE_ALARM_CHANNEL="webhook:$secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  for directives in $'osascript\noff' $'off\nosascript'; do
    : > "$log"
    AP_WEDGE_ALARM_LOG="$log" AP_WEDGE_ALARM_CHANNEL="$directives" \
      wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_darwin_selects_osascript() {
  ...
  PATH="$dir/fakebin:$PATH" AP_WEDGE_ALARM_LOG="$log" AP_FAKE_UNAME=Darwin AP_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_non_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" AP_WEDGE_ALARM_LOG="$log" AP_FAKE_UNAME=Linux AP_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_config_file_multi_channel() {
  ...
  AP_WEDGE_ALARM_LOG="$log" AP_CONFIG_OVERRIDE="$cfgdir" \
...
test_wedge_alarm_failing_channel_degrades_gracefully() {
  ...
  AP_WEDGE_ALARM_LOG="$log" AP_WEDGE_ALARM_FAIL=osascript \
    AP_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_hung_channel_times_out_and_falls_through() {
  ...
  start=$SECONDS
  LOG="$daemon_log" AP_WEDGE_ALARM_EXEC='' AP_WEDGE_ALARM_TIMEOUT_SECS=1 \
    AP_WEDGE_ALARM_CHANNEL=$'command:sleep 30\n'"$channel" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
SH
  chmod +x "$blocker"
  start=$SECONDS
  LOG="$daemon_log" AP_WEDGE_ALARM_EXEC="$blocker" AP_WEDGE_ALARM_TIMEOUT_SECS=1 \
    AP_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  elapsed=$((SECONDS - start))
...
test_inject_wedge_alarm_fires_active_alert_on_non_tmux_backend() {
  ...
  AP_WEDGE_ALARM_LOG="$log" AP_STATE_OVERRIDE="$state" \
    AP_WEDGE_ALARM_CHANNEL=osascript AP_SUPERVISOR_BACKEND=herdr \
...
test_inject_wedge_alarm_throttles_when_marker_cannot_be_written() {
  ...
  chmod u-w "$state"
  WEDGE_ALARM_LAST_EPOCH=0
  LOG="$daemon_log" AP_WEDGE_ALARM_LOG="$log" AP_MAX_DEFER_SECS=600 \
    AP_WEDGE_ALARM_CHANNEL=osascript AP_SUPERVISOR_BACKEND=herdr \
    inject_wedge_alarm "$state" 30600
  LOG="$daemon_log" AP_WEDGE_ALARM_LOG="$log" AP_MAX_DEFER_SECS=600 \
    AP_WEDGE_ALARM_CHANNEL=osascript AP_SUPERVISOR_BACKEND=herdr \
...
test_ap_send_exits_nonzero_on_confirmed_swallow() {
  # ap-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so autopilot learns the instruction did not land
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" AP_HOME="$dir" AP_STATE_OVERRIDE="$dir/state" AP_FAKE_COMPOSER="$dir/composer" \
    AP_SEND_SLEEP=0.05 "$ROOT/bin/ap-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "ap-send exited non-zero on a clean submit: $(cat "$err")"
  # Persistent swallow -> exit non-zero with a clear message.
  printf '╭─────╮\n│ >   │\n╰─────╯\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" AP_HOME="$dir" AP_STATE_OVERRIDE="$dir/state" AP_FAKE_COMPOSER="$dir/composer" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 AP_SEND_SLEEP=0.05 \
    "$ROOT/bin/ap-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    fail "ap-send exited zero despite a swallowed Enter (silent unsubmitted instruction)"
  fi
  grep -F 'not submitted' "$err" >/dev/null || fail "ap-send did not explain the swallowed submit: $(cat "$err")"
  pass "ap-send exits non-zero on a confirmed swallow, zero on a clean submit"
}

test_ap_send_exits_nonzero_on_initial_send_failure() {
  local dir fakebin err
  dir=$(make_bordered_case send-type-failure)
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" AP_HOME="$dir" AP_STATE_OVERRIDE="$dir/state" AP_FAKE_COMPOSER="$dir/composer" \
    AP_FAKE_SEND_FAIL=1 AP_SEND_SLEEP=0.05 \
    "$ROOT/bin/ap-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "ap-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "ap-send did not explain initial send failure: $(cat "$err")"
  pass "ap-send exits non-zero when initial text send fails"
}

test_ap_send_exits_nonzero_on_unproven_submit() {
  local dir fakebin err
  ...
  if PATH="$fakebin:$PATH" AP_HOME="$dir" AP_STATE_OVERRIDE="$dir/state" AP_FAKE_COMPOSER="$dir/composer" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 AP_SEND_SLEEP=0.05 \
    "$ROOT/bin/ap-send.sh" sess:win '修复' >/dev/null 2>"$err"; then
    fail "ap-send exited zero when submit proof remained pending-unproven"
  fi
  grep -F 'verdict=pending-unproven' "$err" >/dev/null \
    || fail "ap-send did not preserve the unproven-submit verdict: $(cat "$err")"
  pass "ap-send exits non-zero unless delivery is proven empty"
}

# --- herdr backend-awareness (ap-turnend-guard-h6-adjacent transport fix) ----
...
test_discover_supervisor_backend_precedence() {
  local out
  out=$(AP_SUPERVISOR_BACKEND=herdr TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "explicit AP_SUPERVISOR_BACKEND override was not honored: $out"

  out=$(AP_SUPERVISOR_BACKEND='' TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = tmux ] || fail "TMUX_PANE should win over HERDR_ENV (tmux nested in herdr resolves to tmux): $out"

  out=$(AP_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "HERDR_ENV=1 with HERDR_PANE_ID present should resolve to herdr: $out"

  if out=$(AP_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_backend); then
...
test_discover_supervisor_target_herdr() {
  local out
  out=$(AP_SUPERVISOR_TARGET=explicit:target TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = "explicit:target" ] || fail "explicit AP_SUPERVISOR_TARGET override was not honored: $out"

  out=$(AP_SUPERVISOR_TARGET='' TMUX_PANE='%3' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = '%3' ] || fail "TMUX_PANE should win over herdr markers: $out"

  out=$(AP_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION='' discover_supervisor_target)
  [ "$out" = "default:w1:p9" ] || fail "herdr target should default HERDR_SESSION to 'default': $out"

  out=$(AP_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION=iso1 discover_supervisor_target)
  [ "$out" = "iso1:w1:p9" ] || fail "herdr target should use an explicit HERDR_SESSION: $out"

  if out=$(AP_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_target); then
    fail "bare fallback should return non-zero"
  fi
  [ "$out" = "autopilot:0" ] || fail "bare fallback should still print autopilot:0: $out"

  pass "discover_supervisor_target: override > TMUX_PANE > herdr '<session>:<pane-id>' composition > autopilot:0 fallback"
}
...
test_pane_is_busy_herdr_native_busy_state() {
  ...
  (
    ap_backend_busy_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected busy_state args: $1 $2"; printf 'busy'; }
    ap_backend_capture() { fail "capture should not be consulted when busy_state is conclusive"; }
    AP_STATE_OVERRIDE="$dir/state" AP_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr \
...
test_primary_busy_guard_is_harness_scoped() {
  (
    ...
    ap_backend_capture() { printf 'esc interrupt\n'; }
    if AP_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr; then
      fail "OpenCode's rendered signature must not classify a Claude primary busy"
    fi
    AP_DAEMON_PRIMARY_HARNESS=opencode pane_is_busy "default:w1:p2" herdr \
...
test_pane_is_busy_defaults_to_tmux_when_backend_omitted() {
  ...
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_CAPTURE="$capture" AP_STATE_OVERRIDE="$dir/state" AP_DAEMON_PRIMARY_HARNESS=grok pane_is_busy "fakepane" \
...
test_pane_input_pending_herdr_dispatch() {
  (
    ap_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
  ...
  (
    ap_backend_composer_state() { printf 'empty'; }
  ...
  (
    ap_backend_composer_state() { printf 'future-state'; }
  ...
  pass "pane_input_pending: dispatches through ap_backend_composer_state for backend=herdr"
}
...
test_inject_msg_herdr_busy_guard_defers() {
  ...
  afk_enter "$state"
  (
    ap_backend_target_exists() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected target_exists args: $1 $2"; return 0; }
    pane_is_busy() { return 0; }
    ap_backend_composer_state() { fail "composer_state should not be consulted once the busy-guard already deferred"; }
    ap_backend_send_text_submit() { fail "send_text_submit should not run when the busy-guard defers"; }
    if AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_composer_guard_defers() {
  ...
  afk_enter "$state"
  (
    ap_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    ap_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
    ap_backend_send_text_submit() { fail "send_text_submit should not run when the composer-guard defers"; }
    if AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_pane_gone_defers() {
  ...
  afk_enter "$state"
  (
    ap_backend_target_exists() { return 1; }
    pane_is_busy() { fail "busy guard should not be consulted once the pane-exists check already failed"; }
    ap_backend_send_text_submit() { fail "send_text_submit should not run when the pane does not exist"; }
    if AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:gone" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state" \
...
# Safety-critical (task ap-composer-shellglyph-safety): the away-mode injector
...
test_inject_msg_defers_on_dead_shell_unknown() {
  ...
  (
    ap_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    ap_backend_composer_state() { printf 'unknown'; }
    ap_backend_send_text_submit() { fail "send_text_submit must NOT run when the composer is a dead shell (unknown)"; }
    if AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_defers_on_unrecognized_composer_state() {
  ...
  (
    ...
    pane_is_busy() { return 1; }
    ap_backend_composer_state() { printf 'future-state'; }
    ap_backend_send_text_submit() { fail "send_text_submit must not run for an unrecognized composer state"; }
    if AP_SUPERVISOR_BACKEND=herdr AP_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
