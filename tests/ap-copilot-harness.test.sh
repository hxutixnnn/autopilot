#!/usr/bin/env bash
# Tests for the copilot-vs-flight crew member harness split, the optional model/effort
# tokens config/copilot-harness carries alongside the harness, and the
# primary->copilot inherited local-material propagation.
#
# Three capabilities are under test:
#   A) Harness split. config/copilot-harness sets the harness the PRIMARY uses
#      to launch COPILOT agents, independent of config/flight-crew-harness (the
#      flight crew member harness). ap-harness.sh copilot resolves the fallback chain
#      config/copilot-harness -> config/flight-crew-harness -> own; an absent or
#      "default" copilot-harness behaves exactly as the flight crew harness did before
#      this knob existed (full backward-compat). ap-spawn.sh resolves a copilot
...
#      (gitignored) config items - config/flight-crew-dispatch.json, config/flight-crew-harness,
#      config/backlog-backend, config/backend, config/herdr-presentation-spaces, and
#      config/startup-memory-budget -
#      down into each copilot home's config/, so the copilot's OWN flight crew members,
...
#      (re-pushed at copilot spawn, on the bootstrap copilot sweep, and by
#      config push).
#      config/copilot-harness is deliberately NOT inherited (copilots do
#      not spawn copilots). After a successful push that changes allowlisted
#      config under an already-running home, a literal-content reread instruction
#      is written to the copilot home and only its pointer is sent via the
#      routed copilot path (exact destination bytes, no summaries); unchanged
#      config sends nothing unless a previous send failure is pending.

#   C) Model/effort pin. config/copilot-harness may carry optional model and
#      effort tokens after the harness ("<harness> [<model>] [<effort>]"), read by
#      ap-harness.sh copilot-model / copilot-effort. A bare harness-only
#      line (today's format) yields empty model/effort - full backward-compat.
#      ap-spawn.sh populates MODEL/EFFORT from those tokens for a --copilot
...
# controlled, but bin/ap-harness.sh checks verified ENV markers before ancestry.
...
unset CLAUDECODE PI_CODING_AGENT AP_PI_HARNESS GROK_AGENT

BASE_PATH=${AP_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
ap_git_identity fmtest fmtest@example.com
TMP_ROOT=$(ap_test_tmproot ap-copilot-harness)
export AP_BACKEND=tmux

# ===========================================================================
# A) ap-harness.sh copilot resolution + fallback (deterministic detect_own)
# ===========================================================================
# detect_own is pinned to claude via CLAUDECODE=1 so the "fall through to own"
# cases are reproducible. Each row sets flight-crew-harness / copilot-harness in a
# fresh config dir (a literal '-' means leave the file absent) and asserts BOTH
# the copilot resolution AND that flight crew resolution is unchanged (backward-compat).
#   <label>^<flight-crew-harness>^<copilot-harness>^<expect-copilot>^<expect-flight-crew>
test_harness_resolution() {
  local label flight_crew copilot exp_sm exp_flight_crew case_dir cfg got_sm got_flight_crew n
  n=0
  while IFS='^' read -r label flight_crew copilot exp_sm exp_flight_crew; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    case_dir="$TMP_ROOT/harness-$n"
    cfg="$case_dir/config"
    mkdir -p "$cfg"
    [ "$flight_crew" = "-" ] || printf '%s\n' "$flight_crew" > "$cfg/flight-crew-harness"
    [ "$copilot" = "-" ] || printf '%s\n' "$copilot" > "$cfg/copilot-harness"
    got_sm=$(CLAUDECODE=1 AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh" copilot)
    got_flight_crew=$(CLAUDECODE=1 AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh" flight-crew)
    [ "$got_sm" = "$exp_sm" ] || fail "$label: copilot resolved '$got_sm', expected '$exp_sm'"
    [ "$got_flight_crew" = "$exp_flight_crew" ] || fail "$label: flight crew resolved '$got_flight_crew', expected '$exp_flight_crew'"
  done <<'ROWS'
both absent -> own (backward-compat)^-^-^claude^claude
flight crew set, copilot absent -> flight crew (backward-compat)^codex^-^codex^codex
flight crew set, copilot set -> copilot wins, flight crew untouched^codex^grok^grok^codex
flight crew absent, copilot set -> copilot value, flight crew own^-^grok^grok^claude
signed Pi wrapper remains a distinct copilot value^codex^pi-signed^pi-signed^codex
copilot=default defers to flight crew^codex^default^codex^codex
flight crew=default resolves to own, copilot follows^default^-^claude^claude
copilot=default with flight crew absent -> own^-^default^claude^claude
ROWS
  pass "A1 ap-harness.sh copilot resolves the fallback chain; flight crew mode unchanged"
}

# ===========================================================================
# C) ap-harness.sh copilot-model / copilot-effort token resolution
# ===========================================================================
# config/copilot-harness holds "<harness> [<model>] [<effort>]" on one line.
...
test_copilot_model_effort_tokens() {
  ...
  while IFS='^' read -r label line exp_harness exp_model exp_effort; do
    ...
    cfg="$case_dir/config"
    mkdir -p "$cfg"
    [ "$line" = ABSENT ] || printf '%b\n' "$line" > "$cfg/copilot-harness"
    got_h=$(CLAUDECODE=1 AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh" copilot)
    got_m=$(CLAUDECODE=1 AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh" copilot-model)
    got_e=$(CLAUDECODE=1 AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh" copilot-effort)
...
default harness token -> falls back to flight crew, empty model/effort^default^claude^^
...
ROWS
  pass "C1 ap-harness.sh copilot-model/copilot-effort resolve the optional tokens; bare harness stays empty (backward-compat)"
}
...
case "$pid:$field:${AP_TEST_SIGNED_SHAPE:-exact}" in
  100:comm=:*) printf '%s\n' '/test/Pi.app/bin/pi' ;;
  100:args=:*) printf '%s\n' 'Pi' ;;
  100:ppid=:*) printf '%s\n' 200 ;;
...
SH
  ...
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "unmarked shared signed-wrapper ancestry resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true AP_PI_HARNESS=pi-signed "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi-signed ] || fail "selected signed wrapper resolved '$got', expected pi-signed"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true AP_PI_HARNESS=pi "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "selected plain Pi resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true AP_PI_HARNESS=pi-signed-helper "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "inexact signed selection marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT -u PI_CODING_AGENT PATH="$fakebin:$BASE_PATH" AP_PI_HARNESS=pi-signed "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "signed selection marker without Pi's family marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true AP_TEST_SIGNED_SHAPE=plain "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "plain Pi marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true AP_TEST_SIGNED_SHAPE=helper "$ROOT/bin/ap-harness.sh")
  [ "$got" = pi ] || fail "unrelated pi-signed-helper ancestry resolved '$got', expected pi"

  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/ap-session-lock-lib.sh"; ap_harness_ancestry_pid' "$ROOT")
  [ "$got" = 100 ] || fail "session-lock ancestry selected '$got', expected the inner Pi engine pid 100"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/ap-session-lock-lib.sh"; kill() { return 0; }; ap_harness_pid_alive 200' "$ROOT" \
    || fail "session-lock liveness rejected exact pi-signed holder"
  if PATH="$fakebin:$BASE_PATH" AP_TEST_SIGNED_SHAPE=helper bash -c \
    '. "$0/bin/ap-session-lock-lib.sh"; kill() { return 0; }; ap_harness_pid_alive 200' "$ROOT"; then
...
SH
  ...
  err="$dir/ap-harness.err"
  got=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/ap-harness.sh" 2>"$err")
  [ "$got" = codex ] || fail "dash-leading shell ancestry resolved '$got', expected codex"
  [ ! -s "$err" ] || fail "ap-harness wrote basename option noise for literal -zsh: $(cat "$err")"

  err="$dir/ap-session-lock-ancestry.err"
  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/ap-session-lock-lib.sh"; ap_harness_ancestry_pid' "$ROOT" 2>"$err")
  ...
  err="$dir/ap-session-lock-alive.err"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/ap-session-lock-lib.sh"; kill() { return 0; }; ap_harness_pid_alive 5252' \
    "$ROOT" 2>"$err"; status=$?
...
test_propagate_lib() {
  ...
  printf '{"default":{"harness":"codex"}}\n' > "$src/flight-crew-dispatch.json"
  printf 'codex\n' > "$src/flight-crew-harness"
  printf 'manual\n' > "$src/backlog-backend"
  printf 'tmux\n' > "$src/backend"
  ...
  [ "$(cat "$dest/flight-crew-dispatch.json")" = '{"default":{"harness":"codex"}}' ] || fail "flight-crew-dispatch.json not propagated"
  [ "$(cat "$dest/flight-crew-harness")" = codex ] || fail "flight-crew-harness not propagated"
  ...
  m1=$(date -r "$dest/flight-crew-harness" +%s 2>/dev/null || stat -c %Y "$dest/flight-crew-harness")
  sleep 1
  ...
  m2=$(date -r "$dest/flight-crew-harness" +%s 2>/dev/null || stat -c %Y "$dest/flight-crew-harness")
  ...
  printf 'zellij\n' > "$src/backend"
  propagate_inheritable_config "$src" "$dest"
  [ "$(cat "$dest/flight-crew-dispatch.json")" = '{"default":{"harness":"claude"}}' ] || fail "changed dispatch profile did not converge"
  [ "$(cat "$dest/flight-crew-harness")" = claude ] || fail "changed value did not converge"
  ...
  rm -f "$dest/flight-crew-harness" "$outside"
  printf 'outside\n' > "$outside"
  ln -s "$outside" "$dest/flight-crew-harness"
  printf 'pi\n' > "$src/flight-crew-harness"
  propagate_inheritable_config "$src" "$dest"
  [ ! -L "$dest/flight-crew-harness" ] || fail "destination symlink was not replaced"
  [ "$(cat "$dest/flight-crew-harness")" = pi ] || fail "destination symlink replacement has wrong content"
  ...
  printf 'herdr\n' > "$dest/backend"
  rm -f "$src/flight-crew-dispatch.json" "$src/flight-crew-harness" "$src/backlog-backend" \
    "$src/backend" "$src/herdr-presentation-spaces"
  propagate_inheritable_config "$src" "$dest"
  [ -e "$dest/flight-crew-dispatch.json" ] && fail "dispatch profile absence not mirrored downstream"
  [ -e "$dest/flight-crew-harness" ] && fail "absence not mirrored downstream"
  ...
  rm -f "$dest/flight-crew-harness"
  ln -s "$d/missing-target" "$dest/flight-crew-harness"
  propagate_inheritable_config "$src" "$dest"
  [ -L "$dest/flight-crew-harness" ] && fail "broken destination symlink not removed on absence mirror"

  mkdir -p "$dest/flight-crew-harness"
  ...
  assert_contains "$(cat "$stderr")" "ap-config-inherit: error: failed to remove flight-crew-harness" \
    "remove error did not emit a stderr diagnostic"
  [ -d "$dest/flight-crew-harness" ] || fail "failed absence mirror removed the wrong path"
  rm -rf "$dest/flight-crew-harness"

  # 5. copilot-harness is never inherited; backend still is
  printf 'grok\n' > "$src/copilot-harness"
  printf '{"default":{"harness":"codex"}}\n' > "$src/flight-crew-dispatch.json"
  printf 'codex\n' > "$src/flight-crew-harness"
  printf 'manual\n' > "$src/backlog-backend"
  ...
  [ -e "$d/home2/config/copilot-harness" ] && fail "copilot-harness was inherited (must not be)"
  [ "$(cat "$d/home2/config/flight-crew-dispatch.json")" = '{"default":{"harness":"codex"}}' ] || fail "flight-crew-dispatch.json not propagated alongside"
  [ "$(cat "$d/home2/config/flight-crew-harness")" = codex ] || fail "flight-crew-harness not propagated alongside"
  ...
  printf 'config/flight-crew-harness\nconfig/backlog-backend\n' > "$guard_repo/.gitignore"
  ...
  AP_INHERITABLE_CONFIG=flight-crew-dispatch.json propagate_inheritable_config "$src" "$guard_repo/config" >"$stdout" 2>"$stderr" \
    || fail "guard skip should not make propagation fail"
  [ ! -s "$stdout" ] || fail "guard skip wrote to stdout"
  err_text=$(cat "$stderr")
  assert_contains "$err_text" "ap-config-inherit: warning: skipped flight-crew-dispatch.json" \
    "guard skip did not emit a stderr warning"
  [ ! -e "$guard_repo/config/flight-crew-dispatch.json" ] || fail "guard skip still copied the unignored item"

  pass "B1 propagate_inheritable_config: copy, idempotence, convergence, absence-mirror, exclusion, no-op, skip diagnostics"
}

# ===========================================================================
# B/A integration: a copilot spawn resolves the copilot harness and
# propagates the flight crew harness into the home's config.
# ===========================================================================
...
# A minimal seeded copilot home (validate_autopilot_home_for_spawn needs the
...
make_seeded_home() {
  ...
  printf '# Autopilot\n' > "$home/AGENTS.md"
  printf '%s\n' "$id" > "$home/.ap-copilot-home"
  printf 'charter\n' > "$home/data/charter.md"
}

# spawn_copilot <world> <id> <home> [explicit-harness]
# Runs ap-spawn.sh in copilot mode. AP_ROOT is the real repo (so ap-harness.sh
...
spawn_copilot() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    AP_ROOT_OVERRIDE="$ROOT" AP_HOME="$world/home" \
    AP_STATE_OVERRIDE="$world/home/state" AP_DATA_OVERRIDE="$world/home/data" \
    AP_PROJECTS_OVERRIDE="$world/home/projects" AP_CONFIG_OVERRIDE="$world/home/config" \
    AP_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/ap-spawn.sh" "${spawn_args[@]}" >/dev/null 2>&1 || true
}

meta_harness() { grep '^harness=' "$1" 2>/dev/null | tail -1 | cut -d= -f2-; }

# Split active: flight-crew-harness=claude + copilot-harness=codex. The copilot
# AGENT launches on codex; its own flight crew members inherit claude; copilot-harness
# does not flow into the home.
test_spawn_split_and_inherit() {
  local w copilot meta
  w="$TMP_ROOT/spawn-split"
  copilot="$w/copilot"
  mkdir -p "$w/home/config"
  printf '{"default":{"harness":"claude","model":"haiku","effort":"low"}}\n' > "$w/home/config/flight-crew-dispatch.json"
  printf 'claude\n' > "$w/home/config/flight-crew-harness"
  printf 'codex\n' > "$w/home/config/copilot-harness"
  printf 'manual\n' > "$w/home/config/backlog-backend"
  printf 'zellij\n' > "$w/home/config/backend"
  make_seeded_home "$copilot" copilot

  spawn_copilot "$w" copilot "$copilot"

  meta="$w/home/state/copilot.meta"
  [ -f "$meta" ] || fail "split: no meta written"
  [ "$(meta_harness "$meta")" = codex ] \
    || fail "split: copilot launched on '$(meta_harness "$meta")', expected codex"
  [ "$(cat "$copilot/config/flight-crew-harness" 2>/dev/null)" = claude ] \
    || fail "split: home flight-crew-harness not inherited as claude (got '$(cat "$copilot/config/flight-crew-harness" 2>/dev/null)')"
  [ "$(cat "$copilot/config/flight-crew-dispatch.json" 2>/dev/null)" = '{"default":{"harness":"claude","model":"haiku","effort":"low"}}' ] \
    || fail "split: home flight-crew-dispatch.json not inherited"
  [ "$(cat "$copilot/config/backlog-backend" 2>/dev/null)" = manual ] \
    || fail "split: home backlog-backend not inherited as manual"
  [ "$(cat "$copilot/config/backend" 2>/dev/null)" = zellij ] \
    || fail "split: home backend not inherited as zellij"
  [ -e "$copilot/config/copilot-harness" ] \
    && fail "split: copilot-harness leaked into the copilot home"
  pass "B2 spawn: copilot runs the copilot harness; its home inherits declared config"
}

# Backward-compat: copilot-harness absent -> the copilot launches on the
# flight crew harness, exactly as before this knob existed, and that flight crew value is the
# one inherited.
test_spawn_backward_compat_flight_crew_fallback() {
  local w copilot meta
  ...
  meta="$w/home/state/copilot.meta"
  [ "$(meta_harness "$meta")" = codex ] \
    || fail "compat: copilot launched on '$(meta_harness "$meta")', expected the flight crew harness codex"
  [ "$(cat "$copilot/config/flight-crew-harness" 2>/dev/null)" = codex ] \
    || fail "compat: home flight-crew-harness not inherited as codex"
  pass "B3 spawn: an absent copilot-harness falls back to the flight crew harness (backward-compat)"
}

# Bare backward-compat: no config at all. The copilot falls through to its own
...
test_spawn_bare_backward_compat() {
  ...
  [ "$(meta_harness "$meta")" = claude ] \
    || fail "bare: copilot launched on '$(meta_harness "$meta")', expected own harness claude"
  [ -e "$copilot/config/flight-crew-dispatch.json" ] && fail "bare: an unset primary still created a home flight-crew-dispatch.json"
  [ -e "$copilot/config/flight-crew-harness" ] && fail "bare: an unset primary still created a home flight-crew-harness"
  pass "B4 spawn: no config at all -> own harness and no propagation side effects"
}

# An explicit per-spawn harness arg wins over config/copilot-harness.
test_spawn_explicit_harness_wins() {
  ...
  spawn_copilot "$w" copilot "$copilot" claude

  meta="$w/home/state/copilot.meta"
  [ "$(meta_harness "$meta")" = claude ] \
    || fail "explicit: launched on '$(meta_harness "$meta")', expected explicit claude over config codex"
  pass "B5 spawn: an explicit per-spawn harness arg overrides config/copilot-harness"
}

# The unverified-adapter guard holds on the resolved copilot path: an unknown
# config/copilot-harness aborts the spawn (no meta written) and names the source.
test_spawn_unverified_copilot_harness_refused() {
  local w copilot fakebin err rc
  ...
  rc=0
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    AP_ROOT_OVERRIDE="$ROOT" AP_HOME="$w/home" \
    ...
    AP_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/ap-spawn.sh" copilot "$copilot" --copilot >/dev/null 2>"$err" || rc=$?
  ...
  assert_contains "$(cat "$err")" "config/copilot-harness" \
    "unverified: error names the copilot-harness source"
  [ -e "$w/home/state/copilot.meta" ] && fail "unverified: a meta was written despite the abort"
  pass "B6 spawn: an unverified resolved copilot harness is refused (guard intact)"
}

# ===========================================================================
# C integration: config/copilot-harness's optional model/effort tokens thread
# into the copilot launch command and meta, durably and without a new file.
# ===========================================================================

meta_field() { grep "^$2=" "$1" 2>/dev/null | tail -1 | cut -d= -f2-; }

# A tmux stub that behaves like make_noop_tmux but also captures the literal
# `send-keys -l <cmd>` launch command into AP_FAKE_LAUNCH_LOG, mirroring the
# capture technique in ap-spawn-dispatch-profile.test.sh so the constructed
# launch command (not just meta) can be asserted on. Also answers the
# `#{pane_current_path}` probe from AP_FAKE_PANE_PATH so this same stub works
# for a flight crew/recon (non-copilot) spawn's treehouse-worktree wait loop.
...
case "${1:-}" in
  ...
  send-keys)
    if [ -n "${AP_FAKE_LAUNCH_LOG:-}" ]; then
      prev=
      for a in "$@"; do
        if [ "$prev" = "-l" ]; then
          printf '%s\n' "$a" >> "$AP_FAKE_LAUNCH_LOG"
        fi
        prev=$a
      done
    fi
...
# spawn_copilot_capture <world> <id> <home> <launchlog> [extra ap-spawn.sh args...]
# Same shape as spawn_copilot but captures the launch command into <launchlog>
...
spawn_copilot_capture() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    AP_STATE_OVERRIDE="$world/home/state" AP_DATA_OVERRIDE="$world/home/data" \
    AP_PROJECTS_OVERRIDE="$world/home/projects" AP_CONFIG_OVERRIDE="$world/home/config" \
    AP_SPAWN_NO_GUARD=1 AP_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/ap-spawn.sh" "$id" "$home" "$@" --copilot
}

test_spawn_backend_precedence_over_inherited_config() {
  local w copilot meta launchlog out status
  ...
  out=$(AP_BACKEND=tmux spawn_copilot_capture \
    "$w" copilot "$copilot" "$launchlog" 2>&1); status=$?
  expect_code 0 "$status" \
    "AP_BACKEND=tmux should beat inherited config/backend=herdr"$'\n'"$out"

  meta="$w/home/state/copilot.meta"
  [ "$(cat "$copilot/config/backend")" = herdr ] \
    || fail "backend precedence fixture did not inherit config/backend=herdr"
  assert_no_grep '^backend=' "$meta" \
    "AP_BACKEND=tmux did not beat inherited config/backend=herdr"
  pass "B5b spawn: AP_BACKEND wins over inherited config/backend"
}
...
test_spawn_explicit_backend_precedence_over_env_and_inherited_config() {
  ...
  out=$(AP_BACKEND=zellij spawn_copilot_capture \
    "$w" copilot "$copilot" "$launchlog" --backend tmux 2>&1); status=$?
  expect_code 0 "$status" \
    "explicit --backend tmux should beat AP_BACKEND=zellij and inherited config/backend=herdr"$'\n'"$out"

  meta="$w/home/state/copilot.meta"
  [ "$(cat "$copilot/config/backend")" = herdr ] \
    || fail "explicit backend precedence fixture did not inherit config/backend=herdr"
  assert_no_grep '^backend=' "$meta" \
    "explicit --backend tmux did not beat AP_BACKEND=zellij and inherited config/backend=herdr"
  pass "B5c spawn: explicit --backend wins over AP_BACKEND and inherited config/backend"
}

# A bare "<harness>" copilot-harness file (today's format) must launch with
...
test_spawn_bare_harness_no_model_effort_flag() {
  local w copilot meta launchlog launch out status
  ...
  out=$(spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" 2>&1); status=$?
  expect_code 0 "$status" "bare-harness copilot spawn should succeed"
  ...
  pass "C2 spawn: a bare harness-only copilot-harness file launches with no model/effort flag (backward-compat)"
}

# "<harness> <model>" durably threads --model into the copilot launch and
# records it in meta, with no --effort flag (no effort token supplied).
test_spawn_copilot_harness_model_token() {
  local w copilot meta launchlog launch
  ...
  spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" >/dev/null 2>&1
  ...
  pass "C3 spawn: config/copilot-harness's model token threads --model into the launch and meta"
}

# "<harness> <model> <effort>" threads both flags into the launch and meta.
test_spawn_copilot_harness_model_and_effort_tokens() {
  local w copilot meta launchlog launch
  ...
  pass "C4 spawn: config/copilot-harness's model+effort tokens thread into the launch and meta"
}

# Precedence: an explicit per-spawn --model overrides the file's model token.
test_spawn_explicit_model_overrides_copilot_harness_token() {
  local w copilot meta launchlog launch
  ...
  spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" --model sonnet >/dev/null 2>&1
  ...
  pass "C5 spawn: an explicit --model overrides config/copilot-harness's model token; the file's effort token still applies"
}

# Precedence: an explicit per-spawn --effort overrides the file's effort token.
test_spawn_explicit_effort_overrides_copilot_harness_token() {
  local w copilot meta launchlog launch
  ...
  spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" --effort low >/dev/null 2>&1
  ...
  pass "C6 spawn: an explicit --effort overrides config/copilot-harness's effort token; the file's model token still applies"
}

test_spawn_explicit_harness_does_not_inherit_copilot_harness_tokens() {
  local w copilot meta launchlog launch
  ...
  spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" --harness codex >/dev/null 2>&1
...
test_spawn_explicit_harness_uses_explicit_profile_axes() {
  ...
  spawn_copilot_capture "$w" copilot "$copilot" "$launchlog" --harness codex --model gpt-5.5 --effort xhigh >/dev/null 2>&1
...
# The harness fallback chain (copilot-harness -> flight-crew-harness -> own) still
# resolves correctly with no model/effort tokens anywhere in the chain, and a
# flight crew/recon (non-copilot) launch is entirely unaffected by this feature: no
# model/effort is invented for it even though its own project has no profile set.
test_spawn_fallback_chain_and_flight_crew_recon_unaffected() {
  local w copilot meta home proj wt fakebin launchlog id launch
  w="$TMP_ROOT/spawn-fallback-and-flight-crew"
  copilot="$w/copilot"
  launchlog="$w/launch.log"
  mkdir -p "$w/home/config"
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  ...
  [ "$(meta_field "$meta" harness)" = codex ] \
    || fail "fallback: copilot harness did not fall back to flight-crew-harness codex"
  ...
  # Flight crew/recon launch: same flight-crew-harness config, no --copilot. Must resolve
  # the flight crew harness and record no model/effort - this codepath must never read
  # config/copilot-harness's tokens at all.
  id="flight-crew-unaffected-z1"
  home="$w/home"
  proj="$w/flight-crew-project"
  wt="$w/flight-crew-wt"
  fakebin=$(make_launch_capturing_tmux "$w/tmux-flight-crew")
  ap_git_worktree "$proj" "$wt" "wt-flight-crew"
  ...
  PATH="$fakebin:$BASE_PATH" TMUX="fake,1,0" CLAUDECODE=1 \
    ...
    AP_SPAWN_NO_GUARD=1 AP_FAKE_PANE_PATH="$wt" AP_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" >/dev/null 2>&1
  meta="$home/state/$id.meta"
  [ "$(meta_field "$meta" kind)" = flight ] || fail "flight-crew-unaffected: expected an ordinary flight task"
  [ "$(meta_field "$meta" harness)" = codex ] || fail "flight-crew-unaffected: flight crew harness resolution changed"
  [ "$(meta_field "$meta" model)" = default ] || fail "flight-crew-unaffected: flight crew task must not invent a model"
  [ "$(meta_field "$meta" effort)" = default ] || fail "flight-crew-unaffected: flight crew task must not invent an effort"
  launch=$(cat "$launchlog")
  assert_not_contains "$launch" "--model" "flight-crew-unaffected: flight crew launch must not carry a --model flag"
  assert_not_contains "$launch" "--effort" "flight-crew-unaffected: flight crew launch must not carry an --effort flag"
  pass "C9 spawn: the harness fallback chain still resolves with no tokens; flight crew/recon launches are unaffected by this feature"
}
...
# A PRIMARY autopilot repo on main with one commit + a home dir, mirroring the
# real gitignore (config/flight-crew-harness ignored, so a propagated value never dirties
# the copilot worktree on a later sweep). Echoes the world dir.
new_world() {
  ...
  {
    printf 'projects/\nstate/\ndata/\n.no-mistakes/\n'
    [ "$dispatch_ignore" = no ] || printf 'config/flight-crew-dispatch.json\n'
    printf 'config/flight-crew-harness\nconfig/copilot-harness\nconfig/backlog-backend\n'
...
# A live copilot home as a DETACHED worktree of the primary at <commit>, with
# its seed marker and a live kind=copilot meta.
...
make_fake_toolchain() {
  ...
  # tmux fake supports ap-send's composer-verified submit path and optional
  # AP_FAKE_TMUX_LOG / AP_FAKE_TMUX_FAIL_LITERAL for reread-nudge assertions.
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
...
run_bootstrap() {
  ...
  if [ -n "$log" ]; then
    PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
...
reread_retry_stage_path() {
  ...
  retry_dir="$home/state/.ap-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.ap-inherited-config-reread.*; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
reread_retry_report_path() {
  local home=$1 id=$2 retry_dir path latest=
  retry_dir="$home/state/.ap-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.ap-inherited-config-reread.*.report; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
assert_no_reread_pending() {
  local home=$1 state path
  state="$home/state"
  for path in "$state"/.ap-inherited-config-reread.*.pending; do
...
# primary clears it - all while never inheriting copilot-harness.
test_bootstrap_sweep_propagates_and_reconverges() {
  local w c1
  w=$(new_world boot-prop)
  c1=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$c1"

  # Initial push: primary flight-crew-harness=codex, copilot-harness=grok (must NOT flow).
  printf '{"default":{"harness":"codex"}}\n' > "$w/home/config/flight-crew-dispatch.json"
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  printf 'manual\n' > "$w/home/config/backlog-backend"
  printf 'tmux\n' > "$w/home/config/backend"
  ...
  [ "$(cat "$w/copilot/config/flight-crew-harness" 2>/dev/null)" = codex ] \
    || fail "sweep: flight-crew-harness not pushed into the live home"
  [ "$(cat "$w/copilot/config/flight-crew-dispatch.json" 2>/dev/null)" = '{"default":{"harness":"codex"}}' ] \
    || fail "sweep: flight-crew-dispatch.json not pushed into the live home"
  [ "$(cat "$w/copilot/config/backlog-backend" 2>/dev/null)" = manual ] \
    || fail "sweep: backlog-backend not pushed into the live home"
  [ "$(cat "$w/copilot/config/backend" 2>/dev/null)" = tmux ] \
    || fail "sweep: backend not pushed into the live home"
  [ -e "$w/copilot/config/copilot-harness" ] \
    && fail "sweep: copilot-harness was inherited (must not be)"
  ...
  printf 'zellij\n' > "$w/home/config/backend"
  run_bootstrap "$w" >/dev/null
  [ "$(cat "$w/copilot/config/flight-crew-harness" 2>/dev/null)" = claude ] \
    || fail "sweep: home did not re-converge to the primary's new flight-crew-harness"
  [ "$(cat "$w/copilot/config/flight-crew-dispatch.json" 2>/dev/null)" = '{"default":{"harness":"claude"}}' ] \
    || fail "sweep: home did not re-converge to the primary's new flight-crew-dispatch.json"
  [ "$(cat "$w/copilot/config/backlog-backend" 2>/dev/null)" = tasks-axi ] \
    || fail "sweep: home did not re-converge to the primary's new backlog-backend"
  [ "$(cat "$w/copilot/config/backend" 2>/dev/null)" = zellij ] \
  ...
  rm -f "$w/home/config/flight-crew-dispatch.json" "$w/home/config/flight-crew-harness" \
    "$w/home/config/backlog-backend" "$w/home/config/backend"
  run_bootstrap "$w" >/dev/null
  [ -e "$w/copilot/config/flight-crew-dispatch.json" ] \
    && fail "sweep: home flight-crew-dispatch.json not removed after the primary cleared it"
  [ -e "$w/copilot/config/flight-crew-harness" ] \
    && fail "sweep: home flight-crew-harness not removed after the primary cleared it"
  [ -e "$w/copilot/config/backlog-backend" ] \
    && fail "sweep: home backlog-backend not removed after the primary cleared it"
  [ -e "$w/copilot/config/backend" ] \
    && fail "sweep: home backend not removed after the primary cleared it"
  pass "B7 bootstrap sweep pushes, re-converges, and mirrors absence; never inherits copilot-harness"
}
...
test_bootstrap_sweep_propagates_when_tracked_current() {
  ...
  add_copilot_worktree "$w" copilot "$head"   # already on the primary's HEAD (ff is a no-op)
  ...
  run_bootstrap "$w" >/dev/null
  [ "$(cat "$w/copilot/config/flight-crew-dispatch.json" 2>/dev/null)" = '{"default":{"harness":"codex"}}' ] \
    || fail "flight-crew-dispatch.json did not propagate to a tracked-current home"
  [ "$(cat "$w/copilot/config/flight-crew-harness" 2>/dev/null)" = codex ] \
...
test_bootstrap_sweep_defers_dispatch_on_stale_unignored_home() {
  ...
  add_copilot_worktree "$w" copilot "$(git -C "$w/main" rev-parse HEAD)"
  printf 'local divergence\n' >> "$w/copilot/README.md"
  git -C "$w/copilot" add README.md
  git -C "$w/copilot" commit -qm local
  printf 'config/flight-crew-dispatch.json\n' >> "$w/main/.gitignore"
  ...
  assert_contains "$out" "COPILOT_SYNC: copilot copilot: skipped: diverged from" \
    "stale dispatch: expected fast-forward skip"
  [ ! -e "$w/copilot/config/flight-crew-dispatch.json" ] \
    || fail "stale dispatch: flight-crew-dispatch.json was copied before the home ignored it"
  [ "$(cat "$w/copilot/config/flight-crew-harness" 2>/dev/null)" = codex ] \
    || fail "stale dispatch: existing ignored config stopped propagating"
  [ "$(cat "$w/copilot/config/backlog-backend" 2>/dev/null)" = manual ] \
    || fail "stale dispatch: backlog backend stopped propagating"
  status=$(git -C "$w/copilot" status --porcelain -- config/flight-crew-dispatch.json)
  [ -z "$status" ] || fail "stale dispatch: flight-crew-dispatch.json dirtied the home: $status"
...
test_bootstrap_sweep_materializes_and_inherits_memory_default() {
  ...
  [ -e "$w/copilot/config/flight-crew-dispatch.json" ] && fail "default-only sweep created a home flight-crew-dispatch.json"
  [ -e "$w/copilot/config/flight-crew-harness" ] && fail "default-only sweep created a home flight-crew-harness"
  [ -e "$w/copilot/config/backend" ] && fail "default-only sweep created a home backend"
  [ "$(cat "$w/home/config/startup-memory-budget")" = 7500 ] \
    || fail "primary bootstrap did not materialize the startup-memory default"
  [ "$(cat "$w/copilot/config/startup-memory-budget")" = 7500 ] \
    || fail "default-only sweep did not converge startup-memory-budget"
  [ "$(git -C "$w/copilot" rev-parse HEAD)" = "$head" ] \
...
test_backend_inheritance_present_and_absent() {
  ...
  head=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$head"
  ...
  [ "$(cat "$w/copilot/config/backend")" = tmux ] || fail "backend present value not pushed"
  instruction=$(reread_instruction_path "$w/copilot") || fail "backend present reread instruction missing"
  assert_contains "$(cat "$instruction")" $'-----BEGIN config/backend-----\ntmux\n-----END config/backend-----' \
    "backend present reread must include exact bytes"

  printf 'herdr\n' > "$w/copilot/config/backend"
  printf 'zellij\n' > "$w/home/config/backend"
  out=$(run_config_push "$w" 2>"$err"); status=$?
  ...
  [ "$(cat "$w/copilot/config/backend")" = zellij ] \
  ...
  [ -e "$w/copilot/config/backend" ] && fail "backend not removed on primary absence"
  instruction=$(reread_instruction_path "$w/copilot") || fail "backend absence reread instruction missing"
  assert_contains "$(cat "$instruction")" $'-----BEGIN config/backend-----\nABSENT\n-----END config/backend-----' \
...
test_bootstrap_sweep_surfaces_config_propagation_failure() {
  ...
  c1=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$c1"
  mkdir -p "$w/copilot/config/flight-crew-harness"

  out=$(run_bootstrap "$w")

  fail_line=$(printf '%s\n' "$out" | grep '^COPILOT_SYNC: copilot copilot: skipped: inheritance failed' || true)
  [ -n "$fail_line" ] || fail "bootstrap did not surface inheritance propagation failure (got: $out)"
  [ -d "$w/copilot/config/flight-crew-harness" ] || fail "failed propagation removed the wrong path"
...
test_bootstrap_rereads_after_partial_propagation() {
  ...
  add_copilot_worktree "$w" copilot "$head"
  printf '{"default":{"harness":"codex"}}\n' > "$w/home/config/flight-crew-dispatch.json"
  printf 'invalid shared header\n' > "$w/home/data/pilot-shared.md"
  log="$w/boot-prop-partial.tmux.log"

  out=$(run_bootstrap "$w" "$log")
  assert_contains "$out" "COPILOT_SYNC: copilot copilot: skipped: inheritance failed" \
    "partial bootstrap propagation did not remain diagnostic"
  [ "$(cat "$w/copilot/config/flight-crew-dispatch.json")" = '{"default":{"harness":"codex"}}' ] \
    || fail "partial bootstrap propagation did not retain the completed config write"
  instruction=$(reread_instruction_path "$w/copilot") || fail "partial bootstrap reread instruction missing"
  assert_present "$instruction" "partial bootstrap propagation did not write a reread instruction"
  pointer="CONFIG_REREAD: $(reread_instruction_path "$w/copilot")"
...
test_config_push_propagates_reports_without_ff_or_nudge() {
  local w c1 copilot_real old_head out err status out2 tmp log instruction
  w=$(new_world config-push-basic)
  c1=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$c1"
  copilot_real=$(cd "$w/copilot" && pwd -P)
  printf -- '- copilot - config push target (home: %s; scope: config; projects: alpha; added 2026-06-30)\n' "$copilot_real" > "$w/home/data/copilots.md"
  tmp="$w/home/state/copilot.meta.tmp"
  grep -v '^home=' "$w/home/state/copilot.meta" > "$tmp"
  mv "$tmp" "$w/home/state/copilot.meta"

  printf 'v2\n' > "$w/main/AGENTS.md"
  git -C "$w/main" add AGENTS.md
  git -C "$w/main" commit -qm c2
  old_head=$(git -C "$w/copilot" rev-parse HEAD)
  ...
  assert_contains "$out" "config-push: $w/home -> live copilot homes" \
    "config push lacked the header"
  assert_contains "$out" "copilot copilot ($copilot_real):" \
    "config push did not discover the live copilot through registry fallback"
  assert_contains "$out" "flight-crew-dispatch.json: pushed" \
    "config push did not report flight-crew-dispatch as pushed"
  assert_contains "$out" "flight-crew-harness: pushed" \
    "config push did not report flight-crew-harness as pushed"
  ...
  [ "$(git -C "$w/copilot" rev-parse HEAD)" = "$old_head" ] \
    || fail "config push fast-forwarded tracked files"
  [ "$(cat "$w/copilot/config/backend")" = tmux ] || fail "config push did not write backend"
  instruction=$(reread_instruction_path "$w/copilot") || fail "config-push reread instruction missing"
  assert_contains "$(cat "$instruction")" $'-----BEGIN config/backend-----\ntmux\n-----END config/backend-----' \
  ...
  assert_contains "$(cat "$log")" "[ap-from-autopilot]" \
    "config reread must use the marked routed copilot path"

  : > "$log"
  out2=$(run_config_push "$w" "$log" 2>"$err"); status=$?
  expect_code 0 "$status" "idempotent config push should succeed"
  assert_contains "$out2" "flight-crew-dispatch.json: unchanged" \
    "idempotent config push did not report flight-crew-dispatch as unchanged"
  assert_contains "$out2" "flight-crew-harness: unchanged" \
    "idempotent config push did not report flight-crew-harness as unchanged"
...
test_config_push_reports_skips_dirty_and_invalid_home() {
  ...
  head=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" dirty "$head"
  add_copilot_worktree "$w" stale "$head"
  dirty_real=$(cd "$w/dirty" && pwd -P)
  stale_real=$(cd "$w/stale" && pwd -P)

  printf 'local edit\n' >> "$w/dirty/README.md"
  tmp="$w/stale/.gitignore.tmp"
  grep -v '^config/flight-crew-dispatch.json$' "$w/stale/.gitignore" > "$tmp"
  mv "$tmp" "$w/stale/.gitignore"

  bad_home="$w/not-copilot"
  mkdir -p "$bad_home"
  {
    printf 'window=autopilot:ap-bad\n'
    printf 'kind=copilot\n'
    printf 'home=%s\n' "$bad_home"
  ...
  assert_contains "$out" "copilot dirty ($dirty_real):" \
  ...
  assert_contains "$out" "copilot stale ($stale_real):" \
    "config push did not report stale home"
  assert_contains "$out" "flight-crew-dispatch.json: skipped - destination does not allow inherited item" \
    "config push did not report non-allowing item skip"
  assert_contains "$out" "copilot bad ($bad_home): skipped - unsafe home: not a seeded copilot home" \
    "config push did not report invalid copilot home"
  err_text=$(cat "$err")
  assert_contains "$err_text" "ap-config-inherit: warning: skipped flight-crew-dispatch.json" \
...
test_config_push_exits_nonzero_on_copy_error() {
  local w head out err status copilot_real err_text
  ...
  assert_contains "$out" "flight-crew-harness: error - failed to copy" \
    "config push did not report the per-item copy error"
  err_text=$(cat "$err")
  assert_contains "$err_text" "ap-config-inherit: error: failed to copy flight-crew-harness" \
...
test_config_push_rereads_after_partial_propagation() {
  ...
  assert_contains "$out" "data/pilot-shared.md: error" \
  ...
  instruction=$(reread_instruction_path "$w/copilot") || fail "partial propagation reread instruction missing"
...
test_config_reread_per_home_changed_sets_and_exact_bytes() {
  ...
  head=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" alpha "$head"
  add_copilot_worktree "$w" beta "$head"
  ...
  printf 'pi\n' > "$w/alpha/config/flight-crew-harness"
  printf 'tasks-axi\n' > "$w/alpha/config/backlog-backend"
  printf '{"default":{"harness":"old"}}\n' > "$w/beta/config/flight-crew-dispatch.json"

  multiline_json=$(printf '{\n  "default": {\n    "harness": "grok",\n    "model": "grok-4.5"\n  },\n  "rules": [\n    {"when": "news", "use": {"harness": "grok"}}\n  ]\n}\n')
  printf '%s' "$multiline_json" > "$w/home/config/flight-crew-dispatch.json"
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  printf 'manual\n' > "$w/home/config/backlog-backend"
  printf 'tmux\n' > "$w/home/config/backend"
  {
  ...
  cmp -s "$w/home/config/flight-crew-dispatch.json" "$w/alpha/config/flight-crew-dispatch.json" \
    || fail "alpha did not receive multiline dispatch"
  cmp -s "$w/home/config/flight-crew-dispatch.json" "$w/beta/config/flight-crew-dispatch.json" \
    || fail "beta did not receive multiline dispatch"
  [ "$(cat "$w/alpha/config/flight-crew-harness")" = codex ] || fail "alpha harness not updated"
  ...
  assert_contains "$(cat "$instr_a")" "config/flight-crew-dispatch.json" "alpha missing dispatch path"
  assert_contains "$(cat "$instr_a")" "config/flight-crew-harness" "alpha missing harness path"
  ...
  # Path order follows AP_INHERITABLE_CONFIG.
  awk '
    /config\/flight-crew-dispatch\.json/ { d=NR }
    /config\/flight-crew-harness/ { h=NR }
  ...
  assert_contains "$(cat "$instr_a")" $'-----BEGIN config/flight-crew-harness-----\ncodex\n-----END config/flight-crew-harness-----' \
  ...
  # No parsed/effective summary, no SHA, no pilot-shared dump.
  ...
  assert_not_contains "$(cat "$instr_a")" "SHA" "must not emit SHA tokens"
  assert_not_contains "$(cat "$instr_a")" "pilot-shared" "pilot-shared path must not appear"
  assert_not_contains "$(cat "$instr_a")" "shared secret preference body" \
    "pilot-shared content must never be inlined"
  assert_not_contains "$(cat "$instr_b")" "shared secret preference body" \
    "beta must not inline pilot-shared either"
  ...
  # Routed send used the from-autopilot marker and carried only the pointer.
  pointer="CONFIG_REREAD: $(reread_instruction_path "$w/alpha")"
  assert_contains "$(cat "$log")" "[ap-from-autopilot]" "reread send must be marked"
...
test_config_reread_isolation_and_absent_and_send_failure() {
  ...
  printf 'old-harness\n' > "$w/alpha/config/flight-crew-harness"
  printf '{"stale":true}\n' > "$w/beta/config/flight-crew-dispatch.json"
  # Primary has only flight-crew-harness set; dispatch and backlog absent.
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  rm -f "$w/home/config/flight-crew-dispatch.json" "$w/home/config/backlog-backend"
  ...
  assert_contains "$(cat "$instr_a")" "config/flight-crew-harness" "alpha should mention harness"
  assert_contains "$(cat "$instr_a")" $'-----BEGIN config/flight-crew-harness-----\ncodex\n-----END config/flight-crew-harness-----' \
    "alpha harness block exact"
  assert_not_contains "$(cat "$instr_a")" "config/flight-crew-dispatch.json" \
  ...
  assert_contains "$(cat "$instr_b")" "config/flight-crew-dispatch.json" "beta should mention dispatch"
  assert_contains "$(cat "$instr_b")" $'-----BEGIN config/flight-crew-dispatch.json-----\nABSENT\n-----END config/flight-crew-dispatch.json-----' \
  ...
  {
    printf '%s\n' $'flight-crew-dispatch.json\tpushed\tmirrored primary absence'
    printf '%s\n' $'flight-crew-harness\tunchanged\t'
    printf '%s\n' $'backlog-backend\tunchanged\t'
    printf '%s\n' $'backend\tunchanged\t'
    printf '%s\n' $'data/pilot-shared.md\tpushed\t'
  } > "$report"
  rm -f "$w/beta/config/flight-crew-dispatch.json"
  ap_config_write_reread_instruction "$w/beta" "$report" "$w/beta/state/.ap-inherited-config-reread-absent" \
    || fail "ABSENT instruction write failed"
  assert_contains "$(cat "$w/beta/state/.ap-inherited-config-reread-absent")" \
    $'-----BEGIN config/flight-crew-dispatch.json-----\nABSENT\n-----END config/flight-crew-dispatch.json-----' \
    "helper ABSENT representation"
  assert_not_contains "$(cat "$w/beta/state/.ap-inherited-config-reread-absent")" "pilot-shared" \
    "helper must ignore pilot-shared even when report says pushed"
  assert_not_contains "$(cat "$w/beta/state/.ap-inherited-config-reread-absent")" "config/flight-crew-harness" \
  ...
  out=$(PATH="$(make_fake_toolchain "$w"):$BASE_PATH" \
    AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" AP_SEND_SETTLE=0 \
    AP_FAKE_TMUX_FAIL_LITERAL=1 \
    "$ROOT/bin/ap-config-push.sh" 2>"$err"); status=$?
  expect_code 1 "$status" "send failure should make config-push exit non-zero"
  assert_contains "$out" "CONFIG_REREAD: copilot" "send failure diagnostic missing"
  ...
  out2=$(PATH="$(make_fake_toolchain "$w"):$BASE_PATH" \
    AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" AP_SEND_SETTLE=0 \
    AP_FAKE_TMUX_FAIL_LITERAL=1 \
    "$ROOT/bin/ap-config-push.sh" 2>"$err"); status2=$?
...
#!/usr/bin/env bash
case "\$*" in
  *"$alpha_state/.ap-inherited-config-reread."*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 "$ROOT/bin/ap-config-push.sh" 2>&1); status=$?
  expect_code 1 "$status" "publication failure should remain diagnostic"
  assert_contains "$out" "CONFIG_REREAD: copilot" "publication failure diagnostic missing"
  assert_not_contains "$out" "config-reread: sent" \
    "publication failure must not claim reread delivery"
  [ "$(cat "$w/alpha/config/flight-crew-harness")" = codex ] \
  ...
  assert_contains "$(cat "$stage")" \
    $'-----BEGIN config/flight-crew-harness-----\ncodex\n-----END config/flight-crew-harness-----' \
...
test_config_reread_write_failure_retains_exact_retry_generation() {
  ...
  head=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$head"
  mkdir -p "$w/copilot/config" "$w/copilot/state"
  printf 'old\n' > "$w/copilot/config/flight-crew-harness"
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  fakebin=$(make_fake_toolchain "$w")
  real_mv=$(command -v mv)
  mkdir -p "$w/home/state/.ap-inherited-config-reread-retry/copilot"
  retry_dir=$(cd "$w/home/state/.ap-inherited-config-reread-retry/copilot" && pwd -P)
  cat > "$fakebin/mv" <<SH
#!/usr/bin/env bash
target=
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.ap-inherited-config-reread.*)
    case "\$target" in
      *.exact) ;;
      *) exit 1 ;;
    esac
...
SH
  ...
  stage_path=$(reread_retry_stage_path "$w/home" copilot) \
  ...
  assert_no_reread_retry_stages "$w/home" copilot
...
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.ap-inherited-config-reread.*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
...
SH
  ...
  (
    PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
      AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
      "$ROOT/bin/ap-config-push.sh" > "$first_out" 2>&1
  ) &
  first_pid=$!
  ...
  first_instr=$(reread_instruction_path "$w/copilot") \
    || fail "first concurrent push did not publish its generation"
  printf 'two\n' > "$w/home/config/flight-crew-harness"
  second_out="$w/second-push.out"
  PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/ap-config-push.sh" > "$second_out" 2>&1
  second_status=$?
  ...
  second_instr=$(reread_instruction_path "$w/copilot") \
  ...
  [ "$(cat "$w/copilot/config/flight-crew-harness")" = two ] \
...
test_config_reread_full_retry_queue_drains_before_new_push() {
  ...
  printf 'old\n' > "$w/copilot/config/flight-crew-harness"
  printf 'new\n' > "$w/home/config/flight-crew-harness"
  retry_dir="$w/home/state/.ap-inherited-config-reread-retry/copilot"
  mkdir -p "$retry_dir"
  for n in $(seq -w 1 16); do
    path="$retry_dir/.ap-inherited-config-reread.20260721T000000.$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  fakebin=$(make_fake_toolchain "$w")
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/ap-config-push.sh" 2>&1); status=$?
  ...
  [ "$(cat "$w/copilot/config/flight-crew-harness")" = new ] \
...
test_config_reread_cleanup_runs_after_mixed_delivery_failure() {
  ...
  head=$(git -C "$w/main" rev-parse HEAD)
  add_copilot_worktree "$w" copilot "$head"
  mkdir -p "$w/copilot/state"
  state_real=$(cd "$w/copilot/state" && pwd -P)
  fail_path="$state_real/.ap-inherited-config-reread.9999-fail"
  for n in $(seq -w 1 18); do
    path="$state_real/.ap-inherited-config-reread.00$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  printf 'failed-generation\n' > "$fail_path"
  chmod 0600 "$fail_path"
  for path in "$state_real"/.ap-inherited-config-reread.*; do
    ap_config_reread_mark_pending "$path" "$path.pending" \
...
SH
  ...
  : > "$report"
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 ap_config_send_reread_nudge copilot "$w/copilot" "$report" 2>&1); status=$?
  expect_code 1 "$status" "mixed delivery failure should remain diagnostic"
  assert_contains "$out" "CONFIG_REREAD: copilot copilot: send failed" \
...
test_config_reread_stops_after_failed_generation() {
  ...
  mkdir -p "$w/copilot/state"
  state_real=$(cd "$w/copilot/state" && pwd -P)
  old="$state_real/.ap-inherited-config-reread.0000-fail"
  new="$state_real/.ap-inherited-config-reread.0001-new"
  printf 'old-generation\n' > "$old"
  printf 'new-generation\n' > "$new"
  chmod 0600 "$old" "$new"
  ap_config_reread_mark_pending "$old" "$old.pending" \
    || fail "could not mark older generation pending"
  ap_config_reread_mark_pending "$new" "$new.pending" \
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$ROOT" \
    AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
    ap_config_send_reread_nudge copilot "$w/copilot" "$report" 2>&1); status=$?
...
test_bootstrap_detect_only_does_not_create_state() {
  ...
  fakebin=$(make_fake_toolchain "$w")
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_STATE_OVERRIDE="$detect_state" AP_BOOTSTRAP_DETECT_ONLY=1 \
    "$ROOT/bin/ap-bootstrap.sh" 2>&1); status=$?
...
test_config_reread_skips_when_unchanged_and_reads_after_push() {
  ...
  assert_no_reread_instructions "$w/copilot"
  ...
  # pushed report line.
  printf '%s' 'destination-post-write' > "$w/copilot/config/flight-crew-harness"
  printf 'primary-source-only\n' > "$w/home/config/flight-crew-harness"
  report="$w/after-push.report"
  printf '%s\n' $'flight-crew-harness\tpushed\t' > "$report"
  instr="$w/copilot/state/.ap-inherited-config-reread-dest"
  ap_config_write_reread_instruction "$w/copilot" "$report" "$instr" \
  ...
  assert_contains "$(cat "$instr")" $'destination-post-write-----END config/flight-crew-harness-----' \
  ...
  assert_contains "$(cat "$instr")" $'-----BEGIN config/flight-crew-harness-----
-----END config/flight-crew-harness-----' \
    "instruction must represent an empty destination without a synthetic byte"
  pending_instruction="$w/copilot/state/.ap-inherited-config-reread.20260721T000000.01"
  printf '%s\n' generation > "$pending_instruction"
  ap_config_reread_mark_pending "$pending_instruction" "$pending_instruction.pending" \
    || fail "could not create bounded-lifecycle pending marker"
  for n in $(seq -w 2 18); do
    path="$w/copilot/state/.ap-inherited-config-reread.20260721T000000.$n"
    printf '%s\n' generation > "$path"
    chmod 0600 "$path"
  done
  ap_config_reread_cleanup_sent "$w/copilot"
  count=0
  for path in "$w/copilot/state"/.ap-inherited-config-reread.*; do
    case "$path" in
      *.pending) continue ;;
    esac
    [ -f "$path" ] && [ ! -L "$path" ] || continue
...
test_config_reread_bootstrap_path_and_spawn_flexibility() {
  local w head log out fakebin copilot launchlog launch instr report stale
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null)
  [ "$(cat "$w/copilot/config/flight-crew-harness")" = codex ] || fail "bootstrap did not push harness"
  instr=$(reread_instruction_path "$w/copilot") || fail "bootstrap reread instruction missing"
  assert_present "$instr" "bootstrap must write a config reread instruction when config changed"
  assert_contains "$(cat "$log")" "[ap-from-autopilot]" \
    "bootstrap config reread must use routed copilot send"
  assert_contains "$(cat "$instr")" \
    $'-----BEGIN config/flight-crew-harness-----\ncodex\n-----END config/flight-crew-harness-----' \
    "bootstrap instruction must carry exact post-write harness bytes"

  # ap-spawn still permits a conscious explicit runtime outside the config
  # (defaults/rules only - never harden spawn against deliberate choice).
  w=$(new_world config-reread-spawn-flex)
  printf 'codex\n' > "$w/home/config/flight-crew-harness"
  printf 'codex\n' > "$w/home/config/copilot-harness"
  copilot="$w/copilot-flex"
  make_seeded_home "$copilot" copilot-flex
  mkdir -p "$copilot/state"
  report="$copilot/state/stale-reread.report"
  printf '%s\n' $'flight-crew-harness\tpushed\t' > "$report"
  stale="$copilot/state/.ap-inherited-config-reread.spawn-stale"
  ap_config_write_reread_instruction "$copilot" "$report" "$stale" \
    || fail "could not create spawn stale reread generation"
  ap_config_reread_mark_pending "$stale" "$stale.pending" \
    || fail "could not create spawn stale reread marker"
  launchlog="$w/spawn-flex.launch.log"
  spawn_copilot_capture "$w" copilot-flex "$copilot" "$launchlog" --harness pi >/dev/null 2>&1
  assert_no_reread_pending "$copilot"
  assert_no_reread_instructions "$copilot"
  launch=$(cat "$launchlog")
...
test_bootstrap_respawns_before_config_reread() {
  ...
  add_copilot_worktree "$w" copilot "$head"
  mkdir -p "$w/copilot/config" "$w/copilot/state"
  printf 'harness=codex\n' >> "$w/home/state/copilot.meta"
  printf '%s' old > "$w/copilot/config/flight-crew-harness"
  printf '%s' codex > "$w/home/config/flight-crew-harness"
  report="$w/copilot/state/stale-reread.report"
  printf '%s\n' $'flight-crew-harness\tpushed\t' > "$report"
  stale="$w/copilot/state/.ap-inherited-config-reread.stale-generation"
  ap_config_write_reread_instruction "$w/copilot" "$report" "$stale" \
...
cat > "$w/main/bin/ap-spawn.sh" <<SH
#!/usr/bin/env bash
. '$w/main/bin/ap-config-inherit-lib.sh'
printf '%s' spawn >> '$log'
printf '%s' codex > '$w/copilot/config/flight-crew-harness'
printf '%s\n' 7500 > '$w/copilot/config/startup-memory-budget'
SH
  chmod +x "$w/main/bin/ap-spawn.sh"
  fakebin=$(make_fake_toolchain "$w")
  cat > "$fakebin/tmux" <<SH
#!/usr/bin/env bash
case "\$*" in
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" AP_HOME="$w/home" AP_ROOT_OVERRIDE="$w/main" \
    AP_SEND_SETTLE=0 AP_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/ap-bootstrap.sh" >/dev/null 2>&1
  assert_contains "$(cat "$log")" "spawn" \
    "bootstrap did not respawn the dead copilot"
  assert_not_contains "$(cat "$log")" "send-keys" \
    "bootstrap nudged a copilot before its respawn completed"
  ...
  ap_config_reread_discard_pending "$w/copilot" || fail "could not clean respawn test generation"
  assert_no_reread_pending "$w/copilot"
  assert_no_reread_instructions "$w/copilot"
  pass "B19 bootstrap respawns before inherited-config reread"
}

test_spawn_quarantines_pending_rereads_on_cleanup_failure() {
  local w copilot report stale fakebin real_rm out status launchlog quarantine_root quarantined_count
  ...
  quarantine_root="$copilot/state/.ap-inherited-config-reread-quarantine"
...
#!/usr/bin/env bash
case "\$*" in
  *'.ap-inherited-config-reread.'*) exit 1 ;;
esac
exec "$real_rm" "\$@"
SH
  chmod +x "$fakebin/rm"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    AP_PROJECTS_OVERRIDE="$w/home/projects" AP_CONFIG_OVERRIDE="$w/home/config" \
    AP_SPAWN_NO_GUARD=1 AP_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/ap-spawn.sh" copilot "$copilot" --copilot 2>&1); status=$?
  expect_code 0 "$status" "spawn should remain available after reread cleanup failure"
  assert_contains "$out" "CONFIG_REREAD: copilot copilot: quarantined pre-relaunch generations" \
  ...
  ap_config_reread_quarantine_pending "$copilot" copilot "$w/home" || true
...
echo "# all ap-copilot-harness tests passed"
