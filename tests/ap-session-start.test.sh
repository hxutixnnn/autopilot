#!/usr/bin/env bash
# tests/ap-session-start.test.sh - behavior tests for bin/ap-session-start.sh,
...
#     watcher ownership
#   - status-tail bounding, default and AP_SESSION_START_STATUS_TAIL override
...
#     tmux and herdr both
#   - composition: the script invokes the real ap-lock.sh/ap-bootstrap.sh/
#     ap-wake-drain.sh (their real, distinctive output appears verbatim), it
...
SESSION_START="$ROOT/bin/ap-session-start.sh"
BASE_PATH=${AP_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
TMP_ROOT=$(ap_test_tmproot ap-session-start-tests)
SESSION_START_COPILOT_ID="fmtest-copilot-${TMP_ROOT##*.}"
SESSION_START_COPILOT_TMP="/tmp/ap-$SESSION_START_COPILOT_ID"
SESSION_START_HERDR_COPILOT_ID="fmtest-herdr-${TMP_ROOT##*.}"
SESSION_START_HERDR_COPILOT_TMP="/tmp/ap-$SESSION_START_HERDR_COPILOT_ID"
AP_TEST_CLEANUP_DIRS+=("$TMP_ROOT" "$SESSION_START_COPILOT_TMP" "$SESSION_START_HERDR_COPILOT_TMP")
trap ap_test_cleanup EXIT
...
# autopilot repo) to use as AP_ROOT_OVERRIDE, plus an empty AP_HOME with
...
# make_fake_toolchain <fakebin>: every tool ap-bootstrap.sh detects, present
# and compatible, so its own detect-only section stays quiet except where a
# test deliberately breaks one. Mirrors ap-bootstrap.test.sh's fixture.
make_fake_toolchain() {
  local fakebin=$1
  ap_fake_exit0 "$fakebin" tmux node gh-axi chrome-devtools-axi lavish-axi
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
...
tasks[2]{id,state,kind,repo,title,blocked_by,hold_kind,hold_reason}:
  compact-startup,in_flight,flight,autopilot,Compact startup digest,none,pilot,pilot choice pending
  blocked-followup,queued,recon,autopilot,Follow compact startup,compact-startup,"-","-"
help[2]:
...
# make_fake_ps_claude <fakebin>: harness_pid()/holder_alive() (ap-lock.sh) walk
...
# deterministically. Mirrors ap-grok-harness.test.sh's fake ps.
make_fake_ps_claude() {
  local fakebin=$1
...
#!/usr/bin/env bash
set -u
harness=${AP_FAKE_HARNESS:-claude}
case "$*" in
...
# ap_backend_target_exists uses for a tmux endpoint liveness read.
make_fake_tmux() {
...
# make_fake_tmux_copilot_recovery <fakebin>: a stateful tmux boundary
# fixture for the real session-start -> bootstrap -> spawn path.
# AP_FAKE_TMUX_MODE selects missing, ambiguous, unreadable, or shell; missing
# reproduces real tmux's active-window fallback while inventory omits the copilot.
make_fake_tmux_copilot_recovery() {
  local fakebin=$1
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
set -u
mode=${AP_FAKE_TMUX_MODE:?}
log=${AP_FAKE_TMUX_LOG:?}
spawned=${AP_FAKE_TMUX_SPAWNED:?}
killed=${spawned}.killed
mate_home=${AP_FAKE_COPILOT_HOME:?}
mate_id=${AP_FAKE_COPILOT_ID:?}
mate_window="ap-$mate_id"
case "${1:-}" in
  display-message)
    target=
    format=
...
make_fake_herdr_copilot_recovery() {
  local fakebin=$1
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
set -u
log=${AP_FAKE_HERDR_LOG:?}
...
case "${1:-} ${2:-}" in
  "status --json")
    ...
    ;;
  "workspace list")
    printf '{"result":{"workspaces":[{"workspace_id":"ws1","label":"copilot-%s"}]}}\n' "$mate_id"
    ;;
  "tab list")
    if [ -e "$spawned" ]; then
      printf '{"result":{"tabs":[{"tab_id":"t-new","workspace_id":"ws1","label":"ap-%s"}]}}\n' "$mate_id"
    elif [ -e "$killed" ]; then
      printf '%s\n' '{"result":{"tabs":[]}}'
    else
      printf '{"result":{"tabs":[{"tab_id":"t-old","workspace_id":"ws1","label":"ap-%s"}]}}\n' "$mate_id"
    fi
    ;;
  "tab create")
    : > "$spawned"
...
# for the given pane id - the exact primitive ap_backend_target_exists uses
# for a herdr endpoint liveness read. No version/server-start calls: a
# liveness check must never auto-start a server (ap-backend.sh's contract).
make_fake_herdr() {
  local fakebin=$1 live=$2
  cat > "$fakebin/herdr" <<SH
#!/usr/bin/env bash
...
# Drop every harness env marker from bin/ap-harness.sh detect_own so the
# surrounding interactive shell cannot leak past the suite's fake ps harness.
# Markers today: CLAUDECODE (claude), PI_CODING_AGENT plus AP_PI_HARNESS
...
run_session_start() {
  ...
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u GROK_AGENT PI_CODING_AGENT=true AP_PI_HARNESS="$pi_harness" \
      AP_HOME="$home" AP_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u PI_CODING_AGENT -u AP_PI_HARNESS -u GROK_AGENT \
      AP_HOME="$home" AP_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}

# prepare_session_start_copilot <name>: a throwaway main home and Pi
# copilot home wired to the real spawn implementation through the fixture
# root. Echoes root|home|fakebin|copilot|log|spawned.
prepare_session_start_copilot() {
  local name=$1 rec root home fakebin w copilot log spawned id=$SESSION_START_COPILOT_ID
  rec=$(new_world "$name")
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  w=${root%/root}
  copilot="$w/copilot-$id"
  log="$w/tmux.log"
  spawned="$w/tmux.spawned"
  mkdir -p "$copilot/bin" "$copilot/data" "$copilot/state" "$copilot/config" "$copilot/projects"
  printf '%s\n' "$id" > "$copilot/.ap-copilot-home"
  printf '# Autopilot\n' > "$copilot/AGENTS.md"
  printf 'Copilot charter.\n' > "$copilot/data/charter.md"
  printf '%s\n' pi > "$home/config/copilot-harness"
  ...
  make_fake_tmux_copilot_recovery "$fakebin"
  : > "$log"
  printf '%s|%s|%s|%s|%s|%s\n' "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned"
}

run_session_start_copilot() {
  local root=$1 home=$2 fakebin=$3 copilot=$4 log=$5 spawned=$6 mode=$7
  TMUX='' AP_BACKEND=tmux AP_FAKE_TMUX_MODE="$mode" AP_FAKE_TMUX_LOG="$log" \
    AP_FAKE_TMUX_SPAWNED="$spawned" AP_FAKE_COPILOT_HOME="$copilot" \
    AP_FAKE_COPILOT_ID="$SESSION_START_COPILOT_ID" \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH"
}

prepare_session_start_herdr_copilot() {
  local name=$1 rec root home fakebin w copilot log state id=$SESSION_START_HERDR_COPILOT_ID
  rec=$(new_world "$name")
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  ...
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_herdr_copilot_recovery "$fakebin"
  : > "$log"
  printf '%s|%s|%s|%s|%s|%s\n' "$root" "$home" "$fakebin" "$copilot" "$log" "$state"
}

run_session_start_herdr_copilot() {
  local root=$1 home=$2 fakebin=$3 copilot=$4 log=$5 state=$6
  AP_BACKEND=herdr AP_FAKE_HERDR_LOG="$log" AP_FAKE_HERDR_STATE="$state" \
    AP_FAKE_COPILOT_ID="$SESSION_START_HERDR_COPILOT_ID" \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH"
}
...
install_pi_turnend_extension_fixture() {
  ...
  cp "$ROOT/.pi/extensions/ap-primary-turnend-guard.ts" "$root/.pi/extensions/ap-primary-turnend-guard.ts"
}

install_pi_watch_extension_fixture() {
  local root=$1
  mkdir -p "$root/.pi/extensions"
  cp "$ROOT/.pi/extensions/ap-primary-pi-watch.ts" "$root/.pi/extensions/ap-primary-pi-watch.ts"
}

write_pi_watch_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/ap-primary-pi-watch.ts")
  printf '%s\n%s\n' "$version" "$pid" > "$home/state/.pi-watch-extension-loaded"
}

write_pi_turnend_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/ap-primary-turnend-guard.ts")
...
EOF
  ...
  : > "$home/data/pilot.md"
  # copilots.md, pilot-shared.md, and learnings.md deliberately absent
  ...
  assert_contains "$out" "data/pilot.md" "digest did not label the pilot.md section"
  assert_contains "$out" "data/pilot-shared.md (shared, main-authoritative, read-only in copilot homes)" \
    "digest did not label the shared pilot section"

  assert_contains "$out" "data/copilots.md" "digest did not label the copilots.md section"
  assert_contains "$out" "data/learnings.md" "digest did not label the learnings.md section"

  # Exactly four context ABSENT markers (copilots.md, pilot-shared.md,
  # learnings.md; backlog.md is covered by its own test) - and the
  # present-but-empty pilot.md must NOT print ABSENT.
  absent_count=$(printf '%s\n' "$out" | grep -c '^ABSENT$')
  [ "$absent_count" -eq 4 ] || fail "expected 4 ABSENT markers (copilots.md, pilot-shared.md, learnings.md, backlog.md), got $absent_count: $out"

  cap_section=$(printf '%s\n' "$out" | awk '/^data\/pilot\.md$/{flag=1;next}/^data\//{flag=0}flag')
  assert_contains "$cap_section" "(present, empty)" "empty-but-present pilot.md was not distinguished from ABSENT"
...
EOF
  ...
  # A live copilot meta with a window pointed at nothing real - if the
  # bootstrap sweep's copilot_sync ran (a MUTATING step), it would try to
  # fast-forward this "home" and/or report a COPILOT_SYNC/NUDGE_COPILOTS
  # line. Absence of any such line is this test's proof that
  # AP_BOOTSTRAP_DETECT_ONLY=1 actually suppressed the mutating sweep.
  mkdir -p "$home/other-copilot/state"
  ap_write_copilot_meta "$home/state/copilot-x.meta" "$home/other-copilot" "autopilot:ap-copilot-x" alpha
  append_wake "$home/state" signal copilot-x "done: surfaced before refusal" || fail "seed wake failed"
  git -C "$root" checkout -q -B ap/read-only-tangle
  ...
  expect_code 0 "$status" "ap-session-start.sh must exit 0 even on a lock refusal"
  assert_contains "$out" "READ-ONLY SESSION" "read-only banner missing on lock refusal"
  assert_contains "$out" "another live autopilot session holds the lock" "read-only banner did not surface ap-lock.sh's own error text"
  ...
  assert_contains "$out" "TANGLE: primary checkout on feature branch 'ap/read-only-tangle'" "read-only bootstrap did not surface the tangle diagnostic"
  ...
  assert_not_contains "$out" "drain them with bin/ap-wake-drain.sh" "read-only guard printed a mutating drain instruction"
  assert_not_contains "$out" "After draining queued wakes" "read-only guard printed a drain-then-rearm instruction"
  assert_not_contains "$out" "run bin/ap-watch-arm.sh" "read-only guard printed a mutating watcher-arm instruction"
  ...
  # The mutating copilot sweep must NOT have run: no COPILOT_SYNC/
  # NUDGE_COPILOTS line, and the sowed copilot meta's target dir is
  # untouched (ap-ff-lib would have tried to fast-forward it otherwise).
  assert_not_contains "$out" "COPILOT_SYNC" "mutating copilot sweep ran during a lock refusal"
  assert_not_contains "$out" "NUDGE_COPILOTS" "mutating copilot sweep ran during a lock refusal"
...
EOF
  ...
  expect_code 0 "$status" "ap-session-start.sh must exit 0 when lock publication fails"
  ...
  assert_not_contains "$out" "ANOTHER LIVE AUTOPILOT SESSION HOLDS THE FLEET LOCK" "lock publication failure falsely claimed a live lock holder"
...
case "$*" in
  *"comm="*)
    if [ -f "$AP_FAKE_LOCK_STATE/harness-$pid" ]; then
...
SH
  ...
  while [ "$i" -le 40 ]; do
    (
      ...
      if AP_HOME="$home" AP_FAKE_LOCK_STATE="$home/state" \
        AP_FAKE_HARNESS_PID="$harness_pid" PATH="$fakebin:$BASE_PATH" \
        "$ROOT/bin/ap-lock.sh" >/dev/null 2>&1; then
        printf '%s\n' "$harness_pid" >> "$winners"
      fi
      : > "$completed/$i"
...
EOF
    ...
    make_fake_ps_claude "$fakebin"
    rm -f "$fakebin/tmux"
    ap_fake_exit0 "$fakebin" herdr jq
    printf '%s\n' manual > "$home/config/backlog-backend"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "ap-sess:live"

  printf 'window=ap-sess:live\nkind=flight\n' > "$home/state/task-a.meta"
  ...
  out=$(AP_SESSION_START_STATUS_TAIL=2 run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "working: step 7" "AP_SESSION_START_STATUS_TAIL=2 tail missing the most recent line"
  assert_not_contains "$out" "working: step 5" "AP_SESSION_START_STATUS_TAIL=2 did not bound the tail to 2 lines"
...
# --- session-start copilot recovery boundary -----------------------------

test_session_start_relaunches_missing_pi_copilot() {
  local rec root home fakebin copilot log spawned out first_calls second_calls
  rec=$(prepare_session_start_copilot copilot-missing-pi)
  IFS='|' read -r root home fakebin copilot log spawned <<EOF
$rec
EOF

  out=$(run_session_start_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned" missing)

  assert_not_contains "$out" "COPILOT_LIVENESS:" "successful missing-window recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "new-window" "session start did not relaunch the missing Pi copilot"
  assert_not_contains "$(cat "$log")" "kill-window" "session start tried to kill an already-absent window"
  assert_contains "$out" "endpoint: alive (backend=tmux window=autopilot:ap-$SESSION_START_COPILOT_ID)" \
    "the later fleet read did not confirm the relaunched window"
  assert_grep 'harness=pi' "$home/state/$SESSION_START_COPILOT_ID.meta" \
    "the real respawn path did not preserve the Pi harness: $(cat "$home/state/$SESSION_START_COPILOT_ID.meta")"

  first_calls=$(grep -c 'new-window' "$log" || true)
  rm -f "$home/state/.lock"
  run_session_start_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned" missing >/dev/null
  second_calls=$(grep -c 'new-window' "$log" || true)
  [ "$first_calls" -eq 1 ] && [ "$second_calls" -eq 1 ] \
    || fail "a second session-start pass duplicated the relaunched Pi copilot: $(cat "$log")"
  pass "session start: an absent recorded tmux window relaunches its Pi copilot exactly once"
}

test_session_start_preserves_ambiguous_pi_process() {
  local rec root home fakebin copilot log spawned out
  rec=$(prepare_session_start_copilot copilot-ambiguous-pi)
  IFS='|' read -r root home fakebin copilot log spawned <<EOF
$rec
EOF

  out=$(run_session_start_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned" ambiguous)

  assert_contains "$out" "COPILOT_LIVENESS: copilot $SESSION_START_COPILOT_ID: skipped: existing endpoint has ambiguous agent process (backend=tmux)" \
...
test_session_start_preserves_transiently_unreadable_tmux() {
  local rec root home fakebin copilot log spawned out
  rec=$(prepare_session_start_copilot copilot-unreadable-pi)
  IFS='|' read -r root home fakebin copilot log spawned <<EOF
$rec
EOF

  out=$(run_session_start_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned" unreadable)

  assert_contains "$out" "COPILOT_LIVENESS: copilot $SESSION_START_COPILOT_ID: skipped: endpoint probe unreadable (backend=tmux)" \
  ...
  assert_contains "$out" "endpoint: dead (backend=tmux window=autopilot:ap-$SESSION_START_COPILOT_ID)" \
...
test_session_start_preserves_proven_bare_shell_recovery() {
  local rec root home fakebin copilot log spawned out
  rec=$(prepare_session_start_copilot copilot-bare-shell)
  IFS='|' read -r root home fakebin copilot log spawned <<EOF
$rec
EOF

  out=$(run_session_start_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$spawned" shell)

  assert_not_contains "$out" "COPILOT_LIVENESS:" "successful bare-shell recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "kill-window -t =autopilot:=ap-$SESSION_START_COPILOT_ID" \
...
test_session_start_relaunches_herdr_husk_copilot() {
  local rec root home fakebin copilot log state out
  rec=$(prepare_session_start_herdr_copilot copilot-herdr-husk)
  IFS='|' read -r root home fakebin copilot log state <<EOF
$rec
EOF

  out=$(run_session_start_herdr_copilot "$root" "$home" "$fakebin" "$copilot" "$log" "$state")

  assert_not_contains "$out" "COPILOT_LIVENESS:" "successful Herdr husk recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "pane close p-old" "session start did not close the confirmed Herdr husk"
  assert_contains "$(cat "$log")" "tab create" "session start did not relaunch the Herdr copilot"
  ...
  assert_grep 'herdr_pane_id=p-new' "$home/state/$SESSION_START_HERDR_COPILOT_ID.meta" \
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "ap-sess:live-window"

  printf 'window=ap-sess:live-window\nkind=flight\n' > "$home/state/task-live.meta"
  printf 'window=ap-sess:dead-window\nkind=flight\n' > "$home/state/task-dead.meta"

  out=$(run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "endpoint: alive (backend=tmux window=ap-sess:live-window)" "live tmux endpoint not reported alive"
  assert_contains "$out" "endpoint: dead (backend=tmux window=ap-sess:dead-window)" "dead tmux endpoint not reported dead"
...
EOF
  ...
  # ap-lock.sh's own exact success text.
  assert_contains "$out" "lock acquired: harness pid" "ap-lock.sh's real output did not appear (composition, not reimplementation)"
  # ap-bootstrap.sh's own exact MISSING-tool line format.
  assert_contains "$out" "MISSING: node (install:" "ap-bootstrap.sh's real detect line did not appear verbatim"
  # ap-wake-drain.sh's real drained record (raw tab-separated queue line).
  assert_contains "$out" "$(printf 'signal\ttask-z.status\tneeds-decision: pick a library')" "ap-wake-drain.sh's real drained record did not appear"
  assert_contains "$out" "wake annotation: latest wake-EVENT observed at drain, not current state: task-z.status: needs-decision: pick a library" "ap-session-start.sh did not preserve the drain's separate annotation line"

  pass "ap-session-start.sh composes the real ap-lock.sh, ap-bootstrap.sh, and ap-wake-drain.sh output verbatim"
}
...
## In flight
- [ ] compact-startup - Compact startup digest (repo: autopilot) (kind: flight) (since 2026-07-15) (hold: pilot choice pending) (hold-kind: external)
...
## Queued
- [ ] blocked-followup - Follow compact startup blocked-by: compact-startup - waits for implementation (repo: autopilot) (kind: recon) (since 2026-07-15)
...
EOF
  ...
  mkdir -p "$home/projects/autopilot"
  printf 'window=ap-sess:compact\nworktree=%s\nproject=autopilot\nkind=flight\n' "$home/projects/autopilot" \
    > "$home/state/compact-startup.meta"
  log="$home/tasks-axi.log"

  out=$(AP_FAKE_TASKS_AXI_LOG="$log" run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "compact-startup,in_flight,flight,autopilot,Compact startup digest,none,pilot,pilot choice pending" \
    "tasks-axi compact listing omitted in-flight identity, state, or hold metadata"
  assert_contains "$out" 'blocked-followup,queued,recon,autopilot,Follow compact startup,compact-startup,"-","-"' \
  ...
  assert_contains "$out" "worktree=$home/projects/autopilot" "in-flight recovery worktree identity disappeared from startup digest"
...
EOF
  ...
  assert_contains "$out" "(hold: pilot choice pending) (hold-kind: external)" \
...
EOF
  ...
  assert_not_contains "$out" "  bin/ap-watch-arm.sh" "AFK next step still told the agent to arm the watcher directly"
...
EOF
  ...
  out=$(AP_FAKE_HARNESS=pi run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "restart plain pi so $root/.pi/extensions/ap-primary-turnend-guard.ts and $root/.pi/extensions/ap-primary-pi-watch.ts auto-load" "pi extension load diagnostic omits the turn-end guard extension"
...
EOF
  ...
  out=$(AP_FAKE_HARNESS=pi-signed run_session_start "$home" "$root" "$fakebin:$BASE_PATH" pi-signed)
  ...
  assert_contains "$out" "restart pi-signed so $root/.pi/extensions/ap-primary-turnend-guard.ts and $root/.pi/extensions/ap-primary-pi-watch.ts auto-load" \
