...
# (bin/ap-claude-stop-autoarm.sh, docs/watcher-continuity.md).
#
...
# through the real ap-lock.sh path. The arm wrapper is a per-test fixture, so no
# real watcher, model, or fleet state is touched.
# shellcheck disable=SC2016 # single quotes are deliberate: $AP_HOME expands inside the fake harness child, and grep needles are literal strings
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-claude-stop-autoarm)
ap_git_identity fmtest fmtest@example.invalid

FAKEBIN=$(ap_fakebin "$TMP_ROOT/fakebin")
...
install_autoarm_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/ap-claude-stop-autoarm.sh" "$dir/bin/ap-claude-stop-autoarm.sh"
  ...
  cp "$ROOT/bin/ap-session-lock-lib.sh" "$dir/bin/ap-session-lock-lib.sh"
  cp "$ROOT/bin/ap-lock.sh" "$dir/bin/ap-lock.sh"
  chmod +x "$dir/bin/ap-claude-stop-autoarm.sh" "$dir/bin/ap-lock.sh"
}
...
make_copilot_dir() {
  ...
  printf 'copilot-autoarm-1\n' > "$dir/.ap-copilot-home"
  printf '%s\n' "$dir"
}

# A genuine linked git worktree: the shape every flight crew member/recon task worktree
# has (git-dir != git-common-dir), which must keep the hook inert.
make_flight_crew_member_worktree_dir() {
  local base=$1 dir=$2
  ap_git_worktree "$base" "$dir" ap/autoarm-test-branch
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_autoarm_scripts "$dir"
  printf '%s\n' "$dir"
...
run_autoarm() {
  ...
  printf '%s\n' '{"session_id":"sess-autoarm","stop_hook_active":false}' \
    | AP_HOME="$dir" "$FAKE_CLAUDE" -c '
        printf "%s\n" "$$" > "$AP_HOME/state/.lock"
        "$AP_HOME/bin/ap-claude-stop-autoarm.sh"
      ' 2>&1 || rc=$?
  printf 'RC=%s\n' "$rc" >&2
  return "$rc"
}

# Arm fixture variants, installed per test as <dir>/bin/ap-watch-arm.sh.
write_arm_fixture() {
  local dir=$1 kind=$2
  case "$kind" in
    actionable)
      cat > "$dir/bin/ap-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
echo "$$" >> "$AP_HOME/state/arm-ran"
...
SH
  ...
  chmod +x "$dir/bin/ap-watch-arm.sh"
}
...
test_inert_in_child_worktree() {
  ...
  base="$TMP_ROOT/flight-crew-base"
  dir="$TMP_ROOT/flight-crew-wt"
...
test_inert_without_session_lock() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | AP_HOME="$dir" bash "$dir/bin/ap-claude-stop-autoarm.sh" 2>&1); status=$?
...
test_reclaims_stale_session_lock_before_arming() {
  ...
  pass "auto-arm: a demonstrably dead recorded session owner is reclaimed through ap-lock.sh before arming"
}
...
test_inert_when_lock_held_by_other_harness() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | AP_HOME="$dir" "$FAKE_CLAUDE" -c '"$AP_HOME/bin/ap-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_stale_lock_recovery_preserves_afk_and_need_gates() {
  ...
  out=$(printf '%s\n' '{"session_id":"stale-afk"}' | AP_HOME="$afk_dir" "$FAKE_CLAUDE" -c '"$AP_HOME/bin/ap-claude-stop-autoarm.sh"' 2>&1); status=$?
  ...
  out=$(printf '%s\n' '{"session_id":"stale-idle"}' | AP_HOME="$idle_dir" "$FAKE_CLAUDE" -c '"$AP_HOME/bin/ap-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_inert_when_fleet_idle() {
  ...
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 0 "$status" "hook must exit 0 in an idle home with no work"
...
test_actionable_close_rewakes_with_reason() {
  ...
  assert_contains "$out" "autopilot watcher wake" "rewake must carry the wake banner"
  assert_contains "$out" "stale: fixture-win actionable" "rewake must carry the arm's reason line"
  assert_contains "$out" "bin/ap-wake-drain.sh" "rewake must direct the drain-first protocol"
  assert_contains "$out" "do NOT run bin/ap-watch-arm.sh" "rewake must forbid a duplicate model re-arm"
...
test_single_flight_admits_exactly_one_owner() {
  ...
  AP_HOME="$dir" "$FAKE_CLAUDE" -c '
    printf "%s\n" "$$" > "$AP_HOME/state/.lock"
    printf "%s\n" "{\"session_id\":\"s\"}" | "$AP_HOME/bin/ap-claude-stop-autoarm.sh" >/dev/null 2>"$AP_HOME/state/err1" &
    p1=$!
    printf "%s\n" "{\"session_id\":\"s\"}" | "$AP_HOME/bin/ap-claude-stop-autoarm.sh" >/dev/null 2>"$AP_HOME/state/err2" &
    p2=$!
    wait "$p1"; echo $? > "$AP_HOME/state/rc1"
    wait "$p2"; echo $? > "$AP_HOME/state/rc2"
  '
...
test_active_in_marked_copilot_home() {
  local dir out status
  dir=$(make_copilot_dir "$TMP_ROOT/copilot")
  : > "$dir/state/task.meta"
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "a marked copilot home must get the same active auto-arm as the main primary"
  [ -e "$dir/state/arm-ran" ] || fail "hook did not arm in a marked copilot home"
  [ "$(epoch_outcome "$dir")" = rewake ] || fail "copilot epoch must record outcome=rewake"
  pass "auto-arm: active in a marked copilot home"
}

test_ap_lock_status_still_works_with_shared_lib() {
  local out
  out=$(AP_HOME="$TMP_ROOT/lock-status-home" bash "$ROOT/bin/ap-lock.sh" status 2>&1)
  assert_contains "$out" "lock: free" "ap-lock.sh status must keep working after the session-lock lib extraction"
  pass "ap-lock: shared session-lock lib preserves the status path"
}
