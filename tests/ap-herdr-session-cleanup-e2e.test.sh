...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/ap-herdr-session-cleanup-e2e.XXXXXX")
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name ap-herdr-session-start-stale-projection-cleanup-r1)
...
production_process_proof() {
  AP_HOME="$HOME_DIR" AP_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
    AP_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1 PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" \
    bash -c '. "$1"; ap_herdr_cleanup_process_is_idle_shell "$2" "$3"' \
      _ "$ROOT/bin/ap-herdr-session-cleanup.sh" "$HERDR_LAB_SESSION" "$PANE"
}
focus_snapshot() {
...
ANCHOR=$(lab workspace create --cwd "$ROOT" --label pilot-anchor --focus) || fail 'could not create focus anchor'
...
AP_HOME="$HOME_DIR" AP_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" "$ROOT/bin/ap-herdr-session-cleanup.sh" \
