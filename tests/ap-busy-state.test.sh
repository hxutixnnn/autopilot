#!/usr/bin/env bash
# Behavior tests for the semantic busy-state contract (bin/ap-busy-lib.sh and
# its only writer bin/ap-busy-event.sh).
#
# Covers the pilot-approved redesign invariants: busy/idle/unknown/dead with
...
. "$ROOT/bin/ap-busy-lib.sh"

TMP_ROOT=$(ap_test_tmproot ap-busy-state)
EV="$ROOT/bin/ap-busy-event.sh"
...
test_arm_seeds_busy_spawn() {
  ...
  out=$(ap_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy ap-spawn" ] || fail "seed should classify 'busy ap-spawn', got '$out'"
  pass "arm mints a gen sidecar and seeds busy ap-spawn at seq=1"
}
...
test_apply_advances_seq_and_source() {
  ...
  seq=$(ap_busy_record_read "$state" t1 | awk '{print $4}')
...
test_apply_current_gen_reset() {
  ...
  "$EV" apply "$state" t1 idle --current-gen --source ap-interrupt --event interrupt \
    || fail "apply --current-gen failed"
  out=$(ap_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "idle ap-interrupt" ] || fail "expected 'idle ap-interrupt', got '$out'"
  "$EV" apply "$state" t1 unknown --current-gen --source ap-recovery --event relaunch \
    || fail "apply unknown failed"
  out=$(ap_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "unknown ap-recovery" ] || fail "expected 'unknown ap-recovery', got '$out'"
  pass "autopilot-owned interrupt and recovery events bind to the current gen"
}
...
test_retire_serializes_and_rejects_stale_gen() {
  ...
  out=$(ap_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy ap-spawn" ] || fail "stale retirement changed the new incarnation, got '$out'"
...
test_stale_gen_event_rejected() {
  ...
  out=$(ap_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy ap-spawn" ] || fail "stale event must not change the record, got '$out'"
...
test_missing_record_unknown_not_idle() {
  ...
  for h in claude opencode pi pi-signed; do
    out=$(ap_busy_classify tmux w1 "$h" t1 "$state")
    [ "$out" = "unknown missing" ] || fail "$h with no record must be 'unknown missing', got '$out'"
  done
  out=$(ap_busy_classify tmux w1 codex t1 "$state")
...
test_source_mismatch_cross_adapter() {
  ...
  out=$(ap_busy_classify tmux w1 grok t1 "$state")
...
Ctrl+c:cancel'
  for h in claude opencode pi pi-signed; do
    out=$(ap_busy_classify tmux w1 "$h" t1 "$state" "$tail")
    [ "$out" = "unknown missing" ] || fail "$h must never classify from footer text, got '$out'"
  done
  out=$(ap_busy_classify tmux w1 codex t1 "$state" "$tail")
...
test_grok_regex_isolated() {
  ...
  out=$(ap_busy_classify tmux w1 grok t1 "$state" 'thinking hard
Ctrl+c:cancel')
  [ "$out" = "busy grok-regex" ] || fail "grok busy tail must classify 'busy grok-regex', got '$out'"
  out=$(ap_busy_classify tmux w1 grok t1 "$state" 'done.
> ')
  ...
  out=$(ap_busy_classify tmux w1 grok t1 "$state" '• Working (6s • esc to interrupt)')
...
test_codex_unverified_gate() {
  ...
  [ -z "$(ap_busy_sources_for_harness codex)" ] \
...
test_kimi_unverified_gate() {
  ...
  out=$(ap_busy_classify tmux w1 kimi t1 "$state")
  [ "$out" = "unknown kimi-unverified" ] || fail "unverified kimi must classify unknown, got '$out'"
  out=$(ap_busy_classify tmux w1 kimi t1 "$state" '🌒 · thinking')
...
test_dead_endpoint_overrides() {
  ...
  gen=$("$EV" arm "$state" t1)
  # shellcheck disable=SC2329 # invoked indirectly through ap_busy_classify_live
  ap_backend_target_exists() { return 1; }
  out=$(ap_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "dead endpoint-gone" ] || fail "gone endpoint must classify dead, got '$out'"
  # shellcheck disable=SC2329 # invoked indirectly through ap_busy_classify_live
  ap_backend_target_exists() { return 0; }
  out=$(ap_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "busy ap-spawn" ] || fail "live endpoint must fall through to the record, got '$out'"
  out=$(ap_busy_classify_live tmux '' claude t1 "$state")
  [ "$out" = "unknown no-target" ] || fail "empty target must classify unknown, got '$out'"
  unset -f ap_backend_target_exists
...
test_herdr_native_busy_only() {
  ...
  # shellcheck disable=SC2329 # invoked indirectly through ap_busy_classify
  ap_backend_busy_state() { printf '%s' "$FAKE_NATIVE"; }
  FAKE_NATIVE=busy
  out=$(ap_busy_classify herdr s:p claude t1 "$state")
...
# flight-crew-state reader), so it must not disturb their shell: no clobbered
...
test_record_read_leaves_caller_shell_intact() {
  ...
  out=$(bash -c '
    set -f
    . "$1/bin/ap-busy-lib.sh"
    set -- keepme second
    ap_busy_record_read "$2" t1 >/dev/null
    printf "%s|%s|%s" "$1" "$#" "$-"
  ' _ "$ROOT" "$state")
  case "$out" in
    keepme\|2\|*f*) : ;;
...
test_boolean_view_never_promotes_unknown() {
  ...
  gen=$("$EV" arm "$state" t1)
  ap_busy_is_busy tmux w1 claude t1 "$state" || fail "busy record must read busy"
  "$EV" apply "$state" t1 idle --gen "$gen" --source claude-hook --event stop
  if ap_busy_is_busy tmux w1 claude t1 "$state"; then
...
echo "all ap-busy-state tests passed"
