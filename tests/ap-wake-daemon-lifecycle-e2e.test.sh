#!/usr/bin/env bash
# tests/ap-wake-daemon-lifecycle-e2e.test.sh - the watcher + supervise-daemon
...
#   drain queued records -> exactly ONE pilot-relevant digest is buffered
...
# ap-watch.sh runs plus the daemon's own functions. The pilot-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (ap-daemon.test.sh, ap-watcher-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(ap_test_tmproot ap-wake-daemon-e2e)
...
# always-on standalone triage is covered by ap-watch-triage.test.sh. fakebin
...
test_routine_then_terminal_after_restart() {
  ...
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after routine signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
    || fail "routine signal was not queued"
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  ...
  : > "$drain_out"
  AP_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after terminal signal failed"
  AP_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "pilot-relevant terminal status was not buffered"
  ...
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_PANE_ALIVE=1 AP_FAKE_TMUX_SENT="$sent" \
    AP_FAKE_TMUX_CAPTURE="$dir/pane.txt" AP_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_stale_pane_transient_persistent_resume() {
  ...
  case "$(AP_STATE_OVERRIDE="$state" classify_stale "$win" "$state")" in
    self\|*) : ;;
  ...
  PATH="$fakebin:$PATH" AP_FAKE_TMUX_WINDOW="$win" AP_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
  ...
  # Resumed: a fresh transient marker but the flight crew is provably working again ->
  # housekeeping clears the marker without escalating. The proof is the flight crew's
  # own semantic busy-state record (bin/ap-busy-lib.sh), not rendered pane text.
  stale_marker_record "$win" "$state"
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  printf 'Working...\n' > "$dir/pane.txt"
  ap_write_meta "$state/stale-w2.meta" "window=$win" "worktree=$dir/wt" "kind=flight" "harness=pi"
  resumed_gen=$("$ROOT/bin/ap-busy-event.sh" arm "$state" stale-w2)
  "$ROOT/bin/ap-busy-event.sh" apply "$state" stale-w2 busy --gen "$resumed_gen" \
