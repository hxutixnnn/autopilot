...
# just "the app", the SAME real running instance the pilot uses day to day.
...
#
# Fails CLOSED: any ambiguity (an empty id, a plain label without the ap-test-
...
# the pilot's own live work.
set -u

# cmux_refuse_if_unsafe: 0 (SAFE to proceed) only if <workspace_id> is
# non-empty, <want_label> carries the ap-test- prefix, and the workspace is
...
cmux_refuse_if_unsafe() {  # <workspace_id> <want_label>
  ...
  case "$want_label" in
    ap-test-*) : ;;
    *) echo "cmux safety guard: refusing - label '$want_label' does not carry the ap-test- prefix" >&2; return 1 ;;
  esac
  want_title=$(ap_backend_cmux_scoped_title "$want_label")
  title=$(ap_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null | jq -r --arg id "$wsid" '.workspaces[]? | select(.id == $id) | .title' 2>/dev/null)
...
# cleanup leaves the isolated, throwaway workspace for that ap-test- label open
# rather than risk the wrong target.
cmux_safe_close_workspace() {  # <workspace_id> <want_label>
  cmux_refuse_if_unsafe "$1" "$2" || return 1
  ap_backend_cmux_cli close-workspace --workspace "$1" >/dev/null 2>&1 || true
}
