...
TMP_ROOT=$(ap_test_tmproot ap-teardown-endpoint-safety)
...
#!/usr/bin/env bash
printf 'tmux' >> "${AP_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${AP_RUNTIME_LOG:?}"
printf '\n' >> "${AP_RUNTIME_LOG:?}"
exit 0
SH
  cat > "$TMP_ROOT/$dir/fakebin/treehouse" <<'SH'
#!/usr/bin/env bash
printf 'treehouse' >> "${AP_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${AP_RUNTIME_LOG:?}"
printf '\n' >> "${AP_RUNTIME_LOG:?}"
exit 0
SH
...
run_case() {  # <case> <id>
  local dir=$1 id=$2
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" \
  AP_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    "$TEARDOWN" "$id" --force
}
...
test_invalid_endpoint_records_refuse_before_mutation() {
  ...
  ap_write_meta "$dir/home/state/$id.meta" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=recon"
...
test_supported_backend_endpoint_records_validate() {
  ...
  id=tmux-task
  ap_write_meta "$dir/home/state/$id.meta" \
    "window=autopilot:ap-$id" "worktree=$dir/worktree" "project=$dir/project"
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint refused"
  [ "$AP_BACKEND_VALIDATED_BACKEND:$AP_BACKEND_VALIDATED_TARGET" = "tmux:autopilot:ap-$id" ] || fail "tmux endpoint validation returned wrong identity"

  id=tmux-spaced-session
  ap_write_meta "$dir/home/state/$id.meta" \
    "window=team work:ap-$id" "worktree=$dir/worktree" "project=$dir/project"
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint with a spaced session name refused"
  [ "$AP_BACKEND_VALIDATED_TARGET" = "team work:ap-$id" ] || fail "tmux validation changed the spaced session identity"
  ...
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Herdr endpoint refused"
  ...
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Zellij endpoint refused"

  id=orca-task
  ap_write_meta "$dir/home/state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-7" \
    "worktree=$dir/worktree" "project=$dir/project" "backend=orca" "orca_worktree_id=worktree-9"
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Orca endpoint refused"
  [ "$AP_BACKEND_VALIDATED_TARGET" = term-7 ] || fail "Orca validation did not select its terminal"
  ...
  ap_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid cmux endpoint refused"

  for backend in tmux herdr zellij orca cmux; do
    set +e
    ap_backend_kill "$backend" "" >/dev/null 2>&1
    target=$?
    set -e
...
test_tmux_empty_target_refuses_without_invocation() {
  ...
  set +e
  AP_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    bash -c '. "$1/bin/ap-backend.sh"; ap_backend_source tmux; ap_backend_tmux_kill ""' _ "$ROOT" \
...
test_isolated_tmux_invalid_and_valid_cleanup() {
  local dir socket socket_id session='endpoint safety' target_id=target control=control target=ap-target
  local prefix_target=ap-prefix prefix_survivor=ap-prefix2 rc
...
[ "\${AP_TEST_TMUX_SOCKET:-}" = '$socket_id' ] || exit 92
...
SH
  ...
  ap_write_meta "$dir/home/state/invalid.meta" \
    "window=" "worktree=$dir/worktree" "project=$dir/project" "kind=recon"
  set +e
  env -u TMUX -u TMUX_PANE AP_TEST_TMUX_SOCKET="$socket_id" \
    AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" AP_RUNTIME_LOG="$dir/runtime.log" \
  ...
  env -u TMUX -u TMUX_PANE AP_TEST_TMUX_SOCKET="$socket_id" AP_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/ap-backend.sh"; ap_backend_source tmux; ap_backend_tmux_kill ""' _ "$ROOT" \
  ...
  env -u TMUX -u TMUX_PANE AP_TEST_TMUX_SOCKET="$socket_id" AP_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/ap-backend.sh"; ap_backend_source tmux; ap_backend_tmux_kill "$2"' _ "$ROOT" "$session:$prefix_target"
  ...
  ap_write_meta "$dir/home/state/$target_id.meta" \
  ...
  pass "ap-teardown: exact tmux cleanup preserves invalid and prefix-matched neighbors while removing only the recorded target"
}
