...
. "$ROOT/bin/ap-pr-lib.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/ap-check-lib.sh"

PR_CHECK="$ROOT/bin/ap-pr-check.sh"
PR_MERGE="$ROOT/bin/ap-pr-merge.sh"
MIGRATE="$ROOT/bin/ap-pr-check-migrate.sh"
POLL="$ROOT/bin/ap-pr-poll.sh"
WATCH="$ROOT/bin/ap-watch.sh"
TEARDOWN="$ROOT/bin/ap-teardown.sh"
REGISTER="$ROOT/bin/ap-check-register.sh"
TMP_ROOT=$(ap_test_tmproot ap-pr-check-security)
...
make_case() {
  ...
  cat > "$fake_root/bin/ap-guard.sh" <<'SH'
#!/usr/bin/env bash
printf 'guard\n' >> "$AP_TEST_GUARD_LOG"
SH
  chmod +x "$fake_root/bin/ap-guard.sh"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$AP_TEST_GH_LOG"
case " $* " in
  *" headRefOid "*) printf '%s\n' "${AP_TEST_GH_HEAD:-0123456789abcdef0123456789abcdef01234567}" ;;
  *" state "*)
    [ "${AP_TEST_GH_FAIL:-0}" = 0 ] || exit 1
    [ "${AP_TEST_GH_SLEEP:-0}" = 0 ] || sleep "$AP_TEST_GH_SLEEP"
    printf '%s\n' "${AP_TEST_GH_STATE:-OPEN}"
    ;;
esac
SH
...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$AP_TEST_GLAB_LOG"
[ "${AP_TEST_GLAB_FAIL:-0}" = 0 ] || exit 1
[ "${AP_TEST_GLAB_SLEEP:-0}" = 0 ] || sleep "$AP_TEST_GLAB_SLEEP"
printf 'title:\tfixture merge request\nstate:\t%s\nauthor:\tsomeone\n' "${AP_TEST_GLAB_STATE:-opened}"
SH
...
write_task_meta() {
  ...
  ap_write_meta "$dir/home/state/$id.meta" \
...
write_manual_poll_pair() {
  ...
  ap_pr_url_parse "$url" || fail "manual poll fixture URL was invalid"
  provider=$AP_PR_PROVIDER
  host=$AP_PR_HOST
  path=$AP_PR_PATH
  number=$AP_PR_NUMBER
...
start_ambiguous_pending_repair() {
  ...
  set +e
  AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
write_watcher_lock() {
  ...
  identity=$(AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_pid_identity "$2"' _ "$ROOT/bin/ap-wake-lib.sh" "$pid")
  [ -n "$identity" ] || fail "could not capture fake older-watcher identity"
  printf '%s\n' "$pid" > "$state/.watch.lock/pid"
  printf '%s\n' "$home" > "$state/.watch.lock/ap-home"
...
assert_valid_migration_marker() {
  ...
  grep -qxF ap-pr-check-migration-v1 "$marker" || fail "migration marker bytes were not exact"
...
assert_valid_scan_marker() {
  ...
  grep -qxF ap-pr-check-migration-scan-v1 "$marker" || fail "migration scan marker bytes were not exact"
...
run_check_entry() {
  local dir=$1
  shift
  AP_ROOT_OVERRIDE="$dir/root" AP_HOME="$dir/home" \
    AP_TEST_GUARD_LOG="$dir/guard.log" AP_TEST_GH_LOG="$dir/gh.log" \
    AP_TEST_GH_AXI_LOG="$dir/gh-axi.log" AP_TEST_GLAB_LOG="$dir/glab.log" \
...
test_parser_matrix() {
  ...
  while IFS='|' read -r url owner repo number; do
    [ -n "$url" ] || continue
    ap_pr_url_parse "$url" || fail "parser rejected canonical URL"
    [ "$AP_PR_URL" = "$url" ] || fail "parser changed canonical URL"
    [ "$AP_PR_OWNER" = "$owner" ] || fail "parser returned wrong owner"
    [ "$AP_PR_REPO" = "$repo" ] || fail "parser returned wrong repository"
    [ "$AP_PR_NUMBER" = "$number" ] || fail "parser returned wrong PR number"
  done <<'EOF'
...
EOF
  while IFS='|' read -r url host path number; do
    [ -n "$url" ] || continue
    ap_pr_url_parse "$url" || fail "parser rejected a canonical merge request URL"
    [ "$AP_PR_PROVIDER" = gitlab ] || fail "parser did not tag a merge request URL as gitlab"
    [ "$AP_PR_URL" = "$url" ] || fail "parser changed a canonical merge request URL"
    [ "$AP_PR_HOST" = "$host" ] || fail "parser returned wrong GitLab host"
    [ "$AP_PR_PATH" = "$path" ] || fail "parser returned wrong GitLab project path"
    [ "$AP_PR_NUMBER" = "$number" ] || fail "parser returned wrong merge request number"
    [ -z "$AP_PR_OWNER" ] && [ -z "$AP_PR_REPO" ] \
...
EOF
  ap_pr_url_parse https://github.com/a/b/pull/1 || fail "parser rejected canonical URL"
  [ "$AP_PR_PROVIDER" = github ] || fail "parser did not tag a pull request URL as github"
  [ "$AP_PR_HOST" = github.com ] || fail "parser returned wrong GitHub host"
  [ "$AP_PR_PATH" = a/b ] || fail "parser returned wrong GitHub project path"
  for row in "${INVALID_URLS[@]}"; do
    ! ap_pr_url_parse "$row" || fail "parser accepted a rejected raw-byte URL class"
  done
  for id in -task task- task--a Task-a task_a task.a; do
    ap_pr_task_id_valid "$id" || fail "task ID validator rejected a safe lifecycle-compatible slug"
  done
  ap_task_id_creation_valid _noncanonical \
  ...
  ap_pr_task_id_valid "$id" || fail "operational validator rejected a path-safe legacy task ID"
  ! ap_task_id_creation_valid "$id" || fail "creation validator accepted an overlong task ID"
...
test_invalid_entrypoints_have_zero_side_effects() {
  ...
  for value in "${UNSAFE_LIFECYCLE_IDS[@]}"; do
    before=$(state_snapshot "$dir/home/state")
    set +e
    AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$dir/root" AP_TEST_GUARD_LOG="$dir/guard.log" \
...
test_valid_recording_and_merge_derivation() {
  ...
  AP_TEST_GH_HEAD=$expected run_check_entry "$dir" task-a https://github.com/my-org/repo_name.with-dots/pull/37 \
  ...
  [ "$(ap_pr_file_link_count "$dir/home/state/task-a.check.sh")" = 1 ] \
    && [ "$(ap_pr_file_link_count "$dir/home/state/task-a.pr-poll")" = 1 ] \
    && [ "$(ap_pr_file_link_count "$dir/home/state/task-a.pr-poll-registration")" = 1 ] \
    || fail "published poll artifacts were not single-link files"
  ap_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
  ...
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged-watch.out" 2> "$dir/merged-watch.err"
  rc=$?
  set -e
  ...
  ap_pr_poll_artifacts_valid "$dir/home/state" Task_A.1 "$POLL" \
...
SH
  ...
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
...
SH
    ...
    ap_pr_poll_artifacts_valid "$dir/home/state" "$id" "$POLL" \
...
run_watcher_bounded() {
  local home=$1 fakebin=$2 check_interval=${AP_TEST_CHECK_INTERVAL:-0} watch_root=${AP_TEST_WATCH_ROOT:-$ROOT}
  shift 2
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 10; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env AP_HOME="$home" AP_ROOT_OVERRIDE="$watch_root" AP_CHECK_INTERVAL="$check_interval" AP_CHECK_TIMEOUT=1 \
      AP_POLL=0.02 AP_HEARTBEAT=999999 AP_SIGNAL_GRACE=0 PATH="$fakebin:$BASE_PATH" "$WATCH" "$@"
}
...
test_rejected_metacharacter_bytes_are_inert() {
  ...
  for family in "${families[@]}"; do
    ...
    ap_pr_poll_prepare "$dir/home/state" safe-check github https://github.com/o/r/pull/99 github.com o/r 99 "$POLL" \
      || fail "could not prepare bounded watcher poll"
    ap_pr_poll_publish_prepared || fail "could not publish bounded watcher poll"

    set +e
    AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
    rc=$?
    set -e
  ...
  AP_TEST_GH_STATE=OPEN run_check_entry "$dir" task-a https://github.com/o/r/pull/1 >/dev/null 2>/dev/null \
...
run_poll() {
  local dir=$1
  AP_TEST_GH_LOG="$dir/gh.log" AP_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$dir/fakebin:$BASE_PATH" \
...
test_static_poll_contract() {
  ...
  make_poll_fixture "$dir"
  set +e
  out=$(AP_STATE_OVERRIDE="$dir/home/state" AP_CHECK_TIMEOUT=1 AP_TEST_GH_LOG="$dir/gh.log" \
    AP_TEST_GH_SLEEP=3 PATH="$dir/fakebin:$BASE_PATH" \
  ...
  ap_pr_poll_prepare "$dir/home/state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
    || fail "could not prepare authenticated watcher poll"
  ap_pr_poll_publish_prepared || fail "could not publish authenticated watcher poll"
  rm -f "$dir/home/state/.last-check"
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
...
SH
  ...
  ! find "$dir/home/state" -name '.ap-pr-poll-*' -print | grep . >/dev/null \
...
SH
    ...
    direct_pid=$!
    i=0
    while [ "$i" -lt 100 ] && ! find "$dir/home/state" -name '.ap-pr-poll-check.*' -print | grep . >/dev/null; do
      sleep 0.01
      i=$((i + 1))
    done
...
test_migration_excludes_older_watcher_before_scan() {
  ...
  ap_write_meta "$state/task-a.meta" \
    'window=ap-task-a' \
...
SH
  ...
  set +e
  AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_migration_initializes_fresh_state() {
  ...
  set +e
  AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_private_artifact_paths_refuse_symlinks_and_directories() {
  ...
  for artifact in task-a.pr-poll task-a.pr-poll-registration task-a.check.sh; do
    for kind in regular dangling directory; do
      ...
      state="$dir/home/state"
      ap_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
        || fail "could not stage poll symlink refusal fixture"
      destination="$state/$artifact"
      make_private_symlink "$dir" "$destination" "$kind"
      if ap_pr_poll_publish_prepared; then
...
#!/usr/bin/env bash
last=${!#}
"${AP_TEST_REAL_MV:?}" "$@" || exit $?
[ "$last" = "${AP_TEST_FINAL_PATH:?}" ] || exit 0
case "${AP_TEST_FINAL_ACTION:?}" in
  type)
    rm -f -- "$last"
    ln -s "${AP_TEST_FAULT_LINK_TARGET:?}" "$last"
    ;;
  mode) "${AP_TEST_REAL_CHMOD:?}" 0644 "$last" ;;
  content) printf 'faulted final bytes\n' > "$last" ;;
  device) : > "${AP_TEST_FAULT_GATE:?}" ;;
  *) exit 2 ;;
esac
SH
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${AP_TEST_FINAL_PATH:-}" ] && [ -e "${AP_TEST_FAULT_GATE:-/nonexistent}" ]; then
  case " $* " in
    *" %d "*) printf '%s\n' 999999; exit 0 ;;
  esac
fi
exec "${AP_TEST_REAL_STAT:?}" "$@"
SH
...
test_postrename_poll_validation_revokes_and_retries() {
  ...
  for artifact in data registration check; do
    for action in type mode device content; do
      ...
      ap_pr_poll_publish_prepared || fail "could not publish prior poll"
      write_poll_meta "$state" task-a https://github.com/o/r/pull/2
      ap_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/2 github.com o/r 2 "$POLL" \
      ...
      if AP_TEST_FINAL_PATH="$destination" AP_TEST_FINAL_ACTION="$action" \
        AP_TEST_FAULT_LINK_TARGET="$link_target" AP_TEST_FAULT_GATE="$gate" \
        AP_TEST_REAL_MV="$REAL_MV" AP_TEST_REAL_STAT="$REAL_STAT" AP_TEST_REAL_CHMOD="$REAL_CHMOD" \
        PATH="$dir/fakebin:$BASE_PATH" ap_pr_poll_publish_prepared; then
      ...
      PATH="$BASE_PATH" ap_pr_poll_publish_prepared || fail "poll retry did not recover after final validation fault"
      ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "poll retry did not publish a valid pair"
    done
  done
...
for arg in "$@"; do
  case "$arg" in
    *"${AP_TEST_MV_MATCH:?}"*) matched=1 ;;
  esac
done
if [ "$matched" -eq 1 ]; then
  case "${AP_TEST_MV_ACTION:?}" in
    fail) exit 1 ;;
    signal)
      kill -TERM "$PPID"
      sleep 0.1
...
test_marker_and_diagnostic_rename_fail_closed() {
  ...
  for action in fail signal; do
    ...
    set +e
    AP_TEST_MV_MATCH=.ap-pr-check-migration. AP_TEST_MV_ACTION="$action" AP_TEST_REAL_MV="$REAL_MV" \
      AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.ap-pr-check-migration.*' -print | grep . >/dev/null \
    ...
    install_mv_fault "$dir"
    set +e
    AP_TEST_MV_MATCH=.ap-pr-check-log. AP_TEST_MV_ACTION="$action" AP_TEST_REAL_MV="$REAL_MV" \
      AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.ap-pr-check-log.*' -print | grep . >/dev/null \
...
test_postrename_marker_and_diagnostic_validation_retries() {
  ...
  for artifact in marker diagnostic obligation; do
    for action in type mode device content; do
      ...
      install_final_publication_fault "$dir"
      set +e
      AP_TEST_FINAL_PATH="$destination" AP_TEST_FINAL_ACTION="$action" \
        AP_TEST_FAULT_LINK_TARGET="$link_target" AP_TEST_FAULT_GATE="$gate" \
        AP_TEST_REAL_MV="$REAL_MV" AP_TEST_REAL_STAT="$REAL_STAT" AP_TEST_REAL_CHMOD="$REAL_CHMOD" \
        AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
      rc=$?
...
last=${!#}
case "$last" in
  ${AP_TEST_CHMOD_MATCH:?}) exit 0 ;;
esac
exec "${AP_TEST_REAL_CHMOD:?}" "$@"
SH
...
test_quarantine_validation_and_retry_contract() {
  ...
  set +e
  AP_TEST_CHMOD_MATCH="$state/.pr-check-quarantine" AP_TEST_REAL_CHMOD="$REAL_CHMOD" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
  ...
  install_chmod_noop_fault "$dir"
  set +e
  AP_TEST_CHMOD_MATCH="$state/.pr-check-quarantine/task-a.check.*" AP_TEST_REAL_CHMOD="$REAL_CHMOD" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
"${AP_TEST_REAL_MV:?}" "$@" || exit $?
case "$last" in
  */.pr-check-quarantine/task-a.check.*) : > "${AP_TEST_FAULT_GATE:?}" ;;
esac
SH
  cat > "$dir/fakebin/stat" <<'SH'
#!/usr/bin/env bash
...
SH
  chmod +x "$dir/fakebin/mv" "$dir/fakebin/stat"
  set +e
  AP_TEST_REAL_MV="$REAL_MV" AP_TEST_REAL_STAT="$REAL_STAT" AP_TEST_FAULT_GATE="$dir/device-fault" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  AP_TEST_REAL_MV="$REAL_MV" AP_TEST_REAL_CP="$REAL_CP" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  AP_TEST_REAL_MV="$REAL_MV" AP_TEST_FAULT_LINK_TARGET="$external" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
test_ambiguous_failure_accepts_validated_replacement() {
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" \
    || fail "replacement registration did not publish a valid poll pair"

  AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-retry.out" 2> "$dir/migrate-retry.err" \
  ...
  for case_name in copied-pair copied-registration metadata-mismatch task-mismatch forged-registration partial-publication; do
    ...
    case "$case_name" in
      ...
      copied-registration)
        ...
        ap_pr_poll_prepare "$donor" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare donor registration fixture"
        ap_pr_poll_publish_prepared || fail "could not publish donor registration fixture"
        ...
        ;;
      metadata-mismatch)
        ap_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare metadata-mismatch fixture"
        ap_pr_poll_publish_prepared || fail "could not publish metadata-mismatch fixture"
        write_poll_meta "$state" task-a https://github.com/o/r/pull/11
        ;;
      task-mismatch)
        ap_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare task-mismatch fixture"
        ap_pr_poll_publish_prepared || fail "could not publish task-mismatch fixture"
      ...
      forged-registration)
        write_manual_poll_pair "$state"
        printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
          ap-pr-poll-registration-v2 task-a github https://github.com/o/r/pull/10 github.com o/r 10 \
    ...
    set +e
    AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/retry.out" 2> "$dir/retry.err"
    rc=$?
    set -e
...
test_complete_single_link_validation() {
  ...
  set +e
  AP_HOME="$dir/home" "$REGISTER" custom > "$dir/register.out" 2> "$dir/register.err"
  rc=$?
  set -e
...
SH
  ...
  set +e
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$BASE_PATH" \
...
test_failed_outcomes_block_every_retry_until_repaired() {
  ...
  for classification in canonical ambiguous; do
    ...
    set +e
    AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
    rc=$?
    set -e
    ...
    printf '%s\n' ap-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
    chmod 0600 "$state/.pr-check-migration-v1"

    set +e
    AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err"
    rc=$?
    set -e
    ...
    rmdir "$state/task-a.pr-poll"
    AP_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-3.out" 2> "$dir/migrate-3.err" \
    ...
    if [ "$classification" = canonical ]; then
      ...
      ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical repaired retry did not arm a valid poll pair"
    else
...
test_canonical_publication_failure_recovers_only_on_retry() {
  ...
  set +e
  AP_TEST_FINAL_PATH="$destination" AP_TEST_FINAL_ACTION=mode \
    AP_TEST_FAULT_LINK_TARGET="$link_target" AP_TEST_FAULT_GATE="$gate" \
    AP_TEST_REAL_MV="$REAL_MV" AP_TEST_REAL_STAT="$REAL_STAT" AP_TEST_REAL_CHMOD="$REAL_CHMOD" \
    AP_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
  rc=$?
  set -e
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical publication retry did not arm a valid pair"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  [ ! -e "$pending" ] && [ ! -L "$pending" ] \
...
test_obligation_namespace_compatibility() {
  ...
  ap_write_meta "$state/_noncanonical.meta" \
    'window=autopilot:ap-_noncanonical' \
...
SH
  ...
  AP_HOME="$dir/home" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  printf 'legacy quarantined bytes\n' \
    > "$state/.pr-check-quarantine/_noncanonical.check.abc123"
  printf 'ap-pr-check-migration-scan-v1\n' > "$state/.pr-check-migration-scan-v1"
  printf 'ap-pr-check-migration-v1\n' > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-quarantine/"* \
    "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
  AP_HOME="$dir/home" "$MIGRATE" --checks-safe > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  state="$dir/home/state"
  ap_write_meta "$state/foo.diagnostic.bar.meta" \
    'window=ap-foo.diagnostic.bar' \
  ...
  ap_pr_poll_artifacts_valid "$state" foo.diagnostic.bar "$POLL" \
...
test_nonexecuting_migration() {
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical migration did not leave a validated armed poll"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  ...
  AP_HOME="$dir/home" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err" \
  ...
  snap_before=$(state_snapshot "$state")
  AP_HOME="$dir/home" "$MIGRATE" >/dev/null 2>/dev/null || fail "completed migration rerun failed"
  snap_after=$(state_snapshot "$state")
  [ "$snap_after" = "$snap_before" ] || fail "completed migration changed a later custom check"

  dir=$(make_case migration-ambiguous)
  state="$dir/home/state"
  ap_write_meta "$state/task-b.meta" \
    'window=ap-task-b' \
    'pr=https://github.com/o/r/pull/10' \
...
test_bootstrap_migrates_before_other_mutations() {
  ...
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
    "$ROOT/bin/ap-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err" \
...
test_bootstrap_isolates_incomplete_poll_migration() {
  ...
  ap_pr_poll_prepare "$state" z-healthy github https://github.com/o/r/pull/13 github.com o/r 13 "$POLL" \
    || fail "could not prepare healthy poll for migration isolation"
  ap_pr_poll_publish_prepared || fail "could not publish healthy poll for migration isolation"
  ap_write_meta "$state/copilot-a.meta" \
    'window=autopilot:ap-copilot-a' \
    'kind=copilot' \
    'harness=codex' \
    'backend=tmux'
  mkdir -p "$dir/home/projects"
  ap_fake_exit0 "$fakebin" jq
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
case " $* " in
  *' list-windows '*) printf 'ap-copilot-a\n' ;;
  *' display-message '*) printf 'node\n' ;;
esac
SH
  cat > "$dir/root/bin/ap-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${AP_TEST_FLEET_MARKER:?}"
printf 'alpha: recovered: continued after isolated migration failure\n'
SH
  chmod +x "$fakebin/tmux" "$dir/root/bin/ap-fleet-sync.sh"

  set +e
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$dir/root" AP_TEST_FLEET_MARKER="$fleet_marker" \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/ap-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err"
  rc=$?
  set -e
  ...
  assert_grep 'COPILOT_SYNC: copilot copilot-a: skipped:' "$dir/bootstrap.out" \
    "incomplete poll migration suppressed copilot sync"
  assert_grep 'COPILOT_LIVENESS: copilot copilot-a: skipped: existing endpoint has ambiguous agent process' "$dir/bootstrap.out" \
  ...
  set +e
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$dir/root" \
    AP_TEST_GH_STATE=MERGED AP_POLL=0 AP_CHECK_INTERVAL=0 AP_SIGNAL_GRACE=0 \
  ...
  AP_HOME="$dir/home" "$REGISTER" b-custom > "$dir/register.out" \
  ...
  set +e
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$dir/root" \
    AP_TEST_GH_STATE=OPEN AP_POLL=0 AP_CHECK_INTERVAL=0 AP_SIGNAL_GRACE=0 \
...
test_custom_snapshot_cleanup_on_signal() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'trap "" TERM' \
    'printf "%s\n" "$$" > "$AP_TEST_CUSTOM_CHILD_PID"' 'while :; do sleep 1; done' \
    > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
...
SH
  ...
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" AP_POLL=0 AP_CHECK_INTERVAL=0 \
    AP_SIGNAL_GRACE=0 AP_TEST_CUSTOM_CHILD_PID="$child_pid_file" \
  ...
  find "$state" -maxdepth 1 -name '.ap-custom-check.*' -print | grep . >/dev/null \
  ...
  ! find "$state" -maxdepth 1 -name '.ap-check-output.*' -print | grep . >/dev/null \
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; open my $ready, ">", $ENV{AP_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; select undef, undef, undef, 4; open my $sentinel, ">", $ENV{AP_TEST_DESCENDANT_SENTINEL} or die $!; print {$sentinel} "late\n"; close $sentinel; select undef, undef, undef, 1' &
printf '%s\n' "$!" > "$AP_TEST_DESCENDANT_PID"
while [ ! -s "$AP_TEST_DESCENDANT_READY" ]; do sleep 0.01; done
: > "$AP_TEST_DIRECT_DONE"
SH
    chmod 0700 "$state/custom.check.sh"
    AP_HOME="$dir/home" "$REGISTER" custom >/dev/null \
...
SH
    ...
    AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" AP_POLL=0.1 AP_CHECK_INTERVAL=999999 \
      AP_CHECK_TIMEOUT=10 AP_HEARTBEAT=999999 AP_SIGNAL_GRACE=0 \
      AP_CHECK_FORCE_FALLBACK="$force_fallback" AP_TEST_DESCENDANT_READY="$ready" \
      AP_TEST_DESCENDANT_SENTINEL="$sentinel" AP_TEST_DESCENDANT_PID="$child_pid_file" \
      AP_TEST_DIRECT_DONE="$direct_done" PATH="$fakebin:$BASE_PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    watcher_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
...
test_teardown_removes_poll_artifacts() {
  ...
  fakebin="$dir/fakebin"
  ap_write_meta "$dir/home/state/task-a.meta" \
    'window=autopilot:ap-task-a' \
    'endpoint_task_id=task-a' \
    "worktree=$dir/missing-worktree" \
    "project=$dir/project" \
...
SH
  ...
  ap_pr_poll_snapshot_capture "$dir/home/state" task-a "$POLL" \
    || fail "could not snapshot teardown receipt fixture"
  ap_pr_poll_retirement_publish "$dir/home/state" task-a "$POLL" merged \
...
SH
  ...
  fakebin="$dir/fakebin"
  ap_write_meta "$dir/home/state/invalid.meta" \
    'window=autopilot:ap-invalid' \
...
SH
    ...
    touch "$dir/home/state/.last-watcher-beat"
    set +e
    AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" AP_FAKE_TMUX_LOG="$dir/tmux.log" \
...
test_gitlab_merge_watch() {
  ...
  ap_pr_poll_prepare "$state" task-a gitlab "$url" gitlab.example group/subgroup/project 7 "$POLL" \
    || fail "could not prepare a GitLab poll"
  ap_pr_poll_publish_prepared || fail "could not publish a GitLab poll"
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" \
...
7" ] || fail "published GitLab sidecar bytes were not exact"
  ...
  # Only an exact merged state wakes autopilot. Every other reading, including
  ...
  for value in opened closed locked '' not-a-state MERGED merged-but-not; do
    out=$(AP_TEST_GLAB_STATE="$value" run_poll "$dir")
    [ -z "$out" ] || fail "GitLab poll emitted for a non-merged state"
  done
  out=$(AP_TEST_GLAB_STATE=merged run_poll "$dir")
  [ "$out" = merged ] || fail "GitLab poll did not emit exactly one merged line"
  out=$(AP_TEST_GLAB_FAIL=1 run_poll "$dir")
...
EOF
  ...
  out=$(AP_TEST_GLAB_STATE=merged AP_TEST_GH_LOG="$dir/gh.log" AP_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$noglab" \
    bash "$state/task-a.check.sh")
  ...
  set +e
  out=$(AP_ROOT_OVERRIDE="$dir/root" AP_HOME="$dir/home" \
    AP_TEST_GUARD_LOG="$dir/guard.log" PATH="$noglab" \
...
seed_canonical_poll() {
  ...
  state="$dir/home/state"
  ap_pr_url_parse "$url" || fail "retirement fixture URL was invalid"
  provider=$AP_PR_PROVIDER
  host=$AP_PR_HOST
  path=$AP_PR_PATH
  number=$AP_PR_NUMBER
  ...
  ap_pr_poll_publish_prepared || fail "could not publish retirement fixture"
  printf '%s\n' ap-pr-check-migration-scan-v1 > "$state/.pr-check-migration-scan-v1"
  printf '%s\n' ap-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
}
...
add_stop_custom_check() {
  ...
  AP_HOME="$dir/home" "$REGISTER" z-stop >/dev/null \
...
test_merged_poll_retires_once() {
  ...
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-1.out" 2> "$dir/watch-1.err"
  rc=$?
  set -e
  ...
  rm -f "$state/.last-check"
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-2.out" 2> "$dir/watch-2.err"
  rc=$?
  set -e
...
test_persistent_copilot_retirement_is_poll_only() {
  local dir state meta_before status_before registry_before endpoint_before rc
  dir=$(make_case merged-retirement-copilot)
  state="$dir/home/state"
  ap_write_meta "$state/domain.meta" \
    'window=session:ap-domain' \
    "worktree=$dir/copilot-home" \
    "project=$dir/project" \
    'kind=copilot' \
    'mode=copilot' \
    'backend=tmux' \
    "home=$dir/copilot-home" \
    'pr=https://github.com/o/r/pull/2' \
    'pr_head=0123456789abcdef0123456789abcdef01234567'
  mkdir -p "$dir/copilot-home"
  printf 'working: persistent endpoint remains healthy\n' > "$state/domain.status"
  printf -- '- domain | scope: test | home: %s\n' "$dir/copilot-home" > "$dir/home/data/copilots.md"
  ...
  registry_before=$(shasum -a 256 "$dir/home/data/copilots.md")
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "persistent copilot merged watcher failed: $(cat "$dir/watch.err")"
  assert_poll_absent "$state" domain
  [ "$(shasum -a 256 "$state/domain.meta")" = "$meta_before" ] || fail "retirement changed copilot metadata"
  [ "$(shasum -a 256 "$state/domain.status")" = "$status_before" ] || fail "retirement changed copilot status"
  [ "$(shasum -a 256 "$dir/home/data/copilots.md")" = "$registry_before" ] || fail "retirement changed copilot registry"
  [ "$(shasum -a 256 "$dir/endpoint-sentinel")" = "$endpoint_before" ] || fail "retirement changed copilot endpoint evidence"
  [ -d "$dir/copilot-home" ] || fail "retirement removed the persistent copilot home"
  pass "merged poll retirement preserves every persistent copilot lifecycle artifact"
}
...
test_retirement_crash_recovery() {
  ...
  AP_STATE_OVERRIDE="$state" bash -c '. "$1"; ap_wake_append check "$2" "$3"' _ \
    "$ROOT/bin/ap-wake-lib.sh" "$state/task-a.check.sh" "check: $state/task-a.check.sh: merged" \
  ...
  AP_HOME="$dir/home" AP_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/ap-wake-drain.sh" > "$dir/drain.out" 2>/dev/null
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt crash fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish crash receipt"
  add_stop_custom_check "$dir"
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  rc=$?
  set -e
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot partial removal fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish partial removal receipt"
  rm -f "$state/task-a.check.sh"
  add_stop_custom_check "$dir"
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  ...
  assert_poll_absent "$state" task-a
  ap_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "completed retirement was not idempotent"

  dir=$(make_case retirement-after-registration-removal)
  state="$dir/home/state"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot sidecar-removal fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish sidecar-removal receipt"
  ...
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt-only fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt-only fixture"
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-ap-pr-poll.sh"
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$historical_poll" \
    || fail "could not snapshot pre-update retirement fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$historical_poll" merged \
...
test_external_merge_transition_retires_only_terminal_poll() {
  ...
  for label in open-green open-red closed-unmerged forge-error malformed; do
    ...
    case "$label" in
      open-green|open-red)
        AP_TEST_GH_STATE=OPEN run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      closed-unmerged)
        AP_TEST_GH_STATE=CLOSED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      forge-error)
        AP_TEST_GH_FAIL=1 run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      malformed)
        AP_TEST_GH_STATE=NOT_A_FORGE_STATE run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
    esac
    rc=$?
    set -e
  ...
  set +e
  AP_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged.out" 2> "$dir/merged.err"
  rc=$?
  set -e
...
test_retirement_refuses_replacement_and_nonterminal_results() {
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot replacement fixture"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/7
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/7
  before=$(state_snapshot "$state")
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged \
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll was not left canonical"

  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot nonterminal fixture"
  for result in OPEN CLOSED ERROR 'merged extra'; do
    ap_pr_poll_retirement_publish "$state" task-a "$POLL" "$result" \
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot rearm-race fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish rearm-race receipt"
  ...
  ap_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "stale receipt did not yield to a canonical replacement"
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll lost canonical provenance"

  dir=$(make_case retirement-template-update-rearm-race)
  state="$dir/home/state"
  historical_poll="$dir/historical-ap-pr-poll.sh"
  current_poll="$dir/current-ap-pr-poll.sh"
  cp "$POLL" "$historical_poll"
  ...
  replacement_registration=$(shasum -a 256 "$state/task-a.pr-poll-registration")
  replacement_meta=$(shasum -a 256 "$state/task-a.meta")
  ap_pr_poll_retirement_recover_one "$state" task-a "$current_poll" \
  ...
  ap_pr_poll_artifacts_valid "$state" task-a "$current_poll" || fail "updated replacement poll lost canonical provenance"

  dir=$(make_case custom-merged-not-retired)
  state="$dir/home/state"
  printf '#!/usr/bin/env bash\nprintf "merged\\n"\n' > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
  AP_HOME="$dir/home" "$REGISTER" custom >/dev/null || fail "could not register merged custom check"
  set +e
...
test_retirement_queue_failure_and_receipt_tampering() {
  ...
  ap_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt tamper fixture"
  ap_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt tamper fixture"
  printf 'extra\n' >> "$state/task-a.pr-poll-retirement"
  before=$(state_snapshot "$state")
  ap_pr_poll_retirement_recover_one "$state" task-a "$POLL" \
...
test_gitlab_merged_poll_retires() {
  ...
  set +e
  AP_TEST_GLAB_STATE=merged run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
