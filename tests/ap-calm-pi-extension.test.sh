...
TMP_ROOT=$(ap_test_tmproot ap-calm-pi-extension)
EXT="$ROOT/.pi/extensions/ap-calm.ts"
ASSISTANT_LAYOUT="$ROOT/.pi/extensions/lib/ap-calm-assistant-layout.ts"
OPERATIONAL_USER_LAYOUT="$ROOT/.pi/extensions/lib/ap-calm-operational-user-layout.ts"
VISIBILITY="$ROOT/.pi/extensions/lib/ap-calm-visibility.ts"
WORKING_FLIGHT="$ROOT/.pi/extensions/lib/ap-calm-working-flight.ts"
WATCH_EXT="$ROOT/.pi/extensions/ap-primary-pi-watch.ts"
OPERATIONAL_INPUT="$ROOT/bin/ap-operational-input.sh"
PI_OPERATIONAL_INPUT="$ROOT/.pi/extensions/lib/ap-operational-input.ts"
PI_PACKAGE_DIR=${AP_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
TMUX_SOCKET="ap-calm-$$"
TMUX_SESSION="ap-calm-e2e"
...
# .pi/extensions/ap-calm.ts) instead of relying on version inference, so a version
...
find_chrome() {
  local candidate
  if [ -n "${AP_CHROME_BIN:-}" ] && [ -x "$AP_CHROME_BIN" ]; then
    printf '%s\n' "$AP_CHROME_BIN"
    return 0
  fi
  for candidate in \
...
test_home_resolution() {
  ...
  mkdir -p \
    ...
    "$fixture/override" \
    "$fixture/launch-cwd"
  cp "$EXT" "$fixture/project/.pi/extensions/ap-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/project/.pi/extensions/lib/ap-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/project/.pi/extensions/lib/ap-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/project/.pi/extensions/lib/ap-calm-visibility.ts"
  cp "$WORKING_FLIGHT" "$fixture/project/.pi/extensions/lib/ap-calm-working-flight.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/project/.pi/extensions/lib/ap-operational-input.ts"
  ...
  out=$(cd "$fixture/launch-cwd" && \
    EXT="$fixture/project/.pi/extensions/ap-calm.ts" \
...
delete process.env.AP_CONFIG_OVERRIDE;
process.env.AP_ROOT_OVERRIDE = process.env.OVERRIDE_HOME;
...
if (readFileSync(`${process.env.OVERRIDE_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm ignored AP_ROOT_OVERRIDE when AP_HOME was unset");
}

delete process.env.AP_ROOT_OVERRIDE;
calm = registerCalm();
calm.sessionStart({ reason: "startup" }, context);
await calm.calmCommand.handler("", context);
if (readFileSync(`${process.env.EXTENSION_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not derive the Autopilot home from its extension path");
}
...
const assistant = await import("./.pi/extensions/lib/ap-calm-assistant-layout.ts");
const operational = await import("./.pi/extensions/lib/ap-calm-operational-user-layout.ts");
...
test_rendering_and_session_lifecycle() {
  ...
  cp "$EXT" "$fixture/ap-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/ap-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/ap-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/ap-calm-visibility.ts"
  cp "$WORKING_FLIGHT" "$fixture/lib/ap-calm-working-flight.ts"
  cp "$ROOT/.pi/extensions/lib/ap-operational-input.ts" "$fixture/lib/ap-operational-input.ts"
  cp "$WATCH_EXT" "$fixture/ap-primary-pi-watch.ts"
...
#!/usr/bin/env bash
printf '%s\n' "${1-}" >>"$AP_OPERATIONAL_INPUT_CALLS"
exec "$AP_OPERATIONAL_INPUT_OWNER" "$@"
SH
  chmod +x "$fixture/operational-input-probe.sh"

  out=$(cd "$fixture" && EXT="$fixture/ap-calm.ts" WATCH_EXT="$fixture/ap-primary-pi-watch.ts" AP_HOME="$fixture/home" AP_OPERATIONAL_INPUT_SCRIPT="$fixture/operational-input-probe.sh" AP_OPERATIONAL_INPUT_OWNER="$OPERATIONAL_INPUT" AP_OPERATIONAL_INPUT_CALLS="$fixture/operational-input-calls" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module 2>&1 <<'JS'
...
extension.default(pi);
const visibility = await import(`${pathToFileURL(`${process.cwd()}/lib/ap-calm-visibility.ts`).href}?policy=${Date.now()}`);
const operationalInput = await import(`${pathToFileURL(`${process.cwd()}/lib/ap-operational-input.ts`).href}?input=${Date.now()}`);
...
if (
  ...
  "Toggle Autopilot's supported conversation-only transcript presentation."
) {
...
const watcherBody =
  "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status\n\n" +
  "Run bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const watcherMessage = operationalInput.encodeAutopilotOperationalInput("watcher", watcherBody);
...
const callsBeforePlainReplay = readFileSync(process.env.AP_OPERATIONAL_INPUT_CALLS, "utf8");
...
if (readFileSync(process.env.AP_OPERATIONAL_INPUT_CALLS, "utf8") !== callsBeforePlainReplay) {
...
const watchTool = tools.find((tool) => tool.name === "ap_watch_arm_pi");
if (!watchTool) throw new Error("Autopilot watcher extension did not register ap_watch_arm_pi");
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("Autopilot watcher tool changed stock rendering while Calm was off");
}
...
const presentationRenderer = entryRenderers.get("autopilot-synthetic-input-presentation");
if (!presentationRenderer) throw new Error("legacy synthetic presentation renderer was not registered");
const presentationEntry = {
  customType: "autopilot-synthetic-input-presentation",
...
if (expanded !== true || workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("autopilot-calm") !== undefined) {
  throw new Error("Calm did not preserve working visibility or apply its thinking and footer presentation controls");
}
if (readFileSync(`${process.env.AP_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not persist the active choice in the effective Autopilot home");
}
presentationComponent.setExpanded(!expanded);
if (presentationComponent.hasContent() || presentationComponent.render(100).length !== 0) {
  throw new Error("Calm left a synthetic Autopilot presentation row or spacer visible");
}
...
const operationalNearMisses = [
  {
    content: `Pilot quote: ${watcherMessage}`,
    visible: "Pilot quote:",
  },
  {
    content: "AUTOPILOT_OP: v1 watcher: ASCII_ONLY_PILOT_MESSAGE",
    visible: "ASCII_ONLY_PILOT_MESSAGE",
  },
  {
    content: `Ordinary pilot text before ${watcherMessage}`,
    visible: "Ordinary pilot text before",
  },
  {
    content: "\u2063ordinary pilot text after an unrelated separator",
    visible: "ordinary pilot text after an unrelated separator",
  },
  {
    content: "\u2063AUTOPILOT_OP: legacy untyped pilot message",
    visible: "legacy untyped pilot message",
  },
  {
    content: "Run `bin/ap-session-start.sh` now, exactly once, before executing any other instructions.",
    visible: "before executing any other instructions",
  },
  {
    content:
      "AUTOPILOT WATCHER WAKE: pilot-authored legacy-shaped message\n\n" +
      "Run bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.",
    visible: "pilot-authored legacy-shaped message",
  },
  {
    content:
      ...
      "pilot-authored legacy-shaped message",
    visible: "pilot-authored legacy-shaped message",
  },
  {
    content: [
    ...
    visible: "AUTOPILOT WATCHER WAKE",
  },
];
...
async function assertStockHtmlRendering(command, submitData) {
  ...
  const exportCases = [
    ...
    ["ap_watch_arm_pi", watchArgs, watchResult],
  ];
...
if (watchActual.render(100).length !== 0) {
  throw new Error("Calm left the ap_watch_arm_pi call/result shell visible");
}
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("ap_watch_arm_pi did not restore its stock call/result shell");
}
if (workingVisible !== true || hiddenThinkingLabel !== undefined || statuses.get("autopilot-calm") !== undefined) {
...
if (readFileSync(`${process.env.AP_HOME}/config/calm`, "utf8") !== "off\n") {
  throw new Error("Calm did not persist the inactive choice in the effective Autopilot home");
}
presentationComponent.setExpanded(expanded);
if (
  !presentationComponent.hasContent() ||
...
for (const reason of ["startup", "new", "resume", "fork", "reload"]) {
  ...
  if (workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("autopilot-calm") !== undefined) {
...
test_operational_followup_turn_e2e() {
  local project home config sessions version label case_name calm_state expected_notifications session_file pane i pilot_line handled_line geometry_gap exact_session
  ...
  ap_git_init_commit "$project"
  cp "$EXT" "$project/.pi/extensions/ap-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$project/.pi/extensions/lib/ap-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$project/.pi/extensions/lib/ap-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$project/.pi/extensions/lib/ap-calm-visibility.ts"
  cp "$WORKING_FLIGHT" "$project/.pi/extensions/lib/ap-calm-working-flight.ts"
  cp "$PI_OPERATIONAL_INPUT" "$project/.pi/extensions/lib/ap-operational-input.ts"
...
import { encodeAutopilotOperationalInput } from "./.pi/extensions/lib/ap-operational-input.ts";

let phase: "idle" | "pilot" | "monitor" = "idle";
let label = "";
let adjacent = false;
let latestInputRole: "user" | "custom" | undefined;

const EXACT_WATCHER_INPUT =
  "\u2063AUTOPILOT_OP: v1 watcher: AUTOPILOT WATCHER WAKE: signal: /home/fixture/github/hxutixnnn/autopilot/state/oss-triage-t4.status\n\n" +
  "Run bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
function monitorInput(suffix: "ONE" | "TWO"): string {
  ...
  return encodeAutopilotOperationalInput("watcher", `MONITOR_${label}_${suffix}`);
}
...
export default function (pi: ExtensionAPI): void {
  pi.on("message_start", (event) => {
    ...
    if (event.message.role !== "assistant" || phase !== "pilot") return;
    phase = "monitor";
  ...
  pi.registerCommand("followup-e2e", {
    description: "Run one pilot prompt followed by typed monitoring input.",
    handler: async (args, ctx) => {
      ...
      phase = "pilot";
      pi.sendUserMessage(`PILOT_PROMPT_${label}`);
    },
  });
}
TS
  ...
  run_followup_case() {
    ...
    if [ "$calm_state" = absent ]; then
      ...
      rm -f "$home/config/calm"
      extensions='-e ./.pi/extensions/ap-calm.ts -e ./followup-e2e.ts'
    else
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env AP_HOME='$home' PI_CODING_AGENT_DIR='$config' AP_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions $extensions $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
    ...
    i=0
    while [ "$i" -lt 240 ]; do
      session_file=$(find "$sessions" -type f -name '*.jsonl' -exec grep -l "PILOT_PROMPT_$label" {} + 2>/dev/null | head -1 || true)
    ...
    # Assert the current rendered frame, not tmux scrollback: a full-screen TUI
    # redraw can leave an earlier copy in scrollback even though only one answer
    # is visible and persisted.
    pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" 2>/dev/null || true)
    [ "$(printf '%s\n' "$pane" | grep -Fc "PILOT_ANSWER_$label" || true)" -eq 1 ] \
      || fail "Pi follow-up $label case rendered a duplicate pilot answer"
    assert_contains "$pane" "PILOT_PROMPT_$label" "Pi follow-up $label case hid the genuine pilot prompt"
    ...
    if [ "$calm_state" = on ]; then
      ...
      if [ "$label" = exact_watcher ]; then
        assert_not_contains "$pane" "AUTOPILOT WATCHER WAKE: signal: /home/fixture/github/hxutixnnn/autopilot/state/oss-triage-t4.status" \
          "Pi exact watcher case rendered the Calm-hidden authoritative payload"
        assert_not_contains "$pane" "Run bin/ap-wake-drain.sh first and handle the queued wake." \
      ...
      pilot_line=$(printf '%s\n' "$pane" | grep -Fn "PILOT_ANSWER_$label" | tail -1 | cut -d: -f1)
      handled_line=$(printf '%s\n' "$pane" | grep -Fn "MONITOR_HANDLED_${label}_ONE" | tail -1 | cut -d: -f1)
      geometry_gap=$((handled_line - pilot_line))
      [ "$geometry_gap" -eq 2 ] \
...
const pilotPrompt = `PILOT_PROMPT_${label}`;
const pilotAnswer = `PILOT_ANSWER_${label}`;
...
const expectedOperationalTexts = Array.from({ length: expected }, (_, index) => {
  ...
  return label === "exact_watcher" && suffix === "ONE"
    ...
    : label === "legacy_away" && suffix === "ONE"
      ...
      : `\u2063AUTOPILOT_OP: v1 watcher: MONITOR_${label}_${suffix}`;
});
...
if (text(userEntries[0].message.content) !== pilotPrompt) throw new Error("genuine pilot prompt changed");
...
if (assistantText.filter((value) => value === pilotAnswer).length !== 1) {
  throw new Error(`pilot answer count changed: ${JSON.stringify(assistantText)}`);
}
...
if (assistantEntries.length !== 2) throw new Error(`expected one pilot and one processing turn, found ${assistantEntries.length}`);
...
JS
  ...
  replay_exact_case() {
    ...
    printf '%s\n' on >"$home/config/calm"
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env AP_HOME='$home' PI_CODING_AGENT_DIR='$config' AP_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions -e ./.pi/extensions/ap-calm.ts -e ./followup-e2e.ts --session '$exact_session'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
    ...
    assert_contains "$pane" "PILOT_PROMPT_exact_watcher" "Pi restart lost the genuine pilot prompt"
    ...
    pilot_line=$(printf '%s\n' "$pane" | grep -Fn 'PILOT_ANSWER_exact_watcher' | tail -1 | cut -d: -f1)
...
const text = (content) => typeof content === "string"
  ? content
  : (content ?? []).filter((item) => item.type === "text").map((item) => item.text).join("");
const exact = "\u2063AUTOPILOT_OP: v1 watcher: AUTOPILOT WATCHER WAKE: signal: /home/fixture/github/hxutixnnn/autopilot/state/oss-triage-t4.status\n\nRun bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
test_hidden_block_geometry_e2e() {
  ...
  mkdir -p \
    "$project/.agents/skills/radio-check" \
  ...
  cat >"$project/.agents/skills/radio-check/SKILL.md" <<'MD'
---
name: radio-check
...
TS
  ...
  start_geometry_pi() {
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 100 -y 44 \
      "cd '$project' && env AP_HOME='$home' PI_CODING_AGENT_DIR='$config' PI_OFFLINE=1 pi --approve --no-context-files --no-prompt-templates --no-extensions -e ./.pi/extensions/ap-calm.ts -e ./geometry-provider.ts $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
  }
  ...
  assert_geometry_gap() {
    ...
    skill_line=$(grep -n -m1 '\[skill\] radio-check' "$file" | cut -d: -f1)
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  sleep 0.1
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l '/skill:radio-check'
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  wait_for_geometry_text "$snapshot" "visible row two" \
    || fail "Pi Calm hidden-block geometry E2E did not complete the /skill:radio-check turn"
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  assert_contains "$(cat "$snapshot")" "[skill] radio-check" "Calm hid the collapsed skill header"
  ...
  assert_geometry_gap "$snapshot" "completed native Calm /skill:radio-check turn"
  ...
  sleep 0.2
  tmux -L "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null || true
  pass "Pi Calm native /skill:radio-check geometry keeps every collapsed thinking and tool block at zero height while preserving expansion, history, restart, and Calm-off rendering"
}

test_working_flight_geometry_and_lifecycle() {
  local fixture out status version
  if ! command -v node >/dev/null 2>&1 || ! command -v npm >/dev/null 2>&1; then
    echo "skip: node or npm not found for Pi Calm working-flight test"
    return 0
  fi
  if [ ! -f "$PI_PACKAGE_DIR/package.json" ]; then
    echo "skip: installed @earendil-works/pi-coding-agent package not found"
  ...
  version=$(node -p "require('$PI_PACKAGE_DIR/package.json').version")
  record_pi_version_evidence "$version" "Pi Calm working-flight assumptions"

  fixture="$TMP_ROOT/working-flight"
  mkdir -p "$fixture/home" "$fixture/lib" "$fixture/node_modules/@earendil-works"
  cp "$EXT" "$fixture/ap-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/ap-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/ap-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/ap-calm-visibility.ts"
  cp "$WORKING_FLIGHT" "$fixture/lib/ap-calm-working-flight.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/lib/ap-operational-input.ts"
  ln -s "$PI_PACKAGE_DIR" "$fixture/node_modules/@earendil-works/pi-coding-agent"
  ln -s "$PI_PACKAGE_DIR/node_modules/@earendil-works/pi-tui" "$fixture/node_modules/@earendil-works/pi-tui"
  ln -s "$PI_PACKAGE_DIR/node_modules/typebox" "$fixture/node_modules/typebox"
  printf '%s\n' '{"type":"module"}' >"$fixture/package.json"

  out=$(cd "$fixture" && EXT="$fixture/ap-calm.ts" AP_HOME="$fixture/home" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
...
const flight = await import(
  `${pathToFileURL(`${process.cwd()}/lib/ap-calm-working-flight.ts`).href}?flight=${Date.now()}`
);
const {
  CALM_WORKING_FLIGHT_WIDGET_KEY,
  CALM_WORKING_FLIGHT_TICK_MS,
  CALM_WORKING_FLIGHT_TICKS_PER_MOVE,
  createCalmWorkingFlightAnimation,
  createCalmWorkingFlightWidget,
} = flight;
...
// --- Calm cadence: the airplane is materially slower than the sky ------------------
{
  // The pre-revision airplane moved one column every 140ms. The revised airplane must be
  // plainly slower in real use while the sky keeps rippling between its steps.
  const msPerColumn = CALM_WORKING_FLIGHT_TICK_MS * CALM_WORKING_FLIGHT_TICKS_PER_MOVE;
  check(msPerColumn >= 700, `airplane cadence ${msPerColumn}ms per column is not materially slower`);
  check(
    CALM_WORKING_FLIGHT_TICKS_PER_MOVE >= 2,
    "the sky cadence is not independent of and faster than the airplane cadence",
  );
  check(
    CALM_WORKING_FLIGHT_TICK_MS < msPerColumn,
    "the sky does not animate faster than the airplane moves",
  );
}

// --- Sky phases loop independently while the airplane stays put --------------------
{
  const width = 40;
  const animation = createCalmWorkingFlightAnimation();
  animation.render(width);
  const startPosition = animation.position();
  const skyRows = new Set();
  const phases = new Set();
  for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE - 1; step += 1) {
    animation.tick();
    check(
      animation.position() === startPosition,
      `the airplane moved on tick ${step + 1} instead of waiting for its own cadence`,
    );
    skyRows.add(strip(animation.render(width)[1]));
    phases.add(animation.skyPhase());
  }
  check(skyRows.size > 1, "the sky did not animate while the airplane was stationary");
  check(phases.size > 1, "the sky phase did not advance between airplane movements");
  // The airplane then moves on its own cadence tick.
  animation.tick();
  check(
    animation.position() !== startPosition,
    "the airplane never moved on its own cadence tick",
  );
  // Sky motion alone must not change the fuselage column.
  const beforeFuselage = strip(animation.render(width)[1]).indexOf("-o-");
  animation.tick();
  const afterFuselage = strip(animation.render(width)[1]).indexOf("-o-");
  check(beforeFuselage === afterFuselage, "advancing only the sky appeared to move the airplane");
}

// --- Sky phases are bounded, fixed-cell, and never change geometry -------------
{
  const width = 30;
  const animation = createCalmWorkingFlightAnimation();
  ...
  for (let step = 0; step < 64; step += 1) {
    ...
    seenPhases.add(animation.skyPhase());
    check(frame.length === 2, `sky phase ${animation.skyPhase()} changed the row count`);
    check(
      visibleWidth(frame[1]) === width,
      `sky phase ${animation.skyPhase()} changed the visible width`,
    );
    animation.tick();
  }
  check(seenPhases.size > 1 && seenPhases.size <= 8, `sky phase set is not bounded: ${seenPhases.size}`);
}
...
// --- Standard ANSI colors, with resets that prevent bleed ------------------------
{
  ...
  for (let step = 0; step < 12; step += 1) {
    const [wingRow, skyRow] = animation.render(width);

    // Standard codes only: no bright variants, no 256-color, no RGB.
    for (const row of [wingRow, skyRow]) {
    ...
    // Wing-row padding must be plain spaces outside any color run.
    const leading = wingRow.slice(0, wingRow.indexOf(ESC));
    check(/^ *$/.test(leading), `wing row padding was colored: ${JSON.stringify(leading)}`);

    // The complete airplane is yellow; every sky cell is blue.
    for (const piece of [`${YELLOW}>${RESET}`, `${YELLOW}<${RESET}`]) {
      if (wingRow.includes(piece.slice(0, -RESET.length))) {
        check(wingRow.includes(piece), `wing was not a closed yellow run: ${JSON.stringify(wingRow)}`);
      }
    }
    check(
      skyRow.includes(`${YELLOW}-o-${RESET}`),
      `fuselage was not a closed yellow run: ${JSON.stringify(skyRow)}`,
    );
    for (const run of skyRow.split(YELLOW)) {
      ...
      for (const blueRun of blueRuns) {
        ...
        check(
          /^[~-]+$/.test(cells),
          `blue run contained a non-sky cell: ${JSON.stringify(cells)}`,
        );
      }
    }
    animation.tick();
...
for (let width = 1; width <= 120; width += 1) {
  ...
  for (let step = 0; step <= width + 8; step += 1) {
    ...
    // The sky row always fills the complete usable width.
    const skyRow = frame[frame.length - 1];
    check(
      visibleWidth(skyRow) === width,
      `width ${width} sky row was ${visibleWidth(skyRow)} cells instead of full width`,
    );
    animation.tick();
  }
}

// --- Directional wing and exact bounce, including tiny spans ---------------------
for (const width of [40, 16, 8, 6, 5, 4, 3, 2]) {
  const animation = createCalmWorkingFlightAnimation();
  animation.render(width);
  const span = width >= 4 ? width - 4 : Math.max(0, width - 2);
  const frames = [];
  for (let step = 0; step < span * CALM_WORKING_FLIGHT_TICKS_PER_MOVE * 3 + 16; step += 1) {
    const frame = animation.render(width);
    frames.push({ position: animation.position(), wing: wingOf(frame) });
    animation.tick();
  }
  ...
  if (width >= 2) {
    ...
    // at or after a reversal shows the old wing.
    for (let index = 1; index < frames.length; index += 1) {
      ...
      if (current.position > previous.position) {
        check(
          previous.wing === ">",
          `width ${width} moved right showing ${previous.wing} at column ${previous.position}`,
        );
      }
      if (current.position < previous.position) {
        check(
          previous.wing === "<",
          `width ${width} moved left showing ${previous.wing} at column ${previous.position}`,
        );
      }
    }
  }
  if (span > 0) {
    const wings = new Set(frames.map((frame) => frame.wing));
    check(wings.has(">") && wings.has("<"), `width ${width} never showed both headings`);
    ...
    for (let index = 1; index < frames.length; index += 1) {
      if (frames[index - 1].wing === ">" && frames[index].wing === "<") rightToLeft = true;
      if (frames[index - 1].wing === "<" && frames[index].wing === ">") leftToRight = true;
    }
...
// --- Shrink and grow resize clamping ----------------------------------------------
{
  ...
  check(animation.position() === 76, `airplane did not reach the wide right edge: ${animation.position()}`);
  ...
  check(visibleWidth(shrunk[1]) === 20, `shrunk sky row was ${visibleWidth(shrunk[1])} cells instead of 20`);
  check(visibleWidth(shrunk[0]) <= 20, "shrunk wing row would wrap");
  check(wingOf(shrunk) === "<", "the airplane did not turn around after being clamped to the right edge");

  for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) animation.tick();
  const afterShrink = animation.render(20);
  check(animation.position() < 16, "the airplane stalled at the edge after a shrink");
  check(visibleWidth(afterShrink[1]) === 20, "motion after a shrink broke the sky row width");

  const grown = animation.render(60);
  check(visibleWidth(grown[1]) === 60, `grown sky row was ${visibleWidth(grown[1])} cells`);
  for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) animation.tick();
  ...
  check(visibleWidth(afterGrow[1]) === 60, "motion after a grow broke the sky row width");
}
...
// --- Deterministic narrow fallbacks ------------------------------------------------
{
  ...
  for (const width of [1, 2, 3]) {
    const fallback = createCalmWorkingFlightAnimation();
    for (let step = 0; step < 12; step += 1) {
      ...
      if (width === 1) {
        check(/^[~-]$/.test(bare), `width 1 fallback was not a single sky cell: ${bare}`);
      } else {
        check(
          bare.includes(">") || bare.includes("<"),
          `width ${width} fallback lost the wing: ${bare}`,
        );
      }
      fallback.tick();
    }
...
// bound to the same animation resumes exactly there; hidden wall time must not jump.
{
  ...
  animation.render(40);
  for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE * 7; step += 1) animation.tick();
  animation.render(40);
  const frozenColumn = animation.position();
  const frozenDirection = animation.direction();
  const frozenPhase = animation.skyPhase();
  check(frozenColumn > 0, `continuity setup never left the left edge: ${frozenColumn}`);

  const first = createCalmWorkingFlightWidget(tui, animation);
  check(first.render(40) && animation.position() === frozenColumn, "binding a widget moved the frozen airplane");
  first.dispose();
  ...
  check(animation.skyPhase() === frozenPhase, "dispose changed the frozen sky phase");

  const resumed = createCalmWorkingFlightWidget(tui, animation);
  ...
  check(wingOf(firstFrame) === (frozenDirection >= 0 ? ">" : "<"), "resume first frame lost wing heading");
  check(animation.skyPhase() === frozenPhase, "resume advanced sky phase without a tick");
  ...
  const beforeHiddenResize = { column: animation.position(), direction: animation.direction(), phase: animation.skyPhase() };
  ...
  check(animation.skyPhase() === beforeHiddenResize.phase, "hidden clamp advanced sky phase");
  // Growing while hidden must not invent motion either.
  animation.clampToWidth(60);
  check(animation.position() === 16, `hidden grow moved the airplane: ${animation.position()}`);
  ...
  for (const scenario of [
    ...
    { label: "after-right", setup(anim) {
      anim.reset(); anim.render(12);
      while (anim.position() < 8) anim.tick();
      for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) anim.tick();
    ...
    const edge = createCalmWorkingFlightAnimation();
    scenario.setup(edge);
    edge.render(12);
    const frozen = { column: edge.position(), direction: edge.direction(), phase: edge.skyPhase() };
    const paused = createCalmWorkingFlightWidget(tui, edge);
    paused.dispose();
    const again = createCalmWorkingFlightWidget(tui, edge);
    again.render(12);
    check(
      edge.position() === frozen.column && edge.direction() === frozen.direction && edge.skyPhase() === frozen.phase,
      `${scenario.label} resume changed frozen edge state`,
    );
    for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) edge.tick();
  ...
  animation.reset();
  check(
    animation.position() === 0 && animation.direction() === 1 && animation.skyPhase() === 0,
    "reset() did not restore the normal initial airplane state",
  );
  animation.render(40);
  check(wingOf(animation.render(40)) === ">", "reset() first frame was not the initial rightward wing");

  // Two controller instances never share motion state.
  const left = createCalmWorkingFlightAnimation();
  const right = createCalmWorkingFlightAnimation();
  left.render(40);
  right.render(40);
  for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE * 3; step += 1) left.tick();
...

{
  ...
  try {
    ...
    const resumed = createCalmWorkingFlightWidget(tui, animation);
    resumed.render(40);
    for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) {
      callbacks[callbacks.length - 1]();
    }
    resumed.render(40);
    check(animation.position() === 1, "unpainted ticks leaked into the resumed cadence");
    check(animation.skyPhase() === 0, "resumed cadence did not restore the rendered sky phase");
    resumed.dispose();

    const committed = createCalmWorkingFlightAnimation();
    const progressing = createCalmWorkingFlightWidget(tui, committed);
    progressing.render(40);
    callbacks[callbacks.length - 1]();
    progressing.render(40);
    const renderedPhase = committed.skyPhase();
    callbacks[callbacks.length - 1]();
    progressing.dispose();
    check(committed.position() === 0, "dispose changed the committed column after an unpainted tick");
    check(committed.skyPhase() === renderedPhase, "dispose changed the committed phase after an unpainted tick");

    const committedResume = createCalmWorkingFlightWidget(tui, committed);
    committedResume.render(40);
    for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE - 2; step += 1) {
      callbacks[callbacks.length - 1]();
    }
    ...
    for (const [targetPosition, targetDirection] of boundaryCases) {
      ...
      const before = { position: edge.position(), direction: edge.direction(), phase: edge.skyPhase() };
      const paused = createCalmWorkingFlightWidget(tui, edge);
      paused.render(12);
      for (let step = 0; step < CALM_WORKING_FLIGHT_TICKS_PER_MOVE; step += 1) {
        callbacks[callbacks.length - 1]();
      ...
      check(
        edge.position() === before.position &&
          ...
          edge.skyPhase() === before.phase,
        `unpainted bounce tick escaped ${targetPosition}/${targetDirection}`,
      );
      const resumedEdge = createCalmWorkingFlightWidget(tui, edge);
...
const extension = await import(`${pathToFileURL(process.env.EXT).href}?flight=${Date.now()}`);
extension.default(pi);
...
const flightWidget = () => ui.widgets.get(CALM_WORKING_FLIGHT_WIDGET_KEY);
...
// --- Turning Calm on while idle shows no airplane until a run starts -------------------
reset();
...
// --- Calm on plus an active run shows the airplane instead of the stock row -----------
reset();
await fire("agent_start");
check(
  ui.widgetOps.length === 1 &&
    ui.widgetOps[0].key === CALM_WORKING_FLIGHT_WIDGET_KEY &&
...
const widget = flightWidget();
check(!!widget, "Calm on did not install the working-flight widget");
...
check(flightWidget() === widget, "repeated starts replaced the running widget");

// --- The animation drives Pi's renderer -------------------------------------------
{
  const before = renderRequests;
  await new Promise((resolve) => setTimeout(resolve, CALM_WORKING_FLIGHT_TICK_MS * 3));
  check(renderRequests > before, "the working animation never requested a TUI render");
}

// --- Settling removes the airplane, stops the animation, and restores the stock row ----
// Drive the live widget far enough that a left-edge reset would be observable.
{
  const moving = flightWidget();
  check(!!moving, "continuity setup lost the live working widget");
  moving.render(40);
  await new Promise((resolve) => setTimeout(resolve, CALM_WORKING_FLIGHT_TICK_MS * CALM_WORKING_FLIGHT_TICKS_PER_MOVE * 5 + 40));
  moving.render(40);
}
const fuselageColumn = (widget) => strip(widget.render(40)[1]).indexOf("-o-");
const freezeColumn = fuselageColumn(flightWidget());
const freezeWing = wingOf(flightWidget().render(40));
...
);
{
  ...
  await new Promise((resolve) => setTimeout(resolve, CALM_WORKING_FLIGHT_TICK_MS * CALM_WORKING_FLIGHT_TICKS_PER_MOVE * 3));
  check(
...
const resumedWidget = flightWidget();
const resumeColumn = fuselageColumn(resumedWidget);
const resumeWing = wingOf(resumedWidget.render(40));
check(
  resumeColumn === freezeColumn && resumeWing === freezeWing,
  `resume reset the airplane instead of continuing: froze ${freezeColumn}/${freezeWing}, resumed ${resumeColumn}/${resumeWing}`,
);
...
for (let cycle = 0; cycle < 3; cycle += 1) {
  ...
  check(
    fuselageColumn(flightWidget()) >= freezeColumn,
...
check(liveTimers === 0 && ui.widgets.size === 0, "fresh session left a stale airplane");
await fire("agent_start");
check(fuselageColumn(flightWidget()) === 0, "fresh session did not restart at the left edge");
check(wingOf(flightWidget().render(40)) === ">", "fresh session lost the initial rightward wing");
await fire("agent_settled");
...
check(ui.widgets.size === 0, "toggling Calm off during a run left the airplane on screen");
check(
  ui.workingVisible[ui.workingVisible.length - 1] === true,
  "toggling Calm off during a run did not restore Pi's stock working row",
);

// Toggling Calm back on during the same run returns the airplane.
reset();
await calmCommand.handler("", ctx);
check(liveTimers === 1, "toggling Calm on during a run did not return the airplane");
check(
  ui.workingVisible[ui.workingVisible.length - 1] === false,
...
// A run started after toggling Calm on while idle uses the airplane.
reset();
await calmCommand.handler("", ctx);
await calmCommand.handler("", ctx);
await fire("agent_start");
check(liveTimers === 1, "a later run did not use the airplane after an idle Calm toggle");
await fire("agent_settled");
...
JS
  ...
  status=$?
  [ "$status" -eq 0 ] || fail "Pi Calm working-flight checks failed: $out"
  [ -z "$out" ] || fail "Pi Calm working-flight test printed output: $out"
  pass "Pi Calm working flight moves on a slow independent cadence over faster fixed-cell blue sky, paints the complete airplane standard yellow with balanced resets, keeps ANSI-stripped width exact, flips the directional wing on the exact bounce at both edges and every width, clamps visible and hidden resizes, falls back deterministically when narrow, freezes and resumes column/direction across settle/start without hidden-time jumps or duplicate timers, resets only on a fresh session, and installs and removes one scheduler-owning widget across starts, settle, abort, failure, shutdown, reload, replacement, and Calm toggles while leaving Calm-off visibility untouched"
}

test_interactive_terminal_e2e() {
  local project config home session_file export_file export_dom default_snapshot expanded_snapshot hidden_snapshot active_before_snapshot active_hidden_snapshot export_snapshot restored_snapshot working_snapshot working_response_snapshot restarted_snapshot resumed_restored_snapshot hash_before hash_after now version chrome chrome_pid chrome_wait active_wait active_screen_wait airplane_frame_one airplane_frame_two airplane_resized_snapshot airplane_focus_snapshot airplane_cleared_snapshot airplane_fuselage_line airplane_wing_line airplane_column_one airplane_column_two airplane_line airplane_color_snapshot airplane_color_line airplane_sky_snapshot airplane_sky_line airplane_sky_first airplane_sky_changed airplane_narrow_snapshot airplane_narrow_wings airplane_freeze_snapshot airplane_resume_snapshot airplane_freeze_column airplane_freeze_wing airplane_resume_column airplane_resume_wing
  if ! command -v pi >/dev/null 2>&1 || ! command -v tmux >/dev/null 2>&1; then
  ...
  airplane_frame_one="$TMP_ROOT/airplane-frame-one.txt"
  airplane_frame_two="$TMP_ROOT/airplane-frame-two.txt"
  airplane_resized_snapshot="$TMP_ROOT/airplane-resized.txt"
  airplane_focus_snapshot="$TMP_ROOT/airplane-focus.txt"
  airplane_cleared_snapshot="$TMP_ROOT/airplane-cleared.txt"
  airplane_color_snapshot="$TMP_ROOT/airplane-color.txt"
  airplane_sky_snapshot="$TMP_ROOT/airplane-sky.txt"
  airplane_narrow_snapshot="$TMP_ROOT/airplane-narrow.txt"
  airplane_freeze_snapshot="$TMP_ROOT/airplane-freeze.txt"
  airplane_resume_snapshot="$TMP_ROOT/airplane-resume.txt"
  ...
  cp "$VISIBILITY" "$project/.pi/extensions/lib/ap-calm-visibility.ts"
  cp "$WORKING_FLIGHT" "$project/.pi/extensions/lib/ap-calm-working-flight.ts"
  cp "$ROOT/.pi/extensions/lib/ap-operational-input.ts" "$project/.pi/extensions/lib/ap-operational-input.ts"
  cp "$WATCH_EXT" "$project/.pi/extensions/ap-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/ap-primary-turnend-guard.ts" "$project/.pi/extensions/ap-primary-turnend-guard.ts"
  cp \
    "$ROOT/bin/ap-sessionstart-nudge.sh" \
    "$ROOT/bin/ap-primary-scope-lib.sh" \
    "$ROOT/bin/ap-gate-refuse-lib.sh" \
    "$ROOT/bin/ap-operational-input.sh" \
    "$project/bin/"
  chmod +x "$project/bin/"*.sh
  cat >"$project/.pi/extensions/ap-calm-e2e-inject.ts" <<'TS'
import {
  type AssistantMessage,
  createAssistantMessageEventStream,
} from "@earendil-works/pi-ai";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { encodeAutopilotOperationalInput } from "./lib/ap-operational-input.ts";
...
export default function (pi: ExtensionAPI): void {
  pi.registerProvider("calm-e2e", {
    ...
    models: [
      ...
      {
        id: "delayed-airplane",
        name: "Long-delay Calm working-flight fixture",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 4096,
    ...
    streamSimple(model, _context, options) {
      ...
      void (async () => {
        ...
        await new Promise<void>((resolve) => {
          const timer = setTimeout(resolve, model.id === "delayed-airplane" ? 90000 : 1500);
  ...
  pi.registerCommand("calm-inject-e2e", {
    ...
    handler: async (args, ctx) => {
      const fixtures = new Map([
        ...
        ["from-autopilot", "corr=0123456789abcdef CURRENT_FROM_AUTOPILOT_E2E"],
        ["launch-brief", "CURRENT_LAUNCH_BRIEF_E2E"],
      ] as const);
      const kind = args.trim() as Parameters<typeof encodeAutopilotOperationalInput>[0];
      ...
      await pi.sendUserMessage(encodeAutopilotOperationalInput(kind, body), {
        deliverAs: "followUp",
      });
    },
  });
  pi.registerCommand("calm-airplane-e2e", {
    description: "Start the long-delay working-flight fixture.",
    handler: async (_args, ctx) => {
      const model = ctx.modelRegistry.find("calm-e2e", "delayed-airplane");
      if (!model || !(await pi.setModel(model))) {
...
{"type":"message","id":"a0000007","parentId":"a0000006","timestamp":"$now","message":{"role":"assistant","content":[{"type":"thinking","thinking":"third internal reasoning block"},{"type":"toolCall","id":"call_watch_e2e","name":"ap_watch_arm_pi","arguments":{}}],"api":"anthropic-messages","provider":"anthropic","model":"claude-sonnet-4-5","usage":{"input":2,"output":1,"cacheRead":0,"cacheWrite":0,"totalTokens":3,"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0,"total":0}},"stopReason":"toolUse","timestamp":7}}
{"type":"message","id":"a0000008","parentId":"a0000007","timestamp":"$now","message":{"role":"toolResult","toolCallId":"call_watch_e2e","toolName":"ap_watch_arm_pi","content":[{"type":"text","text":"watcher: started Pi extension arm child 1"}],"details":{"ok":true,"message":"watcher: started Pi extension arm child 1"},"isError":false,"timestamp":8}}
{"type":"custom","id":"a0000009","parentId":"a0000008","timestamp":"$now","customType":"autopilot-synthetic-input-presentation","data":{"content":"AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","kind":"watcher"}}
{"type":"custom_message","id":"a0000010","parentId":"a0000009","timestamp":"$now","customType":"autopilot-synthetic-input","content":"AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/ap-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","display":false,"details":{"kind":"watcher"}}
{"type":"message","id":"a0000011","parentId":"a0000010","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"AUTOPILOT WATCHER WAKE: can you explain this phrase?"}],"timestamp":11}}
{"type":"message","id":"a0000012","parentId":"a0000011","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Pilot quote: \u2063AUTOPILOT_OP: v1 watcher: QUOTED_CURRENT_NEAR_MISS"}],"timestamp":12}}
{"type":"message","id":"a0000013","parentId":"a0000012","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"AUTOPILOT_OP: v1 watcher: ASCII_ONLY_NEAR_MISS"}],"timestamp":13}}
{"type":"message","id":"a0000014","parentId":"a0000013","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Ordinary pilot text before \u2063AUTOPILOT_OP: v1 watcher: EMBEDDED_CURRENT_NEAR_MISS"}],"timestamp":14}}
{"type":"message","id":"a0000015","parentId":"a0000014","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"\u2063ordinary pilot text after unrelated separator"}],"timestamp":15}}
...
JSON
  ...
  tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 180 -y 44 \
    "cd '$project' && env AP_HOME='$home' PI_CODING_AGENT_DIR='$config' AP_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-skills --no-prompt-templates --no-context-files --session '$session_file'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 30"
  ...
  assert_contains "$(cat "$default_snapshot")" "ap_watch_arm_pi" "Calm-off transcript did not show the Autopilot watcher tool"
  assert_contains "$(cat "$default_snapshot")" "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status" "Calm-off transcript did not show the synthetic Autopilot presentation row"
  assert_contains "$(cat "$default_snapshot")" "Thinking..." "reasoning fixture did not render Pi's collapsed thinking label"
  assert_contains "$(cat "$default_snapshot")" "ap-calm.ts" "project-local Pi calm extension did not auto-load"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$default_snapshot")" 'Run `bin/ap-session-start.sh` now' \
  ...
  while [ "$active_screen_wait" -lt 120 ]; do
    ...
    if ! grep -Fq "CALM_E2E_OUTPUT" "$hidden_snapshot" &&
      ...
      grep -Fq "AUTOPILOT WATCHER WAKE: can you explain this phrase?" "$hidden_snapshot" &&
  ...
  assert_not_contains "$(cat "$hidden_snapshot")" "ap_watch_arm_pi" "/calm left the Autopilot watcher tool call shell in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "watcher: started Pi extension arm child" "/calm left the Autopilot watcher tool result in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status" "/calm left a synthetic Autopilot user-role presentation in the transcript"
  ...
  assert_contains "$(cat "$hidden_snapshot")" "AUTOPILOT WATCHER WAKE: can you explain this phrase?" "/calm hid a genuine near-miss user prompt"
  for near_miss in \
    QUOTED_CURRENT_NEAR_MISS \
    ASCII_ONLY_NEAR_MISS \
    EMBEDDED_CURRENT_NEAR_MISS \
    "ordinary pilot text after unrelated separator"
  do
  ...
  for fixture in \
    ...
    "turn-end-guard|CURRENT_TURN_END_E2E" \
    "away-supervisor|CURRENT_AWAY_E2E" \
    "from-autopilot|CURRENT_FROM_AUTOPILOT_E2E" \
    "launch-brief|CURRENT_LAUNCH_BRIEF_E2E"
  do
    kind=${fixture%%|*}
    needle=${fixture#*|}
...
const nativeSessionStart = entries.find((entry) =>
  entry.type === "custom_message" &&
  entry.customType === "autopilot-sessionstart-nudge"
);
if (
  !nativeSessionStart ||
  nativeSessionStart.display !== false ||
  nativeSessionStart.details?.kind !== "session-start" ||
  !nativeSessionStart.content?.startsWith("\u2063AUTOPILOT_OP: v1 session-start: ")
) {
...
const expected = new Map([
  ...
  ["CURRENT_FROM_AUTOPILOT_E2E", "from-autopilot"],
...
for (const [needle, kind] of expected) {
  ...
  const exactEnvelope = kind === "from-autopilot"
    ? text?.startsWith("[ap-from-autopilot]\u2063corr=0123456789abcdef ")
    : text?.startsWith(`\u2063AUTOPILOT_OP: v1 ${kind}: `);
...
JS
  ...
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$active_hidden_snapshot")" 'Run `bin/ap-session-start.sh` now' \
    "Calm showed the native session-start operational input"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
    CURRENT_FROM_AUTOPILOT_E2E \
...
if (!serialized.includes("autopilot-synthetic-input") || !serialized.includes("/tmp/probe.status")) process.exit(1);
const synthetic = entries.find((entry) => entry.type === "custom_message" && entry.customType === "autopilot-synthetic-input");
...
if (messages.includes("[autopilot-synthetic-input]")) process.exit(1);
for (const current of ["CURRENT_WATCHER_E2E", "CURRENT_TURN_END_E2E", "CURRENT_AWAY_E2E", "CURRENT_FROM_AUTOPILOT_E2E", "CURRENT_LAUNCH_BRIEF_E2E"]) {
  if (!messages.includes(current)) process.exit(1);
}
if (!tree.includes("autopilot-synthetic-input") || !tree.includes("/tmp/probe.status")) process.exit(1);
JS
  else
  ...
  assert_contains "$(cat "$restored_snapshot")" "ap_watch_arm_pi" "second /calm did not restore the Autopilot watcher tool shell"
  assert_contains "$(cat "$restored_snapshot")" "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status" "second /calm did not restore the synthetic Autopilot user row"
  for restored in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  # Calm on plus a genuinely active run replaces Pi's stock working row with the airplane.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/calm-airplane-e2e"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$working_snapshot"
    if grep -Fq -- '-o-' "$working_snapshot"; then
      break
    fi
    sleep 0.025
    active_screen_wait=$((active_screen_wait + 1))
  done
  cp "$working_snapshot" "$airplane_frame_one"
  assert_contains "$(cat "$airplane_frame_one")" '-o-' "Calm did not show the working flight during a real provider wait"
  assert_not_contains "$(cat "$airplane_frame_one")" "Working..." "Calm left Pi's stock working row visible while the flight was shown"
  assert_not_contains "$(cat "$airplane_frame_one")" "calm transcript" "the real provider wait showed a persistent Calm status row"
  assert_not_contains "$(cat "$airplane_frame_one")" "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status" "the real provider wait restored a hidden operational row"
  airplane_fuselage_line=$(grep -F -- '-o-' "$airplane_frame_one" | head -1)
  airplane_wing_line=$(grep -E '[<>]' "$airplane_frame_one" | tail -1)
  case "$airplane_wing_line" in
    *'>'*|*'<'*) : ;;
    *) fail "the working flight lost its directional nose" ;;
  esac
  assert_not_contains "$airplane_fuselage_line" "Working" "the flight row carried extra status copy"
  case "$airplane_fuselage_line" in
    *'·'*) : ;;
    *) fail "the working flight rendered no clouds" ;;
  esac
  # Standard ANSI colors: blue sky, yellow airplane, no theme/bright/256/RGB escapes.
  tmux -L "$TMUX_SOCKET" capture-pane -p -e -t "$TMUX_SESSION" >"$airplane_color_snapshot"
  airplane_color_line=$(grep -F -- '-o-' "$airplane_color_snapshot" | head -1)
  [ -n "$airplane_color_line" ] || fail "could not capture a colored working-flight row"
  case "$airplane_color_line" in
    *'[34m'*) : ;;
    *) fail "the sky was not rendered with standard ANSI blue" ;;
  esac
  case "$airplane_color_line" in
    *'[33m'*) : ;;
    *) fail "the airplane was not rendered with standard ANSI yellow" ;;
  esac
  case "$airplane_color_line" in
    *'[38;2;'*|*'[38;5;'*|*'[9'[0-9]'m'*) fail "the working flight used a non-standard color escape" ;;
    *) : ;;
  esac

  # The sky animates on its own faster cadence while the airplane holds its column.
  airplane_column_one=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_frame_one")
  airplane_sky_changed=0
  airplane_sky_first=$(grep -F -- '-o-' "$airplane_frame_one" | head -1)
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 60 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_sky_snapshot"
    airplane_sky_line=$(grep -F -- '-o-' "$airplane_sky_snapshot" | head -1)
    airplane_column_two=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_sky_snapshot")
    if [ -n "$airplane_sky_line" ] && [ "$airplane_column_two" = "$airplane_column_one" ] &&
      [ "$airplane_sky_line" != "$airplane_sky_first" ]; then
      airplane_sky_changed=1
      break
    fi
    sleep 0.05
    active_screen_wait=$((active_screen_wait + 1))
  done
  [ "$airplane_sky_changed" -eq 1 ] \
    || fail "the sky never animated while the working flight held its column"

  # Two frames at different fuselage columns prove genuine horizontal motion.
  airplane_column_two=""
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_frame_two"
    airplane_column_two=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_frame_two")
    if [ -n "$airplane_column_two" ] && [ "$airplane_column_two" != "$airplane_column_one" ]; then
      break
    fi
    sleep 0.05
    active_screen_wait=$((active_screen_wait + 1))
  done
  [ -n "$airplane_column_two" ] || fail "the working flight disappeared between animation frames"
  [ "$airplane_column_two" != "$airplane_column_one" ] \
    || fail "the working flight never moved horizontally (stuck at column $airplane_column_one)"
  ...
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_resized_snapshot"
    airplane_fuselage_line=$(grep -F -- '-o-' "$airplane_resized_snapshot" | head -1)
    if [ -n "$airplane_fuselage_line" ] && [ "${#airplane_fuselage_line}" -eq 100 ]; then
      break
    fi
    sleep 0.05
    active_screen_wait=$((active_screen_wait + 1))
  done
  assert_contains "$(cat "$airplane_resized_snapshot")" '-o-' "the working flight left the screen after a resize"
  airplane_fuselage_line=$(grep -F -- '-o-' "$airplane_resized_snapshot" | head -1)
  [ "${#airplane_fuselage_line}" -eq 100 ] \
    || fail "after resizing to 100 columns the flight row was ${#airplane_fuselage_line} cells instead of exactly 100"
  # Exactly one cloud row means the sprite reflowed rather than wrapping onto extra rows.
  [ "$(grep -c -F -- '-o-' "$airplane_resized_snapshot")" -eq 1 ] \
    || fail "the working flight wrapped onto more than one sky row after the resize"
  while IFS= read -r airplane_line; do
    [ "${#airplane_line}" -le 100 ] \
      || fail "a rendered line was ${#airplane_line} cells after resizing to 100 columns"
  done <"$airplane_resized_snapshot"
  airplane_column_one=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_resized_snapshot")
  [ "$airplane_column_one" -le 97 ] \
    || fail "the working flight fuselage started at column $airplane_column_one and cannot fit in 100 columns"
  ...
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_resized_snapshot"
    airplane_column_two=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_resized_snapshot")
    if [ -n "$airplane_column_two" ] && [ "$airplane_column_two" != "$airplane_column_one" ]; then
      break
    fi
    sleep 0.05
  ...
  [ -n "$airplane_column_two" ] && [ "$airplane_column_two" != "$airplane_column_one" ] \
    || fail "the working flight stopped moving after the resize"
  [ "$airplane_column_two" -le 97 ] \
    || fail "the working flight moved offscreen after the resize"

  # A narrow terminal shortens the track enough to observe both bounce directions.
  # The wing must show the heading it is about to travel, so a full traverse shows both.
  tmux -L "$TMUX_SOCKET" resize-window -t "$TMUX_SESSION" -x 12 -y 20
  airplane_narrow_wings=""
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 400 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_narrow_snapshot"
    if grep -Fq '>' "$airplane_narrow_snapshot"; then
      case "$airplane_narrow_wings" in *R*) : ;; *) airplane_narrow_wings="${airplane_narrow_wings}R" ;; esac
    fi
    if grep -Fq '<' "$airplane_narrow_snapshot"; then
      case "$airplane_narrow_wings" in *L*) : ;; *) airplane_narrow_wings="${airplane_narrow_wings}L" ;; esac
    fi
    case "$airplane_narrow_wings" in
      *R*L*|*L*R*) break ;;
    esac
    sleep 0.1
    active_screen_wait=$((active_screen_wait + 1))
  ...
  case "$airplane_narrow_wings" in
    *R*L*|*L*R*) : ;;
    *) fail "the working flight never showed both wing headings on a narrow track (saw '$airplane_narrow_wings')" ;;
  esac
  airplane_fuselage_line=$(grep -F -- '-o-' "$airplane_narrow_snapshot" | head -1)
  [ "${#airplane_fuselage_line}" -eq 12 ] \
    || fail "the narrow working-flight row was ${#airplane_fuselage_line} cells instead of exactly 12"
  tmux -L "$TMUX_SOCKET" resize-window -t "$TMUX_SESSION" -x 100 -y 30

  # Typing still reaches the editor while the animation runs.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "FOCUSPROBE"
  wait_for_text "$airplane_focus_snapshot" "FOCUSPROBE" \
    || fail "keyboard input did not reach the editor while the working flight animated"
  i=0
  while [ "$i" -lt 10 ]; do
  ...
  # Capture the last on-screen column and wing before settling so the next working
  # period in this same Pi session can prove freeze/resume continuity.
  tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_freeze_snapshot"
  airplane_freeze_column=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_freeze_snapshot")
  airplane_freeze_wing=$(grep -E '[<>]' "$airplane_freeze_snapshot" | tail -1 || true)
  case "$airplane_freeze_wing" in
    *'>'*) airplane_freeze_wing='>' ;;
    *'<'*) airplane_freeze_wing='<' ;;
    *) fail "could not read the freeze-frame wing heading" ;;
  esac
  [ -n "$airplane_freeze_column" ] && [ "$airplane_freeze_column" -gt 1 ] \
    || fail "freeze frame never left the left edge (column '${airplane_freeze_column:-empty}')"

  # Escape aborts the run, and the abort path removes the flight with no residue.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Escape
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_cleared_snapshot"
    if ! grep -Fq -- '-o-' "$airplane_cleared_snapshot"; then
      break
    fi
    sleep 0.05
    active_screen_wait=$((active_screen_wait + 1))
  done
  assert_not_contains "$(cat "$airplane_cleared_snapshot")" '-o-' "Escape did not remove the working flight"
  assert_not_contains "$(cat "$airplane_cleared_snapshot")" "CALM_WORKING_E2E_RESPONSE" "the long-delay fixture settled instead of aborting on Escape"
  assert_not_contains "$(cat "$airplane_cleared_snapshot")" "FOCUSPROBE" "the editor kept the focus probe text after Escape"

  # A later working period in the same Pi process must resume the frozen column and
  # wing rather than recreating the airplane at the left edge. Capture the first resumed
  # frames quickly so the slow airplane cadence cannot advance before the assertion.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/calm-airplane-e2e"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  airplane_resume_column=""
  airplane_resume_wing=""
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_resume_snapshot"
    if grep -Fq -- '-o-' "$airplane_resume_snapshot"; then
      airplane_resume_column=$(awk 'index($0,"-o-"){print index($0,"-o-"); exit}' "$airplane_resume_snapshot")
      airplane_resume_wing=$(grep -E '[<>]' "$airplane_resume_snapshot" | tail -1 || true)
      case "$airplane_resume_wing" in
        *'>'*) airplane_resume_wing='>' ;;
        *'<'*) airplane_resume_wing='<' ;;
      esac
      break
  ...
  [ -n "$airplane_resume_column" ] \
    || fail "the second working period never showed the working flight"
  [ "$airplane_resume_column" -eq "$airplane_freeze_column" ] \
    || fail "the second working period reset the airplane from column $airplane_freeze_column to $airplane_resume_column instead of resuming"
  [ "$airplane_resume_wing" = "$airplane_freeze_wing" ] \
    || fail "the second working period changed wing from $airplane_freeze_wing to $airplane_resume_wing"
  assert_not_contains "$(cat "$airplane_resume_snapshot")" "Working..." \
  ...
  assert_not_contains "$(cat "$airplane_cleared_snapshot")" '-o-' "Escape did not remove the resumed working flight"

  # Calm off restores Pi's stock working row and never shows the flight.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/calm"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
  ...
  assert_not_contains "$(cat "$working_snapshot")" '-o-' "Calm off showed the working flight"
  ...
  tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$airplane_cleared_snapshot"
  assert_not_contains "$(cat "$airplane_cleared_snapshot")" '-o-' "a settled run left the working flight on screen"
  ...
  assert_not_contains "$(cat "$restarted_snapshot")" "ap_watch_arm_pi" "restart/resume reset Calm and restored the Autopilot watcher tool"
  assert_not_contains "$(cat "$restarted_snapshot")" "AUTOPILOT WATCHER WAKE: signal: /tmp/probe.status" "restart/resume reset Calm and restored a legacy presentation row"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/quit"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  pass "Pi calm native E2E replaces the stock working row with a moving, resize-clamped working flight that freezes and resumes across two working periods in one Pi session, clears on abort, keeps pilot turns visible, hides exact operational user rows without changing persistence, restores stock rendering Calm-off, survives restart, and preserves export plus Ctrl+O behavior"
}
