#!/usr/bin/env bash
# tests/ap-composer-lib.test.sh - the shared composer-content classifier
# (bin/ap-composer-lib.sh), the ONE fleet-wide owner every backend adapter
# delegates its empty|pending|unknown verdict to.
#
# The load-bearing contract, task ap-composer-shellglyph-safety:
...
classify() { ap_composer_classify_content "$@"; }
...
test_bare_shell_glyphs_are_unknown() {
  ...
  pass "ap_composer_classify_content: a bare shell prompt glyph (>/\$/%/#) reads unknown, never empty"
}
...
test_stripped_unbordered_content_uses_plain_content() {
  ...
  pass "ap_composer_classify_content: stripped unbordered content is unknown except verified agent glyphs"
}
...
test_bare_shell_prompt_with_command_is_not_empty() {
  ...
  pass "ap_composer_classify_content: a bare shell prompt carrying a command is not empty"
}
...
test_bordered_shell_glyph_is_empty() {
  ...
  pass "ap_composer_classify_content: a bare prompt glyph inside a bordered composer box reads empty (claude's own idle composer)"
}
...
test_agent_glyphs_are_empty_bordered_and_bare() {
  ...
  pass "ap_composer_classify_content: agent prompt glyphs (❯ claude, › codex) read empty bordered or bare"
}
...
test_empty_content_is_empty() {
  ...
  pass "ap_composer_classify_content: an empty composer reads empty"
}
...
test_idle_placeholder_is_empty() {
  ...
  pass "ap_composer_classify_content: a known idle placeholder reads empty, before and after glyph stripping"
}
...
test_idle_placeholder_case_mode_is_explicit() {
  ...
  pass "ap_composer_classify_content: idle matching preserves the caller's case mode"
}
...
test_real_text_is_pending() {
  ...
  pass "ap_composer_classify_content: real unsubmitted text reads pending (including a popup argument-hint fill)"
}
