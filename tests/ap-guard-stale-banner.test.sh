#!/usr/bin/env bash
# Regression tests for ap-guard's stale-watcher banner deduplication.
#
# The first stale command in one AP_HOME must print the full actionable watcher
# banner.
...
TMP_ROOT=$(ap_test_tmproot ap-guard-stale-banner)
...
make_guard_case() {
  ...
  ap_write_meta "$home/state/task.meta" "window=autopilot:ap-task" "kind=flight"
  printf '%s\n' "$dir"
}
...
run_guard_case() {
  local dir=$1
  AP_ROOT_OVERRIDE="$(case_root "$dir")" \
    AP_HOME="$(case_home "$dir")" \
    AP_GUARD_GRACE=999 \
    "$ROOT/bin/ap-guard.sh" 2>&1
}
...
test_first_stale_call_prints_full_banner() {
  ...
  pass "ap-guard stale banner: first stale call prints the full actionable banner"
}
...
test_repeated_same_episode_prints_reminder_only() {
  ...
  pass "ap-guard stale banner: repeated same-episode calls print a concise reminder only"
}
...
test_healthy_recovery_rearms_next_stale_episode() {
  ...
  pass "ap-guard stale banner: healthy recovery rearms the next stale episode"
}
...
test_concurrent_same_episode_prints_one_full_banner() {
  ...
  pass "ap-guard stale banner: concurrent same-episode calls claim exactly one full banner"
}
...
test_home_isolation() {
  ...
  pass "ap-guard stale banner: deduplication is isolated per AP_HOME"
}
...
test_queued_wake_warning_stays_independent() {
  ...
  pass "ap-guard stale banner: queued-wake warning remains independent"
}
...
test_read_only_before_writable_does_not_consume_full_banner() {
  ...
  pass "ap-guard stale banner: read-only before writable does not consume full banner"
}
...
test_read_only_during_episode_observes_without_mutating_marker() {
  ...
  pass "ap-guard stale banner: read-only during episode observes without mutating marker"
}
...
test_healthy_read_only_does_not_clear_marker() {
  ...
  pass "ap-guard stale banner: healthy read-only does not clear marker"
}
...
test_read_only_never_mutates_stale_banner_state_files() {
  ...
  pass "ap-guard stale banner: read-only never mutates stale-banner state files"
}
