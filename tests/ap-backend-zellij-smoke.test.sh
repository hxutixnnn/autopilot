#!/usr/bin/env bash
# tests/ap-backend-zellij-smoke.test.sh - real zellij smoke test for the
# zellij session-provider adapter (bin/backends/zellij.sh), P3 of
# data/ap-backend-design-d7 (report.md "Zellij Backend"). Mirrors
# tests/ap-backend-herdr-smoke.test.sh's structure: every other suite fakes
# the CLI, this one talks to a REAL zellij server - but ALWAYS on a private,
# named, throwaway session (via AP_ZELLIJ_SESSION, never the real "autopilot"
# session name), so it never touches a pilot's real zellij usage. Skips
...
SESSION="ap-backend-smoke-$$"
export AP_ZELLIJ_SESSION="$SESSION"
...
LONG_CWD="$TMP_CWD/ap-zellij-wrap-$$/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/cccccccccccccccccccccccccccccccccccccccc/dddddddddddddddddddddddddddddddddddddddd"
...
ap_backend_source zellij || fail "ap_backend_source zellij failed"

# --- version gate + container ensure -----------------------------------------

ap_backend_zellij_version_check || fail "version_check failed against the real installed zellij"
pass "real zellij: version_check accepts the installed binary's version"

CONTAINER=$(ap_backend_zellij_container_ensure) || fail "container_ensure failed"
...
CONTAINER2=$(ap_backend_zellij_container_ensure) || fail "second container_ensure failed"
...
LABEL="ap-smoke1"
TASK_IDS=$(ap_backend_zellij_create_task "$SESSION" "$LABEL" /tmp) || fail "create_task failed"
...
if ap_backend_zellij_create_task "$SESSION" "$LABEL" /tmp >/dev/null 2>&1; then
...
ap_backend_zellij_send_key "$TARGET" Escape "$LABEL" \
  || fail "send_key with a matching expected task label should succeed"
if ap_backend_zellij_send_key "$TARGET" Escape "ap-not-$LABEL" >/dev/null 2>&1; then
...
ap_backend_zellij_send_literal "$TARGET" 'echo literal-then-key-pilot' \
  || fail "send_literal failed"
sleep 0.3
ap_backend_zellij_send_key "$TARGET" Enter || fail "send_key Enter failed"
sleep 0.5
out=$(ap_backend_zellij_capture "$TARGET" 20) || fail "capture failed after send_literal+send_key"
case "$out" in
...
ap_backend_zellij_send_text_line "$TARGET" "echo pilot-on-deck-line" \
  || fail "send_text_line failed"
sleep 0.5
out=$(ap_backend_zellij_capture "$TARGET" 20) || fail "capture failed after send_text_line"
case "$out" in
  *pilot-on-deck-line*) : ;;
...
out=$(ap_backend_zellij_capture "$TARGET" 40) || fail "viewport capture failed"
case "$out" in
  *pilot-on-deck-line*) : ;;
  *) fail "real zellij: viewport-sized capture did not include recent output"$'\n'"$out" ;;
esac
out=$(ap_backend_zellij_capture "$TARGET" 80) || fail "full-scrollback capture failed"
case "$out" in
  *pilot-on-deck-line*) : ;;
...
ap_backend_zellij_send_text_line "$TARGET" "cd /tmp"
sleep 0.3
p=$(ap_backend_zellij_current_path "$TARGET") || fail "current_path failed"
case "$p" in
  */tmp) : ;;
...
ap_backend_zellij_send_text_line "$TARGET" "cd $LONG_CWD_Q"
sleep 0.3
p_wrap=$(ap_backend_zellij_current_path "$TARGET") || fail "current_path failed for a long wrapped path"
...
# that subshell is fully interactive. ap_backend_zellij_current_path's active
# pwd-probe (docs/zellij-backend.md) is what ap-spawn.sh's worktree-discovery
...
ap_backend_zellij_send_text_line "$TARGET" 'cd / && bash'
sleep 0.5
ap_backend_zellij_send_text_line "$TARGET" "cd $TMP_CWD_Q"
sleep 0.3
p2=$(ap_backend_zellij_current_path "$TARGET") || fail "current_path failed inside a nested subshell"
...
ap_backend_zellij_send_text_line "$TARGET" 'exit'
sleep 0.3

# --- key names: Escape and Ctrl-C, verified names --------------------------

ap_backend_zellij_send_key "$TARGET" Escape || fail "send_key Escape (normalized to 'Esc') failed"
pass "real zellij: send_key Escape (normalized to the verified 'Esc' name) succeeds"

ap_backend_zellij_send_key "$TARGET" C-c || fail "send_key C-c (normalized to 'Ctrl c') failed"
...
# zellij has no native agent-state adapter function; ap_backend_busy_state's
# dispatcher (bin/ap-backend.sh) falls through to "unknown" for any backend
...
bs=$(ap_backend_busy_state zellij "$TARGET")
[ "$bs" = unknown ] || fail "ap_backend_busy_state should report unknown for zellij (D5: no native primitive), got '$bs'"
pass "real zellij: ap_backend_busy_state reports unknown (D5 - watcher falls back to pane-regex, same as tmux)"

# --- kill -----------------------------------------------------------------

ap_backend_zellij_kill "$TARGET"
sleep 0.3
LIVE_AFTER_KILL=$(ap_backend_zellij_cli "$SESSION" action list-panes --json 2>/dev/null \
...
ap_backend_zellij_kill "$TARGET" || fail "kill on an already-dead target must stay best-effort (never fail)"
pass "real zellij: kill removes the pane+tab and is idempotent/best-effort"

# --- list_live (name-based recovery discovery) --------------------------------

LABEL2="ap-smoke2"
TASK_IDS2=$(ap_backend_zellij_create_task "$SESSION" "$LABEL2" /tmp) || fail "second create_task failed"
read -r _TAB_ID2 PANE_ID2 <<EOF
$TASK_IDS2
EOF
live=$(ap_backend_zellij_list_live "$SESSION")
...
pass "real zellij: list_live discovers a live task tab by ap-<id> name"

ap_backend_zellij_kill "$SESSION:$PANE_ID2"
