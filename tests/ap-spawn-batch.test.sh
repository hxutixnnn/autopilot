#!/usr/bin/env bash
# Behavior tests for ap-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. AP_SPAWN_NO_GUARD=1 keeps them off the
...
TMP_ROOT=$(ap_test_tmproot ap-spawn-batch)
export AP_BACKEND=tmux

# Clear ambient autopilot overrides so the behavior test owns its environment.
run_spawn() {
  AP_ROOT_OVERRIDE='' \
    AP_HOME='' \
    AP_STATE_OVERRIDE='' \
    AP_DATA_OVERRIDE='' \
    AP_PROJECTS_OVERRIDE='' \
    AP_CONFIG_OVERRIDE='' \
    AP_SPAWN_NO_GUARD=1 \
    "$SPAWN" "$@" 2>&1
}
...
# A projects/ path is resolved through the autopilot home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(AP_ROOT_OVERRIDE='' AP_STATE_OVERRIDE='' AP_DATA_OVERRIDE='' AP_CONFIG_OVERRIDE='' \
        AP_HOME="$home" AP_PROJECTS_OVERRIDE="$projects" AP_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(AP_ROOT_OVERRIDE='' AP_STATE_OVERRIDE='' AP_DATA_OVERRIDE='' AP_PROJECTS_OVERRIDE='' AP_CONFIG_OVERRIDE='' \
        AP_HOME="$home" AP_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    fi
    status=$?
...
AP_HOME scopes projects/|no|nope-home-z7
AP_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the autopilot home for single-task spawn"
}
