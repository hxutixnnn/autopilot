#!/usr/bin/env bash
# tests/ap-tmux-submit-busy.test.sh - regression: busy pane + pending composer
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ap-tmux-submit-busy.XXXXXX")
trap 'rm -rf "$TMP_ROOT"' EXIT

# Override ap_pane_is_busy for testing: AP_FAKE_PANE_BUSY=1 means busy.
ap_pane_is_busy() {
  [ "${AP_FAKE_PANE_BUSY:-0}" = 1 ]
}
...
test_busy_pane_pending_returns_empty() {
  ...
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" ap_tmux_composer_state "win" > "$vfile" 2>/dev/null
  ...
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" AP_FAKE_SENT="$sent" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 AP_FAKE_PANE_BUSY=1 \
    ap_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  ...
  pass "ap_tmux_submit_enter_core: busy pane + pending composer returns empty (message queued)"
}
...
test_idle_pane_pending_returns_pending() {
  ...
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" AP_FAKE_SENT="$sent" \
    AP_FAKE_SWALLOW="$dir/.swallow" AP_FAKE_PERSIST_SWALLOW=1 AP_FAKE_PANE_BUSY=0 \
    ap_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = pending ] || fail "idle-pane pending should return pending, got '$(cat "$vfile")'"
  pass "ap_tmux_submit_enter_core: idle pane + pending composer stays pending (genuine swallow preserved)"
}
...
test_busy_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" AP_FAKE_SENT="$sent" AP_FAKE_PANE_BUSY=1 \
    ap_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = empty ] || fail "busy-pane with cleared composer should return empty, got '$(cat "$vfile")'"
  pass "ap_tmux_submit_enter_core: busy pane clears composer on first Enter - returns empty"
}
...
test_idle_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" AP_FAKE_SENT="$sent" AP_FAKE_PANE_BUSY=0 \
    ap_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = empty ] || fail "idle-pane with cleared composer should return empty, got '$(cat "$vfile")'"
  pass "ap_tmux_submit_enter_core: idle pane clears composer on first Enter - returns empty as before"
}
...
test_busy_pane_unknown_stays_unknown() {
  ...
  printf '│ > unbounded\n' > "$composer"
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" AP_FAKE_PANE_BUSY=1 \
  ...
  pass "ap_tmux_submit_enter_core: busy conversion is limited to proven pending input"
}
...
test_busy_pane_ambiguous_pending_retries_without_conversion() {
  ...
  pass "ap_tmux_submit_enter_core: pending-unproven retries without busy conversion"
}
...
test_unrecognized_state_skips_busy_conversion() {
  ...
  (
    ...
    # shellcheck disable=SC2329
    ap_pane_is_busy() { touch "$busy_called"; return 0; }
    PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" \
      ap_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  ...
  pass "ap_tmux_submit_enter_core: unrecognized states skip busy conversion"
}
...
test_claude_busy_signature_uses_real_capture_shapes() {
  ...
  pane_busy() {
    PATH="$fakebin:$PATH" AP_FAKE_COMPOSER="$composer" \
      bash -c '. "$1/bin/ap-tmux-lib.sh"; ap_pane_is_busy "$2" "$3"' \
      _ "$ROOT" "$1" "${2:-}"
  }
  ...
  pass "ap_pane_is_busy: Claude spinner is scoped, multi-frame, and backward-compatible"
}
