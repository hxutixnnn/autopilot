#!/usr/bin/env bash
# tests/ap-backend-orca.test.sh - fake-Orca-CLI unit tests for the Orca
...
TMP_ROOT=$(ap_test_tmproot ap-backend-orca-tests)
...
if [ "${1:-}" = status ] && [ "${AP_ORCA_STATUS_RESPONSE:-ready}" != sequence ]; then
...
neutral_ap_root() {  # <dir> -> echoes a minimal root with a quiet guard
  local root="$1/root"
  mkdir -p "$root/bin"
  cat > "$root/bin/ap-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root/bin/ap-guard.sh"
  printf '%s\n' "$root"
}
...
test_capture_reads_terminal_tail_json() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_capture term-123 40' "$ROOT" )
  ...
  pass "ap_backend_orca_capture: parses result.terminal.tail and calls terminal read"
}
...
test_capture_falls_back_to_text_fields() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_capture term-abc 5' "$ROOT" )
  [ "$out" = "plain text output" ] || fail "capture should fall back to result.text, got '$out'"
  pass "ap_backend_orca_capture: falls back to result text fields"
}
...
test_capture_fails_on_orca_error_json() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_capture term-stale 5' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_capture: fails closed on Orca read error JSON"
}
...
test_runtime_check_accepts_ready_orca_status() {
  ...
  printf '{"ok":true,"result":{"runtime":{"reachable":true,"state":"ready"}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" AP_ORCA_STATUS_RESPONSE=sequence \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_runtime_check' "$ROOT" )
  ...
  pass "ap_backend_orca_runtime_check: accepts reachable ready runtime"
}
...
test_runtime_check_refuses_unready_orca_status() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" AP_ORCA_STATUS_RESPONSE=sequence \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_runtime_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_runtime_check: fails closed when runtime is not ready"
}
...
test_send_text_submit_verifies_empty_composer_after_enter() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_submit term-123 "hello pilot" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty on successful Orca send, got '$out'"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-123'$'\x1f''--text'$'\x1f''hello pilot'$'\x1f''--json' \
  ...
  pass "ap_backend_orca_send_text_submit: verifies empty composer after Enter"
}
...
test_send_text_submit_keeps_current_tail_when_limited() {
  ...
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/1.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["noise","│ > hello pilot │"],"limited":true,"oldestCursor":"cursor-old"},"limited":true,"oldestCursor":"cursor-old"}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["╭──╮","│ > │","╰──╯"],"latestCursor":"cursor-new"}}}\n' > "$RESP/4.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/5.out"
  ...
  pass "ap_backend_orca_send_text_submit: preserves current tail when limited reads fetch older cursor text"
}
...
test_send_text_submit_retries_when_composer_stays_pending() {
  ...
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/1.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > hello pilot │"]}}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/4.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > │"]}}}\n' > "$RESP/5.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_submit term-123 "hello pilot" 3 0.01 0.01' "$ROOT" )
  ...
  pass "ap_backend_orca_send_text_submit: retries Enter while composer remains pending"
}
...
test_composer_state_popup_placeholder_fill_is_pending() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_composer_state term-123' "$ROOT" )
  [ "$out" = pending ] || fail "a popup-close-with-placeholder-fill must still read as pending (not yet submitted), got '$out'"
  pass "ap_backend_orca_composer_state: a slash-command popup's argument-hint placeholder still reads pending"
}

# Dead-shell injection safety (task ap-composer-shellglyph-safety): a pane whose
...
test_composer_state_bare_shell_prompt_is_unknown() {
  ...
  printf '{"ok":true,"result":{"terminal":{"tail":["some earlier output","kunchen@mac autopilot $ "]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_composer_state term-123' "$ROOT" )
  [ "$out" = unknown ] || fail "a bare dead-shell prompt (no bordered composer row) must read unknown, got '$out'"
  pass "ap_backend_orca_composer_state: a bare dead-shell prompt reads unknown (unsafe-for-injection), never empty"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_submit term-123 "/compact" 3 0.01 1.2' "$ROOT" )
  ...
  pass "ap_backend_orca_send_text_submit: a slash-command popup's placeholder fill on Enter #1 does not short-circuit as submitted; Enter #2 is retried and lands it"
}

test_send_literal_constructs_non_enter_send() {
  orca_case send-literal
  PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_literal term-123 "typed only"' "$ROOT"
  ...
  pass "ap_backend_orca_send_literal: sends text without submitting"
}
...
test_send_text_submit_reports_send_failed() {
  ...
  printf '1\n' > "$RESP/1.exit"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_submit term-123 "hello" 1 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "failed Orca send should report send-failed, got '$out'"
  pass "ap_backend_orca_send_text_submit: reports send-failed when Orca send fails"
}
...
test_send_helpers_reject_orca_error_json() {
  ...
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_line term-stale "hello"' "$ROOT" 2>&1 )
  status=$?
  ...
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/2.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_literal term-stale "typed"' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "send_literal should fail on Orca ok:false JSON"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/3.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_key term-stale Enter' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "send_key should fail on Orca ok:false JSON"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/4.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_text_submit term-stale "hello" 1 0.01 0.01' "$ROOT" 2>/dev/null )
...
test_send_key_enter_and_interrupt() {
  orca_case send-key
  PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_key term-123 Enter; ap_backend_orca_send_key term-123 C-c' "$ROOT"
  ...
  pass "ap_backend_orca_send_key: Enter maps to empty enter, C-c maps to interrupt"
}
...
test_send_key_refuses_unknown_key() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_key term-123 F12' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_send_key: refuses unsupported keys loudly"
}
...
test_send_key_refuses_escape_until_supported() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_send_key term-123 Escape' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_send_key: refuses Escape instead of mapping it to interrupt"
}
...
test_kill_is_best_effort_close() {
  ...
  printf '1\n' > "$RESP/1.exit"
  PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_kill term-123' "$ROOT"
  ...
  pass "ap_backend_orca_kill: calls terminal close and stays best-effort"
}
...
test_remove_worktree_refuses_empty_id() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_remove_worktree ""' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_remove_worktree: refuses empty worktree ids"
}
...
test_remove_worktree_rejects_orca_error_json() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_remove_worktree wt-gone' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "ap_backend_orca_remove_worktree: fails closed on ok:false JSON"
}
...
test_worktree_path_resolves_id() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_worktree_path wt-123' "$ROOT" )
  ...
  pass "ap_backend_orca_worktree_path: resolves an Orca worktree id to its path"
}
...
test_json_get_ignores_undocumented_terminal_id_shapes() {
  ...
  out=$( printf '{"ok":true,"result":{"id":"term-root-id"}}\n' | \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_json_get terminal-handle' "$ROOT" )
  status=$?
  set +e
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_worktree_create /repo/path ap-task' "$ROOT" )
  wt_id=${out%%$'\t'*}
  wt_path=${out#*$'\t'}
  term=${wt_path#*$'\t'}
  wt_path=${wt_path%%$'\t'*}
  ...
  pass "ap_backend_orca_json_get: ignores undocumented terminal id shapes"
}
...
test_worktree_and_terminal_helpers_parse_json() {
  ...
  term=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_terminal_create wt-123 ap-task' "$ROOT" )
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''create'$'\x1f''--repo'$'\x1f''id:repo-123'$'\x1f''--name'$'\x1f''ap-task'$'\x1f''--no-parent'$'\x1f''--setup'$'\x1f''skip'$'\x1f''--json' \
    "worktree helper did not create an independent no-hook worktree"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-123'$'\x1f''--title'$'\x1f''ap-task'$'\x1f''--json' \
...
test_worktree_create_removes_worktree_when_path_missing() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; ap_backend_orca_worktree_create /repo/path ap-task' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "worktree helper should fail when Orca omits the worktree path"
  assert_contains "$out" "orca worktree create did not return a path for ap-task" \
  ...
  pass "ap_backend_orca_worktree_create: removes created worktree when path is missing"
}
...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  printf '{"ok":false,"error":{"code":"worktree_not_removed","message":"worktree not removed"}}\n' > "$RESP/5.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$ROOT" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
    AP_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" AP_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" claude --backend orca 2>&1 )
  status=$?
  ...
  assert_grep "window=ap-$id" "$state/$id.meta" "preserved pathless metadata missing stable window alias"
  ...
  pass "ap-spawn.sh --backend orca: preserves metadata when pathless cleanup fails"
}
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    ...
    AP_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" AP_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" claude --backend orca 2>&1 )
  expect_code 0 $? "ap-spawn.sh --backend orca should succeed with fake Orca"$'\n'"$out"
  assert_contains "$out" "spawned $id harness=claude kind=flight mode=no-mistakes yolo=off window=ap-$id worktree=$wt" \
    "spawn output missing Orca window/worktree summary"
  assert_grep "backend=orca" "$state/$id.meta" "meta missing backend=orca"
  assert_grep "window=ap-$id" "$state/$id.meta" "meta missing stable Orca window alias"
  ...
  assert_contains "$(cat "$log")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-spawn'$'\x1f''--text'$'\x1f''export GOTMPDIR=/tmp/ap-orcaspawnz1/gotmp'$'\x1f''--enter'$'\x1f''--json' \
  ...
  rm -rf "/tmp/ap-$id"
  pass "ap-spawn.sh --backend orca: reuses implicit terminal, records metadata, launches harness"
}

test_spawn_refuses_orca_copilot_before_home_mutation() {
  local home copilot_home data state config id out status
  id="orcasmz1"
  home="$TMP_ROOT/copilot-refusal-home"
  copilot_home="$TMP_ROOT/copilot-refusal-copilot_home"
  data="$home/data"
  state="$home/state"
  config="$home/config"
  mkdir -p "$data" "$state" "$config" "$copilot_home/bin" "$copilot_home/data" "$copilot_home/state" "$copilot_home/projects"
  printf '%s\n' "$id" > "$copilot_home/.ap-copilot-home"
  printf 'autopilot\n' > "$copilot_home/AGENTS.md"
  printf 'claude\n' > "$config/flight-crew-harness"
  touch "$state/.last-watcher-beat"
  set +e
  out=$( AP_ROOT_OVERRIDE="$ROOT" AP_HOME="$home" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
    AP_PROJECTS_OVERRIDE="$home/projects" AP_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/ap-spawn.sh" "$id" "$copilot_home" claude --backend orca --copilot 2>&1 )
  status=$?
  set +e
  [ "$status" -ne 0 ] || fail "backend=orca --copilot should be refused"
  assert_contains "$out" "backend=orca does not support --copilot spawns yet" \
    "orca copilot refusal should happen at backend selection"
  assert_absent "$copilot_home/config/flight-crew-harness" \
    "orca copilot refusal should not propagate inherited local material into the copilot home"
  pass "ap-spawn.sh --backend orca --copilot: refuses before copilot-home mutation"
}
...
test_spawn_refuses_orca_when_runtime_not_ready() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" AP_ORCA_STATUS_RESPONSE=sequence \
    ...
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" claude --backend orca 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "ap-spawn.sh --backend orca should refuse when Orca runtime is not ready"
  ...
  pass "ap-spawn.sh --backend orca: refuses before mutation when Orca runtime is not ready"
}
...
test_spawn_refuses_orca_nonisolated_worktree() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    ...
    "$ROOT/bin/ap-spawn.sh" "$id" "$proj" claude --backend orca 2>&1 )
  status=$?
  expect_code 1 "$status" "ap-spawn.sh --backend orca should refuse a primary checkout worktree"
  ...
  pass "ap-spawn.sh --backend orca: refuses non-isolated worktrees and closes implicit terminals"
}
...
test_spawn_removes_orca_worktree_when_terminal_create_fails() {
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-terminal-fail'$'\x1f''--title'$'\x1f'"ap-$id"$'\x1f''--json' \
  ...
  pass "ap-spawn.sh --backend orca: removes worktree when terminal creation fails"
}
...
test_spawn_preserves_orca_metadata_when_abort_cleanup_fails() {
  ...
  assert_grep "window=ap-$id" "$state/$id.meta" "preserved metadata missing stable window alias"
  ...
  pass "ap-spawn.sh --backend orca: preserves metadata when abort cleanup fails"
}
...
test_spawn_releases_orca_resources_when_metadata_write_fails() {
  ...
  pass "ap-spawn.sh --backend orca: releases terminal and worktree on later aborts"
}

test_peek_send_and_flight_crew_state_route_through_orca_meta() {
  local wt state id out neutral
  id="orcaiopathz2"
  wt="$TMP_ROOT/io-wt"
  ap_git_init_commit "$wt"
  state="$TMP_ROOT/io-state"; mkdir -p "$state"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-io" "worktree=$wt" "project=$wt" "harness=claude" "kind=recon" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case io-path
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  printf '{"ok":true,"result":{"terminal":{"tail":["ready"]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_SEND_SETTLE=0 \
    "$ROOT/bin/ap-peek.sh" "ap-$id" 10 )
  [ "$out" = ready ] || fail "ap-peek should read through Orca metadata, got '$out'"
  printf '{"ok":true,"result":{"send":{"handle":"term-io","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-io","accepted":true}}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > │"]}}}\n' > "$RESP/4.out"
  PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_HOME="$neutral" AP_STATE_OVERRIDE="$state" AP_SEND_SETTLE=0 \
    "$ROOT/bin/ap-send.sh" "ap-$id" "hello orca"
  printf '{"ok":true,"result":{"terminal":{"tail":["idle prompt"]}}}\n' > "$RESP/5.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$ROOT" AP_STATE_OVERRIDE="$state" "$ROOT/bin/ap-flight-crew-state.sh" "$id" )
  assert_contains "$out" "state: unknown" "flight-crew-state should fall back cleanly for an idle Orca recon"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f''term-io' \
    "peek/flight-crew-state did not read the recorded Orca terminal"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f'"ap-$id" \
    "flight-crew-state should not read the stable Orca alias as a terminal handle"
  ...
  pass "ap-peek/ap-send/ap-flight-crew-state route through backend=orca metadata"
}

test_peek_and_flight_crew_state_fail_closed_on_orca_error_json() {
  ...
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-stale" "worktree=$wt" "project=$wt" "harness=claude" "kind=recon" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case read-error-json
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" "$ROOT/bin/ap-peek.sh" "ap-$id" 10 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "ap-peek should fail when Orca reports a stale terminal"
  assert_contains "$out" "terminal handle stale" "ap-peek should surface the Orca read error message"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/2.out"
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$ROOT" AP_STATE_OVERRIDE="$state" "$ROOT/bin/ap-flight-crew-state.sh" "$id" )
  assert_contains "$out" "state: unknown" "flight-crew-state should not treat an Orca read error as a live endpoint"
  assert_contains "$out" "backend target gone: term-stale" "flight-crew-state should report the stale Orca terminal as gone"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f''term-stale' \
    "ap-peek/ap-flight-crew-state did not read the recorded Orca terminal"
  pass "ap-peek/ap-flight-crew-state: Orca read error JSON fails closed"
}
...
test_target_exists_rejects_orca_error_json() {
  ...
  set +e
  PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/ap-backend.sh"; ap_backend_target_exists orca term-stale ap-task' "$ROOT"
  status=$?
  set -e
  [ "$status" -ne 0 ] || fail "ap_backend_target_exists should reject Orca ok:false read JSON"
  pass "ap_backend_target_exists: Orca ok:false read JSON is not live"
}

test_recon_teardown_removes_orca_worktree_via_helper() {
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-teardown" "worktree=$wt" "project=$proj" \
  ...
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
    "$ROOT/bin/ap-teardown.sh" "$id" 2>&1 )
  rc=$?
  set -e
  expect_code 0 "$rc" "Orca recon teardown should succeed once report exists"$'\n'"$out"
  ...
  pass "ap-teardown.sh backend=orca: recon report gate then helper-backed worktree removal"
}

test_recon_teardown_refuses_orca_id_path_mismatch() {
  ...
  other_wt="$TMP_ROOT/recon-mismatch-other-wt"
  data="$TMP_ROOT/recon-mismatch-data"
  state="$TMP_ROOT/recon-mismatch-state"
  config="$TMP_ROOT/recon-mismatch-config"
  ap_git_worktree "$proj" "$wt" "ap/$id"
  git -C "$proj" worktree add --quiet -b "ap/$id-other" "$other_wt"
  mkdir -p "$data/$id" "$state" "$config"
  printf 'report\n' > "$data/$id/report.md"
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-recon-mismatch" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=recon" "mode=no-mistakes" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-recon-mismatch" \
    "decisions_reviewed=1" "decision_keys="
  orca_case recon-mismatch
  printf '{"ok":true,"result":{"worktree":{"id":"wt-recon-mismatch","path":"%s"}}}\n' "$other_wt" > "$RESP/1.out"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "Orca recon teardown should refuse when id path differs from worktree="
  assert_contains "$out" "not inspected worktree" \
    "mismatched Orca recon worktree path refusal should name the mismatch"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close' \
    "refused mismatched Orca recon teardown should not close terminals"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''rm' \
    "refused mismatched Orca recon teardown should not remove worktrees"
  assert_present "$state/$id.meta" "refused mismatched recon teardown should preserve metadata"
  pass "ap-teardown.sh backend=orca: recon teardown refuses id/path mismatches"
}
...
test_teardown_removes_orca_worktree_when_path_missing() {
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-missing-path" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=recon" "mode=no-mistakes" "yolo=off" \
  ...
  pass "ap-teardown.sh backend=orca: releases terminal/worktree when path is absent"
}
...
test_teardown_preserves_metadata_when_orca_remove_error_json() {
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-remove-error" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=recon" "mode=no-mistakes" "yolo=off" \
  ...
  pass "ap-teardown.sh backend=orca: preserves metadata on remove ok:false JSON"
}

test_recon_teardown_refuses_orca_missing_report_when_path_missing() {
  local proj wt data state config id out rc neutral
  id="orcanoreportz4"
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-missing-report" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=recon" "mode=no-mistakes" "yolo=off" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "Orca recon teardown should refuse without a report even when the path is absent"
  assert_contains "$out" "has no report" "Orca recon teardown should explain the missing report"
  [ ! -s "$LOG" ] || fail "refused Orca recon teardown should not close terminals or remove worktrees"
  assert_present "$state/$id.meta" "refused Orca recon teardown should preserve metadata"
  pass "ap-teardown.sh backend=orca: recon report gate precedes pathless helper cleanup"
}

test_flight_teardown_refuses_orca_missing_worktree_path() {
  local proj wt data state config id out rc neutral
  id="orcashipmissingz8"
  proj="$TMP_ROOT/missing-flight-project"
  wt="$TMP_ROOT/missing-flight-wt"
  data="$TMP_ROOT/missing-flight-data"
  state="$TMP_ROOT/missing-flight-state"
  config="$TMP_ROOT/missing-flight-config"
  ap_git_init_commit "$proj"
  mkdir -p "$data/$id" "$state" "$config"
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-missing-flight" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=flight" "mode=no-mistakes" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-missing-flight"
  orca_case missing-flight-path
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "Orca flight teardown should refuse a missing worktree path"
  assert_contains "$out" "no inspectable git worktree" \
    "Orca flight teardown should explain the fail-closed worktree requirement"
  [ ! -s "$LOG" ] || fail "refused Orca flight teardown should not close terminals or remove worktrees"
  assert_present "$state/$id.meta" "refused Orca flight teardown should preserve metadata"
  pass "ap-teardown.sh backend=orca: flight teardown fails closed when worktree path is missing"
}

test_flight_teardown_removes_orca_worktree_when_id_path_matches() {
  local proj wt data state config id out rc neutral
  id="orcashipmatchz2"
  proj="$TMP_ROOT/flight-match-project"
  wt="$TMP_ROOT/flight-match-wt"
  data="$TMP_ROOT/flight-match-data"
  state="$TMP_ROOT/flight-match-state"
  config="$TMP_ROOT/flight-match-config"
  ap_git_worktree "$proj" "$wt" "ap/$id"
  mkdir -p "$data/$id" "$state" "$config"
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-flight-match" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=flight" "mode=local-only" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-flight-match"
  orca_case flight-match
  printf '{"ok":true,"result":{"worktree":{"id":"wt-flight-match","path":"%s"}}}\n' "$wt" > "$RESP/1.out"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
  ...
  rc=$?
  set -e
  expect_code 0 "$rc" "Orca flight teardown should succeed when the id path matches the inspected worktree"$'\n'"$out"
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''show'$'\x1f''--worktree'$'\x1f''id:wt-flight-match'$'\x1f''--json' \
    "teardown did not resolve the Orca worktree id before removal"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close'$'\x1f''--terminal'$'\x1f''term-flight-match'$'\x1f''--json' \
    "teardown did not close the matched Orca terminal"
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''rm'$'\x1f''--worktree'$'\x1f''id:wt-flight-match'$'\x1f''--force'$'\x1f''--json' \
  ...
  pass "ap-teardown.sh backend=orca: flight teardown requires a matching Orca id path"
}

test_flight_teardown_refuses_orca_unresolvable_worktree_id() {
  local proj wt data state config id out rc neutral
  id="orcashipunresolvedz1"
  proj="$TMP_ROOT/flight-unresolved-project"
  wt="$TMP_ROOT/flight-unresolved-wt"
  data="$TMP_ROOT/flight-unresolved-data"
  state="$TMP_ROOT/flight-unresolved-state"
  config="$TMP_ROOT/flight-unresolved-config"
  ap_git_worktree "$proj" "$wt" "ap/$id"
  mkdir -p "$data/$id" "$state" "$config"
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-flight-unresolved" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=flight" "mode=local-only" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-flight-unresolved"
  orca_case flight-unresolved
  printf '1\n' > "$RESP/1.exit"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "Orca flight teardown should refuse when the worktree id cannot be resolved"
  assert_contains "$out" "cannot resolve Orca worktree id wt-flight-unresolved" \
    "unresolvable Orca worktree id refusal should explain the fail-closed check"
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''show'$'\x1f''--worktree'$'\x1f''id:wt-flight-unresolved'$'\x1f''--json' \
    "teardown did not attempt to resolve the Orca worktree id"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close' \
    "refused unresolved Orca flight teardown should not close terminals"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''rm' \
    "refused unresolved Orca flight teardown should not remove worktrees"
  assert_present "$state/$id.meta" "refused unresolved Orca flight teardown should preserve metadata"
  pass "ap-teardown.sh backend=orca: flight teardown fails closed when id resolution fails"
}

test_flight_teardown_refuses_orca_id_path_mismatch() {
  local proj wt other_wt data state config id out rc neutral
  id="orcashipmismatchz9"
  proj="$TMP_ROOT/flight-mismatch-project"
  wt="$TMP_ROOT/flight-mismatch-wt"
  other_wt="$TMP_ROOT/flight-mismatch-other-wt"
  data="$TMP_ROOT/flight-mismatch-data"
  state="$TMP_ROOT/flight-mismatch-state"
  config="$TMP_ROOT/flight-mismatch-config"
  ap_git_worktree "$proj" "$wt" "ap/$id"
  git -C "$proj" worktree add --quiet -b "ap/$id-other" "$other_wt"
  mkdir -p "$data/$id" "$state" "$config"
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-flight-mismatch" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=flight" "mode=local-only" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-flight-mismatch"
  orca_case flight-mismatch
  printf '{"ok":true,"result":{"worktree":{"id":"wt-flight-mismatch","path":"%s"}}}\n' "$other_wt" > "$RESP/1.out"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_STATE_OVERRIDE="$state" AP_DATA_OVERRIDE="$data" AP_CONFIG_OVERRIDE="$config" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "Orca flight teardown should refuse when the id path differs from worktree="
  assert_contains "$out" "not inspected worktree" \
    "mismatched Orca worktree path refusal should name the mismatch"
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''show'$'\x1f''--worktree'$'\x1f''id:wt-flight-mismatch'$'\x1f''--json' \
    "teardown did not resolve the mismatched Orca worktree id"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close' \
    "refused mismatched Orca flight teardown should not close terminals"
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''rm' \
    "refused mismatched Orca flight teardown should not remove worktrees"
  assert_present "$state/$id.meta" "refused mismatched Orca flight teardown should preserve metadata"
  pass "ap-teardown.sh backend=orca: flight teardown refuses id/path mismatches"
}
...
test_teardown_refuses_orca_missing_worktree_id() {
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "terminal=term-missing-id" "worktree=$wt" "project=$proj" \
  ...
  pass "ap-teardown.sh backend=orca: refuses missing worktree ids before cleanup"
}
...
test_teardown_refuses_orca_worktree_without_terminal_handle() {
  ...
  touch "$state/.last-watcher-beat"
  ap_write_meta "$state/$id.meta" \
    "window=ap-$id" "endpoint_task_id=$id" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=recon" "mode=no-mistakes" "yolo=off" \
  ...
  pass "ap-teardown.sh backend=orca: refuses incomplete worktree-only endpoint metadata before runtime dispatch"
}

test_copilot_force_teardown_removes_orca_child_via_orca() {
  local home copilot_home childproj childwt child_id neutral out rc
  home="$TMP_ROOT/orca-child-parent"
  copilot_home="$TMP_ROOT/orca-child-copilot"
  childproj="$copilot_home/projects/alpha"
  childwt="$TMP_ROOT/orca-child-worktree"
  child_id="orcachildz6"
  mkdir -p "$home/state" "$home/data" "$copilot_home/state" "$copilot_home/projects"
  printf 'domain\n' > "$copilot_home/.ap-copilot-home"
  ap_git_worktree "$childproj" "$childwt" "ap/$child_id"
  ap_write_meta "$home/state/domain.meta" \
    "window=autopilot:ap-domain" "worktree=$copilot_home" "project=$copilot_home" \
    "harness=echo" "kind=copilot" "mode=copilot" "yolo=off" \
    "home=$copilot_home" "projects=alpha"
  printf '%s\n' "- domain - Orca child cleanup (home: $copilot_home; scope: orca cleanup; projects: alpha; added 2026-07-03)" \
    > "$home/data/copilots.md"
  ap_write_meta "$copilot_home/state/$child_id.meta" \
    "window=ap-$child_id" "endpoint_task_id=$child_id" \
    "terminal=term-child-cleanup" "worktree=$childwt" "project=$childproj" \
    "harness=claude" "kind=flight" "mode=no-mistakes" "yolo=off" \
    "backend=orca" "orca_worktree_id=wt-child-cleanup"
  orca_case copilot-child-cleanup
  printf '{"ok":true,"result":{"worktree":{"id":"wt-child-cleanup","path":"%s"}}}\n' "$childwt" > "$RESP/1.out"
  add_tmux_fake "$FB"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    AP_ROOT_OVERRIDE="$neutral" AP_HOME="$home" "$ROOT/bin/ap-teardown.sh" domain --force 2>&1 )
  rc=$?
  set -e
  expect_code 0 "$rc" "forced copilot teardown should remove Orca child work through Orca"$'\n'"$out"
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close'$'\x1f''--terminal'$'\x1f'"ap-$child_id" \
  ...
  pass "ap-teardown.sh --force: removes Orca copilot children through Orca"
}

test_copilot_force_teardown_refuses_orca_child_id_path_mismatch() {
  local home copilot_home childproj childwt other_wt child_id neutral out rc
  home="$TMP_ROOT/orca-child-mismatch-parent"
  copilot_home="$TMP_ROOT/orca-child-mismatch-copilot"
  childproj="$copilot_home/projects/alpha"
  ...
  printf 'domain\n' > "$copilot_home/.ap-copilot-home"
  ap_git_worktree "$childproj" "$childwt" "ap/$child_id"
  git -C "$childproj" worktree add --quiet -b "ap/$child_id-other" "$other_wt"
  ap_write_meta "$home/state/domain.meta" \
    "window=autopilot:ap-domain" "worktree=$copilot_home" "project=$copilot_home" \
    "harness=echo" "kind=copilot" "mode=copilot" "yolo=off" \
    "home=$copilot_home" "projects=alpha"
  ...
  orca_case copilot-child-mismatch
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "forced copilot teardown should refuse mismatched Orca child id/path"
  assert_contains "$out" "not inspected worktree" \
  ...
  assert_present "$home/state/domain.meta" "refused forced copilot teardown should preserve parent metadata"
  pass "ap-teardown.sh --force: refuses Orca child id/path mismatches"
}

test_copilot_force_teardown_refuses_partial_orca_child() {
  local home copilot_home childproj childwt child_id neutral out rc
  home="$TMP_ROOT/orca-partial-child-parent"
  copilot_home="$TMP_ROOT/orca-partial-child-copilot"
  childproj="$copilot_home/projects/alpha"
  ...
  ap_write_meta "$home/state/domain.meta" \
    ...
    "harness=echo" "kind=copilot" "mode=copilot" "yolo=off" \
    "home=$copilot_home" "projects=alpha"
  printf '%s\n' "- domain - Orca partial child cleanup (home: $copilot_home; scope: orca cleanup; projects: alpha; added 2026-07-03)" \
    > "$home/data/copilots.md"
  ap_write_meta "$copilot_home/state/$child_id.meta" \
    "window=ap-$child_id" "endpoint_task_id=$child_id" \
  ...
  orca_case copilot-partial-child-cleanup
  add_tmux_fake "$FB"
  neutral=$(neutral_ap_root "$CASE_DIR/neutral")
  set +e
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "forced copilot teardown accepted a child with no terminal identity"
  ...
  assert_present "$copilot_home/state/$child_id.meta" "partial child refusal removed child metadata"
  pass "ap-teardown.sh --force: refuses partial Orca copilot children before runtime dispatch"
}
...
test_dispatcher_sources_orca_and_routes_primitives() {
  ...
  out=$( PATH="$FB:$PATH" AP_ORCA_LOG="$LOG" AP_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/ap-backend.sh"; ap_backend_validate orca; ap_backend_capture orca term-123 9' "$ROOT" )
  [ "$out" = "via dispatch" ] || fail "dispatcher should route capture to the Orca adapter, got '$out'"
  pass "ap-backend dispatcher: accepts orca and routes capture through bin/backends/orca.sh"
}
