...
PROOF="$ROOT/bin/ap-test-isolation-proof.sh"
RUNNER="$ROOT/bin/ap-test-run.sh"

assert_present "$PROOF" "bin/ap-test-isolation-proof.sh is missing"
[ -x "$PROOF" ] || fail "bin/ap-test-isolation-proof.sh must be executable"
...
test_candidates_exclude_serial_classes() {
  ...
  listed=$("$PROOF" --list)
  for banned in \
    tests/ap-test-isolation-proof.test.sh \
    tests/ap-backend-tmux-smoke.test.sh \
    tests/ap-watcher-lock.test.sh \
    tests/ap-wake-queue.test.sh \
    tests/ap-backend-herdr-smoke.test.sh \
    tests/ap-afk-inject-e2e.test.sh \
    tests/ap-pi-primary-live-e2e.test.sh \
    tests/ap-pr-check-security.test.sh \
    tests/ap-backend-cmux-smoke.test.sh; do
...
test_extra_hermetic_candidates_present() {
  ...
  for want in \
    ...
    tests/ap-review-diff.test.sh; do
...
test_list_exclusions_documents_reasons() {
  ...
  printf '%s\n' "$out" | grep -Fq 'ap-watcher-lock.test.sh' \
    || fail "exclusions must document watcher-lock serial reason"
  printf '%s\n' "$out" | grep -Fq 'ap-backend-herdr-smoke.test.sh' \
...
test_family_map_labels_this_contract() {
  ...
  printf '%s\n' "$fam" | grep -Fq 'tests/ap-test-isolation-proof.test.sh' \
    || fail "ap-test-isolation-proof.test.sh must map to pure-contract-unit"
