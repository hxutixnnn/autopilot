#!/usr/bin/env bash
# Behavior tests for ap-bootstrap.sh reporting and session-start clone refresh bounds.
#
...
# all is well. autopilot consumes the exact 'MISSING: treehouse (install: ...)',
...
TMP_ROOT=$(ap_test_tmproot ap-bootstrap-tests)
export AP_BACKEND_CMUX_BUNDLE_BIN="$TMP_ROOT/no-bundled-cmux"
...
# ap_backend_name and flip a default-backend case onto a non-tmux backend. Unset
...
# treehouse's `get --help` advertises --lease only when AP_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${AP_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${AP_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.31.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${AP_FAKE_QUOTA_AXI_VERSION:-0.1.16}"
  exit 0
fi
exit 0
SH
...
make_fake_fleet_sync_root() {
  ...
  cat > "$fake_root/bin/ap-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
[ -z "${AP_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] || : > "$AP_FAKE_FLEET_SYNC_STARTED_MARKER"
printf '%s\n' 'alpha: synced'
printf '%s\n' 'beta: skipped: no origin remote'
exec perl -e 'sleep 300'
SH
  chmod +x "$fake_root/bin/ap-fleet-sync.sh"
  printf '%s\n' "$fake_root"
}
...
run_bootstrap_timeout_case() {
  ...
  (
    ...
    git() {
      local tries
      if [ "${AP_FAKE_GIT_WAIT_FOR_FLEET_START:-}" = 1 ] && [ -n "${AP_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ]; then
        tries=0
        while [ "$tries" -lt 5 ] && [ ! -e "$AP_FAKE_FLEET_SYNC_STARTED_MARKER" ]; do
          command sleep 0.01
          tries=$((tries + 1))
        done
      fi
      if [ -n "${AP_FAKE_GIT_SYNC_STARTED_RECORD:-}" ] && [ -n "${AP_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] && [ -e "$AP_FAKE_FLEET_SYNC_STARTED_MARKER" ]; then
        printf '%s\n' "$*" >> "$AP_FAKE_GIT_SYNC_STARTED_RECORD"
      fi
      command git "$@"
    }
    export -f sleep
    ...
    if [ "$override" = __unset__ ]; then
      PATH="$fakebin:$BASE_PATH" AP_HOME="$home" AP_ROOT_OVERRIDE="$fake_root" \
        AP_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        AP_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        AP_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null
    else
      PATH="$fakebin:$BASE_PATH" AP_HOME="$home" AP_ROOT_OVERRIDE="$fake_root" \
        AP_FLEET_SYNC_BOOTSTRAP_TIMEOUT="$override" \
        AP_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        AP_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        AP_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh" 2>/dev/null
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks quota backend mode expect notcontains; do
    ...
    # AP_ROOT_OVERRIDE points the worktree-tangle check at the non-git home dir so
    ...
    out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
      AP_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/ap-bootstrap.sh")
    case "$mode" in
      empty)
...
test_no_mistakes_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    add_tasks_axi "$fakebin" "0.1.1"
    out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
      AP_FAKE_TREEHOUSE_LEASE_HELP=1 AP_FAKE_NO_MISTAKES_VERSION="$version" "$ROOT/bin/ap-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
# produced a pilot-facing "log in" claim for a candidate that never read it. A
...
test_quota_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    add_tasks_axi "$fakebin" "0.1.1"
    out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
      AP_FAKE_TREEHOUSE_LEASE_HELP=1 AP_FAKE_QUOTA_AXI_VERSION="$version" "$ROOT/bin/ap-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$bash_env" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
    AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh")
...
test_herdr_install_requires_manual_action() {
  local out status
  out=$("$ROOT/bin/ap-bootstrap.sh" install herdr 2>&1)
  status=$?
...
test_cmux_bundled_cli_satisfies_dependency() {
  ...
  fakebin=$(make_fake_toolchain_no_tmux "$case_dir")
  ap_fake_exit0 "$case_dir/bundle" cmux
  bundle="$case_dir/bundle/cmux"
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
    AP_BACKEND_CMUX_BUNDLE_BIN="$bundle" AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh")
...
test_treehouse_lease_check_follows_resolved_backend() {
  ...
  fakebin=$(make_fake_toolchain "$case_dir")
  rm -f "$fakebin/tmux"
  ap_fake_exit0 "$fakebin" orca
  # AP_FAKE_TREEHOUSE_LEASE_HELP unset: the fake treehouse advertises NO --lease.
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
    "$ROOT/bin/ap-bootstrap.sh")
...
test_fleet_sync_timeout_explicit_override_wins() {
  ...
  pass "bootstrap preserves AP_FLEET_SYNC_BOOTSTRAP_TIMEOUT as an explicit override"
}
...
make_routine_bootstrap_fixture() {
  local case_dir=$1 fakebin root home copilot c1
  root="$case_dir/root"
  home="$case_dir/home"
  copilot="$case_dir/copilot"
  ap_git_identity
  mkdir -p "$home/config" "$home/state"
  printf '%s\n' codex > "$home/config/flight-crew-harness"
  printf '%s\n' '{"rules":[{"when":"normal work","use":{"harness":"codex"}}],"default":{"harness":"claude","effort":"low"}}' \
    > "$home/config/flight-crew-dispatch.json"
  ...
  printf '%s\n' 'echo ok' > "$root/bin/ap-spawn.sh"
  printf '%s\n' 'skill' > "$root/.agents/skills/example.md"
  git -C "$root" add -A
  git -C "$root" commit -qm initial
  c1=$(git -C "$root" rev-parse HEAD)
  git -C "$root" worktree add -q --detach "$copilot" "$c1"
...
run_routine_bootstrap_fixture() {
  ...
  PATH="$fakebin:$BASE_PATH" AP_BACKEND=tmux AP_HOME="$home" AP_ROOT_OVERRIDE="$root" \
    AP_FAKE_TREEHOUSE_LEASE_HELP=1 \
    "$shell" "$ROOT/bin/ap-bootstrap.sh"
}
...
test_flight_crew_dispatch_active_rules_are_verbose_bootstrap_info() {
  local case_dir fakebin out expect
  case_dir="$TMP_ROOT/dispatch-active"
  mkdir -p "$case_dir/home/config"
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  printf '%s\n' '{"rules":[{"when":"fresh news","use":{"harness":"grok"},"why":"current context"},{"when":"big feature","use":[{"harness":"claude","model":"claude-sonnet-5","effort":"high"},{"harness":"codex","model":"gpt-5.5","effort":"high"}]},{"when":"legacy feature","use":[{"harness":"claude"},{"harness":"codex"}],"select":"quota-balanced"}],"default":[{"harness":"pi","model":"anthropic/claude-sonnet-5","effort":"high"},{"harness":"grok","model":"grok-4.5","effort":"high"}]}' > "$case_dir/home/config/flight-crew-dispatch.json"
  fakebin=$(make_fake_toolchain "$case_dir")
  add_real_jq "$fakebin"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
    AP_BOOTSTRAP_VERBOSE_FACTS=1 AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh")

  expect=$'BOOTSTRAP_INFO: flight crew dispatch active config/flight-crew-dispatch.json\nBOOTSTRAP_INFO: flight crew dispatch rule: fresh news -> grok\nBOOTSTRAP_INFO: flight crew dispatch rule: big feature -> quota-balanced[claude/claude-sonnet-5/high, codex/gpt-5.5/high]\nBOOTSTRAP_INFO: flight crew dispatch rule: legacy feature -> quota-balanced[claude, codex]\nBOOTSTRAP_INFO: flight crew dispatch default: quota-balanced[pi/anthropic/claude-sonnet-5/high, grok/grok-4.5/high]'
  [ "$out" = "$expect" ] || fail "active dispatch verbose info block mismatch"$'\n'"expected: $expect"$'\n'"actual:   $out"
  pass "bootstrap surfaces active flight-crew-dispatch rules only as verbose BOOTSTRAP_INFO"
}

test_flight_crew_dispatch_validation() {
  ...
  while IFS='^' read -r label body mode expect; do
    ...
    mkdir -p "$case_dir/home/config"
    printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
    printf '%s\n' "$body" > "$case_dir/home/config/flight-crew-dispatch.json"
    fakebin=$(make_fake_toolchain "$case_dir")
    add_real_jq "$fakebin"
    out=$(PATH="$fakebin:$BASE_PATH" AP_HOME="$case_dir/home" AP_ROOT_OVERRIDE="$case_dir/home" \
      AP_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/ap-bootstrap.sh")
...
malformed dispatch config is flagged^{"rules":[^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - malformed JSON
unverified dispatch harness is flagged^{"rules":[{"when":"anything","use":{"harness":"spaceship"}}],"default":{"harness":"codex"}}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - unverified harness: spaceship
unsupported codex max effort is flagged^{"rules":[{"when":"big feature","use":{"harness":"codex","model":"gpt-5","effort":"max"}}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: codex:max
unsupported grok max effort is flagged^{"rules":[{"when":"deep current work","use":{"harness":"grok","model":"grok-4","effort":"max"}}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: grok:max
unsupported grok xhigh effort is flagged^{"rules":[{"when":"deep current work","use":{"harness":"grok","model":"grok-4","effort":"xhigh"}}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: grok:xhigh
...
unsupported opencode effort is flagged^{"rules":[{"when":"opencode work","use":{"harness":"opencode","model":"anthropic/claude-sonnet-4-5","effort":"high"}}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: opencode:high
kimi model profile is accepted^{"rules":[{"when":"kimi work","use":{"harness":"kimi","model":"kimi-code/k3"}}]}^empty^
unsupported kimi effort is flagged^{"rules":[{"when":"kimi work","use":{"harness":"kimi","model":"kimi-code/k3","effort":"high"}}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: kimi:high
...
empty array use is flagged^{"rules":[{"when":"big feature","use":[]}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - each rule needs at least one use profile
array profile without harness is flagged^{"rules":[{"when":"big feature","use":[{"model":"gpt-5.5"}]}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - each use profile needs harness
array profile with malformed model is flagged^{"rules":[{"when":"big feature","use":[{"harness":"codex","model":5}]}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - use profile model and effort must be non-empty strings when present
unknown select is flagged^{"rules":[{"when":"big feature","use":[{"harness":"claude"},{"harness":"codex"}],"select":"mystery"}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - unknown select: mystery
array profile unsupported effort is flagged^{"rules":[{"when":"big feature","use":[{"harness":"codex","effort":"max"}]}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - invalid effort: codex:max
empty default array is flagged^{"default":[]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - default needs at least one profile
non-object default array entry is flagged^{"default":["codex"]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - each default profile must be an object
default array profile without harness is flagged^{"default":[{"model":"gpt-5.5"}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - each default profile needs harness
default array malformed effort is flagged^{"default":[{"harness":"codex","effort":3}]}^exact^FLIGHT_CREW_DISPATCH: invalid config/flight-crew-dispatch.json - default profile model and effort must be non-empty strings when present
ROWS
  pass "bootstrap validates flight-crew-dispatch.json and reports malformed or unverified configs"
}
