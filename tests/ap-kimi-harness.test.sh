#!/usr/bin/env bash
# Behavior tests for the verified Kimi Code CLI flight crew member adapter.
set -u
...
KIMI_HOOK="$ROOT/bin/ap-kimi-turnend-hook.sh"
TMP_ROOT=$(ap_test_tmproot ap-kimi-harness)
KIMI_RUNTIME_TASK_TMP=
PYTHON_BIN=
for python_candidate in python3 python3.14 python3.13 python3.12 python3.11; do
  candidate_path=$(command -v "$python_candidate" 2>/dev/null || true)
  [ -n "$candidate_path" ] || continue
  "$candidate_path" -c 'import tomllib' >/dev/null 2>&1 || continue
  PYTHON_BIN=$candidate_path
  break
done
[ -n "$PYTHON_BIN" ] || fail "test needs Python 3 with tomllib"
PYTHON_SHIM_DIR="$TMP_ROOT/python-bin"
mkdir -p "$PYTHON_SHIM_DIR"
ln -s "$PYTHON_BIN" "$PYTHON_SHIM_DIR/python3"
PATH="$PYTHON_SHIM_DIR:$PATH"
export PATH
JQ_BIN=$(command -v jq) || fail "test needs jq"
BASE_PATH=${AP_TEST_BASE_PATH:-$PYTHON_SHIM_DIR:/usr/bin:/bin:/usr/sbin:/sbin}
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$AP_FAKE_TMUX_CALL_LOG"
state=$(cat "$AP_FAKE_KIMI_STATE" 2>/dev/null || true)
fake_screen() {
  case "$state" in
    ready)
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ -n "$literal" ]; then
      case "$literal" in
        *' --auto')
          printf '%s\n' "$literal" >> "$AP_FAKE_LAUNCH_LOG"
          printf 'launched\n' > "$AP_FAKE_KIMI_STATE"
          ;;
        *)
          printf '%s\n' "$literal" >> "$AP_FAKE_POINTER_LOG"
    ...
    case " $* " in
      *' Enter '*)
        case "$state" in
          launched)
            if [ "${AP_FAKE_KIMI_READY:-yes}" = yes ]; then
              printf 'ready\n' > "$AP_FAKE_KIMI_STATE"
            fi
            ;;
          pointer-typed)
            if [ "${AP_FAKE_KIMI_DELIVERY:-yes}" = yes ]; then
              if [ "${AP_FAKE_KIMI_SWALLOW_FIRST:-no}" = yes ] \
                 && [ ! -f "$AP_FAKE_KIMI_SWALLOWED" ]; then
                : > "$AP_FAKE_KIMI_SWALLOWED"
              else
                printf 'delivered\n' > "$AP_FAKE_KIMI_STATE"
              fi
            else
...
make_spawn_case() {
  ...
  printf 'kimi\n' > "$home/config/flight-crew-harness"
...
run_spawn() {
  ...
  shift 6
  HOME="$home" AP_ROOT_OVERRIDE='' AP_HOME="$home" \
    ...
    AP_FAKE_LAUNCH_LOG="$case_dir/launch.log" \
    AP_FAKE_POINTER_LOG="$case_dir/pointer.log" \
    AP_FAKE_KIMI_STATE="$case_dir/kimi.state" \
    AP_FAKE_KIMI_SWALLOWED="$case_dir/kimi.swallowed" \
    AP_FAKE_KIMI_SWALLOW_FIRST="${AP_FAKE_KIMI_SWALLOW_FIRST:-no}" \
    AP_FAKE_TMUX_CALL_LOG="$case_dir/tmux-calls.log" \
    AP_FAKE_BRIEF_REAL="$(cd "$home/data/$id" && pwd -P)/brief.md" \
    AP_KIMI_READY_POLLS=2 AP_KIMI_DELIVERY_POLLS=2 AP_KIMI_POLL_INTERVAL=0 \
...
test_kimi_launch_then_send_is_verified() {
  ...
  out=$(AP_FAKE_KIMI_SWALLOW_FIRST=yes run_spawn \
  ...
  assert_grep 'BEGIN AUTOPILOT KIMI TURN-END HOOK' "$HOME_DIR/.kimi-code/config.toml" \
    "kimi spawn did not install its guarded global hook region"
  assert_grep 'token=' "$WT_DIR/.ap-kimi-turnend" "kimi spawn did not write its token pointer"
  assert_present "$HOME_DIR/state/$id.kimi-turnend-token" "kimi spawn did not record its token"
  pass "ap-spawn: kimi launches, delivers its brief, and registers a guarded turn-end token"
}
...
test_kimi_hook_install_is_surgical_idempotent_and_removable() {
  ...
  cat > "$config" <<'EOF'
# Pilot's leading comment stays exactly here.
...
EOF
  ...
  count=$(grep -c '^# BEGIN AUTOPILOT KIMI TURN-END HOOK' "$config")
  [ "$count" -eq 1 ] || fail "idempotent install left $count Autopilot regions"

  HOME="$home" "$KIMI_HOOK" remove || fail "Kimi hook removal failed"
  cp "$config" "$stripped"
  cmp -s "$original" "$stripped" \
    || fail "config with the Autopilot region excised was not byte-identical to the original"
  assert_absent "$home/.kimi-code/ap-turn-end.sh" "removal left the Autopilot hook script"
  assert_absent "$home/.kimi-code/ap-turn-end.d" "removal left the Autopilot registry"
...
test_kimi_hook_remove_preserves_owned_newline_boundary() {
  ...
  cmp -s "$expected" "$config" \
    || fail "removal did not preserve appended pilot config on its own line"
  "$PYTHON_BIN" - "$config" <<'PY' || fail "config with appended pilot TOML did not parse after removal"
import sys
import tomllib
...
test_kimi_hook_fails_closed_on_missing_malformed_or_partial_config() {
  ...
  assert_absent "$missing/.kimi-code/ap-turn-end.sh" "missing config refusal wrote the hook script"
  ...
  assert_absent "$malformed/.kimi-code/ap-turn-end.sh" "malformed config refusal wrote the hook script"

  printf '# BEGIN AUTOPILOT KIMI TURN-END HOOK\n' > "$partial/.kimi-code/config.toml"
  cp "$partial/.kimi-code/config.toml" "$partial/before"
  rc=0
  out=$(HOME="$partial" "$KIMI_HOOK" install 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "partial Autopilot marker was accepted"
...
test_kimi_hook_install_refuses_without_jq() {
  ...
  assert_absent "$home/.kimi-code/ap-turn-end.sh" "missing-jq refusal wrote the hook script"
  assert_absent "$home/.kimi-code/ap-turn-end.d" "missing-jq refusal wrote the registry"
...
test_kimi_hook_is_silent_and_requires_registered_workspace_token() {
  ...
  hook="$HOME_DIR/.kimi-code/ap-turn-end.sh"
  target="$HOME_DIR/state/$id.turn-ended"
  token=$(sed -n 's/^token=//p' "$WT_DIR/.ap-kimi-turnend")
  assert_present "$HOME_DIR/.kimi-code/ap-turn-end.d/$token" "Kimi registry token is missing"
  ...
  printf 'token=%s\n' "$token" > "$WT_DIR/.ap-kimi-turnend"
  out=$(printf '{"hook_event_name":"Stop","session_id":"flight crew","cwd":"%s","stop_hook_active":false}\n' "$WT_DIR" \
    | HOME="$HOME_DIR" bash "$hook" 2>&1)
  rc=$?
  ...
  rm "$target"
  fakebin=$(ap_fakebin "$CASE_DIR/no-jq")
  ln -s "$(command -v bash)" "$fakebin/bash"
  out=$(printf '{"hook_event_name":"Stop","session_id":"flight crew","cwd":"%s","stop_hook_active":false}\n' "$WT_DIR" \
  ...
  pass "Kimi hook stays silent and inert without an Autopilot registry token"
}
...
test_kimi_spawn_refuses_unsafe_global_config_before_pane_creation() {
  ...
  pass "ap-spawn: unsafe Kimi global config refuses before pane creation"
}
...
test_kimi_teardown_removes_pointer_and_registry_token() {
  ...
  HOME="$HOME_DIR" AP_ROOT_OVERRIDE="$ROOT" AP_HOME="$HOME_DIR" \
    ...
    AP_SPAWN_NO_GUARD=1 PATH="$FAKEBIN_DIR:$BASE_PATH" \
    "$TEARDOWN" "$id" --force >/dev/null 2>&1 || fail "Kimi teardown failed"
  assert_absent "$WT_DIR/.ap-kimi-turnend" "Kimi token pointer survived teardown"
  assert_absent "$HOME_DIR/.kimi-code/ap-turn-end.d/$token" "Kimi registry token survived teardown"
  assert_absent "$HOME_DIR/state/$id.kimi-turnend-token" "Kimi token state survived teardown"
  pass "ap-teardown: Kimi task pointer and registry token are removed"
}
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  ap_fake_exit0 "$(dirname "$fallback")" kimi
  out=$(run_spawn "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id")
  rc=$?
  ...
  pass "ap-spawn: Kimi fallback expands the active HOME"
}
...
test_kimi_missing_binary_refuses_before_pane_creation() {
  ...
  pass "ap-spawn: missing Kimi executable refuses before pane creation"
}
...
test_kimi_unconfirmed_delivery_fails_loudly() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(AP_FAKE_KIMI_DELIVERY=no run_spawn \
  ...
  pass "ap-spawn: kimi treats a silent pointer drop as a failed spawn"
}
...
test_kimi_readiness_gate_precedes_pointer() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(AP_FAKE_KIMI_READY=no run_spawn \
    "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id") || rc=$?
  ...
  pass "ap-spawn: kimi never sends the brief pointer before an observable ready signal"
}
...
SH
  ...
  out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh")
  [ "$out" = kimi ] || fail "kimi ancestry detection returned '$out'"
  out=$(CLAUDECODE=1 PATH="$fakebin:$BASE_PATH" AP_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/ap-harness.sh")
  [ "$out" = claude ] || fail "verified env-marker precedence changed, got '$out'"
  pass "ap-harness: markerless kimi is detected by ancestry after env-marker precedence"
}

test_kimi_session_lock_identity() {
  local home fakebin out
  home="$TMP_ROOT/session-lock-home"
  fakebin=$(ap_fakebin "$TMP_ROOT/session-lock-fake")
  mkdir -p "$home/state"
  cat > "$fakebin/ps" <<'SH'
#!/usr/bin/env bash
case "$*" in
...
SH
  ...
  AP_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/ap-lock.sh" \
    || fail "ap-lock did not acquire from Kimi ancestry"
  case "$(cat "$home/state/.lock")" in
    ''|*[!0-9]*) fail "ap-lock did not record the Kimi harness ancestor" ;;
  esac
  printf '%s\n' "$$" > "$home/state/.lock"
  out=$(AP_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/ap-lock.sh" status)
  assert_contains "$out" "lock: held by live harness pid" \
    "ap-lock did not recognize Kimi as a live holder"
  pass "ap-lock recognizes Kimi ancestry and live lock holders"
}

test_kimi_busy_signature_is_scoped_to_spinner_lines() {
  local capture
  # shellcheck source=/dev/null
  . "$ROOT/bin/ap-tmux-lib.sh"
  unset AP_BUSY_REGEX
  ...
  for phase in 🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘; do
    ...
    ap_pane_is_busy fake kimi || fail "Kimi spinner phase $phase was not recognized as busy"
  done
  printf 'ordinary response ending with 🌕\n│ > │\n' > "$capture"
  if ap_pane_is_busy fake kimi; then
  ...
  if ap_pane_is_busy fake codex; then
...
test_watcher_never_classifies_kimi_from_its_spinner() (
  ...
  unset AP_BUSY_REGEX
  AP_HOME="$TMP_ROOT/watch-home"
  AP_STATE_OVERRIDE="$state"
  export AP_HOME AP_STATE_OVERRIDE
  # shellcheck source=/dev/null
  . "$ROOT/bin/ap-watch.sh"
  ...
  if window_is_busy fake "$busy_capture"; then
    fail "ap-watch classified a Kimi task busy from its spinner instead of unknown"
  fi
  [ "$(ap_busy_classify tmux fake kimi kimi-watch "$state" "$busy_capture")" = "unknown kimi-unverified" ] \
    || fail "a Kimi task must classify unknown kimi-unverified"
  printf 'window=fake\nharness=codex\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
    fail "ap-watch applied Kimi's spinner to a recorded Codex task"
  fi
  printf 'window=fake\nharness=grok\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
  ...
  pass "ap-watch classifies Kimi as unknown rather than from its spinner, and Grok's fallback stays isolated"
)

test_kimi_bordered_prompt_needs_no_override() {
  local out
  # shellcheck source=/dev/null
  . "$ROOT/bin/ap-composer-lib.sh"
  out=$(ap_composer_classify_content 1 '>')
  [ "$out" = empty ] || fail "kimi's bordered bare > composer should read empty, got '$out'"
  out=$(ap_composer_classify_content 0 '>')
