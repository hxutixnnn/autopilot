#!/usr/bin/env bash
# tests/ap-backlog-handoff.test.sh - full item-block handoff (header + indented body).
#
# The happy single-line path and broad safety refusals live in the copilot
...
TMP_ROOT=$(ap_test_tmproot ap-backlog-handoff)

setup_homes() {
  local home=$1 copilot_home=$2 id=${3:-design}
  mkdir -p "$home/data" "$home/state"
  seed_copilot_home_marker "$copilot_home" "$id"
  local sub_abs
  sub_abs=$(cd "$copilot_home" && pwd -P)
  printf -- '- %s - feature work (home: %s; scope: feature work; projects: alpha; added 2026-07-09)\n' \
    "$id" "$sub_abs" > "$home/data/copilots.md"
}
...
EOF
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design body-item >/dev/null \
...
EOF
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design section-tail >/dev/null \
...
test_body_moves_when_last_lines_of_file() {
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design eof-item >/dev/null \
...
test_eof_body_before_seeded_destination_section_keeps_boundary() {
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design seeded-eof-item >/dev/null \
...
test_untouched_eof_line_preserves_terminator() {
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design move-item >/dev/null \
...
EOF
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design idem-item >/dev/null \
  ...
  local out
  out=$(AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design idem-item 2>&1) \
  ...
  [ "$dest_after" = "$(cat "$sub/data/backlog.md")" ] \
    || fail "idempotent re-run mutated the copilot backlog"
...
## Queued
- [ ] resident-item - remains in the copilot backlog (repo: alpha)
  resident body
EOF
  ...
  if out=$(AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design malformed-body 2>&1); then
  ...
  cmp -s "$destination_before" "$sub/data/backlog.md" \
    || fail "noncanonical-continuation refusal changed the copilot backlog"
...
test_indented_heading_is_not_section_boundary() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] ha-codex-fast-default-4e - harness default work (repo: autopilot)
...
- [ ] next-item - after the trap (repo: autopilot)
EOF

  local expected_block
  expected_block=$(extract_item_block "$home/data/backlog.md" ha-codex-fast-default-4e)

  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design ha-codex-fast-default-4e >/dev/null \
...
EOF
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design multi-para >/dev/null \
  ...
  dest_after=$(cat "$sub/data/backlog.md")
  local out
  out=$(AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" design multi-para 2>&1) \
...
EOF
  ...
  AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" "$id" paren-item >/dev/null \
...
EOF
  ...
  local out rc=0
  out=$(AP_HOME="$home" "$ROOT/bin/ap-backlog-handoff.sh" "$id" orphan-item 2>&1) || rc=$?
  ...
  [ ! -f "$sub/data/backlog.md" ] || ! grep -q 'orphan-item' "$sub/data/backlog.md" 2>/dev/null \
    || fail "item appeared in copilot backlog despite missing home"
