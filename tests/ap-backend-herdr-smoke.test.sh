#!/usr/bin/env bash
# tests/ap-backend-herdr-smoke.test.sh - real herdr smoke test for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/ap-backend-design-d7 (herdr-addendum.md), extended for the P3
# workspace-per-home pass (AGENTS.md task herdr-copilot-spaces-k4). Mirrors
# tests/ap-backend-tmux-smoke.test.sh's structure: every other suite fakes the
...
# pilot's real herdr usage. Skips cleanly when herdr (or jq) is not
...
# that command killed the pilot's live default herdr server twice in
...
SESSION="ap-lab-backend-smoke-$$"
export HERDR_SESSION="$SESSION"
COPILOT_SCRATCH=
cleanup_all() {
  [ -n "$COPILOT_SCRATCH" ] && rm -rf "$COPILOT_SCRATCH"
...
# ap_backend_herdr_container_ensure now echoes
...
# single tab character; only ap_backend_herdr_create_task is ever allowed to
# act on the seeded tab id, and only for the container that just created it.
CONTAINER_RAW=$(ap_backend_herdr_container_ensure /tmp) || fail "container_ensure failed"
...
pass "real herdr: container_ensure starts the isolated session's server, creates the autopilot workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
# a tab as prunable.
CONTAINER2_RAW=$(ap_backend_herdr_container_ensure /tmp) || fail "second container_ensure failed"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing autopilot workspace, reports no seeded default tab on adoption)"

# --- create_task + duplicate refusal + default-tab prune ---------------------

LABEL="ap-smoke1"
TASK_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "$LABEL" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# exactly one clean ap-<id> task tab.
...
pass "real herdr: create_task prunes the freshly-created workspace's seeded default tab, leaving exactly one clean ap-<id> task tab"
...
# server restart, and a restored ap-<id> task tab comes back a HUSK: a dead
...
LIVE_DUP_LABEL="ap-smoke-livedup"
LIVE_DUP_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp) || fail "could not create the live-duplicate scenario's tab"
...
herdr pane report-agent "$LIVE_DUP_PANE_ID" --source ap-smoke-test --agent ap-smoke-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the live-duplicate scenario's pane"
if ap_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp >/dev/null 2>&1; then
...
ap_backend_herdr_kill "$SESSION:$LIVE_DUP_PANE_ID"

# 2. A husk (no registered agent at all - the restored-plain-shell shape)
#    must be CLOSED AND REPLACED instead of refused.
HUSK_LABEL="ap-smoke-husk1"
HUSK_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) || fail "could not create the husk-simulation tab"
...
REPLACED_IDS=$(ap_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) \
...
ap_backend_herdr_kill "$SESSION:$NEW_HUSK_PANE_ID"

# --- workspace-per-home: a copilot-shaped home gets its OWN space --------
# (docs/herdr-backend.md "Task container shape", AGENTS.md task
# herdr-copilot-spaces-k4). Reuses this suite's own isolated $SESSION - a SECOND,
...
COPILOT_SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/ap-herdr-smoke-copilot.XXXXXX")
COPILOT_HOME="$COPILOT_SCRATCH/copilot-home"
mkdir -p "$COPILOT_HOME"
printf 'smoketest-copilot1\n' > "$COPILOT_HOME/.ap-copilot-home"

COPILOT_CONTAINER_RAW=$(AP_HOME="$COPILOT_HOME" ap_backend_herdr_container_ensure /tmp) || fail "copilot-shaped container_ensure failed"
COPILOT_CONTAINER=${COPILOT_CONTAINER_RAW%%$'\t'*}
COPILOT_SEEDED_TAB_ID=${COPILOT_CONTAINER_RAW#*$'\t'}
case "$COPILOT_CONTAINER" in
  "$SESSION":w*) : ;;
  *) fail "copilot container_ensure returned an unexpected shape: $COPILOT_CONTAINER" ;;
esac
[ "$COPILOT_CONTAINER" != "$CONTAINER" ] || fail "a copilot-shaped home must get a DIFFERENT workspace than the primary's, got the same: $COPILOT_CONTAINER"
[ -n "$COPILOT_SEEDED_TAB_ID" ] || fail "the copilot-shaped home's container_ensure must CREATE its own workspace and report its seeded default tab id"
pass "real herdr: a copilot-shaped home (.ap-copilot-home) gets its OWN herdr workspace, distinct from the primary's, in the SAME session"

COPILOT_WSID=${COPILOT_CONTAINER#*:}
COPILOT_LABEL_REAL=$(herdr workspace list --session "$SESSION" 2>&1 | jq -r --arg id "$COPILOT_WSID" '.result.workspaces[]? | select(.workspace_id == $id) | .label')
[ "$COPILOT_LABEL_REAL" = "copilot-smoketest-copilot1" ] || fail "the copilot workspace's real herdr label should be copilot-smoketest-copilot1, got '$COPILOT_LABEL_REAL'"
pass "real herdr: the copilot-shaped home's workspace is labeled copilot-<copilot-id> in herdr itself"

COPILOT_TASK_LABEL="ap-smtask1"
COPILOT_TASK_IDS=$(AP_HOME="$COPILOT_HOME" ap_backend_herdr_create_task "$COPILOT_CONTAINER" "$COPILOT_TASK_LABEL" /tmp "$COPILOT_SEEDED_TAB_ID") || fail "copilot create_task failed"
read -r COPILOT_TAB_ID COPILOT_PANE_ID <<EOF
$COPILOT_TASK_IDS
EOF
if [ -z "$COPILOT_TAB_ID" ] || [ -z "$COPILOT_PANE_ID" ]; then
  fail "copilot create_task did not return tab/pane ids"
fi
pass "real herdr: a task spawned into the copilot-shaped home lands as a tab inside the copilot's OWN workspace"

# list_live for each home must never see the OTHER home's task.
PRIMARY_LIVE=$(ap_backend_herdr_list_live "$SESSION")
case "$PRIMARY_LIVE" in
  *"$COPILOT_TASK_LABEL"*) fail "the primary home's list_live must not see a copilot-shaped home's task"$'\n'"$PRIMARY_LIVE" ;;
esac
COPILOT_LIVE=$(AP_HOME="$COPILOT_HOME" ap_backend_herdr_list_live "$SESSION")
case "$COPILOT_LIVE" in
  *"$COPILOT_TASK_LABEL"*) : ;;
  *) fail "the copilot-shaped home's list_live did not see its own task"$'\n'"$COPILOT_LIVE" ;;
esac
case "$COPILOT_LIVE" in
  *"$LABEL"*) fail "the copilot-shaped home's list_live must not see the primary's task ($LABEL)"$'\n'"$COPILOT_LIVE" ;;
esac
...
sleep 0.5
ap_backend_herdr_server_ensure "$SESSION" || fail "the isolated session's server did not come back up after the stop"

POST_LIST=$(herdr workspace list --session "$SESSION" 2>&1)
POST_PRIMARY_ID=$(printf '%s' "$POST_LIST" | jq -r '.result.workspaces[]? | select(.label == "autopilot") | .workspace_id')
POST_COPILOT_ID=$(printf '%s' "$POST_LIST" | jq -r --arg l "copilot-smoketest-copilot1" '.result.workspaces[]? | select(.label == $l) | .workspace_id')
...
[ "$POST_COPILOT_ID" = "$COPILOT_WSID" ] || fail "the copilot workspace id did not survive the restart: before=$COPILOT_WSID after=$POST_COPILOT_ID"
...
POST_COPILOT_PANE=$(herdr pane get "$COPILOT_PANE_ID" --session "$SESSION" 2>/dev/null | jq -r '.result.pane.pane_id // empty')
[ "$POST_COPILOT_PANE" = "$COPILOT_PANE_ID" ] || fail "the copilot task's pane id did not survive the restart: before=$COPILOT_PANE_ID after=$POST_COPILOT_PANE"
...
ap_backend_herdr_kill "$SESSION:$COPILOT_PANE_ID"

# --- send_text_line (atomic run) ---------------------------------------------

ap_backend_herdr_send_text_line "$TARGET" "echo pilot-on-deck-line" \
  || fail "send_text_line failed"
sleep 0.5
out=$(ap_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_text_line"
case "$out" in
  *pilot-on-deck-line*) : ;;
...
ap_backend_herdr_send_literal "$TARGET" 'echo literal-then-key-pilot' \
  || fail "send_literal failed"
sleep 0.2
ap_backend_herdr_send_key "$TARGET" Enter || fail "send_key Enter failed"
sleep 0.5
out=$(ap_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_literal+send_key"
case "$out" in
  *literal-then-key-pilot*) : ;;
...
ap_backend_herdr_send_text_line "$TARGET" "cd /tmp"
sleep 0.3
p=$(ap_backend_herdr_current_path "$TARGET") || fail "current_path failed"
case "$p" in
  */tmp) : ;;
...
if [ "${AP_HERDR_SMOKE_REAL_CLAUDE:-0}" = 1 ] && command -v claude >/dev/null 2>&1; then
  ap_backend_herdr_send_literal "$TARGET" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions --print 'say the word HERDRSMOKEOK and nothing else'"
  sleep 0.2
  ap_backend_herdr_send_key "$TARGET" Enter
  found_working=0
  for _ in $(seq 1 20); do
    bs=$(ap_backend_herdr_busy_state "$TARGET" 2>/dev/null)
  ...
  out=$(ap_backend_herdr_capture "$TARGET" 30)
  case "$out" in
...
elif [ "${AP_HERDR_SMOKE_REAL_CLAUDE:-0}" != 1 ]; then
  echo "note: AP_HERDR_SMOKE_REAL_CLAUDE=1 not set; skipping the real-agent busy_state check" >&2
else
...
ap_backend_herdr_kill "$TARGET" || fail "kill on an already-dead target must stay best-effort (never fail)"
pass "real herdr: kill removes the pane and is idempotent/best-effort"

# --- list_live (label-based recovery discovery) ------------------------------

# Real autopilot spawns always re-run container_ensure immediately before
# create_task (bin/ap-spawn.sh), never reusing a container reference from an
...
# workspace.
CONTAINER_RAW=$(ap_backend_herdr_container_ensure /tmp) || fail "container_ensure for the second task failed"
CONTAINER=${CONTAINER_RAW%%$'\t'*}
SEEDED_TAB_ID=${CONTAINER_RAW#*$'\t'}
...
LABEL2="ap-smoke2"
TASK_IDS2=$(ap_backend_herdr_create_task "$CONTAINER" "$LABEL2" /tmp "$SEEDED_TAB_ID") || fail "second create_task failed"
read -r _TAB_ID2 PANE_ID2 <<EOF
$TASK_IDS2
EOF
live=$(ap_backend_herdr_list_live "$SESSION")
...
pass "real herdr: list_live discovers a live task tab by ap-<id> label"

ap_backend_herdr_kill "$SESSION:$PANE_ID2"
