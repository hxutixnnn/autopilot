#!/usr/bin/env bash
# tests/ap-backend-herdr-launcher-workspace-e2e.test.sh - mandatory ISOLATED
# end-to-end real-Herdr test for worker PLACEMENT with presentation spaces
# disabled.
#
# The guarantee under test: a flight crew member or recon is created in the exact Herdr
# workspace of the autopilot or copilot process that launched it, identified
...
# "autopilot", and the previous label-first-match resolution put the worker in
# whichever one sorted first - visibly the wrong space whenever the launcher was
# not in it.
#
# This drives the REAL bin/ap-spawn.sh and bin/ap-teardown.sh, because the
# guarantee spans the whole spawn handoff (ap-spawn.sh's herdr arm ->
# ap_backend_herdr_container_ensure -> ap_backend_herdr_workspace_ensure ->
# ap_backend_herdr_launcher_identity) and no adapter primitive holds it alone.
# The headline duplicate-label case additionally runs ap-spawn.sh INSIDE a real
...
# operation goes through bin/ap-herdr-lab.sh, which appends the named session
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/ap-herdr-launcher-e2e.XXXXXX")
HERDR_LAB_HELPER="$ROOT/bin/ap-herdr-lab.sh"
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name ap-herdr-launcher-ws) || {
  rm -rf "$TMP_ROOT"
...
# spawn_from_launcher <launcher-pane|""> <home> <task-id> <project> [extra ap-spawn args...]
# Composes exactly the Herdr identity Herdr itself injects into a pane's
# processes. An empty launcher pane means "this autopilot is not running inside
# Herdr at all".
...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      AP_SPAWN_NO_GUARD=1 AP_HOME="$home" AP_ROOT_OVERRIDE="$ROOT" \
      "$ROOT/bin/ap-spawn.sh" "$id" "$proj" "sh -c 'echo launcher-ws-ok'" --backend herdr "$@" \
...
$(make_workspace pilot-other)
EOF
[ -n "$WS_OTHER" ] || fail "could not create the unrelated pilot workspace"
lab tab focus "$WS_OTHER_TAB" >/dev/null 2>&1 || fail "could not focus the unrelated pilot workspace"

focused_workspace() {
  lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.focused == true) | .workspace_id][0] // empty' 2>/dev/null
}
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the unrelated pilot workspace did not take focus"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = autopilot ] || fail "uniqA did not land in a 'autopilot' workspace"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the spawn stole focus from the pilot's workspace"
pass "real herdr E2E: with one 'autopilot' workspace and no herdr parent, a flight crew member still lands in this home's own workspace without stealing focus"

# --- 2. unique label, WITH a launcher pane: same workspace, now by identity --

read -r _ _ LAUNCH_PRIMARY_PANE <<EOF
$(lab tab create --workspace "$WS_PRIMARY" --cwd "$TMP_ROOT" --label pilot-shell --no-focus 2>/dev/null \
  | jq -r '["x","x", .result.root_pane.pane_id] | @tsv' | tr '\t' ' ')
EOF
[ -n "$LAUNCH_PRIMARY_PANE" ] || fail "could not create a launcher pane inside the 'autopilot' workspace"
...
[ "$(workspace_of_pane "$UNIQB_PANE")" = "$WS_PRIMARY" ] \
  || fail "a flight crew member launched from the 'autopilot' workspace must stay in it"
...
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "a projected spawn stole focus from the pilot's workspace"
...
$(make_workspace autopilot)
EOF
[ -n "$WS_PRIMARY_DUP" ] || fail "could not create the second 'autopilot' workspace"
[ "$WS_PRIMARY_DUP" != "$WS_PRIMARY" ] || fail "the two 'autopilot' workspaces must be distinct"
DUP_COUNT=$(lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.label == "autopilot")] | length')
[ "$DUP_COUNT" = 2 ] || fail "expected exactly two 'autopilot' workspaces, got $DUP_COUNT"
...
AP_SPAWN_NO_GUARD=1 AP_HOME="$PRIMARY_HOME" AP_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/ap-spawn.sh" dupC "$PROJ" "sh -c 'echo launcher-ws-ok'" --backend herdr \\
...
lab pane run "$LAUNCH_DUP_PANE" "$TMP_ROOT/spawn-in-pane.sh" >/dev/null 2>&1 \
  || fail "could not run ap-spawn.sh inside the launcher's herdr pane"
i=0
while [ ! -f "$TMP_ROOT/dupC.rc" ] && [ "$i" -lt 120 ]; do sleep 2; i=$((i + 1)); done
[ -f "$TMP_ROOT/dupC.rc" ] || fail "ap-spawn.sh never finished inside the launcher's herdr pane"
...
[ "$DUPC_WS" = "$WS_PRIMARY_DUP" ] \
  || fail "a worker launched from the second 'autopilot' workspace ($WS_PRIMARY_DUP) landed in '$DUPC_WS' instead"
...
pass "real herdr E2E: with two 'autopilot' workspaces, a worker spawned from inside the second one lands in that exact workspace"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = autopilot ] \
  || fail "the other same-labeled workspace was renamed"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the in-pane spawn stole focus from the pilot's workspace"
...
assert_contains_local "$(cat "$SPAWN_ERR")" "labeled 'autopilot'" \
...
DUP_TABS=$(lab tab list --workspace "$WS_PRIMARY" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "ap-dupD")] | length')
DUP_TABS2=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "ap-dupD")] | length')
...
# An autopilot whose own pane was closed under it has an identity that no longer
...
STALE_TABS=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "ap-staleF")] | length')
...
# --- 6. a copilot launching its own worker gets the same guarantee -------

read -r WS_COPILOT_DECOY _ _ <<EOF
$(make_workspace "copilot-$COPILOT_ID")
EOF
read -r WS_COPILOT_LAUNCH _ LAUNCH_COPILOT_PANE <<EOF
$(make_workspace "copilot-$COPILOT_ID")
EOF
[ -n "$WS_COPILOT_DECOY" ] && [ -n "$WS_COPILOT_LAUNCH" ] || fail "could not create the two copilot-labeled workspaces"
WS_COPILOT_DECOY_TABS_BEFORE=$(tab_labels_of_workspace "$WS_COPILOT_DECOY")

spawn_from_launcher "$LAUNCH_COPILOT_PANE" "$COPILOT_HOME" smE "$PROJ"
[ "$SPAWN_RC" -eq 0 ] || fail "a copilot-owned flight crew member spawn failed"$'\n'"$(cat "$SPAWN_ERR")"
SME_META="$COPILOT_HOME/state/smE.meta"
record_worktree "$SME_META"
SME_PANE=$(grep '^herdr_pane_id=' "$SME_META" | cut -d= -f2-)
SME_WS=$(workspace_of_pane "$SME_PANE")
[ "$SME_WS" = "$WS_COPILOT_LAUNCH" ] \
  || fail "a copilot's own worker must land in the copilot's exact workspace ($WS_COPILOT_LAUNCH), got '$SME_WS'"
[ "$(tab_labels_of_workspace "$WS_COPILOT_DECOY")" = "$WS_COPILOT_DECOY_TABS_BEFORE" ] \
  || fail "the duplicate copilot-labeled workspace was mutated"
pass "real herdr E2E: a copilot launching its own worker gets the same exact-workspace guarantee, and its same-labeled sibling is untouched"

# --- 7. a --copilot launch is NOT collapsed into the launcher's workspace -

spawn_from_launcher "$LAUNCH_DUP_PANE" "$PRIMARY_HOME" "$SM2_ID" "$SM2_HOME" --copilot
[ "$SPAWN_RC" -eq 0 ] || fail "the primary's --copilot launch failed"$'\n'"$(cat "$SPAWN_ERR")"
...
[ "$SM2_WS" != "$WS_PRIMARY_DUP" ] \
  || fail "a --copilot launch must stand up the copilot's own workspace, not join the launcher's"
[ "$(label_of_workspace "$SM2_WS")" = "copilot-$SM2_ID" ] \
  || fail "a --copilot launch should land in 'copilot-$SM2_ID', got '$(label_of_workspace "$SM2_WS")'"
pass "real herdr E2E: a --copilot launch still stands up that copilot's own workspace instead of inheriting the launcher's"
...
AP_ROOT_OVERRIDE="$ROOT" AP_STATE_OVERRIDE="$PRIMARY_HOME/state" AP_DATA_OVERRIDE="$PRIMARY_HOME/data" \
  ...
  "$ROOT/bin/ap-teardown.sh" dupC >"$TMP_ROOT/teardown.out" 2>&1
status=$?
[ "$status" -eq 0 ] || fail "ap-teardown.sh failed for dupC"$'\n'"$(cat "$TMP_ROOT/teardown.out")"
[ ! -f "$DUPC_META" ] || fail "ap-teardown.sh did not remove dupC's meta"
if lab pane get "$DUPC_PANE" >/dev/null 2>&1; then
  fail "ap-teardown.sh did not close dupC's own pane"
fi
...
[ "$(label_of_workspace "$WS_PRIMARY_DUP")" = autopilot ] || fail "teardown removed or renamed the launcher's workspace"
