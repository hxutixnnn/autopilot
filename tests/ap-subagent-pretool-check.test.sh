...
CHECK="$ROOT/bin/ap-subagent-pretool-check.sh"
TMP_ROOT=$(ap_test_tmproot ap-subagent-pretool-tests)
...
BRIEF_ONLY_ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/ap-brief.sh followed by bin/ap-spawn.sh for dispatched work'
RECON_ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a recon goes to bin/ap-recon.sh "<question>" [project], while authorized flight work and its bounded research go to bin/ap-brief.sh then bin/ap-spawn.sh'
...
# not flight that Claude-only permissions layer.
...
run_tool() {
  ...
  : > "$OUT"
  : > "$ERR"
  env AP_ROOT_OVERRIDE="$PRIMARY" AP_HOME="$PRIMARY" AP_STATE_OVERRIDE="$STATE" "$@" \
...
test_deny_message_defers_to_intake_classification() {
  local actual
  printf '#!/usr/bin/env bash\n' > "$PRIMARY/bin/ap-recon.sh"
  run_tool Agent && fail "recon-present case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$RECON_ROUTE"*) ;;
    *) fail "deny must reserve bin/ap-recon.sh for classified recon work: $actual" ;;
  esac
  case "$actual" in
    *'investigation or diagnosis goes to bin/ap-recon.sh'*) fail "deny must not classify all investigation or diagnosis as recon work: $actual" ;;
  esac
  rm -f "$PRIMARY/bin/ap-recon.sh"
  run_tool Agent && fail "recon-absent case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$BRIEF_ONLY_ROUTE"*) ;;
    *) fail "deny must degrade to brief-then-spawn when ap-recon.sh is absent: $actual" ;;
  esac
  pass "deny defers to intake classification and degrades gracefully without ap-recon.sh"
}

test_escape_hatch_allows_deliberate_use() {
  local rc value
  expect_allow "escape hatch set" Agent AP_ALLOW_SUBAGENT=1
  expect_deny "escape hatch unset" Agent
  for value in '' 0 yes true 11; do
    rc=0
    run_tool Agent "AP_ALLOW_SUBAGENT=$value" || rc=$?
    [ "$rc" -eq 2 ] || fail "AP_ALLOW_SUBAGENT='$value' must not release the guard, got exit $rc"
  done
  pass "the single documented escape hatch releases the guard only on the exact opt-in value"
}

test_task_worktree_and_non_autopilot_repo_are_inert() {
  ...
  : > "$OUT"
  : > "$ERR"
  AP_ROOT_OVERRIDE="$child" AP_HOME="$child" AP_STATE_OVERRIDE="$child/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 0 ] || fail "a flight crew member task worktree must be out of scope, got exit $rc: $(cat "$ERR")"
  ...
  git -C "$plain" init -q
  rc=0
  AP_ROOT_OVERRIDE="$plain" AP_HOME="$plain" AP_STATE_OVERRIDE="$plain/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 0 ] || fail "a non-autopilot repo must be out of scope, got exit $rc"
  pass "the guard is inert in a flight crew member task worktree and in a non-autopilot repo"
}

test_copilot_home_is_in_scope() {
  local second="$TMP_ROOT/second" rc=0
  git -C "$PRIMARY" worktree add -q -b fixture-second "$second"
  mkdir -p "$second/bin" "$second/state"
  printf '# fixture\n' > "$second/AGENTS.md"
  printf 'copilot-fixture\n' > "$second/.ap-copilot-home"
  AP_ROOT_OVERRIDE="$second" AP_HOME="$second" AP_STATE_OVERRIDE="$second/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 2 ] || fail "a marked copilot home operates a fleet and must be guarded, got exit $rc"
  pass "a marked copilot home is guarded even though it is a linked worktree"
}
...
test_stdin_transports_and_output_shapes() {
  ...
  printf '%s' '{"tool_name":"Agent","tool_input":{"prompt":"go"}}' \
    | AP_ROOT_OVERRIDE="$PRIMARY" AP_HOME="$PRIMARY" AP_STATE_OVERRIDE="$STATE" \
...
test_missing_jq_stdin_transport_fails_open() {
  ...
  printf '%s' '{"tool_name":"Agent"}' \
    | env PATH="$fakebin" AP_ROOT_OVERRIDE="$PRIMARY" AP_HOME="$PRIMARY" AP_STATE_OVERRIDE="$STATE" \
      "$CHECK" --claude > "$OUT" 2> "$ERR" || rc=$?
