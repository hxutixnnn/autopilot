#!/usr/bin/env bash
# Behavior tests for bin/ap-flight-crew-state.sh - the deterministic flight-crew-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. ap-flight-crew-state
...
#   (i) kind=recon skips the run lookup                           -> pane/status-log
#   (j) torn-down worktree / missing meta                         -> unknown/none
#   (k) flight_crew_is_provably_working end-to-end over the REAL helper (not a canned
#       fake ap-flight-crew-state.sh verdict): cross-branch attribution via the runs
...
FLIGHT_CREW_STATE="$ROOT/bin/ap-flight-crew-state.sh"
TMP_ROOT=$(ap_test_tmproot ap-flight-crew-state)
ap_git_identity fmtest fmtest@example.invalid

# A real git repo checked out on <branch>, so the helper's branch attribution
# (git symbolic-ref) resolves like it would for a live flight crew worktree.
make_repo_on_branch() {  # <dir> <branch>
  ...
  # Real worktree HEAD for run head-binding (fixtures read AP_FAKE_RUN_HEAD).
  AP_FAKE_RUN_HEAD=$(git -C "$dir" rev-parse HEAD)
  export AP_FAKE_RUN_HEAD
}
...
case "${1:-}" in
  axi)
    ...
    case "${1:-}" in
      status)
        shift
        if [ "${1:-}" = --run ]; then printf '%s\n' "${AP_FAKE_AXI_STATUS_RUN:-}"
        else printf '%s\n' "${AP_FAKE_AXI_STATUS:-}"; fi ;;
      logs)
        printf '%s\n' "${AP_FAKE_CI_LOGS:-}" ;;
    esac
    ;;
...
case "${1:-}" in
  display-message)
    [ "${AP_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    printf '%%1\n' ;;
  capture-pane)
    [ "${AP_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${AP_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\n%s\n' "${AP_FAKE_BUSY_TEXT:-esc to interrupt}"
...
case "${1:-}" in
  ...
  pane)
    case "${2:-}" in
      read)
        [ "${AP_FAKE_HERDR_MISSING:-0}" = 1 ] && exit 1
        if [ "${AP_FAKE_HERDR_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
        else printf 'all quiet\n> \n'; fi
        exit 0 ;;
    esac ;;
  agent)
    case "${2:-}" in
      get)
        [ -n "${AP_FAKE_HERDR_AGENT_STATUS:-}" ] || exit 1
        printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$AP_FAKE_HERDR_AGENT_STATUS"
        exit 0 ;;
    esac ;;
esac
exit 0
...
# Run the helper for one case dir. AP_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_flight_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" AP_STATE_OVERRIDE="$1/state" "$FLIGHT_CREW_STATE" "$2"
}
...
reset_fakes() {
  ...
  AP_FAKE_BUSY=0
  AP_FAKE_BUSY_TEXT=
  AP_FAKE_TMUX_MISSING=0
  AP_FAKE_HERDR_BUSY=0
  AP_FAKE_HERDR_MISSING=0
  AP_FAKE_HERDR_AGENT_STATUS=""
  AP_FAKE_CI_LOGS=""
  export AP_FAKE_AXI_STATUS AP_FAKE_AXI_STATUS_RUN AP_FAKE_RUNS_LIST AP_FAKE_BUSY AP_FAKE_BUSY_TEXT AP_FAKE_TMUX_MISSING
  export AP_FAKE_HERDR_BUSY AP_FAKE_HERDR_MISSING AP_FAKE_HERDR_AGENT_STATUS AP_FAKE_CI_LOGS
}
...
test_active_run_is_authoritative() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-a
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-a.meta" "window=ap:ap-feat-a" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_running ap/feat-a)"
  local out; out=$(run_flight_crew_state "$d" feat-a)
...
test_stale_needs_decision_superseded() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-b
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-b.meta" "window=ap:ap-feat-b" "worktree=$d/wt" "kind=flight"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  AP_FAKE_AXI_STATUS="$(run_fixing ap/feat-b)"
  local out; out=$(run_flight_crew_state "$d" feat-b)
...
test_stale_blocked_superseded() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-bb
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-bb.meta" "window=ap:ap-feat-bb" "worktree=$d/wt" "kind=flight"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  AP_FAKE_AXI_STATUS="$(run_running ap/feat-bb)"
  local out; out=$(run_flight_crew_state "$d" feat-bb)
...
test_genuine_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-c
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-c.meta" "window=ap:ap-feat-c" "worktree=$d/wt" "kind=flight"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  AP_FAKE_AXI_STATUS="$(run_parked ap/feat-c)"
  local out; out=$(run_flight_crew_state "$d" feat-c)
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cs
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cs.meta" "window=ap:ap-feat-cs" "worktree=$d/wt" "kind=flight"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  AP_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running ap/feat-cs)"
  local out; out=$(run_flight_crew_state "$d" feat-cs)
...
test_gate_block_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cb
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cb.meta" "window=ap:ap-feat-cb" "worktree=$d/wt" "kind=flight"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  AP_FAKE_AXI_STATUS="$(run_parked_in_gate_block ap/feat-cb)"
  local out; out=$(run_flight_crew_state "$d" feat-cb)
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-ci
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-ci.meta" "window=ap:ap-feat-ci" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-ci)"
  local out; out=$(run_flight_crew_state "$d" feat-ci)
...
# Regression for the PR #252 incident: the flight crew's own status log never got a
...
# merge/close. ap-flight-crew-state must surface this as done, not "validating
# (running)", so a green PR is never silently absorbed as still-in-progress.
test_ci_monitoring_checks_green_surfaces_done() {
  reset_fakes
  local d; d=$(new_case ci-green)
  make_repo_on_branch "$d/wt" ap/feat-cigreen
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cigreen.meta" "window=ap:ap-feat-cigreen" "worktree=$d/wt" "kind=flight"
  # No status-log line at all: the flight crew never reported its own checks-green line.
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cigreen)"
  AP_FAKE_CI_LOGS=$(cat <<'EOF'
CI checks running, waiting for results...
all CI checks passed - still monitoring until merged or closed
EOF
)
  local out; out=$(run_flight_crew_state "$d" feat-cigreen)
...
test_top_level_ci_checks_green_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-topcigreen
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-topcigreen.meta" "window=ap:ap-feat-topcigreen" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_top_level_ci ap/feat-topcigreen)"
  AP_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_flight_crew_state "$d" feat-topcigreen)
...
test_ci_monitoring_no_checks_terminal_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cinochecks
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cinochecks.meta" "window=ap:ap-feat-cinochecks" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cinochecks)"
  AP_FAKE_CI_LOGS="no CI checks reported - still monitoring until merged or closed"
  local out; out=$(run_flight_crew_state "$d" feat-cinochecks)
...
test_ci_monitoring_green_then_rearm_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cirearm
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cirearm.meta" "window=ap:ap-feat-cirearm" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cirearm)"
  AP_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
EOF
)
  local out; out=$(run_flight_crew_state "$d" feat-cirearm)
...
test_ci_monitoring_no_checks_yet_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cinochecksyet
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cinochecksyet.meta" "window=ap:ap-feat-cinochecksyet" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cinochecksyet)"
  AP_FAKE_CI_LOGS=$(cat <<'EOF'
...
EOF
  ...
  local out; out=$(run_flight_crew_state "$d" feat-cinochecksyet)
...
test_ci_monitoring_still_waiting_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-ciwait
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-ciwait.meta" "window=ap:ap-feat-ciwait" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-ciwait)"
  AP_FAKE_CI_LOGS="CI checks running, waiting for results..."
  local out; out=$(run_flight_crew_state "$d" feat-ciwait)
...
test_ci_monitoring_green_then_new_issue_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cirelapse
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cirelapse.meta" "window=ap:ap-feat-cirelapse" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cirelapse)"
  AP_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
...
EOF
  ...
  local out; out=$(run_flight_crew_state "$d" feat-cirelapse)
...
test_ci_ready_done_log_relapse_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cireadyrelapse
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cireadyrelapse.meta" "window=ap:ap-feat-cireadyrelapse" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cireadyrelapse.status"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/feat-cireadyrelapse)"
  AP_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
CI checks running, waiting for results...
EOF
  ...
  local out; out=$(run_flight_crew_state "$d" feat-cireadyrelapse)
...
test_ci_fixing_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-cifixing
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-cifixing.meta" "window=ap:ap-feat-cifixing" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cifixing.status"
  AP_FAKE_AXI_STATUS="$(run_ci_fixing ap/feat-cifixing)"
  AP_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_flight_crew_state "$d" feat-cifixing)
...
test_top_level_fixing_ci_running_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-topfixingci
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-topfixingci.meta" "window=ap:ap-feat-topfixingci" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_fixing_ci_running ap/feat-topfixingci)"
  AP_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_flight_crew_state "$d" feat-topfixingci)
...
test_top_level_fixing_done_log_stays_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-topfixing
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-topfixing.meta" "window=ap:ap-feat-topfixing" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-topfixing.status"
  AP_FAKE_AXI_STATUS="$(run_fixing ap/feat-topfixing)"
  AP_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_flight_crew_state "$d" feat-topfixing)
...
test_terminal_passed() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-d
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-d.meta" "window=ap:ap-feat-d" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_passed ap/feat-d)"
  local out; out=$(run_flight_crew_state "$d" feat-d)
...
test_terminal_failed() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-e
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-e.meta" "window=ap:ap-feat-e" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_failed ap/feat-e)"
  local out; out=$(run_flight_crew_state "$d" feat-e)
...
# routine case once more than one flight crew validates the same underlying repo
...
# the repo-wide answer was not this flight crew's own branch.
test_cross_branch_attribution_via_runs_list() {
  reset_fakes
  local d short; d=$(new_case crossbranch)
  make_repo_on_branch "$d/wt" ap/feat-f
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-f.meta" "window=ap:ap-feat-f" "worktree=$d/wt" "kind=flight"
  # The repo-wide active/most-recent run belongs to a different flight crew's branch.
  AP_FAKE_AXI_STATUS="$(run_running ap/other-flight-crew)"
  # Real `no-mistakes runs` shape: plain text, newest-first, no run id, no
  # quoting - "<status> <branch> <short-sha> <date> [<pr-url>]".
  AP_FAKE_RUNS_LIST="$(cat <<EOF
  running    ap/other-flight-crew aaaaaaa  2026-07-02 22:10
  running    ap/feat-f ${short}  2026-07-02 22:05
EOF
)"
  local out; out=$(run_flight_crew_state "$d" feat-f)
...
test_cross_branch_attribution_picks_most_recent_row() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-fq
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-fq.meta" "window=ap:ap-feat-fq" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_running ap/other-flight-crew)"
  AP_FAKE_RUNS_LIST="$(cat <<EOF
  running    ap/other-flight-crew aaaaaaa  2026-07-02 22:10
  running    ap/feat-fq ${short}  2026-07-02 21:50
  completed  ap/feat-fq bbbbbbb  2026-07-02 20:00  https://github.com/o/r/pull/1
EOF
)"
  local out; out=$(run_flight_crew_state "$d" feat-fq)
...
test_coarse_run_does_not_probe_other_branch_ci_log_for_ready_status() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-coarseready
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-coarseready.meta" "window=ap:ap-feat-coarseready" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/4 checks green\n' > "$d/state/feat-coarseready.status"
  AP_FAKE_AXI_STATUS="$(run_ci_monitoring ap/other-flight-crew)"
  AP_FAKE_RUNS_LIST="$(cat <<EOF
  running    ap/other-flight-crew aaaaaaa  2026-07-02 22:10
  running    ap/feat-coarseready ${short}  2026-07-02 22:05
EOF
)"
  AP_FAKE_CI_LOGS="CI checks running, waiting for results..."
  local out; out=$(run_flight_crew_state "$d" feat-coarseready)
...
test_other_branch_run_ignored() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-g
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-g.meta" "window=ap:ap-feat-g" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  AP_FAKE_AXI_STATUS="$(run_running ap/some-other)"
  AP_FAKE_RUNS_LIST="$(cat <<'EOF'
  running    ap/some-other aaaaaaa  2026-07-02 22:10
EOF
)"
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-g
  local out; out=$(run_flight_crew_state "$d" feat-g)
...
# (f) no run for this flight crew + a busy pane -> working via pane
test_no_run_busy_pane() {
  reset_fakes
  local d; d=$(new_case busy)
  make_repo_on_branch "$d/wt" ap/feat-h
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-h.meta" "window=ap:ap-feat-h" "worktree=$d/wt" "kind=flight" "harness=claude"
  # No matching run anywhere. The busy verdict comes from the flight crew's own
  # semantic lifecycle record (bin/ap-busy-lib.sh), not from rendered text.
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$d/state" feat-h)
  "$ROOT/bin/ap-busy-event.sh" apply "$d/state" feat-h busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  local out; out=$(run_flight_crew_state "$d" feat-h)
...
test_no_run_footer_text_alone_is_not_working() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-h2
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-h2.meta" "window=ap:ap-feat-h2" "worktree=$d/wt" "kind=flight" "harness=claude"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_BUSY=1
  printf 'done: stale completion event\n' > "$d/state/feat-h2.status"
  local out; out=$(run_flight_crew_state "$d" feat-h2)
...
# lifecycle is live-verified, so a grok flight crew still reads working from its own
# verified signature.
test_no_run_grok_uses_isolated_fallback() {
  reset_fakes
  local d; d=$(new_case busy-grok)
  make_repo_on_branch "$d/wt" ap/feat-h3
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-h3.meta" "window=ap:ap-feat-h3" "worktree=$d/wt" "kind=flight" "harness=grok"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_BUSY=1
  AP_FAKE_BUSY_TEXT='Ctrl+c:cancel'
  export AP_FAKE_BUSY_TEXT
  local out; out=$(run_flight_crew_state "$d" feat-h3)
...
test_no_run_herdr_unknown_uses_backend_capture() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-herdr
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-herdr.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=flight" \
    "backend=herdr" "harness=claude"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_TMUX_MISSING=1
  AP_FAKE_HERDR_BUSY=1
  AP_FAKE_HERDR_AGENT_STATUS=working
  local out; out=$(run_flight_crew_state "$d" feat-herdr)
...
# actively streaming - docs/herdr-backend.md "Busy state"), not "this flight crew's
# turn is still in progress". A flight crew blocked on its own long-running foreground
# `no-mistakes axi run` (no --yes; blocks until a gate or outcome) is not
# generating for that whole span, so agent.get reads idle. The flight crew's own
# semantic lifecycle record still says busy for the whole turn, and it outranks
# the narrower native verdict - so the flight crew is no longer misread as not-working.
test_no_run_herdr_idle_agent_status_outranked_by_record() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-herdr-idle
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-herdr-idle.meta" "window=default:w1:p3" "worktree=$d/wt" "kind=flight" \
    "backend=herdr" "harness=claude"
  # No run attributable (mirrors a no-mistakes run-step lookup that found no
  # matching row within the configured runs-list window): the flight crew's semantic
  # busy state is the only remaining signal.
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_TMUX_MISSING=1
  AP_FAKE_HERDR_AGENT_STATUS=idle
  AP_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$d/state" feat-herdr-idle)
  "$ROOT/bin/ap-busy-event.sh" apply "$d/state" feat-herdr-idle busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  local out; out=$(run_flight_crew_state "$d" feat-herdr-idle)
  ...
  pass "a mid-tool-call flight crew stays working because its record outranks herdr's generation state"
}
...
test_no_run_herdr_idle_agent_status_and_idle_record_stays_idle() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-herdr-stopped
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-herdr-stopped.meta" "window=default:w1:p4" "worktree=$d/wt" "kind=flight" \
    "backend=herdr" "harness=claude"
  ...
  AP_FAKE_HERDR_AGENT_STATUS=idle
  AP_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$d/state" feat-herdr-stopped)
  "$ROOT/bin/ap-busy-event.sh" apply "$d/state" feat-herdr-stopped idle --gen "$gen" \
    --source claude-hook --event stop
  local out; out=$(run_flight_crew_state "$d" feat-herdr-stopped)
...
test_no_run_idle_pane_uses_log() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-i
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-i.meta" "window=ap:ap-feat-i" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'needs-decision: which database?\n' > "$d/state/feat-i.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-i
  local out; out=$(run_flight_crew_state "$d" feat-i)
...
test_no_run_idle_pane_uses_keyed_log() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-keyed
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-keyed.meta" "window=ap:ap-feat-keyed" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'needs-decision [key=q1]: which database?\n' > "$d/state/feat-keyed.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-keyed
  local out; out=$(run_flight_crew_state "$d" feat-keyed)
...
# supervisor reading the flight crew sees a distinct pause (and its reason) rather than a
# wedge-suspect idle. This is the reader half the watcher/daemon build on.
test_no_run_idle_pane_paused() {
  reset_fakes
  local d; d=$(new_case paused)
  make_repo_on_branch "$d/wt" ap/feat-pause
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-pause.meta" "window=ap:ap-feat-pause" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'paused: holding for the upstream tool release\n' > "$d/state/feat-pause.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-pause
  local out; out=$(run_flight_crew_state "$d" feat-pause)
...
test_no_run_idle_pane_custom_paused_verb() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-custom-pause
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-custom-pause.meta" "window=ap:ap-feat-custom-pause" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'awaiting: vendor maintenance window\n' > "$d/state/feat-custom-pause.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-custom-pause
  local out; out=$(AP_CLASSIFY_PAUSED_VERB=awaiting run_flight_crew_state "$d" feat-custom-pause)
  ...
  out=$(AP_CLASSIFY_PAUSED_VERB=awaiting run_flight_crew_state "$d" feat-custom-pause)
...
# detail: a healthy idle copilot that just closed a keyed decision falls through
# to the idle default (unknown/none), not `unknown` with the resolution note as its
# `doing`. Regression for the bearings render bug where such a copilot showed
# state=unknown with resolution prose. The one-owner keyed fold in ap-classify-lib.sh
# is untouched; this only stops the deriver from reading a non-state event as state.
test_no_run_idle_copilot_resolved_event_not_state() {
  reset_fakes
  local d; d=$(new_case resolved-idle)
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/copilot.meta" "window=ap:ap-copilot" "worktree=$d/wt" "kind=copilot" "home=$d/wt"
  printf 'needs-decision [key=race]: pick subscribe order\n' > "$d/state/copilot.status"
  printf 'resolved [key=race]: went with subscribe-before-write\n' >> "$d/state/copilot.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_BUSY=0
  local out; out=$(run_flight_crew_state "$d" copilot)
  assert_contains "$out" "state: unknown" "resolved-then-idle copilot is not a spurious run-state"
  ...
  out=$(run_flight_crew_state "$d" copilot)
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-dead
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-dead.meta" "window=ap:ap-feat-dead" "worktree=$d/wt" "kind=flight"
  printf 'done: old completion event\n' > "$d/state/feat-dead.status"
  AP_FAKE_AXI_STATUS=""
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_TMUX_MISSING=1
  local out; out=$(run_flight_crew_state "$d" feat-dead)
...
# run-step, not the shell. The common case is a finished flight crew whose agent has
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-dead-done
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-dead-done.meta" "window=ap:ap-feat-dead-done" "worktree=$d/wt" "kind=flight"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  AP_FAKE_AXI_STATUS="$(run_passed ap/feat-dead-done)"
  AP_FAKE_TMUX_MISSING=1   # the flight crew's window has closed
  local out; out=$(run_flight_crew_state "$d" feat-dead-done)
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-dead-act
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-dead-act.meta" "window=ap:ap-feat-dead-act" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_running ap/feat-dead-act)"
  AP_FAKE_TMUX_MISSING=1
  local out; out=$(run_flight_crew_state "$d" feat-dead-act)
...
test_no_timeout_uses_perl_bound() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-timeout
  make_fakebin "$d" >/dev/null
  calls_file="$d/no-mistakes.calls"
  : > "$calls_file"
  cat > "$d/fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${AP_FAKE_NM_CALLS:-/dev/null}"
while :; do :; done
SH
  chmod +x "$d/fakebin/no-mistakes"
  toolbin=$(make_no_timeout_toolbin "$d")
  ap_write_meta "$d/state/feat-timeout.meta" "window=ap:ap-feat-timeout" "worktree=$d/wt" "kind=flight" \
    "harness=claude"
  AP_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$d/state" feat-timeout)
  "$ROOT/bin/ap-busy-event.sh" apply "$d/state" feat-timeout busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  start=$SECONDS
  out=$(AP_FAKE_NM_CALLS="$calls_file" PATH="$d/fakebin:$toolbin" AP_STATE_OVERRIDE="$d/state" AP_FLIGHT_CREW_STATE_NM_TIMEOUT=1 "$FLIGHT_CREW_STATE" feat-timeout)
...
# (i) kind=recon skips the run lookup entirely (its deliverable is a report).
test_recon_skips_run_lookup() {
  reset_fakes
  local d; d=$(new_case recon)
  make_repo_on_branch "$d/wt" ap/recon-j
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/recon-j.meta" "window=ap:ap-recon-j" "worktree=$d/wt" "kind=recon" \
    "harness=claude"
  # Even if a run existed on this branch, a recon must not read it.
  AP_FAKE_AXI_STATUS="$(run_running ap/recon-j)"
  AP_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/ap-busy-event.sh" arm "$d/state" recon-j)
  "$ROOT/bin/ap-busy-event.sh" apply "$d/state" recon-j busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  local out; out=$(run_flight_crew_state "$d" recon-j)
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/gone-k.meta" "window=ap:ap-gone-k" "worktree=$d/no-such-worktree" "kind=flight"
  local out rc
  out=$(run_flight_crew_state "$d" gone-k); rc=$?
...
test_missing_meta() {
  ...
  make_fakebin "$d" >/dev/null
  local out rc
  out=$(run_flight_crew_state "$d" ghost-z); rc=$?
...
# (k) flight_crew_is_provably_working end-to-end over the REAL ap-flight-crew-state.sh (not a
# canned fake verdict, unlike tests/ap-watch-triage.test.sh's classifier
# coverage). This is the direct regression pair for the 2026-07-02 herdr
# incident: a validating flight crew whose bare `axi status` answer belongs to
# another branch must still be absorbed by the watcher via the runs-list
# fallback (working), while a flight crew with genuinely no run anywhere and an idle
...
test_provably_working_via_runs_list_fallback() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-provable
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-provable.meta" "window=ap:ap-feat-provable" "worktree=$d/wt" "kind=flight"
  AP_FAKE_AXI_STATUS="$(run_running ap/other-flight-crew)"
  AP_FAKE_RUNS_LIST="$(cat <<EOF
  running    ap/other-flight-crew aaaaaaa  2026-07-02 22:10
  running    ap/feat-provable ${short}  2026-07-02 22:05
EOF
)"
  PATH="$d/fakebin:$PATH" AP_STATE_OVERRIDE="$d/state" flight_crew_is_provably_working feat-provable \
    || fail "cross-branch attribution via the runs list was not treated as provably working"
  pass "flight_crew_is_provably_working absorbs a validating flight crew found only via the runs-list fallback"
}

test_not_provably_working_when_stopped() {
  reset_fakes
  local d; d=$(new_case provably-working-stopped)
  make_repo_on_branch "$d/wt" ap/feat-stopped
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/feat-stopped.meta" "window=ap:ap-feat-stopped" "worktree=$d/wt" "kind=flight"
...
EOF
  ...
  AP_FAKE_BUSY=0
  PATH="$d/fakebin:$PATH" AP_STATE_OVERRIDE="$d/state" flight_crew_is_provably_working feat-stopped \
    && fail "a stopped flight crew with no run anywhere and an idle pane was treated as provably working"
  pass "flight_crew_is_provably_working still surfaces a genuinely stopped flight crew (safety property preserved)"
}
...
test_usage_error() {
  reset_fakes
  local rc
  "$FLIGHT_CREW_STATE" >/dev/null 2>&1; rc=$?
...
test_historical_same_branch_rewritten_head_not_current() {
  ...
  make_repo_on_branch "$d/wt" ap/todo-flag
  ...
  git -C "$d/wt" branch -q -M ap/todo-flag
  new_head=$(git -C "$d/wt" rev-parse HEAD)
  [ "$old_head" != "$new_head" ] || fail "rewrite did not produce a new head"
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/wishlist.meta" "window=ap:ap-wishlist" "worktree=$d/wt" "kind=flight" "harness=claude"
  ...
  AP_FAKE_RUN_HEAD="$old_head"
  AP_FAKE_AXI_STATUS="$(run_parked ap/todo-flag)"
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" wishlist
  out=$(run_flight_crew_state "$d" wishlist)
...
test_active_run_descendant_fix_head_remains_current() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-pipeline
  ...
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/pipe.meta" "window=ap:ap-pipe" "worktree=$d/wt" "kind=flight"
  AP_FAKE_RUN_HEAD="$fix_head"
  AP_FAKE_AXI_STATUS="$(run_fixing ap/feat-pipeline)"
  out=$(run_flight_crew_state "$d" pipe)
...
test_local_advanced_past_run_head_invalidates() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-adv
  run_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" commit -q --allow-empty -m 'local stage-2 work after prior run'
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/adv.meta" "window=ap:ap-adv" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'working: stage 2 implementation in progress\n' > "$d/state/adv.status"
  AP_FAKE_RUN_HEAD="$run_head"
  AP_FAKE_AXI_STATUS="$(run_parked ap/feat-adv)"
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" adv
  out=$(run_flight_crew_state "$d" adv)
...
test_missing_run_head_falls_back_to_current_state() {
  ...
  make_repo_on_branch "$d/wt" ap/feat-no-head
  make_fakebin "$d" >/dev/null
  ap_write_meta "$d/state/no-head.meta" "window=ap:ap-no-head" "worktree=$d/wt" "kind=flight" "harness=claude"
  printf 'working: current stage still in progress\n' > "$d/state/no-head.status"
  AP_FAKE_AXI_STATUS=$(run_parked ap/feat-no-head | grep -v '^  head:')
  AP_FAKE_RUNS_LIST=""
  AP_FAKE_BUSY=0
  arm_idle_record "$d/state" no-head
  out=$(run_flight_crew_state "$d" no-head)
...
echo "all ap-flight-crew-state tests passed"
