#!/usr/bin/env bash
# Tests for bin/ap-teardown.sh's landed-work safety and stale-lock recovery.
#
# The check refuses to tear down a worktree whose work has not LANDED, because
# treehouse return hard-resets the worktree. "Landed" means reachable from a remote
# OR - for a normal flight task whose commits are not so reachable - its PR is merged
...
#     in main) as landed.
#   - teardown-lock-race: a killed flight crew process can leave a transient worktree
...
#   (o) ap-pr-check rerun after HEAD moved                      -> no stale pr_head
#   (p) ap-pr-check when local HEAD lags                        -> record remote PR head
#   (q) no-mistakes + NO pr= recorded, PR discovered by branch  -> ALLOW  (yolo/no-CI merge)
#
# Also covers backlog teardown-lock-race: a git index.lock left in the worktree by a
# killed flight crew process (bin/ap-teardown.sh's teardown_treehouse_return).
...
TMP_ROOT=$(ap_test_tmproot ap-teardown-tests)
REAL_GIT_FOR_TEST=$(command -v git)
export REAL_GIT_FOR_TEST

# Build a fresh sandbox for one test case. Sets up:
#   $CASE/state/        - autopilot state dir (with a fresh watcher beacon)
...
#   $CASE/project/      - clone of origin; acts as the autopilot project dir
...
SH
  ...
  # Add a worktree on a fresh task branch; that branch is where the flight crew member commits.
  git -C "$case_dir/project" worktree add -q -b ap/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so ap-guard stays quiet.
...
write_meta() {
  ...
  ap_write_meta "$case_dir/state/task-x1.meta" \
    "window=autopilot:ap-task-x1" \
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/ap-task-x1 is visible from the worktree (shared object db).
  git -C "$case_dir/wt" push -q fork ap/task-x1
...
# ap-teardown.sh's own retry-then-stale-cleanup logic (teardown_treehouse_return
# in bin/ap-teardown.sh) rather than hand-simulating that logic in the test.
...
# (simulating a dying flight crew git process finishing) so the next retry succeeds.
...
run_teardown() {
  ...
  AP_STATE_OVERRIDE="$case_dir/state" \
  AP_CONFIG_OVERRIDE="$case_dir/config" \
...
test_local_only_fork_remote_allows() {
  ...
  write_meta "$case_dir" local-only flight
...
test_teardown_prompts_tasks_axi_done_when_compatible() {
  ...
  write_meta "$case_dir" no-mistakes flight
  printf '%s\n' 'pr=https://github.com/example/repo/pull/7' >> "$case_dir/state/task-x1.meta"
...
test_no_mistakes_origin_remote_allows() {
  ...
  git -C "$case_dir/wt" push -q origin ap/task-x1
...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  # recorded in meta at all (ap-pr-check.sh was never run, e.g. a yolo merge on
  ...
  # never pushing ap/task-x1 at all, so no refs/remotes/origin/ap/task-x1
...
test_squash_merged_pr_allows_replayed_unpushed_patch() {
  ...
  git -C "$case_dir/wt" push -q origin "$parent_head:refs/heads/ap/task-x1"
  git -C "$case_dir/project" fetch -q origin ap/task-x1
  wt_commit_file "$case_dir" feature.txt hello "add feature"
  append_pr_meta_url "$case_dir"
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "ap-pr-check does not refresh PR head after HEAD moves"
}
...
test_pr_check_records_remote_head_when_local_lags() {
  ...
  pass "ap-pr-check records the remote PR head when the local worktree lags"
}
...
test_non_linked_index_lock_path_is_checked_from_worktree() {
  ...
  git -C "$case_dir/wt" checkout -q -b ap/task-x1
  write_meta "$case_dir" no-mistakes flight
...
test_transient_index_lock_clears_after_first_attempt_and_retry_succeeds() {
  ...
  AP_TREEHOUSE_RETURN_LOCK_RETRIES=2 \
  AP_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=0 \
  AP_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_empty_retry_wait_uses_default_without_aborting() {
  ...
  set +e
  TREEHOUSE_ATTEMPT_FILE="$attempt_file" \
  AP_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  AP_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  AP_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS='' \
  AP_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_fractional_legacy_retry_wait_refuses_without_arithmetic_error() {
  ...
  AP_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  AP_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  AP_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0.1 \
  AP_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_teardown_missing_busy_sidecar_completes() {
  ...
  write_meta "$case_dir" local-only flight
  gen=$("$ROOT/bin/ap-busy-event.sh" arm "$case_dir/state" task-x1)
...
case "${1:-} ${2:-}" in
  "workspace list")
    if [ -e "${AP_FAKE_HERDR_RESTORED:?}" ]; then
      printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w2","active_tab_id":"w2:t2","label":"copilot-bravo","focused":true},{"workspace_id":"w3","active_tab_id":"w3:t1","label":"copilot-alpha","focused":false}]}}'
    elif [ -e "${AP_FAKE_HERDR_CLOSED:?}" ]; then
      printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w2","active_tab_id":"w2:t2","label":"copilot-bravo","focused":false},{"workspace_id":"w3","active_tab_id":"w3:t1","label":"copilot-alpha","focused":true}]}}'
    else
      printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","active_tab_id":"w1:t2","label":"autopilot/task-x1 · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w2","active_tab_id":"w2:t2","label":"copilot-bravo","focused":true},{"workspace_id":"w3","active_tab_id":"w3:t1","label":"copilot-alpha","focused":false}]}}'
    fi
    ;;
  "tab list")
    case "$*" in
  ...
  "session list")
    ...
    ;;
  "pane close")
    if [ "${AP_FAKE_HERDR_CLOSE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${AP_FAKE_HERDR_CLOSED:?}"
    ;;
  "pane get")
    if [ -e "${AP_FAKE_HERDR_CLOSED:?}" ]; then
...
test_herdr_projection_teardown_retires_journal_only_after_confirmed_close() {
  ...
  AP_FAKE_HERDR_LOG="$log" AP_FAKE_HERDR_CLOSED="$closed" AP_FAKE_HERDR_RESTORED="$restored" \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
test_herdr_projection_teardown_retains_journal_when_close_unconfirmed() {
  ...
  AP_FAKE_HERDR_LOG="$log" AP_FAKE_HERDR_CLOSED="$closed" AP_FAKE_HERDR_RESTORED="$restored" AP_FAKE_HERDR_CLOSE_FAIL=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
