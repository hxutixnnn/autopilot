#!/usr/bin/env bash
# ap-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# popup settles lets it swallow the Enter, so the line never submits. ap-send
# absorbs this by pausing `settle` seconds AFTER typing and BEFORE the (retried)
# Enter - the first sleep ap_tmux_submit_core makes. These tests pin the
...
#
# The popup-settle is the FIRST sleep recorded: ap_tmux_submit_core types the text,
# then `sleep "$settle"`, then the Enter-retry loop (sleep 0.4 each) and finally
# ap-send's own post-submit AP_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from ap-send-settle.test.sh which pins
# the TAIL (post-submit) pause. The retried Enter in ap_tmux_submit_core remains the
...
TMP_ROOT=$(ap_test_tmproot ap-send-popup-settle)

# Same stub shape as ap-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into AP_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. AP_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. AP_ROOT_OVERRIDE points at a non-repo dir so
# ap-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  if [ "$harness" = --explicit ]; then
    ...
    ap_write_meta "$home/state/$meta_id.meta" "window=sess:win" "harness=$harness"
  fi
  : > "$log"
  env AP_SEND_SETTLE=0 PATH="$fb:$PATH" \
  ...
  pass "ap-send popup-settle: $label -> ${expected}s"
}
...
# task id, not only by the legacy `ap-<id>` window label.
