...
This document is the authoritative human-readable contract for the guard that stops an autopilot primary from delegating work outside the fleet.

The delivered mechanism is `bin/ap-subagent-pretool-check.sh`, a PreToolUse guard that denies a delegation-SHAPED tool name in a genuine primary home.
...
On 2026-07-22 an autopilot primary ran four workers through Claude Code's built-in subagent tool instead of `bin/ap-spawn.sh`.
...
- When the primary session restarted, two of those workers died mid-flight and their work was lost.
  A real flight crew member lives in its own backend session with durable state and survives a primary restart.
- The supervision cycle then stayed down for 73 minutes unnoticed, which silently killed the pilot's Workflowy intake channel, since that channel only fires while a watch cycle runs.
...
Only `bin/ap-spawn.sh` writes `state/<id>.meta`, so untracked project work contributes nothing to the in-flight count used by `bin/ap-supervision-lib.sh` and `bin/ap-turnend-guard.sh`.
...
`bin/ap-subagent-pretool-check.sh` is the delivered layer.
...
- `PLAN_ONLY_TOOLS`: the exact names `taskcreate` and `taskupdate` are allowed.
  ...
  That list has no executor: it spawns no agent, allocates no worktree, registers no schedule, and starts nothing that could outlive the session or escape an autopilot guard.
  So it is not the "work, agent, schedule, or isolated workspace that autopilot would not know about" the guard exists to stop, and the stem match on `task` is a false positive rather than a policy.
  The cost of the false positive was concrete: the primary could not track its own plan, and the deny text told it to run `bin/ap-brief.sh` and `bin/ap-spawn.sh` to create a todo entry.
...
- A tracked `.claude/settings.json` propagates into linked worktrees and disarms legitimate flight crew members.
  This was verified when a Claude session in a task worktree of this repo lost its `Agent` tool.

The width of the list remains a pilot-owned decision, because denying some of these changes how the pilot works with the primary session.
...
The hook deliberately allows those five, so the delivered guard can never strand a runaway task with no way to inspect or end it, and it allows `TaskCreate` and `TaskUpdate` too, so it can never be the reason the primary cannot track its own plan.
...
Denying them there would instead reproduce at a stronger layer the exact false positive the delivered guard now avoids, leaving anyone who adopts this list verbatim unable to let a primary track its own plan.
Narrowing the list further, including the five observe-or-stop names, is the pilot's call, and this local list is the only layer that can remove a todo tool from the primary's schema.
...
The delivered hook fires only in a genuine autopilot primary home, using the shared predicate `ap_primary_scope_matches` from `bin/ap-primary-scope-lib.sh`.
This is the same predicate `bin/ap-sessionstart-nudge.sh` and `bin/ap-turnend-guard.sh` use, so the three tracked primary-scoped hooks cannot drift apart.

A home is in scope when it has `AGENTS.md`, a `bin/` directory, an existing state directory, and either a plain checkout where git-dir equals git-common-dir or a valid `.ap-copilot-home` marker.
A marked copilot home is in scope on purpose: it operates its own fleet and must dispatch through it for the same durability reasons.

A flight crew member's disposable task worktree is a linked git worktree, which is the shape `bin/ap-spawn.sh` always hands out, so it is out of scope.
A flight crew member using delegation tools inside its own task worktree is legitimate and stays allowed.
A non-autopilot repo is out of scope.
...
The hook scope is the delivered enforcement boundary, and the linked-worktree negative case proves the script itself does not block legitimate flight crew member delegation.

## Escape hatch

`AP_ALLOW_SUBAGENT=1` in the session environment allows the call at the delivered hook.
...
When `bin/ap-recon.sh` exists in the home the message first defers to the `AGENTS.md` intake classification, then routes work already classified as a recon there and authorized flight work with its bounded research to `bin/ap-brief.sh` then `bin/ap-spawn.sh`.
When that script is absent the message still defers to intake classification and degrades to naming `bin/ap-brief.sh` then `bin/ap-spawn.sh` for dispatched work, rather than pointing at a script that is not there.

## Harness wiring

Every supported primary harness was reviewed.
Applicability turns on one question: does the harness expose built-in delegation tools that a primary session could use instead of `bin/ap-spawn.sh`?
...
The integration surface of each was inspected and each is structurally wireable for the delivered guard.

- Grok's tracked hooks (`.grok/hooks/ap-primary-pretool-check.json`, `.grok/hooks/ap-primary-cd-check.json`) use a `PreToolUse` matcher, currently `Bash`, and pipe stdin to a checker.
...
This repo's rule in the `autopilot-coding-guidelines` skill is that a harness hook must be validated in a scratch project before it is trusted, and `arm-pretool-check.md` records the concrete cost of guessing: a Grok hook whose `command` string is even slightly wrong fails to launch the hook at all.
...
`bin/ap-subagent-pretool-check.sh` needs no change for any of them: it already accepts Grok's stdin shape and the `--tool` CLI form OpenCode and Pi use, and it already emits the Grok stdout decision object by default.
...
Run in a scratch autopilot-shaped project containing `AGENTS.md`, `state/`, a full copy of `bin/`, and a Claude settings file containing the local deny list exactly as recommended on that date, which was the 18-name form that still included `TaskCreate` and `TaskUpdate`.
...
To reproduce a future tool that appears before a local deny list is updated, `Workflow` was removed from the deny list in the same scratch project while the guard stayed wired.
...
> [subagent-dispatch] the autopilot primary dispatches through the fleet, not the harness's own
> delegation tools... (blocked tool: Workflow). Launch the session with AP_ALLOW_SUBAGENT=1 for a
...
Same hook, same bytes, deny in the primary home and allow in a flight crew member-shaped worktree.
This is the scoping contract working end to end rather than a hook that simply never fires.

### Escape hatch

The same `Workflow` prompt in the scratch primary home, launched as `AP_ALLOW_SUBAGENT=1 claude -p ...`:
...
`tests/ap-subagent-pretool-check.test.sh` asserts stdout is empty on every `--claude` deny and that default mode still emits the Grok object on stdout.
...
`tests/ap-subagent-pretool-check.test.sh` owns the acceptance matrix and is registered in the `pure-contract-unit` family in `bin/ap-test-run.sh`.
It covers the tracked Claude settings boundary that forbids a `permissions` key; the match-all Claude hook registration; denial of every work-creating delegation tool by shape; denial of twelve hypothetical future tool names that appear on no list; the observe-or-stop, plan-only, and MCP exclusions; the exactness of the plan-only exclusion against six near-miss names a substring or shorter-stem widening would release; the recon-present and recon-absent message variants; the escape hatch including its fail-closed values; inertness in a linked task worktree and in a non-autopilot repo; in-scope enforcement for a marked copilot home; both stdin transports; the empty-stdout requirement; fail-open transport behavior; and the preserved `Bash` seatbelts and `Stop` guard.

Run:

```sh
bash -n bin/ap-subagent-pretool-check.sh
bin/ap-lint.sh
...
Every autopilot guard's in-flight-work branch keys off `state/<id>.meta`, and only `bin/ap-spawn.sh` writes that record.
...
That is a separate change to `bin/ap-supervision-lib.sh` and `bin/ap-turnend-guard.sh` and is out of scope here.
