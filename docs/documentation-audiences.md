...
`bin/ap-doc-audience-check.sh` validates exact inventory coverage, README setup routing, required owner pointers, and local link targets.
...
- `agent-runtime` is loaded or rendered as an operating contract for Autopilot agents rather than read as product documentation.

The knowledge-placement policy is owned by [`autopilot-coding-guidelines`](../.agents/skills/autopilot-coding-guidelines/SKILL.md).
