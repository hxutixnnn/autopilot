...
Codex App is not a selectable Autopilot runtime backend.
Codex Desktop host tools can create and supervise visible threads and those threads can write Autopilot status files when given an authorized path, but Autopilot has no supported shell-callable bridge to those host tools.
...
5. Let the thread append Autopilot's normal lifecycle lines to `state/<id>.status`.

The status return channel is mandatory.
A visible thread that cannot report into Autopilot's normal lifecycle is not a complete backend.

## Current blocker

Autopilot backend scripts are shell entry points and can call tmux, Herdr, Zellij, Orca, and cmux directly.
Codex Desktop host tools are available to a Desktop conversation, not to arbitrary Autopilot subprocesses.
...
A one-process probe could create and archive a thread record, but no supported bridge was found that lets Autopilot create, continue, read, and archive the same visible Desktop-owned endpoint over its full lifetime.
...
Once available, Autopilot should add a real `bin/backends/codex-app.sh`, persist `backend=codex-app` and `codex_app_thread_id=`, and route spawn, send, peek, watch, and cleanup through the shared dispatcher.

## Rollout

Flight and recon tasks come first.
Copilot support remains out of scope until create, send, read, status return, and archive are proven through the normal backend dispatcher.
