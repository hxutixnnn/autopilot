...
The away-mode sub-supervisor (`bin/ap-supervise-daemon.sh`) buffers escalations and injects them into Autopilot's own pane.
When injection cannot confirm a submit past `AP_MAX_DEFER_SECS`, `inject_wedge_alarm` raises a loud, rate-limited alarm so the stall never stays invisible.
The active alert is pane-independent because a tmux status-line flash has no cross-backend equivalent and cannot reach an unattended pilot reliably.
...
`AP_WEDGE_ALARM_CHANNEL` overrides the file with one directive for focused testing.
...
Every invocation is process-group bounded by `AP_WEDGE_ALARM_TIMEOUT_SECS`, which defaults to 10 seconds, including `command:`, `osascript`, `herdr`, and the test seam.
...
Every notifier routes through `AP_WEDGE_ALARM_EXEC` in `wedge_alarm_emit`.
...
`tests/ap-daemon.test.sh` covers directive parsing, rate limiting, timeout and process-group cleanup, argv-safe dispatch, channel fallback, and safe `command:` summary delivery.
