...
`tests/ap-pr-check-security.test.sh` asserts that neither `bin/ap-pr-lib.sh` nor `bin/ap-pr-poll.sh` contains the string `gitlab.com` at all.
...
Its JSON would need a JSON processor, and `jq` is not one of autopilot's common tools, so the state is read from glab's own field output instead.
Only an exact `merged` wakes autopilot, so a changed output format produces no wake rather than a false merge.
...
```
$ ap-pr-check.sh e1 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
armed: state/e1.check.sh
$ ap-pr-check.sh e2 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
armed: state/e2.check.sh
$ ap-pr-check.sh e3 https://gitlab.example/group/subgroup/project/-/merge_requests/7
...
```
$ PATH="$noglab" ap-pr-poll.sh --validated $(tr '\n' ' ' < state/e1.pr-poll)
$ PATH="$noglab" ap-pr-poll.sh --validated $(tr '\n' ' ' < state/e3.pr-poll)
```
...
```
$ PATH="$noglab" ap-pr-check.sh e5 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
...
```
$ PATH="$noglab" ap-pr-check.sh e6 https://github.com/hxutixnnn/autopilot/pull/750
armed: state/e6.check.sh
```

## Upgrade path from an existing armed watch

The stored record gained the provider tag, so its version moved to `ap-pr-poll-registration-v2` and a record written by the previous release no longer parses.
...
ap-pr-poll-registration-v1
$ ap-pr-check-migrate.sh --checks-safe
...
`bin/ap-pr-merge.sh` still addresses GitHub only, by owner and repository.
...
`gh` exposes the head commit as a selectable field, while plain `glab` exposes it only inside its JSON output, which would need a JSON processor autopilot does not require.
Both consumers already treat it as optional: `bin/ap-teardown.sh` reads the head from the forge at teardown rather than from metadata and falls back to its provider-agnostic content check, and `bin/ap-review-diff.sh` resolves the head from the remote when none is recorded.
