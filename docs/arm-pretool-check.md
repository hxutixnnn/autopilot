...
`bin/ap-arm-command-policy.mjs` is the single semantic owner.
`bin/ap-arm-pretool-check.sh` is only the stable harness transport and output renderer.
The tracked harness adapters forward command text without classifying it.
`bin/ap-arm-command-policy.mjs` is also the sole owner of autopilot's shell classification: it exports the tokenizer and command-position analysis, which the sibling cd-guard seatbelt (`bin/ap-cd-pretool-check.sh`, `docs/cd-guard.md`) reuses instead of duplicating shell lexing.

## Purpose and boundary

An autopilot primary must arm `bin/ap-watch-arm.sh` or run `bin/ap-watch-checkpoint.sh` through an observable harness call.
...
`bin/ap-guard.sh`, `bin/ap-turnend-guard.sh`, the watcher lock, and the watcher beacon still prove whether supervision is healthy after an allowed call.
...
`bin/ap-arm-pretool-check.sh` supports these entry forms:
...
The active autopilot home is `${AP_HOME:-<code-root>}`.
It passes both roots and the exact command string to the Node policy owner.

The wrapper fast-allows a command without invoking the Node policy owner only when the command cannot contain the `ap-watch` byte sequence even after the classifier's decoders run.
The fast path may allow only when both of these hold:

1. The stripped text lacks the `ap-watch` watcher substring, after mirroring the classifier's cheapest byte normalizations - dropping line-continuation and escape backslashes, quotes, and newlines.
...
Any `ap-watch` match or any quoting-decoder marker delegates to the classifier.
Normalizing first keeps this a strict superset: a protected watcher path obfuscated as `ap-watc\<newline>h-arm.sh` or `ap-"watch"-arm.sh` still delegates, and stripping only those non-alphanumeric bytes can never destroy an existing `ap-watch` run.
The quoting-decoder marker closes the case the byte strip cannot: `bin/ap-$'\x77'atch-arm.sh` and `bin/ap-$"watch"-arm.sh` both resolve to `bin/ap-watch-arm.sh` only after the classifier decodes the encoded character, so a cheap byte strip would otherwise lose the `ap-watch` bytes and fast-allow them.
This marker set is coupled to the classifier's decoder set in `bin/ap-arm-command-policy.mjs`: adding any new quote or expansion form the classifier decodes requires extending this marker set in the same change, or the prefilter stops being a strict superset.
...
```text
bin/ap-watch-arm.sh          (arm; blessed entry point)
bin/ap-watch-checkpoint.sh   (checkpoint; blessed entry point)
bin/ap-watch.sh              (watch; protected but never blessed)
```

The relative form, the `<code-root>`-anchored absolute form, and any word ending in `/bin/<script>` all resolve to that identity.
Suffix matching recognizes an expanded-path prefix statically, so `$AP_HOME/bin/ap-watch-arm.sh`, `$HOME/autopilot/bin/ap-watch-arm.sh`, and `~/autopilot/bin/ap-watch-arm.sh` are the arm identity.
The classifier never expands the variable or tilde; it matches the literal bytes only.
Static quote forms are cooked before the suffix match, so a command word split by ordinary quotes (`ap-"watch"-arm.sh`), ANSI-C quoting (`ap-$'\x77'atch-arm.sh`), or a bash locale string (`ap-$"watch"-arm.sh`) all resolve to the same identity; this reads the fixed literal bytes as the shell would cook them and never runs an expansion or a command.
...
`bin/ap-watch.sh` is protected but is not a blessed entry point.
A direct `bin/ap-watch.sh` execution - relative, `<code-root>`-anchored, `$VAR`-prefixed, or `~`-prefixed - always denies with `watcher-direct`, whose reason points the caller at `bin/ap-watch-arm.sh` and `bin/ap-watch-checkpoint.sh`.
...
`bin/ap-watch-arm.sh` and `bin/ap-watch-checkpoint.sh` are the only blessed final nodes, including their expanded-path forms; a `bin/ap-watch.sh` final node is never blessed and denies with `watcher-direct`.
...
An actually executed `pkill` command is denied when its parsed pattern arguments target `ap-watch`.
Path-qualified `pkill`, `command pkill`, and `sudo pkill` are recognized.

`kill "$(pgrep -f '/bin/ap-watch.sh')"` is also denied because the executed `kill` consumes an executed watcher-wide `pgrep` substitution.
A standalone read-only `pgrep` is allowed.
Quoted text such as `echo 'pkill -f ap-watch'` is data and is allowed.
...
When the command carries such grammar and its raw bytes reference both a `ap-watch` target and a `pkill` or `kill` verb, the classifier cannot prove which command position the kill occupies, so it denies with `broad-watcher-kill` rather than allowing.
This backstop mirrors the protected-execution fail-closed rule and covers forms like `while true; do pkill -f ap-watch; done`, `for x in 1; do pkill -f ap-watch; done`, `case x in x) pkill -f ap-watch ;; esac`, and `until false; do kill $(pgrep -f ap-watch); done`.
It is gated on the grammar being unsupported: in grammar the classifier does model, command-position analysis is authoritative, so data mentions such as `echo 'pkill -f ap-watch'` and a loop that only names the watcher without a kill verb such as `for f in 1; do echo ap-watch; done` remain allowed.
...
| `watcher-direct` | A direct `bin/ap-watch.sh` execution; the watcher must be reached through `bin/ap-watch-arm.sh` or `bin/ap-watch-checkpoint.sh`. |
...
| Grok | `.toolInput.command` | `.grok/hooks/ap-primary-pretool-check.json` forwards stdin and Grok consumes the stdout `decision=deny` object. |
| OpenCode | `output.args.command` | `.opencode/plugins/ap-primary-pretool-check.js` passes one `--command` argument and throws only for exit 2. |
| Pi / pi-signed | `event.input.command` | `.pi/extensions/ap-primary-turnend-guard.ts` passes one `--command` argument and returns `{block: true}` only for exit 2. |
...
Validation ran in a git-initialized scratch autopilot-shaped project under this task worktree.
...
The OpenCode interactive check used the dedicated tmux socket `ap-pretool-smoke`.
...
pi -p -e .pi/extensions/ap-primary-turnend-guard.ts --no-context-files --no-session "$PROMPT"
```
...
The Grok debug transcript showed four exit-0 results from `project/ap-primary-pretool-check`, then exit 2 with 145 stdout bytes, 214 stderr bytes, and `hook denied` for the backgrounded arm.
OpenCode displayed the four allowed command outputs and then `bin/ap-watch-arm.sh & failed` with the stderr deny object.
...
- Claude ran `bin/ap-watch-arm.sh --restart` with its native tracked background option and produced `watcher: started pid=<scratch> (scratch)`.
...
- OpenCode ran in an interactive TUI on `tmux -L ap-pretool-smoke`, reached `session.idle`, and its unchanged watch-arm plugin created the scratch automatic-arm marker.
- Pi loaded both primary extensions, called `ap_watch_arm_pi`, and created the scratch automatic-arm marker.

Every native-path automatic marker was present and every deny sentinel remained absent.

## Automated validation

`tests/ap-arm-pretool-check.test.sh` owns the adversarial acceptance matrix.
...
```sh
bash -n bin/ap-arm-pretool-check.sh
shellcheck bin/ap-arm-pretool-check.sh tests/ap-arm-pretool-check.test.sh
node --check bin/ap-arm-command-policy.mjs
tests/ap-arm-pretool-check.test.sh
bin/ap-test-run.sh --all
```
