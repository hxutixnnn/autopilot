...
[`configuration.md`](configuration.md#runtime-backend-configbackend--ap_backend) owns shared selection and metadata semantics.

## Setup

Pick cmux when you already use the app as your terminal and want task workspaces in its sidebar.
cmux is macOS-only, GUI-first, and unsuitable for a headless or SSH-only Autopilot session.
...
cmux defaults to a control mode that rejects external shells, while Autopilot always controls it from an external process.
Open Settings > Automation and choose a viable Socket Control Mode before the first cmux-backed spawn.

| Setting | Value | Autopilot support | Security boundary |
| --- | --- | --- | --- |
...
For Password mode, store the password as the first line of local gitignored `config/cmux-socket-password` or provide `CMUX_SOCKET_PASSWORD` in Autopilot's environment.
...
Select cmux with local `config/backend` containing `cmux`, `AP_BACKEND=cmux` for one launch, or an explicit request to Autopilot.
It can also be runtime auto-detected when Autopilot itself runs inside cmux.
...
Routine supervision uses `bin/ap-peek.sh <id>` and `AP_HOME=<home> bin/ap-send.sh <id> '<text>'` without bringing the cmux window forward.
...
The caller-facing label remains `ap-<id>`, while the visible workspace title is `ap-<home-label>-<id>`.
The home label is `autopilot` or `copilot-<id>` plus a stable short hash of the resolved Autopilot root.
cmux does not enforce title uniqueness, so create, recovery, list, and cleanup paths all validate this scoped title.
Relocating the Autopilot installation changes the hash and leaves old titles unmatched, consistent with recorded worktree paths also becoming stale.
...
The composer verifier locates the last bordered composer row and delegates the content decision to `bin/ap-composer-lib.sh`.
...
When the task is last in its window, Autopilot creates one unfocused unnamed sibling workspace in that same window, closes the task workspace, and leaves the window with cmux's fresh default workspace.
The sibling never carries an `ap-` title and is ignored by recovery.
...
Autopilot does not attempt to close the macOS window because cmux's socket cannot close a window holding a live terminal.

Real tests share the pilot's running app rather than creating an isolated cmux session.
`tests/cmux-test-safety.sh` permits cleanup only for an exact currently listed `ap-test-` workspace and never enumerates and closes unrelated workspaces or relaunches the app.
...
- Copilot spawns are unsupported until a per-home lifecycle design is verified.
