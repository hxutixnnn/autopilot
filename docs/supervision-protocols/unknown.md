...
Use `bin/ap-watch-arm.sh` only when the harness has a tracked background mechanism that survives the tool call and notifies the model on process exit.
Use a bounded foreground wait over `bin/ap-watch.sh` when that wake mechanism is not verified.
