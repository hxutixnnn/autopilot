...
8. Waiting is silent.
9. Never use shell `&` for autopilot supervision.
10. Never bundle the arm onto another command.
    A shell `&`, a truncating pipe, or bundling is denied automatically by the PreToolUse seatbelt (`bin/ap-arm-pretool-check.sh`) whenever this project's Grok hooks are trusted.
...
1. Run `bin/ap-wake-drain.sh` first.
...
The primary project Stop hook runs `bin/ap-turnend-guard-grok.sh` as a backstop, not the normal wake path.
...
Headless `grok -p` may wait for background process exit but does not reliably surface full auto-wake model output; do not run the primary autopilot as a one-shot headless process.
