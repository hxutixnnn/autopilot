...
2. Routine watcher arm and re-arm are owned by the Stop `asyncRewake` hook (`bin/ap-claude-stop-autoarm.sh`), never by you.
...
3. On a `Stop hook feedback` wake (`signal:`, `stale:`, `check:`, or `heartbeat`), run `bin/ap-wake-drain.sh` first and handle the wake.
   Do not run `bin/ap-watch-arm.sh` after an ordinary wake; the next turn end re-arms automatically when supervision is still needed.
...
5. Manual arm is recovery only.
   When a repair is genuinely needed - the Stop hook did not claim this home, or a forced restart is required - run `bin/ap-watch-arm.sh` (or `bin/ap-watch-arm.sh --restart`) as its own Claude Code background task, never bundled with other commands, never with shell `&`.
   A shell `&`, a truncating pipe, or bundling is denied automatically by the PreToolUse seatbelt (`bin/ap-arm-pretool-check.sh`) registered in `.claude/settings.json`.
...
8. The turn-end guard (`bin/ap-turnend-guard.sh --claude`) remains the final backstop.
...
The watcher itself remains `bin/ap-watch.sh`, and `bin/ap-watch-arm.sh` remains the verified arm wrapper that the Stop hook foregrounds.
