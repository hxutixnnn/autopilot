...
`bin/ap-cd-command-policy.mjs` is the single decision owner.
`bin/ap-cd-pretool-check.sh` is the stable harness transport, primary-checkout scope, and output renderer.
...
the watcher-arm PreToolUse seatbelt (`bin/ap-arm-pretool-check.sh`, `docs/arm-pretool-check.md`) and the turn-end supervision guard (`bin/ap-turnend-guard.sh`, `docs/turnend-guard.md`).

## Purpose and boundary

The primary autopilot shell persists its working directory across tool calls.
A stray persistent top-level `cd projects/<clone>` therefore silently relocates the shell, so the next autopilot-owned command - a backlog write, an `ap-*` lifecycle call, `tasks-axi` - runs inside a project clone instead of the home.
That has actually happened: a persistent top-level `cd` caused an autopilot-owned backlog write to execute inside a project clone rather than the home.
...
## Scope: plain autopilot checkouts only

The guard fires only in a plain autopilot checkout where git-dir equals git-common-dir.
It is a silent no-op (exit 0, no output) everywhere else, so it never interferes with a flight crew member or recon that legitimately works inside its own project or autopilot task worktree.

`bin/ap-cd-pretool-check.sh` owns its checkout detection; the turn-end guard's marker-aware scope is a separate contract (`docs/turnend-guard.md`).
A plain, non-worktree checkout has `git rev-parse --git-dir` equal to `git rev-parse --git-common-dir`.
A flight crew member or recon task worktree - the shape `bin/ap-spawn.sh` always hands out - is a linked git worktree where the two differ, so the guard is inert there.
...
The cd-guard does not inspect `.ap-copilot-home`.
It therefore applies in a git-cloned copilot home where git-dir equals git-common-dir, but remains inert in a treehouse-leased copilot home that is itself a linked worktree.
Copilot child flight crew and recon worktrees are likewise inert under the linked-worktree test.
...
`bin/ap-cd-pretool-check.sh` supports all five harness-engine entry shapes used by the tracked adapters, with pi-signed sharing Pi's shape:
...
The quoting-decoder marker set is coupled to the classifier's decoder set in `bin/ap-arm-command-policy.mjs`: adding any new quote or expansion form the classifier decodes requires extending the prefilter marker set in the same change, or it stops being a strict superset.
...
`bin/ap-cd-command-policy.mjs` imports the shell tokenizer and command-position analysis (`Lexer`, `splitProgram`, `commandPosition`) from `bin/ap-arm-command-policy.mjs`, the sole owner of autopilot's shell classification.
...
`bin/ap-arm-command-policy.mjs` runs its own CLI entry point only when invoked directly, never on import, so the two policies stay independent CLIs over one parser.
...
| Codex | `.codex/hooks.json` PreToolUse hook that anchors from `pwd -P`, verifies the hook-loaded autopilot root, and forwards the payload | Blocks on exit 2 and displays stderr. |
| Grok | `.grok/hooks/ap-primary-cd-check.json` PreToolUse hook anchored on `${GROK_WORKSPACE_ROOT:-}` | Consumes the stdout `decision=deny` object. |
| OpenCode | `.opencode/plugins/ap-primary-cd-check.js` `tool.execute.before` | Throws, which surfaces as the failed tool result. |
| Pi | `.pi/extensions/ap-primary-turnend-guard.ts` `tool_call` handler | Returns `{block: true}`; piggybacks on the already-loaded primary extension so no extra `-e` flag is needed. |
...
`tests/ap-cd-pretool-check.test.sh` owns the acceptance matrix.
...
The suite also proves the end-to-end cwd-leak regression (an autopilot-owned backlog write leaking into a project clone, then denied at the exact command), the checkout scoping (fires in a git-cloned copilot fixture, inert in a flight crew member/recon linked worktree, inert outside an autopilot checkout, inert outside a git repo), the fail-open transport behavior, the prefilter fast path, the policy CLI output contract, and the per-harness wiring.

Run:

```sh
bash -n bin/ap-cd-pretool-check.sh
shellcheck bin/ap-cd-pretool-check.sh tests/ap-cd-pretool-check.test.sh
node --check bin/ap-cd-command-policy.mjs
...
Each harness ran against a scratch primary-shaped autopilot checkout: a plain git repo with `AGENTS.md`, `bin/` holding the real `ap-cd-pretool-check.sh`, `ap-cd-command-policy.mjs`, and `ap-arm-command-policy.mjs` plus a no-op dummy `ap-arm-pretool-check.sh`, a `projects/foo/` stand-in clone, and the tracked harness hook config.
No live watcher, fleet state, or the pilot's real primary checkout was involved.
...
- **Claude Code 2.1.207** - blocked. Claude reported the top-level command "denied by the `PreToolUse` hook (`ap-cd-pretool-check.sh`)", the `BLOCKED` sentinel was absent, and the subshell form was permitted to run. A prior control `touch` proved the harness executed commands.
...
- **grok 0.2.93** - inconclusive live run: the Grok Build API returned `402 Payment Required: Grok Build usage balance exhausted`, so the model never issued the probe commands. The grok cd hook (`.grok/hooks/ap-primary-cd-check.json`) is structurally identical to the arm-seatbelt grok hook already live-validated on 2026-07-09 (`docs/arm-pretool-check.md`) - same `${GROK_WORKSPACE_ROOT:-}` anchoring and same PreToolUse deny consumption - and the grok-shaped stdin path (`.toolInput.command` in, `{"decision":"deny"}` out) is covered by `tests/ap-cd-pretool-check.test.sh`. Re-run once the Grok balance is restored to close the live gap.
