...
The flight crew member harness remains the agent process launched inside that endpoint.
Autopilot agents load [`autopilot-orca`](../.agents/skills/autopilot-orca/SKILL.md) before operating or recovering this backend.
...
Orca is macOS-only, explicit-only, and does not support copilot spawns.
...
Select Orca with local `config/backend` containing `orca`, `AP_BACKEND=orca` for one launch, or an explicit request to Autopilot.
It is never auto-detected.

Before any spawn mutates repository state, Autopilot requires `orca status --json` to report `reachable=true` and `state="ready"`.
...
Routine supervision uses the recorded endpoint through `bin/ap-peek.sh <id>` and `AP_HOME=<home> bin/ap-send.sh <id> '<text>'`.
Enter and Ctrl-C are supported; Escape is not.

## Task shape and metadata

Each task has one Orca-managed git worktree and one Orca terminal.
`ap-spawn.sh` does not call Treehouse for Orca tasks.
...
`window=` remains the caller-facing Autopilot alias.
...
`ap-peek.sh` reads with `orca terminal read`.
`ap-send.sh` types and verifies composer clearance, follows `oldestCursor` when Orca returns a limited page, and retries Enter without retyping when a slash popup first fills an argument placeholder.
...
Cleanup keeps all shared Autopilot safety checks.
A recon still requires its report and completed decision inventory.
A flight still refuses dirty or unlanded work.
...
After those checks, Autopilot closes the exact terminal and releases the exact worktree with Orca's worktree command.
...
- Copilot spawns are unsupported.
