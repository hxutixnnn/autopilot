...
The predicate lives in `bin/ap-turnend-guard.sh`.
Primary scope lives in `bin/ap-primary-scope-lib.sh`, shared with the native session-start nudge in [`sessionstart-nudge.md`](sessionstart-nudge.md).
...
`bin/ap-guard.sh` is a pull-based warning that runs only when another supervision command invokes it.
...
A copilot home runs its own primary Autopilot session, so a genuine `.ap-copilot-home` marker includes it whether the home is a linked worktree or plain clone.
...
That check keeps flight crew member and recon linked worktrees inert because their git dir differs from their git common dir.
It also requires `AGENTS.md`, `bin/`, and the effective state directory.

For an in-scope primary, the guard counts in-flight work from `state/*.meta`.
The default cross-harness mode exits silently with no work in flight.
Otherwise it calls `ap_watcher_healthy <state-dir> <watch-path> [grace-seconds] [home]` from `bin/ap-wake-lib.sh`, the same identity-matched lock and fresh-beacon check used by `bin/ap-watch-arm.sh`.
...
`AP_STATE_OVERRIDE` wins over `AP_HOME/state`, and `AP_HOME` wins over repository-root `state/`.
`AP_GUARD_GRACE` controls beacon freshness and defaults to 300 seconds.
If `jq` is missing or hook stdin is empty, the guard exits 0 because it cannot safely read loop-guard fields.

## Harness integrations

- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/ap-turnend-guard.sh --claude`, and `bin/ap-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies an Autopilot-shaped hook-bearing root, and passes the original payload to the shared guard.
- OpenCode listens for `session.idle` in `.opencode/plugins/ap-primary-turnend-guard.js`, lets the watcher coordinator act first, and calls `client.session.promptAsync` once when the guard returns 2.
- Pi listens for `agent_settled` in `.pi/extensions/ap-primary-turnend-guard.ts`, runs once per logical agent run, and calls `pi.sendUserMessage(..., { deliverAs: "followUp" })` once when the guard returns 2.
- Grok registers a `Stop` hook in `.grok/hooks/ap-primary-turnend-guard.json` and delegates capability selection to `bin/ap-turnend-guard-grok.sh`.
...
The Claude mode waits up to `AP_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds) and allows the stop when the watcher is healthy, `state/.claude-autoarm.lock` has a live owner, or `state/.claude-autoarm-epoch` contains a fresh rewake outcome.
When none of those proofs appears, it re-blocks up to `AP_CLAUDE_TURNEND_BLOCK_BUDGET` times (default 3, below Claude's 8-block override), then allows degraded with a visible `systemMessage`.
...
The generated prompts use the canonical `turn-end-guard` kind after the U+2063 `AUTOPILOT_OP: ` prefix, so Radio check does not treat them as pilot messages.
...
If a passive adapter cannot invoke its SDK, or the Grok legacy fallback cannot find `grok` or a session id, the next pull-based `ap-guard.sh` call reports the problem.
That warning uses `bin/ap-supervision-instructions.sh --repair-line`, so it always points to the active harness protocol rather than embedding another repair command.

## Compatibility limits

- Child flight crew member and recon worktrees are outside scope.
...
- Pilot-approved Kimi flight crew wake support uses `bin/ap-kimi-turnend-hook.sh` to edit only one marker-delimited Autopilot region in that global config and install a silent always-zero hook.
- The hook remains inert unless the payload `cwd` contains a per-task token pointer that resolves through Autopilot's private registry to one `state/<id>.turn-ended` marker.
...
`tests/ap-turnend-guard.test.sh` covers the predicate, main and copilot primary scope, child-worktree exclusion, `AP_HOME` and `AP_STATE_OVERRIDE` precedence, the cooperative `--claude` claim wait, epoch allow, re-block budget, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
`tests/ap-kimi-harness.test.sh` covers the separate Kimi flight crew hook's format preservation, idempotence, refusal cases, token guard, spawn registration, and teardown cleanup.
`tests/ap-supervision-instructions.test.sh` covers recovery-line ownership and pi-signed's identity-preserving reuse of Pi's protocol.
`AP_PI_LIVE_E2E=1 tests/ap-pi-primary-live-e2e.test.sh` is the opt-in isolated Pi path.
