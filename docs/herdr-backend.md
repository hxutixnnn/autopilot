...
Autopilot requires Herdr protocol 14 or newer; versions 0.7.1, 0.7.3, 0.7.4, and 0.7.5 are verified, with protocol-16 features enabled only when available.
Herdr provides the terminal session while Treehouse continues to provide task worktrees.
[`configuration.md`](configuration.md#runtime-backend-configbackend--ap_backend) owns shared backend selection and metadata semantics.
...
Autopilot invokes its CLI as a separate process.

Select Herdr with local `config/backend` containing `herdr`, `AP_BACKEND=herdr` for one launch, or an explicit request to Autopilot.
...
The required CI lane uses the pinned installers in `bin/ap-install-herdr.sh` and `bin/ap-install-treehouse.sh`.
...
The ordinary topology puts one task tab per endpoint in the exact workspace of the Autopilot or copilot that launches it.
When the launcher has no Herdr workspace to inherit, the adapter maintains one durable home-labeled workspace instead.
The primary home label is `autopilot`.
A copilot home label is `copilot-<copilot-id>`, derived from its validated `.ap-copilot-home` marker.
A copilot launched by the primary receives a narrowly scoped home override during container creation.

Attach to the selected named Herdr session and switch to the relevant home workspace to watch its task tabs.
Routine supervision uses `bin/ap-peek.sh <id>` and `AP_HOME=<home> bin/ap-send.sh <id> '<text>'` without attaching.
...
Herdr 0.7.5 exports `HERDR_ENV`, `HERDR_PANE_ID`, `HERDR_SESSION`, `HERDR_SOCKET_PATH`, `HERDR_TAB_ID`, and `HERDR_WORKSPACE_ID` into every process it manages a pane for, and an Autopilot or copilot agent's own commands inherit them.
Older injection shapes are unverified, so a claimed launcher pane without the injected socket identity cannot be trusted.
With presentation spaces disabled, a flight crew member or recon is created in the exact workspace that identity currently resolves to, read live from Herdr rather than from the injected snapshot, so the worker always appears beside the agent that launched it.
Duplicate labels elsewhere in the session are irrelevant, and the globally focused workspace is never the target.
A `--copilot` launch is the deliberate exception: it stands up that copilot home's own workspace instead of joining the launcher's.
...
Autopilot running outside Herdr entirely has no launcher workspace to inherit, so its workers use this home's own labeled workspace, created on first use.
...
Avoid naming a personal workspace `autopilot` or `copilot-<id>` for that reason, and because the adapter cannot distinguish that label collision from its own container.
An older copilot workspace using `autopilot-<id>` is not migrated automatically; rename it manually before expecting new tasks or recovery to use it.
...
Create local gitignored `config/herdr-presentation-spaces` to request a disposable one-task workspace for each new flight crew member or recon.
The setting is inherited into copilot homes through the normal configuration-convergence owner.
A copilot agent itself always stays in its ordinary parent workspace; only children launched by that home are eligible.
...
Autopilot atomically publishes a three-field version 1 journal containing a random 128-bit base64url token before asking Herdr to create anything.
...
The owning parent is the launcher's own exact workspace, resolved from the same identity the flat path uses, and falls back to a unique home-label lookup only for an Autopilot outside Herdr.
...
The normal `ap-<id>` task tab is created in the exact new workspace returned by Herdr.
Only the exact seeded default tab returned by the same workspace-create response can be pruned.
Before and after create, prune, order, abort cleanup, and normal cleanup, Autopilot verifies exact workspace, tab, pane, and active-focus ids.
...
Autopilot does not retry, adopt, reuse, close, delete, or rename anything in response to an unavailable method, lock contention, ambiguous socket, lost response, failed move, or verification mismatch.
...
Before any recovery mutation, Autopilot holds both the task spawn lock and the named-session presentation lock.
...
The reclaim path never moves, closes, deletes, or renames a workspace and never touches a parent, sibling, pilot, or foreign pane.
...
It runs only when the current home has at least one ordinary presentation journal and considers only that home; a primary never recursively sweeps a copilot home.
...
Before cleanup, Autopilot acquires the existing task-id spawn lock and then the shared named-session presentation lock.
...
Autopilot immediately revalidates the same journal, metadata absence, workspace title and token uniqueness, one-tab and one-pane topology, exact pane relationship, absent agent, process proof, and non-target focus before calling the existing exact-pane focus-preserving close helper.
...
- Spaces have no cross-home cleanup path, and a copilot child can clean up only from its exact home.
...
`tests/ap-backend-herdr-presentation-e2e.test.sh` covers multi-home ordering, concurrency, lock contention, legacy coexistence, focus preservation, exact same-identity restart replacement, ambiguous bindings and tokens, and exact-pane cleanup through the guarded lab path.
`tests/ap-herdr-session-cleanup.test.sh` covers every discovery, ownership, topology, process, locking, revalidation, focus, retirement, and continue-on-error boundary.
`tests/ap-herdr-session-cleanup-e2e.test.sh` covers the restored-shell cleanup in a guarded non-default named lab; [`verification/runtime-backends.md`](verification/runtime-backends.md#per-home-and-presentation-topology) owns the active versioned evidence.

## Default-tab prune safety

`herdr workspace create` seeds one default tab.
Autopilot prunes it only after a real task tab exists and only when the same create response supplied the seeded tab id.
An adopted workspace never supplies that id and can never enter the prune path, regardless of labels or tab count.
Immediately before close, Autopilot rechecks the exact tab, expected seed label, and native agent state.
A working seed pane is never closed.

This created-versus-adopted gate is a destructive safety boundary.
A prior label heuristic could adopt a pilot-owned workspace named `autopilot` and close its live seed-shaped tab.
The current structural gate removes label inference from cleanup authority.
`tests/ap-backend-herdr-prune-safety-e2e.test.sh` reproduces the collision in an isolated named session and proves the adopted pane remains untouched.
...
Every Herdr invocation goes through `ap_backend_herdr_cli`, which sets the environment and passes an explicit trailing `--session <name>`.
...
The shared flight-crew-state path therefore accepts a native `busy` as evidence of activity but never a native `idle` as evidence that a worker has stopped; the task's own semantic busy state (`bin/ap-busy-lib.sh`) decides that.
...
`bin/ap-composer-lib.sh` is the fleet-wide owner that strips dim or faint runs and dark truecolor placeholders while retaining bright typed input.
...
The current operational envelope starts with U+2063 and `AUTOPILOT_OP: `.
The separate routed-request carrier uses `[ap-from-autopilot]` plus U+2063.
...
`bin/ap-operational-input.sh` owns current operational construction and parsing, and the AFK skill owns legacy away-input compatibility.
...
Mid-session copilot liveness is not implemented because idle copilots are deliberately exempt from stale-pane escalation and need a separate periodic identity signal.

## Push events and polling fallback

Protocol 16 can subscribe to `pane.agent_status_changed` over one bounded Unix-socket reader.
`bin/ap-transition-lib.sh` owns the backend-neutral transition vocabulary and policy.
...
The watcher maps the pane back to the task and skips copilot endpoints and declared `paused:` waits.
...
`tests/ap-backend-herdr-eventwait-smoke.test.sh`, `tests/ap-transition-lib.test.sh`, and `tests/ap-supervision-events.test.sh` cover capability, subscribe-then-reconcile ordering, dedupe, exemptions, and polling fallback.
...
`bin/ap-afk-launch.sh` therefore creates a dedicated unfocused Herdr workspace, runs the daemon there with an explicit supervisor target and backend, records the exact daemon pane, and closes only that pane on stop.
It never splits the pilot's active tab and never uses shell `&`.
...
Never use ambient `herdr server stop` for Autopilot verification.
...
`bin/ap-herdr-lab.sh` is the sole supported lifecycle helper for isolated verification.
It provisions only non-default names beginning with `ap-lab-`, appends an explicit `--session` to allowed task commands, refuses caller-supplied session flags and server/session lifecycle subcommands, and performs destructive stop/delete only through its guarded lifecycle actions.
...
- A Autopilot outside Herdr cannot resolve a launcher workspace, so a colliding home label refuses new spawns until the collision is cleared.
- Ghost and placeholder recognition depends on ANSI de-emphasis and fails safely to pending when unavailable.
- Mid-session copilot liveness is not implemented.
...
tests/ap-backend-herdr-smoke.test.sh
...
tests/ap-herdr-session-cleanup.test.sh
tests/ap-herdr-session-cleanup-e2e.test.sh
tests/ap-afk-inject-herdr-e2e.test.sh
