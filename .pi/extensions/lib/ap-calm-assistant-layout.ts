...
// if it is missing; ap-calm.ts catches that and skips only this adapter with a diagnostic
...
const CALM_ASSISTANT_LAYOUT_PATCH = Symbol.for(
  "autopilot:calm-assistant-layout:pi-0.81.1",
);
...
export function installCalmAssistantLayout(): void {
  ...
  if (typeof AssistantMessageComponent !== "function") {
    throw new Error("Autopilot Calm requires Pi AssistantMessageComponent");
  }
  const originalUpdateContent = AssistantMessageComponent.prototype.updateContent;
  if (typeof originalUpdateContent !== "function") {
    throw new Error("Autopilot Calm requires Pi AssistantMessageComponent.updateContent");
  }
