...
// and throws if it is missing; ap-calm.ts catches that and skips only this adapter with a
...
import { calmPresentationHides } from "./ap-calm-visibility.ts";
import { classifyAutopilotCurrentOperationalText } from "./ap-operational-input.ts";
...
const CALM_OPERATIONAL_USER_LAYOUT_PATCH = Symbol.for(
  "autopilot:calm-operational-user-layout:pi-0.81.1",
);
...
export function installCalmOperationalUserLayout(): void {
  ...
  const isOperationalInput = (text: string): boolean => {
    ...
    return (
      classifyAutopilotCurrentOperationalText(text) !== undefined ||
  ...
  if (typeof InteractiveMode !== "function") {
    throw new Error("Autopilot Calm requires Pi InteractiveMode");
  }
  ...
  if (typeof originalAddMessageToChat !== "function") {
    throw new Error("Autopilot Calm requires Pi InteractiveMode.addMessageToChat");
  }

  const UserMessageComponent = PiCodingAgent.UserMessageComponent;
  if (typeof UserMessageComponent !== "function") {
    throw new Error("Autopilot Calm requires Pi UserMessageComponent");
  }
