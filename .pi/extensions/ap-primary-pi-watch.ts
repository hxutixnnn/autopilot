// Autopilot primary watcher bridge for Pi.
//
...
import {
  ...
  AUTOPILOT_CALM_PRESENTATION_EVENT,
} from "./lib/ap-calm-visibility.ts";
...
const autopilotHome = process.env.AP_HOME || process.env.AP_ROOT_OVERRIDE || root;
const autopilotRoot = process.env.AP_ROOT_OVERRIDE || root;
const state = process.env.AP_STATE_OVERRIDE || `${autopilotHome}/state`;
const config = process.env.AP_CONFIG_OVERRIDE || `${autopilotHome}/config`;
const armScript = `${autopilotRoot}/bin/ap-watch-arm.sh`;
const marker = `${state}/.pi-watch-extension-loaded`;
const extensionVersion = `sha256:${createHash("sha256").update(readFileSync(extensionFile)).digest("hex")}`;
const retryBaseMs = positiveInteger("AP_WATCH_REARM_RETRY_BASE_MS", 250);
const retryMaxMs = positiveInteger("AP_WATCH_REARM_RETRY_MAX_MS", 4000);
const retryLimit = positiveInteger("AP_WATCH_REARM_RETRY_LIMIT", 5);
// 35s on Windows so the budget stays above arm's MSYS confirm default (30s in
// bin/ap-watch-arm.sh): a slow but successful Git Bash cold start must not be
// SIGTERMed mid-confirmation. Conditioned on win32 so other platforms keep 12s.
const armReadyTimeoutMs = positiveInteger(
  "AP_PI_ARM_READY_TIMEOUT_MS",
  process.platform === "win32" ? 35000 : 12000,
);
const armRetireTimeoutMs = positiveInteger("AP_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call ap_watch_arm_pi again only after a later notification says the cycle is missing, failed, or unhealthy";
...
function classifyClose(stdout: string, stderr: string, code: number | null, signal: NodeJS.Signals | null): CloseClassification {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - ap-watch-arm.sh exited ${code}${combined ? `\n${combined}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.events?.on?.(AUTOPILOT_CALM_PRESENTATION_EVENT, (data) => {
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    const ownership = lockOwnership();
    if (ownership === "other") return { ok: false, message: "watcher: read-only - session lock is held by another autopilot session" };
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/ap-session-start.sh to reclaim it, then call ap_watch_arm_pi to re-arm",
      };
    }
    markLoaded();
    if (owner.child) {
    ...
    const env = {
      ...process.env,
      AP_HOME: autopilotHome,
      AP_ROOT_OVERRIDE: autopilotRoot,
      AP_CONFIG_OVERRIDE: config,
      AP_WATCH_ARM_SCRIPT: armScript,
      AP_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
    };
    const armChild = spawn("bash", ["-lc", "exec \"$AP_WATCH_ARM_SCRIPT\" --restart"], {
      cwd: autopilotRoot,
      env,
  ...
  pi.registerCommand?.("ap-watch-arm-pi", {
    description: "Arm autopilot watcher supervision through the Pi extension instead of foreground bash.",
  ...
  pi.registerTool?.({
    name: "ap_watch_arm_pi",
    label: "Arm autopilot watcher",
    description: "Start the first required Pi watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the Pi extension re-arms automatically. Never run bin/ap-watch-arm.sh through bash.",
    ...
    promptGuidelines: [
      "Call ap_watch_arm_pi only for the first required cycle or after a notification says the cycle is missing, failed, or unhealthy. Do not call it after ordinary work, turn completion, or ordinary signal, stale, check, or heartbeat handling because the Pi extension owns re-arming. Never run bin/ap-watch-arm.sh through bash.",
    ],
    ...
    renderCall: (_args, theme, context) => {
      ...
      if (calmPresentation.stockExportRendering) {
        return new Text(theme.fg("toolTitle", theme.bold("ap_watch_arm_pi")), 0, 0);
      }
      const state = context.state as WatchToolShellState;
      state.call = new Text(theme.fg("toolTitle", theme.bold("ap_watch_arm_pi")), 0, 0);
