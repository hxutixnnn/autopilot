// Autopilot's home-persistent Pi transcript presentation toggle.
//
...
// ./lib/ap-calm-working-flight.ts owns the animated working presentation this file
...
import { installCalmAssistantLayout } from "./lib/ap-calm-assistant-layout.ts";
import { installCalmOperationalUserLayout } from "./lib/ap-calm-operational-user-layout.ts";
import {
  CALM_WORKING_FLIGHT_WIDGET_KEY,
  createCalmWorkingFlightAnimation,
  createCalmWorkingFlightWidget,
} from "./lib/ap-calm-working-flight.ts";
import {
  calmPresentationHides,
  calmPresentationIsActive,
  AUTOPILOT_CALM_PRESENTATION_EVENT,
  registerAutopilotSyntheticPresentation,
  setCalmPresentation,
...
function installCalmPresentationAdapter(name: string, install: () => void): void {
  try {
    ...
    console.error(`Autopilot Calm: ${name} presentation adapter unavailable, skipping. ${reason}`);
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  // from turns or tool calls, so the airplane never flickers between tool calls, automatic
  // continuations, retries, or compaction that stay inside the same run.
  let agentRunActive = false;
  let workingFlightShown = false;
  ...
  const workingFlightAnimation = createCalmWorkingFlightAnimation();
  ...
  const applyWorkingPresentation = (
    ...
    const showFlight = agentRunActive && calmPresentationIsActive();
    if (showFlight !== workingFlightShown) {
      workingFlightShown = showFlight;
      ui.setWidget(
        CALM_WORKING_FLIGHT_WIDGET_KEY,
        showFlight
          ? (tui) => createCalmWorkingFlightWidget(tui, workingFlightAnimation)
          : undefined,
      );
      ui.setWorkingVisible(!showFlight);
    } else if (forceStockVisibility && !showFlight) {
      ui.setWorkingVisible(true);
    }
  };

  const autopilotHome = process.env.AP_HOME || process.env.AP_ROOT_OVERRIDE || root;
  const configDirectory = process.env.AP_CONFIG_OVERRIDE || resolve(autopilotHome, "config");
  ...
  const publishPresentationState = (): void => {
    pi.events.emit(AUTOPILOT_CALM_PRESENTATION_EVENT, {
      active: calmPresentationIsActive(),
      stockExportRendering: exportRendering,
    });
  };

  registerAutopilotSyntheticPresentation(pi);
  ...
  function registerBuiltIn<TParams extends TSchema, TDetails, TState>(
    ...
    if (!originalRenderCall || !originalRenderResult) {
      throw new Error(`Autopilot calm mode requires both render slots for Pi built-in tool ${original.name}`);
    }
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    workingFlightShown = false;
    // A genuine new session lifetime starts the airplane at the normal initial position.
    workingFlightAnimation.reset();
    applyWorkingPresentation(ctx.ui, true);
    ctx.ui.setHiddenThinkingLabel(calmPresentationIsActive() ? "" : undefined);
    ctx.ui.setStatus("autopilot-calm", undefined);
  ...
  pi.registerCommand("calm", {
    description: "Toggle Autopilot's supported conversation-only transcript presentation.",
