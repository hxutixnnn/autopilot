...
import { encodeAutopilotOperationalInput } from "./lib/ap-operational-input.js";

const COORDINATOR_KEY = "__autopilotOpenCodeWatchArm";
...
const ARM_READY_TIMEOUT_MS = positiveInteger("AP_OPENCODE_ARM_READY_TIMEOUT_MS", ARM_READY_TIMEOUT_DEFAULT_MS);
const ARM_RETIRE_TIMEOUT_MS = positiveInteger("AP_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const REARM_RETRY_BASE_MS = positiveInteger("AP_WATCH_REARM_RETRY_BASE_MS", 250);
const REARM_RETRY_MAX_MS = positiveInteger("AP_WATCH_REARM_RETRY_MAX_MS", 4000);
const REARM_RETRY_LIMIT = positiveInteger("AP_WATCH_REARM_RETRY_LIMIT", 5);
...
function effectivePaths(root) {
  ...
  const autopilotHome = process.env.AP_HOME || process.env.AP_ROOT_OVERRIDE || autopilotRoot;
  ...
  return { root: autopilotRoot, home: autopilotHome, state, config };
}

async function isPrimaryRoot(root, home) {
  if (!root) return false;
  if (!existsSync(`${root}/AGENTS.md`) || !existsSync(`${root}/bin`)) return false;
  if (existsSync(`${root}/.ap-copilot-home`)) return false;
  if (home && home !== root && existsSync(`${home}/.ap-copilot-home`)) return false;
...
function classifyArmClose(stdout, stderr, code, signal) {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - ap-watch-arm.sh exited ${code}${combined.trim() ? `\n${combined.trim()}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
async function sendPrompt(paths, client, sessionID, text) {
  const encoded = await encodeAutopilotOperationalInput(paths.root, "watcher", text);
...
function wakePrompt(reason) {
  return `WATCHER FIRED - drain queued wakes with bin/ap-wake-drain.sh and handle the reported wake. Watcher continuity is plugin-owned.\n\n${reason}`;
}
...
function spawnArm(paths, sessionID, client, predecessorArmPid = "") {
  ...
  const env = {
    ...process.env,
    AP_HOME: paths.home,
    AP_ROOT_OVERRIDE: paths.root,
    AP_CONFIG_OVERRIDE: paths.config,
    AP_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
  };
  const armChild = spawn("bash", ["-lc", 'exec "$AP_ROOT_OVERRIDE/bin/ap-watch-arm.sh" --restart'], {
    cwd: paths.root,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
...
export const AutopilotPrimaryWatchArm = async ({ client, directory, worktree }) => {
