...
// ap-primary-watch-arm.js (a plugin-owned child process, never a model tool
// call), so the residual risk here is the AGENT shelling `bin/ap-watch-arm.sh`
// wrong through its own bash tool - the anti-pattern bin/ap-arm-pretool-check.sh
...
export const AutopilotPrimaryPretoolCheck = async ({ directory, worktree }) => {
  const root = worktree ? (() => {
    try {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const result = await runProcess(`${root}/bin/ap-arm-pretool-check.sh`, ["--command", command]);
