...
// the primary autopilot checkout before the agent's bash tool relocates the
// shell out of the home (see bin/ap-cd-pretool-check.sh and docs/cd-guard.md).
// This mirrors ap-primary-pretool-check.js, calling the cd-guard owner instead
...
// real primary checkout, so a flight crew member/recon worktree is never affected.
...
export const AutopilotPrimaryCdCheck = async ({ directory, worktree }) => {
  const root = worktree ? (() => {
    try {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const result = await runProcess(`${root}/bin/ap-cd-pretool-check.sh`, ["--command", command]);
