...
AP_ROOT="${AP_ROOT_OVERRIDE:-$REPO_ROOT}"
...
usage() {
  cat <<'EOF'
Usage: ap-supervision-instructions.sh [--harness <name>] [--read-only 0|1] [--afk 0|1] [--repair-line] [--queue-pending 0|1]
...
if [ -z "$HARNESS" ]; then
  HARNESS=$("$SCRIPT_DIR/ap-harness.sh" 2>/dev/null || printf unknown)
fi
...
checkpoint_seconds=${AP_CODEX_WATCH_CHECKPOINT:-180}
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    line=${line//__AP_PI_EXT__/$pi_ext}
    line=${line//__AP_PI_TURNEND_EXT__/$pi_turnend_ext}
    printf '%s\n' "$line"
  done < "$SNIPPET"
}
...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/ap-watch-arm.sh as its own Claude Code background task, never shell &.'
      ;;
    codex)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/ap-watch-checkpoint.sh --seconds ' "$checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool ap_watch_arm_pi, or restart Pi with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    opencode)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision by letting the OpenCode TUI plugin arm after idle; use bin/ap-watch-arm.sh only as a manual recovery probe if the plugin reports failure.'
      ;;
    grok)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/ap-watch-arm.sh as its own Grok tracked background task, never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      printf '%s\n' '- Ordinary wake: the Stop-owned auto-arm (bin/ap-claude-stop-autoarm.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: take the next foreground bin/ap-watch-checkpoint.sh checkpoint as directed below.'
      ;;
    pi|pi-signed)
    ...
    opencode)
      ...
      ;;
    grok)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/ap-watch-arm.sh Grok tracked background task as directed below.'
      ;;
    *)
