...
#
# An autopilot primary that delegates through a harness's own delegation,
# scheduling, or background-work tool creates work with no `state/<id>.meta` and
# no `data/<id>/brief.md`. Only `bin/ap-spawn.sh` writes that metadata, and
# untracked project work contributes nothing to the in-flight predicate in
# bin/ap-supervision-lib.sh or bin/ap-turnend-guard.sh. Such work therefore makes
# the guard stack structurally inert, and it dies with the primary session instead
...
# worktrees where they disarm legitimate flight crew members.
...
# tool that appears before anyone updates a local deny list is still refused.
#
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/ap-subagent-pretool-check.sh
#   bin/ap-subagent-pretool-check.sh --tool '<tool-name>'
#
...
#
# Exit/output contract (identical shape to bin/ap-cd-pretool-check.sh):
#   ALLOW - exit 0 and no output.
#   DENY - exit 2, a Claude-shaped deny object on stderr, and a Grok-shaped
#          deny object on stdout unless --claude was supplied.
#   INERT - not a genuine primary home (a flight crew member/recon task worktree or a
#           non-autopilot repo): exit 0 with no output, exactly like ALLOW.
#   ESCAPE - AP_ALLOW_SUBAGENT=1 in the environment allows deliberately.
...
# work, an agent, a schedule, or an isolated workspace that autopilot would not
...
# and starts nothing that could outlive the session or escape an autopilot
# guard. Denying them stops the primary tracking its own plan while granting no
# delegation power, and the deny text would tell it to run bin/ap-brief.sh for a
...
usage() {
  cat <<'EOF'
Usage: ap-subagent-pretool-check.sh [--tool <tool-name>] [--claude]
...
Do not include that Claude-only list in tracked project settings, because linked
worktrees inherit it and legitimate flight crew members would lose their delegation tools.
This hook remains as the delivered guard for future delegation-shaped names
outside any local fixed list.
Fires only in a genuine autopilot primary home; it is a silent no-op in a
flight crew member/recon task worktree or any non-autopilot repo, where a worker using
delegation tools is legitimate.
Exits 0 to allow and 2 to deny, naming the real flight crew member dispatch path instead.
Set AP_ALLOW_SUBAGENT=1 in the session environment to allow deliberately.
...
[ "${AP_ALLOW_SUBAGENT:-}" != "1" ] || exit 0

SCRIPT_DIR=$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P) || exit 0
AP_ROOT=${AP_ROOT_OVERRIDE:-$(CDPATH='' cd -- "$SCRIPT_DIR/.." 2>/dev/null && pwd -P)} || exit 0
...
# turn-end guard do. ap_primary_scope_matches accepts a plain checkout or a
# marked copilot home - both operate a fleet and must dispatch through it -
# and rejects a linked task worktree, which is the shape bin/ap-spawn.sh always
# hands a flight crew member. A flight crew member using delegation tools inside its own task
...
if [ -f "$AP_ROOT/bin/ap-recon.sh" ]; then
  ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a recon goes to bin/ap-recon.sh "<question>" [project], while authorized flight work and its bounded research go to bin/ap-brief.sh then bin/ap-spawn.sh'
else
  ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/ap-brief.sh followed by bin/ap-spawn.sh for dispatched work'
fi

REASON="[subagent-dispatch] the autopilot primary dispatches through the fleet, not the harness's own delegation tools: work started that way has no durable fleet record, leaves every autopilot guard inert, and dies with this session. Instead, $ROUTE (blocked tool: $TOOL, delegation-shaped on \"$MATCHED\"). Launch the session with AP_ALLOW_SUBAGENT=1 for a deliberate exception."
