#!/usr/bin/env bash
# ap-install-treehouse.sh - install CI's pinned, verified Treehouse build.
#
...
# ap-install-herdr.sh: official release URL, exact asset, SHA-256, bounded
# download, post-install version check. Never a floating package-manager latest.
#
# Usage:
#   ap-install-treehouse.sh <destination-directory>
#
# Pins Treehouse v2.0.1, the version exercised by the local real-Herdr suite.
set -eu

AP_TREEHOUSE_CI_VERSION=2.0.1
AP_TREEHOUSE_CI_TAG="v${AP_TREEHOUSE_CI_VERSION}"
# Bounded download ceiling (bytes). Official 2.0.1 archives are under 8 MiB.
AP_TREEHOUSE_CI_MAX_BYTES=15000000
AP_TREEHOUSE_CI_REPO=kunchenguid/treehouse

die() {
  printf 'ap-install-treehouse.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: ap-install-treehouse.sh <destination-directory>}

os=$(uname -s)
arch=$(uname -m)
case "${os}-${arch}" in
  Linux-x86_64)
    ARCHIVE=treehouse-v${AP_TREEHOUSE_CI_VERSION}-linux-amd64.tar.gz
    SHA256=1d5a32751ab921670103fd201ddb2b91b47338cb13976f45642b827cf8976af2
    ;;
  Linux-aarch64|Linux-arm64)
    ARCHIVE=treehouse-v${AP_TREEHOUSE_CI_VERSION}-linux-arm64.tar.gz
    SHA256=eaccc9c5b98125df8bd77425598eeecee66cb0371db4eb1cf75f0d813c18fab9
    ;;
  Darwin-arm64)
    ARCHIVE=treehouse-v${AP_TREEHOUSE_CI_VERSION}-darwin-arm64.tar.gz
    SHA256=7ee5078f3d1f33c01196548797fce65408e459d53530b77d4ba56e074fa1c1a2
    ;;
  Darwin-x86_64)
    ARCHIVE=treehouse-v${AP_TREEHOUSE_CI_VERSION}-darwin-amd64.tar.gz
...
URL="https://github.com/${AP_TREEHOUSE_CI_REPO}/releases/download/${AP_TREEHOUSE_CI_TAG}/${ARCHIVE}"
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/ap-treehouse.XXXXXX")
trap 'rm -rf "$TMP"' EXIT

printf 'ap-install-treehouse.sh: downloading %s from %s\n' "$ARCHIVE" "$URL" >&2
curl -fsSL --max-filesize "$AP_TREEHOUSE_CI_MAX_BYTES" "$URL" -o "$TMP/$ARCHIVE" \
  || die "download failed for $URL (bounded at $AP_TREEHOUSE_CI_MAX_BYTES bytes)"
...
if [ -f "$TMP/treehouse" ]; then
  BIN="$TMP/treehouse"
elif [ -f "$TMP/treehouse-v${AP_TREEHOUSE_CI_VERSION}/treehouse" ]; then
  BIN="$TMP/treehouse-v${AP_TREEHOUSE_CI_VERSION}/treehouse"
else
...
case "$installed_version" in
  "v${AP_TREEHOUSE_CI_VERSION}"|"${AP_TREEHOUSE_CI_VERSION}") ;;
  *)
    die "installed treehouse version is '${installed_version:-<empty>}', expected exact pin v${AP_TREEHOUSE_CI_VERSION}"
    ;;
esac

printf 'ap-install-treehouse.sh: installed treehouse %s to %s\n' \
