...
//
// This file is the sole owner of autopilot's shell command classification.
...
// (bin/ap-cd-command-policy.mjs) reuses the same proven parser instead of
...
const REASONS = {
  ...
  "broad-watcher-kill": "a broad process kill targeting the autopilot watcher is forbidden",
  "unclassifiable-protected-command": "unsupported or malformed shell syntax contains a protected watcher command",
  "watcher-direct": "bin/ap-watch.sh must not be run directly; arm the watcher with bin/ap-watch-arm.sh or run bin/ap-watch-checkpoint.sh instead",
};
...
function rawMentionsProtected(command) {
  return /(?:^|[/\s'"`(])ap-watch(?:-(?:arm|checkpoint))?\.sh\b/.test(normalizeLineContinuations(command));
}

function rawMentionsBroadKill(command) {
  const normalized = normalizeLineContinuations(command);
  return /ap-watch/.test(normalized) && /\b(?:pkill|kill)\b/.test(normalized);
}
...
const PROTECTED_SCRIPTS = [
  { relative: "bin/ap-watch-arm.sh", kind: "arm" },
  { relative: "bin/ap-watch-checkpoint.sh", kind: "checkpoint" },
  { relative: "bin/ap-watch.sh", kind: "watch" },
];
...
function hasUnclassifiableProtectedExpansion(word, root) {
  ...
  return /(?:^|\/)ap-watch/.test(word.value);
}
...
function contextWithAssignments(context, words) {
  ...
  for (const word of words) {
    ...
    if (/ap-watch/.test(value) || wordReferencesAny(word, watcherPatterns)) watcherPatterns.add(name);
...
function isWatcherPgrep(position, context) {
  ...
  return position.words.slice(position.index + 1).some((word) => /(?:^|\/)ap-watch(?:\.sh)?\b/.test(word.value) || wordReferencesAny(word, context.watcherPatterns));
}
...
function analyzeProgram(command, context, depth = 0) {
  ...
  for (const tokens of program.nodes) {
    ...
    if (commandName === "pkill" && args.some((word) => /ap-watch/.test(word.value) || wordReferencesAny(word, nodeContext.watcherPatterns))) broadKill = true;
...
// Run the CLI only when invoked directly (node ap-arm-command-policy.mjs ...),
// never when imported by a sibling policy such as bin/ap-cd-command-policy.mjs.
