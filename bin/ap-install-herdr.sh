#!/usr/bin/env bash
# ap-install-herdr.sh - install CI's pinned, verified Herdr build.
#
...
#
# Usage:
#   ap-install-herdr.sh <destination-directory>
#
...
AP_HERDR_CI_VERSION=0.7.4
AP_HERDR_CI_TAG="v${AP_HERDR_CI_VERSION}"
AP_HERDR_CI_MIN_PROTOCOL=16
# Bounded download ceiling (bytes). The largest official 0.7.4 asset is under 20 MiB.
AP_HERDR_CI_MAX_BYTES=25000000
AP_HERDR_CI_REPO=ogulcancelik/herdr

die() {
  printf 'ap-install-herdr.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: ap-install-herdr.sh <destination-directory>}
...
URL="https://github.com/${AP_HERDR_CI_REPO}/releases/download/${AP_HERDR_CI_TAG}/${ASSET}"
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/ap-herdr.XXXXXX")
trap 'rm -rf "$TMP"' EXIT

printf 'ap-install-herdr.sh: downloading %s from %s\n' "$ASSET" "$URL" >&2
# --fail: HTTP errors; --location: follow redirects; --max-filesize: bound.
curl -fsSL --max-filesize "$AP_HERDR_CI_MAX_BYTES" "$URL" -o "$TMP/$ASSET" \
  || die "download failed for $URL (bounded at $AP_HERDR_CI_MAX_BYTES bytes)"
...
[ "$installed_version" = "$AP_HERDR_CI_VERSION" ] \
  || die "installed herdr version is '${installed_version:-<empty>}', expected exact pin $AP_HERDR_CI_VERSION"
...
[ "$protocol" -ge "$AP_HERDR_CI_MIN_PROTOCOL" ] \
  || die "herdr protocol $protocol is below the required floor $AP_HERDR_CI_MIN_PROTOCOL"

printf 'ap-install-herdr.sh: installed herdr %s (protocol %s) to %s\n' \
