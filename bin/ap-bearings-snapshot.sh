#!/usr/bin/env bash
# ap-bearings-snapshot.sh - compact, bounded, TOON-by-default bearings projection.
#
# A thin wrapper OVER the canonical bin/ap-fleet-snapshot.sh. It does not parse
# fleet state itself: it shells out to `ap-fleet-snapshot.sh --json`, projects that
...
# backlog roles, unresolved blockers, and pilot actionability. It never infers
...
# copilot_landed roll-up (ap-fleet-snapshot.sh), so merges a copilot managed -
...
# copilot home whose backlog was unreadable; no GitHub/network call is involved.
...
#   --all-copilots include every aggregated copilot record
...
#   -h,--help        usage
#
# Output contract: `ap-bearings.v1`. Read-only; no locks, no mutation, no reports.
set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FLEET="$SCRIPT_DIR/ap-fleet-snapshot.sh"

# Bounds (overridable for tests / large fleets).
AP_BEARINGS_LANDED=${AP_BEARINGS_LANDED:-6}
AP_BEARINGS_LANDED_PER_HOME=${AP_BEARINGS_LANDED_PER_HOME:-$AP_BEARINGS_LANDED}
AP_BEARINGS_IN_FLIGHT=${AP_BEARINGS_IN_FLIGHT:-20}
AP_BEARINGS_DECISIONS=${AP_BEARINGS_DECISIONS:-20}
AP_BEARINGS_COPILOTS=${AP_BEARINGS_COPILOTS:-20}
AP_BEARINGS_GATES=${AP_BEARINGS_GATES:-20}
AP_BEARINGS_REPORTS=${AP_BEARINGS_REPORTS:-20}
AP_BEARINGS_RECORDED_PRS=${AP_BEARINGS_RECORDED_PRS:-20}
AP_BEARINGS_UNHEALTHY=${AP_BEARINGS_UNHEALTHY:-20}
AP_BEARINGS_PR_REPOS=${AP_BEARINGS_PR_REPOS:-10}
AP_BEARINGS_PR_LIMIT=${AP_BEARINGS_PR_LIMIT:-20}
AP_BEARINGS_PR_TIMEOUT=${AP_BEARINGS_PR_TIMEOUT:-20}
case "$AP_BEARINGS_PR_TIMEOUT" in ''|*[!0-9]*|0) AP_BEARINGS_PR_TIMEOUT=20 ;; esac
validate_bound() {  # <name> <value>
  case "$2" in ''|*[!0-9]*|0) echo "ap-bearings-snapshot: $1 must be a positive integer" >&2; exit 2 ;; esac
}
validate_bound AP_BEARINGS_LANDED "$AP_BEARINGS_LANDED"
validate_bound AP_BEARINGS_LANDED_PER_HOME "$AP_BEARINGS_LANDED_PER_HOME"
validate_bound AP_BEARINGS_IN_FLIGHT "$AP_BEARINGS_IN_FLIGHT"
validate_bound AP_BEARINGS_DECISIONS "$AP_BEARINGS_DECISIONS"
validate_bound AP_BEARINGS_COPILOTS "$AP_BEARINGS_COPILOTS"
validate_bound AP_BEARINGS_GATES "$AP_BEARINGS_GATES"
validate_bound AP_BEARINGS_REPORTS "$AP_BEARINGS_REPORTS"
validate_bound AP_BEARINGS_RECORDED_PRS "$AP_BEARINGS_RECORDED_PRS"
validate_bound AP_BEARINGS_UNHEALTHY "$AP_BEARINGS_UNHEALTHY"
validate_bound AP_BEARINGS_PR_REPOS "$AP_BEARINGS_PR_REPOS"
validate_bound AP_BEARINGS_PR_LIMIT "$AP_BEARINGS_PR_LIMIT"

usage() {
  cat <<'EOF'
usage: ap-bearings-snapshot.sh [--json] [--include-prs] [--fields <list>]
                               [--all-in-flight] [--all-decisions]
                               [--all-copilots] [--all-landed]
                               [--all-reports] [--all-queued]
                               [--all-recorded-prs] [--all-unhealthy]
                               [--all-pr-repos]

Compact bearings projection over ap-fleet-snapshot.sh. TOON by default.
...
Default fields: schema, home, generated, prs, in_flight{id,kind,state,doing},
  copilots{id,state,doing,provenance,freshness,age_seconds,contradiction,reason},
  decisions_open{id,key,verb,summary,owner}, landed{id,what,artifact,owner},
  gates{id,title,blocked_by,reason,owner}, reports{id,path}, recorded_prs{id,url},
  unhealthy_endpoints{...} (only when non-empty), omitted{surface,reveal}.
landed merges this home's Done with registered copilot homes' Done, bounded by
  a per-home cap (AP_BEARINGS_LANDED_PER_HOME) and an overall cap (AP_BEARINGS_LANDED),
...
For every registered copilot, readable structured facts from its own home are
...
Opt-in surfaces: --fields bodies|paths|actions|endpoints, --all-in-flight,
  --all-decisions, --all-copilots, --all-landed, --all-reports, --all-queued, --all-recorded-prs,
  --all-unhealthy, --all-pr-repos, --include-prs (adds candidate_prs).
Raise AP_BEARINGS_PR_LIMIT to expand per-repository open-PR results.
EOF
}
...
command -v jq >/dev/null 2>&1 || { echo "ap-bearings-snapshot: jq not found" >&2; exit 1; }

# The deterministic return-catch-up owner must clear before this or any other
# ordinary pilot request proceeds. Bearings does not reproduce that policy;
# it only consults the shared read-only gate.
"$SCRIPT_DIR/ap-afk-return.sh" guard || exit $?

NOW=${AP_BEARINGS_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ "$ALL_LANDED" = 1 ] || [ "$ALL_COPILOTS" = 1 ]; then
  if [ "$ALL_LANDED" = 1 ]; then
    SNAP=$(AP_SNAPSHOT_NOW="$NOW" AP_SNAPSHOT_COPILOTS=0 AP_SNAPSHOT_COPILOT_LANDED_PER_HOME=0 "$FLEET" --json) || exit $?
  else
    SNAP=$(AP_SNAPSHOT_NOW="$NOW" AP_SNAPSHOT_COPILOTS=0 "$FLEET" --json) || exit $?
  fi
else
  SNAP=$(AP_SNAPSHOT_NOW="$NOW" "$FLEET" --json) || exit $?
fi
HOME_LABEL=$(printf '%s' "$SNAP" | jq -er '.ap_home | strings | split("/") | (.[-2:] | join("/"))') \
  || { echo "ap-bearings-snapshot: invalid canonical snapshot" >&2; exit 1; }
...
gh_bounded() {  # <args...>
  if command -v timeout >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 timeout "$AP_BEARINGS_PR_TIMEOUT" gh "$@"
  elif command -v gtimeout >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 gtimeout "$AP_BEARINGS_PR_TIMEOUT" gh "$@"
  elif command -v perl >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 perl -e 'my $t = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0); exec @ARGV } local $SIG{ALRM} = sub { kill "TERM", -$pid; select undef, undef, undef, 0.2; kill "KILL", -$pid; exit 124 }; alarm $t; waitpid $pid, 0; exit($? >> 8)' "$AP_BEARINGS_PR_TIMEOUT" gh "$@"
  else
    return 124
  fi
}
...
$(printf '%s' "$SNAP" | jq -r '.tasks[] | select(.kind != "copilot") | .paths.worktree.path // empty')
EOF

    for repo in $repos; do PR_REPOS_TOTAL=$((PR_REPOS_TOTAL + 1)); done
    nrepos=0; npr=0; nwarn=0; ncapped=0; rows='[]'
    pr_fetch_limit=$((AP_BEARINGS_PR_LIMIT + 1))
    for repo in $repos; do
      if [ "$ALL_PR_REPOS" != 1 ] && [ "$nrepos" -ge "$AP_BEARINGS_PR_REPOS" ]; then break; fi
      nrepos=$((nrepos + 1))
      ...
      [ -n "$out" ] || out='[]'
      repo_result=$(printf '%s' "$out" | jq --arg repo "$repo" --argjson limit "$AP_BEARINGS_PR_LIMIT" '
        [ .[] | {
          num:(.number|tostring),
          repo:$repo,
          task:(if (.headRefName // "" | startswith("ap/")) then (.headRefName | ltrimstr("ap/")) else "-" end),
          url:(.url // "-"),
      ...
      [ "$returned" -gt "$AP_BEARINGS_PR_LIMIT" ] && ncapped=$((ncapped + 1))
      npr=$((npr + cnt))
...
# --- projection: canonical snapshot -> ap-bearings.v1 model (JSON) ----------
MODEL=$(printf '%s' "$SNAP" | jq \
  --arg home "$HOME_LABEL" \
  --arg now "$NOW" \
  --arg prs "$PR_STATUS" \
  --arg fields "$FIELDS" \
  --argjson landed_n "$AP_BEARINGS_LANDED" \
  --argjson landed_per_home_n "$AP_BEARINGS_LANDED_PER_HOME" \
  --argjson in_flight_n "$AP_BEARINGS_IN_FLIGHT" \
  --argjson decisions_n "$AP_BEARINGS_DECISIONS" \
  --argjson copilots_n "$AP_BEARINGS_COPILOTS" \
  --argjson gates_n "$AP_BEARINGS_GATES" \
  --argjson reports_n "$AP_BEARINGS_REPORTS" \
  --argjson recorded_prs_n "$AP_BEARINGS_RECORDED_PRS" \
  --argjson unhealthy_n "$AP_BEARINGS_UNHEALTHY" \
  --argjson include_prs "$INCLUDE_PRS" \
  --argjson all_in_flight "$ALL_IN_FLIGHT" \
  --argjson all_decisions "$ALL_DECISIONS" \
  --argjson all_copilots "$ALL_COPILOTS" \
  ...
  | ((.copilot_landed.records) // []) as $copilot_done
  | ($main_done + $copilot_done) as $all_landed_rows
  ...
  | ([.tasks[] | select(.kind != "copilot") | .id]) as $live_ids
  | ([.tasks[] | select(.kind != "copilot" and .current_state.state == "working") | .id]) as $working_ids
  ...
  | ([ .tasks[]
     ...
     + [ (.copilot_current.records // [])[] as $m | $m.endpoints[]?
         | select(.endpoint.exists == false or .endpoint.agent_alive == "dead")
         | {id:($m.id + "/" + .id),backend:"copilot-home",target:(.endpoint.target // "-"),exists:.endpoint.exists,agent:.endpoint.agent_alive} ]) as $unhealthy_all
  | ([ (.copilot_current.records // [])[]
       | ([.decisions_open[]? | select(.source == "backlog" and .verb == "pilot-hold")]) as $pilot_holds
       | ([.holds[]? | select(.source == "backlog")]) as $backlog_holds
       | . + {
           bearings_pilot_holds:$pilot_holds,
           bearings_holds:(if .current.state == "pilot_decision" then $backlog_holds else .holds end),
           bearings_state:(
             if .current.state == "pilot_decision" then
               if ($pilot_holds | length) > 0 then "pilot_decision"
  ...
  | ([ if .copilot_current.registry.available == false then
         {id:"(registry)",state:"unknown",doing:(.copilot_current.registry.reason // "Registered copilot table unavailable"),
          provenance:(.copilot_current.registry.provenance // "registered-table"),
          freshness:(.copilot_current.registry.freshness.status // "unavailable"),
          age_seconds:null,contradiction:false,reason:(.copilot_current.registry.reason // "Registered copilot table unavailable")}
       else empty end ]
     + [ $copilot_views[]
       | {id,state:.bearings_state,
          doing:((if .bearings_state == "active_child_work" then
                    ([.active_children[] | .id + ": " + (.doing // .state)] | join("; "))
                  elif .bearings_state == "pilot_decision" then
                    ([.bearings_pilot_holds[] | .summary] | join("; "))
          ...
          reason:(.current.reason // "-")} ]) as $copilots_all
  | ([ .tasks[]
       | select(.kind != "copilot")
     ...
     + [ $copilot_views[]
         ...
         | {id,kind:"copilot",state:.bearings_state,
            doing:([.active_children[] | .id + ": " + (.doing // .state)] | join("; ") | trunc(90))} ]) as $in_flight_all
  | ([ .backlog.records[]
         | select(.structured and .pilot_actionable == true)
         | {id,key:.id,verb:"pilot-hold",
            summary:((.title + ": " + .hold_reason) | trunc(90)),owner:"(main)"} ]
     + [ (.copilot_current.records // [])[] as $m | $m.decisions_open[]?
         | select(.source == "backlog" and .verb == "pilot-hold")
         | {id:($m.id + "/" + .id),key,verb,
            summary:(((.summary // .id) + ": " + (.reason // "pilot decision pending")) | trunc(90)),owner:$m.id} ]) as $decisions_all
  ...
  | ([ .tasks[] | select(.kind != "copilot" and .pr.url != null and .pr.source == "meta") | {id, url:.pr.url} ]) as $recorded_prs_all
  | . as $snap
  | {
      ...
      copilots: (if $all_copilots == 1 then $copilots_all else $copilots_all[:$copilots_n] end),
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (if $all_landed == 0 and ($per_home_capped | length) > ($done | length) then {surface:("landed showing \($done | length) of \($per_home_capped | length)" + (($done | map(.home_id) | unique | map(select(. != "(main)")) | length) as $k | if $k > 0 then " (incl. \($k) copilot home(s))" else "" end)), reveal:"--all-landed"} else empty end),
        ...
        (if (($snap.copilot_landed.unreadable // []) | length) > 0 then {surface:("copilot home(s) with unreadable backlog: \(($snap.copilot_landed.unreadable // []) | length)"), reveal:"inspect the listed copilot home backlogs"} else empty end),
        (if $all_landed == 0 and (($snap.copilot_landed.truncated // []) | length) > 0 then {surface:("copilot home Done capped at the snapshot layer for \(($snap.copilot_landed.truncated // []) | length) home(s)"), reveal:"--all-landed"} else empty end),
        ...
        (if $all_copilots == 0 and ($copilots_all | length) > $copilots_n then {surface:("copilots showing \($copilots_n) of \($copilots_all | length)"), reveal:"--all-copilots"} else empty end),
        (if (($snap.copilot_current.truncated // 0) > 0) then {surface:("registered copilots omitted by snapshot bound: \($snap.copilot_current.truncated)"), reveal:"raise AP_SNAPSHOT_COPILOTS"} else empty end),
        (if $snap.copilot_current.registry.input_truncated == true then {surface:"copilot registry input truncated by bounded read", reveal:"raise AP_SNAPSHOT_REGISTRY_LINES or AP_SNAPSHOT_REGISTRY_BYTES"} else empty end),
        (if $snap.copilot_current.registry.records_truncated == true then {surface:"copilot registry records omitted by bounded read", reveal:"raise AP_SNAPSHOT_REGISTRY_RECORDS"} else empty end),
        (if $snap.copilot_current.registry.available == false then {surface:("copilot registry unavailable: " + ($snap.copilot_current.registry.reason // "read failed")), reveal:"inspect data/copilots.md"} else empty end),
        (([($snap.copilot_current.records // [])[] | select(.parent_event.activity_scan.input_truncated == true or .parent_event.activity_scan.retained_truncated == true)] | length) as $n | if $n > 0 then {surface:("copilot parent activity evidence truncated for \($n) record(s)"), reveal:"raise AP_SNAPSHOT_PARENT_ACTIVITY_LINES, AP_SNAPSHOT_PARENT_ACTIVITY_BYTES, or AP_SNAPSHOT_PARENT_ACTIVITIES"} else empty end),
        (([($snap.copilot_current.records // [])[] | select(.parent_event.activity_scan.available == false)] | length) as $n | if $n > 0 then {surface:("copilot parent activity evidence unavailable for \($n) record(s)"), reveal:"inspect the parent status logs"} else empty end),
        ...
        (if $include_prs == 1 and $pr_rows_capped > 0 then {surface:("candidate_prs showing \($candidate_prs | length) of at least \($pr_rows_min_total); capped in \($pr_rows_capped) repo(s)"), reveal:"raise AP_BEARINGS_PR_LIMIT"} else empty end),
        (if $include_prs == 1 then empty else {surface:"live PR discovery + checks", reveal:"--include-prs"} end) ]) }
') || { echo "ap-bearings-snapshot: projection failed" >&2; exit 1; }
...
') || { echo "ap-bearings-snapshot: TOON rendering failed" >&2; exit 1; }
printf '%s\n' "$TOON"
