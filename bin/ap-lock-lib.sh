...
#
# ONE owner for the staleness proof that ap-teardown.sh (a worktree index.lock)
# and ap-fleet-sync.sh (a clone's .git/packed-refs.lock) both rely on: a lock is
...
# Diagnostics print to stderr prefixed by ${AP_LOCK_LOG_PREFIX:-ap-lock} so each
# caller's output stays recognizable.

ap_lock_log() {
  echo "${AP_LOCK_LOG_PREFIX:-ap-lock}: $*" >&2
}
...
# ap_lock_lsof_holder <target>: 0 a process holds it, 1 provably none, 2 lsof
...
ap_lock_lsof_holder() {
  ...
  if [ -n "$output" ]; then
    while IFS= read -r line; do
      ap_lock_log "lsof check failed: $line"
    done <<< "$output"
  else
    ap_lock_log "lsof check failed for $target with exit $status"
  fi
  return 2
}

# ap_lock_has_live_holder <lock> <dir>: 0 if a live process holds $lock or the
...
ap_lock_has_live_holder() {
  local lock=$1 dir=$2 status
  command -v lsof >/dev/null 2>&1 || return 0
  if [ -n "$lock" ]; then
    if ap_lock_lsof_holder "$lock"; then
      return 0
    else
      status=$?
  ...
  if [ -n "$dir" ]; then
    if ap_lock_lsof_holder "$dir"; then
      return 0
    else
      status=$?
      [ "$status" -eq 1 ] || return 0
...
# ap_lock_age <lock>: prints the lock's mtime age in whole seconds, or fails.
ap_lock_age() {
  local lock=$1 m now
  m=$(ap_lock_path_mtime "$lock") || return 1
...
# ap_lock_is_provably_stale <lock> <dir> <min_age_secs>: THE proof. Returns 0 iff
...
ap_lock_is_provably_stale() {
  local lock=$1 dir=$2 min_age=$3 age
  [ -n "$lock" ] && [ -e "$lock" ] || return 1
  ap_lock_has_live_holder "$lock" "$dir" && return 1
  if ! age=$(ap_lock_age "$lock"); then
    ap_lock_log "cannot read mtime for git lock $lock; leaving it in place"
    return 1
  fi
  [ "$age" -ge "$min_age" ]
}
