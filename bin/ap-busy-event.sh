#!/usr/bin/env bash
# ap-busy-event.sh - the ONLY writer of the semantic busy-state contract
# owned by bin/ap-busy-lib.sh (record format, gen binding, and classification
...
#       the record at seq=1 (default: busy, source ap-spawn, event
...
#       hook that outlives its incarnation fails closed here. Autopilot-owned
#       paths (ap-interrupt, ap-recovery) may pass --current-gen to bind to
...
usage() {
  cat >&2 <<'EOF'
usage:
  ap-busy-event.sh arm <state-dir> <id> [--state busy|idle|unknown] [--source S] [--event E]
  ap-busy-event.sh apply <state-dir> <id> <busy|idle|unknown> (--gen G | --current-gen) --source S --event E
  ap-busy-event.sh retire <state-dir> <id> (--gen G | --current-gen)
...
if [ "$CMD" != retire ]; then
  ...
  ap_busy_token_valid "$SOURCE" || { echo "error: invalid --source" >&2; exit 1; }
  ap_busy_token_valid "$EVENT" || { echo "error: invalid --event" >&2; exit 1; }
fi

REC=$(ap_busy_record_path "$STATE" "$ID")
GEN_FILE=$(ap_busy_gen_path "$STATE" "$ID")
LOCK="$REC.lock"

# Serialize writers. The lock protects seq advancement and the sidecar/record
# pair; a holder that died mid-write is broken after AP_BUSY_LOCK_STALE_SECS.
lock_acquire() {
  ...
  while ! mkdir "$LOCK" 2>/dev/null; do
    ...
    if [ "$tries" -ge 40 ]; then
      ...
      age=$((now - mtime))
      if [ "$age" -ge "${AP_BUSY_LOCK_STALE_SECS:-5}" ]; then
...
if [ "$USE_CURRENT_GEN" = 1 ] && [ "$CMD" != retire ]; then
  GEN=$(ap_busy_current_gen "$STATE" "$ID") || {
    umask "$old_umask"
...
if [ "$USE_CURRENT_GEN" != 1 ] || [ "$CMD" != retire ]; then
  ap_busy_token_valid "$GEN" || { umask "$old_umask"; echo "error: invalid --gen" >&2; exit 1; }
fi

lock_acquire || { umask "$old_umask"; exit 1; }
CURRENT=$(ap_busy_current_gen "$STATE" "$ID") || {
