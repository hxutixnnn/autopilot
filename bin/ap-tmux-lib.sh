#!/usr/bin/env bash
# ap-tmux-lib.sh — shared tmux pane primitives for autopilot.
#
...
# daemon (bin/ap-supervise-daemon.sh) and bin/ap-send.sh so the composer/submit
...
# idle pane as holding pending input and the daemon deferred injection / autopilot
...
# ap_composer_strip_ghost (bin/ap-composer-lib.sh), which drops every
...
# (ap-peek and every human/LLM-facing path stay plain). This is harness-generic:
...
# sent to a busy opencode pane, and `ap-send` exits non-zero on a normal
# pilot instruction. The submit core now falls back to `ap_pane_is_busy` once
...
#
# Overrides: AP_COMPOSER_IDLE_RE matches an empty composer after ghost and
# structural border stripping. AP_BUSY_REGEX overrides the rendered busy-footer
# matching used here.
#
# NOT a task-state source: task busy state is owned by bin/ap-busy-lib.sh's
...
# composer) is NOT owned here: it is the shared bin/ap-composer-lib.sh, sourced
# below and reused by every backend adapter so the decision cannot drift.

# shellcheck source=bin/ap-composer-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/ap-composer-lib.sh"
...
AP_TMUX_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel'
AP_TMUX_CLAUDE_BUSY_REGEX_DEFAULT='esc to interrupt|…[[:space:]]+\([0-9]+[smh]'
AP_TMUX_CODEX_BUSY_REGEX_DEFAULT='esc to interrupt'
AP_TMUX_OPENCODE_BUSY_REGEX_DEFAULT='esc interrupt'
AP_TMUX_PI_BUSY_REGEX_DEFAULT='Working\.\.\.'
AP_TMUX_GROK_BUSY_REGEX_DEFAULT='Ctrl\+c:cancel'
AP_TMUX_KIMI_BUSY_REGEX_DEFAULT='^[[:space:]]*(🌑|🌒|🌓|🌔|🌕|🌖|🌗|🌘)[[:space:]]+·[[:space:]]+'

ap_busy_lines_match() {  # [harness]
  local harness=${1:-} lines regex
  IFS= read -r -d '' lines || true
  if [ -n "${AP_BUSY_REGEX:-}" ]; then
    regex=$AP_BUSY_REGEX
  else
    case "$harness" in
      claude) regex=$AP_TMUX_CLAUDE_BUSY_REGEX_DEFAULT ;;
      codex) regex=$AP_TMUX_CODEX_BUSY_REGEX_DEFAULT ;;
      opencode) regex=$AP_TMUX_OPENCODE_BUSY_REGEX_DEFAULT ;;
      pi|pi-signed) regex=$AP_TMUX_PI_BUSY_REGEX_DEFAULT ;;
      grok) regex=$AP_TMUX_GROK_BUSY_REGEX_DEFAULT ;;
      kimi) regex=$AP_TMUX_KIMI_BUSY_REGEX_DEFAULT ;;
      '') regex=$AP_TMUX_BUSY_REGEX_DEFAULT ;;
      *)
...
# ap_tmux_strip_ghost: thin adapter over the shared, fleet-wide ghost extractor
# ap_composer_strip_ghost (bin/ap-composer-lib.sh). It drops de-emphasised
...
ap_tmux_strip_ghost() { ap_composer_strip_ghost; }

# ap_tmux_composer_row_state: classify one raw styled candidate row.
# A structural caller forces bordered=1; the compatibility fallback passes 0
# and may recognize a busy footer.
ap_tmux_composer_row_state() {  # <raw-row> [bordered] [allow-busy] -> empty|pending|unknown
  local raw=$1 bordered=${2:-0} allow_busy=${3:-1} plain stripped
  plain=$(printf '%s\n' "$raw" | ap_composer_strip_ansi)
  plain="${plain#"${plain%%[![:space:]]*}"}"
  plain="${plain%"${plain##*[![:space:]]}"}"
  stripped=$(printf '%s\n' "$raw" | ap_composer_strip_ghost)
  ...
  if [ "$allow_busy" = 1 ] && [ -n "$stripped" ] \
     && printf '%s' "$stripped" | grep -qiE "${AP_BUSY_REGEX:-$AP_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'empty'; return 0
  fi
  ap_composer_classify_content "$bordered" "$stripped" "${AP_COMPOSER_IDLE_RE:-}" insensitive "$plain"
}

ap_tmux_row_has_composer_edge() {  # <plain-row>
  local row=$1
  row="${row#"${row%%[![:space:]]*}"}"
  row="${row%"${row##*[![:space:]]}"}"
  case "$row" in
...
ap_tmux_composer_geometry_spaces() {  # <content-inner> -> spaces
...
# ap_tmux_find_composer_box: print the zero-based top and bottom rows of the
...
ap_tmux_find_composer_box() {  # <cursor-y> <plain-visible-pane> -> "<top> <bottom> <ambiguous>"
  ...
  while IFS= read -r line; do
    ...
    if [ "$row" -eq "$cy" ] && ap_tmux_row_has_composer_edge "$trimmed"; then
      cursor_structural=1
    fi
      ...
      case "$current_family:$side_family" in
        rounded:single|light:single|heavy:heavy|double:double|ascii:ascii)
          ...
          if [ "$geometry_check" = 1 ]; then
            ...
            if content_spaces=$(ap_tmux_composer_geometry_spaces "$content_inner"); then
...
# ap_tmux_composer_state classification contract:
...
ap_tmux_composer_state() {  # <target> -> empty|pending|pending-unproven|unknown
  ...
  plain=$(printf '%s\n' "$pane" | ap_composer_strip_ansi)
  if box=$(ap_tmux_find_composer_box "$cy" "$plain"); then
    top=${box%% *}
    box=${box#* }
    bottom=${box%% *}
    ...
    while [ "$row" -lt "$bottom" ]; do
      row_raw=$(printf '%s\n' "$pane" | sed -n "$((row + 1))p")
      state=$(ap_tmux_composer_row_state "$row_raw" 1 0)
      case "$state" in
        pending)
  ...
  if ap_tmux_row_has_composer_edge "$(printf '%s\n' "$raw" | ap_composer_strip_ansi)"; then
    printf 'unknown'
    return 0
  fi
  ap_tmux_composer_row_state "$raw" 0
}

# ap_pane_input_pending: 0 when the composer is not proven empty, so pending
# text, ambiguous structure, unreadable state, and future verdicts all defer.
ap_pane_input_pending() {  # <target>
  [ "$(ap_tmux_composer_state "$1")" != empty ]
}

# ap_pane_is_busy: 0 if the pane's last few non-blank lines show a busy footer
# (an agent mid-turn). Scans a 40-line tail like ap-watch.sh.
ap_pane_is_busy() {  # <target> [harness]
...
# ap_tmux_submit_core: type <text> into <target> ONCE, then submit with Enter,
...
# `ap_pane_is_busy`: a busy pane means the Enter was accepted and queued (report
...
ap_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep>
  local target=$1 retries=$2 sleep_s=$3 i=0 state
  while :; do
    tmux send-keys -t "$target" Enter 2>/dev/null || true
    sleep "$sleep_s"
    state=$(ap_tmux_composer_state "$target")
    case "$state" in
  ...
  if ap_pane_is_busy "$target"; then
    printf 'empty'
  else
    printf 'pending'
  fi
...
ap_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5
  tmux send-keys -t "$target" -l "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
  ap_tmux_submit_enter_core "$target" "$retries" "$sleep_s"
}
