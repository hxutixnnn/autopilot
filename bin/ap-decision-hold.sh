#!/usr/bin/env bash
# ap-decision-hold.sh - deterministic mechanics for durable pilot decisions.
#
...
# and verifies structured tasks-axi pilot holds, records completion attestation
...
# All backlog mutations run in the active AP_HOME, which keeps main-home and
# copilot-home ownership aligned with the work that discovered the decision.
#
# Usage:
#   ap-decision-hold.sh id <origin-id> <decision-key>
#   ap-decision-hold.sh hold <origin-id> <decision-key> \
#     --title <title> --reason <reason> [--repo <repo>]
#   ap-decision-hold.sh complete <origin-id> (--none | <decision-key>...)
#   ap-decision-hold.sh verify <origin-id>
#   ap-decision-hold.sh resolve <origin-id> <decision-key> \
#     --decision-file <path> --routed-to <task-id> [--routed-to <task-id>...]
#
# `complete` is the shared investigation and visual-review completion gate.
# `--none` is an explicit semantic attestation that the just-reviewed surface has
# no unresolved pilot decision. Later review passes may add keys; a live task's
# metadata inventory is unioned idempotently. A post-teardown visual review can
# complete against the surviving report and holds without recreating task state.
# `verify` is read-only and is called by recon teardown so teardown cannot erase a
# source before this gate has succeeded.
#
# `resolve` requires every --routed-to task to exist and to be blocked by the hold.
# It writes the pilot decision and routed identities into the hold body, clears
# those dependency edges, and only then marks the hold Done. A failure before the
# final step leaves the pilot hold open.
set -eu
...
tasks_axi() {
  (cd "$AP_HOME" && tasks-axi "$@")
}

require_tasks_axi() {
  ap_tasks_axi_compatible || fail "compatible tasks-axi is required"
}

# tasks-axi owns a neutral `external` dispatch hold kind and allows product task
# kinds to remain open-ended. Pilot decisions therefore use kind=pilot for the
# Autopilot-owned identity and hold-kind=external for the dependency's neutral
# scheduling mechanism.
activate_pilot_hold() {  # <id> <reason>
  tasks_axi hold "$1" --reason "$2" --kind external >/dev/null
}
...
verify_hold_active() {  # <hold-id>
  ...
  show=$(task_show "$id") || fail "pilot hold $id is absent from $AP_HOME/data/backlog.md"
  state=$(show_field "$show" state)
  held=$(show_field "$show" held)
  kind=$(show_field "$show" kind)
  hold_kind=$(show_field "$show" hold_kind)
  [ "$state" = queued ] || fail "pilot hold $id is not queued (state=$state)"
  [ "$held" = yes ] || fail "pilot hold $id is not active"
  [ "$kind" = pilot ] || fail "backlog item $id is not kind pilot"
  [ "$hold_kind" = external ] || fail "backlog item $id is not held for the pilot"
}
...
verify_hold_resolved() {  # <hold-id>
  ...
  [ "$kind" = pilot ] || return 1
  case "$body" in
    *"Resolution recorded by ap-decision-hold."*"Routed work:"*) return 0 ;;
  esac
  return 1
}
...
verify_hold_durable() {  # <hold-id>
  ...
  show=$(task_show "$id") || fail "pilot decision $id is absent from $AP_HOME/data/backlog.md"
  state=$(show_field "$show" state)
  held=$(show_field "$show" held)
  kind=$(show_field "$show" kind)
  hold_kind=$(show_field "$show" hold_kind)
  body=$(show_field "$show" body)
  if [ "$state" = queued ] && [ "$held" = yes ] && [ "$kind" = pilot ] && [ "$hold_kind" = external ]; then
    return 0
  fi
  if [ "$state" = "done" ] && [ "$kind" = pilot ]; then
    case "$body" in
      *"Resolution recorded by ap-decision-hold."*"Routed work:"*) return 0 ;;
    esac
  fi
  fail "pilot decision $id is neither actively held nor durably resolved"
}
...
verify_resolution_identity() {
  ...
  resolution_prefix='"Resolution recorded by ap-decision-hold.\nDecision digest: '
  case "$hold_body" in
    "$resolution_prefix"*) resolution_fields=${hold_body#"$resolution_prefix"} ;;
    *) fail "pilot hold $id has no retry identity record" ;;
  esac
  case "$resolution_fields" in
    *'\nRouted identities: '*'\n\nPilot decision:'*) : ;;
    *) fail "pilot hold $id has an invalid retry identity record" ;;
  esac
  ...
  [ "$recorded_digest" = "$decision_digest" ] \
    || fail "pilot hold $id records a different pilot decision"
  [ "$recorded_routes" = "$routed_csv" ] \
    || fail "pilot hold $id records different routed work"
}
...
command_hold() {
  ...
  require_tasks_axi
  origin_exists_here "$origin" || fail "origin $origin is not owned by the active home $AP_HOME"
  id=$(hold_id "$origin" "$key")
  if show=$(task_show "$id"); then
    state=$(show_field "$show" state)
    kind=$(show_field "$show" kind)
    existing_title=$(show_field "$show" title)
    [ "$state" != "done" ] || fail "pilot decision $id is already durably resolved; use a new decision key for a new decision"
    [ "$kind" = pilot ] || fail "existing backlog identity $id is not kind pilot"
    [ "$existing_title" = "$title" ] || fail "existing pilot hold $id has a different title"
  else
    ...
    [ -n "$repo" ] || repo=autopilot
    validate_one_line repo "$repo"
    body=$(printf 'Origin: %s\nDecision key: %s\nState: awaiting pilot decision.' "$origin" "$key")
    tasks_axi add "$id" "$title" --kind pilot --repo "$repo" --body "$body" >/dev/null \
      || fail "could not create pilot decision item $id"
  fi
  activate_pilot_hold "$id" "$reason" \
    || fail "could not activate pilot hold $id"
...
EOF
  ...
  while IFS=$'\t' read -r key _verb _summary; do
    ...
    list_has_key "$keys" "$key" \
      || fail "open structured decision $origin/$key has no pilot-held inventory entry"
  done <<EOF
$open
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    # live status fold does not duplicate the same Pilot's Call item.
    while IFS=$'\t' read -r key _verb _summary; do
      [ -n "$key" ] || continue
      list_has_key "$keys" "$key" || continue
      printf 'pilot-held [key=%s]: tracked by %s\n' "$key" "$(hold_id "$origin" "$key")" >> "$status_file"
      key_seen=1
    done <<EOF
$raw_open
EOF
...
command_resolve() {
  ...
  hold_body=$(show_field "$hold_show" body)
  case "$hold_body" in
    *"Resolution recorded by ap-decision-hold."*)
      verify_resolution_identity "$id" "$hold_body" "$decision_digest" "$routed_csv"
  ...
  for dep in $routed; do
    ...
    case ",$blocked," in
      ...
      *)
        case "$hold_body" in
          *"Resolution recorded by ap-decision-hold."*"- $dep"*) : ;;
  ...
  body=$(printf 'Resolution recorded by ap-decision-hold.\nDecision digest: %s\nRouted identities: %s\n\nPilot decision:\n%s\n\nRouted work:\n' "$decision_digest" "$routed_csv" "$decision")
  for dep in $routed; do
    body="${body}- ${dep}"$'\n'
  done
  tasks_axi update "$id" --body "$body" >/dev/null \
    || fail "could not record the pilot decision on $id"
  for dep in $routed; do
  ...
  tasks_axi "done" "$id" >/dev/null || fail "could not close resolved pilot hold $id"
  verify_hold_resolved "$id" || fail "pilot hold $id did not retain its durable resolution record"
  printf 'resolved: %s -> %s\n' "$id" "$routed"
}
