...
# worktree, or retire a copilot home; kill the recorded runtime endpoint,
# clear volatile state, refresh/prune the project's clone for PR-based flight
# tasks, then print a backlog-refresh reminder for flight and recon teardowns
# (a copilot teardown prints none, since copilots are not backlog items).
# REFUSES if the worktree holds work that has not LANDED, because cleanup
# hard-resets/removes the worktree and kills its processes. Work has landed when it is
# reachable from any remote-tracking branch (a fork counts as a remote, so
# upstream-contribution PRs pushed to a fork satisfy this in any mode), OR - for a
# normal flight task whose commits are not so reachable - when its PR is merged and
...
# where the usual "checks green" ap-pr-check.sh trigger never fires) - by looking
...
# branch (autopilot performs that merge after configured approval) as a fallback
# for the common case where there is no remote at all.
# Recon tasks (kind=recon in meta) carve out of that check: their worktree is
# declared scratch and the report at data/<task-id>/report.md is the work
# product. Teardown proceeds only once the report exists and the shared
# unresolved-decision completion gate verifies its pilot-held inventory.
...
# pilot's active tab, and restore the exact response-derived pre-close tab
# if Herdr's last-pane cleanup focuses an unrelated neighboring workspace.
# Copilots (kind=copilot in meta) are retired explicitly. Normal
# teardown refuses while their home has in-flight crew member meta files; --force
...
# Usage: ap-teardown.sh <task-id> [--force]
#   --force skips ordinary-task dirty and landed-work checks, skips recon report
#   checks, and discards copilot child work for kind=copilot. Only use it
#   when the pilot has explicitly said to discard the work.
#
# Transient / stale worktree git lock recovery (teardown-lock-race): a flight crew process
...
#   1. Retries up to AP_TREEHOUSE_RETURN_LOCK_RETRIES times (default 3), waiting
#      AP_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS (default 1s; falls back to the older
#      AP_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS name when the new one is unset) between
...
#      bin/ap-lock-lib.sh's ap_lock_is_provably_stale, passing the worktree dir as the
#      companion directory and AP_STALE_WORKTREE_LOCK_AGE_SECS (default 30s) as the age
...
# shellcheck source=bin/ap-lock-lib.sh
. "$SCRIPT_DIR/ap-lock-lib.sh"
...
if [ "$#" -lt 1 ] || ! ap_task_id_path_safe "$1"; then
...
# down a worktree (see bin/ap-gate-refuse-lib.sh).
ap_refuse_if_gate_agent
AP_LOCK_LOG_PREFIX=teardown

META="$STATE/$ID.meta"
[ -f "$META" ] || { echo "error: no meta for task $ID at $META" >&2; exit 1; }
# This is the first cleanup authorization check. It is metadata-only and must
# complete before ap-guard, a backend command, file removal, branch deletion,
# worktree return, registry change, or process termination can run.
ap_backend_validate_task_endpoint "$META" "$ID" || exit 1
BACKEND=$AP_BACKEND_VALIDATED_BACKEND
T=$AP_BACKEND_VALIDATED_TARGET
WT=$(ap_meta_get "$META" worktree)
PROJ=$(ap_meta_get "$META" project)
T_ORCA=
[ "$BACKEND" != orca ] || T_ORCA=$T
"$AP_ROOT/bin/ap-guard.sh" || true
HOME_PATH=$(grep '^home=' "$META" | cut -d= -f2- || true)
PR_URL=$(grep '^pr=' "$META" | tail -1 | cut -d= -f2- || true)
# tasktmp is recorded by ap-spawn for tasks that set up a per-task temp root
# (/tmp/ap-<id>/); absent for tasks spawned before that change, so tolerate empty.
TASK_TMP=$(grep '^tasktmp=' "$META" | cut -d= -f2- || true)
BUSY_GEN=$(ap_meta_get "$META" busy_gen)
if [ -z "$BUSY_GEN" ]; then
  BUSY_GEN=$(cat "$STATE/$ID.busy-gen" 2>/dev/null || true)
fi
ORCA_WORKTREE_ID=$(ap_meta_get "$META" orca_worktree_id)
...
remove_grok_turnend_auth() {
  ...
  hooks_dir="${GROK_HOME:-$HOME/.grok}/hooks/ap-turn-end.d"
...
remove_kimi_turnend_auth() {
  ...
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 0 ;; esac
  hooks_dir="$HOME/.kimi-code/ap-turn-end.d"
  rm -f "$hooks_dir/$token"
}
...
retire_busy_state() {
  ...
  if [ -n "$gen" ]; then
    "$SCRIPT_DIR/ap-busy-event.sh" retire "$state_dir" "$id" --gen "$gen"
  elif [ -f "$state_dir/$id.busy-gen" ]; then
    "$SCRIPT_DIR/ap-busy-event.sh" retire "$state_dir" "$id" --current-gen
  fi
}

validate_pr_poll_cleanup() {
  local state_dir=$1 id=$2 quarantine state_device artifact has_artifact=0
  ap_task_id_path_safe "$id" || return 0
  ...
  state_device=$(ap_pr_file_device "$state_dir") || return 1
  for artifact in "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.check-trust"; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    if [ ! -f "$artifact" ] || [ -L "$artifact" ] \
      || [ "$(ap_pr_file_device "$artifact")" != "$state_device" ] \
      || [ "$(ap_pr_file_link_count "$artifact")" != 1 ]; then
  ...
  if [ -e "$state_dir/$id.pr-poll-retirement" ] \
    ...
    ap_pr_poll_retirement_state_valid "$state_dir" "$id" || {
  ...
  if [ "$(ap_pr_file_device "$quarantine")" != "$state_device" ] \
    || [ "$(ap_pr_file_mode "$quarantine")" != 700 ]; then
    echo "REFUSED: PR-check quarantine is not on the task state device; preserving task state." >&2
    return 1
  fi
  for artifact in "$quarantine/$id."*; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    if ! ap_pr_private_file_valid "$artifact" 600 "$state_device"; then
...
remove_pr_poll_artifacts() {
  ...
  ap_pr_poll_retirement_recover_one "$state_dir" "$id" "$SCRIPT_DIR/ap-pr-poll.sh" || return 1
  rm -f "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.check-trust" || return 1
  if ap_task_id_path_safe "$id"; then
    quarantine="$state_dir/.pr-check-quarantine"
...
STALE_WORKTREE_LOCK_AGE_SECS=${AP_STALE_WORKTREE_LOCK_AGE_SECS:-30}
# Bounded patience window for transient index.lock after killing a flight crew process.
# New knobs are preferred; AP_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS remains an alias
# for the per-attempt wait so existing tests and operators keep working.
TREEHOUSE_RETURN_LOCK_RETRIES=${AP_TREEHOUSE_RETURN_LOCK_RETRIES:-3}
TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=${AP_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS:-${AP_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS:-1}}
...
# is owned by bin/ap-lock-lib.sh's ap_lock_is_provably_stale, sourced above.
...
cleanup_stale_lock_for_safety_check() {
  ...
  if ap_lock_is_provably_stale "$lock" "$dir" "$STALE_WORKTREE_LOCK_AGE_SECS"; then
    rm -f "$lock"
...
# stale git index.lock left by a killed flight crew process. See the script header.
...
validate_worktree_teardown_safety() {
  ...
  if ! dirty_raw=$(git -C "$WT" status --porcelain 2>/dev/null); then
    ...
    echo "Restore the git index state, or get the pilot's explicit OK to discard, then --force." >&2
    return 1
  fi
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE '^\?\? (\.claude/|\.ap-(grok|kimi)-turnend$)' | head -1 || true)
  ...
  if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
    ...
    if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
      ...
      echo "Merge the branch into local $DEFAULT first (bin/ap-merge-local.sh after the pilot approves), or push to a fork/remote, or get the pilot's explicit OK to discard, then --force." >&2
      return 1
    fi
  elif [ -n "$dirty" ]; then
    echo "REFUSED: worktree $WT has uncommitted changes." >&2
    echo "uncommitted changes present" >&2
    echo "Commit them (or get the pilot's explicit OK to discard, then --force)." >&2
    return 1
    ...
    if ! work_is_landed "$branch"; then
      ...
      echo "Push the branch, land its PR, or get the pilot's explicit OK to discard, then --force." >&2
      return 1
    fi
  fi
}
...
require_orca_worktree_path_match() {
  ...
  resolved=$(ap_backend_worktree_path orca "$worktree_id") || {
...
autopilot_home_has_treehouse_slot() {
  local home=$1
  worktree_registered_for_project "$AP_ROOT" "$home"
}
...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$AP_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$AP_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active autopilot home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the autopilot repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active autopilot home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the autopilot repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active autopilot home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the autopilot repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_autopilot_operational_dirs_for_removal() {
  ...
  for name in data state config projects; do
    ...
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      echo "REFUSED: unsafe $label $name directory $dir resolves outside the copilot home" >&2
      return 1
    fi
    if [ -d "$dir" ]; then
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$AP_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active autopilot home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$AP_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the autopilot repo" >&2
    return 1
  fi
...
validate_autopilot_home_for_removal() {
  ...
  if [ ! -f "$abs_home_path/$COPILOT_HOME_MARKER" ]; then
    echo "REFUSED: unsafe $label removal target $home is not a seeded copilot home" >&2
    return 1
  fi
  if [ -n "$expected_id" ]; then
    marker_id=$(cat "$abs_home_path/$COPILOT_HOME_MARKER" 2>/dev/null || true)
    if [ "$marker_id" != "$expected_id" ]; then
      echo "REFUSED: unsafe $label removal target $home is marked for copilot ${marker_id:-unknown}, expected $expected_id" >&2
      return 1
    fi
  fi
  validate_autopilot_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=$(registered_descendant_home_for_removal "$COPILOT_REG" "$abs_home_path" || true)
  if [ -z "$conflict" ]; then
    conflict=$(registered_descendant_home_for_removal "$abs_home_path/data/copilots.md" "$abs_home_path" || true)
  fi
  if [ -n "$conflict" ]; then
    IFS=$'\t' read -r child_id child_home <<EOF
$conflict
EOF
    echo "REFUSED: unsafe $label removal target $home contains registered copilot home $child_home for $child_id" >&2
    return 1
  fi
  printf '%s\n' "$abs_home_path"
}

remove_autopilot_home() {
  local home=$1 label=$2 expected_id=${3:-} abs_home_path
  [ -n "$home" ] || return 0
  [ -e "$home" ] || return 0
  abs_home_path=$(validate_autopilot_home_for_removal "$home" "$label" "$expected_id") || return 1
  [ -n "$abs_home_path" ] || return 0
  if autopilot_home_has_treehouse_slot "$abs_home_path"; then
    command -v treehouse >/dev/null 2>&1 || {
      echo "error: treehouse command not found; cannot return $label $abs_home_path" >&2
      return 1
    }
    teardown_treehouse_return "$abs_home_path" "$AP_ROOT" "$label" || {
...
validate_autopilot_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    ap_backend_validate_task_endpoint "$child_meta" "$child_id" || return 1
    validate_pr_poll_cleanup "$sub_state" "$child_id" || return 1
    child_wt=$(meta_value "$child_meta" worktree)
    child_kind=$(meta_value "$child_meta" kind)
    [ -n "$child_kind" ] || child_kind=flight
    child_backend=$(ap_backend_of_meta "$child_meta")
    if [ "$child_kind" = copilot ]; then
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      validate_autopilot_home_for_removal "$child_home" "child autopilot home" "$child_id" >/dev/null || return 1
      validate_autopilot_home_children_removal "$child_home" || return 1
...
cleanup_autopilot_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_backend" = orca ]; then
      ...
      child_t=$(ap_backend_target_of_meta "$child_meta")
    fi
    if [ "$child_backend" = orca ] && [ "$child_kind" != copilot ]; then
      child_orca_worktree_id=$(require_orca_worktree_id "$child_meta") || return 1
      if [ -n "$child_wt" ] && [ -e "$child_wt" ]; then
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
      fi
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = zellij ]; then
        # Zellij titles are scoped by the owning home tag, so forced copilot
        # cleanup must verify child tabs as that child home, not the parent.
        ( unset AP_ROOT_OVERRIDE; AP_HOME=$home AP_ROOT=$home ap_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "ap-$child_id" ) 2>/dev/null || true
      else
        ap_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "ap-$child_id" 2>/dev/null || true
      fi
    fi
    if [ "$child_kind" = copilot ]; then
      child_home=$(meta_value "$child_meta" home)
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_autopilot_home_children "$child_home"
        remove_autopilot_home "$child_home" "child autopilot home" "$child_id"
      fi
    elif [ "$child_backend" = orca ]; then
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/ap-turn-end.js" \
          "$child_wt/.ap-grok-turnend" "$child_wt/.ap-kimi-turnend"
      fi
      ap_backend_remove_worktree "$child_backend" "$child_orca_worktree_id" || return 1
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
      rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/ap-turn-end.js" \
        "$child_wt/.opencode/plugins/ap-busy-state.js" \
        "$child_wt/.ap-grok-turnend" "$child_wt/.ap-kimi-turnend"
...
remove_copilot_registry_entry() {
  local id=$1 tmp
  [ -f "$COPILOT_REG" ] || return 0
  tmp="$COPILOT_REG.tmp.$$"
  grep -vE "^- $id( |$)" "$COPILOT_REG" > "$tmp" || true
  mv "$tmp" "$COPILOT_REG"
}
...
if [ "$KIND" = copilot ]; then
  ...
  validate_autopilot_home_for_removal "$HOME_PATH" "copilot home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_autopilot_home_children_removal "$HOME_PATH" || exit 1
  fi
fi

if [ "$KIND" = copilot ] && [ "$FORCE" != "--force" ]; then
  COPILOT_STATE="$HOME_PATH/state"
  if [ -d "$COPILOT_STATE" ]; then
    for child_meta in "$COPILOT_STATE"/*.meta; do
      [ -e "$child_meta" ] || continue
      echo "REFUSED: copilot $ID still has in-flight work in $COPILOT_STATE." >&2
...
if [ "$KIND" = copilot ] && [ "$FORCE" = "--force" ]; then
  cleanup_autopilot_home_children "$HOME_PATH"
fi

if [ "$KIND" = recon ] && [ "$FORCE" != "--force" ]; then
  REPORT="$DATA/$ID/report.md"
  if [ ! -f "$REPORT" ]; then
    echo "REFUSED: recon task $ID has no report at $REPORT." >&2
    echo "The report is the work product. Have the flight crew member write it, or use --force after explicit discard approval." >&2
    exit 1
  fi
  if ! AP_HOME="$AP_HOME" AP_STATE_OVERRIDE="$STATE" AP_DATA_OVERRIDE="$DATA" \
      AP_CONFIG_OVERRIDE="$CONFIG" "$SCRIPT_DIR/ap-decision-hold.sh" verify "$ID" >/dev/null; then
    echo "REFUSED: recon task $ID has not passed the unresolved-decision completion gate." >&2
    echo "Inventory its report and any visual review through bin/ap-decision-hold.sh before teardown." >&2
    exit 1
  fi
fi

if [ "$BACKEND" = orca ] && [ "$KIND" != recon ] && [ "$KIND" != copilot ] && [ "$FORCE" != "--force" ]; then
  if ! inspectable_git_worktree "$WT"; then
    echo "REFUSED: Orca flight task $ID has no inspectable git worktree at ${WT:-<missing>}." >&2
...
if [ "$BACKEND" = orca ] && [ "$KIND" != copilot ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/ap-turn-end.js" \
      "$WT/.opencode/plugins/ap-busy-state.js" \
      "$WT/.ap-grok-turnend" "$WT/.ap-kimi-turnend"
  fi
  [ -z "$T_ORCA" ] || ap_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "ap-$ID" 2>/dev/null || true
  ap_backend_remove_worktree "$BACKEND" "$ORCA_WORKTREE_ID"
elif [ -d "$WT" ] && [ "$KIND" != copilot ]; then
  branch=$(git -C "$WT" rev-parse --abbrev-ref HEAD 2>/dev/null || echo HEAD)
  if [ "$branch" != "HEAD" ]; then
    if git -C "$WT" checkout --detach -q 2>/dev/null; then
      git -C "$WT" branch -D "$branch" >/dev/null 2>&1 || true
  ...
  # left by a killed flight crew process; see the script header for retry and stale-lock proof.
  post_lock_cleanup_check=
  if [ "$FORCE" != "--force" ] && [ "$KIND" != recon ] && [ "$KIND" != copilot ]; then
...
if [ "$BACKEND" = herdr ] \
  ...
  if [ -n "$HERDR_PRESENTATION_SESSION" ] \
     ...
     && ap_backend_herdr_projection_endpoint_matches_journal \
...
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  ...
  if HERDR_PRESENTATION_FOCUS_LOCK=$(ap_backend_herdr_presentation_session_lock_path "$HERDR_PRESENTATION_SESSION"); then
    while [ "$HERDR_PRESENTATION_FOCUS_LOCK_ATTEMPT" -lt 50 ]; do
      if ap_lock_try_acquire "$HERDR_PRESENTATION_FOCUS_LOCK"; then
  ...
  if [ "$HERDR_PRESENTATION_FOCUS_LOCK_HELD" = 1 ]; then
    ...
    HERDR_PRESENTATION_FOCUS_LOCK_HELD=0
    ap_lock_release "$HERDR_PRESENTATION_FOCUS_LOCK" || true
  else
    echo "warning: herdr presentation focus lock unavailable; refusing a concurrent focus-unsafe pane close" >&2
  fi
elif [ "$BACKEND" != orca ]; then
  ap_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "ap-$ID" 2>/dev/null || true
fi
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  if [ "$(ap_backend_herdr_pane_agent_state "$HERDR_PRESENTATION_SESSION" "$HERDR_PRESENTATION_PANE")" = dead ]; then
...
if [ "$KIND" = copilot ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  remove_autopilot_home "$HOME_PATH" "copilot home" "$ID"
  remove_copilot_registry_entry "$ID"
fi
remove_grok_turnend_auth "$STATE" "$ID"
remove_kimi_turnend_auth "$STATE" "$ID"
ap_backend_clear_transition "$BACKEND" "$STATE" "$T" || true
# Remove the per-task temp root (/tmp/ap-<id>/, incl. its gotmp/) recorded by spawn.
...
if [ "$KIND" != recon ] && [ "$KIND" != copilot ] && [ "$MODE" != local-only ]; then
  "$AP_ROOT/bin/ap-fleet-sync.sh" "$PROJ" || true
fi
