...
# ap-wake-drain.sh after it empties queued wakes, and by ap-session-start.sh in
# read-only advisory mode whenever session-lock ownership was not verified.
# First, always warn if the autopilot primary checkout (AP_ROOT) is on a named
# non-default branch, because that means autopilot-on-itself work landed in the
...
# missing or older than AP_GUARD_GRACE seconds, prints a loud, clearly delimited
...
# distinct staleness episode in this AP_HOME (keyed to beacon mtime or absence);
...
queue_pending=false
READ_ONLY=${AP_GUARD_READ_ONLY:-0}
case "$READ_ONLY" in 1|true|TRUE|yes|YES) READ_ONLY=1 ;; *) READ_ONLY=0 ;; esac
CONTINUE_LINE=${AP_GUARD_CONTINUE_LINE:-This is a supervision warning only; the guarded operation WILL still run.}
...
# shellcheck source=bin/ap-tangle-lib.sh
...
ap_guard_clear_stale_banner() {
  rm -f "$STALE_BANNER_MARKER" 2>/dev/null || true
}

# Worktree-tangle alarm, checked FIRST and independent of in-flight tasks: the
# autopilot PRIMARY checkout (AP_ROOT) must stay on its default branch. If a
# flight crew member's branch/commits landed here instead of in its own isolated worktree,
...
# only: detached HEAD (linked worktrees, copilot homes) never trips this.
tangle_branch=$(ap_primary_tangle_branch "$AP_ROOT" || true)
if [ -n "$tangle_branch" ]; then
  ...
  trule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    printf "●  %s is on '%s', not its default branch '%s'.\n" "$AP_ROOT" "$tangle_branch" "$tangle_default"
    printf '●  A flight crew member likely branched/committed in the primary instead of its own worktree.\n'
...
# grace-based predicate (bin/ap-supervision-lib.sh). Only act with tasks in
# flight; count them so the banner can say how much is riding on an absent
# watcher.
ap_supervision_status "$STATE" "$GRACE"
in_flight=$AP_SUP_IN_FLIGHT
watcher_fresh=$AP_SUP_WATCHER_FRESH
beacon_desc=$AP_SUP_BEACON_DESC
if [ "$in_flight" -eq 0 ]; then
  ...
  [ "$READ_ONLY" -eq 1 ] || ap_guard_clear_stale_banner
  exit 0
fi

[ -s "$AP_WAKE_QUEUE" ] && queue_pending=true
...
if [ "$watcher_fresh" = false ]; then
  episode_key=$(ap_guard_stale_episode_key "$STATE")
  episode_key=${episode_key%$'\n'}
  print_full_banner=0
  if [ "$READ_ONLY" -eq 1 ]; then
    ap_guard_stale_banner_seen "$STATE" "$episode_key" || print_full_banner=1
  elif ap_guard_claim_stale_banner "$STATE" "$episode_key"; then
    print_full_banner=1
  fi
  if [ "$print_full_banner" -eq 1 ]; then
    ...
    fix=$("$SCRIPT_DIR/ap-supervision-instructions.sh" \
      --read-only "$READ_ONLY" \
      --afk "$afk" \
...
if "$queue_pending"; then
  if [ "$READ_ONLY" -eq 1 ]; then
    ...
    echo "WARNING: queued wakes pending - drain them with bin/ap-wake-drain.sh before anything else." >&2
  fi
fi
exit 0
