#!/usr/bin/env bash
# Provision and route persistent copilot homes.
#
# Usage:
#   ap-home-seed.sh <id> <home|-> {<project>...|--no-projects}
#       Provision <home> as an isolated autopilot home. If <home> is "-", acquire
#       a fresh autopilot worktree via "treehouse get --lease", which durably
#       leases the worktree under the copilot <id> so the home survives with
#       no live process and is never recycled until the lease is released with
#       "treehouse return". Projects are cloned
#       from the active home into the copilot home's projects/ directory.
...
#       subject is the autopilot repo itself; it is mutually exclusive with a
...
#       initialized, an ignored .ap-copilot-home identity marker is written, and
#       data/copilots.md is updated.
...
#       Set AP_COPILOT_CHARTER='<charter>' to seed from inline charter text
#       when no filled charter brief exists. Set AP_COPILOT_SCOPE='<scope>'
#       to override the registry routing scope. Otherwise the registry summary
#       and scope are derived from the filled charter brief.
#   ap-home-seed.sh validate
...
usage() {
  echo "usage: ap-home-seed.sh <id> <home|-> {<project>...|--no-projects}" >&2
  echo "       ap-home-seed.sh validate" >&2
}
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${AP_COPILOT_CHARTER:-}" ]; then
    printf '%s\n' "$AP_COPILOT_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${AP_COPILOT_SCOPE:-}" ]; then
    printf '%s\n' "$AP_COPILOT_SCOPE" | normalize_registry_text
  else
...
validate_registry_home_text() {
  ...
  case "$home" in
    *';'*|*')'*|*$'\n'*)
      echo "error: copilot home path contains registry delimiters: $home" >&2
      return 1
      ;;
  esac
}
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/ap-autopilots.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate copilot home assignment:\n%s\n' "$duplicate_homes" >&2
    return 1
  }
  duplicate_ids=$(awk -F '\t' '
    {
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate copilot id assignment:\n%s\n' "$duplicate_ids" >&2
    return 1
  }
  overlaps=$(awk -F '\t' '
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: overlapping copilot home assignment:\n%s\n' "$overlaps" >&2
    return 1
  }
  rm -f "$tmp"
  return 0
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$AP_HOME")
  abs_root=$(resolved_path "$AP_ROOT")
  if [ "$abs_home" = "/" ]; then
...
validate_seed_leaf_files() {
  ...
  abs_home=$(resolved_path "$home")
  for label in "data/projects.md" "data/charter.md" "$COPILOT_HOME_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
      echo "error: copilot leaf file must not be a symlink: $path" >&2
      return 1
    fi
    [ -e "$path" ] || continue
    ...
    case "$abs_path" in
      "$abs_home"/*) ;;
      *)
        echo "error: copilot leaf file must resolve inside the copilot home: $path" >&2
        return 1
        ;;
    esac
  done
...
validate_project_destination() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "error: copilot projects directory must resolve inside the copilot home: $projects_dir" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_dst"; then
    echo "error: seeded project $project destination must resolve inside the copilot projects directory: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active autopilot home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the autopilot repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
acquire_treehouse_home() {
  local id=$1 home
  # Durably lease an autopilot worktree from the pool. The lease persists with no
  ...
  home=$(cd "$AP_ROOT" && treehouse get --lease --lease-holder "$id") || {
    echo "error: treehouse get --lease failed to lease an autopilot home" >&2
    return 1
  }
  [ -n "$home" ] || { echo "error: treehouse get --lease did not report an autopilot home" >&2; return 1; }
  printf '%s\n' "$home"
}
...
verify_autopilot_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not an autopilot home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not an autopilot home (missing bin/)" >&2; return 1; }
...
validate_home_assignment() {
  ...
  if [ -f "$home/$COPILOT_HOME_MARKER" ]; then
    marker_id=$(cat "$home/$COPILOT_HOME_MARKER" 2>/dev/null || true)
    if [ "$marker_id" != "$id" ]; then
      echo "error: copilot home $home is already marked for ${marker_id:-unknown}" >&2
      return 1
    fi
  fi
  id_conflict=$(registry_id_conflict_for_assignment "$id" "$home" || true)
  if [ -n "$id_conflict" ]; then
    echo "error: copilot id $id is already registered to home $id_conflict; retire it before assigning $home" >&2
    return 1
  fi
...
EOF
  if [ "$conflict_type" = exact ]; then
    echo "error: copilot home $home is already registered to $owner" >&2
    return 1
  fi
  echo "error: copilot home $home overlaps registered copilot home $registered_home for $owner" >&2
  return 1
}
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(AP_HOME="$AP_HOME" AP_DATA_OVERRIDE="$DATA" "$AP_ROOT/bin/ap-project-mode.sh" "$project")
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; copilot routes support only no-mistakes and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$AP_HOME")
  abs_root=$(resolved_path "$AP_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active autopilot home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the autopilot repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active autopilot home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the autopilot repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active autopilot home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the autopilot repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_return_treehouse_home() {
  ...
  ( cd "$AP_ROOT" && treehouse return --force "$abs_home" >/dev/null ) || {
...
seed_project_rollback_target() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "REFUSED: unsafe created project rollback target $target has projects directory outside the copilot home" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_target"; then
    echo "REFUSED: unsafe created project rollback target $target is outside the copilot projects directory" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
  ...
  if [ -n "${SEED_HOME:-}" ] && [ "$SEED_HOME" != "/" ]; then
    if [ "$SEED_HOME_ACQUIRED" = 1 ]; then
      ...
      if [ -n "${SEED_BACKUP_DIR:-}" ] && [ "${SEED_HOME_BACKED_UP:-0}" = 1 ]; then
        restore_seed_file "$SEED_MARKER_EXISTED" "$SEED_BACKUP_DIR/marker" "$SEED_HOME/$COPILOT_HOME_MARKER"
        restore_seed_file "$SEED_CHARTER_EXISTED" "$SEED_BACKUP_DIR/charter.md" "$SEED_HOME/data/charter.md"
        restore_seed_file "$SEED_COPILOT_REG_EXISTED" "$SEED_BACKUP_DIR/sub-projects.md" "$SEED_HOME/data/projects.md"
      fi
    fi
  fi

  if [ -n "${SEED_BACKUP_DIR:-}" ]; then
    restore_seed_file "$SEED_PARENT_REG_EXISTED" "$SEED_BACKUP_DIR/parent-copilots.md" "$REG"
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(AP_ROOT_OVERRIDE='' AP_STATE_OVERRIDE='' AP_DATA_OVERRIDE='' AP_PROJECTS_OVERRIDE='' AP_CONFIG_OVERRIDE='' AP_HOME="$home" "$AP_ROOT/bin/ap-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
refuse_populated_projectless_home() {
  ...
  echo "error: cannot seed project-less copilot home $home because it contains project data" >&2
  if [ "${#clones[@]}" -gt 0 ]; then
...
refuse_projectful_projectless_charter() {
  ...
  printf 'error: cannot seed project-less copilot home because existing charter brief at %s conflicts with --no-projects\n' "$brief" >&2
  printf 'error: re-scaffold it with ap-brief.sh %s --copilot --no-projects or remove the stale brief before seeding\n' "$id" >&2
  return 1
}
...
seed_home() {
  ...
  if [ "$no_projects" -eq 1 ]; then
    ...
    [ $# -gt 0 ] || { echo "error: copilot needs at least one project, or --no-projects for a project-less home" >&2; return 1; }
  fi
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ap-home-seed.XXXXXX")
  ...
  if [ -f "$REG" ]; then
    ...
    cp "$REG" "$SEED_BACKUP_DIR/parent-copilots.md"
  fi
  ...
  if [ "$requested_home" = "-" ]; then
    ...
    home=$(acquire_treehouse_home "$id")
    SEED_HOME="$home"
    home=$(verify_autopilot_home "$home")
  else
  ...
  if [ -f "$home/$COPILOT_HOME_MARKER" ]; then
    ...
    cp "$home/$COPILOT_HOME_MARKER" "$SEED_BACKUP_DIR/marker"
  fi
  SEED_HOME_BACKED_UP=1

  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${AP_COPILOT_CHARTER:-}" ] || {
      echo "error: no filled copilot charter brief at $SEED_PARENT_BRIEF; set AP_COPILOT_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    if [ "$no_projects" -eq 1 ]; then
      "$AP_ROOT/bin/ap-brief.sh" "$id" --copilot --no-projects
    else
      "$AP_ROOT/bin/ap-brief.sh" "$id" --copilot "$@"
    fi
    SEED_PARENT_BRIEF_CREATED=1
  fi
  if grep -F '{TASK}' "$SEED_PARENT_BRIEF" >/dev/null 2>&1; then
    echo "error: copilot charter brief at $SEED_PARENT_BRIEF still contains {TASK}; fill it before seeding" >&2
    return 1
  fi
  charter_summary=$(registry_summary_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_summary" ] || {
    echo "error: copilot charter brief at $SEED_PARENT_BRIEF has an empty Charter section; fill it before seeding" >&2
    return 1
  }
  charter_scope=$(registry_scope_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_scope" ] || {
    echo "error: copilot charter brief at $SEED_PARENT_BRIEF has an empty Routing scope section; fill it before seeding" >&2
    return 1
  }
