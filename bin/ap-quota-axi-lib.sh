...
# produce a pilot-facing sign-out claim for a candidate that never read it.
#
# This file is the single owner of that version number. bin/ap-bootstrap.sh
...
AP_QUOTA_AXI_MIN=0.1.16

ap_quota_axi_compatible() {
  ...
  # The floor is compared from AP_QUOTA_AXI_MIN so bumping it needs one edit.
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$AP_QUOTA_AXI_MIN"
