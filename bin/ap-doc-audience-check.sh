#!/usr/bin/env bash
# ap-doc-audience-check.sh - validate the tracked documentation audience inventory.
#
# Usage:
#   bin/ap-doc-audience-check.sh
#   bin/ap-doc-audience-check.sh --root <repo> [--inventory <path>]
#
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Autopilot documentation audiences and local links.")
    ...
    try:
        ...
        print(f"ap-doc-audience-check: {exc}", file=sys.stderr)
        return 1
    print(f"ap-doc-audience-check: ok surfaces={surfaces} local_links={links}")
    return 0
