...
#
# An autopilot primary must arm the watcher or run a Codex checkpoint as a
# standalone verified harness call.
# bin/ap-arm-command-policy.mjs is the sole owner of shell classification,
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/ap-arm-pretool-check.sh
#   bin/ap-arm-pretool-check.sh --command '<cmd>' [--background true|false]
#
...
usage() {
  cat <<'EOF'
Usage: ap-arm-pretool-check.sh [--command <cmd>] [--background true|false] [--claude]
...
# ap-watch byte sequence AFTER the classifier's byte normalization, so a command
# that cannot contain ap-watch even after that normalization can never be a
...
# protected paths such as ap-watc\<newline>h-arm.sh or ap-"watch"-arm.sh still
# delegate. Stripping only these non-alphanumeric bytes can never destroy an
# existing ap-watch run.
#
# The fast path may allow ONLY when BOTH hold: (a) the stripped/normalized text
# lacks the ap-watch watcher substring, AND (b) the raw command carries no
...
# decodes and can therefore reconstruct ap-watch from bytes this cheap byte
# strip cannot. This marker set is COUPLED to the classifier's decoder set in
# bin/ap-arm-command-policy.mjs: adding any new quote/expansion form the
