#!/usr/bin/env bash
# ap-gate-refuse-lib.sh - fail-closed refusal that keeps a no-mistakes GATE agent
# out of autopilot's fleet lifecycle.
#
# The hazard (data/nm-gate-ambient-authority-containment-c3/report.md): a
# no-mistakes gate agent runs inside an autopilot checkout with a free shell, so
# it auto-loads autopilot's AGENTS.md, adopts the pilot identity, and - seeing
# flight crew "in flight" - reaches for ap-spawn.sh/ap-send.sh/ap-teardown.sh to
...
# THIS is the autopilot capability-removal half: an enforceable script refusal,
...
#
# A NORMAL autopilot session - a real primary checkout, a real treehouse/Orca
# flight crew worktree - has NEITHER signal and is COMPLETELY unaffected: the function
# returns 0 and the lifecycle proceeds exactly as before.
#
# This mirrors the unspoofable-marker precedent in bin/ap-marker-lib.sh: a signal
# the agent cannot forge, keyed on at a chokepoint, keeping the pattern familiar
# to autopilot maintainers. It layers ABOVE no-mistakes' separately-delivery
# HEAD-continuity guard, which remains the adversarial/residual backstop.
#
# TEST-HARNESS ESCAPE HATCH (AP_GATE_REFUSE_BYPASS=1): autopilot's own test suite
# must exercise the REAL ap-spawn/ap-send/ap-teardown, but the no-mistakes gate
...
# environment this guard refuses. So both signals would fire during autopilot's
# own validation and break unrelated tests. AP_GATE_REFUSE_BYPASS=1 makes the
# guard a no-op; autopilot's shared test helpers (tests/lib.sh and the backend
...
# bin/ap-spawn.sh directly after adopting autopilot's identity - it never sources
# autopilot's test helpers, so it never carries the bypass; and the adversarial
...
# tests/ap-gate-refuse.test.sh strips the bypass so it still verifies real refusal.
#
# Sourced by bin/ap-spawn.sh, bin/ap-send.sh, bin/ap-teardown.sh,
# bin/ap-sessionstart-nudge.sh, and the tests.
...
AP_GATE_REFUSE_EXIT=3

# ap_is_gate_agent: return 0 without output when this process looks like a
...
ap_is_gate_agent() {
  ...
  if [ "${AP_GATE_REFUSE_BYPASS:-}" = 1 ]; then
    return 1
  fi
  if [ "${NO_MISTAKES_GATE+x}" = x ]; then
    AP_GATE_REFUSE_REASON='env'
    return 0
  fi
  ...
  case "$common" in
    */.no-mistakes/repos/*.git)
      AP_GATE_REFUSE_REASON='path'
      AP_GATE_REFUSE_COMMON=$common
      return 0 ;;
  esac
  return 1
}

# ap_refuse_if_gate_agent: exit AP_GATE_REFUSE_EXIT with a clear stderr message if
# this process looks like a no-mistakes gate agent. Call before any fleet
# mutation. No-ops (returns 0) for a normal autopilot session, or when autopilot's
# own test harness sets AP_GATE_REFUSE_BYPASS=1 (see the header).
ap_refuse_if_gate_agent() {
  ap_is_gate_agent "${1:-.}" || return 0
  if [ "$AP_GATE_REFUSE_REASON" = env ]; then
    echo "error: no-mistakes gate agent must not drive the fleet (NO_MISTAKES_GATE set)" >&2
  else
    echo "error: refusing fleet lifecycle from inside a no-mistakes gate worktree ($AP_GATE_REFUSE_COMMON)" >&2
  fi
  exit "$AP_GATE_REFUSE_EXIT"
}
