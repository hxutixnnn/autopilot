...
# teardown, and copilot backlog handoff.
# Usage: . bin/ap-tasks-axi-lib.sh
...
# by copilot handoffs (introduced in tasks-axi 0.2.2).
# `config/backlog-backend=manual` opts out of tasks-axi for routine autopilot
# backlog mutations, but validated copilot handoffs always use `tasks-axi mv`.
# Absent or any other value keeps the default tasks-axi backend path, falling
# back to manual mutation when the tool is not compatible.

ap_tasks_axi_version_parts() {
  local output
...
ap_tasks_axi_compatible() {
  local parts major minor patch rest
  parts=$(ap_tasks_axi_version_parts) || return 1
  [ -n "$parts" ] || return 1
  major=${parts%% *}
  rest=${parts#* }
  minor=${rest%% *}
  ...
  if [ "$major" -gt 0 ] ||
    ...
    ap_tasks_axi_update_has_archive_body && ap_tasks_axi_mv_has_multi_id
    return $?
  fi
  return 1
}

ap_tasks_axi_update_has_archive_body() {
  local output
  command -v tasks-axi >/dev/null 2>&1 || return 1
...
ap_backlog_backend_manual() {
  local config_dir=$1
  [ "$(ap_backlog_backend_value "$config_dir")" = manual ]
}

ap_tasks_axi_backend_available() {
  local config_dir=$1
  ap_backlog_backend_manual "$config_dir" && return 1
  ap_tasks_axi_compatible
}
