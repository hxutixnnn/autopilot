...
# for Claude primaries (main home and marked copilot homes):
#
#   - Scope: only a genuine primary checkout (plain checkout or validly marked
#     copilot home) with AGENTS.md, bin/, and the effective state dir - the
#     exact ap-turnend-guard.sh scope. Child flight crew/recon worktrees stay inert.
...
#     the hook delegates guarded recovery to bin/ap-lock.sh and then re-verifies
...
#   - Need: arms only while work is in flight (state/*.meta); an idle home exits 0.
...
#   - Foreground arm: the owner runs bin/ap-watch-arm.sh in the FOREGROUND of
...
# outcome so the synchronous Stop guard (bin/ap-turnend-guard.sh --claude) can
...
if ! ap_session_lock_owned_by_self "$STATE"; then
  LOCK_PID=$(cat "$STATE/.lock" 2>/dev/null || true)
  case "$LOCK_PID" in
    ''|*[!0-9]*) exit 0 ;;
  esac
  ap_harness_pid_alive "$LOCK_PID" && exit 0
...
# --- need: in-flight work ------------------------------------------------------
need_supervision() {
  ap_supervision_needed "$STATE" "$GRACE"
}
need_supervision || exit 0

# --- stale session-lock recovery ---------------------------------------------
# Delegate the claim to ap-lock.sh so its live-owner refusal and write semantics
# remain the single acquisition owner, then re-verify current-session identity
# before touching any auto-arm state.
if [ "$RECOVER_SESSION_LOCK" -eq 1 ]; then
  "$SCRIPT_DIR/ap-lock.sh" >/dev/null 2>&1 || exit 0
  ap_session_lock_owned_by_self "$STATE" || exit 0
fi
...
ap_lock_try_acquire "$OWNER_LOCK" || exit 0
trap 'ap_lock_release "$OWNER_LOCK"' EXIT
...
if [ -n "$OUT" ]; then
  "$SCRIPT_DIR/ap-watch-arm.sh" >"$OUT" 2>&1
  RC=$?
else
  "$SCRIPT_DIR/ap-watch-arm.sh" >/dev/null 2>&1
  RC=$?
fi
...
# The need may have vanished mid-cycle because the fleet was cleaned up: nothing
...
if [ "$FAILED" -eq 1 ]; then
  {
    printf 'autopilot watcher cycle FAILED - supervision is down while this home still needs it.\n'
    [ -n "$OUT" ] && grep -E '^(watcher:|signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Run bin/ap-wake-drain.sh first. Then repair supervision with bin/ap-watch-arm.sh as its own Claude Code background task (never shell &). If the failure repeats, treat it as a blocker and report it instead of ending blind.\n'
  } >&2
else
  {
    printf 'autopilot watcher wake - one supervision event needs a handling turn now.\n'
    [ -n "$OUT" ] && grep -E '^(signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Run bin/ap-wake-drain.sh first and handle the wake. This Stop hook owns watcher continuity: when the handling turn ends, the next needed cycle arms automatically - do NOT run bin/ap-watch-arm.sh after an ordinary wake.\n'
  } >&2
fi
[ -z "$OUT" ] || rm -f "$OUT" 2>/dev/null || true
exit 2
