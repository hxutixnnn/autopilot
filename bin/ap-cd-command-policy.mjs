...
// PRIMARY autopilot shell's own working directory?
//
// A stray persistent top-level `cd projects/<clone>` silently relocates the
// primary shell, so the next autopilot-owned command (a backlog write, an
// ap-* lifecycle call, tasks-axi) runs inside a project clone instead of the
// home. This policy blocks exactly that class of command; the environmental
// scoping to the real primary checkout lives in the bin/ap-cd-pretool-check.sh
...
// bin/ap-arm-command-policy.mjs, the sole owner of autopilot's shell
...
import { Lexer, splitProgram, commandPosition } from "./ap-arm-command-policy.mjs";
import { realpathSync } from "node:fs";
import { fileURLToPath } from "node:url";

const REASONS = {
  "persistent-cd":
    "a persistent top-level directory change in the primary autopilot checkout is blocked; it would move the shell out of the home so a later autopilot-owned command runs inside a project clone. Reach the target without moving the shell - use git -C <dir> or an absolute path on the command itself - or scope the cd to a subshell like (cd <dir> && ...).",
};
