#!/usr/bin/env bash
# ap-afk-return.sh - deterministic away-mode return catch-up gate.
#
# Usage:
#   ap-afk-return.sh          Stop away mode, drain catch-up, and open/check gate.
#   ap-afk-return.sh begin    Same as the default command.
#   ap-afk-return.sh check    Re-drain and close the gate only after blockers resolve.
#   ap-afk-return.sh guard    Read-only refusal while away or catch-up is pending.
#
# `blocked:` is the flight crew member protocol's autopilot-actionable verb. A live task's
...
# ordinary pilot request may proceed. `needs-decision:` belongs to the
...
# read entrypoints such as ap-bearings-snapshot.sh.
set -u
...
print_blockers() {  # <file>
  ...
  while IFS="$(printf '\t')" read -r tag id key summary; do
    ...
    printf 'autopilot-actionable blocker: %s [key=%s] %s\n' "$id" "$key" "$summary"
  done < "$file"
}
...
return_guard() {
  if [ -e "$STATE/.afk" ]; then
    printf 'ap-afk-return: away mode is still active; run bin/ap-afk-return.sh before ordinary pilot work\n' >&2
    return 3
  fi
  if [ -e "$GATE" ]; then
    printf 'ap-afk-return: return catch-up is pending; remediate or durably reclassify every listed blocker, then run bin/ap-afk-return.sh check\n' >&2
...
return_reconcile() {
  ...
  if [ "$lifecycle_ok" -ne 1 ] || [ -s "$blockers" ]; then
    ...
    printf 'ap-afk-return: catch-up must finish before the pilot request\n' >&2
    print_evidence "$GATE" >&2
    print_blockers "$GATE" >&2
    printf 'ap-afk-return: handle each blocker now, or close it with resolved [key=...] and append a durable reclassification reason, then run bin/ap-afk-return.sh check\n' >&2
  ...
  clear_delivery_artifacts
  rm -f "$evidence" "$blockers"
  printf 'ap-afk-return: catch-up clear; ordinary pilot work may proceed\n'
  return 0
}
...
main() {
  ...
  # `guard` returned above without sourcing ap-wake-lib.sh, whose initialization
  ...
  trap 'ap_lock_release "$LOCK"' EXIT
  write_pending_seed || { ap_lock_release "$LOCK"; trap - EXIT; return 1; }
  return_reconcile
  rc=$?
