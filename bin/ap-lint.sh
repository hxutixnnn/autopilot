#!/usr/bin/env bash
# ap-lint.sh - the single owner of autopilot's shell-lint definition.
#
...
# deterministic shard and root order after every worker finishes. AP_LINT_JOBS=1
...
#
# Usage:
#   ap-lint.sh                         lint the canonical file set
#   ap-lint.sh <path>...               lint explicit roots with the same config
#   ap-lint.sh --jobs <1|2> [path]...  override bounded worker count
#   ap-lint.sh --telemetry <path> ...  write a quiet metrics snapshot
#   ap-lint.sh --required-version      print the ShellCheck pin
#   ap-lint.sh --list-files            print the canonical file set
#   ap-lint.sh --help                  print this usage
set -u

REQUIRED_SHELLCHECK=0.11.0
SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SELF="$SELF_DIR/ap-lint.sh"
ROOT="$(cd "$SELF_DIR/.." && pwd)"
cd "$ROOT" || exit 1

AP_LINT_WORKER_SHELLCHECK_PID=
# shellcheck disable=SC2329 # Registered by the private worker's signal traps.
ap_lint_worker_stop() {
  [ -n "$AP_LINT_WORKER_SHELLCHECK_PID" ] || return 0
  kill "$AP_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  wait "$AP_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  AP_LINT_WORKER_SHELLCHECK_PID=
}

ap_lint_worker() {  # <manifest> <output-dir> <shard-index>
  ...
  if [ "${#roots[@]}" -gt 0 ]; then
    trap 'ap_lint_worker_stop; exit 129' HUP
    trap 'ap_lint_worker_stop; exit 130' INT
    trap 'ap_lint_worker_stop; exit 143' TERM
    "$AP_LINT_SHELLCHECK" --norc --external-sources -- "${roots[@]}" > "$output.out" 2>&1 &
    AP_LINT_WORKER_SHELLCHECK_PID=$!
    wait "$AP_LINT_WORKER_SHELLCHECK_PID" || rc=$?
    AP_LINT_WORKER_SHELLCHECK_PID=
    trap - HUP INT TERM
  else
    : > "$output.out"
...
if [ "${1:-}" = "--internal-worker" ]; then
  [ "${AP_LINT_INTERNAL:-}" = 1 ] || {
    printf 'ap-lint.sh: --internal-worker is private to the lint owner.\n' >&2
    exit 2
  }
  [ "$#" -eq 4 ] && [ -n "${AP_LINT_SHELLCHECK:-}" ] || exit 2
  ap_lint_worker "$2" "$3" "$4"
  exit $?
fi
...
JOBS=${AP_LINT_JOBS:-2}
TELEMETRY=${AP_LINT_TELEMETRY:-}
LIST_FILES=0
while [ "$#" -gt 0 ]; do
  case "$1" in
    --jobs)
      [ "$#" -ge 2 ] || { printf 'ap-lint.sh: --jobs requires 1 or 2.\n' >&2; exit 2; }
      JOBS=$2
      shift 2
      ;;
    --jobs=*)
      ...
      ;;
    --telemetry)
      [ "$#" -ge 2 ] || { printf 'ap-lint.sh: --telemetry requires a path.\n' >&2; exit 2; }
      TELEMETRY=$2
      shift 2
      ;;
    --telemetry=*)
...
case "$JOBS" in
  1|2) ;;
  *) printf 'ap-lint.sh: jobs must be 1 or 2, got %s.\n' "$JOBS" >&2; exit 2 ;;
esac
...
if [ "$LIST_FILES" -eq 1 ]; then
  [ "$#" -eq 0 ] || {
    printf 'ap-lint.sh: --list-files does not accept explicit paths.\n' >&2
    exit 2
  }
  printf '%s\n' "${ROOTS[@]}"
  exit 0
...
if ! command -v shellcheck >/dev/null 2>&1; then
  printf 'ap-lint.sh: ShellCheck not found; install ShellCheck %s for CI parity.\n' \
...
if ! PERL_BIN=$(command -v perl); then
  printf 'ap-lint.sh: perl is required for bounded worker cleanup.\n' >&2
  exit 127
fi
resolved=$("$SHELLCHECK_BIN" --version | awk '/^version:/ {print $2; exit}')
printf 'ap-lint.sh: ShellCheck %s (pinned %s)\n' "$resolved" "$REQUIRED_SHELLCHECK" >&2
if [ "$resolved" != "$REQUIRED_SHELLCHECK" ]; then
  printf 'ap-lint.sh: ShellCheck %s required for CI parity, found %s. Install %s.\n' \
...
if [ -n "$TELEMETRY" ]; then
  ...
  [ -d "$telemetry_parent" ] || {
    printf 'ap-lint.sh: telemetry directory does not exist: %s\n' "$telemetry_parent" >&2
    exit 2
  }
fi

TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ap-lint.XXXXXX") || exit 1
ACTIVE_PIDS=()
...
for path in "${ROOTS[@]}"; do
  case "$path" in
    *"$TAB"*|*$'\n'*)
      printf 'ap-lint.sh: paths containing tabs or newlines are not supported: %s\n' "$path" >&2
      exit 2
      ;;
  esac
  if [ -f "$path" ]; then
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_START=$(ap_lint_shellcheck_count)
  TELEMETRY_LOAD_START=$(ap_lint_load_average)
  TELEMETRY_CPU_START=$(ap_lint_aggregate_cpu)
fi

ap_lint_run_worker() {  # <worker-index>
  ...
  if [ -n "$TELEMETRY" ] && [ -x /usr/bin/time ]; then
    if [ "$(uname)" = Darwin ]; then
      exec "$PERL_BIN" -e 'setpgrp(0, 0) or die "setpgrp: $!"; exec @ARGV or die "exec: $!"' \
        ...
        env AP_LINT_INTERNAL=1 AP_LINT_SHELLCHECK="$SHELLCHECK_BIN" \
...
ap_lint_start_worker() {
  ap_lint_run_worker "$1" &
  ACTIVE_PIDS+=("$!")
}

ap_lint_wait_workers() {
  local pid
...
while [ "$worker" -lt "$SHARD_COUNT" ]; do
  ...
  if [ -f "$output.rc" ]; then
    ...
    printf 'ap-lint.sh: worker produced no result for shard %s.\n' "$worker" >&2
    rc=2
  fi
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_END=$(ap_lint_shellcheck_count)
  TELEMETRY_LOAD_END=$(ap_lint_load_average)
  TELEMETRY_CPU_END=$(ap_lint_aggregate_cpu)
...
EOF
  ...
  {
    printf 'format\tap-lint-telemetry-v1\n'
    printf 'git_head\t%s\n' "$git_head"
  ...
  if ! mv -f "$telemetry_tmp" "$TELEMETRY"; then
    printf 'ap-lint.sh: could not write telemetry to %s.\n' "$TELEMETRY" >&2
