...
#
# A stray persistent top-level `cd projects/<clone>` in the PRIMARY autopilot
# shell silently relocates the shell, so a later autopilot-owned command (a
# backlog write, an ap-* lifecycle call, tasks-axi) runs inside a project clone
# instead of the home. This seatbelt denies such a command before it runs.
# bin/ap-cd-command-policy.mjs is the sole owner of the block/allow decision; it
# reuses the shell classifier owned by bin/ap-arm-command-policy.mjs. This
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/ap-cd-pretool-check.sh
#   bin/ap-cd-pretool-check.sh --command '<cmd>'
#
...
#
# Exit/output contract (identical shape to bin/ap-arm-pretool-check.sh):
#   ALLOW - exit 0 and no output.
#   DENY - exit 2, a Claude-shaped deny object on stderr, and a Grok-shaped
#          deny object on stdout unless --claude was supplied.
#   INERT - not the real primary checkout (a flight crew member/recon task worktree or a
...
usage() {
  cat <<'EOF'
Usage: ap-cd-pretool-check.sh [--command <cmd>] [--claude]

With no --command, reads a PreToolUse-style JSON payload on stdin (Grok
toolInput.command, or Claude/Codex tool_input.command).
Fires only in the real primary autopilot checkout; it is a silent no-op in a
flight crew member/recon task worktree or any non-autopilot repo.
...
# decoder set in bin/ap-arm-command-policy.mjs: adding any new quote/expansion
...
# Scope to a plain, non-worktree autopilot checkout, where git-dir equals
# git-common-dir. A flight crew member/recon task worktree - the shape bin/ap-spawn.sh
# always hands out - is a linked git worktree where the two differ. This guard
# does not inspect .ap-copilot-home, so it applies in a git-cloned copilot
# home but remains inert when the copilot home is itself a treehouse-leased
...
[ -f "$AP_ROOT/AGENTS.md" ] || exit 0
[ -d "$AP_ROOT/bin" ] || exit 0
command -v git >/dev/null 2>&1 || exit 0
GIT_DIR=$(git -C "$AP_ROOT" rev-parse --git-dir 2>/dev/null) || exit 0
GIT_COMMON_DIR=$(git -C "$AP_ROOT" rev-parse --git-common-dir 2>/dev/null) || exit 0
[ "$GIT_DIR" = "$GIT_COMMON_DIR" ] || exit 0

POLICY="$AP_ROOT/bin/ap-cd-command-policy.mjs"
