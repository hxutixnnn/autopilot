#!/usr/bin/env bash
# ap-copilot-report.sh - optional helper to append a correlated parent report.
#
# A copilot answering a marked from-autopilot request must report on the
...
# (bin/ap-pending-reply-lib.sh).
#
# Usage:
#   ap-copilot-report.sh <status-file> <verb> <corr_id> <note...>
#   ap-copilot-report.sh --doc <status-file> <verb> <corr_id> <doc-path> <note...>
#
# Examples:
#   ap-copilot-report.sh "$STATUS" done abcdef0123456789 "audit clean"
#   ap-copilot-report.sh --doc "$STATUS" done abcdef0123456789 data/x/report.md "see report"
#
# The status file must be the absolute parent route from the copilot charter
# (state/<id>.status under the PARENT home), never a path relative to this
# copilot home. Writing under the wrong home is detected as supporting
