#!/usr/bin/env bash
# ap-vendor-auth-probe.sh - one hard-bounded, non-destructive authentication
...
# eligible. The dispatching autopilot owns that judgment from `quota-axi`'s data
...
# pilot's 2026-07-30 `autopilot-grok-auth-preflight` decision approved exactly
...
#
# Usage:
#   ap-vendor-auth-probe.sh <probe>
#
# Environment:
#   AP_VENDOR_AUTH_PROBE_TIMEOUT   hard per-command bound in seconds; must be a
...
usage() {
  cat <<'EOF'
ap-vendor-auth-probe.sh - one hard-bounded, non-destructive authentication probe
...
eligibility. The dispatching autopilot owns that judgment.
...
Environment:
  AP_VENDOR_AUTH_PROBE_TIMEOUT   hard per-command bound in seconds (default 20);
...
die_usage() {
  printf 'ap-vendor-auth-probe: %s\n' "$1" >&2
  printf 'usage: ap-vendor-auth-probe.sh <probe>   (registered probes: grok)\n' >&2
  exit 2
}
...
TIMEOUT=${AP_VENDOR_AUTH_PROBE_TIMEOUT:-20}
case "$TIMEOUT" in
  ''|*[!0-9]*|0*) TIMEOUT=20 ;;
esac

# Bounded execution, mirroring bin/ap-fleet-snapshot.sh's run_timed selection so
