#!/usr/bin/env bash
# ap-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/ap-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no autopilot turn) or
# ESCALATES a batched, distilled digest to the supervisor pane on
# pilot-relevant events plus bounded declared-pause rechecks. This is the
# token-efficient replacement for the prior always-inject daemon: routine
# signal/stale/heartbeat wakes cost zero autopilot context; only done/
...
# user message clears it and autopilot resumes full responsiveness.
# When afk is off, normal ap-watch.sh always-on triage is the active mechanism.
...
#
# IN-BAND OPERATIONAL INPUT. bin/ap-operational-input.sh constructs every
# current daemon injection as the typed away-supervisor kind after the stable
# AP_OPERATIONAL_PREFIX. A human cannot type its leading U+2063 from a normal
# keyboard at the start of a message, and Herdr transports it as text.
# Autopilot's contract: a message that starts with the current prefix, or a
# legacy bare-marker daemon escalation, is internal (stay afk); an unmarked
# message means the pilot is back (exit afk, flush catch-up, resume per-wake
...
#     crash/restart/missed injection is recovered on the next ap-wake-drain.sh.
...
#     (configurable), rechecked once. A wedged flight crew member is therefore detected
...
#     Flight crew members are autonomous, so a delayed stale response does not stall a
#     healthy flight crew member's own progress.
#     Buffered escalation delivery also has a max-defer alarm: if a digest stays
#     undelivered past AP_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#     state/*.status for a pilot-relevant line the per-wake classifier might
#     have missed (e.g. a status verb outside PILOT_RE) and escalates it.
#
...
#          AP_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   pilot-relevant escalation for matching
#                                   kinds.
#          AP_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          AP_PAUSE_RESURFACE_SECS  idle seconds before a declared external wait
#                                   re-surfaces as a recheck (default 3600)
#          AP_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          AP_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          AP_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          AP_BUSY_REGEX            optional rendered busy-signature override
#                                   for delivery guards and Grok's fallback
#          AP_COMPOSER_IDLE_RE      empty-composer regex applied after dim-ghost
#                                   and structural border stripping (default:
#                                   bare prompt glyphs plus busy footers)
#          AP_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          AP_WEDGE_ALARM_CHANNEL   override config/wedge-alarm with a single
...
#          AP_WEDGE_ALARM_EXEC      notifier seam: when set, every notifier
...
#          AP_WEDGE_ALARM_TIMEOUT_SECS seconds allowed for each notifier before
#                                   its watchdog terminates it and continues to the
#                                   next channel (default 10; invalid/zero uses the
#                                   default).
#          AP_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/ap-tmux-lib.sh):
...
#          AP_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          AP_LOG_MAX_BYTES / AP_LOG_KEEP_LINES / AP_CRASH_*  log + crash guards
#          AP_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing ap-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

AP_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AP_ROOT="${AP_ROOT_OVERRIDE:-$(cd "$AP_DAEMON_DIR/.." && pwd)}"
...
# corrected composer detection. Stale task rechecks use ap-backend.sh below.
# shellcheck source=bin/ap-tmux-lib.sh
. "$AP_DAEMON_DIR/ap-tmux-lib.sh"

# shellcheck source=bin/ap-backend.sh
. "$AP_DAEMON_DIR/ap-backend.sh"

# Canonical construction and parsing for every Autopilot operational input.
# shellcheck source=bin/ap-operational-input.sh
. "$AP_DAEMON_DIR/ap-operational-input.sh"

# Shared wake classifier (last_status_line, status_is_pilot_relevant,
# window_to_task, scan_pilot_relevant_statuses). The SAME library backs the
# always-on watcher's triage, so the pilot-relevant verb set and the
# classification predicates have exactly one definition.
# shellcheck source=bin/ap-classify-lib.sh
. "$AP_DAEMON_DIR/ap-classify-lib.sh"

# Supervisor-pane discovery (AP_SUPERVISOR_TARGET_DEFAULT,
# AP_SUPERVISOR_BACKEND_DEFAULT, discover_supervisor_target,
# discover_supervisor_backend). Shared with the script-owned away launcher
# (bin/ap-afk-launch.sh) so the pilot-pane resolution has exactly one owner.
# shellcheck source=bin/ap-supervisor-target-lib.sh
. "$AP_DAEMON_DIR/ap-supervisor-target-lib.sh"

# The single owner of semantic busy state for recorded tasks
# (ap_busy_classify).
# shellcheck source=bin/ap-busy-lib.sh
. "$AP_DAEMON_DIR/ap-busy-lib.sh"

# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today. zellij, orca,
# and cmux are real backends elsewhere in autopilot (bin/ap-backend.sh) but this
...
# pane.
AP_SUPERVISOR_SUPPORTED_BACKENDS="tmux herdr"
...
# The pilot-relevant verb set and the status classifiers (last_status_line,
# status_is_pilot_relevant, window_to_task, scan_pilot_relevant_statuses) now
# live in bin/ap-classify-lib.sh, shared with the always-on watcher.
# Composer-empty detection, submit acknowledgement, and the harness-scoped
# supervisor-pane busy guard live in bin/ap-tmux-lib.sh.
# AP_BUSY_REGEX also overrides Grok's isolated task-state fallback.
...
# --- presence-gating --------------------------------------------------------
# bin/ap-operational-input.sh owns the U+2063 AUTOPILOT_OP bytes and typed
...
# Resolve the effective state dir. AP_STATE_OVERRIDE wins (testing); otherwise
# $AP_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${AP_STATE_OVERRIDE:-$AP_HOME/state}"; }

# --- portable stat (same trap as ap-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by autopilot on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes autopilot's afk-exit contract as a testable function.
...
#   anything else           -> 0 (pilot is back; exit afk)
# Bias toward exit: only the marker and an explicit /afk invocation keep afk
# alive. A false exit is self-correcting (the pilot re-runs /afk).
...
# marker (a daemon escalation), 1 otherwise (a real user message). Autopilot's
# afk-exit contract uses this: marker present -> stay afk; absent -> pilot is
# back. Bias ambiguous cases toward exit (a false exit is self-correcting).
message_is_injection() {  # <message-text>
  local msg=$1
  [ -n "$msg" ] || return 1
  case "$msg" in
    "$AP_INJECT_MARK"*) return 0 ;;
  esac
  return 1
}

# strip_injection_marker: remove a current typed away envelope, the landed
# untyped AUTOPILOT_OP prefix, or the legacy bare sentinel. Current grammar is
# delegated to its owner rather than reimplemented here.
strip_injection_marker() {  # <message-text>
  local msg=$1 body
  if ap_operational_input_body "$msg" body; then
    printf '%s' "$body"
    return
  fi
  case "$msg" in
    "$AP_OPERATIONAL_PREFIX"*) msg=${msg#"$AP_OPERATIONAL_PREFIX"} ;;
    "$AP_INJECT_MARK"*) msg=${msg#"$AP_INJECT_MARK"} ;;
  esac
  printf '%s' "$msg"
}
...
# bin/ap-supervisor-target-lib.sh (sourced above). ap_super_main below calls
# them exactly as before; the away launcher reuses the identical resolution to
# pass the pilot pane in as AP_SUPERVISOR_TARGET.

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_pilot_relevant, window_to_task, and
# scan_pilot_relevant_statuses come from bin/ap-classify-lib.sh (sourced above),
# the single classifier shared with bin/ap-watch.sh. The decision-string wrappers
...
# summary autopilot would otherwise have to re-read.
...
classify_stale() {  # <window> <state>
  ...
  if [ -n "$last" ] && status_is_paused "$last"; then
    # A DECLARED external-wait pause (ap-classify-lib.sh): an idle pane is EXPECTED,
    ...
    # status line already read, no ap-flight-crew-state.sh call, mirroring the daemon's
    # existing status-log classification.
    printf 'pause|paused (awaiting external), rechecked on a long cadence: %s' "$last"
    return
  fi
  if [ -n "$last" ] && status_is_pilot_relevant "$last"; then
    # Independent of free-text pilot-relevant matching: a nonterminal progress
    ...
    # prose once looked pilot-relevant. Real terminal verbs and legacy free-text
    # pilot lines without those verbs keep the terminal escalate/dedupe path.
    if ! status_is_terminal_verb "$last"; then
      case "$(status_line_verb "$last")" in
        working|resolved|pilot-held)
...
classify_check() {  # <full reason>  — check scripts print only when autopilot should wake
  printf 'escalate|%s' "$1"
}
...
migrate_watcher_pause_markers() {  # <state>
  ...
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    win=$(ap_backend_target_of_meta "$meta")
    [ -n "$win" ] || continue
...
# Record the seen-status marker for a pilot-relevant status line so the
...
# Mark every pilot-relevant status line a per-wake classification escalated as
...
# bin/ap-backend.sh's generic per-backend primitives rather than a hand-rolled
...
ap_daemon_primary_harness() {
  if [ -z "${AP_DAEMON_PRIMARY_HARNESS:-}" ]; then
    AP_DAEMON_PRIMARY_HARNESS=$("$AP_DAEMON_DIR/ap-harness.sh" 2>/dev/null || printf 'unknown')
    [ -n "$AP_DAEMON_PRIMARY_HARNESS" ] || AP_DAEMON_PRIMARY_HARNESS=unknown
  fi
  printf '%s' "$AP_DAEMON_PRIMARY_HARNESS"
}

pane_is_busy() {  # <target> [backend]
  local target=$1 backend=${2:-tmux} native tail40 harness
  harness=$(ap_daemon_primary_harness)
  native=$(ap_backend_busy_state "$backend" "$target" 2>/dev/null)
  case "$native" in
    busy) return 0 ;;
  esac
  tail40=$(ap_backend_capture "$backend" "$target" 40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -12 \
    | ap_busy_lines_match "$harness"
}

# pane_input_pending dispatches through ap_backend_composer_state and treats
...
pane_input_pending() {  # <target> [backend]
  ...
  [ "$(ap_backend_composer_state "$backend" "$target" 2>/dev/null)" != empty ]
}
...
# semantic busy-state contract (bin/ap-busy-lib.sh), 1 when it is not, and 2
...
stale_window_is_busy() {  # <window> <state>
  ...
  task=$(window_to_task "$win" "$state")
  label="ap-$task"
  tail40=$(ap_backend_capture "$backend" "$win" 40 "$label" 2>/dev/null) || return 2
  verdict=$(ap_busy_classify "$backend" "$win" "$harness" "$task" "$state" "$tail40")
  [ "${verdict%% *}" = busy ]
}
...
# herdr notification, or a pilot-supplied command (push to a phone, etc.).
...
# non-empty, non-comment line. AP_WEDGE_ALARM_CHANNEL overrides the file with a
...
# pilot explicitly disables it.

# Print the configured channel directives, one per line. AP_WEDGE_ALARM_CHANNEL
...
wedge_alarm_configured_channels() {
  ...
  if [ -n "${AP_WEDGE_ALARM_CHANNEL:-}" ]; then
    printf '%s\n' "$AP_WEDGE_ALARM_CHANNEL"
    return 0
  fi
  cfg="${AP_CONFIG_OVERRIDE:-$AP_HOME/config}/wedge-alarm"
  if [ -f "$cfg" ]; then
...
# pilot via an osascript Notification Center banner; other platforms have no
# built-in OS channel (the pilot wires a command: directive), so this prints
...
wedge_alarm_run_bounded() {
  ...
  shift
  timeout=${AP_WEDGE_ALARM_TIMEOUT_SECS:-$WEDGE_ALARM_TIMEOUT_SECS_DEFAULT}
  case "$timeout" in
...
# AP_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier: the resolved channel
# name and summary are handed to that command instead of ever invoking osascript
# or herdr or a pilot-supplied command. This is the one injection point the test harness forces to a recorder
...
wedge_alarm_os_notifier_override() {  # <channel> <summary>
  local channel=$1 summary=$2 rc exec_override=${AP_WEDGE_ALARM_EXEC:-}
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    -e 'display notification (item 1 of argv) with title "autopilot: away-mode escalations WEDGED" sound name "Basso"' \
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "autopilot: away-mode escalations WEDGED" \
    --body "$summary" --sound request >/dev/null 2>&1 && return 0
  log "wedge alarm: herdr notification failed"
  return 1
}

# Run a pilot-supplied command with the summary on $1 and on stdin, so an
# alert can reach a phone/pager (ntfy, Slack, SMS) even when the pilot is away
...
wedge_alarm_via_command() {  # <cmd> <summary>
  ...
  wedge_alarm_run_bounded command sh -c "$cmd" ap-wedge-alarm "$summary" \
...
wedge_alarm_emit() {  # <channel> <summary>
  local channel=$1 summary=$2 cmd=${3:-} rc exec_override=${AP_WEDGE_ALARM_EXEC:-} WEDGE_ALARM_EMIT_ACTIVE=1
  case "$exec_override" in
    '') ;;
    discard) return 0 ;;
    *)
...
# an ERROR, drops a durable marker autopilot/recovery can surface, flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  max_defer="${AP_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}"
  ...
  {
    printf 'ap away-mode inject WEDGED: %ss undelivered as of %s\n' "$age" "$(date '+%Y-%m-%dT%H:%M:%S%z')"
    printf 'The supervisor pane could not accept an escalation. Buffered items:\n'
    cat "$state/.subsuper-escalations" 2>/dev/null
  } 2>/dev/null > "$marker" || true
  target="${AP_SUPERVISOR_TARGET:-$AP_SUPERVISOR_TARGET_DEFAULT}"
  backend="${AP_SUPERVISOR_BACKEND:-$AP_SUPERVISOR_BACKEND_DEFAULT}"
  ...
  if [ "$backend" = tmux ]; then
    tmux display-message -t "$target" "ap: away-mode escalations WEDGED ${age}s — see $marker" 2>/dev/null || true
  fi
  # Backend-independent active alert. Unlike the tmux flash above (skipped on
  # every non-tmux backend), this can reach the pilot even when every pane and
...
#     pilot-relevant line the per-wake classifier missed and escalate it.
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${AP_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${AP_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${AP_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
  ...
  # the window repeats.
  pause_secs=${AP_PAUSE_RESURFACE_SECS:-$AP_PAUSE_RESURFACE_SECS_DEFAULT}
  ...
  # (3) heartbeat scan (catch-all for a pilot-relevant status the per-wake
  #     classifier may have missed). Cheap: status files only, no tmux. The
  #     pilot-relevant filtering is the shared classifier's
  #     scan_pilot_relevant_statuses; the daemon layers its digest dedup on top.
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${AP_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
    ...
    while IFS="$(printf '\t')" read -r f task last; do
      ...
      mark_status_seen "$state" "$task" "$last"
    done < <(scan_pilot_relevant_statuses "$state")
  fi
}
...
window_for_task() {  # <task-key> [state]
  ...
  for meta in "$state"/*.meta; do
    ...
    w=$(ap_backend_target_of_meta "$meta")
    [ -n "$w" ] && { printf '%s' "$w"; return 0; }
  done
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':ap-' || true); do
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; autopilot drives the normal always-on
  ...
  ap_operational_input_encode away-supervisor "$msg" encoded || return 1
  msg=$encoded
  target="${AP_SUPERVISOR_TARGET:-$AP_SUPERVISOR_TARGET_DEFAULT}"
  # BACKEND-AWARE (previously a raw `tmux display-message` pane-exists probe):
  # dispatches through bin/ap-backend.sh so a herdr supervisor pane is checked
  # via the herdr adapter instead of always assuming tmux. Falls back to tmux
  # when unset (sourced/test contexts that never ran ap_super_main's startup
  # discovery), matching this function's pre-existing default assumption.
  backend="${AP_SUPERVISOR_BACKEND:-tmux}"
  ap_backend_target_exists "$backend" "$target" || return 1
  ...
  #      composer. The shared classifier (ap_backend_composer_state ->
  #      ap_composer_classify_content, bin/ap-composer-lib.sh) reports 'pending'
  ...
  composer=$(ap_backend_composer_state "$backend" "$target" 2>/dev/null)
  ...
  # Dispatches through ap_backend_send_text_submit (bin/ap-backend.sh): for
  # backend=tmux this calls ap_backend_tmux_send_text_submit, a verbatim
  # re-export of ap_tmux_submit_core - byte-identical to calling it directly.
  retries=${AP_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${AP_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(ap_backend_send_text_submit "$backend" "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
...
should_force_self() {  # <reason>
  local reason=$1 skip="${AP_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (AP_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  case "$action" in
    escalate)
      ...
      mark_escalated_seen "$kind" "$arg" "$state"
      [ "${AP_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
      ;;
    pause)
    ...
    *)
      ...
      # housekeeping can age it, and drop any pause marker (a flight crew that left its
      # pause reverts to normal wedge aging). The persistence recheck, not this
      # wake, escalates a wedge.
      if [ "$kind" = "stale" ]; then
        task=$(window_to_task "$arg" "$state")
        last=$(last_status_line "$state/$task.status")
        # Clear wedge aging only for terminal (or legacy free-text) pilot lines.
        # Nonterminal progress verbs keep possible-wedge markers even if free text
        # once looked pilot-relevant or was written into a seen marker.
        _clear_wedge=0
        if [ -n "$last" ] && status_is_pilot_relevant "$last"; then
          if status_is_terminal_verb "$last"; then
            _clear_wedge=1
            ...
            case "$(status_line_verb "$last")" in
              working|resolved|pilot-held) _clear_wedge=0 ;;
              *) _clear_wedge=1 ;;
            esac
          fi
        fi
...
# --- log --------------------------------------------------------------------
# Uses LOG set by ap_super_main; harmless no-op-ish if unset (tests source fns
...
trim_log() {
  ...
  [ "$sz" -ge "${AP_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/ap-daemon-log.XXXXXX") || return 0
  tail -n "${AP_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/ap-daemon.test.sh).
# ============================================================================
...
ap_super_main() {
  ...
  # Export AP_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/ap-wake-lib.sh
  AP_STATE_OVERRIDE="$STATE" . "$AP_DAEMON_DIR/ap-wake-lib.sh"

  local WATCH="$AP_DAEMON_DIR/ap-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${AP_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${AP_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${AP_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${AP_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${AP_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}

  [ -x "$WATCH" ] || { echo "error: watcher not found or not executable: $WATCH" >&2; exit 1; }

  # --- single instance (portable lock, no flock dependency) ------------------
  if ! ap_lock_try_acquire "$LOCK"; then
    if [ -n "${AP_LOCK_HELD_PID:-}" ]; then
      echo "error: another ap-supervise-daemon is already running (pid $AP_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another ap-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"
  ap_pid_identity "${BASHPID:-$$}" > "$LOCK/pid-identity" 2>/dev/null || true

  # --- auto-discover the supervisor BACKEND (tmux vs herdr) first -----------
  # Priority: AP_SUPERVISOR_BACKEND override > $TMUX_PANE (tmux) > $HERDR_ENV=1
  ...
  # into AP_SUPERVISOR_BACKEND makes inject_msg/pane_is_busy/pane_input_pending
  ...
  backend_source="AP_SUPERVISOR_BACKEND"
  if [ -z "${AP_SUPERVISOR_BACKEND:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      backend_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      backend_source="HERDR_ENV"
    else
      backend_source="FALLBACK($AP_SUPERVISOR_BACKEND_DEFAULT)"
    fi
  fi
  discovered_backend=$(discover_supervisor_backend) || true
  AP_SUPERVISOR_BACKEND="$discovered_backend"
  local BACKEND="$AP_SUPERVISOR_BACKEND"
  ...
  if ! ap_backend_list_contains "$AP_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $AP_SUPERVISOR_SUPPORTED_BACKENDS); set AP_SUPERVISOR_BACKEND=tmux|herdr and AP_SUPERVISOR_TARGET to run autopilot's own pane under a supported backend" >&2
    log "startup failed: unsupported supervisor backend '$BACKEND' (source=$backend_source)"
    ap_lock_release "$LOCK" 2>/dev/null || true
    rm -f "$PIDFILE" 2>/dev/null || true
    exit 1
  fi

  # --- auto-discover the supervisor target (the pane running autopilot) -----
  # Priority: AP_SUPERVISOR_TARGET override > $TMUX_PANE (tmux; inherited from
  # the pane that launched the daemon, normally autopilot's own) >
  # $HERDR_PANE_ID (herdr, composed into "<session>:<pane-id>") > autopilot:0
  # fallback. Exporting the result into AP_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="AP_SUPERVISOR_TARGET"
  if [ -z "${AP_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      target_source="HERDR_ENV(HERDR_PANE_ID)"
    else
      target_source="FALLBACK(autopilot:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no AP_SUPERVISOR_TARGET, TMUX_PANE, or HERDR_ENV/HERDR_PANE_ID); falling back to '$discovered' — verify this is autopilot's pane" >&2
  fi
  AP_SUPERVISOR_TARGET="$discovered"
  local TARGET="$AP_SUPERVISOR_TARGET"

  # --- validate supervisor target at startup (a missing target is a typo) ---
  # Dispatches through bin/ap-backend.sh instead of a raw `tmux display-message`
  ...
  if ! ap_backend_target_exists "$BACKEND" "$TARGET"; then
    echo "error: supervisor target '$TARGET' does not resolve to a $BACKEND pane; set AP_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; backend=$BACKEND; backend_source=$backend_source; afk=$afk_status; inject_skip='${AP_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${AP_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${AP_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  migrate_watcher_pause_markers "$STATE"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/ap-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and autopilot itself is the consumer of escalations.
    ...
    # The watcher child runs on its own AP_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${AP_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  # execs the daemon, see bin/ap-afk-start.sh). Make it structurally impossible
  # for a sourced context to fire a real desktop notification from the wedge
  # alarm: default the AP_WEDGE_ALARM_EXEC notifier seam to "discard" unless the
  ...
  : "${AP_WEDGE_ALARM_EXEC:=discard}"
  export AP_WEDGE_ALARM_EXEC
fi
