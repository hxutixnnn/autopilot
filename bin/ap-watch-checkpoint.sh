...
SECONDS_ARG=${AP_CODEX_WATCH_CHECKPOINT:-180}

usage() {
  cat <<'EOF'
Usage: ap-watch-checkpoint.sh [--seconds <n>]

Run bin/ap-watch.sh in the foreground for a bounded checkpoint.
...
OUT=$(mktemp "${TMPDIR:-/tmp}/ap-watch-checkpoint.out.XXXXXX") || exit 1
ERR=$(mktemp "${TMPDIR:-/tmp}/ap-watch-checkpoint.err.XXXXXX") || {
  rm -f "$OUT"
  exit 1
}
...
run_with_perl_timeout() {
  perl -e '
    ...
    waitpid $pid, 0;
    exit($? >> 8);
  ' "$SECONDS_ARG" "$SCRIPT_DIR/ap-watch.sh"
}

set +e
if command -v timeout >/dev/null 2>&1; then
  timeout "$SECONDS_ARG" "$SCRIPT_DIR/ap-watch.sh" >"$OUT" 2>"$ERR"
  RC=$?
elif command -v gtimeout >/dev/null 2>&1; then
  gtimeout "$SECONDS_ARG" "$SCRIPT_DIR/ap-watch.sh" >"$OUT" 2>"$ERR"
  RC=$?
else
