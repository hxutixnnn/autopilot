#!/usr/bin/env bash
# Install or remove Autopilot's guarded Kimi flight crew turn-end hook.
#
...
# serializes it: install adds or replaces one marker-delimited Autopilot region,
...
# hook payload, checks for a .ap-kimi-turnend pointer before registry work, and
# touches a task turn-end marker only when the pointer names an Autopilot-created
# token in $HOME/.kimi-code/ap-turn-end.d/.
#
# Usage:
#   ap-kimi-turnend-hook.sh install
#   ap-kimi-turnend-hook.sh remove
set -u
...
if [ -z "${HOME:-}" ]; then
  printf 'ap-kimi-turnend-hook: refused: HOME is unset.\n' >&2
  exit 1
fi
if ! command -v python3 >/dev/null 2>&1; then
  printf 'ap-kimi-turnend-hook: refused: python3 with tomllib is required to validate config.toml.\n' >&2
  exit 1
fi
if [ "$ACTION" = install ] && ! command -v jq >/dev/null 2>&1; then
  printf 'ap-kimi-turnend-hook: refused: jq is required by the installed Kimi turn-end hook.\n' >&2
  exit 1
fi
...
try:
    ...
    print(
        "ap-kimi-turnend-hook: refused: python3 with tomllib is required to validate config.toml.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
HOOK = os.path.join(CONFIG_DIR, "ap-turn-end.sh")
REGISTRY = os.path.join(CONFIG_DIR, "ap-turn-end.d")
BEGIN = b"# BEGIN AUTOPILOT KIMI TURN-END HOOK"
BEGIN_OWNS_NEWLINE = BEGIN + b" (OWNS PRECEDING NEWLINE)"
END = b"# END AUTOPILOT KIMI TURN-END HOOK"
IDENTIFIER = b"AUTOPILOT KIMI TURN-END HOOK"
HOOK_NAME = b"ap-turn-end.sh"
TOKEN_NAME = re.compile(r"ap\.[A-Za-z0-9]{12}\Z")

HOOK_BYTES = b'''#!/usr/bin/env bash
# Autopilot Kimi turn-end hook. Managed by ap-kimi-turnend-hook.sh.
...
pointer="$workspace/.ap-kimi-turnend"
[ -f "$pointer" ] || exit 0
first=
...
auth_dir=${HOME:-}/.kimi-code/ap-turn-end.d
[ -n "${HOME:-}" ] || exit 0
...
def refuse(reason: str) -> None:
    print(f"ap-kimi-turnend-hook: refused: {reason}", file=sys.stderr)
    raise SystemExit(1)
...
def locate_region(data: bytes):
    ...
    if normal_count + owned_count != 1 or end_count != 1 or identifier_count != 2:
        refuse("config.toml has partial, duplicated, or altered Autopilot region markers.")
    ...
    if marker_at != 0 and data[marker_at - 1 : marker_at] != b"\n":
        refuse("the Autopilot begin marker is not at a line boundary.")
    start = marker_at
    if owned_count:
        if marker_at == 0 or data[marker_at - 1 : marker_at] != b"\n":
            refuse("the Autopilot region claims a preceding newline that is absent.")
        start -= 1
    end_at = data.find(END, marker_at + len(marker))
    if end_at < 0:
        refuse("the Autopilot end marker is missing.")
    after = end_at + len(END)
    if after < len(data):
        if data[after : after + 1] != b"\n":
            refuse("the Autopilot end marker is not a complete line.")
        after += 1
...
def block(marker: bytes) -> bytes:
    return b"\n".join(
        (
            ...
            b'event = "Stop"',
            b'matcher = "^$"',
            b'command = "bash \\"$HOME/.kimi-code/ap-turn-end.sh\\" >/dev/null 2>&1 || true"',
            b"timeout = 1",
            END,
            b"",
        )
...
def without_region(data: bytes, region) -> bytes:
    ...
    # Retain a newline so removal cannot join the pilot's preceding line to content appended after installation.
...
def validate_autopilot_files_for_remove() -> None:
    if os.path.lexists(HOOK):
        info = regular_not_symlink(HOOK, "Autopilot hook script")
        with open(HOOK, "rb") as stream:
            if stream.read() != HOOK_BYTES:
                refuse(f"Autopilot hook script has unexpected content at {HOOK}.")
        if stat.S_IMODE(info.st_mode) & 0o077:
            refuse(f"Autopilot hook script has unexpectedly broad permissions at {HOOK}.")
    if os.path.lexists(REGISTRY):
        info = os.lstat(REGISTRY)
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            refuse(f"Autopilot registry is not a regular directory at {REGISTRY}.")
        for name in os.listdir(REGISTRY):
            ...
            if not TOKEN_NAME.fullmatch(name) or stat.S_ISLNK(child.st_mode) or not stat.S_ISREG(child.st_mode):
                refuse(f"Autopilot registry contains an unexpected entry at {path}.")
...
try:
    ...
    if HOOK_NAME in outside:
        refuse("config.toml references ap-turn-end.sh outside the Autopilot-owned region.")
    ...
    if ACTION == "install":
        if os.path.lexists(REGISTRY):
            ...
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
                refuse(f"Autopilot registry is not a regular directory at {REGISTRY}.")
        if os.path.lexists(HOOK):
            regular_not_symlink(HOOK, "Autopilot hook script")
            with open(HOOK, "rb") as stream:
                existing_hook = stream.read()
            if existing_hook != HOOK_BYTES and not existing_hook.startswith(
                b"#!/usr/bin/env bash\n# Autopilot Kimi turn-end hook."
            ):
                refuse(f"Autopilot hook path has unexpected content at {HOOK}.")
        if region is None:
        ...
        validate_autopilot_files_for_remove()
        candidate = outside
        parse_toml(candidate, "config.toml after Autopilot region removal")
        if candidate != original:
            atomic_write(CONFIG, candidate, stat.S_IMODE(config_info.st_mode))
        if os.path.lexists(HOOK):
            os.unlink(HOOK)
