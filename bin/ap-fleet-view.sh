#!/usr/bin/env bash
# ap-fleet-view.sh - human renderer over ap-fleet-snapshot.sh.
#
# This command intentionally does not parse fleet state itself.
# It shells out to ap-fleet-snapshot.sh --json and renders that stable
...
usage() {
  cat <<'EOF'
usage: ap-fleet-view.sh [--json]

Render a human fleet view from ap-fleet-snapshot.sh.
...
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
  --json) "$SCRIPT_DIR/ap-fleet-snapshot.sh" --json; exit $? ;;
  "") ;;
  *) usage >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "ap-fleet-view: jq not found" >&2; exit 1; }

SNAPSHOT=$("$SCRIPT_DIR/ap-fleet-snapshot.sh" --json) || exit $?
...
printf '%s\n' "$SNAPSHOT" | jq -r '
  ...
  def endpoint_of($t):
    if $t.kind == "copilot" then "\(endpoint_exists($t)) / \($t.endpoint.agent_alive)"
  ...
  def action_of($t):
    if $t.kind == "copilot" then "\($t.actions.send) - \($t.actions.watch)"
