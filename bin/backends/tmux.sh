...
#
# Reference backend (AGENTS.md section 8; data/ap-backend-design-d7). P1 moves
# the tmux command sequences that ap-send.sh, ap-peek.sh, ap-watch.sh,
# ap-spawn.sh, and ap-teardown.sh already ran inline into named functions
...
# through bin/ap-backend.sh's ap_backend_source, never directly.
#
...
# provider, not the worktree provider, so ap-spawn.sh still drives that part
...
# already live in bin/ap-tmux-lib.sh, shared with the away-mode daemon
# (bin/ap-supervise-daemon.sh); this adapter sources that file and re-exports
...
. "$AP_BACKEND_LIB_DIR/ap-tmux-lib.sh"

# ap_backend_tmux_resolve_bare_selector: the live-window-listing fallback for a
...
# ap-send.sh's and ap-peek.sh's own (until now duplicated) resolve().
ap_backend_tmux_resolve_bare_selector() {  # <name>
  local name=$1
  tmux list-windows -a -F '#{session_name}:#{window_name}' | grep -m1 ":$name\$" \
    || { echo "error: no window named $name" >&2; return 1; }
}

# ap_backend_tmux_capture: bounded plain-text pane capture. Mirrors
# ap-peek.sh's and ap-watch.sh's `tmux capture-pane -p -t "$T" -S -"$N"`.
ap_backend_tmux_capture() {  # <target> <lines>
  tmux capture-pane -p -t "$1" -S -"$2"
}

# ap_backend_tmux_send_key: one named key. Mirrors ap-send.sh's --key path:
# `tmux display-message -p -t "$T" '#{pane_id}' >/dev/null`, then
# `tmux send-keys -t "$T" "$2"`.
ap_backend_tmux_send_key() {  # <target> <key>
  tmux display-message -p -t "$1" '#{pane_id}' >/dev/null
  tmux send-keys -t "$1" "$2"
}

# ap_backend_tmux_send_text_submit: type <text> into <target> once, then
# submit with Enter, retried (Enter only, never retyped) until the composer
# clears. Re-exports ap_tmux_submit_core (bin/ap-tmux-lib.sh) verbatim; see
# that file for the composer-verification contract and echoed verdicts.
ap_backend_tmux_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle>
  ap_tmux_submit_core "$@"
}

# ap_backend_tmux_container_ensure: reuse the current tmux session when
# autopilot itself runs inside tmux, else ensure a dedicated detached
# "autopilot" session exists. Mirrors ap-spawn.sh's container-ensure block;
# prints the resolved session name.
ap_backend_tmux_container_ensure() {
  if [ -n "${TMUX:-}" ]; then
    tmux display-message -p '#S'
  else
    tmux has-session -t autopilot 2>/dev/null || tmux new-session -d -s autopilot
    printf 'autopilot'
  fi
}

# ap_backend_tmux_create_task: create the task's window in <proj-abs>,
# refusing an existing <window-name> in <session>. Mirrors ap-spawn.sh's
# duplicate-check-then-new-window sequence, including the exact error text
# (session:window, matching how ap-spawn.sh composed its own $T). Prints the
# created window's stable window id on stdout for the caller to target.
#
# Robustness (ap-spawn tmux window handling under a non-default pilot config):
...
#     new window: the pilot's tmux may rename the window away from ap-<id> once
#     treehouse cd's into the worktree, which would break name-based targeting.
# The returned window id lets callers target the window even if its name is ever
# lost, so worktree discovery cannot fall back to the active client's window.
ap_backend_tmux_create_task() {  # <session> <window-name> <proj-abs> -> prints window id
...
# ap_backend_tmux_current_path: the live pane's current working directory, or
# empty on any tmux error. Mirrors ap-spawn.sh's worktree-discovery poll:
# `tmux display-message -p -t "$T" '#{pane_current_path}'`.
ap_backend_tmux_current_path() {  # <target>
  tmux display-message -p -t "$1" '#{pane_current_path}' 2>/dev/null
}

# ap_backend_tmux_send_text_line: send one line of TEXT then Enter, with no
# composer verification - used for the fixed spawn-time commands
# (`treehouse get`, the GOTMPDIR export) that already ran this exact sequence
# inline in ap-spawn.sh. Mirrors `tmux send-keys -t "$T" "<text>" Enter`.
ap_backend_tmux_send_text_line() {  # <target> <text>
  tmux send-keys -t "$1" "$2" Enter
}

# ap_backend_tmux_send_literal: send TEXT as literal bytes with no
# submission - the caller sends Enter separately (ap-spawn.sh's launch-command
# send pauses between the literal send and Enter for the harness to settle).
# Mirrors `tmux send-keys -t "$T" -l "<text>"`.
ap_backend_tmux_send_literal() {  # <target> <text>
  tmux send-keys -t "$1" -l "$2"
}

# ap_backend_tmux_kill: remove one explicitly named task window, best-effort.
...
# ap_backend_tmux_current_command: <target>'s live foreground process name -
...
ap_backend_tmux_current_command() {  # <target>
  tmux display-message -p -t "$1" '#{pane_current_command}' 2>/dev/null
}

# ap_backend_tmux_agent_state: recovery-grade harness-agent state for one
# recorded target. See bin/ap-backend.sh's ap_backend_agent_state for the
...
ap_backend_tmux_agent_state() {  # <target>
  ...
  comm=$(ap_backend_tmux_current_command "$target") || {
    printf 'unreadable'
    return 0
  }
  comm=${comm#-}
...
ap_backend_tmux_agent_alive() {  # <target>
  case "$(ap_backend_tmux_agent_state "$1")" in
    alive) printf 'alive' ;;
