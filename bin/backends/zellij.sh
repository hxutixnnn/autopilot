...
#
# Design: data/ap-backend-design-d7/report.md ("Zellij Backend" section - the
...
# bin/ap-backend.sh's ap_backend_source in normal operation; the unit tests
# source it directly.
#
# Session shape (report "Zellij implementation choices" #1, unchanged by
# empirical verification): ONE zellij session (default name "autopilot",
# overridable via AP_ZELLIJ_SESSION for test isolation - mirrors herdr's
# HERDR_SESSION), ONE tab per task, with caller-facing label "ap-<id>" and a
...
# task in every autopilot home - primary or copilot - shares this ONE
# session's tab bar with no per-home split, and zellij enforces no tab-name
# uniqueness at all, two autopilot homes whose task ids happen to collide
# could send/peek/close each other's tabs. This is the exact gap a
# pilot-directed no-mistakes review gate caught for the cmux backend
# (docs/cmux-backend.md) and this same tag mechanism (bin/backends/cmux.sh's
# ap_backend_cmux_scoped_title, now shared via bin/ap-backend-hometag-lib.sh)
# is ported here for the identical reason. Every NEW tab is created with a
# title tagged with this installation's home label (ap_backend_zellij_scoped_title,
# "ap-<hometag>-<id>"); every list/find/recover/kill path is scoped to this
# home's own tag. A tab created before this change carries the old untagged
# "ap-<id>" title; target_ready, kill, and ad hoc selector fallback still
# match it, but ONLY when that bare title is unambiguous (exactly one live tab
# in the session carries it) - see ap_backend_zellij_tab_matches_label and
# docs/zellij-backend.md "Home-scoped tab titles" for the full migration
# posture. Moving/relocating an autopilot installation changes its tag
...
#      ap_backend_zellij_current_path below and docs/zellij-backend.md
...
#      Mitigated (ap_backend_zellij_create_task): capture the previously
...
#     (ap_backend_zellij_session_exists, a passive list-sessions query, never
...
# ap_backend_required_tools only when zellij is the resolved backend; this
...
# AP_HOME/AP_ROOT now also feed ap_backend_zellij_home_label's tab-title tag
# below.
AP_BACKEND_ZELLIJ_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
AP_ROOT="${AP_ROOT_OVERRIDE:-${AP_ROOT:-$AP_BACKEND_ZELLIJ_ROOT}}"
AP_HOME="${AP_HOME:-${AP_ROOT_OVERRIDE:-$AP_ROOT}}"

# shellcheck source=bin/ap-backend-hometag-lib.sh
. "$AP_BACKEND_ZELLIJ_ROOT/bin/ap-backend-hometag-lib.sh"
...
AP_BACKEND_ZELLIJ_MIN_MAJOR=0
AP_BACKEND_ZELLIJ_MIN_MINOR=44

# ap_backend_zellij_session: the session name this spawn/op uses.
# AP_ZELLIJ_SESSION mirrors herdr's HERDR_SESSION ambient-selection knob: an
# operator (or autopilot's own isolated test harness) sets it explicitly;
# absent means the shared "autopilot" session. Do not use this alone for
# destructive test cleanup; tests/zellij-test-safety.sh documents and guards
# that path (mirrors tests/herdr-test-safety.sh).
ap_backend_zellij_session() {
  printf '%s' "${AP_ZELLIJ_SESSION:-autopilot}"
}

# ap_backend_zellij_home_label: readable home prefix plus a short hash of the
# resolved AP_ROOT path (bin/ap-backend-hometag-lib.sh). Zellij has one
# session-global tab namespace shared by every autopilot home, so the path
...
ap_backend_zellij_home_label() {
  ap_backend_hometag
}

# ap_backend_zellij_scoped_title: the actual tab title a NEW task's tab is
# created with - the caller-facing "ap-<id>" label, home-tagged as
# "ap-<hometag>-<id>" (mirrors bin/backends/cmux.sh's identical
# ap_backend_cmux_scoped_title). Every list/find/recover/kill path below
# scopes its own-home matches through this.
ap_backend_zellij_scoped_title() {  # <ap-task-label>
  local label=$1 rest home
  home=$(ap_backend_zellij_home_label)
  case "$label" in
    ap-*) rest=${label#ap-} ;;
    *) rest=$label ;;
  esac
...
# ap_backend_zellij_tool_check: refuse loudly if zellij or jq is missing.
ap_backend_zellij_tool_check() {
...
# ap_backend_zellij_version_check: refuse loudly on a missing/incompatible
...
ap_backend_zellij_version_check() {
  ap_backend_zellij_tool_check || return 1
  ...
  if [ "$major" -lt "$AP_BACKEND_ZELLIJ_MIN_MAJOR" ] || { [ "$major" -eq "$AP_BACKEND_ZELLIJ_MIN_MAJOR" ] && [ "$minor" -lt "$AP_BACKEND_ZELLIJ_MIN_MINOR" ]; }; then
    echo "error: zellij $ver is older than the verified minimum $AP_BACKEND_ZELLIJ_MIN_MAJOR.$AP_BACKEND_ZELLIJ_MIN_MINOR; update zellij before using backend=zellij" >&2
    return 1
  fi
  return 0
}

# ap_backend_zellij_cli: run `zellij --session <session> action <args...>`,
...
# for defense in depth, mirroring bin/backends/herdr.sh's ap_backend_herdr_cli
# rationale even though no equivalent env-var-unreliable incident has been
# observed for zellij).
ap_backend_zellij_cli() {  # <session> <action-subcommand-and-args...>
  local session=$1
  shift
  ZELLIJ_SESSION_NAME="$session" zellij --session "$session" "$@"
}

# ap_backend_zellij_session_exists: passive, READ-ONLY liveness check - never
...
ap_backend_zellij_session_exists() {  # <session>
  zellij list-sessions --short --no-formatting 2>/dev/null | grep -qxF "$1"
}

# ap_backend_zellij_server_ensure: create the named session in the background,
...
ap_backend_zellij_server_ensure() {  # <session>
  local session=$1 i
  ap_backend_zellij_session_exists "$session" && return 0
...
# ap_backend_zellij_container_ensure: the full spawn-time container-ensure
...
ap_backend_zellij_container_ensure() {
  local session
  ap_backend_zellij_version_check || return 1
  session=$(ap_backend_zellij_session)
  ap_backend_zellij_server_ensure "$session" || return 1
  printf '%s' "$session"
}

# ap_backend_zellij_pane_for_tab: the terminal (non-plugin) pane id for
...
ap_backend_zellij_pane_for_tab() {  # <session> <tab_id>
  local session=$1 tab_id=$2
  ap_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -r --argjson t "$tab_id" '.[]? | select(.tab_id == $t and .is_plugin == false) | .id' 2>/dev/null | head -1
}

# ap_backend_zellij_tab_for_pane: the owning tab id for <pane_id> in
...
ap_backend_zellij_tab_for_pane() {  # <session> <pane_id>
  local session=$1 pane_id=$2
  ap_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -r --argjson p "$pane_id" '.[]? | select(.id == $p and .is_plugin == false) | .tab_id' 2>/dev/null | head -1
}

ap_backend_zellij_pane_exists() {  # <session> <pane_id>
  local session=$1 pane_id=$2
  ap_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -e --argjson p "$pane_id" '[.[]? | select(.id == $p and .is_plugin == false)] | length > 0' >/dev/null 2>&1
}

# ap_backend_zellij_tab_matches_label: does <tab_id> in <session> carry the
# tab name autopilot expects for the caller-facing task label <label>?
# Checks the home-scoped, tagged title first (ap_backend_zellij_scoped_title
# - what every NEW tab is created with), then falls back to the legacy
# untagged bare title (the plain <label>, e.g. "ap-<id>") for a tab created
# before this home-scoping change delivered - but ONLY when that bare name is
...
# same-named tab from a different autopilot home sharing this one zellij
...
ap_backend_zellij_tab_matches_label() {  # <session> <tab_id> <label>
  local session=$1 tab_id=$2 label=$3 scoped tabs count
  scoped=$(ap_backend_zellij_scoped_title "$label")
  tabs=$(ap_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null)
...
# ap_backend_zellij_create_task: create the task's tab (one terminal pane) in
...
# (ap_backend_zellij_scoped_title), never the bare caller-facing label - see
...
# no-op when no client is attached (the common case: an unattended autopilot
# spawn). Best-effort: a failure to restore never fails the spawn.
#
# Echoes "<tab_id> <pane_id>" on success.
ap_backend_zellij_create_task() {  # <session> <label> <cwd>
  local session=$1 label=$2 cwd=$3 title tabs dup prev_active tab_id pane_id
  ap_backend_zellij_session_exists "$session" || { echo "error: zellij session '$session' does not exist; run container_ensure first" >&2; return 1; }
  title=$(ap_backend_zellij_scoped_title "$label")
  tabs=$(ap_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null)
  ...
  tab_id=$(ap_backend_zellij_cli "$session" action new-tab --cwd "$cwd" --name "$title" 2>/dev/null | tr -d '[:space:]')
  case "$tab_id" in
    ''|*[!0-9]*)
  ...
  pane_id=$(ap_backend_zellij_pane_for_tab "$session" "$tab_id")
  if [ -z "$pane_id" ]; then
  ...
  if [ -n "$prev_active" ] && [ "$prev_active" != "$tab_id" ]; then
    ap_backend_zellij_cli "$session" action go-to-tab-by-id "$prev_active" >/dev/null 2>&1 || true
  fi
  printf '%s %s' "$tab_id" "$pane_id"
}

# ap_backend_zellij_parse_target: split "<session>:<pane_id>" on the FIRST
...
# AP_BACKEND_ZELLIJ_SESSION and AP_BACKEND_ZELLIJ_PANE for the caller.
ap_backend_zellij_parse_target() {  # <target>
  local target=$1
  AP_BACKEND_ZELLIJ_SESSION=${target%%:*}
  AP_BACKEND_ZELLIJ_PANE=${target#*:}
  [ -n "$AP_BACKEND_ZELLIJ_SESSION" ] && [ -n "$AP_BACKEND_ZELLIJ_PANE" ] && [ "$AP_BACKEND_ZELLIJ_PANE" != "$target" ]
}

# ap_backend_zellij_target_ready: parse the target and verify its session and
# pane are alive. When the caller knows the owning autopilot task label, verify
# the pane belongs to that named tab before trusting the numeric pane id.
ap_backend_zellij_target_ready() {  # <target> [expected-label]
  local expected_label=${2:-} tab_id
  ap_backend_zellij_parse_target "$1" || return 1
  ap_backend_zellij_session_exists "$AP_BACKEND_ZELLIJ_SESSION" || return 1
  if [ -n "$expected_label" ]; then
    tab_id=$(ap_backend_zellij_tab_for_pane "$AP_BACKEND_ZELLIJ_SESSION" "$AP_BACKEND_ZELLIJ_PANE" 2>/dev/null)
    [ -n "$tab_id" ] || return 1
    ap_backend_zellij_tab_matches_label "$AP_BACKEND_ZELLIJ_SESSION" "$tab_id" "$expected_label"
    return $?
  fi
  ap_backend_zellij_pane_exists "$AP_BACKEND_ZELLIJ_SESSION" "$AP_BACKEND_ZELLIJ_PANE"
}

# ap_backend_zellij_current_path: the live pane's cwd, or empty on any error.
...
# and read only that marker line. Scoped to ap-spawn.sh's own worktree-discovery
...
# answer.
ap_backend_zellij_current_path() {  # <target> [expected-label]
  local target=$1 expected_label=${2:-} out line marker_begin="__AP_ZELLIJ_CWD_BEGIN__" marker_end="__AP_ZELLIJ_CWD_END__" in_block=0 chunk="" last=""
  ap_backend_zellij_target_ready "$target" "$expected_label" || return 0
  ap_backend_zellij_send_text_line "$target" "printf '%s\n' '$marker_begin'; pwd; printf '%s\n' '$marker_end'" "$expected_label" || return 0
  sleep 0.3
  out=$(ap_backend_zellij_capture "$target" 200 "$expected_label") || return 0
...
# ap_backend_zellij_send_literal: send TEXT as literal, UNSUBMITTED input via
...
ap_backend_zellij_send_literal() {  # <target> <text> [expected-label]
  ap_backend_zellij_target_ready "$1" "${3:-}" || return 1
  ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action paste --pane-id "$AP_BACKEND_ZELLIJ_PANE" -- "$2" >/dev/null 2>&1
}

# ap_backend_zellij_normalize_key: map autopilot's key vocabulary (Enter,
...
# words all fail.
ap_backend_zellij_normalize_key() {  # <key>
  case "$1" in
...
# ap_backend_zellij_send_key: one named special key, targeted at the pane by
# its EXPLICIT --pane-id (never the ambient "focused pane" default - verified
# unreliable, see file header). Mirrors ap-send.sh's --key path.
ap_backend_zellij_send_key() {  # <target> <key> [expected-label]
  ap_backend_zellij_target_ready "$1" "${3:-}" || return 1
  local key
  key=$(ap_backend_zellij_normalize_key "$2")
  ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action send-keys --pane-id "$AP_BACKEND_ZELLIJ_PANE" "$key" >/dev/null 2>&1
}

# ap_backend_zellij_send_text_line: send one line of TEXT then submit,
...
ap_backend_zellij_send_text_line() {  # <target> <text> [expected-label]
  ap_backend_zellij_send_literal "$1" "$2" "${3:-}" || return 1
  ap_backend_zellij_send_key "$1" Enter "${3:-}"
}

# ap_backend_zellij_capture: bounded plain-text pane capture. Mirrors
# ap-peek.sh's/ap-watch.sh's `tmux capture-pane -p -t T -S -N`. `dump-screen`
...
ap_backend_zellij_capture() {  # <target> <lines> [expected-label]
  ap_backend_zellij_target_ready "$1" "${3:-}" || return 1
  local lines=${2:-40} out
  case "$lines" in ''|*[!0-9]*) lines=40 ;; esac
  if [ "$lines" -le 40 ]; then
    out=$(ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action dump-screen --pane-id "$AP_BACKEND_ZELLIJ_PANE" 2>/dev/null) || return 1
  else
    out=$(ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action dump-screen --pane-id "$AP_BACKEND_ZELLIJ_PANE" --full 2>/dev/null) || return 1
  fi
  printf '%s' "$out" | tail -n "$lines"
}

# ap_backend_zellij_send_text_submit: type <text> into <target> once (raw,
...
ap_backend_zellij_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 expected_label=${6:-} typed after i=0
  ap_backend_zellij_send_literal "$target" "$text" "$expected_label" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  typed=$(ap_backend_zellij_capture "$target" 6 "$expected_label") || { printf 'unknown'; return 0; }
  while :; do
    ap_backend_zellij_send_key "$target" Enter "$expected_label" || true
    sleep "$sleep_s"
    after=$(ap_backend_zellij_capture "$target" 6 "$expected_label") || { printf 'unknown'; return 0; }
    if [ "$after" != "$typed" ]; then
      printf 'empty'
      return 0
    fi
...
# ap_backend_zellij_kill: remove the task's tab, best-effort (mirrors
...
# pane id when possible via ap_backend_zellij_tab_for_pane; teardown also
...
ap_backend_zellij_kill() {  # <target> [tab_id] [expected_label]
  ap_backend_zellij_parse_target "$1" || return 0
  ap_backend_zellij_session_exists "$AP_BACKEND_ZELLIJ_SESSION" || return 0
  local tab_id fallback_tab_id=${2:-} expected_label=${3:-}
  tab_id=$(ap_backend_zellij_tab_for_pane "$AP_BACKEND_ZELLIJ_SESSION" "$AP_BACKEND_ZELLIJ_PANE" 2>/dev/null)
  if [ -n "$tab_id" ] && [ -n "$expected_label" ] && ! ap_backend_zellij_tab_matches_label "$AP_BACKEND_ZELLIJ_SESSION" "$tab_id" "$expected_label"; then
    tab_id=
  fi
  case "$fallback_tab_id" in
    ...
    *)
      if [ -z "$tab_id" ]; then
        if [ -z "$expected_label" ] || ap_backend_zellij_tab_matches_label "$AP_BACKEND_ZELLIJ_SESSION" "$fallback_tab_id" "$expected_label"; then
  ...
  if [ -n "$tab_id" ]; then
    ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action close-tab-by-id "$tab_id" >/dev/null 2>&1 || true
  elif [ -z "$expected_label" ]; then
    ap_backend_zellij_cli "$AP_BACKEND_ZELLIJ_SESSION" action close-pane --pane-id "$AP_BACKEND_ZELLIJ_PANE" >/dev/null 2>&1 || true
  fi
}

# ap_backend_zellij_list_live: recovery/orphan discovery. Lists every tab in
# <session> whose title carries THIS autopilot home's own tag
# (ap-<hometag>-, ap_backend_zellij_home_label) - never any other home's
# tagged tabs, and never a bare untagged "ap-<id>" tab either (this sweep
# deliberately does NOT attempt the legacy-bare-title fallback
# ap_backend_zellij_tab_matches_label uses for a single already-known tab:
...
# fallback. One "<session>:<pane_id>\t<plain ap-<id> label>" line per live,
...
ap_backend_zellij_list_live() {  # <session>
  local session=$1 home prefix tabs tab_id name pane_id plain
  ap_backend_zellij_session_exists "$session" || return 0
  home=$(ap_backend_zellij_home_label)
  prefix="ap-$home-"
  tabs=$(ap_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null) || return 0
  while IFS=$'\t' read -r tab_id name; do
    [ -n "$tab_id" ] || continue
    plain=${name#"$prefix"}
    [ -n "$plain" ] || continue
    pane_id=$(ap_backend_zellij_pane_for_tab "$session" "$tab_id") || continue
    [ -n "$pane_id" ] || continue
    printf '%s:%s\tap-%s\n' "$session" "$pane_id" "$plain"
  done < <(printf '%s' "$tabs" | jq -r --arg prefix "$prefix" '.[]? | select(.name | startswith($prefix)) | "\(.tab_id)\t\(.name)"' 2>/dev/null)
}

# ap_backend_zellij_resolve_bare_selector: the live-tab-listing fallback for
...
# (ap_backend_zellij_scoped_title), then falls back to an exact bare <name>
# match ONLY when unambiguous - exactly one tab across every active session
# carries it - mirroring ap_backend_zellij_tab_matches_label's migration
# posture. Rare path in practice (zellij tasks normally carry meta);
# best-effort. Not wired into ap_backend_resolve_selector's dispatcher
# (bin/ap-backend.sh), mirroring herdr: that bare-selector fallback stays
# tmux-only by design, and zellij/herdr tasks are targeted via task-selector
# meta or an explicit recorded target.
ap_backend_zellij_resolve_bare_selector() {  # <name>
  local name=$1 scoped sessions session tabs tab_id count=0 pane_id bare_session='' bare_tab_id=''
  scoped=$(ap_backend_zellij_scoped_title "$name")
...
EOF
  if [ "$count" -eq 1 ]; then
    pane_id=$(ap_backend_zellij_pane_for_tab "$bare_session" "$bare_tab_id") || pane_id=
    if [ -n "$pane_id" ]; then
