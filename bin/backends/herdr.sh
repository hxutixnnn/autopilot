...
#
# Design: data/ap-backend-design-d7/herdr-addendum.md ("Interface mapping",
# decisions D1-D6) and the empirical verification recorded in
# data/ap-backend-design-d7/herdr-verification-p2.md (real herdr v0.7.1,
# protocol 14, macOS aarch64), refined by docs/herdr-backend.md's
# "workspace-per-home" pass (AGENTS.md task herdr-copilot-spaces-k4). Herdr is a
# session provider ONLY (D3): the worktree provider stays treehouse, exactly
# like tmux. Sourced only through bin/ap-backend.sh's ap_backend_source in
# normal operation; the unit tests source it directly, so the AP_HOME fallback
# below keeps that path sane without ap-backend.sh's preamble.
#
...
# AUTOPILOT HOME (the primary, and each copilot, gets its own), ONE herdr TAB
...
# remainder is the whole pane id - ap_backend_herdr_parse_target splits on the
# first colon only). This is the value stored in a herdr task's meta window=
# field and is what ap_backend_resolve_selector already returns unchanged for
# exact task-id, legacy ap-<id>, and explicit backend-target forms (that
...
# verification doc) uses LABEL matching (ap-<id> tab labels), never trusts a
# stored pane id blindly: ap_backend_herdr_list_live. The presentation journal
# is deliberately excluded from that path.
#
# Requires: herdr (CLI + socket), jq (JSON parsing). Bootstrap detects these
# through ap_backend_required_tools only when herdr is the resolved backend;
# this adapter also gates them again before spawning.

# AP_HOME fallback: every real caller (ap-spawn.sh, ap-peek.sh, ap-send.sh,
# ap-teardown.sh, ap-watch.sh, ap-flight-crew-state.sh) already sets AP_HOME as a
# global before sourcing ap-backend.sh (which sources this file), so this
...
# default (the autopilot repo root - never a copilot home, so
# ap_backend_herdr_workspace_label falls through to "autopilot" exactly like
# pre-P3 behavior when a test does not care about home-specific labeling).
AP_BACKEND_HERDR_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
AP_ROOT="${AP_ROOT_OVERRIDE:-${AP_ROOT:-$AP_BACKEND_HERDR_ROOT}}"
...
. "$AP_BACKEND_HERDR_ROOT/bin/ap-composer-lib.sh"

# Shared, backend-neutral normalized-transition shape and the single-owner
# status->action policy table (bin/ap-transition-lib.sh). This adapter's event
# subscriber (ap_backend_herdr_wait_transition) normalizes every
# pane.agent_status_changed edge through ap_transition_record and routes it
# through ap_transition_policy - it never re-encodes the mapping.
# shellcheck source=bin/ap-transition-lib.sh
. "$AP_BACKEND_HERDR_ROOT/bin/ap-transition-lib.sh"

AP_BACKEND_HERDR_MIN_PROTOCOL=14
...
# (ap_backend_herdr_events_capable). Distinct from AP_BACKEND_HERDR_MIN_PROTOCOL
# (14): the adapter's spawn/capture/send primitives work on 14, only the push
# subscriber needs 16.
AP_BACKEND_HERDR_MIN_EVENTS_PROTOCOL=16
...
AP_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL=16
...
# blocked pane. Mirrors bin/ap-watch.sh's .stale-<key> naming.
AP_BACKEND_HERDR_ESCALATED_PREFIX=".herdr-escalated-"
# .ap-copilot-home is written by bin/ap-home-seed.sh (AGENTS.md section 6)
# at a seeded copilot home's root, containing exactly that copilot's id.
# The primary autopilot home never carries this marker.
AP_BACKEND_HERDR_COPILOT_MARKER=".ap-copilot-home"
...
AP_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX=".herdr-presentation"

# ap_backend_herdr_workspace_label: the per-autopilot-HOME herdr workspace
# label (docs/herdr-backend.md "Default task container shape"). The PRIMARY home (no
# copilot marker) resolves to the constant "autopilot", byte-identical to
# every pre-existing task's recorded label - no forced migration. A COPILOT
# home resolves to "copilot-<copilot-id>", so its tasks land in their own
# workspace, obviously distinguishable from the primary's (and from every
# other copilot's) in herdr's spaces sidebar. Read fresh from AP_HOME on
# every call rather than cached at source time: AP_HOME is the home's own
...
# that home. ap-spawn.sh briefly shadows AP_HOME to a copilot's own home
# when the PRIMARY spawns that copilot (its own process's AP_HOME still
# names the primary at that point) - see ap-spawn.sh's herdr case arm.
ap_backend_herdr_workspace_label() {
  local marker="$AP_HOME/$AP_BACKEND_HERDR_COPILOT_MARKER" id
  if [ -f "$marker" ]; then
...
# ap_backend_herdr_cli: run `herdr <args...>` scoped to <session>, setting
...
# ap_backend_herdr_version_check, which is intentionally session-independent
# (reads only .client.* fields).
ap_backend_herdr_cli() {  # <session> <herdr-subcommand-and-args...>
  local session=$1
  shift
  HERDR_SESSION="$session" herdr "$@" --session "$session"
}

# ap_backend_herdr_tool_check: refuse loudly if herdr or jq is missing.
ap_backend_herdr_tool_check() {
...
# ap_backend_herdr_version_check: refuse loudly on a missing/incompatible
...
ap_backend_herdr_version_check() {
  ap_backend_herdr_tool_check || return 1
  ...
  if [ "$protocol" -lt "$AP_BACKEND_HERDR_MIN_PROTOCOL" ]; then
    echo "error: herdr protocol $protocol (version ${version:-unknown}) is older than the verified minimum $AP_BACKEND_HERDR_MIN_PROTOCOL; update herdr (herdr update) before using backend=herdr" >&2
    return 1
  fi
  return 0
}

# ap_backend_herdr_session: resolve which named herdr session this normal
# spawn/op uses. HERDR_SESSION mirrors tmux's $TMUX ambient-selection for
# adapter workspace/tab/pane operations: an operator (or autopilot's own
...
ap_backend_herdr_session() {
  printf '%s' "${HERDR_SESSION:-default}"
}

# ap_backend_herdr_projection_id: generate a compact 128-bit base64url token.
# The token is a non-adversarial visual correlator, never destructive
# authority.
ap_backend_herdr_projection_id() {
  local token
...
ap_backend_herdr_projection_journal_path() {  # <state-dir> <task-id>
  printf '%s/%s%s' "$1" "$2" "$AP_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX"
}

# ap_backend_herdr_projection_journal_create: atomically publish the
...
ap_backend_herdr_projection_journal_create() {  # <state-dir> <task-id>
...
ap_backend_herdr_projection_journal_field() {  # <journal> <key>
...
# ap_backend_herdr_projection_journal_snapshot: validate a version 1 attempt
# journal or a version 2 exact projection binding without sourcing shell code.
# Version 2 sets AP_BACKEND_HERDR_JOURNAL_* globals for same-process callers.
ap_backend_herdr_projection_journal_snapshot() {  # <journal> <task-id>
  local journal=$1 id=$2 lines expected_label expected_task_label exact
  AP_BACKEND_HERDR_JOURNAL_VERSION=""
  AP_BACKEND_HERDR_JOURNAL_TASK_ID=""
  AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID=""
  AP_BACKEND_HERDR_JOURNAL_HOME=""
  AP_BACKEND_HERDR_JOURNAL_SESSION=""
  ...
  AP_BACKEND_HERDR_JOURNAL_PARENT_LABEL=""
  AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL=""
  AP_BACKEND_HERDR_JOURNAL_TASK_LABEL=""
  [ -f "$journal" ] && [ ! -L "$journal" ] || return 1
  lines=$(wc -l < "$journal" 2>/dev/null | tr -d '[:space:]')
  AP_BACKEND_HERDR_JOURNAL_VERSION=$(ap_backend_herdr_projection_journal_field "$journal" version) || return 1
  AP_BACKEND_HERDR_JOURNAL_TASK_ID=$(ap_backend_herdr_projection_journal_field "$journal" task_id) || return 1
  AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID=$(ap_backend_herdr_projection_journal_field "$journal" projection_id) || return 1
  [ "$AP_BACKEND_HERDR_JOURNAL_TASK_ID" = "$id" ] || return 1
  [ "${#AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID}" -eq 22 ] || return 1
  case "$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID" in
    *[!A-Za-z0-9_-]*) return 1 ;;
  esac
  case "$AP_BACKEND_HERDR_JOURNAL_VERSION:$lines" in
    1:3) return 0 ;;
    2:12) ;;
    *) return 1 ;;
  esac
  AP_BACKEND_HERDR_JOURNAL_HOME=$(ap_backend_herdr_projection_journal_field "$journal" home) || return 1
  AP_BACKEND_HERDR_JOURNAL_SESSION=$(ap_backend_herdr_projection_journal_field "$journal" session) || return 1
  AP_BACKEND_HERDR_JOURNAL_WORKSPACE_ID=$(ap_backend_herdr_projection_journal_field "$journal" workspace_id) || return 1
  AP_BACKEND_HERDR_JOURNAL_TAB_ID=$(ap_backend_herdr_projection_journal_field "$journal" tab_id) || return 1
  AP_BACKEND_HERDR_JOURNAL_PANE_ID=$(ap_backend_herdr_projection_journal_field "$journal" pane_id) || return 1
  AP_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID=$(ap_backend_herdr_projection_journal_field "$journal" parent_workspace_id) || return 1
  AP_BACKEND_HERDR_JOURNAL_PARENT_LABEL=$(ap_backend_herdr_projection_journal_field "$journal" parent_label) || return 1
  AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL=$(ap_backend_herdr_projection_journal_field "$journal" workspace_label) || return 1
  AP_BACKEND_HERDR_JOURNAL_TASK_LABEL=$(ap_backend_herdr_projection_journal_field "$journal" task_label) || return 1
  case "$AP_BACKEND_HERDR_JOURNAL_HOME" in
    /*) ;;
    *) return 1 ;;
  esac
  for exact in \
    ...
    "$AP_BACKEND_HERDR_JOURNAL_TAB_ID" \
    "$AP_BACKEND_HERDR_JOURNAL_PANE_ID" \
    "$AP_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID"; do
    case "$exact" in
      ''|*[[:space:]]*) return 1 ;;
    esac
  done
  [ -n "$AP_BACKEND_HERDR_JOURNAL_PARENT_LABEL" ] \
    && [ -n "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" ] \
    && [ -n "$AP_BACKEND_HERDR_JOURNAL_TASK_LABEL" ] || return 1
  expected_label=$(ap_backend_herdr_projection_workspace_label "$id" "$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID")
  expected_task_label="ap-$id"
  [ "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" = "$expected_label" ] \
    && [ "$AP_BACKEND_HERDR_JOURNAL_TASK_LABEL" = "$expected_task_label" ]
}

# ap_backend_herdr_projection_journal_token: validate and read either journal
# version's non-authoritative visual correlator.
ap_backend_herdr_projection_journal_token() {  # <journal> <task-id>
  ap_backend_herdr_projection_journal_snapshot "$1" "$2" || return 1
  printf '%s' "$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID"
}

ap_backend_herdr_projection_home_identity() {  # <home>
  local home=$1
  [ -d "$home" ] || return 1
  (cd "$home" 2>/dev/null && pwd -P)
}

ap_backend_herdr_projection_journal_write_v2() {  # <journal> <task-id> <token> <home> <session> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
...
# ap_backend_herdr_projection_journal_bind: upgrade one exact version 1
...
ap_backend_herdr_projection_journal_bind() {  # <journal> <task-id> <home> <session> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
  ...
  ap_backend_herdr_projection_journal_snapshot "$journal" "$id" || return 1
  [ "$AP_BACKEND_HERDR_JOURNAL_VERSION" = 1 ] || return 1
  token=$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID
  ap_backend_herdr_projection_journal_write_v2 \
    "$journal" "$id" "$token" "$home" "$session" "$workspace" "$tab" "$pane" \
    "$parent_workspace" "$parent_label" "$workspace_label" "$task_label"
}

# ap_backend_herdr_projection_journal_replace_endpoint: atomically advance one
# exact version 2 binding after its old husk was replaced successfully.
ap_backend_herdr_projection_journal_replace_endpoint() {  # <journal> <task-id> <old-tab> <old-pane> <new-tab> <new-pane>
  local journal=$1 id=$2 old_tab=$3 old_pane=$4 new_tab=$5 new_pane=$6
  ap_backend_herdr_projection_journal_snapshot "$journal" "$id" || return 1
  [ "$AP_BACKEND_HERDR_JOURNAL_VERSION" = 2 ] \
    && [ "$AP_BACKEND_HERDR_JOURNAL_TAB_ID" = "$old_tab" ] \
    && [ "$AP_BACKEND_HERDR_JOURNAL_PANE_ID" = "$old_pane" ] || return 1
  ap_backend_herdr_projection_journal_write_v2 \
    "$journal" "$id" "$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID" \
    "$AP_BACKEND_HERDR_JOURNAL_HOME" "$AP_BACKEND_HERDR_JOURNAL_SESSION" \
    "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" "$new_tab" "$new_pane" \
    "$AP_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" "$AP_BACKEND_HERDR_JOURNAL_PARENT_LABEL" \
    "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" "$AP_BACKEND_HERDR_JOURNAL_TASK_LABEL"
}

# ap_backend_herdr_projection_concise_task_label: strip redundant owner
# prefixes from a task id used only in the presentation workspace label.
# Removes autopilot/, copilot-<id>/, and a presentation-level ap- owner
# prefix when present. The ordinary task tab remains ap-<id> and is not
# built by this helper.
ap_backend_herdr_projection_concise_task_label() {  # <task-id>
  local task=$1
  case "$task" in
    autopilot/*) task=${task#autopilot/} ;;
    copilot-*/*) task=${task#*/} ;;
  esac
  case "$task" in
    ap-*) task=${task#ap-} ;;
...
# ap_backend_herdr_projection_workspace_label: presentation-only child label.
...
ap_backend_herdr_projection_workspace_label() {  # <task-id> <projection-id>
  printf '└ %s · p:%s' "$(ap_backend_herdr_projection_concise_task_label "$1")" "$2"
}

# ap_backend_herdr_presentation_session_lock_path: one machine-private lock
# path per live named Herdr session/socket, shared across every Autopilot home
# that uses that session.
# The path is never under any one home's state/ and copilots never write the
# primary home. Returns non-zero when the named session's socket cannot be
# resolved unambiguously.
ap_backend_herdr_presentation_lock_namespace() {
  printf '%s' '/tmp/autopilot-herdr-presentation'
}

ap_backend_herdr_presentation_lock_namespace_mode() {
...
ap_backend_herdr_presentation_lock_namespace_uid() {
  if [ "$(uname -s 2>/dev/null)" = Darwin ]; then
...
ap_backend_herdr_presentation_lock_namespace_valid() {
  local dir=$1 expected_uid owner mode
  [ -d "$dir" ] && [ ! -L "$dir" ] || return 1
  expected_uid=$(id -u 2>/dev/null) || return 1
  owner=$(ap_backend_herdr_presentation_lock_namespace_uid "$dir") || return 1
  mode=$(ap_backend_herdr_presentation_lock_namespace_mode "$dir") || return 1
...
# ap_backend_herdr_canonical_socket_path: normalize one absolute Unix-socket
...
# proof both use it).
ap_backend_herdr_canonical_socket_path() {  # <socket-path>
...
ap_backend_herdr_presentation_session_socket_path() {  # <session>
  local session=$1 sessions socket
  [ -n "$session" ] || return 1
  sessions=$(ap_backend_herdr_cli "$session" session list --json 2>/dev/null) || return 1
  ...
  ' 2>/dev/null) || return 1
  ap_backend_herdr_canonical_socket_path "$socket"
}

ap_backend_herdr_presentation_session_lock_path() {  # <session>
  local session=$1 socket key dir hash
  [ -n "$session" ] || return 1
  socket=$(ap_backend_herdr_presentation_session_socket_path "$session") || return 1
  ...
  [ -n "$hash" ] || return 1
  key=${hash:0:32}
  dir=$(ap_backend_herdr_presentation_lock_namespace) || return 1
  [ -n "$dir" ] || return 1
  if [ ! -e "$dir" ] && [ ! -L "$dir" ]; then
    if ! mkdir -m 700 "$dir" 2>/dev/null; then
      ap_backend_herdr_presentation_lock_namespace_valid "$dir" || return 1
    fi
  fi
  ap_backend_herdr_presentation_lock_namespace_valid "$dir" || return 1
...
# ap_backend_herdr_projection_focus_snapshot: print the exact active
...
ap_backend_herdr_projection_focus_snapshot() {  # <session>
  local session=$1 list snapshot workspace tab tabs
  list=$(ap_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 1
...
# ap_backend_herdr_projection_focus_restore: verify that one presentation
...
ap_backend_herdr_projection_focus_restore() {  # <session> <snapshot> <operation>
  ...
  after=$(ap_backend_herdr_projection_focus_snapshot "$session") || after=
  [ "$after" != "$before" ] || return 0
  workspace=${before%%$'\t'*}
  tab=${before#*$'\t'}
  info=$(ap_backend_herdr_cli "$session" tab get "$tab" 2>/dev/null) || {
  ...
  ap_backend_herdr_cli "$session" tab focus "$tab" >/dev/null 2>&1 || {
    echo "warning: herdr presentation $operation changed focus and exact-tab restoration failed" >&2
    return 1
  }
  restored=$(ap_backend_herdr_projection_focus_snapshot "$session") || restored=
...
# ap_backend_herdr_projection_close_pane_focus_preserving: close one exact
# response-derived projection pane without leaving the pilot focused
# anywhere else.
# If the target belongs to the active tab, exact tab preservation is
# impossible, so cleanup refuses instead of changing focus.
ap_backend_herdr_projection_close_pane_focus_preserving() {  # <session> <pane-id> [required-agent-state]
  local session=$1 pane_id=$2 required_agent_state=${3:-}
  local before active_tab info target_pane target_tab close_status state
  AP_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE=""
  [ -n "$pane_id" ] || return 0
  before=$(ap_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  active_tab=${before#*$'\t'}
  info=$(ap_backend_herdr_cli "$session" pane get "$pane_id" 2>/dev/null) || {
  ...
  if [ "$target_tab" = "$active_tab" ]; then
    echo "warning: herdr presentation cleanup target is the pilot's active tab; refusing a close that cannot preserve focus" >&2
    return 1
  fi
  if [ -n "$required_agent_state" ]; then
    state=$(ap_backend_herdr_pane_agent_state "$session" "$pane_id")
    AP_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE=$state
    [ "$state" = "$required_agent_state" ] || return 1
  fi
  if ap_backend_herdr_cli "$session" pane close "$pane_id" >/dev/null 2>&1; then
    close_status=0
  else
    close_status=$?
  fi
  ap_backend_herdr_projection_focus_restore "$session" "$before" "pane close" || return 2
  [ "$close_status" -eq 0 ]
}

# ap_backend_herdr_projection_order_best_effort: place the exact workspace id
# returned by THIS projected create immediately after its owning parent's
# contiguous child block and before the next parent.
#
# <parent-label> is the owning AP_HOME label (autopilot or copilot-<id>).
...
# adjacent old-format autopilot/... or copilot-<id>/... projections may extend
...
ap_backend_herdr_projection_order_best_effort() {  # <session> <created-workspace-id> <parent-label> [<parent-workspace-id>]
  ...
  analysis=$(printf '%s' "$list" | jq -c --arg created "$created" --arg parent "$parent" --arg parent_ws "$parent_ws" '
    ...
    def is_top_level_parent:
      (.label | type) == "string"
      and ((.label == "autopilot") or (.label | test("^copilot-[^/]+$")));
    def is_new_child:
      (.label | type) == "string"
      and (.label | test("^└ .+ · p:[A-Za-z0-9_-]{22}$"));
    def is_legacy_child:
      (.label | type) == "string"
      and (.label | test("^(autopilot|copilot-[^/]+)/.+ · p:[A-Za-z0-9_-]{22}$"));
  ...
  protocol=$(ap_backend_herdr_cli "$session" status --json 2>/dev/null | jq -r '.client.protocol // empty' 2>/dev/null)
  case "$protocol" in
    ''|*[!0-9]*)
  ...
  if [ "$protocol" -lt "$AP_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL" ]; then
    echo "warning: herdr presentation ordering needs protocol $AP_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL or newer; leaving worker in Herdr's current order" >&2
    return 0
  fi
  schema=$(ap_backend_herdr_cli "$session" api schema --json 2>/dev/null) || {
  ...
  socket=$(ap_backend_herdr_presentation_session_socket_path "$session") || {
  ...
  mover=${AP_BACKEND_HERDR_WORKSPACE_MOVER:-$AP_BACKEND_HERDR_ROOT/bin/backends/herdr-workspace-move.py}
  focus_before=$(ap_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace move" || true
...
# ap_backend_herdr_server_ensure: start the herdr server for <session>
...
ap_backend_herdr_server_ensure() {  # <session>
  local session=$1 running out i
  running=$(ap_backend_herdr_cli "$session" status --json 2>/dev/null | jq -r '.server.running // false' 2>/dev/null)
  [ "$running" = "true" ] && return 0
  ( ap_backend_herdr_cli "$session" server >/dev/null 2>&1 & ) || return 1
...
# ap_backend_herdr_workspace_find_all: EVERY workspace id inside <session>
# whose label equals this HOME's own label (ap_backend_herdr_workspace_label),
...
# can legitimately return MORE THAN ONE id: a pilot-owned workspace can
...
# duplicate means for them - ap_backend_herdr_workspace_ensure refuses to guess
# which one is the caller's, while the read-only recovery path below keeps its
# historical first-match behavior.
ap_backend_herdr_workspace_find_all() {  # <session>
  local session=$1 label list
  label=$(ap_backend_herdr_workspace_label)
  list=$(ap_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 0
  ...
  # ALWAYS return empty and every spawn mint a fresh "autopilot" workspace
  # (the workspace leak).
  printf '%s' "$list" | jq -r --arg want "$label" \
    '.result.workspaces[]? | select(.label == $want) | .workspace_id' 2>/dev/null
}

# ap_backend_herdr_workspace_find: this HOME's own workspace id inside
...
# exactly the defect ap_backend_herdr_workspace_ensure now refuses.
ap_backend_herdr_workspace_find() {  # <session>
  ap_backend_herdr_workspace_find_all "$1" | head -1
}

# ap_backend_herdr_launcher_identity: the EXACT herdr workspace that the
...
# manages a pane for (docs/verification/runtime-backends.md), and an autopilot
# or copilot agent's own tool calls inherit them. Older injection shapes are
...
# uniqueness on them, so a label search cannot tell one `autopilot` workspace
# from another, and herdr's globally focused workspace is whatever the pilot
...
#   AP_BACKEND_HERDR_LAUNCHER_PANE_ID
#   AP_BACKEND_HERDR_LAUNCHER_TAB_ID
#   AP_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID
#
# Returns:
...
#       SELECTION marker (bin/ap-backend.sh's ap_backend_detect), never a
...
ap_backend_herdr_launcher_identity() {  # <session>
  ...
  claimed_session=$(ap_backend_herdr_session)
  ...
  claimed_socket=$(ap_backend_herdr_canonical_socket_path "$claimed_socket") || {
  ...
  session_socket=$(ap_backend_herdr_presentation_session_socket_path "$session") || {
  ...
  pane_out=$(ap_backend_herdr_cli "$session" pane get "$pane" 2>/dev/null) || {
  ...
  tab_out=$(ap_backend_herdr_cli "$session" tab get "$tab" 2>/dev/null) || {
  ...
  AP_BACKEND_HERDR_LAUNCHER_PANE_ID=$pane
  # shellcheck disable=SC2034  # callers consume the verified binding's parts
  AP_BACKEND_HERDR_LAUNCHER_TAB_ID=$tab
  AP_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID=$workspace
  return 0
}

# ap_backend_herdr_workspace_prune_seeded_default_tab: close EXACTLY
# <seeded_tab_id>, the auto-created default tab id that THIS SAME
# ap_backend_herdr_workspace_ensure call captured straight from its own
...
# here never fails the caller, mirroring the ap_backend_herdr_kill `|| true`
# contract.
#
# Live-fire incident fix (2026-07-02): the prior implementation
# (ap_backend_herdr_workspace_prune_default_tabs, removed) re-derived
# "prunable" at create_task time from a pure label heuristic - exactly one
# tab, labeled "1" - run against whatever workspace ap_backend_herdr_workspace_find
...
# its pane cwd's basename, so a pilot launching herdr directly inside a
# directory named "autopilot" produces a workspace that looks byte-identical,
# by label alone, to autopilot's own auto-created container - one tab, label
# "1". workspace_find adopted that pre-existing (pilot-owned, LIVE) workspace
# by the label match, the heuristic matched too, and the very next spawn
# closed the pilot's own live pane 27ms after creating its task tab. The
# fix is structural, not another heuristic: only a workspace THIS SAME
# ap_backend_herdr_workspace_ensure call just created carries a non-empty
# seeded_tab_id at all (see AP_BACKEND_HERDR_WS_SEEDED_TAB_ID below); an
...
ap_backend_herdr_workspace_prune_seeded_default_tab() {  # <session> <workspace_id> <seeded_tab_id> [focus-preserving]
  local session=$1 wsid=$2 tab_id=$3 close_mode=${4:-direct} tabs tab_count current_label pane_id agent_out agent_status
  [ -n "$tab_id" ] || return 0
  tabs=$(ap_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 0
  ...
  pane_id=$(ap_backend_herdr_pane_for_tab "$session" "$wsid" "$tab_id") || return 0
  [ -n "$pane_id" ] || return 0
  agent_out=$(ap_backend_herdr_cli "$session" agent get "$pane_id" 2>/dev/null)
  ...
  if [ "$close_mode" = focus-preserving ]; then
    ap_backend_herdr_projection_close_pane_focus_preserving "$session" "$pane_id"
  else
    ap_backend_herdr_cli "$session" pane close "$pane_id" >/dev/null 2>&1 || true
  fi
}

# ap_backend_herdr_workspace_ensure: the workspace this spawn's task tab
...
# discard them:
#   AP_BACKEND_HERDR_WS_ID          - the resolved workspace_id (also echoed,
#                                      for callers that only need the id)
#   AP_BACKEND_HERDR_WS_SEEDED_TAB_ID - non-empty ONLY when THIS call just
...
#                                      (ap_backend_herdr_launcher_identity) or
#                                      a single label match
#                                      (ap_backend_herdr_workspace_find_all -
...
#                                      labeled - see
#                                      ap_backend_herdr_workspace_prune_seeded_default_tab.
...
# container being ensured belongs to the SAME autopilot home as the process
# calling this:
#   launcher-home - a flight crew member or recon for the caller's own home. When the
#                   caller is itself running in a herdr pane, the worker MUST
#                   land in that exact workspace
#                   (ap_backend_herdr_launcher_identity), never in whichever
#                   same-labeled workspace happens to sort first.
#   other-home    - a --copilot launch, which stands up a DIFFERENT home's
...
ap_backend_herdr_workspace_ensure() {  # <session> <cwd> [<launcher-relationship>]
  local session=$1 cwd=$2 relationship=${3:-launcher-home} wsid out label matches count status
  AP_BACKEND_HERDR_WS_ID=""
  AP_BACKEND_HERDR_WS_SEEDED_TAB_ID=""
  if [ "$relationship" = launcher-home ]; then
    ap_backend_herdr_launcher_identity "$session" && status=0 || status=$?
    case "$status" in
      0)
        AP_BACKEND_HERDR_WS_ID=$AP_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID
        printf '%s' "$AP_BACKEND_HERDR_WS_ID"
        return 0
        ;;
      2) ;;
      *) return 3 ;;
  ...
  label=$(ap_backend_herdr_workspace_label)
  matches=$(ap_backend_herdr_workspace_find_all "$session")
  count=$(printf '%s' "$matches" | grep -c '[^[:space:]]' || true)
  if [ "$count" -gt 1 ]; then
    echo "error: ${count} herdr workspaces in session '$session' are labeled '$label' (${matches//$'\n'/ }) and this spawn has no herdr parent pane to identify which one is its own; rename or close the extras, or run autopilot inside the workspace its workers belong in" >&2
    return 3
  fi
  wsid=${matches%%$'\n'*}
  if [ -n "$wsid" ]; then
    AP_BACKEND_HERDR_WS_ID=$wsid
    printf '%s' "$wsid"
    return 0
  fi
  out=$(ap_backend_herdr_cli "$session" workspace create --cwd "$cwd" --label "$label" --no-focus 2>/dev/null) || return 1
  wsid=$(printf '%s' "$out" | jq -r '.result.workspace.workspace_id // empty' 2>/dev/null)
  [ -n "$wsid" ] || return 1
  AP_BACKEND_HERDR_WS_ID=$wsid
  # Herdr seeds a new workspace with one auto-created default tab autopilot
  ...
  # workspace we just created. ap_backend_herdr_create_task prunes it instead,
  # once the first real task tab exists alongside it, and only ever targets
  # this exact captured tab_id.
  AP_BACKEND_HERDR_WS_SEEDED_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  printf '%s' "$wsid"
}

# ap_backend_herdr_container_ensure: the full spawn-time container-ensure
...
# must be threaded through to ap_backend_herdr_create_task, which is the only
# function allowed to prune it (ap_backend_herdr_workspace_prune_seeded_default_tab).
# <launcher-relationship> is passed straight through to
# ap_backend_herdr_workspace_ensure, which owns its meaning.
ap_backend_herdr_container_ensure() {  # <cwd-for-a-fresh-workspace> [<launcher-relationship>]
  local cwd=${1:-$PWD} relationship=${2:-launcher-home} session label status
  ap_backend_herdr_version_check || return 1
  session=$(ap_backend_herdr_session)
  ap_backend_herdr_server_ensure "$session" || return 1
  ap_backend_herdr_workspace_ensure "$session" "$cwd" "$relationship" >/dev/null && status=0 || status=$?
  ...
  if [ "$status" -ne 0 ] || [ -z "$AP_BACKEND_HERDR_WS_ID" ]; then
    label=$(ap_backend_herdr_workspace_label)
    echo "error: failed to ensure herdr workspace '$label' in session '$session'" >&2
    return 1
  fi
  printf '%s:%s\t%s' "$session" "$AP_BACKEND_HERDR_WS_ID" "$AP_BACKEND_HERDR_WS_SEEDED_TAB_ID"
}

# ap_backend_herdr_pane_agent_state: classify <pane_id> in <session> as one of
...
ap_backend_herdr_pane_agent_state() {  # <session> <pane_id>
  ...
  out=$(ap_backend_herdr_cli "$session" pane get "$pane_id" 2>&1)
  ...
  out=$(ap_backend_herdr_cli "$session" agent get "$pane_id" 2>&1)
  code=$(printf '%s' "$out" | jq -r '.error.code // empty' 2>/dev/null)
  if [ -n "$code" ]; then
...
# ap_backend_herdr_tab_is_husk: true (0) only for the two conservative husk
# states (dead, no-agent) ap_backend_herdr_pane_agent_state can positively
...
ap_backend_herdr_tab_is_husk() {  # <session> <pane_id>
  case "$(ap_backend_herdr_pane_agent_state "$1" "$2")" in
    dead|no-agent) return 0 ;;
    *) return 1 ;;
  esac
}

# ap_backend_herdr_agent_state: recovery-grade state for the same session-start
...
ap_backend_herdr_agent_state() {  # <target>
  local target=$1
  ap_backend_herdr_parse_target "$target" || { printf 'unreadable'; return 0; }
  case "$(ap_backend_herdr_pane_agent_state "$AP_BACKEND_HERDR_SESSION" "$AP_BACKEND_HERDR_PANE")" in
    dead) printf 'missing' ;;
...
# agent verdict. The detailed state contract is owned by ap_backend_agent_state.
ap_backend_herdr_agent_alive() {  # <target>
  case "$(ap_backend_herdr_agent_state "$1")" in
    alive) printf 'alive' ;;
...
# ap_backend_herdr_create_task: create the task's tab (one pane) in
...
# panes) across a server restart, including a reboot, and a restored ap-<id>
# task tab comes back a HUSK - a dead pane, or (today, and unconditionally
# once a future `resume_agents_on_restore = false` config flights) a plain
# agent-less shell sitting in the saved cwd, never the flight crew member that used to
# be there. Before this fix, every fleet respawn after such a restart needed
# the operator to manually close each husk pane first before autopilot could
# spawn into it again. ap_backend_herdr_tab_is_husk classifies the existing
...
# This mirrors ap_backend_herdr_workspace_prune_seeded_default_tab's own
...
# ap_backend_herdr_workspace_ensure captured as AP_BACKEND_HERDR_WS_SEEDED_TAB_ID
...
# ap_backend_herdr_workspace_prune_seeded_default_tab for the incident and
# the safety argument). An ADOPTED workspace's caller always passes an empty
# 4th arg, so this function never even queries for a prune candidate in that
# case. Echoes "<tab_id> <pane_id>" on success.
ap_backend_herdr_create_task() {  # <container> <label> <cwd> <seeded_default_tab_id>
  local container=$1 label=$2 cwd=$3 seeded_tab_id=${4:-} session wsid list dup_tabs dup dup_pane dup_tab_ids out tab_id pane_id remaining_dup_tabs
  session=${container%%:*}
  wsid=${container#*:}
  list=$(ap_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 1
  ...
  if [ -n "$dup_tabs" ]; then
    while IFS= read -r dup; do
      [ -n "$dup" ] || continue
      dup_pane=$(ap_backend_herdr_pane_for_tab "$session" "$wsid" "$dup")
      if [ -z "$dup_pane" ] || ! ap_backend_herdr_tab_is_husk "$session" "$dup_pane"; then
...
EOF
  ...
  out=$(ap_backend_herdr_cli "$session" tab create --workspace "$wsid" --cwd "$cwd" --label "$label" --no-focus 2>/dev/null) || return 1
  ...
  [ -z "$seeded_tab_id" ] || ap_backend_herdr_workspace_prune_seeded_default_tab "$session" "$wsid" "$seeded_tab_id"
  if [ -n "$dup_tab_ids" ]; then
    while IFS= read -r dup; do
      [ -n "$dup" ] || continue
      ap_backend_herdr_cli "$session" tab close "$dup" >/dev/null 2>&1 || true
    done <<EOF
$dup_tab_ids
EOF
    list=$(ap_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || {
...
# ap_backend_herdr_projection_create_task: create one disposable presentation
# workspace and its normal ap-<id> task tab without looking up, adopting, or
...
#   AP_BACKEND_HERDR_PROJECTION_SESSION
...
#   AP_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE
...
# cleanup authority.
ap_backend_herdr_projection_create_task() {  # <cwd> <workspace-label> <task-label>
  ...
  AP_BACKEND_HERDR_PROJECTION_TAB_ID=""
  AP_BACKEND_HERDR_PROJECTION_PANE_ID=""
  AP_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE=0
  ...
  if out=$(ap_backend_herdr_cli "$session" workspace create --cwd "$cwd" --label "$workspace_label" --no-focus 2>/dev/null); then
    :
  else
    ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace create" || true
    echo "error: herdr presentation workspace create failed ambiguously; leaving its journal quarantined" >&2
    return 1
  fi
  ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace create" || {
  ...
  AP_BACKEND_HERDR_PROJECTION_SESSION=$session
  AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID=$(printf '%s' "$out" | jq -r '.result.workspace.workspace_id // empty' 2>/dev/null)
  AP_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  AP_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID=$(printf '%s' "$out" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null)
  if [ -z "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" ] \
     || [ -z "$AP_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" ] \
     || [ -z "$AP_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID" ]; then
  ...
  if out=$(ap_backend_herdr_cli "$session" tab create \
    --workspace "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
    --cwd "$cwd" --label "$task_label" --no-focus 2>/dev/null); then
    :
  else
    ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "task-tab create" || true
    echo "error: herdr presentation task-tab create failed ambiguously; leaving its journal quarantined" >&2
    return 1
  fi
  ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "task-tab create" || {
    echo "error: herdr presentation task-tab create did not preserve exact active focus; leaving its journal quarantined" >&2
    return 1
  }
  AP_BACKEND_HERDR_PROJECTION_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  AP_BACKEND_HERDR_PROJECTION_PANE_ID=$(printf '%s' "$out" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null)
  if [ -z "$AP_BACKEND_HERDR_PROJECTION_TAB_ID" ] || [ -z "$AP_BACKEND_HERDR_PROJECTION_PANE_ID" ]; then
  ...
  AP_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE=1
  focus_before=$(ap_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  if ! ap_backend_herdr_workspace_prune_seeded_default_tab \
    "$session" \
    "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
    "$AP_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
  ...
  ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "seeded-tab prune" || {
    echo "error: herdr presentation seeded-tab prune did not preserve exact active focus; leaving its journal quarantined" >&2
    return 1
  }

  tabs=$(ap_backend_herdr_cli "$session" tab list --workspace "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" 2>/dev/null) || {
    echo "error: could not verify the disposable herdr presentation workspace shape" >&2
    return 1
  }
  panes=$(ap_backend_herdr_cli "$session" pane list --workspace "$AP_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" 2>/dev/null) || {
  ...
  if [ "$tab_count" != 1 ] || [ "$pane_count" != 1 ] \
     || ! printf '%s' "$tabs" | jq -e --arg task "$AP_BACKEND_HERDR_PROJECTION_TAB_ID" \
       --arg seeded "$AP_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
       '.result.tabs[0].tab_id == $task and ([.result.tabs[] | select(.tab_id == $seeded)] | length) == 0' >/dev/null 2>&1 \
     || ! printf '%s' "$panes" | jq -e --arg pane "$AP_BACKEND_HERDR_PROJECTION_PANE_ID" \
       --arg tab "$AP_BACKEND_HERDR_PROJECTION_TAB_ID" \
...
# ap_backend_herdr_projection_cleanup_exact: same-process abort cleanup for a
# projection whose create calls returned complete exact IDs.
# It performs no lookup and never calls workspace close.
ap_backend_herdr_projection_cleanup_exact() {  # <session> <task-pane> <seeded-pane>
  local session=$1 task_pane=$2 seeded_pane=$3
  [ -z "$task_pane" ] || ap_backend_herdr_projection_close_pane_focus_preserving "$session" "$task_pane" || true
  if [ -n "$seeded_pane" ] && [ "$seeded_pane" != "$task_pane" ]; then
    ap_backend_herdr_projection_close_pane_focus_preserving "$session" "$seeded_pane" || true
  fi
}

# ap_backend_herdr_projection_parent_workspace_exact: resolve one exact parent
# workspace only when its presentation label is unique in the named session.
ap_backend_herdr_projection_parent_workspace_exact() {  # <session> <parent-label>
...
# ap_backend_herdr_projection_live_binding_matches: verify one exact projected
...
ap_backend_herdr_projection_live_binding_matches() {  # <session> <token> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
...
ap_backend_herdr_projection_reclaim_rollback() {  # <session> <new-pane>
  local session=$1 new_pane=$2 state
  state=$(ap_backend_herdr_pane_agent_state "$session" "$new_pane")
  case "$state" in
    dead) return 0 ;;
    no-agent) ;;
    live|unknown) return 1 ;;
  esac
  ap_backend_herdr_projection_close_pane_focus_preserving "$session" "$new_pane" no-agent || return 1
  [ "$(ap_backend_herdr_pane_agent_state "$session" "$new_pane")" = dead ]
}

# ap_backend_herdr_projection_reclaim_task: replace one exact agent-free
...
ap_backend_herdr_projection_reclaim_task() {  # <session> <journal> <task-id> <home> <meta-workspace> <meta-tab> <meta-pane> <parent-label> <task-label> <cwd>
  ...
  canonical_home=$(ap_backend_herdr_projection_home_identity "$home") || {
    echo "warning: herdr presentation home for $id could not be resolved exactly; spawning flat" >&2
    return 2
  }
  if [ "$AP_BACKEND_HERDR_JOURNAL_HOME" != "$canonical_home" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_SESSION" != "$session" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" != "$meta_workspace" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_TAB_ID" != "$meta_tab" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_PANE_ID" != "$meta_pane" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_PARENT_LABEL" != "$parent_label" ] \
     || [ "$AP_BACKEND_HERDR_JOURNAL_TASK_LABEL" != "$task_label" ]; then
    echo "warning: herdr presentation binding for $id does not match its exact home, endpoint, or parent; spawning flat" >&2
    return 2
  fi
  if ! ap_backend_herdr_projection_live_binding_matches \
    "$session" "$AP_BACKEND_HERDR_JOURNAL_PROJECTION_ID" \
    "$meta_workspace" "$meta_tab" "$meta_pane" \
    "$AP_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" "$parent_label" \
    "$AP_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" "$task_label"; then
    echo "warning: herdr presentation binding for $id has an ambiguous, renamed, foreign, or non-nested live shape; spawning flat" >&2
    return 2
  fi
  state=$(ap_backend_herdr_pane_agent_state "$session" "$meta_pane")
  case "$state" in
    no-agent) ;;
    dead)
  ...
  if ! out=$(ap_backend_herdr_cli "$session" tab create \
    ...
    ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "husk replacement create" || return 1
  ...
  ap_backend_herdr_projection_focus_restore "$session" "$focus_before" "husk replacement create" || return 1
  info=$(ap_backend_herdr_cli "$session" tab get "$new_tab" 2>/dev/null) || info=
  if ! printf '%s' "$info" | jq -e --arg tab "$new_tab" --arg workspace "$meta_workspace" '
    .result.tab.tab_id == $tab and .result.tab.workspace_id == $workspace
  ' >/dev/null 2>&1; then
    ap_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    echo "warning: herdr presentation reclaim for $id could not verify its replacement tab; spawning flat" >&2
    return 2
  fi
  info=$(ap_backend_herdr_cli "$session" pane get "$new_pane" 2>/dev/null) || info=
  ...
  if ap_backend_herdr_projection_close_pane_focus_preserving "$session" "$meta_pane" no-agent; then
    close_status=0
  else
    close_status=$?
  fi
  if [ "$close_status" -ne 0 ]; then
    ...
    state=$AP_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE
    ap_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    case "$state" in
      live|unknown)
  ...
  if [ "$(ap_backend_herdr_pane_agent_state "$session" "$meta_pane")" != dead ]; then
    ap_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    return 1
  fi
  if ! ap_backend_herdr_projection_live_binding_matches \
  ...
  if ! ap_backend_herdr_projection_journal_replace_endpoint \
  ...
  AP_BACKEND_HERDR_PROJECTION_TAB_ID=$new_tab
  AP_BACKEND_HERDR_PROJECTION_PANE_ID=$new_pane
  return 0
}

# ap_backend_herdr_projection_recovery_allows_flat: inspect an existing
...
ap_backend_herdr_projection_recovery_allows_flat() {  # <session> <journal> <task-id>
  local session=$1 journal=$2 id=$3 token list wsids count wsid panes pane_ids pane state
  token=$(ap_backend_herdr_projection_journal_token "$journal" "$id") || {
  ...
  while IFS= read -r wsid; do
    [ -n "$wsid" ] || continue
    panes=$(ap_backend_herdr_cli "$session" pane list --workspace "$wsid" 2>/dev/null) || {
...
# ap_backend_herdr_projection_endpoint_matches_journal: read-only correlation
...
ap_backend_herdr_projection_endpoint_matches_journal() {  # <session> <workspace-id> <journal> <task-id>
  local session=$1 workspace_id=$2 journal=$3 id=$4 token list matches
  token=$(ap_backend_herdr_projection_journal_token "$journal" "$id") || return 1
  list=$(ap_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 1
...
# ap_backend_herdr_parse_target: split "<session>:<pane_id>" (pane_id itself
# contains a colon, e.g. "w1:p2") on the FIRST colon only. Sets
# AP_BACKEND_HERDR_SESSION and AP_BACKEND_HERDR_PANE for the caller.
ap_backend_herdr_parse_target() {  # <target>
  local target=$1
  AP_BACKEND_HERDR_SESSION=${target%%:*}
  AP_BACKEND_HERDR_PANE=${target#*:}
  [ -n "$AP_BACKEND_HERDR_SESSION" ] && [ -n "$AP_BACKEND_HERDR_PANE" ] && [ "$AP_BACKEND_HERDR_PANE" != "$target" ]
}

ap_backend_herdr_target_ready() {  # <target>
  ap_backend_herdr_parse_target "$1" || return 1
  ap_backend_herdr_server_ensure "$AP_BACKEND_HERDR_SESSION" || return 1
}

# ap_backend_herdr_current_path: the live FOREGROUND process's cwd, or empty on
...
# would make ap-spawn.sh's worktree-discovery poll never see the pane "leave"
...
ap_backend_herdr_current_path() {  # <target>
  ap_backend_herdr_target_ready "$1" || return 0
  ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane get "$AP_BACKEND_HERDR_PANE" 2>/dev/null \
    | jq -r '.result.pane.foreground_cwd // empty' 2>/dev/null
}

# ap_backend_herdr_send_text_line: send one line of TEXT then submit,
# ATOMICALLY - mirrors tmux's `send-keys -t T text Enter`. Used for the fixed
# spawn-time commands (treehouse get, the GOTMPDIR export). `pane run` types
# the command and submits it in one call (verified).
ap_backend_herdr_send_text_line() {  # <target> <text>
  ap_backend_herdr_target_ready "$1" || return 1
  ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane run "$AP_BACKEND_HERDR_PANE" "$2" >/dev/null 2>&1
}

# ap_backend_herdr_send_literal: send TEXT as literal, UNSUBMITTED input - the
...
ap_backend_herdr_send_literal() {  # <target> <text>
  ap_backend_herdr_target_ready "$1" || return 1
  ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane send-text "$AP_BACKEND_HERDR_PANE" "$2" >/dev/null 2>&1
}

# ap_backend_herdr_normalize_key: map autopilot's key vocabulary (Enter,
# Escape, C-c, as used by ap-send.sh --key and stuck-flight-crew-recovery) onto
...
ap_backend_herdr_normalize_key() {  # <key>
  case "$1" in
...
# ap_backend_herdr_send_key: one named special key. Mirrors ap-send.sh's --key
# path (tmux's `send-keys -t T key`).
ap_backend_herdr_send_key() {  # <target> <key>
  ap_backend_herdr_target_ready "$1" || return 1
  local key
  key=$(ap_backend_herdr_normalize_key "$2")
  ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane send-keys "$AP_BACKEND_HERDR_PANE" "$key" >/dev/null 2>&1
}

# ap_backend_herdr_capture: bounded plain-text pane capture. Mirrors
# ap-peek.sh's/ap-watch.sh's `tmux capture-pane -p -t T -S -N`. --source recent
...
ap_backend_herdr_capture() {  # <target> <lines>
  ap_backend_herdr_target_ready "$1" || return 1
  local lines=${2:-200} fetch out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  out=$(ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane read "$AP_BACKEND_HERDR_PANE" --source recent --lines "$fetch" 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}

ap_backend_herdr_capture_ansi() {  # <target> <lines>
  ap_backend_herdr_target_ready "$1" || return 1
  local lines=${2:-200} fetch out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  out=$(ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane read "$AP_BACKEND_HERDR_PANE" --source recent --lines "$fetch" --format ansi 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}

# Thin adapter over the shared plain-text stripper (bin/ap-composer-lib.sh),
...
# the shared ap_composer_strip_ghost instead.
ap_backend_herdr_strip_ansi() {  # <text>
  printf '%s' "$1" | ap_composer_strip_ansi
}

# ap_backend_herdr_composer_state: classify the composer's own row as
...
#             recognized by the shared ap_composer_strip_ghost extractor
...
# shared ap_composer_strip_ghost (bin/ap-composer-lib.sh), which drops dim/faint
...
AP_BACKEND_HERDR_COMPOSER_LINES=${AP_BACKEND_HERDR_COMPOSER_LINES:-20}
...
AP_BACKEND_HERDR_IDLE_RE=${AP_BACKEND_HERDR_IDLE_RE:-'^Type a message\.\.\.$'}
...
AP_BACKEND_HERDR_BARE_PROMPT_RE=${AP_BACKEND_HERDR_BARE_PROMPT_RE:-'^(❯|›)'}
# Pi allows a multi-line composer between its horizontal separators. Bound the
# structural candidate so two unrelated transcript rules with an arbitrarily
# large region between them can never be promoted into a composer.
AP_BACKEND_HERDR_PI_COMPOSER_MAX_LINES=${AP_BACKEND_HERDR_PI_COMPOSER_MAX_LINES:-8}

ap_backend_herdr_pi_separator_row() {  # <plain-row>
  local row=$1
  row="${row#"${row%%[![:space:]]*}"}"
  row="${row%"${row##*[![:space:]]}"}"
  [ "${#row}" -ge 8 ] || return 1
...
ap_backend_herdr_pi_composer_find() {  # <ansi-capture>
  local cap=$1 line plain open=0 lines=0 candidate="" max row=0 open_row=0
  max=$AP_BACKEND_HERDR_PI_COMPOSER_MAX_LINES
  case "$max" in ''|*[!0-9]*|0) max=8 ;; esac
  AP_BACKEND_HERDR_PI_PAIR_FOUND=0
  AP_BACKEND_HERDR_PI_PAIR_VALID=0
  AP_BACKEND_HERDR_PI_PAIR_OPEN_LINE=0
  AP_BACKEND_HERDR_PI_PAIR_LINE=0
  AP_BACKEND_HERDR_PI_LAST_SEPARATOR_LINE=0
  AP_BACKEND_HERDR_PI_CONTENT=""
  while IFS= read -r line; do
    row=$((row + 1))
    plain=$(ap_backend_herdr_strip_ansi "$line")
    if ap_backend_herdr_pi_separator_row "$plain"; then
      AP_BACKEND_HERDR_PI_LAST_SEPARATOR_LINE=$row
      if [ "$open" -eq 1 ]; then
        AP_BACKEND_HERDR_PI_PAIR_FOUND=1
        AP_BACKEND_HERDR_PI_PAIR_OPEN_LINE=$open_row
        AP_BACKEND_HERDR_PI_PAIR_LINE=$row
        if [ "$lines" -le "$max" ]; then
          AP_BACKEND_HERDR_PI_PAIR_VALID=1
          AP_BACKEND_HERDR_PI_CONTENT=$candidate
        else
          AP_BACKEND_HERDR_PI_PAIR_VALID=0
          AP_BACKEND_HERDR_PI_CONTENT=""
        fi
...
ap_backend_herdr_agent_identity_raw() {  # <session> <pane> -> <agent>\t<status>
  local out
  out=$(ap_backend_herdr_cli "$1" agent get "$2" 2>/dev/null) || return 1
  printf '%s' "$out" | jq -r '[.result.agent.agent // "", .result.agent.agent_status // ""] | @tsv' 2>/dev/null
}

ap_backend_herdr_composer_state() {  # <target> -> empty|pending|unknown
  ...
  ap_backend_herdr_parse_target "$target" || { printf 'unknown'; return 0; }
  session=$AP_BACKEND_HERDR_SESSION
  pane=$AP_BACKEND_HERDR_PANE
  cap=$(ap_backend_herdr_capture_ansi "$target" "$AP_BACKEND_HERDR_COMPOSER_LINES" 2>/dev/null \
    || ap_backend_herdr_capture "$target" "$AP_BACKEND_HERDR_COMPOSER_LINES") || { printf 'unknown'; return 0; }
  # Structural scan: locate the bottom-most composer row and remember its RAW
  # (styled) bytes. Shape detection runs on the plain row (ap_backend_herdr_strip_ansi
  ...
  while IFS= read -r line; do
    row=$((row + 1))
    trimmed=$(ap_backend_herdr_strip_ansi "$line")
    ...
    case "$trimmed" in
      '│'*'│'|'┃'*'┃'|'|'*'|')
        ...
        ;;
      *)
        if printf '%s' "$trimmed" | grep -qE "$AP_BACKEND_HERDR_BARE_PROMPT_RE"; then
          shape=bare
          raw_match=$line
          generic_line=$row
          found=1
  ...
  ap_backend_herdr_pi_composer_find "$cap"
  if [ "$AP_BACKEND_HERDR_PI_PAIR_FOUND" -eq 1 ] \
     && [ "$AP_BACKEND_HERDR_PI_PAIR_LINE" -gt "$generic_line" ] \
     && [ "$generic_line" -lt "$AP_BACKEND_HERDR_PI_PAIR_OPEN_LINE" ]; then
    identity=$(ap_backend_herdr_agent_identity_raw "$session" "$pane" 2>/dev/null || true)
    IFS=$'\t' read -r agent agent_status <<EOF
$identity
EOF
    case "$agent:$agent_status" in
      pi:idle|pi:done|pi:blocked)
        if [ "$AP_BACKEND_HERDR_PI_PAIR_VALID" -eq 1 ]; then
          shape=separated
          raw_match=$AP_BACKEND_HERDR_PI_CONTENT
          found=1
        else
          found=0
        fi
  ...
  elif [ "$AP_BACKEND_HERDR_PI_PAIR_FOUND" -eq 0 ] \
       && [ "$AP_BACKEND_HERDR_PI_LAST_SEPARATOR_LINE" -gt "$generic_line" ]; then
  ...
  # fleet-wide ghost stripper (bin/ap-composer-lib.sh), which drops dim/faint AND
  ...
  stripped=$(printf '%s\n' "$raw_match" | ap_composer_strip_ghost)
  ...
  # shape only ever starts with an AGENT glyph (AP_BACKEND_HERDR_BARE_PROMPT_RE
  # is '^(❯|›)'), so a bare shell prompt never reaches here - it stays 'unknown'
  # via the no-composer-row path above, exactly as before.
  ap_composer_classify_content "$bordered" "$stripped" "$AP_BACKEND_HERDR_IDLE_RE"
}

# ap_backend_herdr_send_text_submit: type <text> into <target> once (raw,
...
# submission is confirmed by ap_backend_herdr_wait_for_working observing a
...
# empty-box guard, still dispatched via ap_backend_composer_state /
# ap_backend_herdr_composer_state) and for submit attempts whose pre-Enter
...
#   - Slow transition: ap_backend_herdr_wait_for_working samples repeatedly
...
#     ap-send.sh/ap-supervise-daemon.sh: retyping only happens if a caller
...
ap_backend_herdr_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle>
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 i=0 verdict baseline confirm_sleep
  ap_backend_herdr_parse_target "$target" || { printf 'unknown'; return 0; }
  ap_backend_herdr_send_literal "$target" "$text" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  baseline=$(ap_backend_herdr_classify_submit_agent_status \
    "$(ap_backend_herdr_agent_status_raw "$AP_BACKEND_HERDR_SESSION" "$AP_BACKEND_HERDR_PANE")")
  confirm_sleep=$(ap_backend_herdr_submit_confirm_budget "$sleep_s")
  while :; do
    ap_backend_herdr_send_key "$target" Enter || true
    if [ "$baseline" = idle ]; then
      verdict=$(ap_backend_herdr_wait_for_working "$AP_BACKEND_HERDR_SESSION" "$AP_BACKEND_HERDR_PANE" \
        "$confirm_sleep" "$AP_BACKEND_HERDR_SUBMIT_POLLS")
    else
      sleep "$sleep_s"
      verdict=$(ap_backend_herdr_composer_state "$target")
    fi
    case "$verdict" in
...
# ap_backend_herdr_kill: remove the task's pane, best-effort (mirrors
...
ap_backend_herdr_kill() {  # <target>
  ap_backend_herdr_target_ready "$1" || return 0
  ap_backend_herdr_cli "$AP_BACKEND_HERDR_SESSION" pane close "$AP_BACKEND_HERDR_PANE" >/dev/null 2>&1 || true
}

# ap_backend_herdr_classify_agent_status: map a raw `agent get` agent_status
...
ap_backend_herdr_classify_agent_status() {  # <raw-agent_status>
  case "$1" in
    working) printf 'busy' ;;
...
ap_backend_herdr_classify_submit_agent_status() {  # <raw-agent_status>
  case "$1" in
...
# ap_backend_herdr_agent_status_raw: one `agent get` read, echoing the raw
# agent_status string (working/idle/done/blocked/...), or empty on any
# failure. Deliberately skips ap_backend_herdr_target_ready's server-ensure
# round trip (an extra `status --json` call) that ap_backend_herdr_busy_state
# pays on every call: ap_backend_herdr_wait_for_working polls this in a tight
# loop right after a caller has already parsed the target and confirmed the
# server is live (e.g. ap_backend_herdr_send_text_submit, immediately after a
# successful send-text), so re-checking server liveness on every poll would
# only add latency without adding safety.
ap_backend_herdr_agent_status_raw() {  # <session> <pane_id>
  local session=$1 pane_id=$2 out
  out=$(ap_backend_herdr_cli "$session" agent get "$pane_id" 2>/dev/null) || { printf ''; return 0; }
  printf '%s' "$out" | jq -r '.result.agent.agent_status // empty' 2>/dev/null
}

# ap_backend_herdr_busy_state: semantic busy state from herdr's native
# agent-state detection (agent.get), the "first backend where ap_session_busy_state
# gets real semantics" per the design report. See
# ap_backend_herdr_classify_agent_status for the status->busy/idle/unknown
# mapping.
ap_backend_herdr_busy_state() {  # <target>
  ap_backend_herdr_target_ready "$1" || { printf 'unknown'; return 0; }
  ap_backend_herdr_classify_agent_status \
    "$(ap_backend_herdr_agent_status_raw "$AP_BACKEND_HERDR_SESSION" "$AP_BACKEND_HERDR_PANE")"
}

# ap_backend_herdr_wait_for_working: poll <session>:<pane_id>'s NATIVE
...
# AP_BACKEND_HERDR_SUBMIT_POLLS (default 6): how many samples
# ap_backend_herdr_send_text_submit spreads across each Enter attempt's
...
AP_BACKEND_HERDR_SUBMIT_POLLS=${AP_BACKEND_HERDR_SUBMIT_POLLS:-6}
AP_BACKEND_HERDR_SUBMIT_MIN_SLEEP=${AP_BACKEND_HERDR_SUBMIT_MIN_SLEEP:-0.6}

ap_backend_herdr_submit_confirm_budget() {  # <caller-budget-seconds>
  awk -v b="${1:-0}" -v m="$AP_BACKEND_HERDR_SUBMIT_MIN_SLEEP" 'BEGIN {
    b += 0
    m += 0
    if (b < 0) b = 0
    if (m < 0) m = 0
...
ap_backend_herdr_wait_for_working() {  # <session> <pane_id> <budget-seconds> <polls>
  ...
  for ((i = 0; i < polls; i++)); do
    ...
    raw=$(ap_backend_herdr_agent_status_raw "$session" "$pane_id")
    bs=$(ap_backend_herdr_classify_submit_agent_status "$raw")
    case "$bs" in
...
# ap_backend_herdr_pane_for_tab: the root pane id for <tab_id> in <workspace_id>
...
ap_backend_herdr_pane_for_tab() {  # <session> <workspace_id> <tab_id>
  local session=$1 wsid=$2 tab_id=$3 panes
  panes=$(ap_backend_herdr_cli "$session" pane list --workspace "$wsid" 2>/dev/null) || return 1
  printf '%s' "$panes" | jq -r --arg tab "$tab_id" \
    '.result.panes[]? | select(.tab_id == $tab) | .pane_id' 2>/dev/null | head -1
}

# ap_backend_herdr_resolve_bare_selector: the live-tab-listing fallback for an
...
ap_backend_herdr_resolve_bare_selector() {  # <name>
  ...
  while IFS= read -r session; do
    [ -n "$session" ] || continue
    tabs=$(ap_backend_herdr_cli "$session" tab list 2>/dev/null) || continue
    ...
    [ -n "$wsid" ] || continue
    pane_id=$(ap_backend_herdr_pane_for_tab "$session" "$wsid" "$tab_id") || continue
    [ -n "$pane_id" ] || continue
...
# ap_backend_herdr_list_live: recovery/orphan discovery. Lists every tab whose
# label looks like an autopilot task window (ap-<id>) in <session>'s, THIS
# HOME'S OWN workspace (ap_backend_herdr_workspace_label - never another
...
# "ID stability"). A caller running as a given home (e.g. a copilot
# recovering its own in-flight work) naturally scopes to that home's own
# workspace because AP_HOME already names it - no glue needed, unlike the
# primary-spawns-a-copilot path in ap-spawn.sh. Read-only: a session/
# workspace that does not exist yet simply lists nothing. One
# "<session>:<pane_id>\t<label>" line per live task tab.
ap_backend_herdr_list_live() {  # <session>
  local session=$1 wsid tabs tab_id label pane_id
  wsid=$(ap_backend_herdr_workspace_find "$session") || return 0
  [ -n "$wsid" ] || return 0
  tabs=$(ap_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 0
  ...
  done < <(printf '%s' "$tabs" | jq -r '.result.tabs[]? | select(.label | startswith("ap-")) | "\(.tab_id)\t\(.label)"' 2>/dev/null)
}
...
# ap_backend_herdr_wait_transition is the watcher's bounded wait primitive for
...
# a flight crew waiting on the human wakes its supervisor sub-second instead of after
...
# ap_backend_herdr_socket_path: the control-socket path for <session>, read from
...
ap_backend_herdr_socket_path() {  # <session>
  local session=$1
...
# ap_backend_herdr_events_capable: the version/capability gate for the event
...
# reader override is configured); client protocol >= AP_BACKEND_HERDR_MIN_EVENTS_PROTOCOL;
# and both `events.subscribe` and `pane.agent_status_changed` present in `herdr
# api schema`. AP_BACKEND_HERDR_EVENTS_FORCE overrides the whole verdict for
...
ap_backend_herdr_events_capable() {  # <session>
  local session=$1 protocol schema
  case "${AP_BACKEND_HERDR_EVENTS_FORCE:-}" in
    1) return 0 ;;
    0) return 1 ;;
  esac
  ap_backend_herdr_tool_check || return 1
  if [ -z "${AP_BACKEND_HERDR_EVENT_READER:-}" ]; then
    command -v python3 >/dev/null 2>&1 || return 1
  fi
  protocol=$(herdr status --json 2>/dev/null | jq -r '.client.protocol // empty' 2>/dev/null)
  case "$protocol" in ''|*[!0-9]*) return 1 ;; esac
  [ "$protocol" -ge "$AP_BACKEND_HERDR_MIN_EVENTS_PROTOCOL" ] || return 1
...
# ap_backend_herdr_normalize_event: THE single normalize point (report section 5
...
ap_backend_herdr_normalize_event() {  # <pane_id> <workspace_id> <agent_status> <agent>
  ap_transition_record "${1:-}" "${2:-}" "" "${3:-}" "${4:-}"
}

# ap_backend_herdr_event_reader_cmd: emit the reader argv (one word per line) for
# the raw-socket subscriber. Default: `python3 <this dir>/herdr-eventwait.py`.
# AP_BACKEND_HERDR_EVENT_READER overrides it with a whitespace-split command so
# tests can substitute a fake reader that replays canned stream lines.
ap_backend_herdr_event_reader_cmd() {
  local word
  if [ -n "${AP_BACKEND_HERDR_EVENT_READER:-}" ]; then
    for word in $AP_BACKEND_HERDR_EVENT_READER; do
      printf '%s\n' "$word"
    done
    return 0
  fi
  printf 'python3\n'
  printf '%s\n' "$AP_BACKEND_HERDR_ROOT/bin/backends/herdr-eventwait.py"
}

# ap_backend_herdr_escalation_marker: the per-pane dedupe marker path for a
# <window> ("<session>:<pane_id>"), keyed identically to the watcher's
# .stale-<key> (tr ':/.' '___'), under <state_dir>.
ap_backend_herdr_escalation_marker() {  # <state_dir> <window>
  local state=$1 window=$2 key
  key=$(printf '%s' "$window" | tr ':/.' '___')
  printf '%s/%s%s' "$state" "$AP_BACKEND_HERDR_ESCALATED_PREFIX" "$key"
}

# ap_backend_herdr_apply_transition: route one normalized record through the
...
ap_backend_herdr_apply_transition() {  # <state_dir> <session> <record>
  ...
  action=$(ap_transition_policy "$to")
  window="$session:$pane_id"
  marker=$(ap_backend_herdr_escalation_marker "$state" "$window")
  case "$action" in
    actionable)
      if [ ! -e "$marker" ]; then
        printf '%s' "$record"
...
ap_backend_herdr_commit_transition() {  # <state_dir> <session> <record>
...
ap_backend_herdr_clear_transition() {  # <state_dir> <window>
...
# ap_backend_herdr_wait_transition: the bounded event wait. Blocks up to
...
ap_backend_herdr_wait_transition() {  # <session> <timeout_secs> <state_dir> <pane_window...>
  local session=$1 timeout=$2 state=$3
  shift 3
  local windows=("$@")
  [ "${#windows[@]}" -gt 0 ] || return 2
  if [ "${AP_BACKEND_EVENTS_CAPABILITY_CONFIRMED:-0}" != 1 ]; then
    ap_backend_herdr_events_capable "$session" || return 2
  fi
  local sock
  sock=$(ap_backend_herdr_socket_path "$session")
  [ -n "$sock" ] || return 2
  ...
  if [ "$rc" -ne 2 ]; then
    for w in "${windows[@]}"; do
      ...
      raw=$(ap_backend_herdr_agent_status_raw "$session" "$pane_id")
      [ -n "$raw" ] || continue
      record=$(ap_backend_herdr_normalize_event "$pane_id" "" "$raw" "")
      if hit=$(ap_backend_herdr_apply_transition "$state" "$session" "$record"); then
        printf '%s' "$hit"
        rc=0
        break
      fi
  ...
  while [ "$rc" -eq 1 ] && IFS= read -r line <&9; do
    ...
    [ -n "$pane_id" ] || continue
    record=$(ap_backend_herdr_normalize_event "$pane_id" "$ws" "$status" "$agent")
    if hit=$(ap_backend_herdr_apply_transition "$state" "$session" "$record"); then
      printf '%s' "$hit"
      rc=0
      break
