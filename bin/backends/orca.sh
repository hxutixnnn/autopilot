...
ap_backend_orca_json_get() {  # <field> ; fields: worktree-id worktree-path terminal-handle worktree-terminal-handle repo-id
...
ap_backend_orca_run_json() {
  local out
  out=$("$@") || return 1
  printf '%s' "$out" | ap_backend_orca_json_ok
}

ap_backend_orca_repo_ensure() {  # <project-path>
  local project=$1 out repo_id
  ap_backend_orca_tool_check || return 1
  out=$(orca repo show --repo "path:$project" --json 2>/dev/null || true)
  if repo_id=$(printf '%s' "$out" | ap_backend_orca_json_get repo-id 2>/dev/null); then
    printf '%s' "$repo_id"
    return 0
  fi
  out=$(orca repo add --path "$project" --json) || return 1
  repo_id=$(printf '%s' "$out" | ap_backend_orca_json_get repo-id) || {
...
ap_backend_orca_worktree_create() {  # <project-path> <name>
  local project=$1 name=$2 repo_id out wt_id wt_path terminal
  repo_id=$(ap_backend_orca_repo_ensure "$project") || return 1
  out=$(orca worktree create --repo "id:$repo_id" --name "$name" --no-parent --setup skip --json) || return 1
  wt_id=$(printf '%s' "$out" | ap_backend_orca_json_get worktree-id) || {
    echo "error: orca worktree create did not return a worktree id for $name" >&2
    return 1
  }
  terminal=$(printf '%s' "$out" | ap_backend_orca_json_get worktree-terminal-handle 2>/dev/null || true)
  wt_path=$(printf '%s' "$out" | ap_backend_orca_json_get worktree-path) || {
    echo "error: orca worktree create did not return a path for $name" >&2
    [ -z "$terminal" ] || ap_backend_orca_kill "$terminal" >/dev/null 2>&1 || true
    if ap_backend_orca_remove_worktree "$wt_id" >/dev/null; then
      return 1
    fi
    if [ -n "$terminal" ]; then
...
ap_backend_orca_send_text_line() {  # <terminal-id> <text>
  local terminal=$1 text=$2
  ap_backend_orca_tool_check || return 1
  ap_backend_orca_run_json orca terminal send --terminal "$terminal" --text "$text" --enter --json
}

ap_backend_orca_send_literal() {  # <terminal-id> <text>
  local terminal=$1 text=$2
  ap_backend_orca_tool_check || return 1
  ap_backend_orca_run_json orca terminal send --terminal "$terminal" --text "$text" --json
}

ap_backend_orca_remove_worktree() {  # <worktree-id>
  local worktree_id=${1:-}
  [ -n "$worktree_id" ] || { echo "error: missing Orca worktree id; cannot remove worktree" >&2; return 1; }
  ap_backend_orca_tool_check || return 1
  ap_backend_orca_run_json orca worktree rm --worktree "id:$worktree_id" --force --json
}

ap_backend_orca_worktree_path() {
  ...
  path=$(printf '%s' "$out" | ap_backend_orca_json_get worktree-path) || {
...
ap_backend_orca_capture() {  # <terminal-id> <lines>
  local terminal=$1 lines=${2:-40} out
  ap_backend_orca_tool_check || return 1
  out=$(orca terminal read --terminal "$terminal" --limit "$lines" --json) || return 1
  ap_backend_orca_json_text "$out"
}

ap_backend_orca_json_text() {  # <json>
  printf '%s' "$1" | node -e '
const fs = require("fs");
const data = JSON.parse(fs.readFileSync(0, "utf8"));
if (data.ok === false) {
...
ap_backend_orca_json_field() {  # <field> <json>
  local field=$1
  printf '%s' "$2" | node -e '
const fs = require("fs");
const field = process.argv[1];
...
ap_backend_orca_read_text_paged() {  # <terminal-id> <limit>
  ...
  printf '%s' "$out" | ap_backend_orca_json_ok || return 1
  text=$(ap_backend_orca_json_text "$out") || return 1
  limited=$(ap_backend_orca_json_field limited "$out" 2>/dev/null || true)
  oldest=$(ap_backend_orca_json_field oldestCursor "$out" 2>/dev/null || true)
  if [ "$limited" = true ] && [ -n "$oldest" ]; then
    cursor_out=$(orca terminal read --terminal "$terminal" --cursor "$oldest" --limit "$limit" --json) || return 1
    printf '%s' "$cursor_out" | ap_backend_orca_json_ok || return 1
    older_text=$(ap_backend_orca_json_text "$cursor_out") || return 1
    text="${older_text}"$'\n'"${text}"
  fi
  printf '%s' "$text"
}

AP_BACKEND_ORCA_COMPOSER_LINES=${AP_BACKEND_ORCA_COMPOSER_LINES:-200}
AP_BACKEND_ORCA_IDLE_RE=${AP_BACKEND_ORCA_IDLE_RE:-'^Type a message\.\.\.$'}

# ap_backend_orca_composer_state: classify the composer's own bordered row as
...
ap_backend_orca_composer_state() {  # <terminal-id> -> empty|pending|unknown
  local terminal=$1 cap line trimmed stripped="" found=0
  cap=$(ap_backend_orca_read_text_paged "$terminal" "$AP_BACKEND_ORCA_COMPOSER_LINES") || { printf 'unknown'; return 0; }
  ...
  ap_composer_classify_content 1 "$stripped" "$AP_BACKEND_ORCA_IDLE_RE"
}

ap_backend_orca_send_key() {  # <terminal-id> <key>
  local terminal=$1 key=$2
  ap_backend_orca_tool_check || return 1
  case "$key" in
    C-c|ctrl+c|Ctrl-c|Ctrl-C)
      ap_backend_orca_run_json orca terminal send --terminal "$terminal" --interrupt --json
      ;;
    Enter|enter)
      ap_backend_orca_run_json orca terminal send --terminal "$terminal" --text "" --enter --json
      ;;
    *)
...
# ap_backend_orca_send_text_submit: type <text> once, then retry Enter until
...
ap_backend_orca_send_text_submit() {  # <terminal-id> <text> <retries> <enter-sleep> <settle>
  local terminal=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 i=0 state
  ap_backend_orca_tool_check || { printf 'send-failed'; return 0; }
  ap_backend_orca_send_literal "$terminal" "$text" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  while :; do
    ap_backend_orca_send_key "$terminal" Enter || true
    sleep "$sleep_s"
    state=$(ap_backend_orca_composer_state "$terminal")
...
ap_backend_orca_kill() {  # <terminal-id>
  ap_backend_orca_tool_check || return 0
