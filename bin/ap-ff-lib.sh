...
# Shared fast-forward machinery for autopilot self-sync.
# Usage: . bin/ap-ff-lib.sh   (after AP_ROOT and AP_HOME are set)
#
# This is the one implementation of "advance an autopilot checkout to a base by a
# clean fast-forward, never forcing, merging, or stashing" used by every sync
# path:
#   - /updateautopilot (bin/ap-update.sh) pulls from origin: base_mode "origin".
#   - the local-HEAD copilot sync (bin/ap-spawn.sh on launch, bin/ap-bootstrap.sh
...
#
# A linked-worktree copilot home already holds the primary's commit in the
...
# copilot's backlog, projects, or in-flight work.
# The seeded .ap-copilot-home identity marker is gitignored too; the local
...
COPILOT_HOME_MARKER="${COPILOT_HOME_MARKER:-.ap-copilot-home}"
...
# sync target every copilot follows. Reads the default branch *ref* rather than
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    dir="$abs_home/$name"
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      VALIDATION_ERROR="copilot $name directory must resolve inside the copilot home"
      return 1
    fi
    if [ -d "$dir" ]; then
      abs_dir=$(cd "$dir" && pwd -P) || {
        VALIDATION_ERROR="copilot $name directory cannot be resolved"
        return 1
      }
    elif [ -e "$dir" ]; then
      VALIDATION_ERROR="copilot $name path is not a directory"
      return 1
    else
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="copilot $name directory cannot be inside the active autopilot home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="copilot $name directory cannot be inside the autopilot repo"
      return 1
    fi
  done
}
...
validate_copilot_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$AP_HOME") || {
    VALIDATION_ERROR="active autopilot home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$AP_ROOT") || {
    VALIDATION_ERROR="autopilot repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
    VALIDATION_ERROR="copilot home cannot be the filesystem root"
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="copilot home cannot be the active autopilot home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="copilot home cannot be the autopilot repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="copilot home cannot be inside the active autopilot home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="copilot home cannot be inside the autopilot repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="copilot home cannot be an ancestor of the active autopilot home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="copilot home cannot be an ancestor of the autopilot repo"
    return 1
  fi
  validate_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ -L "$abs_home/$COPILOT_HOME_MARKER" ]; then
    VALIDATION_ERROR="copilot marker must not be a symlink"
    return 1
  fi
  if [ ! -f "$abs_home/$COPILOT_HOME_MARKER" ]; then
    VALIDATION_ERROR="not a seeded copilot home"
    return 1
  fi
  marker_id=$(cat "$abs_home/$COPILOT_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    VALIDATION_ERROR="marked for copilot ${marker_id:-unknown}, expected $id"
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not an autopilot home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not an autopilot home (missing bin/)"
    return 1
  fi
  VALIDATED_HOME="$abs_home"
}

# The self-update fetch source is intentionally narrower than ordinary project
# remotes: only the canonical Autopilot repository is accepted. Equivalent
# GitHub HTTPS and SSH transports are identity-equivalent; local paths, the old
# repository, forks, mirrors, and unrelated origins are refused before fetch.
autopilot_source_is_canonical() {
  local url=${1%/}
  url=${url%.git}
  case "$url" in
    https://github.com/hxutixnnn/autopilot|\
    git@github.com:hxutixnnn/autopilot|\
    ssh://git@github.com/hxutixnnn/autopilot) return 0 ;;
    *) return 1 ;;
  esac
}
...
dirty_status() {
  ...
  if [ "$ignore_seed_marker" = yes ]; then
    git -C "$dir" status --porcelain 2>/dev/null | awk -v marker="?? $COPILOT_HOME_MARKER" '$0 != marker { print; exit }'
  else
    git -C "$dir" status --porcelain 2>/dev/null | head -1
  fi
}

copilot_registry_field() {
...
# List this home's LIVE copilot direct reports from state/<id>.meta records.
# The meta file is the liveness signal; data/copilots.md is only the fallback
...
live_copilot_meta_records() {
  ...
  for meta in "$state"/*.meta; do
    ...
    if [ -z "$home" ] && [ -n "$registry" ]; then
      home=$(copilot_registry_field "$registry" "$id" home || true)
    fi
...
#   origin       - fetch origin and advance to origin/<default> (the /updateautopilot
...
#                  dependency (the local-HEAD copilot sync). The commit must
...
ff_target() {
  ...
  local default base cur instr local_rev base_rev before after out origin_url

  # Resolve and validate the actual fetch source before reading or fetching any
  # remote ref. Self-update never falls back to another remote.
  if [ "$base_mode" = origin ]; then
    ...
    if ! autopilot_source_is_canonical "$origin_url"; then
      echo "$label: skipped: origin is not canonical hxutixnnn/autopilot"
      return 0
    fi
  fi
...
# Validate and fast-forward one copilot home, accumulating its stable
# ap-<id> task selector into FF_NUDGE_WINDOWS when it should be live-converged.
# Args:
...
# autopilot repo itself (AP_ROOT) is never processed as its own copilot, and
# each resolved home is processed at most once.
process_copilot() {
  local id=$1 home=$2 window=${3:-} base_mode=$4 nudge_requires_instr=${5:-no} home_real ap_root_real
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  ap_root_real=$(resolve_path "$AP_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$ap_root_real" ] || return 0
  if ! validate_copilot_home "$id" "$home"; then
    echo "copilot $id: skipped: unsafe home: $VALIDATION_ERROR"
    return 0
  fi
  ...
  ff_target "$home_real" "copilot $id" "$base_mode" yes yes
  if [ "$FF_STATUS" = "updated" ] && [ -n "$window" ]; then
    if [ "$nudge_requires_instr" = yes ] && [ -z "$FF_INSTR" ]; then
      return 0
    fi
    FF_NUDGE_WINDOWS="$FF_NUDGE_WINDOWS ap-$id"
    if [ "$nudge_requires_instr" = yes ] && [ -n "$FF_INSTR" ] \
      && type ap_ff_after_instruction_update >/dev/null 2>&1; then
      ap_ff_after_instruction_update "$id" "$home_real" "$window" "$FF_INSTR"
    fi
  fi
}

# Sweep this home's LIVE copilot direct reports - state/<id>.meta files with
# kind=copilot - fast-forwarding each to base_mode. Passes base_mode and
# nudge_requires_instr through to process_copilot. Accumulates into
...
sweep_live_copilot_metas() {
  local state=$1 base_mode=$2 nudge_requires_instr=${3:-no} registry=${4:-$AP_HOME/data/copilots.md} id home window meta
  [ -d "$state" ] || return 0
  while IFS='|' read -r id home window meta; do
    process_copilot "$id" "$home" "$window" "$base_mode" "$nudge_requires_instr"
  done < <(live_copilot_meta_records "$state" "$registry")
}
