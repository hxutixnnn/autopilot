#!/usr/bin/env bash
# bin/ap-composer-lib.sh - the ONE fleet-wide owner of composer-content
# classification, shared by every session-provider adapter: the tmux path
# through bin/ap-tmux-lib.sh, and bin/backends/{herdr,orca,cmux}.sh directly.
#
# WHY THIS EXISTS (task ap-composer-shellglyph-safety): the four adapters each
...
# AGENT composer. The away-mode escalation injector (bin/ap-supervise-daemon.sh)
...
# consolidation). ap_composer_strip_ghost is the ONE ANSI-aware extractor of
...
# adapters (tmux via bin/ap-tmux-lib.sh, herdr via bin/backends/herdr.sh) cannot
...
# ap_composer_strip_ghost for the real-typed-content extraction, strips the box
# borders, trims, and hands the result plus a <bordered> flag to
# ap_composer_classify_content for the shared
...
# include guard (matching bin/ap-tmux-lib.sh).

# ap_composer_strip_ansi: drop every CSI escape sequence, leaving plain text.
...
# uses ap_composer_strip_ghost instead. Reads the styled text on stdin and prints
# plain text (stdin-only, matching ap_composer_strip_ghost). The character class
# includes ':' so an ITU colon-form SGR (38:2::r:g:b) is stripped whole, not left
# with a dangling tail.
ap_composer_strip_ansi() {
  local esc; esc=$(printf '\033')
  LC_ALL=C sed "s/${esc}\\[[0-9;:?]*[[:alpha:]]//g"
}

# ap_composer_strip_ghost: the ONE fleet-wide ANSI-aware extractor of "real typed
...
#     AP_COMPOSER_GHOST_LUMA_MAX (default 128): how grok renders its placeholder
...
#     dark-foreground run. This assumes a DARK terminal theme, the autopilot
...
ap_composer_strip_ghost() {
  LC_ALL=C awk -v lumamax="${AP_COMPOSER_GHOST_LUMA_MAX:-128}" '
...
# ap_composer_classify_content: the single shared composer-content verdict.
...
ap_composer_classify_content() {  # <bordered> <content> [idle_re] [idle_case] [plain_content]
  ...
  if ap_composer_idle_matches "$content" "$idle_re" "$idle_case"; then
    printf 'empty'; return 0
  fi
