...
# worktree, so it cannot discard unlanded work; set AP_FLEET_PRUNE=0 to disable it.
...
# fetch_with_packed_refs_lock_guard and the AP_FLEET_SYNC_PACKED_REFS_LOCK_* knobs.
# Usage: ap-fleet-sync.sh [<project-dir-or-name>]
# The single-project form accepts either a path (absolute, or relative to the
# caller's cwd) or a bare "<name>"/"projects/<name>" form, resolved against
# this home's projects dir ($AP_HOME/projects, or $AP_PROJECTS_OVERRIDE).
...
# `ap-fleet-sync.sh dotfiles-private` syncs just that one clone, same as
...
FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=${AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES:-3}
FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=${AP_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS:-1}
FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=${AP_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS:-30}
...
usage() {
  echo "usage: ap-fleet-sync.sh [<project-dir-or-name>]" >&2
}
...
# is provably stale per ap-lock-lib.sh (still present, mtime age past the
...
fetch_with_packed_refs_lock_guard() {
  ...
  if [ -n "$lock" ] && [ -e "$lock" ]; then
    if ap_lock_is_provably_stale "$lock" "$PROJ" "$FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS"; then
      if ! rm -f "$lock"; then
...
prune_gone_branches() {
  ...
  # net. Set AP_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${AP_FLEET_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  mode_line=$("$AP_ROOT/bin/ap-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
