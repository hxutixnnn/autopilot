#!/usr/bin/env bash
# ap-herdr-ci-cleanup.sh - bounded cleanup of CI-owned Herdr lab sessions.
#
# Snapshot the session list before the real-Herdr suite, then at job end only
# stop/delete sessions that:
#   1. match the guarded ap-lab-* name pattern,
...
#
# Usage:
#   ap-herdr-ci-cleanup.sh snapshot <path>
#   ap-herdr-ci-cleanup.sh teardown <snapshot-path>
set -eu

die() {
  printf 'ap-herdr-ci-cleanup.sh: %s\n' "$*" >&2
  exit 1
}
...
cmd=${1:-}
path=${2:-}
[ -n "$cmd" ] && [ -n "$path" ] || die "usage: ap-herdr-ci-cleanup.sh snapshot|teardown <path>"
...
is_lab_name() {
  local name=$1
  [[ "$name" =~ ^ap-lab-[a-zA-Z0-9][a-zA-Z0-9_-]*$ ]]
}
...
case "$cmd" in
  ...
  teardown)
    ...
    candidates=$(printf '%s' "$after_json" | jq -r --argjson before "$before" '
      ...
      | select(.name | test("^ap-lab-[a-zA-Z0-9][a-zA-Z0-9_-]*$"))
      | select((.name as $n | $before | index($n) | not))
      | .name
    ')
    failed=0
    if [ -z "$candidates" ]; then
      log "no job-owned ap-lab-* sessions to clean"
      exit 0
    fi
