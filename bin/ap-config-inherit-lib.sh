...
# Inheritance propagation: the PRIMARY autopilot pushes a declared, extensible
# set of LOCAL (gitignored) config items down into each copilot home's
# config/, so a copilot's OWN flight crew members inherit the primary's settings
# (e.g. primary config/flight-crew-dispatch.json makes a copilot use the same dispatch
# profile rules, primary config/flight-crew-harness=codex makes a copilot's flight crew members
...
# pilot-preference file, data/pilot-shared.md, into each copilot home's
# data/ as a read-only copy.
#
# Usage: . bin/ap-config-inherit-lib.sh   (no AP_* setup required)
#
# Why this is separate from the tracked-files fast-forward (ap-ff-lib.sh): config/
...
# copilot spawn (bin/ap-spawn.sh), the bootstrap copilot sweep
# (bin/ap-bootstrap.sh), and the focused mid-session config push
# (bin/ap-config-push.sh). It is PRIMARY-AUTHORITATIVE: the primary's value wins
...
# After successful config/* changes under an already-running copilot, callers
# invoke ap_config_send_reread_nudge so the live agent re-reads exact post-write
# bytes (spawn/respawn already re-reads at launch and needs no redundant nudge).
#
# Extensible by design: AP_INHERITABLE_CONFIG is the single declared list of
# config-dir-relative items the primary propagates. Add an item there and every
# convergence point inherits it - no other change needed. config/copilot-harness
# is deliberately NOT in the list: it is the primary's own setting for launching
# copilots, and a copilot never spawns copilots, so it must not flow
# downstream.
#
# shellcheck source=bin/ap-startup-memory-budget-lib.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/ap-startup-memory-budget-lib.sh"

# The one shared data file in this inheritance contract. There is deliberately
# no shared learnings file.
AP_SHARED_PILOT_FILE="pilot-shared.md"
AP_SHARED_PILOT_REL="data/$AP_SHARED_PILOT_FILE"
AP_SHARED_PILOT_MODE="444"

# The declared inheritable set (space-separated, config-dir-relative item paths).
# Extend here to inherit more of the primary's local config; override via the
# environment only in tests. Items must not contain whitespace.
AP_INHERITABLE_CONFIG="${AP_INHERITABLE_CONFIG:-flight-crew-dispatch.json flight-crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget}"

ap_inherit_file_mode() {
...
copy_inheritable_file() {
  ...
  tmp=$(mktemp "$dest_parent/.ap-inherit.XXXXXX" 2>/dev/null) || return 1
...
# copilot home's config dir (dest). SILENT on stdout - callers parse stdout,
...
# complete no-op (it leaves the copilot home exactly as it was - the
# backward-compatible path). When AP_CONFIG_INHERIT_REPORT points at a writable
...
record_inheritable_config_result() {
  ...
  [ -n "${AP_CONFIG_INHERIT_REPORT:-}" ] || return 0
  printf '%s\t%s\t%s\n' "$item" "$status" "$reason" >> "$AP_CONFIG_INHERIT_REPORT" 2>/dev/null || true
}
...
warn_inheritable_config_skip() {
  ...
  echo "ap-config-inherit: warning: skipped $item for $dest_config: $reason" >&2
}

warn_inheritable_config_error() {
  local item=$1 dest=$2 reason=$3
  echo "ap-config-inherit: error: $reason $item at $dest" >&2
}

shared_pilot_header_valid() {
  local src=$1 head
  head=$(sed -n '1,12p' "$src" 2>/dev/null) || return 1
  case "$head" in *main-authoritative*) ;; *) return 1 ;; esac
  case "$head" in *"read-only in copilot homes"*) ;; *) return 1 ;; esac
  case "$head" in *"must not be edited there"*) ;; *) return 1 ;; esac
  case "$head" in *"main autopilot"*) ;; *) return 1 ;; esac
  case "$head" in *"marked status"*|*"document pointer"*) ;; *) return 1 ;; esac
}

shared_pilot_dir_safe() {
  local dir=$1
  [ -n "$dir" ] || return 1
  if [ -e "$dir" ] || [ -L "$dir" ]; then
...
shared_pilot_file_safe_existing() {
  local path=$1
  [ -f "$path" ] && [ ! -L "$path" ] || return 1
  [ "$(ap_inherit_file_link_count "$path")" = 1 ]
}

restore_shared_pilot_readonly() {
  local dest=$1
  [ -e "$dest" ] || [ -L "$dest" ] || return 0
  shared_pilot_file_safe_existing "$dest" || return 1
  chmod "$AP_SHARED_PILOT_MODE" "$dest" 2>/dev/null || return 1
}

shared_pilot_quarantine_existing_for_hash() {
  local parent=$1 hash=$2 artifact artifact_hash
  for artifact in "$parent"/."$AP_SHARED_PILOT_FILE".quarantine.*."$hash" "$parent"/."$AP_SHARED_PILOT_FILE".quarantine.*."$hash".[0-9]*; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    shared_pilot_file_safe_existing "$artifact" || return 1
    artifact_hash=$(ap_inherit_sha256 "$artifact") || return 1
...
quarantine_shared_pilot_dest() {
  local dest=$1 dest_parent=$2 dest_hash artifact existing
  shared_pilot_file_safe_existing "$dest" || return 1
  dest_hash=$(ap_inherit_sha256 "$dest") || return 1
  if existing=$(shared_pilot_quarantine_existing_for_hash "$dest_parent" "$dest_hash" 2>/dev/null); then
    chmod u+w "$dest" 2>/dev/null || return 1
    if rm -f -- "$dest" 2>/dev/null; then
      printf '%s\n' "$existing"
      return 0
    fi
    restore_shared_pilot_readonly "$dest" || true
    return 1
  fi
  artifact=$(shared_pilot_quarantine_name "$dest_parent" "$dest_hash") || return 1
  chmod u+w "$dest" 2>/dev/null || return 1
...
copy_shared_pilot_file() {
  local src=$1 dest=$2 dest_parent tmp
  dest_parent=${dest%/*}
  shared_pilot_dir_safe "$dest_parent" || return 1
  tmp=$(mktemp "$dest_parent/.ap-pilot-shared.XXXXXX" 2>/dev/null) || return 1
  if ! cp "$src" "$tmp" 2>/dev/null; then
    rm -f "$tmp" 2>/dev/null || true
    return 1
  fi
  chmod 0600 "$tmp" 2>/dev/null || { rm -f "$tmp" 2>/dev/null || true; return 1; }
  shared_pilot_file_safe_existing "$tmp" || { rm -f "$tmp" 2>/dev/null || true; return 1; }
  if mv -f -- "$tmp" "$dest" 2>/dev/null; then
    chmod "$AP_SHARED_PILOT_MODE" "$dest" 2>/dev/null || return 1
    shared_pilot_file_safe_existing "$dest" || return 1
    return 0
...
propagate_shared_pilot_preferences() {
  local src_data=$1 dest_data=$2 src dest src_hash dest_hash dest_parent dest_home quarantine reason rc
  [ -n "$src_data" ] || return 1
  [ -n "$dest_data" ] || return 1
  src="$src_data/$AP_SHARED_PILOT_FILE"
  dest="$dest_data/$AP_SHARED_PILOT_FILE"
  dest_parent=${dest%/*}
  dest_home=${dest_data%/data}
  rc=0

  if [ -e "$src" ] || [ -L "$src" ]; then
    if ! shared_pilot_file_safe_existing "$src"; then
      reason="unsafe primary source"
      warn_inheritable_config_error "$AP_SHARED_PILOT_REL" "$src" "$reason"
      record_inheritable_config_result "$AP_SHARED_PILOT_REL" error "$reason"
      return 1
    fi
    if ! shared_pilot_header_valid "$src"; then
      reason="primary source header missing required main-authoritative warning"
      warn_inheritable_config_error "$AP_SHARED_PILOT_REL" "$src" "$reason"
      record_inheritable_config_result "$AP_SHARED_PILOT_REL" error "$reason"
      return 1
    fi
    src_hash=$(ap_inherit_sha256 "$src") || {
    ...
    if [ -e "$dest" ] || [ -L "$dest" ]; then
      if ! shared_pilot_file_safe_existing "$dest"; then
        reason="unsafe destination"
        warn_inheritable_config_error "$AP_SHARED_PILOT_REL" "$dest" "$reason"
        record_inheritable_config_result "$AP_SHARED_PILOT_REL" error "$reason"
        return 1
      fi
      dest_hash=$(ap_inherit_sha256 "$dest") || {
      ...
      if [ "$src_hash" = "$dest_hash" ]; then
        if restore_shared_pilot_readonly "$dest"; then
          record_inheritable_config_result "$AP_SHARED_PILOT_REL" unchanged ""
          return 0
        fi
      ...
      if ! shared_pilot_dir_safe "$dest_parent"; then
        reason="unsafe destination directory"
        warn_inheritable_config_error "$AP_SHARED_PILOT_REL" "$dest_parent" "$reason"
        record_inheritable_config_result "$AP_SHARED_PILOT_REL" error "$reason"
        restore_shared_pilot_readonly "$dest" || true
        return 1
      fi
      if ! quarantine=$(quarantine_shared_pilot_dest "$dest" "$dest_parent"); then
      ...
      printf 'COPILOT_SYNC: copilot home %s: quarantined %s drift at %s\n' "$dest_home" "$AP_SHARED_PILOT_REL" "$quarantine"
    elif ! shared_pilot_dir_safe "$dest_parent"; then
      reason="unsafe destination directory"
      warn_inheritable_config_error "$AP_SHARED_PILOT_REL" "$dest_parent" "$reason"
      record_inheritable_config_result "$AP_SHARED_PILOT_REL" error "$reason"
      return 1
    fi
    if copy_shared_pilot_file "$src" "$dest"; then
      if [ -n "${quarantine:-}" ]; then
        record_inheritable_config_result "$AP_SHARED_PILOT_REL" pushed "quarantined local drift at $quarantine"
      else
        record_inheritable_config_result "$AP_SHARED_PILOT_REL" pushed ""
      fi
    else
    ...
    if quarantine=$(quarantine_shared_pilot_dest "$dest" "$dest_parent"); then
      printf 'COPILOT_SYNC: copilot home %s: quarantined %s drift at %s\n' "$dest_home" "$AP_SHARED_PILOT_REL" "$quarantine"
      record_inheritable_config_result "$AP_SHARED_PILOT_REL" pushed "mirrored primary absence after quarantining local copy at $quarantine"
    else
...
propagate_copilot_inheritance() {
  ...
  propagate_shared_pilot_preferences "$src_data" "$dest_home/data" || rc=1
  return "$rc"
}
...
propagate_inheritable_config() {
  ...
  rc=0
  for item in $AP_INHERITABLE_CONFIG; do
    case "$item" in
      ''|/*|.|..|../*|*/../*|*/..) return 1 ;;
    esac
    src="$src_config/$item"
    ...
    if [ "$item" = "$AP_STARTUP_MEMORY_BUDGET_FILE" ]; then
      if [ -e "$src_config" ] || [ -L "$src_config" ]; then
        if ! ap_startup_memory_budget_config_dir_safe "$src_config"; then
          reason="unsafe primary config directory: $AP_STARTUP_MEMORY_BUDGET_ERROR"
      ...
      if [ -e "$dest_config" ] || [ -L "$dest_config" ]; then
        if ! ap_startup_memory_budget_config_dir_safe "$dest_config"; then
          reason="unsafe destination config directory: $AP_STARTUP_MEMORY_BUDGET_ERROR"
      ...
      if [ -e "$src" ] || [ -L "$src" ]; then
        if ! ap_startup_memory_budget_file_valid "$src"; then
          reason="unsafe or invalid primary source: $AP_STARTUP_MEMORY_BUDGET_ERROR"
      ...
      if [ -e "$dest" ] || [ -L "$dest" ]; then
        if ! ap_startup_memory_budget_file_valid "$dest"; then
          reason="unsafe or invalid destination: $AP_STARTUP_MEMORY_BUDGET_ERROR"
...
# config push so the live copilot can re-read exact post-write bytes.
# Kept under state/ (gitignored operational dir) so it never dirties the home.
AP_CONFIG_REREAD_INSTRUCTION_PREFIX_REL="state/.ap-inherited-config-reread"
AP_CONFIG_REREAD_MAX_SENT=16
AP_CONFIG_REREAD_RETRY_ROOT_REL="state/.ap-inherited-config-reread-retry"
AP_CONFIG_REREAD_MAX_PENDING=16
AP_CONFIG_REREAD_MAX_QUARANTINE=16
AP_CONFIG_INHERIT_LOCK_REL="state/.ap-inherited-config.lock"
...
AP_CONFIG_REREAD_FRAMING='These inherited config files changed. Re-read and apply their exact contents at every future intake. They are defaults/rules and do not remove your judgment to choose differently when warranted.'

# ap_config_reread_is_allowlisted_item <item>
# True only for the declared inheritable config allowlist (bare item name as
# recorded in AP_CONFIG_INHERIT_REPORT). data/pilot-shared.md is never
# allowlisted here and must never be inlined into a reread instruction.
ap_config_reread_is_allowlisted_item() {
  local item=$1 candidate
  for candidate in $AP_INHERITABLE_CONFIG; do
    [ "$candidate" = "$item" ] && return 0
  done
  return 1
}

# ap_config_reread_changed_items <report>
# Print bare allowlisted config item names whose report status is "pushed",
# in AP_INHERITABLE_CONFIG order (deterministic path order). Empty when none.
ap_config_reread_changed_items() {
...
ap_config_inherit_lock_path() {
  local dest_home=$1
  [ -n "$dest_home" ] || return 1
  printf '%s/%s\n' "$dest_home" "$AP_CONFIG_INHERIT_LOCK_REL"
}

ap_config_reread_retry_dir() {
  local source_home=$1 id=$2 token
  [ -n "$source_home" ] && [ -n "$id" ] || return 1
  token=${id//[^a-zA-Z0-9_.-]/_}
  [ -n "$token" ] || token=unknown
  printf '%s/%s/%s\n' "$source_home" "$AP_CONFIG_REREAD_RETRY_ROOT_REL" "$token"
}

ap_config_reread_pending_stages() {
  local source_home=$1 id=$2 retry_dir stage
  retry_dir=$(ap_config_reread_retry_dir "$source_home" "$id") || return 1
  for stage in "$retry_dir"/.ap-inherited-config-reread.*; do
    case "$stage" in
      *.report) continue ;;
    esac
...
ap_config_reread_pending_reports() {
  local source_home=$1 id=$2 retry_dir report
  retry_dir=$(ap_config_reread_retry_dir "$source_home" "$id") || return 1
  for report in "$retry_dir"/.ap-inherited-config-reread.*.report; do
    [ -f "$report" ] && [ ! -L "$report" ] || continue
    printf '%s\n' "$report"
  done | LC_ALL=C sort
}

ap_config_reread_has_staged() {
  local source_home=$1 id=$2 stage
  while IFS= read -r stage; do
    [ -n "$stage" ] && return 0
  done < <(ap_config_reread_pending_stages "$source_home" "$id")
  while IFS= read -r stage; do
    [ -n "$stage" ] && return 0
  done < <(ap_config_reread_pending_reports "$source_home" "$id")
  return 1
}

ap_config_reread_retry_queue_is_full() {
  local source_home=$1 id=$2 count report_count
  count=$(ap_config_reread_pending_stages "$source_home" "$id" | wc -l | tr -d ' ')
  report_count=$(ap_config_reread_pending_reports "$source_home" "$id" | wc -l | tr -d ' ')
  count=$((count + report_count))
  [ "$count" -ge "$AP_CONFIG_REREAD_MAX_PENDING" ]
}

ap_config_reread_retry_pending() {
  local id=$1 dest_home=$2 report retry_out rc
  report=$(mktemp "${TMPDIR:-/tmp}/ap-config-reread-retry.XXXXXX" 2>/dev/null) || {
    printf 'CONFIG_REREAD: copilot %s: send failed: could not create retry report\n' "$id"
    return 1
  }
  retry_out=$(ap_config_send_reread_nudge "$id" "$dest_home" "$report" 2>&1)
  rc=$?
  rm -f "$report"
...
ap_config_reread_new_retry_stage_path() {
  ...
  stage=$(umask 077; mktemp "$retry_dir/.ap-inherited-config-reread.$generation.XXXXXX" 2>/dev/null) || return 1
  printf '%s\n' "$stage"
}

ap_config_reread_save_retry_report() {
  local report=$1 stage_path=$2 report_path tmp parent
  parent=${stage_path%/*}
  report_path="$stage_path.report"
  tmp=$(umask 077; mktemp "$parent/.ap-config-reread-report.XXXXXX" 2>/dev/null) || return 1
...
# ap_config_write_reread_instruction <dest-home> <report> <instruction-path>
...
ap_config_write_reread_instruction() {
  ...
  while IFS= read -r item; do
    [ -n "$item" ] || continue
    ap_config_reread_is_allowlisted_item "$item" || continue
    rel="config/$item"
    dest="$dest_home/config/$item"
    if [ "$first" = 1 ]; then
      printf '%s\n' "$AP_CONFIG_REREAD_FRAMING" >> "$tmp" || { rm -f "$tmp"; return 1; }
      first=0
    fi
    {
      printf '\n'
  ...
  done < <(ap_config_reread_changed_items "$report")
  if [ "$first" = 1 ]; then
    rm -f "$tmp"
    return 1
  fi
  if ! mv -f "$tmp" "$instruction_path" 2>/dev/null; then
    AP_CONFIG_REREAD_FAILED_TEMP="$tmp"
    return 1
  fi
  return 0
}

ap_config_reread_adopt_exact_temp() {
...
ap_config_reread_pending_instructions() {
  local state=$1 pending instruction
  for pending in "$state"/.ap-inherited-config-reread.*.pending; do
...
ap_config_reread_has_pending() {
  local dest_home=$1 state pending
  state="$dest_home/${AP_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  for pending in "$state"/.ap-inherited-config-reread.*.pending; do
    [ -f "$pending" ] && [ ! -L "$pending" ] || continue
    return 0
  done
...
ap_config_reread_cleanup_sent() {
  ...
  sorted=$(printf '%s\n' "$paths" | LC_ALL=C sort)
  total=$(printf '%s\n' "$sorted" | wc -l | tr -d ' ')
  remove=$((total - AP_CONFIG_REREAD_MAX_SENT))
...
ap_config_reread_mark_pending() {
  local instruction_path=$1 pending_path=$2 parent tmp
  parent=${pending_path%/*}
  [ -n "$parent" ] && [ "$parent" != "$pending_path" ] || return 1
  mkdir -p "$parent" 2>/dev/null || return 1
  tmp=$(umask 077; mktemp "$parent/.ap-config-reread-pending.XXXXXX" 2>/dev/null) || return 1
...
ap_config_reread_publish_stage() {
  ...
  tmp=$(umask 077; mktemp "$state/.ap-config-reread-publish.XXXXXX" 2>/dev/null) || return 1
  if ! cat "$stage" > "$tmp" || ! chmod 0600 "$tmp" 2>/dev/null || ! mv -f "$tmp" "$final" 2>/dev/null; then
    rm -f "$tmp"
    return 1
  fi
  if ! ap_config_reread_mark_pending "$final" "$final.pending"; then
    rm -f "$final"
    return 1
  fi
  printf '%s\n' "$final"
...
ap_config_reread_send_failure() {
  local id=$1 instruction_path=$2 pending_path=$3 detail=$4
  if ! ap_config_reread_mark_pending "$instruction_path" "$pending_path"; then
    detail="$detail; could not record retry marker"
  fi
  printf 'CONFIG_REREAD: copilot %s: send failed: %s\n' "$id" "$detail"
  return 1
}

# ap_config_reread_send_pointer <id> <instruction-path>
ap_config_reread_send_pointer() {
  ...
  if [ ! -f "$instruction_path" ] || [ -L "$instruction_path" ]; then
    printf 'CONFIG_REREAD: copilot %s: send failed: pending instruction file is missing\n' "$id"
    return 1
  fi
  pending_pointer=$(cat "$pending_path" 2>/dev/null || true)
  if [ "$pending_pointer" != "$instruction_path" ]; then
    printf 'CONFIG_REREAD: copilot %s: send failed: pending instruction file is mismatched\n' "$id"
    return 1
  fi
  selector="ap-$id"
  send_bin="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/ap-send.sh"
  if [ ! -x "$send_bin" ]; then
    ap_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "ap-send.sh not executable at $send_bin"
    return 1
  fi
  if [ -z "${AP_HOME:-}" ]; then
    ap_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "AP_HOME is not set"
    return 1
  fi
  message="CONFIG_REREAD: $instruction_path"
  out=$(AP_HOME="$AP_HOME" \
    AP_ROOT_OVERRIDE="${AP_ROOT_OVERRIDE:-}" \
    AP_STATE_OVERRIDE="${AP_STATE_OVERRIDE:-}" \
    AP_SEND_SETTLE="${AP_SEND_SETTLE:-0}" \
  ...
  out=${out%%$'\n'*}
  [ -n "$out" ] || out="ap-send exited $rc"
  ap_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "$out"
  return 1
}

# ap_config_reread_discard_pending <dest-home>
ap_config_reread_discard_pending() {
  ...
  if [ -n "$id" ] && [ -n "$source_home" ]; then
    retry_dir=$(ap_config_reread_retry_dir "$source_home" "$id") || rc=1
    if [ -d "$retry_dir" ]; then
      for retry_stage in "$retry_dir"/.ap-inherited-config-reread.*; do
...
ap_config_reread_quarantine_prune() {
  local root=$1 keep=${2:-$AP_CONFIG_REREAD_MAX_QUARANTINE} dirs dir oldest path remove
  case "$keep" in
    ''|*[!0-9]*) keep=$AP_CONFIG_REREAD_MAX_QUARANTINE ;;
  esac
  [ -d "$root" ] || return 0
  dirs=""
...
ap_config_reread_quarantine_dir() {
  local home=$1 state root quarantine
  state="$home/${AP_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  root="$state/.ap-inherited-config-reread-quarantine"
  mkdir -p "$root" 2>/dev/null || return 1
  chmod 0700 "$root" 2>/dev/null || return 1
  ap_config_reread_quarantine_prune "$root" $((AP_CONFIG_REREAD_MAX_QUARANTINE - 1)) || return 1
  quarantine=$(umask 077; mktemp -d "$root/generation.XXXXXX" 2>/dev/null) || return 1
  chmod 0700 "$quarantine" 2>/dev/null || return 1
  printf '%s\n' "$quarantine"
}

ap_config_reread_quarantine_pending() {
  ...
  if [ "$dest_has_artifacts" -eq 1 ]; then
    dest_quarantine=$(ap_config_reread_quarantine_dir "$dest_home" 2>/dev/null || true)
  fi
  for pending in "$state"/.ap-inherited-config-reread.*.pending; do
    [ -f "$pending" ] && [ ! -L "$pending" ] || continue
    instruction=${pending%.pending}
  ...
  if [ -n "$id" ] && [ -n "$source_home" ]; then
    retry_dir=$(ap_config_reread_retry_dir "$source_home" "$id") || retry_dir=
    ...
    if [ "$source_has_artifacts" -eq 1 ]; then
      source_quarantine=$(ap_config_reread_quarantine_dir "$source_home" 2>/dev/null || true)
    fi
    if [ -d "$retry_dir" ]; then
      for retry_stage in "$retry_dir"/.ap-inherited-config-reread.*; do
        [ -f "$retry_stage" ] && [ ! -L "$retry_stage" ] || continue
...
# ap_config_send_reread_nudge <id> <dest-home> <report>
...
# single-line pointers to those files through the routed copilot path
# (ap-send). The files contain only changed config paths, clear delimiters, and
# the destination's full exact post-write bytes (or ABSENT) - never summaries,
# SHA values, selected profiles, or data/pilot-shared.md. No-op (return 0) when
...
ap_config_send_reread_nudge() {
  ...
  dest_home_abs=$(cd "$dest_home" 2>/dev/null && pwd -P) || {
    printf 'CONFIG_REREAD: copilot %s: send failed: destination home is not readable\n' "$id"
    return 1
  }
  state="$dest_home_abs/${AP_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  changed_items=$(ap_config_reread_changed_items "$report")
  pending_paths=""
  stage_paths=""
  retry_report_paths=""
  if [ "${AP_CONFIG_REREAD_SKIP_PENDING:-0}" != 1 ]; then
    pending_paths=$(ap_config_reread_pending_instructions "$state")
    source_home_abs=$(cd "${AP_HOME:-}" 2>/dev/null && pwd -P || true)
    if [ -n "$source_home_abs" ]; then
      stage_paths=$(ap_config_reread_pending_stages "$source_home_abs" "$id")
      retry_report_paths=$(ap_config_reread_pending_reports "$source_home_abs" "$id")
    fi
  fi
  send_failures=0
  while IFS= read -r retry_report_path; do
    ...
    if ap_config_write_reread_instruction "$dest_home_abs" "$retry_report_path" "$retry_stage_path"; then
      rm -f "$retry_report_path" 2>/dev/null || send_failures=1
      ...
      exact_tmp=${AP_CONFIG_REREAD_FAILED_TEMP:-}
      if [ -n "$exact_tmp" ] \
        && ap_config_reread_adopt_exact_temp "$exact_tmp" "$retry_stage_path"; then
        rm -f "$retry_report_path" 2>/dev/null || send_failures=1
        if [ -n "$stage_paths" ]; then
          stage_paths+=$'\n'
        fi
        ...
        printf 'CONFIG_REREAD: copilot %s: send failed: retained exact retry temporary %s\n' "$id" "$exact_tmp"
        send_failures=1
        break
      else
        printf 'CONFIG_REREAD: copilot %s: send failed: could not rebuild retry instruction\n' "$id"
        send_failures=1
        break
      fi
    fi
...
EOF
  if [ "$send_failures" -ne 0 ]; then
    ap_config_reread_cleanup_sent "$dest_home_abs"
    return 1
  fi
  if [ -n "$changed_items" ]; then
    source_home_abs=$(cd "${AP_HOME:-}" 2>/dev/null && pwd -P || true)
    if [ -z "$source_home_abs" ]; then
      printf 'CONFIG_REREAD: copilot %s: send failed: could not reserve retry instruction\n' "$id"
      return 1
    fi
    if ap_config_reread_retry_queue_is_full "$source_home_abs" "$id"; then
      printf 'CONFIG_REREAD: copilot %s: send failed: retry instruction queue is full\n' "$id"
      return 1
    fi
    current_stage_path=$(ap_config_reread_new_retry_stage_path "$source_home_abs" "$id") || {
      printf 'CONFIG_REREAD: copilot %s: send failed: could not reserve retry instruction\n' "$id"
      return 1
    }
    if ! ap_config_write_reread_instruction "$dest_home_abs" "$report" "$current_stage_path"; then
      exact_tmp=${AP_CONFIG_REREAD_FAILED_TEMP:-}
      if [ -n "$exact_tmp" ] \
        && ap_config_reread_adopt_exact_temp "$exact_tmp" "$current_stage_path"; then
        printf 'CONFIG_REREAD: copilot %s: send failed: could not publish retry instruction; retained exact retry generation %s\n' "$id" "$current_stage_path"
      elif [ -n "$exact_tmp" ] && [ -f "$exact_tmp" ]; then
        rm -f "$current_stage_path" 2>/dev/null || true
        printf 'CONFIG_REREAD: copilot %s: send failed: retained exact retry temporary %s\n' "$id" "$exact_tmp"
      elif retry_record_path=$(ap_config_reread_save_retry_report "$report" "$current_stage_path"); then
        rm -f "$current_stage_path" 2>/dev/null || true
        printf 'CONFIG_REREAD: copilot %s: send failed: could not write retry instruction; retained retry report %s\n' "$id" "$retry_record_path"
      else
        rm -f "$current_stage_path" 2>/dev/null || true
        printf 'CONFIG_REREAD: copilot %s: send failed: could not write retry instruction or retain retry report\n' "$id"
      fi
      return 1
    fi
    if [ -n "$stage_paths" ]; then
  ...
  while IFS= read -r stage_path; do
    ...
    if instruction_path=$(ap_config_reread_publish_stage "$dest_home_abs" "$stage_path"); then
      ...
      printf 'CONFIG_REREAD: copilot %s: send failed: could not publish retry instruction\n' "$id"
      send_failures=1
      break
    fi
  done <<EOF
...
EOF
  ...
  while IFS= read -r instruction_path; do
    ...
    if ap_config_reread_send_pointer "$id" "$instruction_path"; then
      while IFS= read -r stage_path; do
        [ -n "$stage_path" ] || continue
