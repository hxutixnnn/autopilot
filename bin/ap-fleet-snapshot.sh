#!/usr/bin/env bash
# ap-fleet-snapshot.sh - read-only structured fleet snapshot.
#
# Output contract: `--json` prints one object with schema
# `ap-fleet-snapshot.v1`.
...
#   ap_home: resolved operational home.
...
#     Structured rows preserve pilot-hold metadata such as hold_kind and
...
#     pilot_actionable fields. Repeated blocker tokens remain ordered; a blocker
#     resolves only when its structured record is Done, and missing ids stay open.
#   tasks[]: one row per state/<id>.meta, sorted by id.
#     current_state is parsed from bin/ap-flight-crew-state.sh <id> and preserves
#     state, source, detail, and raw line separately.
#     paths.status_log.last_event is historical wake-event data only, never
#     current state.
#     hints.open_decisions is the keyed open-decision set returned by
#     ap-classify-lib.sh's authoritative status_open_decisions fold and reconciled
#     against current_state; hints.pending_decision and hints.blocked_event are
#     booleans derived from that set.
#     endpoint.exists is the cheap backend endpoint-presence read.
#     endpoint.agent_alive is populated for copilots only, where it is useful
#     return-channel supervision data; other tasks use "not_checked".
#   recon_reports[]: present data/<id>/report.md pointers.
#   main_inventory: {valid,reason,orphan_in_flight[],unstructured_current_count} -
#     main-home current-inventory checks shared with copilot_home_summary_json
...
#   copilot_current: {records[],total,shown,truncated} - bounded current summaries
#     for registered copilots, selected from validated structured state inside
...
#     queued, landed, endpoints, counts, and omitted. Actionable pilot holds
#     appear in decisions_open; blocked pilot holds remain queued with metadata.
#   copilot_landed: {records[],truncated[],unreadable[],partial[]} - the
#     compatibility landed-work roll-up derived from copilot_current. Readable
...
#   copilot_guidance: return-channel action note for renderers and bearings.
#
...
SNAPSHOT_NOW=${AP_SNAPSHOT_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ -n "${AP_SNAPSHOT_NOW_EPOCH:-}" ]; then
  SNAPSHOT_EPOCH=$AP_SNAPSHOT_NOW_EPOCH
else
...
AP_SNAPSHOT_COPILOTS=${AP_SNAPSHOT_COPILOTS:-20}
AP_SNAPSHOT_COPILOT_TIMEOUT=${AP_SNAPSHOT_COPILOT_TIMEOUT:-8}
AP_SNAPSHOT_COPILOT_MAX_BYTES=${AP_SNAPSHOT_COPILOT_MAX_BYTES:-262144}
AP_SNAPSHOT_COPILOT_CHILDREN=${AP_SNAPSHOT_COPILOT_CHILDREN:-20}
AP_SNAPSHOT_COPILOT_QUEUED=${AP_SNAPSHOT_COPILOT_QUEUED:-20}
AP_SNAPSHOT_COPILOT_DECISIONS=${AP_SNAPSHOT_COPILOT_DECISIONS:-20}
AP_SNAPSHOT_TERMINAL_LINES=${AP_SNAPSHOT_TERMINAL_LINES:-8}
AP_SNAPSHOT_TERMINAL_BYTES=${AP_SNAPSHOT_TERMINAL_BYTES:-4096}
AP_SNAPSHOT_TERMINAL_TIMEOUT=${AP_SNAPSHOT_TERMINAL_TIMEOUT:-2}
AP_SNAPSHOT_PARENT_ACTIVITY_LINES=${AP_SNAPSHOT_PARENT_ACTIVITY_LINES:-256}
AP_SNAPSHOT_PARENT_ACTIVITY_BYTES=${AP_SNAPSHOT_PARENT_ACTIVITY_BYTES:-65536}
AP_SNAPSHOT_PARENT_ACTIVITIES=${AP_SNAPSHOT_PARENT_ACTIVITIES:-20}
AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT=${AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT:-2}
AP_SNAPSHOT_REGISTRY_LINES=${AP_SNAPSHOT_REGISTRY_LINES:-256}
AP_SNAPSHOT_REGISTRY_BYTES=${AP_SNAPSHOT_REGISTRY_BYTES:-65536}
AP_SNAPSHOT_REGISTRY_RECORDS=${AP_SNAPSHOT_REGISTRY_RECORDS:-40}
AP_SNAPSHOT_REGISTRY_TIMEOUT=${AP_SNAPSHOT_REGISTRY_TIMEOUT:-2}
validate_positive_bound() {  # <name> <value>
  case "$2" in
    ''|*[!0-9]*|0)
      printf 'ap-fleet-snapshot: %s must be a positive integer\n' "$1" >&2
      exit 2
      ;;
  esac
}
case "$AP_SNAPSHOT_COPILOTS" in
  ''|*[!0-9]*)
    echo "ap-fleet-snapshot: AP_SNAPSHOT_COPILOTS must be a non-negative integer" >&2
    exit 2
    ;;
esac
validate_positive_bound AP_SNAPSHOT_COPILOT_TIMEOUT "$AP_SNAPSHOT_COPILOT_TIMEOUT"
validate_positive_bound AP_SNAPSHOT_COPILOT_MAX_BYTES "$AP_SNAPSHOT_COPILOT_MAX_BYTES"
validate_positive_bound AP_SNAPSHOT_COPILOT_CHILDREN "$AP_SNAPSHOT_COPILOT_CHILDREN"
validate_positive_bound AP_SNAPSHOT_COPILOT_QUEUED "$AP_SNAPSHOT_COPILOT_QUEUED"
validate_positive_bound AP_SNAPSHOT_COPILOT_DECISIONS "$AP_SNAPSHOT_COPILOT_DECISIONS"
validate_positive_bound AP_SNAPSHOT_TERMINAL_LINES "$AP_SNAPSHOT_TERMINAL_LINES"
validate_positive_bound AP_SNAPSHOT_TERMINAL_BYTES "$AP_SNAPSHOT_TERMINAL_BYTES"
validate_positive_bound AP_SNAPSHOT_TERMINAL_TIMEOUT "$AP_SNAPSHOT_TERMINAL_TIMEOUT"
validate_positive_bound AP_SNAPSHOT_PARENT_ACTIVITY_LINES "$AP_SNAPSHOT_PARENT_ACTIVITY_LINES"
validate_positive_bound AP_SNAPSHOT_PARENT_ACTIVITY_BYTES "$AP_SNAPSHOT_PARENT_ACTIVITY_BYTES"
validate_positive_bound AP_SNAPSHOT_PARENT_ACTIVITIES "$AP_SNAPSHOT_PARENT_ACTIVITIES"
validate_positive_bound AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT "$AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT"
validate_positive_bound AP_SNAPSHOT_REGISTRY_LINES "$AP_SNAPSHOT_REGISTRY_LINES"
validate_positive_bound AP_SNAPSHOT_REGISTRY_BYTES "$AP_SNAPSHOT_REGISTRY_BYTES"
validate_positive_bound AP_SNAPSHOT_REGISTRY_RECORDS "$AP_SNAPSHOT_REGISTRY_RECORDS"
validate_positive_bound AP_SNAPSHOT_REGISTRY_TIMEOUT "$AP_SNAPSHOT_REGISTRY_TIMEOUT"
...
# shellcheck disable=SC1091
. "$SCRIPT_DIR/ap-classify-lib.sh"
# shellcheck source=bin/ap-ff-lib.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/ap-ff-lib.sh"  # validate_copilot_home: shared seeded-home boundary checks

usage() {
  cat <<'EOF'
usage: ap-fleet-snapshot.sh --json
       ap-fleet-snapshot.sh --copilot-home-summary

Print a read-only structured snapshot of the autopilot fleet.
JSON is the stable machine-readable output contract.

--copilot-home-summary emits the bounded structured summary used after a
validated registered-home handoff. It is local-only, skips nested copilot
...
Actionable tasks-axi pilot holds appear as decisions_open and stay visible in
queued with hold_reason, hold_kind, and plural blocker fields for downstream
projections. A pilot hold is actionable only when every blocker is Done.
Cross-home reads use AP_SNAPSHOT_COPILOTS (default 20, 0 lifts the count
bound), AP_SNAPSHOT_COPILOT_TIMEOUT, and AP_SNAPSHOT_COPILOT_MAX_BYTES.
Terminal contradiction evidence uses
AP_SNAPSHOT_TERMINAL_LINES, AP_SNAPSHOT_TERMINAL_BYTES, and
AP_SNAPSHOT_TERMINAL_TIMEOUT and never becomes canonical current state.
Parent activity evidence uses AP_SNAPSHOT_PARENT_ACTIVITY_LINES,
AP_SNAPSHOT_PARENT_ACTIVITY_BYTES, AP_SNAPSHOT_PARENT_ACTIVITIES, and
AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT, with truncation disclosed in the result.
The registered copilot table uses AP_SNAPSHOT_REGISTRY_LINES,
AP_SNAPSHOT_REGISTRY_BYTES, AP_SNAPSHOT_REGISTRY_RECORDS, and
AP_SNAPSHOT_REGISTRY_TIMEOUT, with unavailability and truncation disclosed.
EOF
}
...
case "${1:---json}" in
  --json) ;;
  --copilot-home-summary) OUTPUT_MODE=copilot-home-summary ;;
  -h|--help) usage; exit 0 ;;
  *) usage >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "ap-fleet-snapshot: jq not found" >&2; exit 1; }
...
flight_crew_state_json() {  # <id>
  local id=$1 raw rest state source detail sep
  raw=$(
    AP_ROOT_OVERRIDE="$AP_ROOT" \
      AP_HOME="$AP_HOME" \
      AP_STATE_OVERRIDE="$STATE" \
      AP_DATA_OVERRIDE="$DATA" \
      AP_PROJECTS_OVERRIDE="$PROJECTS" \
      AP_CONFIG_OVERRIDE="$CONFIG" \
      "$SCRIPT_DIR/ap-flight-crew-state.sh" "$id" 2>/dev/null || true
  )
  raw=$(printf '%s\n' "$raw" | head -1)
  sep=' · '
  state=unknown
  ...
  jq -Rn --arg path "$backlog" '
    ...
    | .records |= map(
        if .structured then
          ...
          | .pilot_actionable =
              (.state == "queued" and .kind == "pilot" and .hold_kind == "external"
...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    current_json=$(flight_crew_state_json "$id")
    ...
    # (ap-classify-lib.sh's status_open_decisions) so a later unrelated event can
    # never mask a still-open pilot decision. The set is derived purely from the
    # keyed fold - never from report bodies or decision-like prose - and then
    # reconciled against the flight crew LIFECYCLE, which only clears a stale decision the
    # flight crew has provably moved past. Two lifecycle signals clear it, neither of which
    # reads any report content:
    #   - a live activity read (run-step or busy pane) that is working/done, so a
    #     flight crew that resumed past a gate is not still reported as parked; and
    #   - a TERMINAL done/failed state on a single-owner task (recon or flight), whose
    #     deliverable is its report or PR, so a COMPLETED recon surfaces only as a
    #     report POINTER, never as a reopened pending decision.
    # Copilots are excluded from lifecycle clearing: they are persistent and
    ...
    if [ -n "$target" ]; then
      if ap_backend_target_exists "$backend" "$target" "ap-$id" >/dev/null 2>&1; then
    ...
    if [ "$kind" = copilot ] && [ -n "$target" ]; then
      agent_alive=$(ap_backend_agent_alive "$backend" "$target" 2>/dev/null || printf unknown)
    fi
    ...
    jq -n \
      ...
      '{
        ...
        copilot_projects:($projects | if . == "" then [] else split(",") | map(gsub("^[[:space:]]+|[[:space:]]+$"; "")) | map(select(. != "")) end),
        ...
        hints:{
          ...
          recon_report_present:$report_present,
          last_event_text:$last_event_raw
        },
        actions:(
          if $kind == "copilot" then
            {send:"bin/ap-send.sh ap-\($id) \u0027<request>\u0027",
             watch:"read status/doc return channel; do not routinely ap-peek a copilot for answers",
             return_channel_note:"Copilot answers come back through status/doc paths after a marked ap-send request."}
          else
            {watch:"bin/ap-peek.sh ap-\($id)",
             steer:"bin/ap-send.sh ap-\($id) \u0027<instruction>\u0027",
...
# used by copilot_home_summary_json, without inventing live task rows.
...
# nested copilots.
copilot_home_summary_json() {  # <backlog-json> <tasks-json>
  jq -n \
    --arg generated "$SNAPSHOT_NOW" \
    --arg home "$AP_HOME" \
    --argjson child_n "$AP_SNAPSHOT_COPILOT_CHILDREN" \
    --argjson queued_n "$AP_SNAPSHOT_COPILOT_QUEUED" \
    --argjson decisions_n "$AP_SNAPSHOT_COPILOT_DECISIONS" \
    --argjson landed_n "$AP_SNAPSHOT_COPILOT_LANDED_PER_HOME" \
    --argjson backlog "$1" \
    --argjson tasks "$2" '
    def trunc($n):
      tostring | gsub("\\s+"; " ")
    ...
    | ([ $queued_all[]
         | select(.pilot_actionable == true)
         | {id,key:.id,verb:"pilot-hold",summary:(.title | trunc(160)),
            reason:(.hold_reason | trunc(160)),source:"backlog"} ]) as $pilot_holds_all
    | ([ $backlog.records[]? | select(.state == "done" and .structured and .kind != "pilot")
    ...
    | (if $valid | not then "unknown"
       elif any($decisions_all[]; .verb == "needs-decision" or .verb == "pilot-hold") then "pilot_decision"
    ...
    | {
        schema:"ap-copilot-home-summary.v1",
        generated:$generated,
        home:$home,
        valid:$valid,
        reason:$reason,
        ...
        queued:([$queued_all[] | {id:(.id | trunc(120)),title:(.title | trunc(120)),
          ...
          pilot_actionable:(.pilot_actionable // false),
...
# Current registered-copilot aggregation.
...
AP_SNAPSHOT_COPILOT_LANDED_PER_HOME=${AP_SNAPSHOT_COPILOT_LANDED_PER_HOME:-10}
case "$AP_SNAPSHOT_COPILOT_LANDED_PER_HOME" in ''|*[!0-9]*) AP_SNAPSHOT_COPILOT_LANDED_PER_HOME=10 ;; esac
...
registry_copilots_json() {
  local reg="$DATA/copilots.md" out rc reason mode script parse_filter output_filter
  if [ ! -f "$reg" ]; then
  ...
  if [ -z "$mode" ] || [ $((8#$mode & 0444)) -eq 0 ]; then
    jq -n --arg path "$reg" --arg observed "$SNAPSHOT_NOW" \
      --arg reason "registered copilot table is unreadable" \
...
BASH
  ...
  parse_filter=$(cat <<'JQ'
      ...
      | group_by(.id)
      | map(if length > 1 then .[0] + {registry_error:"duplicate copilot id in registry"} else .[0] end)
JQ
  )
...
JQ
  ...
  out=$(run_timed "$AP_SNAPSHOT_REGISTRY_TIMEOUT" bash -c "$script" \
    ap-copilot-registry "$reg" "$AP_SNAPSHOT_REGISTRY_LINES" \
    "$AP_SNAPSHOT_REGISTRY_BYTES" "$AP_SNAPSHOT_REGISTRY_RECORDS" "$reg" "$SNAPSHOT_NOW" \
  ...
  [ "$rc" -eq 124 ] && reason="registered copilot table read timed out" \
    || reason="registered copilot table is unreadable"
...
BASH
  ...
  out=$(run_timed "$AP_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT" bash -c "$script" \
    ap-parent-activities "$SCRIPT_DIR/ap-classify-lib.sh" "$f" \
    "$AP_SNAPSHOT_PARENT_ACTIVITY_LINES" "$AP_SNAPSHOT_PARENT_ACTIVITY_BYTES" \
    "$AP_SNAPSHOT_PARENT_ACTIVITIES" "$SNAPSHOT_STAT_STYLE" 2>/dev/null)
  rc=$?
  if [ "$rc" -eq 0 ] && printf '%s' "$out" | jq -e '
...
terminal_evidence_json() {  # <parent-task-json> <event-note> <evidence-contradicts>
  ...
  expected=$(printf '%s' "$task" | jq -r '"ap-" + (.id // "")')
  ...
  out=$(run_timed "$AP_SNAPSHOT_TERMINAL_TIMEOUT" bash -c \
    '. "$1"; ap_backend_capture "$2" "$3" "$4" "$5" | LC_ALL=C head -c "$6"; rc=${PIPESTATUS[0]}; [ "$rc" -eq 141 ] && rc=0; exit "$rc"' \
    ap-terminal-capture "$SCRIPT_DIR/ap-backend.sh" "$backend" "$target" "$AP_SNAPSHOT_TERMINAL_LINES" "$expected" "$AP_SNAPSHOT_TERMINAL_BYTES" 2>/dev/null)
  rc=$?
  if [ "$rc" -ne 0 ]; then
  ...
  clean=$(printf '%s' "$out" | tail -n "$AP_SNAPSHOT_TERMINAL_LINES" | LC_ALL=C head -c "$AP_SNAPSHOT_TERMINAL_BYTES")
...
copilot_current_json() {  # <parent-tasks-json>
  ...
  registry=$(registry_copilots_json) || return 1
  union=$(jq -n --argjson registry "$registry" --argjson tasks "$tasks" '
    ...
    | ([ $registered[] as $r
       ...
       + [ $tasks[] | select(.kind == "copilot") as $t
           ...
           | {id:$t.id,home:($t.paths.home.path // null),
              ...
              registry_error:(if $registry.complete == true
                              then "copilot metadata is not registered"
                              else "copilot registration is unknown because the registry read is incomplete or unavailable" end),
              parent_task:$t} ])
    | sort_by(.id)
  ...
  rows=$(printf '%s' "$union" | jq -c --argjson cap "$AP_SNAPSHOT_COPILOTS" '(if $cap == 0 then .records else .records[:$cap] end)[]')
  shown=$(printf '%s\n' "$rows" | grep -c . || true)
  ...
  while IFS= read -r row; do
    ...
    summary='{}'
    summary_valid=false
    if [ -z "$reason" ] && [ -z "$home" ]; then reason="no recorded copilot home"; fi
    if [ -z "$reason" ]; then
      case "$home" in
        /*) : ;;
    ...
    if [ -z "$reason" ]; then
      if ! validate_copilot_home "$id" "$home" 2>/dev/null; then
    ...
    if [ -z "$reason" ]; then
      summary=$(run_timed "$AP_SNAPSHOT_COPILOT_TIMEOUT" env \
        AP_ROOT_OVERRIDE="$AP_ROOT" \
        AP_HOME="$home" \
        AP_STATE_OVERRIDE="$home/state" \
        AP_DATA_OVERRIDE="$home/data" \
        AP_CONFIG_OVERRIDE="$home/config" \
        AP_PROJECTS_OVERRIDE="$home/projects" \
        AP_SNAPSHOT_NOW="$SNAPSHOT_NOW" \
        AP_SNAPSHOT_NOW_EPOCH="$SNAPSHOT_EPOCH" \
        AP_SNAPSHOT_COPILOT_CHILDREN="$AP_SNAPSHOT_COPILOT_CHILDREN" \
        AP_SNAPSHOT_COPILOT_QUEUED="$AP_SNAPSHOT_COPILOT_QUEUED" \
        AP_SNAPSHOT_COPILOT_DECISIONS="$AP_SNAPSHOT_COPILOT_DECISIONS" \
        AP_SNAPSHOT_COPILOT_LANDED_PER_HOME="$AP_SNAPSHOT_COPILOT_LANDED_PER_HOME" \
        "$SCRIPT_DIR/ap-fleet-snapshot.sh" --copilot-home-summary 2>/dev/null)
      summary_rc=$?
      if [ "$summary_rc" -ne 0 ]; then
        ...
        if [ "$summary_bytes" -gt "$AP_SNAPSHOT_COPILOT_MAX_BYTES" ]; then
          reason="structured home snapshot exceeded byte limit"
        elif ! printf '%s' "$summary" | jq -e --arg home "$home" --arg generated "$SNAPSHOT_NOW" '
          .schema == "ap-copilot-home-summary.v1" and .home == $home and .generated == $generated
...
copilot_landed_from_current_json() {  # <copilot-current-json>
  jq -n --argjson current "$1" '
    {records:[ $current.records[]
      | select(.provenance.selected == "structured-home") as $copilot
      | $copilot.landed[]
      | . + {home:$copilot.home,home_id:$copilot.id}],
...
BACKLOG_JSON=$(backlog_json) || { echo "ap-fleet-snapshot: backlog read failed" >&2; exit 1; }
TASKS_JSON=$(task_json_lines) || { echo "ap-fleet-snapshot: task snapshot failed" >&2; exit 1; }

if [ "$OUTPUT_MODE" = copilot-home-summary ]; then
  copilot_home_summary_json "$BACKLOG_JSON" "$TASKS_JSON" \
    || { echo "ap-fleet-snapshot: copilot home summary failed" >&2; exit 1; }
  exit 0
fi

RECON_REPORTS_JSON=$(recon_report_lines)
MAIN_INVENTORY_JSON=$(main_inventory_json "$BACKLOG_JSON" "$TASKS_JSON") \
  || { echo "ap-fleet-snapshot: main inventory summary failed" >&2; exit 1; }
COPILOT_CURRENT_JSON=$(copilot_current_json "$TASKS_JSON") \
  || { echo "ap-fleet-snapshot: registered copilot aggregation failed" >&2; exit 1; }
COPILOT_LANDED_JSON=$(copilot_landed_from_current_json "$COPILOT_CURRENT_JSON") \
  || { echo "ap-fleet-snapshot: copilot landed projection failed" >&2; exit 1; }
...
jq -n \
  ...
  --argjson recon_reports "$RECON_REPORTS_JSON" \
  --argjson copilot_current "$COPILOT_CURRENT_JSON" \
  --argjson copilot_landed "$COPILOT_LANDED_JSON" \
  'def backlog_by_id($id): ($backlog.records[]? | select(.structured == true and .id == $id) | .) // null;
   ...
   def report_kind($id): (task_by_id($id).kind // backlog_by_id($id).kind // "recon");
   {
     ...
     generated:$generated,
     ap_home:$ap_home,
     roots:{ap_root:$ap_root,state:$state,data:$data,config:$config,projects:$projects},
     backlog:$backlog,
     tasks:($tasks | map(. + {backlog:backlog_by_id(.id)})),
     main_inventory:$main_inventory,
     recon_reports:($recon_reports | map(. + {kind:report_kind(.id)})),
     copilot_current:$copilot_current,
     copilot_landed:$copilot_landed,
     copilot_guidance:{
       note:"For kind=copilot, bearings selects validated structured state from that registered home; parent events and bounded terminal evidence are fallback-only supplements and never current-state authority."
     }
   }'
