#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the autopilot watcher, with honest verification.
#
# The watcher (bin/ap-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so autopilot must run this script as the harness's
...
# (bin/ap-claude-stop-autoarm.sh), where the harness owns the process group and
...
# On a harness with a PreToolUse-equivalent hook, bin/ap-arm-pretool-check.sh
...
# liveness beacon (state/.last-watcher-beat) is fresh within AP_GUARD_GRACE (the
# single source of truth, shared with ap-watch.sh and ap-guard.sh), and prints
...
#
# --restart: stop ONLY this AP_HOME's watcher (the pid recorded in THIS home's
...
# bin/ap-watch.sh`: that pattern matches every autopilot home's watcher
# (copilot homes run the same script) and would kill siblings.
set -u
...
CONFIRM_TIMEOUT=${AP_ARM_CONFIRM_TIMEOUT:-$ARM_CONFIRM_DEFAULT}
# Poll interval while attached to an existing healthy watcher.
ATTACH_POLL=${AP_ARM_ATTACH_POLL:-0.5}
CYCLE_LOG="$STATE/.watch-cycle-exits.log"
CYCLE_LOG_LOCK="$STATE/.watch-cycle-exits.lock"
CYCLE_LOG_MAX_BYTES=${AP_WATCH_CYCLE_LOG_MAX_BYTES:-262144}
CYCLE_LOG_KEEP_LINES=${AP_WATCH_CYCLE_LOG_KEEP_LINES:-1000}
ARM_PID=${BASHPID:-$$}
...
cycle_log_append() {
  ...
  [ "$cycle_active" -eq 1 ] || return 0
  ended_at=$(date +%s)
  beacon_age=$(ap_path_age "$BEAT")
  lock_after=$(lock_snapshot)

  i=0
  while ! ap_lock_try_acquire "$CYCLE_LOG_LOCK"; do
    [ "$i" -lt 20 ] || return 0
    sleep 0.02
    i=$((i + 1))
  done
  ...
  ap_lock_release "$CYCLE_LOG_LOCK"
  cycle_active=0
}
...
cycle_mark_predecessor_successor() {
  local successor=$1 predecessor=${AP_WATCH_PREDECESSOR_ARM_PID:-} i tmp
  case "$predecessor" in
    ''|*[!0-9]*) return 0 ;;
  esac
...
clear_stale_recorded_watcher_lock() {
  ...
  lock_home=$(cat "$WATCH_LOCK/ap-home" 2>/dev/null || true)
  lock_path=$(cat "$WATCH_LOCK/watcher-path" 2>/dev/null || true)
  lock_identity=$(cat "$WATCH_LOCK/pid-identity" 2>/dev/null || true)
  [ "$lock_home" = "$AP_HOME" ] || return 0
  [ "$lock_path" = "$WATCH" ] || return 0
  [ -n "$lock_identity" ] || return 0
  ap_lock_remove_path "$WATCH_LOCK" || true
}
...
healthy_watcher() {
  HEALTHY_PID=
  ap_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$AP_HOME" || return 1
  HEALTHY_PID=$AP_WATCHER_HEALTHY_PID
}
...
if [ "$mode" = restart ]; then
  ...
  if ap_pid_alive "$lock_pid"; then
    if ap_watcher_lock_matches_pid "$STATE" "$WATCH" "$lock_pid" "$AP_HOME"; then
      kill -TERM "$lock_pid" 2>/dev/null || true
      # Wait for it to actually exit before relaunching, so the fresh watcher
      # either takes a released lock or reclaims a now-dead-pid stale lock instead
      # of seeing the dying one as a live holder and no-opping.
      i=0
      while [ "$i" -lt 50 ] && ap_pid_alive "$lock_pid"; do
        sleep 0.1
        i=$((i + 1))
      done
    else
...
# wake exit propagates out so the harness re-notifies autopilot.
child=
child_out=
cleanup_child() {
  if [ -n "$child" ] && ap_pid_alive "$child"; then
...
while :; do
  ...
  if [ "$child_done" -eq 0 ] && ! ap_pid_alive "$child"; then
    wait "$child"
    rc=$?
    child_done=1
    owned_child_finished "$rc"
