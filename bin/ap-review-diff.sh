#!/usr/bin/env bash
# Review a flight crew member branch against the authoritative base.
#
...
# Usage: ap-review-diff.sh <task-id> [--stat]
...
usage() {
  echo "usage: ap-review-diff.sh <task-id> [--stat]" >&2
}
...
if ! git -C "$WT" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null; then
  ...
  [ -n "$BRANCH" ] || { echo "error: branch ap/$ID does not exist and worktree $WT is detached" >&2; exit 1; }
...
fetch_pull_head() {
  ...
  git -C "$WT" fetch --quiet origin \
    "+refs/pull/$n/head:refs/ap-review/pull/$n/head" >/dev/null 2>&1 || return 1
  resolved=$(git -C "$WT" rev-parse --verify "refs/ap-review/pull/$n/head^{commit}" 2>/dev/null) || return 1
  [ -n "$resolved" ] || return 1
  printf '%s' "$resolved"
}
