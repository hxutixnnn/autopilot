#!/usr/bin/env bash
# ap-busy-lib.sh - the ONE owner of autopilot's semantic busy-state contract.
#
# Design source: the pilot-approved semantic busy-state redesign
...
# by bin/ap-busy-event.sh (the only writer):
#
...
# wiring is armed (ap-spawn, or a documented recovery re-arm). Every event
...
# newer record.
#
# Semantic sources written by adapters (ap_busy_sources_for_harness owns the
...
#                    ap_busy_codex_semantic_source
#   kimi-wire, kimi-hook  reserved: standalone Kimi, gated by ap_busy_kimi_verified
# Autopilot-owned sources accepted for every converted adapter:
#   ap-spawn         the launch-brief turn seeded at spawn
#   ap-interrupt     an autopilot-controlled interruption of the worker
#   ap-recovery      a documented recovery reset after relaunch
...
#
# Classification (ap_busy_classify): busy | idle | unknown | dead, always
# with the producing source as the second token. Precedence:
#   1. dead endpoint (ap_busy_classify_live only) -> dead endpoint-gone
...
# another adapter. The delivery guards in bin/ap-tmux-lib.sh match rendered
# footers for submit acknowledgement and away-mode supervisor injection only;
# neither is a recorded worker state source.
#
# Codex negotiation (ap_busy_codex_appserver_observable,
# ap_busy_codex_hooks_verified): the approved contract prefers Codex's
...
# falling back to idle, and ap-spawn installs no Codex busy wiring.
...
# unknown kimi-unverified and ap-spawn wires no Kimi busy events. Kimi's
...
# real turn on an autopilot-launched worker including the interrupt path,
...
# and land the wiring in ap-spawn behind this same gate in the same change.
AP_BUSY_KIMI_VERIFIED_VERSIONS=""

ap_busy_kimi_verified() {
  [ -n "$AP_BUSY_KIMI_VERIFIED_VERSIONS" ]
}

# ap_busy_codex_appserver_observable: capability/version negotiation for the
...
ap_busy_codex_appserver_observable() {
  return 1
}

# ap_busy_codex_hooks_verified: the sanctioned intermediate - Codex's stable
...
# autopilot-launched worker. codex-cli 0.145.0 verdict (live, 2026-07-28):
# NOT verified. Autopilot-written project hooks under <worktree>/.codex/
...
ap_busy_codex_hooks_verified() {
  return 1
}

# ap_busy_codex_semantic_source: 0 when ANY verified Codex semantic source
# exists. ap-spawn arms and wires Codex only behind this gate, and the
# classifier reports unknown codex-unverified until it opens.
ap_busy_codex_semantic_source() {
  ap_busy_codex_appserver_observable || ap_busy_codex_hooks_verified
}

ap_busy_record_path() {  # <state-dir> <id>
  printf '%s/%s.busy-state' "$1" "$2"
}

ap_busy_gen_path() {  # <state-dir> <id>
  printf '%s/%s.busy-gen' "$1" "$2"
}

# ap_busy_token_valid: conservative token charset shared by gen, source, and
# event fields. Anything else is malformed.
ap_busy_token_valid() {  # <value>
  case "${1:-}" in
...
# ap_busy_current_gen: the task's armed gen token, or failure when the busy
# contract has never been armed for this task.
ap_busy_current_gen() {  # <state-dir> <id>
  local gen_file gen
  gen_file=$(ap_busy_gen_path "$1" "$2")
  [ -f "$gen_file" ] || return 1
  IFS= read -r gen < "$gen_file" 2>/dev/null || gen=
  ap_busy_token_valid "$gen" || return 1
  printf '%s' "$gen"
}

# ap_busy_sources_for_harness: the semantic sources trusted to classify a
# task recorded with <harness>. One line, space-separated, possibly empty.
# The autopilot-owned sources are appended for every converted adapter.
...
ap_busy_sources_for_harness() {  # <harness>
  local adapter=
  case "${1:-}" in
    claude*) adapter=claude-hook ;;
    codex*)
      ap_busy_codex_semantic_source || { printf ''; return 0; }
      adapter='codex-hook codex-appserver'
      ;;
    opencode*) adapter=opencode-plugin ;;
    pi|pi-signed) adapter=pi-ext ;;
    kimi*)
      ap_busy_kimi_verified || { printf ''; return 0; }
      adapter='kimi-wire kimi-hook'
      ;;
    *) printf ''; return 0 ;;
  esac
  printf '%s ap-spawn ap-interrupt ap-recovery' "$adapter"
}

ap_busy_source_trusted() {  # <harness> <source>
  local trusted
  trusted=$(ap_busy_sources_for_harness "$1")
  case " $trusted " in
    *" $2 "*) return 0 ;;
  esac
  return 1
...
# ap_busy_record_read: parse and validate state/<id>.busy-state against the
...
ap_busy_record_read() {  # <state-dir> <id>
  local state=$1 id=$2 rec gen line extra ver f
  local r_gen='' r_seq='' r_state='' r_source='' r_event='' r_ts=''
  rec=$(ap_busy_record_path "$state" "$id")
  if [ ! -f "$rec" ]; then
    printf 'missing'
    return 1
  fi
  if ! gen=$(ap_busy_current_gen "$state" "$id"); then
  ...
  ver=${fields[0]:-}
  [ "$ver" = "$AP_BUSY_LIB_VERSION" ] || { printf 'malformed'; return 1; }
  ...
  ap_busy_token_valid "$r_gen" || { printf 'malformed'; return 1; }
  ap_busy_token_valid "$r_source" || { printf 'malformed'; return 1; }
  ap_busy_token_valid "$r_event" || { printf 'malformed'; return 1; }
...
# ap_busy_grok_tail_busy: the Grok-only temporary rendered-tail fallback.
# Consumes the tail on stdin; 0 when Grok's verified busy signature matches.
# AP_BUSY_REGEX still globally overrides the signature, mirroring the
# historical operator escape hatch.
ap_busy_grok_tail_busy() {
  grep -v '^[[:space:]]*$' | tail -12 \
    | grep -qiE "${AP_BUSY_REGEX:-${AP_TMUX_GROK_BUSY_REGEX_DEFAULT:-Ctrl\\+c:cancel}}"
}

# ap_busy_classify: semantic classification for a task whose endpoint the
# caller has already established as present. Prints "<verdict> <source>":
# busy|idle|unknown plus the producing source (see header). Never probes
# process state. <tail40> is optional pre-captured plain output used only by
# the Grok arm; when absent the Grok arm captures through ap_backend_capture
# if available, else reports unknown capture-failed.
ap_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  out=$(ap_busy_record_read "$state" "$id") && rc=0 || rc=$?
  if [ "$rc" = 0 ]; then
    r_state=${out%% *}
    out=${out#* }
    r_source=${out%% *}
    if ap_busy_source_trusted "$harness" "$r_source"; then
  ...
  # unknown here.
  if [ "$backend" = herdr ] && command -v ap_backend_busy_state >/dev/null 2>&1; then
    native=$(ap_backend_busy_state "$backend" "$target" 2>/dev/null || true)
    if [ "$native" = busy ]; then
  ...
  case "$harness" in
    grok*)
      if [ -z "$tail40" ]; then
        if command -v ap_backend_capture >/dev/null 2>&1; then
          tail40=$(ap_backend_capture "$backend" "$target" 40 2>/dev/null) || {
      ...
      if printf '%s' "$tail40" | ap_busy_grok_tail_busy; then
...
# ap_busy_classify_live: ap_busy_classify behind the one process-level
# override - a gone endpoint is dead, never busy. Requires ap-backend.sh to
# be sourced for ap_backend_target_exists.
ap_busy_classify_live() {  # <backend> <target> <harness> <id> <state-dir> [expected-label]
  local backend=$1 target=$2 harness=$3 id=$4 state=$5 label=${6-}
  if [ -z "$target" ]; then
    printf 'unknown no-target'
    return 0
  fi
  if ! ap_backend_target_exists "$backend" "$target" "$label" 2>/dev/null; then
    printf 'dead endpoint-gone'
    return 0
  fi
  ap_busy_classify "$backend" "$target" "$harness" "$id" "$state"
}

# ap_busy_classify_meta: classify a task from its recorded metadata, so every
# consumer resolves backend, target, and harness the same way instead of
# re-deriving them. Requires ap-backend.sh to be sourced. <tail40> is
# optional pre-captured plain output reused by the Grok arm.
ap_busy_classify_meta() {  # <meta-file> <id> <state-dir> [tail40]
  ...
  ap_busy_classify "$backend" "$target" "$harness" "$id" "$state" "$tail40"
}

# ap_busy_is_busy: boolean view for callers that only gate on provable
...
ap_busy_is_busy() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  local verdict
  verdict=$(ap_busy_classify "$@")
  [ "${verdict%% *}" = busy ]
}
