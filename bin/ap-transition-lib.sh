...
#
#   2. The STATUS -> ACTION POLICY TABLE (ap_transition_policy) - the SINGLE
...
# (ap_backend_herdr_wait_transition) for the herdr producer and bin/ap-watch.sh
...
AP_TRANSITION_FIELD_SEP=$'\t'

# ap_transition_record: THE constructor for a normalized transition record.
...
ap_transition_record() {  # <pane_id> <workspace_id> <from_status> <to_status> <agent>
  local pane_id ws from to agent
  pane_id=$(ap_transition_clean_field "${1:-}")
  ws=$(ap_transition_clean_field "${2:-}")
  from=$(ap_transition_clean_field "${3:-}")
  to=$(ap_transition_clean_field "${4:-}")
  agent=$(ap_transition_clean_field "${5:-}")
  printf '%s\t%s\t%s\t%s\t%s' "$pane_id" "$ws" "$from" "$to" "$agent"
}

# ap_transition_clean_field: collapse any TAB/CR/LF in a field value to spaces
# so a stray control char can never desync the fixed five-field record.
ap_transition_clean_field() {  # <value>
  printf '%s' "${1:-}" | LC_ALL=C tr '\t\r\n' '   '
}

# Field accessors (1-based), so consumers never hardcode the column layout.
ap_transition_field() {  # <record> <n>
  printf '%s' "$1" | cut -d"$AP_TRANSITION_FIELD_SEP" -f"$2"
}

ap_transition_pane_id()      { ap_transition_field "$1" 1; }
ap_transition_workspace_id() { ap_transition_field "$1" 2; }
ap_transition_from_status()  { ap_transition_field "$1" 3; }
ap_transition_to_status()    { ap_transition_field "$1" 4; }
ap_transition_agent()        { ap_transition_field "$1" 5; }

# ap_transition_policy: THE single-owner status -> supervision-action table.
...
#                (a flight crew resumed/started a turn) is the clearing edge.
...
# immediate action.
ap_transition_policy() {  # <to_status> -> actionable|absorb|defer|fallback
  case "$1" in
