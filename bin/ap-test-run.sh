#!/usr/bin/env bash
# ap-test-run.sh - single owner of Autopilot's behavior-test runner, lane
...
#   ap-test-run.sh --all
#   ap-test-run.sh --family <name>
#   ap-test-run.sh --changed [--base <git-ref>]
#   ap-test-run.sh --lane portable-parallel-1|portable-parallel-2|portable-serial
#   ap-test-run.sh --proven-isolated
#   ap-test-run.sh tests/<name>.test.sh [more scripts...]
#
# Inspection (no execution):
#   ap-test-run.sh --list --all
#   ap-test-run.sh --list --family <name>
#   ap-test-run.sh --list --lane portable-parallel-1
#   ap-test-run.sh --list-families
#   ap-test-run.sh --list-lanes
#   ap-test-run.sh --check-coverage
#
# Aggregation (no suite execution):
#   ap-test-run.sh --aggregate-json <out.json> <lane.json> [more lane.json...]
#
# Options:
...
#                   (bin/ap-test-isolation-proof.sh --list). Cap is 8. Stateful
#                   families never schedule under --jobs.
#   -h, --help      print this header
#
# Per-script machine-parseable markers (stdout):
#   AP_TEST_BEGIN <iso8601> <script> family=<family> expected_gate_skip=<class>
#   AP_TEST_END <iso8601> <script> exit=<code> duration_ms=<n> gate_skip=<true|false>
#
# After all scripts (stdout):
#   AP_TEST_SUMMARY total=<n> failed=<n> skipped_gate=<n> duration_ms=<n>
#   AP_TEST_SUMMARY_FAMILY family=<name> count=<n> duration_ms=<n> failed=<n>
#   AP_TEST_SLOWEST rank=<k> script=<path> duration_ms=<n>
#
...
# owned by bin/ap-test-isolation-proof.sh; portable parallel shards are a
# duration-balanced partition of that exact set (see docs/ap-test-portable-shards.md).
...
family_for_basename() {
  case "$1" in
    ap-arm-pretool-check.test.sh|ap-ask-user-authority.test.sh|\
    ap-brief.test.sh|ap-vendor-auth-probe.test.sh|\
    ap-calm-pi-extension.test.sh|ap-cd-pretool-check.test.sh|\
    ap-composer-ghost.test.sh|ap-composer-lib.test.sh|\
    ap-flight-crew-state.test.sh|ap-decision-hold-lifecycle.test.sh|\
    ap-documentation-audiences.test.sh|ap-ensure-agents-md.test.sh|ap-grok-harness.test.sh|\
    ap-kimi-harness.test.sh|ap-herdr-lab.test.sh|ap-lint.test.sh|\
    ap-operational-input.test.sh|ap-pi-primary-types.test.sh|\
    ap-send-popup-settle.test.sh|ap-send-settle.test.sh|\
    ap-subagent-pretool-check.test.sh|\
    ap-supervision-instructions.test.sh|ap-tmux-submit-busy.test.sh|ap-transition-lib.test.sh|\
    ap-test-run.test.sh|ap-test-isolation-proof.test.sh)
      printf '%s\n' pure-contract-unit
      ;;
    ap-daemon.test.sh|ap-guard-stale-banner.test.sh|ap-pi-watch-extension.test.sh|\
    ap-supervision-events.test.sh|ap-turnend-guard.test.sh|ap-wake-daemon-lifecycle-e2e.test.sh|\
    ap-wake-queue.test.sh|ap-watch-checkpoint.test.sh|ap-watch-triage.test.sh|\
    ap-watcher-lock.test.sh)
      printf '%s\n' watcher-wake-lock
      ;;
    ap-afk-inject-herdr-e2e.test.sh|ap-afk-launch.test.sh|ap-backend-autodetect-smoke.test.sh|\
    ap-backend-herdr-eventwait-smoke.test.sh|ap-backend-herdr-presentation-e2e.test.sh|\
    ap-backend-herdr-launcher-workspace-e2e.test.sh|\
    ap-backend-herdr-prune-safety-e2e.test.sh|ap-backend-herdr-respawn-idem-e2e.test.sh|\
    ap-herdr-session-cleanup-e2e.test.sh|\
    ap-backend-herdr-smoke.test.sh|ap-backend-herdr-workspace-per-home-e2e.test.sh)
      printf '%s\n' real-herdr-gated
      ;;
    ap-backlog-handoff.test.sh|ap-copilot-harness.test.sh|ap-copilot-lifecycle-e2e.test.sh|\
    ap-copilot-liveness.test.sh|ap-copilot-safety.test.sh|ap-copilot-sync.test.sh|\
    ap-startup-memory-budget.test.sh|\
    ap-send-copilot-marker.test.sh|ap-shared-pilot-inheritance.test.sh)
      printf '%s\n' copilot
      ;;
    ap-bootstrap.test.sh|ap-fleet-sync.test.sh|ap-gate-refuse.test.sh|ap-gotmp.test.sh|\
    ap-session-start.test.sh|ap-sessionstart-nudge.test.sh|ap-tangle-guard.test.sh|\
    ap-update.test.sh|autopilot-command.test.sh)
      printf '%s\n' session-bootstrap
      ;;
    ap-afk-pi-herdr-return-e2e.test.sh|\
    ap-codex-continuity-live-e2e.test.sh|ap-grok-continuity-live-e2e.test.sh|\
    ap-grok-stop-live-e2e.test.sh|ap-opencode-primary-live-e2e.test.sh|ap-pi-primary-live-e2e.test.sh|\
    ap-send-copilot-marker-herdr-e2e.test.sh)
      printf '%s\n' live-harness-optin
      ;;
    ap-backend-herdr.test.sh|ap-backend-tmux-smoke.test.sh|ap-backend.test.sh|\
    ap-herdr-session-cleanup.test.sh|ap-send-strict.test.sh|ap-spawn-batch.test.sh|\
    ap-spawn-dispatch-profile.test.sh|ap-spawn-worktree-settle.test.sh|\
    ap-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    ap-pr-check-security.test.sh|ap-pr-merge.test.sh|ap-review-diff.test.sh|\
    ap-teardown.test.sh)
      printf '%s\n' pr-forge
      ;;
    ap-afk-inject-e2e.test.sh|ap-afk-return.test.sh)
      printf '%s\n' afk
      ;;
    ap-bearings-snapshot.test.sh|ap-fleet-snapshot-view.test.sh)
      printf '%s\n' snapshot-bearings
      ;;
    ap-backend-cmux.test.sh|ap-backend-cmux-smoke.test.sh)
      printf '%s\n' cmux
      ;;
    ap-backend-zellij.test.sh|ap-backend-zellij-smoke.test.sh)
      printf '%s\n' zellij
      ;;
    ap-backend-orca.test.sh)
      printf '%s\n' orca
      ;;
    *)
      printf '%s\n' unclassified
...
# bin/ap-test-isolation-proof.sh --list). Do not expand without a new concurrent
...
# current concurrent-proof durations in docs/ap-test-isolation-proof.json.
...
run_coverage_guard() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-coverage.XXXXXX")
  ...
  if [ -x "$ROOT/bin/ap-test-isolation-proof.sh" ]; then
    "$ROOT/bin/ap-test-isolation-proof.sh" --list | LC_ALL=C sort -u >"$tmp/proof_list"
    if ! cmp -s "$tmp/proven" "$tmp/proof_list"; then
      log "coverage guard: embedded proven-isolated set diverges from bin/ap-test-isolation-proof.sh --list"
  ...
  printf 'AP_TEST_COVERAGE ok total=%s parallel=%s serial=%s herdr=%s\n' \
    "$(wc -l <"$tmp/all" | tr -d ' ')" \
...
print(f"AP_TEST_AGGREGATE lanes={len(lanes)} total={total} failed={failed} skipped_gate={skipped} critical_path_duration_ms={wall_ms}")
PY
}
...
families_for_changed_path() {
  ...
  case "$path" in
    tests/ap-test-run.test.sh)
      printf '%s\n' pure-contract-unit
      ;;
    tests/ap-backend-herdr-eventwait.test.py)
      printf '%s\n' real-herdr-gated
      printf '%s\n' backend-dispatch
      ;;
    tests/*.test.sh)
      ...
      ;;
    bin/ap-test-run.sh|bin/ap-test-isolation-proof.sh)
      printf '%s\n' pure-contract-unit
      ;;
    bin/backends/herdr*|bin/ap-herdr-lab.sh|tests/herdr-test-safety.sh)
      printf '%s\n' real-herdr-gated
      printf '%s\n' backend-dispatch
      printf '%s\n' pure-contract-unit
      ;;
    bin/ap-herdr-session-cleanup.sh)
      printf '%s\n' session-bootstrap
      printf '%s\n' real-herdr-gated
      printf '%s\n' backend-dispatch
      ;;
    ...
    bin/backends/orca*|bin/backends/tmux.sh)
      ...
      printf '%s\n' orca
      ;;
    bin/ap-backend.sh|bin/ap-backend-hometag-lib.sh)
      printf '%s\n' backend-dispatch
      printf '%s\n' real-herdr-gated
      ;;
    bin/ap-watch*|bin/ap-wake*|\
    bin/ap-classify-lib.sh|bin/ap-daemon*|bin/ap-turnend-guard*|bin/ap-guard.sh)
      printf '%s\n' watcher-wake-lock
      ;;
    bin/ap-afk*)
      printf '%s\n' afk
      printf '%s\n' real-herdr-gated
      ;;
    bin/ap-supervisor-target-lib.sh)
      printf '%s\n' watcher-wake-lock
      printf '%s\n' real-herdr-gated
      printf '%s\n' live-harness-optin
      printf '%s\n' afk
      ;;
    bin/ap-startup-memory-budget.sh|bin/ap-startup-memory-budget-lib.sh)
      printf '%s\n' copilot
      printf '%s\n' session-bootstrap
      ;;
    bin/ap-copilot*|bin/ap-home-seed.sh|bin/ap-backlog-handoff.sh|\
    bin/ap-config-inherit-lib.sh|bin/ap-config-push.sh|bin/ap-shared*)
      printf '%s\n' copilot
      ;;
    bin/ap-session-start.sh|bin/ap-bootstrap.sh|bin/ap-fleet-sync.sh|\
    bin/ap-sessionstart-nudge.sh|bin/ap-tangle*|bin/ap-update.sh|\
    bin/ap-gate-refuse*|bin/ap-lock*|bin/ap-quota-axi-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
    bin/ap-pr-*|bin/ap-merge-local.sh|bin/ap-teardown.sh|bin/ap-review-diff.sh|\
    bin/ap-check*)
      printf '%s\n' pr-forge
      ;;
    bin/ap-spawn.sh|bin/ap-send.sh|bin/ap-harness.sh|\
    bin/ap-peek.sh|bin/ap-composer*)
      printf '%s\n' backend-dispatch
      printf '%s\n' pure-contract-unit
      ;;
    bin/ap-bearings-snapshot.sh|bin/ap-fleet-snapshot.sh|bin/ap-fleet-view.sh)
      printf '%s\n' snapshot-bearings
      ;;
    bin/ap-install-herdr.sh|bin/ap-install-treehouse.sh|bin/ap-herdr-ci-cleanup.sh)
      printf '%s\n' pure-contract-unit
      # Pin or cleanup changes also select the real-Herdr family so the required
      # lane's contract coverage re-runs.
      printf '%s\n' real-herdr-gated
      ;;
    bin/ap-lint.sh|bin/ap-install-shellcheck.sh|\
    bin/ap-brief.sh|bin/ap-ensure-agents-md.sh|bin/ap-flight-crew-state.sh|\
    bin/ap-decision-hold.sh|bin/ap-supervision*|bin/ap-transition-lib.sh|\
    bin/ap-tmux-lib.sh|bin/ap-marker-lib.sh|bin/ap-operational-input.sh|bin/ap-tasks-axi-lib.sh|\
    bin/ap-vendor-auth-probe.sh|\
    bin/ap-primary-scope-lib.sh|bin/ap-project-mode.sh|bin/ap-promote.sh|\
    ...
    .github/workflows/ci.yml|.no-mistakes.yaml)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    docs/ap-test-portable-shards.md|docs/ap-test-isolation-proof.md|\
    docs/ap-test-isolation-proof.json)
      printf '%s\n' pure-contract-unit
      ;;
...
if [ "${#SCRIPTS[@]}" -eq 0 ]; then
  log "nothing to run"
  printf 'AP_TEST_SUMMARY total=0 failed=0 skipped_gate=0 duration_ms=0\n'
  if [ -n "$JSON_PATH" ]; then
    empty_rec=$(mktemp)
    empty_fam=$(mktemp)
    : >"$empty_rec"
...
if [ "$JOBS" -gt 1 ]; then
  for s in "${SCRIPTS[@]}"; do
    if ! is_proven_isolated_script "$s"; then
      die "--jobs $JOBS refused: $s is not in the proven-isolated set (see bin/ap-test-isolation-proof.sh --list). Stateful families stay serial."
    fi
  done
fi

RUN_TMP=$(mktemp -d "${TMPDIR:-/tmp}/ap-test-run.XXXXXX")
...
RUN_ID="ap-test-run-${RUN_STARTED_MS}-$$"
TOTAL=0
FAILED=0
SKIPPED_GATE=0
AGG_RC=0
...
record_script_result() {
  ...
  printf 'AP_TEST_END %s %s exit=%s duration_ms=%s gate_skip=%s\n' \
...
run_one_serial() {
  ...
  printf 'AP_TEST_BEGIN %s %s family=%s expected_gate_skip=%s\n' \
...
if [ "$JOBS" -eq 1 ]; then
  ...
  for script in "${SCRIPTS[@]}"; do
    ...
    (
      ...
      export TMP="$work/tmp"
      unset AP_HOME AP_STATE_OVERRIDE AP_DATA_OVERRIDE AP_ROOT_OVERRIDE \
        AP_PROJECTS_OVERRIDE AP_CONFIG_OVERRIDE AP_BACKEND 2>/dev/null || true
      cd "$ROOT" || exit 1
      begin_ms=$(now_ms)
...
printf 'AP_TEST_SUMMARY total=%s failed=%s skipped_gate=%s duration_ms=%s\n' \
...
if [ -s "$FAMILIES_TSV" ]; then
  ...
  sort -t$'\t' -k1,1 "$FAMILIES_TSV" | while IFS=$'\t' read -r name count duration failed_count; do
    printf 'AP_TEST_SUMMARY_FAMILY family=%s count=%s duration_ms=%s failed=%s\n' \
...
if [ -s "$RECORDS" ]; then
  ...
  sort -t$'\t' -k5,5nr "$RECORDS" | head -n 15 | while IFS=$'\t' read -r path _family _expected _rc duration _gate; do
    printf 'AP_TEST_SLOWEST rank=%s script=%s duration_ms=%s\n' \
      "$rank" "$path" "$duration"
    rank=$((rank + 1))
  done
fi
