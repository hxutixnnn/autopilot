...
# in a genuine autopilot primary home.
# This file is sourced by hook entrypoints and has no side effects on source.

# Return 0 when $1 carries a genuine copilot-home marker.
ap_root_is_copilot_home() {
  local marker="$1/.ap-copilot-home" id LC_ALL=C
  [ -L "$marker" ] && return 1
  [ -f "$marker" ] || return 1
...
# A valid copilot marker force-includes a linked copilot home.
# Otherwise only a plain checkout is primary, never a linked task worktree.
ap_primary_scope_matches() {
  local root=$1 state=$2 git_dir git_common_dir
  if ! ap_root_is_copilot_home "$root"; then
