#!/usr/bin/env bash
# Grok Stop-hook adapter for the autopilot PRIMARY turn-end guard.
#
...
[ -n "$ROOT" ] || exit 0
ROOT=${ROOT%/}
[ -x "$ROOT/bin/ap-turnend-guard.sh" ] || exit 0

if [ "$CAPABILITY" = native ]; then
  printf '%s' "$PAYLOAD" | "$ROOT/bin/ap-turnend-guard.sh"
  RC=$?
  case "$RC" in
    0|2) exit "$RC" ;;
    *) exit 0 ;;
...
ERR=$(mktemp "${TMPDIR:-/tmp}/ap-turnend-grok.XXXXXX") || exit 0
trap 'rm -f "$ERR"' EXIT

printf '%s' "$PAYLOAD" | "$ROOT/bin/ap-turnend-guard.sh" 2>"$ERR"
RC=$?
[ "$RC" -eq 2 ] || exit 0
...
ap_operational_input_encode turn-end-guard \
